...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasSfadNewBlEdit
    /// </summary>
    public static SubAktivitasSfadNewBlEdit subAktivitasSfadNewBlEdit
    {
        get => HttpData.Get<SubAktivitasSfadNewBlEdit>("subAktivitasSfadNewBlEdit")!;
        set => HttpData["subAktivitasSfadNewBlEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasSFADNewBL
    /// </summary>
    public class SubAktivitasSfadNewBlEdit : SubAktivitasSfadNewBlEditBase
    {
        // Constructor
        public SubAktivitasSfadNewBlEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasSfadNewBlEdit() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasSfadNewBlEditBase : SubAktivitasSfadNewBl
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasSfadNewBlEdit";
        ...
        // Constructor
        public SubAktivitasSfadNewBlEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasSfadNewBlEdit";
        ...
        // Constructor
        public SubAktivitasSfadNewBlEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasSfadNewBlList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasSfadNewBlList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasSfadNewBlList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasSfadNewBlEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'ApakahTerdapatROB'
            ApakahTerdapatROB.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(ApakahTerdapatROB, "ApakahTerdapatROB", "x_ApakahTerdapatROB");
            ...
            // Standard handling for 'AR45B_barrels'
            AR45B_barrels.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AR45B_barrels, "AR45B_barrels", "x_AR45B_barrels");
            ...
            if (RowType == RowType.View) {
                ...
                if (Tujuan.Lookup != null && IsDictionary(Tujuan.Lookup?.Options) && Tujuan.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    ...
                    if (curVal7 == "") {
                        ...
                        filterWrk7 = SearchFilter(Tujuan.Lookup?.GetTable()?.Fields["IdPlant"].SearchExpression, "=", Tujuan.CurrentValue, Tujuan.Lookup?.GetTable()?.Fields["IdPlant"].SearchDataType, "");
                    }
                    string? sqlWrk7 = Tujuan.Lookup?.GetSql(true, filterWrk7, null, this, false, true);
        ...
        private void ValidateCustomApakahTerdapatROB() {
            if (ApakahTerdapatROB.Visible && ApakahTerdapatROB.Required) {
                if (Empty(ApakahTerdapatROB.FormValue)) {
                    ApakahTerdapatROB.AddErrorMessage(ConvertToString(AR45B_mt.RequiredErrorMessage).Replace("%s", AR45B_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomAR45B_barrels() {
            if (AR45B_barrels.Visible && AR45B_barrels.Required) {
                if (!AR45B_barrels.IsDetailKey && Empty(AR45B_barrels.FormValue)) {
                    AR45B_barrels.AddErrorMessage(ConvertToString(AR45B_mt.RequiredErrorMessage).Replace("%s", AR45B_mt.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomApakahTerdapatROB();
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasSfadNewBlList")), "", TableVar, true);
            string pageId = "edit";
