...
public partial class SnDOne {
    ...
    public class MasterUserAddBase : MasterUser
    {
        ...
        // Constructor
        public MasterUserAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("MasterUserList"); // No matching record, return to List page // DN
                    }
                    break;
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'IdIdaman' before field var 'x_IdIdaman'
            val = CurrentForm.HasValue("IdIdaman") ? CurrentForm.GetValue("IdIdaman") : CurrentForm.GetValue("x_IdIdaman");
            if (!IdIdaman.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("IdIdaman") && !CurrentForm.HasValue("x_IdIdaman")) // DN
                    IdIdaman.Visible = false; // Disable update for API request
                else
                    IdIdaman.SetFormValue(val);
            }
