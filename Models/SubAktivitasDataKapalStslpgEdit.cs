...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasDataKapalStslpgEdit
    /// </summary>
    public static SubAktivitasDataKapalStslpgEdit subAktivitasDataKapalStslpgEdit
    {
        get => HttpData.Get<SubAktivitasDataKapalStslpgEdit>("subAktivitasDataKapalStslpgEdit")!;
        set => HttpData["subAktivitasDataKapalStslpgEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasDataKapalSTSLPG
    /// </summary>
    public class SubAktivitasDataKapalStslpgEdit : SubAktivitasDataKapalStslpgEditBase
    {
        // Constructor
        public SubAktivitasDataKapalStslpgEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasDataKapalStslpgEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasDataKapalStslpgEditBase : SubAktivitasDataKapalStslpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasDataKapalStslpgEdit";
        ...
        // Constructor
        public SubAktivitasDataKapalStslpgEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasDataKapalStslpgEdit";
        ...
        // Constructor
        public SubAktivitasDataKapalStslpgEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasDataKapalStslpgList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasDataKapalStslpgList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasDataKapalStslpgList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasDataKapalStslpgEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedBy() {
            if (lastUpdatedBy.Visible && lastUpdatedBy.Required) {
                if (!lastUpdatedBy.IsDetailKey && Empty(lastUpdatedBy.FormValue)) {
                    lastUpdatedBy.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        private void ValidateCustomnamakapal() {
            if (namakapal.Visible && namakapal.Required) {
                if (!namakapal.IsDetailKey && Empty(namakapal.FormValue)) {
                    namakapal.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        private void ValidateCustomproduk() {
            if (produk.Visible && produk.Required) {
                if (!produk.IsDetailKey && Empty(produk.FormValue)) {
                    produk.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        private void ValidateCustomnominasi() {
            if (nominasi.Visible && nominasi.Required) {
                if (!nominasi.IsDetailKey && Empty(nominasi.FormValue)) {
                    nominasi.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        private void ValidateCustomtgltiba() {
            if (tgltiba.Visible && tgltiba.Required) {
                if (!tgltiba.IsDetailKey && Empty(tgltiba.FormValue)) {
                    tgltiba.AddErrorMessage(ConvertToString(tgltiba.RequiredErrorMessage).Replace("%s", tgltiba.Caption));
                }
            }
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasDataKapalStslpgList")), "", TableVar, true);
            string pageId = "edit";
