...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasSfadNewBlList
    /// </summary>
    public static SubAktivitasSfadNewBlList subAktivitasSfadNewBlList
    {
        get => HttpData.Get<SubAktivitasSfadNewBlList>("subAktivitasSfadNewBlList")!;
        set => HttpData["subAktivitasSfadNewBlList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasSFADNewBL
    /// </summary>
    public class SubAktivitasSfadNewBlList : SubAktivitasSfadNewBlListBase
    {
        // Constructor
        public SubAktivitasSfadNewBlList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasSfadNewBlList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasSfadNewBlListBase : SubAktivitasSfadNewBl
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasSfadNewBlList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasSFADNewBLlist";
        ...
        // Constructor
        public SubAktivitasSfadNewBlListBase()
        {
            TableName = "SubAktivitasSFADNewBL";
            ...
            // Table object (subAktivitasSfadNewBl)
            if (subAktivitasSfadNewBl == null || subAktivitasSfadNewBl is SubAktivitasSfadNewBl)
                subAktivitasSfadNewBl = this;

            // Initialize URLs
            AddUrl = "SubAktivitasSfadNewBlAdd";
            ...
            MultiDeleteUrl = "SubAktivitasSfadNewBlDelete";
            MultiUpdateUrl = "SubAktivitasSfadNewBlUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasSfadNewBlList";
        ...
        // Constructor
        public SubAktivitasSfadNewBlListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasSfadNewBlView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasSFADNewBLgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasSfadNewBlList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(ApakahTerdapatROB, ctrl); // ApakahTerdapatROB
                ...
                UpdateSort(AR45B_barrels, ctrl); // AR45B_barrels
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasSFADNewBL"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("ApakahTerdapatROB")); // DN
                ...
                CreateColumnOption(option.Add("AR45B_barrels")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasSFADNewBLsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasSFADNewBLsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasSFADNewBL");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasSfadNewBlList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasSfadNewBlList.RowCount) + "_SubAktivitasSFADNewBL");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasSfadNewBlList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasSfadNewBlList.RowType == RowType.Add || IsEdit && subAktivitasSfadNewBlList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasSFADNewBLsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
