...
public partial class SnDOne {
    ...
    public class SubAktivitasDataGerbongKetelRtwAddBase : SubAktivitasDataGerbongKetelRtw
    {
        ...
        // Constructor
        public SubAktivitasDataGerbongKetelRtwAddBase()
        {
        ...
        // Constructor
        public SubAktivitasDataGerbongKetelRtwAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("SubAktivitasDataGerbongKetelRtwList"); // No matching record, return to List page // DN
                    }
                    break;
                    ...
                    if (res) { // Add successful
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasDataGerbongKetelRtwList") {
                                ...
                                returnUrl = "SubAktivitasDataGerbongKetelRtwList"; // Return list page content
                            }
                        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'TanggalTiba' before field var 'x_TanggalTiba'
            val = CurrentForm.HasValue("TanggalTiba") ? CurrentForm.GetValue("TanggalTiba") : CurrentForm.GetValue("x_TanggalTiba");
            if (!TanggalTiba.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TanggalTiba") && !CurrentForm.HasValue("x_TanggalTiba")) // DN
                    TanggalTiba.Visible = false; // Disable update for API request
                else
                    TanggalTiba.SetFormValue(val, true, validate);
                TanggalTiba.CurrentValue = UnformatDateTime(TanggalTiba.CurrentValue, TanggalTiba.FormatPattern);
            }

            // Check field name 'Nominasi' before field var 'x_Nominasi'
            val = CurrentForm.HasValue("Nominasi") ? CurrentForm.GetValue("Nominasi") : CurrentForm.GetValue("x_Nominasi");
            if (!Nominasi.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Nominasi") && !CurrentForm.HasValue("x_Nominasi")) // DN
                    Nominasi.Visible = false; // Disable update for API request
                else
                    Nominasi.SetFormValue(val);
            }
            ...
            // Check field name 'DetailGerbong' before field var 'x_DetailGerbong'
            val = CurrentForm.HasValue("DetailGerbong") ? CurrentForm.GetValue("DetailGerbong") : CurrentForm.GetValue("x_DetailGerbong");
            if (!DetailGerbong.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("DetailGerbong") && !CurrentForm.HasValue("x_DetailGerbong")) // DN
                    DetailGerbong.Visible = false; // Disable update for API request
                else
                    DetailGerbong.SetFormValue(val);
            }

            // Check field name 'DetailQuantity' before field var 'x_DetailQuantity'
            val = CurrentForm.HasValue("DetailQuantity") ? CurrentForm.GetValue("DetailQuantity") : CurrentForm.GetValue("x_DetailQuantity");
            if (!DetailQuantity.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("DetailQuantity") && !CurrentForm.HasValue("x_DetailQuantity")) // DN
                    DetailQuantity.Visible = false; // Disable update for API request
                else
                    DetailQuantity.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (TanggalTiba.Visible && TanggalTiba.Required) {
                    if (!TanggalTiba.IsDetailKey && Empty(TanggalTiba.FormValue)) {
                        TanggalTiba.AddErrorMessage(ConvertToString(TanggalTiba.RequiredErrorMessage).Replace("%s", TanggalTiba.Caption));
                    }
                }
                ...
                if (Nominasi.Visible && Nominasi.Required) {
                    if (!Nominasi.IsDetailKey && Empty(Nominasi.FormValue)) {
                        Nominasi.AddErrorMessage(ConvertToString(Nominasi.RequiredErrorMessage).Replace("%s", Nominasi.Caption));
                    }
                }
                ...
                if (DetailGerbong.Visible && DetailGerbong.Required) {
                    if (!DetailGerbong.IsDetailKey && Empty(DetailGerbong.FormValue)) {
                        DetailGerbong.AddErrorMessage(ConvertToString(DetailGerbong.RequiredErrorMessage).Replace("%s", DetailGerbong.Caption));
                    }
                }
