...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasFormInputNilaiBLnAlEdit
    /// </summary>
    public static SubAktivitasFormInputNilaiBLnAlEdit subAktivitasFormInputNilaiBLnAlEdit
    {
        get => HttpData.Get<SubAktivitasFormInputNilaiBLnAlEdit>("subAktivitasFormInputNilaiBLnAlEdit")!;
        set => HttpData["subAktivitasFormInputNilaiBLnAlEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasFormInputNilaiBLnAL
    /// </summary>
    public class SubAktivitasFormInputNilaiBLnAlEdit : SubAktivitasFormInputNilaiBLnAlEditBase
    {
        // Constructor
        public SubAktivitasFormInputNilaiBLnAlEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasFormInputNilaiBLnAlEdit() : base()
        {
        }
        ...
        public override void PageRender() {
            ...
            if (CurrentPageID() != "edit")
                return;
            string noRef = Convert.ToString(NoReferensi.CurrentValue);
            string sql = @"
            SELECT
                COALESCE(SUM(TRY_CONVERT (DECIMAL (18, 2), s.Volume)), 0) AS Volume
            FROM
                Penerimaan p
                LEFT JOIN Penyaluran pe ON p.IdPenyaluran = pe.IdPenyaluran
                LEFT JOIN SubAktivitasLogbookPenerimaanPenyaluran s ON pe.NomorPenyaluran = s.NoReferensi
            WHERE p.NomorPenerimaan = @ref";
            var param = new Dictionary<string, object> { { "@ref", noRef } };
            decimal data = DatabaseQueryHelper.ExecuteSelectSingle<decimal>(query: sql, parameters: param, mapFunc: r =>
            {
                int i = r.GetOrdinal("Volume");
                return r.IsDBNull(i) ? 0m : Convert.ToDecimal(r.GetValue(i));
            });
            BL_KLObs.EditValue = data;
            AL_KLObs.EditValue = data;
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasFormInputNilaiBLnAlEditBase : SubAktivitasFormInputNilaiBLnAl
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasFormInputNilaiBLnAlEdit";
        ...
        // Constructor
        public SubAktivitasFormInputNilaiBLnAlEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasFormInputNilaiBLnAlEdit";
        ...
        // Constructor
        public SubAktivitasFormInputNilaiBLnAlEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasFormInputNilaiBLnAlList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "SubAktivitasFormInputNilaiBLnAlList") {
                        ...
                        returnUrl = "SubAktivitasFormInputNilaiBLnAlList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasFormInputNilaiBLnAlEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'AL_KLObs'
            AL_KLObs.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AL_KLObs, "AL_KLObs", "x_AL_KLObs");

            // Standard handling for 'AL_KL15'
            AL_KL15.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AL_KL15, "AL_KL15", "x_AL_KL15");
        ...
        private void ValidateCustomAL_KLObs() {
            if (AL_KLObs.Visible && AL_KLObs.Required) {
                if (!AL_KLObs.IsDetailKey && Empty(AL_KLObs.FormValue)) {
                    AL_KLObs.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }

        private void ValidateCustomAL_KL15() {
            if (AL_KL15.Visible && AL_KL15.Required) {
                if (!AL_KL15.IsDetailKey && Empty(AL_KL15.FormValue)) {
                    AL_KL15.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        private void ValidateCustomAL_Barrels() {
            if (AL_Barrels.Visible && AL_Barrels.Required) {
                if (!AL_Barrels.IsDetailKey && Empty(AL_Barrels.FormValue)) {
                    AL_Barrels.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        private void ValidateCustomAL_LT() {
            if (AL_LT.Visible && AL_LT.Required) {
                if (!AL_LT.IsDetailKey && Empty(AL_LT.FormValue)) {
                    AL_LT.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        private void ValidateCustomAL_MT() {
            if (AL_MT.Visible && AL_MT.Required) {
                if (!AL_MT.IsDetailKey && Empty(AL_MT.FormValue)) {
                    AL_MT.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomBL_LT();
                ValidateCustomBL_MT();
                ValidateCustomAL_KLObs();
                ValidateCustomAL_KL15();
