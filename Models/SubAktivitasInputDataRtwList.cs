...
public partial class SnDOne {
    ...
    public class SubAktivitasInputDataRtwListBase : SubAktivitasInputDataRtw
    {
        ...
        // Constructor
        public SubAktivitasInputDataRtwListBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasInputDataRtwListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            if (filter.TryGetValue("x_NomorGerbongKertel", out sv)) {
                RestoreFieldFilter(NomorGerbongKertel, filter, "NomorGerbongKertel", sv);
            }

            // Field MeterAwal
            if (filter.TryGetValue("x_MeterAwal", out sv)) {
                RestoreFieldFilter(MeterAwal, filter, "MeterAwal", sv);
            }

            // Field MeterAkhir
            if (filter.TryGetValue("x_MeterAkhir", out sv)) {
                RestoreFieldFilter(MeterAkhir, filter, "MeterAkhir", sv);
            }

            // Field Total
            if (filter.TryGetValue("x_Total", out sv)) {
                RestoreFieldFilter(Total, filter, "Total", sv);
            }
            ...
            // Field NoMeter
            if (filter.TryGetValue("x_NoMeter", out sv)) {
                RestoreFieldFilter(NoMeter, filter, "NoMeter", sv);
            }

            // Field Quantity
            if (filter.TryGetValue("x_Quantity", out sv)) {
                RestoreFieldFilter(Quantity, filter, "Quantity", sv);
            }

            // Field HasilPengukuranT2
            if (filter.TryGetValue("x_HasilPengukuranT2", out sv)) {
                RestoreFieldFilter(HasilPengukuranT2, filter, "HasilPengukuranT2", sv);
            }

            // Field PICPengisian
            if (filter.TryGetValue("x_PICPengisian", out sv)) {
                RestoreFieldFilter(PICPengisian, filter, "PICPengisian", sv);
            }

            // Field NomorGerbongKertel2
            if (filter.TryGetValue("x_NomorGerbongKertel2", out sv)) {
                RestoreFieldFilter(NomorGerbongKertel2, filter, "NomorGerbongKertel2", sv);
            }

            // Field NomorGerbongKertel3
            if (filter.TryGetValue("x_NomorGerbongKertel3", out sv)) {
                RestoreFieldFilter(NomorGerbongKertel3, filter, "NomorGerbongKertel3", sv);
            }

            // Field Quantity2
            if (filter.TryGetValue("x_Quantity2", out sv)) {
                RestoreFieldFilter(Quantity2, filter, "Quantity2", sv);
            }

            // Field Quantity3
            if (filter.TryGetValue("x_Quantity3", out sv)) {
                RestoreFieldFilter(Quantity3, filter, "Quantity3", sv);
            }

            // Field HasilPengukuranT2_2
            if (filter.TryGetValue("x_HasilPengukuranT2_2", out sv)) {
                RestoreFieldFilter(HasilPengukuranT2_2, filter, "HasilPengukuranT2_2", sv);
            }

            // Field HasilPengukuranT2_3
            if (filter.TryGetValue("x_HasilPengukuranT2_3", out sv)) {
                RestoreFieldFilter(HasilPengukuranT2_3, filter, "HasilPengukuranT2_3", sv);
            }

            // Field TotalGK
            if (filter.TryGetValue("x_TotalGK", out sv)) {
                RestoreFieldFilter(TotalGK, filter, "TotalGK", sv);
            }

            // Field Selisih
            if (filter.TryGetValue("x_Selisih", out sv)) {
                RestoreFieldFilter(Selisih, filter, "Selisih", sv);
            }
        }
