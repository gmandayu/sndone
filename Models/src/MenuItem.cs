...
public partial class SnDOne {
    ...
    public class MenuItem
    {
        ...
        // Constructor
        public MenuItem(Menu menu, int id, string name, string text, string url, int parentId = -1, bool allowed = true, bool isHeader = false, bool isCustomUrl = false, string icon = "", string label = "", bool isNavbarItem = false, bool isSidebarItem = false)
        {
            Menu = menu;
            Id = id;
            Name = name;
        ...
        // Constructor
        public MenuItem(int id, string name, string url, int parentId = -1, object? allowed = null, bool isHeader = false, bool isCustomUrl = false, string icon = "", string label = "", bool isNavbarItem = false, bool isSidebarItem = false)
        {
            Id = id;
            Name = name;
            Text = Language.MenuPhrase(id.ToString(), "MenuText");
            Url = !Empty(url) ? AppPath(url) : "";
            ParentId = parentId;
            Allowed = allowed is string s
                ? (s.EndsWith("?") ? (IsLoggedIn() || AllowList(s.TrimEnd('?'))) : AllowList(s))
                : (allowed is bool b ? (b ? true : IsLoggedIn()) : false);
            IsHeader = isHeader;
            IsCustomUrl = isCustomUrl;
            Icon = icon;
            Label = label;
        ...
        // Set attribute
        public MenuItem SetAttribute(string name, string value)
        {
        ...
        {
            if (Active || Submenu != null && Url != "#" && (Menu?.IsNavbar ?? false) && (Menu?.IsRoot ?? false)) { // Active or navbar root menu item with submenu // DN
            ...
            string[] faClassNames = ["fa-solid", "fa-regular", "fa-light", "fa-thin", "fa-duotone", "fa-sharp", "fa-brands"];
