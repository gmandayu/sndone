...
public partial class SnDOne {
    ...
    public class SubAktivitasFormInputBLsesuaiCqlrtwEditBase : SubAktivitasFormInputBLsesuaiCqlrtw
    {
        ...
        // Constructor
        public SubAktivitasFormInputBLsesuaiCqlrtwEditBase()
        {
        ...
        // Constructor
        public SubAktivitasFormInputBLsesuaiCqlrtwEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasFormInputBLsesuaiCqlrtwList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasFormInputBLsesuaiCqlrtwList") {
                                ...
                                returnUrl = "SubAktivitasFormInputBLsesuaiCqlrtwList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'ML' before field var 'x_ML'
            val = CurrentForm.HasValue("ML") ? CurrentForm.GetValue("ML") : CurrentForm.GetValue("x_ML");
            if (!ML.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("ML") && !CurrentForm.HasValue("x_ML")) // DN
                    ML.Visible = false; // Disable update for API request
                else
                    ML.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (ML.Visible && ML.Required) {
                    if (!ML.IsDetailKey && Empty(ML.FormValue)) {
                        ML.AddErrorMessage(ConvertToString(ML.RequiredErrorMessage).Replace("%s", ML.Caption));
                    }
                }
