...
public partial class SnDOne {
    ...
    public class MasterFungsiKunjunganEditBase : MasterFungsiKunjungan
    {
        ...
        // Constructor
        public MasterFungsiKunjunganEditBase(Controller? controller)
        {
        ...
        // Constructor
        public MasterFungsiKunjunganEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("MasterFungsiKunjunganList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "MasterFungsiKunjunganList") {
                        ...
                        returnUrl = "MasterFungsiKunjunganList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'FungsiKunjungan'
            FungsiKunjungan.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(FungsiKunjungan, "FungsiKunjungan", "x_FungsiKunjungan");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomID() {
            if (ID.Visible && ID.Required) {
                if (!ID.IsDetailKey && Empty(ID.FormValue)) {
                    ID.AddErrorMessage(ConvertToString(FungsiKunjungan.RequiredErrorMessage).Replace("%s", FungsiKunjungan.Caption));
                }
            }
        }

        private void ValidateCustomFungsiKunjungan() {
