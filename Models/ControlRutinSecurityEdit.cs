...
public partial class SnDOne {
    /// <summary>
    /// controlRutinSecurityEdit
    /// </summary>
    public static ControlRutinSecurityEdit controlRutinSecurityEdit
    {
        get => HttpData.Get<ControlRutinSecurityEdit>("controlRutinSecurityEdit")!;
        set => HttpData["controlRutinSecurityEdit"] = value;
    }

    /// <summary>
    /// Page class for ControlRutinSecurity
    /// </summary>
    public class ControlRutinSecurityEdit : ControlRutinSecurityEditBase
    {
        // Constructor
        public ControlRutinSecurityEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ControlRutinSecurityEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class ControlRutinSecurityEditBase : ControlRutinSecurity
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "controlRutinSecurityEdit";
        ...
        // Constructor
        public ControlRutinSecurityEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "ControlRutinSecurityEdit";
        ...
        // Constructor
        public ControlRutinSecurityEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdCRS", out rv)) {
                    IdCRS.FormValue = UrlDecode(rv);
                    IdCRS.OldValue = IdCRS.FormValue;
                } else if (CurrentForm.HasValue("x_IdCRS")) {
                    IdCRS.FormValue = CurrentForm.GetValue("x_IdCRS");
                    IdCRS.OldValue = IdCRS.FormValue;
                } else if (!Empty(keyValues)) {
                    IdCRS.OldValue = ConvertToString(keyValues[0]);
                } else {
                    loaded = false;
                }
                if (loaded)
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdCRS", out rv)) {
                        ...
                        IdCRS.CurrentValue = DbNullValue;
                    }
                }
                if (IsShow) {
                    loaded = await LoadRow();
        ...
        private async Task HandleApiFormKeys()
        {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                IdCRS.FormValue = ConvertToString(keyValues[0]);
            }
        }
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        controlRutinSecurityEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            if (!IdCRS.IsDetailKey) {
                SetNormalField(IdCRS, "IdCRS", "x_IdCRS");
            }
        ...
        {
            IdCRS.CurrentValue = IdCRS.FormValue;
        ...
        private void ValidateCustomIdCRS() {
            if (IdCRS.Visible && IdCRS.Required) {
                if (!IdCRS.IsDetailKey && Empty(IdCRS.FormValue)) {
                    IdCRS.AddErrorMessage(ConvertToString(Catatan.RequiredErrorMessage).Replace("%s", Catatan.Caption));
                }
            }
        }
