...
public partial class SnDOne {
    ...
    public class SubaktivitasNilaiBlalbdPipaListBase : SubaktivitasNilaiBlalbdPipa
    {
        ...
        // Constructor
        public SubaktivitasNilaiBlalbdPipaListBase(Controller? controller)
        {
        ...
        // Constructor
        public SubaktivitasNilaiBlalbdPipaListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field AL_klobs
            if (filter.TryGetValue("x_AL_klobs", out sv)) {
                RestoreFieldFilter(AL_klobs, filter, "AL_klobs", sv);
            }

            // Field AL_kl15
            if (filter.TryGetValue("x_AL_kl15", out sv)) {
                RestoreFieldFilter(AL_kl15, filter, "AL_kl15", sv);
            }

            // Field AL_barrels
            if (filter.TryGetValue("x_AL_barrels", out sv)) {
                RestoreFieldFilter(AL_barrels, filter, "AL_barrels", sv);
            }

            // Field AL_lt
            if (filter.TryGetValue("x_AL_lt", out sv)) {
                RestoreFieldFilter(AL_lt, filter, "AL_lt", sv);
            }

            // Field AL_mt
            if (filter.TryGetValue("x_AL_mt", out sv)) {
                RestoreFieldFilter(AL_mt, filter, "AL_mt", sv);
            }

            // Field BD_klobs
            if (filter.TryGetValue("x_BD_klobs", out sv)) {
                RestoreFieldFilter(BD_klobs, filter, "BD_klobs", sv);
            }

            // Field BD_kl15
            if (filter.TryGetValue("x_BD_kl15", out sv)) {
                RestoreFieldFilter(BD_kl15, filter, "BD_kl15", sv);
            }

            // Field BD_barrels
            if (filter.TryGetValue("x_BD_barrels", out sv)) {
                RestoreFieldFilter(BD_barrels, filter, "BD_barrels", sv);
            }

            // Field BD_lt
            if (filter.TryGetValue("x_BD_lt", out sv)) {
                RestoreFieldFilter(BD_lt, filter, "BD_lt", sv);
            }

            // Field BD_mt
            if (filter.TryGetValue("x_BD_mt", out sv)) {
                RestoreFieldFilter(BD_mt, filter, "BD_mt", sv);
            }
