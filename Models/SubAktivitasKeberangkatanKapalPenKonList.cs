...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasKeberangkatanKapalPenKonList
    /// </summary>
    public static SubAktivitasKeberangkatanKapalPenKonList subAktivitasKeberangkatanKapalPenKonList
    {
        get => HttpData.Get<SubAktivitasKeberangkatanKapalPenKonList>("subAktivitasKeberangkatanKapalPenKonList")!;
        set => HttpData["subAktivitasKeberangkatanKapalPenKonList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasKeberangkatanKapalPenKon
    /// </summary>
    public class SubAktivitasKeberangkatanKapalPenKonList : SubAktivitasKeberangkatanKapalPenKonListBase
    {
        // Constructor
        public SubAktivitasKeberangkatanKapalPenKonList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasKeberangkatanKapalPenKonList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasKeberangkatanKapalPenKonListBase : SubAktivitasKeberangkatanKapalPenKon
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasKeberangkatanKapalPenKonList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasKeberangkatanKapalPenKonlist";
        ...
        // Constructor
        public SubAktivitasKeberangkatanKapalPenKonListBase()
        {
            TableName = "SubAktivitasKeberangkatanKapalPenKon";

            // CSS class name as context
            TableVar = "SubAktivitasKeberangkatanKapalPenKon";
            ...
            // Table object (subAktivitasKeberangkatanKapalPenKon)
            if (subAktivitasKeberangkatanKapalPenKon == null || subAktivitasKeberangkatanKapalPenKon is SubAktivitasKeberangkatanKapalPenKon)
                subAktivitasKeberangkatanKapalPenKon = this;

            // Initialize URLs
            AddUrl = "SubAktivitasKeberangkatanKapalPenKonAdd";
            ...
            MultiDeleteUrl = "SubAktivitasKeberangkatanKapalPenKonDelete";
            MultiUpdateUrl = "SubAktivitasKeberangkatanKapalPenKonUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasKeberangkatanKapalPenKonList";
        ...
        // Constructor
        public SubAktivitasKeberangkatanKapalPenKonListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasKeberangkatanKapalPenKonView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasKeberangkatanKapalPenKongrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasKeberangkatanKapalPenKonList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasKeberangkatanKapalPenKon"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasKeberangkatanKapalPenKonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasKeberangkatanKapalPenKonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasKeberangkatanKapalPenKonsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasKeberangkatanKapalPenKonsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasKeberangkatanKapalPenKonlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasKeberangkatanKapalPenKon");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasKeberangkatanKapalPenKonList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasKeberangkatanKapalPenKonList.RowCount) + "_SubAktivitasKeberangkatanKapalPenKon");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasKeberangkatanKapalPenKonList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasKeberangkatanKapalPenKonList.RowType == RowType.Add || IsEdit && subAktivitasKeberangkatanKapalPenKonList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasKeberangkatanKapalPenKonsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
