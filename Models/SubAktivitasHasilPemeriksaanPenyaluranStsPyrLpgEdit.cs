...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit
    /// </summary>
    public static SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit subAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit
    {
        get => HttpData.Get<SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit>("subAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit")!;
        set => HttpData["subAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasHasilPemeriksaanPenyaluranSTSPyrLPG
    /// </summary>
    public class SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit : SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEditBase
    {
        // Constructor
        public SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEditBase : SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit";
        ...
        // Constructor
        public SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit";
        ...
        // Constructor
        public SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasHasilPemeriksaanPenyaluranStsPyrLpgEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomNamaKapal() {
            if (NamaKapal.Visible && NamaKapal.Required) {
                if (!NamaKapal.IsDetailKey && Empty(NamaKapal.FormValue)) {
                    NamaKapal.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNamaKegiatan() {
            if (NamaKegiatan.Visible && NamaKegiatan.Required) {
                if (!NamaKegiatan.IsDetailKey && Empty(NamaKegiatan.FormValue)) {
                    NamaKegiatan.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomDensity() {
            if (Density.Visible && Density.Required) {
                if (!Density.IsDetailKey && Empty(Density.FormValue)) {
                    Density.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTemperature() {
            if (Temperature.Visible && Temperature.Required) {
                if (!Temperature.IsDetailKey && Empty(Temperature.FormValue)) {
                    Temperature.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomPressure() {
            if (Pressure.Visible && Pressure.Required) {
                if (!Pressure.IsDetailKey && Empty(Pressure.FormValue)) {
                    Pressure.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasHasilPemeriksaanPenyaluranStsPyrLpgList")), "", TableVar, true);
            string pageId = "edit";
