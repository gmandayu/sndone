...
public partial class SnDOne {
    ...
    public class ExportHandler
    {
        ...
        // Export data
        public async Task<IActionResult> ExportData(string exportType, string table, string[]? recordKey = null)
        {
            ...
            if (isList && tbl.Type != "REPORT") { // List/View page
                if (Empty(Param("key_m[]"))) { // Skip selected keys // DN
                    ...
                    recordKey = recordKeys.Count > 0
                        ? (recordKeys[0] is string[]) ? recordKeys[0] : [recordKeys[0]]
                        : new string[0];
                    isList = Empty(recordKey);
                }
            }
            ...
            string pageClassName = tbl.GetApiPageClassName(isList ? Config.ApiListAction : Config.ApiViewAction); // DN
            ...
            if (page != null) {
                ...
                if (Post("data", out StringValues ct)) {
                    ...
                    if (custom) { // Custom export / Report
                        if (!page.IsTerminated) {
                            string html = await GetViewOutput(pageName, page: page);
                ...
                // Write export log
                if (!Empty(fileId) || Config.LogAllExportRequests) {
                    if (Empty(fileId)) { // Create unique key for file Id
                        fileId = DbCurrentDateTime() + "/" + CurrentUserIdentifier() + "/" + exportType + "/" + table;
                        if (fileId.Length > 36)
                            fileId = fileId.Substring(0, 36);
                    }
                    await WriteExportLog(fileId, DbCurrentDateTime(), CurrentUserIdentifier(), exportType, table, keyValue, fileName, Request?.QueryString.ToString() ?? "");
                }
            } else {
                result = new { success = false, error = Language.Phrase("InvalidParameter") + ": table = " + table + ", export type = " + exportType };
            }

            // Clean up export files
            if (Config.ExportFilesExpiryTime > 0) {
                CleanPath(ExportPath(true), false, file => { // DN
                    if (FileExists(file)) {
        ...
        protected async Task<string> ReplaceCharts(string text, Dictionary<string, string> files, string exportType)
        {
            ...
            foreach (var chart in doc.QuerySelectorAll(".ew-chart")) {
                ...
                if (files.TryGetValue(id, out string? filePath)) {
                    ...
                    byte[] data = await FileReadAllBytesAsync(filePath);
        ...
        {
            List<string> zipNames = [Config.ExportLogArchivePrefix];
            ...
            if (Int32.TryParse(limitValue, out int limit) && limit > 0) {
                ...
                rows = await tbl.Connection.GetRowsAsync(tbl.Connection.SelectLimitSql(sql, limit, -1, true));
            } else if (!Empty(filter)) {
                rows = await tbl.Connection.GetRowsAsync(tbl.GetSql(filter));
            } else {
                return new EmptyResult();
            }
            List<string> fileIds = rows.Where(row => !Empty(row[Config.ExportLogFieldNameFileId]) && CheckGuid(ConvertToString(row[Config.ExportLogFieldNameFileId]))).Select(row => ConvertToString(row[Config.ExportLogFieldNameFileId])).ToList();
            if (output && fileIds.Count >= 1) {
                if (fileIds.Count == 1) { // Single file, just output
                    ...
                    try {
                        ...
                        byte[] data = await FileReadAllBytesAsync(zipFile);
                        File.Delete(zipFile);
