...
public partial class SnDOne {
    ...
    public class MasterRegionListBase : MasterRegion
    {
        ...
        // Constructor
        public MasterRegionListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected bool RestoreFilterList() {
            ...
            // Field RegionCode
            if (filter?.TryGetValue("x_RegionCode", out sv) ?? false) {
                RegionCode.AdvancedSearch.SearchValue = sv;
                RegionCode.AdvancedSearch.SearchOperator = filter["z_RegionCode"];
                RegionCode.AdvancedSearch.SearchCondition = filter["v_RegionCode"];
                RegionCode.AdvancedSearch.SearchValue2 = filter["y_RegionCode"];
                RegionCode.AdvancedSearch.SearchOperator2 = filter["w_RegionCode"];
                RegionCode.AdvancedSearch.Save();
            }
        ...
        public void ShowFilterList()
        {
            ...
            // Field IdRegion
            filter = QueryBuilderWhere("IdRegion");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdRegion, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdRegion.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field RegionCode
            filter = QueryBuilderWhere("RegionCode");
            if (Empty(filter))
                BuildSearchSql(ref filter, RegionCode, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + RegionCode.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        protected void LoadSearchValues() {
            ...
            // IdRegion
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdRegion[]"))
            ...
            // RegionCode
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_RegionCode"))
                    RegionCode.AdvancedSearch.SearchValue = Get("x_RegionCode", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z_RegionCode"))
                RegionCode.AdvancedSearch.SearchOperator = Get("z_RegionCode", Config.FilterOptionSeparator);
        }
