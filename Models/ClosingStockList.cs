...
public partial class SnDOne {
    ...
    /// closingStockList
    /// </summary>
    public static ClosingStockList closingStockList
    {
        get => HttpData.Get<ClosingStockList>("closingStockList")!;
        set => HttpData["closingStockList"] = value;
    }

    /// <summary>
    /// Page class for ClosingStock
    /// </summary>
    public class ClosingStockList : ClosingStockListBase
    {
        // Constructor
        public ClosingStockList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ClosingStockList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class ClosingStockListBase : ClosingStock
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "closingStockList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fClosingStocklist";
        ...
        // Constructor
        public ClosingStockListBase(Controller? controller)
        {
            TableName = "ClosingStock";

            // CSS class name as context
            TableVar = "ClosingStock";
        ...
        if (controller != null)
            ...
            MultiDeleteUrl = "ClosingStockDelete";
            MultiUpdateUrl = "ClosingStockUpdate";
        ...
        // Page name
        public string PageName => "ClosingStockList";
        ...
        // Constructor
        public ClosingStockListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(TanggalClosingStock.AdvancedSearch.ToJson())); // Field TanggalClosingStock
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field TanggalClosingStock
            if (filter.TryGetValue("x_TanggalClosingStock", out sv)) {
                RestoreFieldFilter(TanggalClosingStock, filter, "TanggalClosingStock", sv);
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, NomorClosingStock, def, true); // NomorClosingStock
            ...
            BuildSearchSql(ref where, TanggalClosingStock, def, false); // TanggalClosingStock
            BuildSearchSql(ref where, Produk, def, false); // Produk
            ...
            if (!def && Command == "search") {
                TanggalClosingStock.AdvancedSearch.Save(); // TanggalClosingStock
                Produk.AdvancedSearch.Save(); // Produk
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("TanggalClosingStock", TanggalClosingStock, false, config);
            filterList += ProcessFieldFilter("Produk", Produk, false, config);
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NomorClosingStock);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (NomorClosingStock.AdvancedSearch.IssetSession)
                return true;
            ...
            if (IdTangki.AdvancedSearch.IssetSession)
                return true;
            if (TanggalClosingStock.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            NomorClosingStock.AdvancedSearch.UnsetSession();
            ...
            TanggalClosingStock.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            NomorClosingStock.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(TanggalClosingStock, ctrl); // TanggalClosingStock
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    IdClosingStock.Sort = "";
                    IdTemplate.Sort = "";
                    NomorClosingStock.Sort = "";
                    Proses2.Sort = "";
                    LookupPlant.Sort = "";
                    Plant.Sort = "";
                    TipeProdukSTS.Sort = "";
                    IdTangki.Sort = "";
                    TanggalClosingStock.Sort = "";
                    Produk.Sort = "";
                    StatusProses.Sort = "";
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""ClosingStock"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""ClosingStock"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""ClosingStock"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fClosingStocklist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fClosingStocklist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdClosingStock.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""ClosingStock"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fClosingStocklist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("TanggalClosingStock")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fClosingStocksrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fClosingStocksrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fClosingStocklist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_ClosingStock");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(closingStockList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(closingStockList.RowCount) + "_ClosingStock");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(closingStockList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && closingStockList.RowType == RowType.Add || IsEdit && closingStockList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldNomorClosingStockSearchValues()
        {
            // NomorClosingStock
            LoadPrimarySearchValueNomorClosingStock();
            LoadSearchOperatorNomorClosingStock();
        }

        private void LoadPrimarySearchValueNomorClosingStock()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NomorClosingStock[]")) {
                NomorClosingStock.AdvancedSearch.SearchValue = Get("x_NomorClosingStock[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NomorClosingStock.AdvancedSearch.SearchValue = Get("NomorClosingStock"); // Default Value // DN
            }
            if (!Empty(NomorClosingStock.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNomorClosingStock()
        {
            if (Query.ContainsKey("z_NomorClosingStock"))
                NomorClosingStock.AdvancedSearch.SearchOperator = Get("z_NomorClosingStock", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldTanggalClosingStockSearchValues()
        {
            // TanggalClosingStock
            LoadPrimarySearchValueTanggalClosingStock();
            LoadSearchOperatorTanggalClosingStock();
        }

        private void LoadPrimarySearchValueTanggalClosingStock()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TanggalClosingStock")) {
                TanggalClosingStock.AdvancedSearch.SearchValue = Get("x_TanggalClosingStock");
            }
            else if (!IsAddOrEdit) {
                TanggalClosingStock.AdvancedSearch.SearchValue = Get("TanggalClosingStock"); // Default Value // DN
            }
            if (!Empty(TanggalClosingStock.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTanggalClosingStock()
        {
            if (Query.ContainsKey("z_TanggalClosingStock"))
                TanggalClosingStock.AdvancedSearch.SearchOperator = Get("z_TanggalClosingStock");
        }
        ...
        private void LoadPrimarySearchValueProduk()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Produk")) {
                Produk.AdvancedSearch.SearchValue = Get("x_Produk");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorProduk()
        {
            if (Query.ContainsKey("z_Produk"))
                Produk.AdvancedSearch.SearchOperator = Get("z_Produk");
        }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fClosingStocksrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"ClosingStock\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("ClosingStockSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("ClosingStockSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
