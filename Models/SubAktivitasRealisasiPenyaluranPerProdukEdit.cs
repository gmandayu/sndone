...
public partial class SnDOne {
    ...
    public class SubAktivitasRealisasiPenyaluranPerProdukEditBase : SubAktivitasRealisasiPenyaluranPerProduk
    {
        ...
        // Constructor
        public SubAktivitasRealisasiPenyaluranPerProdukEditBase()
        {
        ...
        // Constructor
        public SubAktivitasRealisasiPenyaluranPerProdukEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasRealisasiPenyaluranPerProdukList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasRealisasiPenyaluranPerProdukList") {
                                ...
                                returnUrl = "SubAktivitasRealisasiPenyaluranPerProdukList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'Shift1' before field var 'x_Shift1'
            val = CurrentForm.HasValue("Shift1") ? CurrentForm.GetValue("Shift1") : CurrentForm.GetValue("x_Shift1");
            if (!Shift1.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Shift1") && !CurrentForm.HasValue("x_Shift1")) // DN
                    Shift1.Visible = false; // Disable update for API request
                else
                    Shift1.SetFormValue(val);
            }

            // Check field name 'Shift2' before field var 'x_Shift2'
            val = CurrentForm.HasValue("Shift2") ? CurrentForm.GetValue("Shift2") : CurrentForm.GetValue("x_Shift2");
            if (!Shift2.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Shift2") && !CurrentForm.HasValue("x_Shift2")) // DN
                    Shift2.Visible = false; // Disable update for API request
                else
                    Shift2.SetFormValue(val);
            }

            // Check field name 'Shift3' before field var 'x_Shift3'
            val = CurrentForm.HasValue("Shift3") ? CurrentForm.GetValue("Shift3") : CurrentForm.GetValue("x_Shift3");
            if (!Shift3.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Shift3") && !CurrentForm.HasValue("x_Shift3")) // DN
                    Shift3.Visible = false; // Disable update for API request
                else
                    Shift3.SetFormValue(val);
            }

            // Check field name 'Shift4' before field var 'x_Shift4'
            val = CurrentForm.HasValue("Shift4") ? CurrentForm.GetValue("Shift4") : CurrentForm.GetValue("x_Shift4");
            if (!Shift4.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Shift4") && !CurrentForm.HasValue("x_Shift4")) // DN
                    Shift4.Visible = false; // Disable update for API request
                else
                    Shift4.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (Shift1.Visible && Shift1.Required) {
                    if (!Shift1.IsDetailKey && Empty(Shift1.FormValue)) {
                        Shift1.AddErrorMessage(ConvertToString(Shift1.RequiredErrorMessage).Replace("%s", Shift1.Caption));
                    }
                }
                if (Shift2.Visible && Shift2.Required) {
                    if (!Shift2.IsDetailKey && Empty(Shift2.FormValue)) {
                        Shift2.AddErrorMessage(ConvertToString(Shift2.RequiredErrorMessage).Replace("%s", Shift2.Caption));
                    }
                }
                if (Shift3.Visible && Shift3.Required) {
                    if (!Shift3.IsDetailKey && Empty(Shift3.FormValue)) {
                        Shift3.AddErrorMessage(ConvertToString(Shift3.RequiredErrorMessage).Replace("%s", Shift3.Caption));
                    }
                }
                if (Shift4.Visible && Shift4.Required) {
                    if (!Shift4.IsDetailKey && Empty(Shift4.FormValue)) {
                        Shift4.AddErrorMessage(ConvertToString(Shift4.RequiredErrorMessage).Replace("%s", Shift4.Caption));
                    }
                }
