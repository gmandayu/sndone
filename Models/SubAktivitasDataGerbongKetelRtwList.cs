...
public partial class SnDOne {
    ...
    public class SubAktivitasDataGerbongKetelRtwListBase : SubAktivitasDataGerbongKetelRtw
    {
        ...
        // Constructor
        public SubAktivitasDataGerbongKetelRtwListBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasDataGerbongKetelRtwListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field Shipment
            if (filter.TryGetValue("x_Shipment", out sv)) {
                RestoreFieldFilter(Shipment, filter, "Shipment", sv);
            }

            // Field JenisProduk
            if (filter.TryGetValue("x_JenisProduk", out sv)) {
                RestoreFieldFilter(JenisProduk, filter, "JenisProduk", sv);
            }

            // Field TanggalTiba
            if (filter.TryGetValue("x_TanggalTiba", out sv)) {
                RestoreFieldFilter(TanggalTiba, filter, "TanggalTiba", sv);
            }

            // Field Nominasi
            if (filter.TryGetValue("x_Nominasi", out sv)) {
                RestoreFieldFilter(Nominasi, filter, "Nominasi", sv);
            }
            ...
            // Field DetailGerbong
            if (filter.TryGetValue("x_DetailGerbong", out sv)) {
                RestoreFieldFilter(DetailGerbong, filter, "DetailGerbong", sv);
            }

            // Field DetailQuantity
            if (filter.TryGetValue("x_DetailQuantity", out sv)) {
                RestoreFieldFilter(DetailQuantity, filter, "DetailQuantity", sv);
            }
        }
