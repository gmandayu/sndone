...
public partial class SnDOne {
    ...
    public class MasterPositionEditBase : MasterPosition
    {
        ...
        // Constructor
        public MasterPositionEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdPosition", out rv)) { // DN
                    IdPosition.FormValue = UrlDecode(rv); // DN
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdPosition", out rv)) { // DN
                        IdPosition.QueryValue = UrlDecode(rv); // DN
                        loadByQuery = true;
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("MasterPositionList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "MasterPositionList") {
                                ...
                                returnUrl = "MasterPositionList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NamaPosition' before field var 'x_NamaPosition'
            val = CurrentForm.HasValue("NamaPosition") ? CurrentForm.GetValue("NamaPosition") : CurrentForm.GetValue("x_NamaPosition");
            if (!NamaPosition.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NamaPosition") && !CurrentForm.HasValue("x_NamaPosition")) // DN
                    NamaPosition.Visible = false; // Disable update for API request
                else
                    NamaPosition.SetFormValue(val);
            }
            ...
            // Check field name 'Role' before field var 'x_Role'
            val = CurrentForm.HasValue("Role") ? CurrentForm.GetValue("Role") : CurrentForm.GetValue("x_Role");
            if (!Role.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Role") && !CurrentForm.HasValue("x_Role")) // DN
                    Role.Visible = false; // Disable update for API request
                else
                    Role.SetFormValue(val);
            }
        }
        #pragma warning restore 1998
            ...
            bool validateForm = true;
                ...
                if (NamaPosition.Visible && NamaPosition.Required) {
                    if (!NamaPosition.IsDetailKey && Empty(NamaPosition.FormValue)) {
                        NamaPosition.AddErrorMessage(ConvertToString(NamaPosition.RequiredErrorMessage).Replace("%s", NamaPosition.Caption));
                    }
                }
