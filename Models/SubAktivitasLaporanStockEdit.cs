...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasLaporanStockEdit
    /// </summary>
    public static SubAktivitasLaporanStockEdit subAktivitasLaporanStockEdit
    {
        get => HttpData.Get<SubAktivitasLaporanStockEdit>("subAktivitasLaporanStockEdit")!;
        set => HttpData["subAktivitasLaporanStockEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasLaporanStock
    /// </summary>
    public class SubAktivitasLaporanStockEdit : SubAktivitasLaporanStockEditBase
    {
        // Constructor
        public SubAktivitasLaporanStockEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasLaporanStockEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasLaporanStockEditBase : SubAktivitasLaporanStock
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasLaporanStockEdit";
        ...
        // Constructor
        public SubAktivitasLaporanStockEditBase(Controller? controller)
        {
            TableName = "SubAktivitasLaporanStock";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasLaporanStock)
            if (subAktivitasLaporanStock == null || subAktivitasLaporanStock is SubAktivitasLaporanStock)
                subAktivitasLaporanStock = this;
        ...
        // Page name
        public string PageName => "SubAktivitasLaporanStockEdit";
        ...
        // Constructor
        public SubAktivitasLaporanStockEditBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                ...
                // List / View / Master View page
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasLaporanStockView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasLaporanStockList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasLaporanStockList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasLaporanStockList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasLaporanStockEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'StockAwal'
            StockAwal.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(StockAwal, "StockAwal", "x_StockAwal");

            // Standard handling for 'Penerimaan2'
            Penerimaan2.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Penerimaan2, "Penerimaan", "x_Penerimaan2");

            // Standard handling for 'Penyaluran2'
            Penyaluran2.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Penyaluran2, "Penyaluran", "x_Penyaluran2");

            // Standard handling for 'StockAkhir'
            StockAkhir.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(StockAkhir, "StockAkhir", "x_StockAkhir");

            // Standard handling for 'WorkingLoss'
            WorkingLoss.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(WorkingLoss, "WorkingLoss", "x_WorkingLoss");

            // Standard handling for 'Presentase'
            Presentase.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Presentase, "Presentase", "x_Presentase");
            if (!id.IsDetailKey) {
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }

        private void ValidateCustomStockAwal() {
            if (StockAwal.Visible && StockAwal.Required) {
                if (!StockAwal.IsDetailKey && Empty(StockAwal.FormValue)) {
                    StockAwal.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }

        private void ValidateCustomPenerimaan2() {
            if (Penerimaan2.Visible && Penerimaan2.Required) {
                if (!Penerimaan2.IsDetailKey && Empty(Penerimaan2.FormValue)) {
                    Penerimaan2.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }

        private void ValidateCustomPenyaluran2() {
            if (Penyaluran2.Visible && Penyaluran2.Required) {
                if (!Penyaluran2.IsDetailKey && Empty(Penyaluran2.FormValue)) {
                    Penyaluran2.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }

        private void ValidateCustomStockAkhir() {
            if (StockAkhir.Visible && StockAkhir.Required) {
                if (!StockAkhir.IsDetailKey && Empty(StockAkhir.FormValue)) {
                    StockAkhir.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }

        private void ValidateCustomWorkingLoss() {
            if (WorkingLoss.Visible && WorkingLoss.Required) {
                if (!WorkingLoss.IsDetailKey && Empty(WorkingLoss.FormValue)) {
                    WorkingLoss.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                }
            }
        }

        private void ValidateCustomPresentase() {
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomStockAwal();
                ValidateCustomPenerimaan2();
                ValidateCustomPenyaluran2();
                ValidateCustomStockAkhir();
                ValidateCustomWorkingLoss();
                ValidateCustomPresentase();
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasLaporanStockList")), "", TableVar, true);
            string pageId = "edit";
