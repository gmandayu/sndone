...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiSfbdPenKonList
    /// </summary>
    public static SubAktivitasNilaiSfbdPenKonList subAktivitasNilaiSfbdPenKonList
    {
        get => HttpData.Get<SubAktivitasNilaiSfbdPenKonList>("subAktivitasNilaiSfbdPenKonList")!;
        set => HttpData["subAktivitasNilaiSfbdPenKonList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiSFBDPenKon
    /// </summary>
    public class SubAktivitasNilaiSfbdPenKonList : SubAktivitasNilaiSfbdPenKonListBase
    {
        // Constructor
        public SubAktivitasNilaiSfbdPenKonList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiSfbdPenKonList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiSfbdPenKonListBase : SubAktivitasNilaiSfbdPenKon
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiSfbdPenKonList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasNilaiSFBDPenKonlist";
        ...
        // Constructor
        public SubAktivitasNilaiSfbdPenKonListBase()
        {
            ...
            // Initialize URLs
            AddUrl = "SubAktivitasNilaiSfbdPenKonAdd";
            ...
            MultiDeleteUrl = "SubAktivitasNilaiSfbdPenKonDelete";
            MultiUpdateUrl = "SubAktivitasNilaiSfbdPenKonUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiSfbdPenKonList";
        ...
        // Constructor
        public SubAktivitasNilaiSfbdPenKonListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasNilaiSFBDPenKongrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasNilaiSfbdPenKonList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(SFBD_KLObs);
            searchFlds.Add(SFBD_KL15);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(SFBD_KLObs, ctrl); // SFBD_KLObs
                UpdateSort(SFBD_KL15, ctrl); // SFBD_KL15
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasNilaiSFBDPenKon"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiSFBDPenKonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiSFBDPenKonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("SFBD_KLObs")); // DN
                CreateColumnOption(option.Add("SFBD_KL15")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasNilaiSFBDPenKonsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasNilaiSFBDPenKonsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiSFBDPenKonlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasNilaiSFBDPenKon");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasNilaiSfbdPenKonList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasNilaiSfbdPenKonList.RowCount) + "_SubAktivitasNilaiSFBDPenKon");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasNilaiSfbdPenKonList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasNilaiSfbdPenKonList.RowType == RowType.Add || IsEdit && subAktivitasNilaiSfbdPenKonList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasNilaiSFBDPenKonsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
