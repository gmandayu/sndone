...
public partial class SnDOne {
    ...
    public class SubAktivitasNilaiEtcetaPipaEditBase : SubAktivitasNilaiEtcetaPipa
    {
        ...
        // Constructor
        public SubAktivitasNilaiEtcetaPipaEditBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasNilaiEtcetaPipaEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasNilaiEtcetaPipaList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "SubAktivitasNilaiEtcetaPipaList") {
                        ...
                        returnUrl = "SubAktivitasNilaiEtcetaPipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'IsiPipa'
            IsiPipa.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(IsiPipa, "IsiPipa", "x_IsiPipa");

            // Standard handling for 'IsiPipa2'
            IsiPipa2.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(IsiPipa2, "IsiPipa2", "x_IsiPipa2");

            // Standard handling for 'IsiPipa3'
            IsiPipa3.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(IsiPipa3, "IsiPipa3", "x_IsiPipa3");

            // Standard handling for 'Flowrate'
            Flowrate.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Flowrate, "Flowrate", "x_Flowrate");

            // Standard handling for 'LamaPengisian'
            LamaPengisian.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(LamaPengisian, "LamaPengisian", "x_LamaPengisian");

            // Standard handling for 'LamaPengiriman'
            LamaPengiriman.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(LamaPengiriman, "LamaPengiriman", "x_LamaPengiriman");

            // Standard handling for 'LamaPengiriman2'
            LamaPengiriman2.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(LamaPengiriman2, "LamaPengiriman2", "x_LamaPengiriman2");

            // Standard handling for 'LamaPengiriman3'
            LamaPengiriman3.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(LamaPengiriman3, "LamaPengiriman3", "x_LamaPengiriman3");

            // Standard handling for 'ETC'
            ETC.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(ETC, "ETC", "x_ETC", false, false, true);

            // Standard handling for 'ETA'
            ETA.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(ETA, "ETA", "x_ETA", false, false, true);

            // Standard handling for 'ETA2'
            ETA2.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(ETA2, "ETA2", "x_ETA2", false, false, true);

            // Standard handling for 'ETA3'
            ETA3.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(ETA3, "ETA3", "x_ETA3", false, false, true);
            if (!id.IsDetailKey) {
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomIsiPipa() {
            if (IsiPipa.Visible && IsiPipa.Required) {
                if (!IsiPipa.IsDetailKey && Empty(IsiPipa.FormValue)) {
                    IsiPipa.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomIsiPipa2() {
            if (IsiPipa2.Visible && IsiPipa2.Required) {
                if (!IsiPipa2.IsDetailKey && Empty(IsiPipa2.FormValue)) {
                    IsiPipa2.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomIsiPipa3() {
            if (IsiPipa3.Visible && IsiPipa3.Required) {
                if (!IsiPipa3.IsDetailKey && Empty(IsiPipa3.FormValue)) {
                    IsiPipa3.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomFlowrate() {
            if (Flowrate.Visible && Flowrate.Required) {
                if (!Flowrate.IsDetailKey && Empty(Flowrate.FormValue)) {
                    Flowrate.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomLamaPengisian() {
            if (LamaPengisian.Visible && LamaPengisian.Required) {
                if (!LamaPengisian.IsDetailKey && Empty(LamaPengisian.FormValue)) {
                    LamaPengisian.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomLamaPengiriman() {
            if (LamaPengiriman.Visible && LamaPengiriman.Required) {
                if (!LamaPengiriman.IsDetailKey && Empty(LamaPengiriman.FormValue)) {
                    LamaPengiriman.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomLamaPengiriman2() {
            if (LamaPengiriman2.Visible && LamaPengiriman2.Required) {
                if (!LamaPengiriman2.IsDetailKey && Empty(LamaPengiriman2.FormValue)) {
                    LamaPengiriman2.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomLamaPengiriman3() {
            if (LamaPengiriman3.Visible && LamaPengiriman3.Required) {
                if (!LamaPengiriman3.IsDetailKey && Empty(LamaPengiriman3.FormValue)) {
                    LamaPengiriman3.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomETC() {
            if (ETC.Visible && ETC.Required) {
                if (!ETC.IsDetailKey && Empty(ETC.FormValue)) {
                    ETC.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomETA() {
            if (ETA.Visible && ETA.Required) {
                if (!ETA.IsDetailKey && Empty(ETA.FormValue)) {
                    ETA.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomETA2() {
            if (ETA2.Visible && ETA2.Required) {
                if (!ETA2.IsDetailKey && Empty(ETA2.FormValue)) {
                    ETA2.AddErrorMessage(ConvertToString(ETA3.RequiredErrorMessage).Replace("%s", ETA3.Caption));
                }
            }
        }

        private void ValidateCustomETA3() {
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomIsiPipa();
                ValidateCustomIsiPipa2();
                ValidateCustomIsiPipa3();
                ValidateCustomFlowrate();
                ValidateCustomLamaPengisian();
                ValidateCustomLamaPengiriman();
                ValidateCustomLamaPengiriman2();
                ValidateCustomLamaPengiriman3();
                ValidateCustomETC();
                ValidateCustomETA();
                ValidateCustomETA2();
                ValidateCustomETA3();
