...
public partial class SnDOne {
    ...
    public class MwtOnlineDetailListBase : MwtOnlineDetail
    {
        ...
        // Constructor
        public MwtOnlineDetailListBase(Controller? controller)
        {
        ...
        // Constructor
        public MwtOnlineDetailListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field Penjelasan
            if (filter.TryGetValue("x_Penjelasan", out sv)) {
                RestoreFieldFilter(Penjelasan, filter, "Penjelasan", sv);
            }

            // Field UploadEvidence
            if (filter.TryGetValue("x_UploadEvidence", out sv)) {
                RestoreFieldFilter(UploadEvidence, filter, "UploadEvidence", sv);
            }
            ...
            // Field FileType
            if (filter.TryGetValue("x_FileType", out sv)) {
                RestoreFieldFilter(FileType, filter, "FileType", sv);
            }

            // Field OriginalFileName
            if (filter.TryGetValue("x_OriginalFileName", out sv)) {
                RestoreFieldFilter(OriginalFileName, filter, "OriginalFileName", sv);
            }

            // Field Flag
            if (filter.TryGetValue("x_Flag", out sv)) {
                RestoreFieldFilter(Flag, filter, "Flag", sv);
            }
        }
