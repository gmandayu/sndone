...
public partial class SnDOne {
    ...
    /// setUserModList
    /// </summary>
    public static SetUserModList setUserModList
    {
        get => HttpData.Get<SetUserModList>("setUserModList")!;
        set => HttpData["setUserModList"] = value;
    }

    /// <summary>
    /// Page class for SetUserMOD
    /// </summary>
    public class SetUserModList : SetUserModListBase
    {
        // Constructor
        public SetUserModList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SetUserModList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class SetUserModListBase : SetUserMod
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "setUserModList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSetUserMODlist";
        ...
        // Constructor
        public SetUserModListBase(Controller? controller)
        {
        ...
        if (controller != null)
            ...
            MultiDeleteUrl = "SetUserModDelete";
            MultiUpdateUrl = "SetUserModUpdate";
        ...
        // Page name
        public string PageName => "SetUserModList";
        ...
        // Constructor
        public SetUserModListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(LookupUserMod.AdvancedSearch.ToJson())); // Field LookupUserMod
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field LookupUserMod
            if (filter.TryGetValue("x_LookupUserMod", out sv)) {
                RestoreFieldFilter(LookupUserMod, filter, "LookupUserMod", sv);
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, id, def, true); // id
            ...
            BuildSearchSql(ref where, LookupUserMod, def, true); // LookupUserMod
            BuildSearchSql(ref where, UserMOD, def, true); // UserMOD
            ...
            if (!def && Command == "search") {
                ...
                LookupUserMod.AdvancedSearch.Save(); // LookupUserMod
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("UserMOD", UserMOD, true, config);
        ...
        protected bool CheckSearchParms() {
            ...
            if (Plant.AdvancedSearch.IssetSession)
                return true;
            if (LookupUserMod.AdvancedSearch.IssetSession)
                return true;
            if (UserMOD.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            LookupUserMod.AdvancedSearch.UnsetSession();
            UserMOD.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            LookupUserMod.AdvancedSearch.Load();
            UserMOD.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(UserMOD, ctrl); // UserMOD
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""SetUserMOD"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SetUserMOD"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""SetUserMOD"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSetUserMODlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSetUserMODlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""SetUserMOD"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fSetUserMODlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("UserMOD")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSetUserMODsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSetUserMODsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSetUserMODlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(setUserModList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(setUserModList.RowCount) + "_SetUserMOD");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(setUserModList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && setUserModList.RowType == RowType.Add || IsEdit && setUserModList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldSearchValues()
        {
            ...
            LoadFieldLookupUserModSearchValues();
            LoadFieldUserMODSearchValues();
        ...
        private void LoadPrimarySearchValueid()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_id[]")) {
                id.AdvancedSearch.SearchValue = Get("x_id[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorid()
        {
            if (Query.ContainsKey("z_id"))
                id.AdvancedSearch.SearchOperator = Get("z_id", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldLookupUserModSearchValues()
        {
            // LookupUserMod
            LoadPrimarySearchValueLookupUserMod();
            LoadSearchOperatorLookupUserMod();
        }

        private void LoadPrimarySearchValueLookupUserMod()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_LookupUserMod[]")) {
                LookupUserMod.AdvancedSearch.SearchValue = Get("x_LookupUserMod[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                LookupUserMod.AdvancedSearch.SearchValue = Get("LookupUserMod"); // Default Value // DN
            }
            if (!Empty(LookupUserMod.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorLookupUserMod()
        {
            if (Query.ContainsKey("z_LookupUserMod"))
                LookupUserMod.AdvancedSearch.SearchOperator = Get("z_LookupUserMod", Config.FilterOptionSeparator);
        }

        private void LoadFieldUserMODSearchValues()
        {
            // UserMOD
            LoadPrimarySearchValueUserMOD();
            LoadSearchOperatorUserMOD();
        }

        private void LoadPrimarySearchValueUserMOD()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UserMOD[]")) {
                UserMOD.AdvancedSearch.SearchValue = Get("x_UserMOD[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                UserMOD.AdvancedSearch.SearchValue = Get("UserMOD"); // Default Value // DN
            }
            if (!Empty(UserMOD.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorUserMOD()
        {
            if (Query.ContainsKey("z_UserMOD"))
                UserMOD.AdvancedSearch.SearchOperator = Get("z_UserMOD", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // UserMOD
                if (UserMOD.UseFilter && !Empty(UserMOD.AdvancedSearch.SearchValue)) {
                    UserMOD.EditValue = ConvertToString(UserMOD.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSetUserMODsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
