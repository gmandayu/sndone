...
public partial class SnDOne {
    /// <summary>
    /// subaktivitasNilaiBlalbdPipaList
    /// </summary>
    public static SubaktivitasNilaiBlalbdPipaList subaktivitasNilaiBlalbdPipaList
    {
        get => HttpData.Get<SubaktivitasNilaiBlalbdPipaList>("subaktivitasNilaiBlalbdPipaList")!;
        set => HttpData["subaktivitasNilaiBlalbdPipaList"] = value;
    }

    /// <summary>
    /// Page class for SubaktivitasNilaiBLALBDPipa
    /// </summary>
    public class SubaktivitasNilaiBlalbdPipaList : SubaktivitasNilaiBlalbdPipaListBase
    {
        // Constructor
        public SubaktivitasNilaiBlalbdPipaList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubaktivitasNilaiBlalbdPipaList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubaktivitasNilaiBlalbdPipaListBase : SubaktivitasNilaiBlalbdPipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subaktivitasNilaiBlalbdPipaList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubaktivitasNilaiBLALBDPipalist";
        ...
        // Constructor
        public SubaktivitasNilaiBlalbdPipaListBase()
        {
            TableName = "SubaktivitasNilaiBLALBDPipa";

            // CSS class name as context
            TableVar = "SubaktivitasNilaiBLALBDPipa";
            ...
            // Initialize URLs
            AddUrl = "SubaktivitasNilaiBlalbdPipaAdd";
            ...
            MultiDeleteUrl = "SubaktivitasNilaiBlalbdPipaDelete";
            MultiUpdateUrl = "SubaktivitasNilaiBlalbdPipaUpdate";
        ...
        // Page name
        public string PageName => "SubaktivitasNilaiBlalbdPipaList";
        ...
        // Constructor
        public SubaktivitasNilaiBlalbdPipaListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubaktivitasNilaiBLALBDPipagrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subaktivitasNilaiBlalbdPipaList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(BL_klobs.AdvancedSearch.ToJson())); // Field BL_klobs
            filters.Merge(JObject.Parse(BL_kl15.AdvancedSearch.ToJson())); // Field BL_kl15
            filters.Merge(JObject.Parse(BL_barrels.AdvancedSearch.ToJson())); // Field BL_barrels
            filters.Merge(JObject.Parse(BL_lt.AdvancedSearch.ToJson())); // Field BL_lt
            filters.Merge(JObject.Parse(BL_mt.AdvancedSearch.ToJson())); // Field BL_mt
            filters.Merge(JObject.Parse(AL_klobs.AdvancedSearch.ToJson())); // Field AL_klobs
            filters.Merge(JObject.Parse(AL_kl15.AdvancedSearch.ToJson())); // Field AL_kl15
            filters.Merge(JObject.Parse(AL_barrels.AdvancedSearch.ToJson())); // Field AL_barrels
            filters.Merge(JObject.Parse(AL_lt.AdvancedSearch.ToJson())); // Field AL_lt
            filters.Merge(JObject.Parse(AL_mt.AdvancedSearch.ToJson())); // Field AL_mt
            filters.Merge(JObject.Parse(BD_klobs.AdvancedSearch.ToJson())); // Field BD_klobs
            filters.Merge(JObject.Parse(BD_kl15.AdvancedSearch.ToJson())); // Field BD_kl15
            filters.Merge(JObject.Parse(BD_barrels.AdvancedSearch.ToJson())); // Field BD_barrels
            filters.Merge(JObject.Parse(BD_lt.AdvancedSearch.ToJson())); // Field BD_lt
            filters.Merge(JObject.Parse(BD_mt.AdvancedSearch.ToJson())); // Field BD_mt
        ...
        protected bool RestoreFilterList() {
            ...
            // Field BL_klobs
            if (filter?.TryGetValue("x_BL_klobs", out sv) ?? false) {
                BL_klobs.AdvancedSearch.SearchValue = sv;
                BL_klobs.AdvancedSearch.SearchOperator = filter["z_BL_klobs"];
                BL_klobs.AdvancedSearch.SearchCondition = filter["v_BL_klobs"];
                BL_klobs.AdvancedSearch.SearchValue2 = filter["y_BL_klobs"];
                BL_klobs.AdvancedSearch.SearchOperator2 = filter["w_BL_klobs"];
                BL_klobs.AdvancedSearch.Save();
            }

            // Field BL_kl15
            if (filter?.TryGetValue("x_BL_kl15", out sv) ?? false) {
                BL_kl15.AdvancedSearch.SearchValue = sv;
                BL_kl15.AdvancedSearch.SearchOperator = filter["z_BL_kl15"];
                BL_kl15.AdvancedSearch.SearchCondition = filter["v_BL_kl15"];
                BL_kl15.AdvancedSearch.SearchValue2 = filter["y_BL_kl15"];
                BL_kl15.AdvancedSearch.SearchOperator2 = filter["w_BL_kl15"];
                BL_kl15.AdvancedSearch.Save();
            }

            // Field BL_barrels
            if (filter?.TryGetValue("x_BL_barrels", out sv) ?? false) {
                BL_barrels.AdvancedSearch.SearchValue = sv;
                BL_barrels.AdvancedSearch.SearchOperator = filter["z_BL_barrels"];
                BL_barrels.AdvancedSearch.SearchCondition = filter["v_BL_barrels"];
                BL_barrels.AdvancedSearch.SearchValue2 = filter["y_BL_barrels"];
                BL_barrels.AdvancedSearch.SearchOperator2 = filter["w_BL_barrels"];
                BL_barrels.AdvancedSearch.Save();
            }

            // Field BL_lt
            if (filter?.TryGetValue("x_BL_lt", out sv) ?? false) {
                BL_lt.AdvancedSearch.SearchValue = sv;
                BL_lt.AdvancedSearch.SearchOperator = filter["z_BL_lt"];
                BL_lt.AdvancedSearch.SearchCondition = filter["v_BL_lt"];
                BL_lt.AdvancedSearch.SearchValue2 = filter["y_BL_lt"];
                BL_lt.AdvancedSearch.SearchOperator2 = filter["w_BL_lt"];
                BL_lt.AdvancedSearch.Save();
            }

            // Field BL_mt
            if (filter?.TryGetValue("x_BL_mt", out sv) ?? false) {
                BL_mt.AdvancedSearch.SearchValue = sv;
                BL_mt.AdvancedSearch.SearchOperator = filter["z_BL_mt"];
                BL_mt.AdvancedSearch.SearchCondition = filter["v_BL_mt"];
                BL_mt.AdvancedSearch.SearchValue2 = filter["y_BL_mt"];
                BL_mt.AdvancedSearch.SearchOperator2 = filter["w_BL_mt"];
                BL_mt.AdvancedSearch.Save();
            }

            // Field AL_klobs
            if (filter?.TryGetValue("x_AL_klobs", out sv) ?? false) {
                AL_klobs.AdvancedSearch.SearchValue = sv;
                AL_klobs.AdvancedSearch.SearchOperator = filter["z_AL_klobs"];
                AL_klobs.AdvancedSearch.SearchCondition = filter["v_AL_klobs"];
                AL_klobs.AdvancedSearch.SearchValue2 = filter["y_AL_klobs"];
                AL_klobs.AdvancedSearch.SearchOperator2 = filter["w_AL_klobs"];
                AL_klobs.AdvancedSearch.Save();
            }

            // Field AL_kl15
            if (filter?.TryGetValue("x_AL_kl15", out sv) ?? false) {
                AL_kl15.AdvancedSearch.SearchValue = sv;
                AL_kl15.AdvancedSearch.SearchOperator = filter["z_AL_kl15"];
                AL_kl15.AdvancedSearch.SearchCondition = filter["v_AL_kl15"];
                AL_kl15.AdvancedSearch.SearchValue2 = filter["y_AL_kl15"];
                AL_kl15.AdvancedSearch.SearchOperator2 = filter["w_AL_kl15"];
                AL_kl15.AdvancedSearch.Save();
            }

            // Field AL_barrels
            if (filter?.TryGetValue("x_AL_barrels", out sv) ?? false) {
                AL_barrels.AdvancedSearch.SearchValue = sv;
                AL_barrels.AdvancedSearch.SearchOperator = filter["z_AL_barrels"];
                AL_barrels.AdvancedSearch.SearchCondition = filter["v_AL_barrels"];
                AL_barrels.AdvancedSearch.SearchValue2 = filter["y_AL_barrels"];
                AL_barrels.AdvancedSearch.SearchOperator2 = filter["w_AL_barrels"];
                AL_barrels.AdvancedSearch.Save();
            }

            // Field AL_lt
            if (filter?.TryGetValue("x_AL_lt", out sv) ?? false) {
                AL_lt.AdvancedSearch.SearchValue = sv;
                AL_lt.AdvancedSearch.SearchOperator = filter["z_AL_lt"];
                AL_lt.AdvancedSearch.SearchCondition = filter["v_AL_lt"];
                AL_lt.AdvancedSearch.SearchValue2 = filter["y_AL_lt"];
                AL_lt.AdvancedSearch.SearchOperator2 = filter["w_AL_lt"];
                AL_lt.AdvancedSearch.Save();
            }

            // Field AL_mt
            if (filter?.TryGetValue("x_AL_mt", out sv) ?? false) {
                AL_mt.AdvancedSearch.SearchValue = sv;
                AL_mt.AdvancedSearch.SearchOperator = filter["z_AL_mt"];
                AL_mt.AdvancedSearch.SearchCondition = filter["v_AL_mt"];
                AL_mt.AdvancedSearch.SearchValue2 = filter["y_AL_mt"];
                AL_mt.AdvancedSearch.SearchOperator2 = filter["w_AL_mt"];
                AL_mt.AdvancedSearch.Save();
            }

            // Field BD_klobs
            if (filter?.TryGetValue("x_BD_klobs", out sv) ?? false) {
                BD_klobs.AdvancedSearch.SearchValue = sv;
                BD_klobs.AdvancedSearch.SearchOperator = filter["z_BD_klobs"];
                BD_klobs.AdvancedSearch.SearchCondition = filter["v_BD_klobs"];
                BD_klobs.AdvancedSearch.SearchValue2 = filter["y_BD_klobs"];
                BD_klobs.AdvancedSearch.SearchOperator2 = filter["w_BD_klobs"];
                BD_klobs.AdvancedSearch.Save();
            }

            // Field BD_kl15
            if (filter?.TryGetValue("x_BD_kl15", out sv) ?? false) {
                BD_kl15.AdvancedSearch.SearchValue = sv;
                BD_kl15.AdvancedSearch.SearchOperator = filter["z_BD_kl15"];
                BD_kl15.AdvancedSearch.SearchCondition = filter["v_BD_kl15"];
                BD_kl15.AdvancedSearch.SearchValue2 = filter["y_BD_kl15"];
                BD_kl15.AdvancedSearch.SearchOperator2 = filter["w_BD_kl15"];
                BD_kl15.AdvancedSearch.Save();
            }

            // Field BD_barrels
            if (filter?.TryGetValue("x_BD_barrels", out sv) ?? false) {
                BD_barrels.AdvancedSearch.SearchValue = sv;
                BD_barrels.AdvancedSearch.SearchOperator = filter["z_BD_barrels"];
                BD_barrels.AdvancedSearch.SearchCondition = filter["v_BD_barrels"];
                BD_barrels.AdvancedSearch.SearchValue2 = filter["y_BD_barrels"];
                BD_barrels.AdvancedSearch.SearchOperator2 = filter["w_BD_barrels"];
                BD_barrels.AdvancedSearch.Save();
            }

            // Field BD_lt
            if (filter?.TryGetValue("x_BD_lt", out sv) ?? false) {
                BD_lt.AdvancedSearch.SearchValue = sv;
                BD_lt.AdvancedSearch.SearchOperator = filter["z_BD_lt"];
                BD_lt.AdvancedSearch.SearchCondition = filter["v_BD_lt"];
                BD_lt.AdvancedSearch.SearchValue2 = filter["y_BD_lt"];
                BD_lt.AdvancedSearch.SearchOperator2 = filter["w_BD_lt"];
                BD_lt.AdvancedSearch.Save();
            }

            // Field BD_mt
            if (filter?.TryGetValue("x_BD_mt", out sv) ?? false) {
                BD_mt.AdvancedSearch.SearchValue = sv;
                BD_mt.AdvancedSearch.SearchOperator = filter["z_BD_mt"];
                BD_mt.AdvancedSearch.SearchCondition = filter["v_BD_mt"];
                BD_mt.AdvancedSearch.SearchValue2 = filter["y_BD_mt"];
                BD_mt.AdvancedSearch.SearchOperator2 = filter["w_BD_mt"];
                BD_mt.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(BL_klobs);
            searchFlds.Add(BL_kl15);
            searchFlds.Add(BL_barrels);
            searchFlds.Add(BL_lt);
            searchFlds.Add(BL_mt);
            searchFlds.Add(AL_klobs);
            searchFlds.Add(AL_kl15);
            searchFlds.Add(AL_barrels);
            searchFlds.Add(AL_lt);
            searchFlds.Add(AL_mt);
            searchFlds.Add(BD_klobs);
            searchFlds.Add(BD_kl15);
            searchFlds.Add(BD_barrels);
            searchFlds.Add(BD_lt);
            searchFlds.Add(BD_mt);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(BL_klobs, ctrl); // BL_klobs
                UpdateSort(BL_kl15, ctrl); // BL_kl15
                UpdateSort(BL_barrels, ctrl); // BL_barrels
                UpdateSort(BL_lt, ctrl); // BL_lt
                UpdateSort(BL_mt, ctrl); // BL_mt
                UpdateSort(AL_klobs, ctrl); // AL_klobs
                UpdateSort(AL_kl15, ctrl); // AL_kl15
                UpdateSort(AL_barrels, ctrl); // AL_barrels
                UpdateSort(AL_lt, ctrl); // AL_lt
                UpdateSort(AL_mt, ctrl); // AL_mt
                UpdateSort(BD_klobs, ctrl); // BD_klobs
                UpdateSort(BD_kl15, ctrl); // BD_kl15
                UpdateSort(BD_barrels, ctrl); // BD_barrels
                UpdateSort(BD_lt, ctrl); // BD_lt
                UpdateSort(BD_mt, ctrl); // BD_mt
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubaktivitasNilaiBLALBDPipa"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubaktivitasNilaiBLALBDPipalist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubaktivitasNilaiBLALBDPipalist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("BL_klobs")); // DN
                CreateColumnOption(option.Add("BL_kl15")); // DN
                CreateColumnOption(option.Add("BL_barrels")); // DN
                CreateColumnOption(option.Add("BL_lt")); // DN
                CreateColumnOption(option.Add("BL_mt")); // DN
                CreateColumnOption(option.Add("AL_klobs")); // DN
                CreateColumnOption(option.Add("AL_kl15")); // DN
                CreateColumnOption(option.Add("AL_barrels")); // DN
                CreateColumnOption(option.Add("AL_lt")); // DN
                CreateColumnOption(option.Add("AL_mt")); // DN
                CreateColumnOption(option.Add("BD_klobs")); // DN
                CreateColumnOption(option.Add("BD_kl15")); // DN
                CreateColumnOption(option.Add("BD_barrels")); // DN
                CreateColumnOption(option.Add("BD_lt")); // DN
                CreateColumnOption(option.Add("BD_mt")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubaktivitasNilaiBLALBDPipasrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubaktivitasNilaiBLALBDPipasrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubaktivitasNilaiBLALBDPipalist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubaktivitasNilaiBLALBDPipa");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subaktivitasNilaiBlalbdPipaList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subaktivitasNilaiBlalbdPipaList.RowCount) + "_SubaktivitasNilaiBLALBDPipa");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subaktivitasNilaiBlalbdPipaList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subaktivitasNilaiBlalbdPipaList.RowType == RowType.Add || IsEdit && subaktivitasNilaiBlalbdPipaList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // BL_klobs
                BL_klobs.HrefValue = "";
                BL_klobs.TooltipValue = "";

                // BL_kl15
                BL_kl15.HrefValue = "";
                BL_kl15.TooltipValue = "";
                ...
                // AL_klobs
                AL_klobs.HrefValue = "";
                AL_klobs.TooltipValue = "";

                // AL_kl15
                AL_kl15.HrefValue = "";
                AL_kl15.TooltipValue = "";
                ...
                // BD_klobs
                BD_klobs.HrefValue = "";
                BD_klobs.TooltipValue = "";

                // BD_kl15
                BD_kl15.HrefValue = "";
                BD_kl15.TooltipValue = "";

                // BD_barrels
                BD_barrels.HrefValue = "";
                BD_barrels.TooltipValue = "";
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fSubaktivitasNilaiBLALBDPipalist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fSubaktivitasNilaiBLALBDPipalist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fSubaktivitasNilaiBLALBDPipalist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubaktivitasNilaiBLALBDPipasrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
