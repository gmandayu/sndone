...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasLaporanStockStscsbbmEdit
    /// </summary>
    public static SubAktivitasLaporanStockStscsbbmEdit subAktivitasLaporanStockStscsbbmEdit
    {
        get => HttpData.Get<SubAktivitasLaporanStockStscsbbmEdit>("subAktivitasLaporanStockStscsbbmEdit")!;
        set => HttpData["subAktivitasLaporanStockStscsbbmEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasLaporanStockSTSCSBBM
    /// </summary>
    public class SubAktivitasLaporanStockStscsbbmEdit : SubAktivitasLaporanStockStscsbbmEditBase
    {
        // Constructor
        public SubAktivitasLaporanStockStscsbbmEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasLaporanStockStscsbbmEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasLaporanStockStscsbbmEditBase : SubAktivitasLaporanStockStscsbbm
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasLaporanStockStscsbbmEdit";
        ...
        // Constructor
        public SubAktivitasLaporanStockStscsbbmEditBase()
        {
            TableName = "SubAktivitasLaporanStockSTSCSBBM";
            ...
            // Table object (subAktivitasLaporanStockStscsbbm)
            if (subAktivitasLaporanStockStscsbbm == null || subAktivitasLaporanStockStscsbbm is SubAktivitasLaporanStockStscsbbm)
                subAktivitasLaporanStockStscsbbm = this;
        ...
        // Page name
        public string PageName => "SubAktivitasLaporanStockStscsbbmEdit";
        ...
        public void SetVisibility()
        {
            ...
            StockAwal.SetVisibility();
            Penerimaan2.SetVisibility();
            Penyaluran2.SetVisibility();
            StockAkhir.SetVisibility();
            WorkingLoss.SetVisibility();
            Presentase.SetVisibility();
            userInput.Visible = false;
            etlDate.Visible = false;
            Tangki.Visible = false;
            Produk.Visible = false;
        }

        // Constructor
        public SubAktivitasLaporanStockStscsbbmEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (SameString(pageName, GetPageName(ListUrl)) ||
                            ...
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasLaporanStockStscsbbmView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasLaporanStockStscsbbmList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = ReturnUrl;
                    if (GetPageName(returnUrl) == "SubAktivitasLaporanStockStscsbbmList")
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasLaporanStockStscsbbmList") {
                                ...
                                returnUrl = "SubAktivitasLaporanStockStscsbbmList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasLaporanStockStscsbbmEdit?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'StockAwal' before field var 'x_StockAwal'
            val = CurrentForm.HasValue("StockAwal") ? CurrentForm.GetValue("StockAwal") : CurrentForm.GetValue("x_StockAwal");
            if (!StockAwal.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("StockAwal") && !CurrentForm.HasValue("x_StockAwal")) // DN
                    StockAwal.Visible = false; // Disable update for API request
                else
                    StockAwal.SetFormValue(val);
            }

            // Check field name 'Penerimaan' before field var 'x_Penerimaan2'
            val = CurrentForm.HasValue("Penerimaan") ? CurrentForm.GetValue("Penerimaan") : CurrentForm.GetValue("x_Penerimaan2");
            if (!Penerimaan2.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Penerimaan") && !CurrentForm.HasValue("x_Penerimaan2")) // DN
                    Penerimaan2.Visible = false; // Disable update for API request
                else
                    Penerimaan2.SetFormValue(val);
            }

            // Check field name 'Penyaluran' before field var 'x_Penyaluran2'
            val = CurrentForm.HasValue("Penyaluran") ? CurrentForm.GetValue("Penyaluran") : CurrentForm.GetValue("x_Penyaluran2");
            if (!Penyaluran2.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Penyaluran") && !CurrentForm.HasValue("x_Penyaluran2")) // DN
                    Penyaluran2.Visible = false; // Disable update for API request
                else
                    Penyaluran2.SetFormValue(val);
            }

            // Check field name 'StockAkhir' before field var 'x_StockAkhir'
            val = CurrentForm.HasValue("StockAkhir") ? CurrentForm.GetValue("StockAkhir") : CurrentForm.GetValue("x_StockAkhir");
            if (!StockAkhir.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("StockAkhir") && !CurrentForm.HasValue("x_StockAkhir")) // DN
                    StockAkhir.Visible = false; // Disable update for API request
                else
                    StockAkhir.SetFormValue(val);
            }

            // Check field name 'WorkingLoss' before field var 'x_WorkingLoss'
            val = CurrentForm.HasValue("WorkingLoss") ? CurrentForm.GetValue("WorkingLoss") : CurrentForm.GetValue("x_WorkingLoss");
            if (!WorkingLoss.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("WorkingLoss") && !CurrentForm.HasValue("x_WorkingLoss")) // DN
                    WorkingLoss.Visible = false; // Disable update for API request
                else
                    WorkingLoss.SetFormValue(val);
            }

            // Check field name 'Presentase' before field var 'x_Presentase'
            val = CurrentForm.HasValue("Presentase") ? CurrentForm.GetValue("Presentase") : CurrentForm.GetValue("x_Presentase");
            if (!Presentase.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Presentase") && !CurrentForm.HasValue("x_Presentase")) // DN
                    Presentase.Visible = false; // Disable update for API request
                else
                    Presentase.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            lastUpdatedDate.CurrentValue = UnformatDateTime(lastUpdatedDate.CurrentValue, lastUpdatedDate.FormatPattern);
            StockAwal.CurrentValue = StockAwal.FormValue;
            Penerimaan2.CurrentValue = Penerimaan2.FormValue;
            Penyaluran2.CurrentValue = Penyaluran2.FormValue;
            StockAkhir.CurrentValue = StockAkhir.FormValue;
            WorkingLoss.CurrentValue = WorkingLoss.FormValue;
            Presentase.CurrentValue = Presentase.FormValue;
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("StockAwal", StockAwal.DefaultValue ?? DbNullValue); // DN
            row.Add("Penerimaan", Penerimaan2.DefaultValue ?? DbNullValue); // DN
            row.Add("Penyaluran", Penyaluran2.DefaultValue ?? DbNullValue); // DN
            row.Add("StockAkhir", StockAkhir.DefaultValue ?? DbNullValue); // DN
            row.Add("WorkingLoss", WorkingLoss.DefaultValue ?? DbNullValue); // DN
            row.Add("Presentase", Presentase.DefaultValue ?? DbNullValue); // DN
            row.Add("userInput", userInput.DefaultValue ?? DbNullValue); // DN
            row.Add("etlDate", etlDate.DefaultValue ?? DbNullValue); // DN
            row.Add("Tangki", Tangki.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // StockAwal
            StockAwal.RowCssClass = "row";

            // Penerimaan
            Penerimaan2.RowCssClass = "row";

            // Penyaluran
            Penyaluran2.RowCssClass = "row";

            // StockAkhir
            StockAkhir.RowCssClass = "row";

            // WorkingLoss
            WorkingLoss.RowCssClass = "row";

            // Presentase
            Presentase.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                StockAwal.EditValue = HtmlEncode(StockAwal.CurrentValue);
                ...
                Penerimaan2.EditValue = HtmlEncode(Penerimaan2.CurrentValue);
                ...
                Penyaluran2.EditValue = HtmlEncode(Penyaluran2.CurrentValue);
                ...
                StockAkhir.EditValue = HtmlEncode(StockAkhir.CurrentValue);
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                if (StockAwal.Visible && StockAwal.Required) {
                    if (!StockAwal.IsDetailKey && Empty(StockAwal.FormValue)) {
                        StockAwal.AddErrorMessage(ConvertToString(StockAwal.RequiredErrorMessage).Replace("%s", StockAwal.Caption));
                    }
                }
                if (Penerimaan2.Visible && Penerimaan2.Required) {
                    if (!Penerimaan2.IsDetailKey && Empty(Penerimaan2.FormValue)) {
                        Penerimaan2.AddErrorMessage(ConvertToString(Penerimaan2.RequiredErrorMessage).Replace("%s", Penerimaan2.Caption));
                    }
                }
                if (Penyaluran2.Visible && Penyaluran2.Required) {
                    if (!Penyaluran2.IsDetailKey && Empty(Penyaluran2.FormValue)) {
                        Penyaluran2.AddErrorMessage(ConvertToString(Penyaluran2.RequiredErrorMessage).Replace("%s", Penyaluran2.Caption));
                    }
                }
                if (StockAkhir.Visible && StockAkhir.Required) {
                    if (!StockAkhir.IsDetailKey && Empty(StockAkhir.FormValue)) {
                        StockAkhir.AddErrorMessage(ConvertToString(StockAkhir.RequiredErrorMessage).Replace("%s", StockAkhir.Caption));
                    }
                }
                if (WorkingLoss.Visible && WorkingLoss.Required) {
                    if (!WorkingLoss.IsDetailKey && Empty(WorkingLoss.FormValue)) {
                        WorkingLoss.AddErrorMessage(ConvertToString(WorkingLoss.RequiredErrorMessage).Replace("%s", WorkingLoss.Caption));
                    }
                }
                if (Presentase.Visible && Presentase.Required) {
                    if (!Presentase.IsDetailKey && Empty(Presentase.FormValue)) {
                        Presentase.AddErrorMessage(ConvertToString(Presentase.RequiredErrorMessage).Replace("%s", Presentase.Caption));
                    }
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // StockAwal
            StockAwal.SetDbValue(rsnew, StockAwal.CurrentValue, StockAwal.ReadOnly);

            // Penerimaan
            Penerimaan2.SetDbValue(rsnew, Penerimaan2.CurrentValue, Penerimaan2.ReadOnly);

            // Penyaluran
            Penyaluran2.SetDbValue(rsnew, Penyaluran2.CurrentValue, Penyaluran2.ReadOnly);

            // StockAkhir
            StockAkhir.SetDbValue(rsnew, StockAkhir.CurrentValue, StockAkhir.ReadOnly);
            return rsnew;
        }
        ...
        {
            object? value;
            if (row.TryGetValue("StockAwal", out value)) // StockAwal
                StockAwal.CurrentValue = value;
            if (row.TryGetValue("Penerimaan", out value)) // Penerimaan
                Penerimaan2.CurrentValue = value;
            if (row.TryGetValue("Penyaluran", out value)) // Penyaluran
                Penyaluran2.CurrentValue = value;
            if (row.TryGetValue("StockAkhir", out value)) // StockAkhir
                StockAkhir.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasLaporanStockStscsbbmList")), "", TableVar, true);
            string pageId = "edit";
