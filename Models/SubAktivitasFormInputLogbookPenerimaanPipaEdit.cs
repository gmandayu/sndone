...
public partial class SnDOne {
    ...
    public class SubAktivitasFormInputLogbookPenerimaanPipaEditBase : SubAktivitasFormInputLogbookPenerimaanPipa
    {
        ...
        // Constructor
        public SubAktivitasFormInputLogbookPenerimaanPipaEditBase()
        {
        ...
        // Constructor
        public SubAktivitasFormInputLogbookPenerimaanPipaEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasFormInputLogbookPenerimaanPipaList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasFormInputLogbookPenerimaanPipaList") {
                                ...
                                returnUrl = "SubAktivitasFormInputLogbookPenerimaanPipaList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NamaBatch' before field var 'x_NamaBatch'
            val = CurrentForm.HasValue("NamaBatch") ? CurrentForm.GetValue("NamaBatch") : CurrentForm.GetValue("x_NamaBatch");
            if (!NamaBatch.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NamaBatch") && !CurrentForm.HasValue("x_NamaBatch")) // DN
                    NamaBatch.Visible = false; // Disable update for API request
                else
                    NamaBatch.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (NamaBatch.Visible && NamaBatch.Required) {
                    if (!NamaBatch.IsDetailKey && Empty(NamaBatch.FormValue)) {
                        NamaBatch.AddErrorMessage(ConvertToString(NamaBatch.RequiredErrorMessage).Replace("%s", NamaBatch.Caption));
                    }
                }
