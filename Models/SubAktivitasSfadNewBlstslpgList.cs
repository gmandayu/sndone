...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasSfadNewBlstslpgList
    /// </summary>
    public static SubAktivitasSfadNewBlstslpgList subAktivitasSfadNewBlstslpgList
    {
        get => HttpData.Get<SubAktivitasSfadNewBlstslpgList>("subAktivitasSfadNewBlstslpgList")!;
        set => HttpData["subAktivitasSfadNewBlstslpgList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasSFADNewBLSTSLPG
    /// </summary>
    public class SubAktivitasSfadNewBlstslpgList : SubAktivitasSfadNewBlstslpgListBase
    {
        // Constructor
        public SubAktivitasSfadNewBlstslpgList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasSfadNewBlstslpgList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasSfadNewBlstslpgListBase : SubAktivitasSfadNewBlstslpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasSfadNewBlstslpgList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasSFADNewBLSTSLPGlist";
        ...
        // Constructor
        public SubAktivitasSfadNewBlstslpgListBase(Controller? controller)
        {
            TableName = "SubAktivitasSFADNewBLSTSLPG";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasSfadNewBlstslpg)
            if (subAktivitasSfadNewBlstslpg == null || subAktivitasSfadNewBlstslpg is SubAktivitasSfadNewBlstslpg)
                subAktivitasSfadNewBlstslpg = this;

            // Initialize URLs
            AddUrl = "SubAktivitasSfadNewBlstslpgAdd";
            ...
            MultiDeleteUrl = "SubAktivitasSfadNewBlstslpgDelete";
            MultiUpdateUrl = "SubAktivitasSfadNewBlstslpgUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasSfadNewBlstslpgList";
        ...
        // Constructor
        public SubAktivitasSfadNewBlstslpgListBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                var result = new Dictionary<string, string> { { "url", GetUrl(url) }, { "modal", "1" } }; // modal=1
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasSfadNewBlstslpgView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void PrepareInitialSettings()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasSFADNewBLSTSLPGgrid";
        ...
        private void PageRunEnd()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasSfadNewBlstslpgList?.PageRender();
            }
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasSFADNewBLSTSLPG"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLSTSLPGlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLSTSLPGlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasSFADNewBLSTSLPGsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasSFADNewBLSTSLPGsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLSTSLPGlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_SubAktivitasSFADNewBLSTSLPG");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasSfadNewBlstslpgList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasSfadNewBlstslpgList.RowCount) + "_SubAktivitasSFADNewBLSTSLPG");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasSfadNewBlstslpgList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasSfadNewBlstslpgList.RowType == RowType.Add || IsEdit && subAktivitasSfadNewBlstslpgList.RowType == RowType.Edit) // Inline-Add/Edit row
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasSFADNewBLSTSLPGsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
