...
public partial class SnDOne {
        ...
        where T : DbField
    {
        ...
        public List<string> FilterFieldVars = [];

        public List<string> AutoFillSourceFields = [];

        public List<string> AutoFillTargetFields = [];
        ...
        /// <param name="isAutoFillTargetField">Is autoFill target field</param>
        ...
        public Lookup(
            ...
            bool isAutoFillTargetField = false,
            string orderBy = "",
            string template = "",
        ...
        )
        {
            ...
            IsAutoFillTargetField = isAutoFillTargetField;
            UserOrderBy = orderBy;
            Template = template;
        ...
        public string GetSql(bool useParentFilter = true, string currentFilter = "", Func<string>? lookupFilter = null, dynamic? page = null, bool skipFilterFields = false, bool clearUserFilter = false)
        {
            ...
            if (clearUserFilter)
                UserFilter = "";
            string filter = GetWhere(useParentFilter);
            string newFilter = filter;
            if (page != null)
                if (page.Fields.TryGetValue(Field?.Name ?? "", out T fld))
                    Invoke(page, "LookupSelecting", (object[])[(DbField)fld, newFilter]); // Call Lookup Selecting
            ...
            return GetSqlPart("", true, useParentFilter, skipFilterFields);
        }
        ...
        {
            string[] keys = ["lf", "df", "df2", "df3", "df4", "ff", "ff2", "ff3", "ff4"];
        ...
        /// <param name="options">
        /// Options, e.g. [
        /// 	["1", "option1", "df1", "df2", "df3"],
        /// 	["2", "option2", "df1", "df2"],
        /// 	["3", "option3", "df1", "df3", "df3a"],
        /// ]
        /// </param>
        /// <returns>Whether the options are setup successfully</returns>
        ...
        public virtual async Task<Dictionary<string, object>> ToJson(dynamic? page = null)
        {
            ...
            bool isReport = page.TableReportType == "dashboard"
                ? (tbl.Type == "REPORT")
                : page is ReportTable report && ((List<string>)[report.ReportSourceTable, report.TableVar]).Contains(tbl.TableVar);
            ...
            try {
                rs = await GetRecordset(sql, orderBy, pageSize, offset);
                if (!IsList(rs))
                    return emptyResult;
            } catch (Exception e) {
                if (Config.Debug)
                    LogError(e.Message, orderBy, pageSize, offset);
                return emptyResult;
            }
        ...
        public dynamic RenderViewRow(dynamic row, dynamic? renderer = null)
        {
            ...
            if (renderer == null || GetPropertyValue(renderer, "PageID") == "dashboard") // DN
            ...
            // Handle List
                ...
                for (int i = 1; i < row.Count; i++) {
                    ...
                    if (!Empty(name) && renderer.Fields.ContainsKey(name)) {
                        ...
                        if (mi != null && !Empty(field.ViewValue) && !(sameTable && SameString(name, Field?.Name) && !((List<DataType>)[DataType.Date, DataType.Time]).Contains(field.DataType) && field.OptionCount == 0))
        ...
        protected string GetSqlPart(string part = "", bool isUser = true, bool useParentFilter = true, bool skipFilterFields = false)
        {
            ...
            // Set up WHERE
            string where = ConvertToString(tbl.Invoke("ApplyUserIDFilters", (object[])["", "lookup"]));

            // Set up current filter
            int cnt = FilterValues.Count;
            if (cnt > 0 && !(SameText(LookupType, "updateoption") && IsAutoFillTargetField)) { // Load all records if IsAutoFillTargetField
                ...
                if (useParentFilter && !(isUser && Regex.IsMatch(UserFilter, @"\{v(\d)\}", RegexOptions.IgnoreCase))) { // UserFilter does not contain ({v<n>})
                    int i = 1;
                    foreach (var (key, value) in FilterFields) {
                        if (!Empty(key)) {
                            var filterOpr = value;
            ...
            if (!Empty(SearchValue)) {
                ...
                if (SameText(LookupType, "autosuggest") && !LookupAllDisplayFields)
                    AddFilter(ref where, GetAutoSuggestFilter(SearchValue, tbl.DbId));
            ...
            if (!Empty(UserFilter) && isUser)
                AddFilter(ref where, GetUserFilter());
        ...
        protected string GetUserFilter()
        {
            ...
            string dbid = GetTable()?.DbId ?? "DB";
            foreach (Match match in Regex.Matches(filter, @"\{v(\d)\}", RegexOptions.IgnoreCase)) {
                int index = ConvertToInt(match.Groups[1].Value);
                string value = FilterValues[index] ?? "";
                filter = filter.Replace(match.Groups[0].Value, AdjustSql(value, dbid));
            }
            return filter;
        }
        ...
        protected string GetFilter(T fld, string opr, string val, string dbid)
        {
            ...
            string[] values = Field?.IsMultiSelect ?? false ? val.Split(Config.MultipleOptionSeparator) : [val];
        ...
        protected string GetWhere(bool useParentFilter = false) =>
        ...
        protected async Task<List<Dictionary<string, object>>?> GetRecordset(string sql, string orderBy, int pageSize, int offset)
        {
            ...
            if (pageSize > 0) {
                ...
                return SameString(dbType, "MSSQL")
                    ? await conn.GetRowsAsync(conn.SelectLimitSql(sql, pageSize, offset, !Empty(orderBy)))
                    : await conn.GetRowsAsync(conn.SelectLimitSql(sql, pageSize, offset));
            }
            return await conn.GetRowsAsync(sql);
        }
        ...
        protected string GetAutoSuggestFilter(string sv, string dbid) => Table != null
            ? SearchExpression + Like(Wildcard(sv, "STARTS WITH", dbid), dbid)
            : "";
        ...
        protected string GetModalSearchFilter(string sv, string dbid)
        {
            ...
            foreach (string keyword in keywords) {
                if (!Empty(keyword)) {
                    string thisFilter = SearchExpression + Like(Wildcard(keyword, ModalLookupSearchOperator, dbid), dbid);
