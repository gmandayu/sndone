...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiSfbdrtwAdd
    /// </summary>
    public static SubAktivitasNilaiSfbdrtwAdd subAktivitasNilaiSfbdrtwAdd
    {
        get => HttpData.Get<SubAktivitasNilaiSfbdrtwAdd>("subAktivitasNilaiSfbdrtwAdd")!;
        set => HttpData["subAktivitasNilaiSfbdrtwAdd"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiSFBDRTW
    /// </summary>
    public class SubAktivitasNilaiSfbdrtwAdd : SubAktivitasNilaiSfbdrtwAddBase
    {
        // Constructor
        public SubAktivitasNilaiSfbdrtwAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiSfbdrtwAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiSfbdrtwAddBase : SubAktivitasNilaiSfbdrtw
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiSfbdrtwAdd";
        ...
        // Constructor
        public SubAktivitasNilaiSfbdrtwAddBase()
        {
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiSfbdrtwAdd";
        ...
        // Constructor
        public SubAktivitasNilaiSfbdrtwAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("SubAktivitasNilaiSfbdrtwList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        string returnUrl = "";
                        returnUrl = ReturnUrl;
                        if (GetPageName(returnUrl) == "SubAktivitasNilaiSfbdrtwList")
                            returnUrl = AddMasterUrl(ListUrl); // List page, return to List page with correct master key if necessary
                        else if (GetPageName(returnUrl) == "SubAktivitasNilaiSfbdrtwView")
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasNilaiSfbdrtwList") {
                                ...
                                returnUrl = "SubAktivitasNilaiSfbdrtwList"; // Return list page content
                            }
                        }
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasNilaiSfbdrtwAdd?.PageRender();
            }
            return PageResult();
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'SFBD_KL_Obs' before field var 'x_SFBD_KL_Obs'
            val = CurrentForm.HasValue("SFBD_KL_Obs") ? CurrentForm.GetValue("SFBD_KL_Obs") : CurrentForm.GetValue("x_SFBD_KL_Obs");
            if (!SFBD_KL_Obs.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("SFBD_KL_Obs") && !CurrentForm.HasValue("x_SFBD_KL_Obs")) // DN
                    SFBD_KL_Obs.Visible = false; // Disable update for API request
                else
                    SFBD_KL_Obs.SetFormValue(val);
            }

            // Check field name 'SFBD_KL_15' before field var 'x_SFBD_KL_15'
            val = CurrentForm.HasValue("SFBD_KL_15") ? CurrentForm.GetValue("SFBD_KL_15") : CurrentForm.GetValue("x_SFBD_KL_15");
            if (!SFBD_KL_15.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("SFBD_KL_15") && !CurrentForm.HasValue("x_SFBD_KL_15")) // DN
                    SFBD_KL_15.Visible = false; // Disable update for API request
                else
                    SFBD_KL_15.SetFormValue(val);
            }

            // Check field name 'SFBD_Barrels' before field var 'x_SFBD_Barrels'
            val = CurrentForm.HasValue("SFBD_Barrels") ? CurrentForm.GetValue("SFBD_Barrels") : CurrentForm.GetValue("x_SFBD_Barrels");
            if (!SFBD_Barrels.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("SFBD_Barrels") && !CurrentForm.HasValue("x_SFBD_Barrels")) // DN
                    SFBD_Barrels.Visible = false; // Disable update for API request
                else
                    SFBD_Barrels.SetFormValue(val);
            }

            // Check field name 'SFBD_LT' before field var 'x_SFBD_LT'
            val = CurrentForm.HasValue("SFBD_LT") ? CurrentForm.GetValue("SFBD_LT") : CurrentForm.GetValue("x_SFBD_LT");
            if (!SFBD_LT.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("SFBD_LT") && !CurrentForm.HasValue("x_SFBD_LT")) // DN
                    SFBD_LT.Visible = false; // Disable update for API request
                else
                    SFBD_LT.SetFormValue(val);
            }

            // Check field name 'SFBD_MT' before field var 'x_SFBD_MT'
            val = CurrentForm.HasValue("SFBD_MT") ? CurrentForm.GetValue("SFBD_MT") : CurrentForm.GetValue("x_SFBD_MT");
            if (!SFBD_MT.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("SFBD_MT") && !CurrentForm.HasValue("x_SFBD_MT")) // DN
                    SFBD_MT.Visible = false; // Disable update for API request
                else
                    SFBD_MT.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            SFBD_KL_Obs.CurrentValue = SFBD_KL_Obs.FormValue;
            SFBD_KL_15.CurrentValue = SFBD_KL_15.FormValue;
            SFBD_Barrels.CurrentValue = SFBD_Barrels.FormValue;
            SFBD_LT.CurrentValue = SFBD_LT.FormValue;
            SFBD_MT.CurrentValue = SFBD_MT.FormValue;
        ...
        public async Task RenderRow()
        {
            ...
            // SFBD_KL_Obs
            SFBD_KL_Obs.RowCssClass = "row";

            // SFBD_KL_15
            SFBD_KL_15.RowCssClass = "row";

            // SFBD_Barrels
            SFBD_Barrels.RowCssClass = "row";

            // SFBD_LT
            SFBD_LT.RowCssClass = "row";

            // SFBD_MT
            SFBD_MT.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                // SFBD_KL_Obs
                SFBD_KL_Obs.SetupEditAttributes();
                if (!SFBD_KL_Obs.Raw)
                    SFBD_KL_Obs.CurrentValue = HtmlDecode(SFBD_KL_Obs.CurrentValue);
                SFBD_KL_Obs.EditValue = HtmlEncode(SFBD_KL_Obs.CurrentValue);
                SFBD_KL_Obs.PlaceHolder = RemoveHtml(SFBD_KL_Obs.Caption);

                // SFBD_KL_15
                SFBD_KL_15.SetupEditAttributes();
                if (!SFBD_KL_15.Raw)
                    SFBD_KL_15.CurrentValue = HtmlDecode(SFBD_KL_15.CurrentValue);
                SFBD_KL_15.EditValue = HtmlEncode(SFBD_KL_15.CurrentValue);
                SFBD_KL_15.PlaceHolder = RemoveHtml(SFBD_KL_15.Caption);

                // SFBD_Barrels
                SFBD_Barrels.SetupEditAttributes();
                if (!SFBD_Barrels.Raw)
                    SFBD_Barrels.CurrentValue = HtmlDecode(SFBD_Barrels.CurrentValue);
                SFBD_Barrels.EditValue = HtmlEncode(SFBD_Barrels.CurrentValue);
                SFBD_Barrels.PlaceHolder = RemoveHtml(SFBD_Barrels.Caption);

                // SFBD_LT
                SFBD_LT.SetupEditAttributes();
                if (!SFBD_LT.Raw)
                    SFBD_LT.CurrentValue = HtmlDecode(SFBD_LT.CurrentValue);
                SFBD_LT.EditValue = HtmlEncode(SFBD_LT.CurrentValue);
                SFBD_LT.PlaceHolder = RemoveHtml(SFBD_LT.Caption);

                // SFBD_MT
                SFBD_MT.SetupEditAttributes();
                if (!SFBD_MT.Raw)
                    SFBD_MT.CurrentValue = HtmlDecode(SFBD_MT.CurrentValue);
                SFBD_MT.EditValue = HtmlEncode(SFBD_MT.CurrentValue);
                SFBD_MT.PlaceHolder = RemoveHtml(SFBD_MT.Caption);
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                if (SFBD_KL_Obs.Visible && SFBD_KL_Obs.Required) {
                    if (!SFBD_KL_Obs.IsDetailKey && Empty(SFBD_KL_Obs.FormValue)) {
                        SFBD_KL_Obs.AddErrorMessage(ConvertToString(SFBD_KL_Obs.RequiredErrorMessage).Replace("%s", SFBD_KL_Obs.Caption));
                    }
                }
                if (SFBD_KL_15.Visible && SFBD_KL_15.Required) {
                    if (!SFBD_KL_15.IsDetailKey && Empty(SFBD_KL_15.FormValue)) {
                        SFBD_KL_15.AddErrorMessage(ConvertToString(SFBD_KL_15.RequiredErrorMessage).Replace("%s", SFBD_KL_15.Caption));
                    }
                }
                if (SFBD_Barrels.Visible && SFBD_Barrels.Required) {
                    if (!SFBD_Barrels.IsDetailKey && Empty(SFBD_Barrels.FormValue)) {
                        SFBD_Barrels.AddErrorMessage(ConvertToString(SFBD_Barrels.RequiredErrorMessage).Replace("%s", SFBD_Barrels.Caption));
                    }
                }
                if (SFBD_LT.Visible && SFBD_LT.Required) {
                    if (!SFBD_LT.IsDetailKey && Empty(SFBD_LT.FormValue)) {
                        SFBD_LT.AddErrorMessage(ConvertToString(SFBD_LT.RequiredErrorMessage).Replace("%s", SFBD_LT.Caption));
                    }
                }
                if (SFBD_MT.Visible && SFBD_MT.Required) {
                    if (!SFBD_MT.IsDetailKey && Empty(SFBD_MT.FormValue)) {
                        SFBD_MT.AddErrorMessage(ConvertToString(SFBD_MT.RequiredErrorMessage).Replace("%s", SFBD_MT.Caption));
                    }
                }
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // SFBD_KL_Obs
                SFBD_KL_Obs.SetDbValue(rsnew, SFBD_KL_Obs.CurrentValue);

                // SFBD_KL_15
                SFBD_KL_15.SetDbValue(rsnew, SFBD_KL_15.CurrentValue);

                // SFBD_Barrels
                SFBD_Barrels.SetDbValue(rsnew, SFBD_Barrels.CurrentValue);

                // SFBD_LT
                SFBD_LT.SetDbValue(rsnew, SFBD_LT.CurrentValue);

                // SFBD_MT
                SFBD_MT.SetDbValue(rsnew, SFBD_MT.CurrentValue);
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("SFBD_KL_Obs", out value)) // SFBD_KL_Obs
                SFBD_KL_Obs.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("SFBD_KL_15", out value)) // SFBD_KL_15
                SFBD_KL_15.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("SFBD_Barrels", out value)) // SFBD_Barrels
                SFBD_Barrels.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("SFBD_LT", out value)) // SFBD_LT
                SFBD_LT.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("SFBD_MT", out value)) // SFBD_MT
                SFBD_MT.SetFormValue(ConvertToString(value));
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasNilaiSfbdrtwList")), "", TableVar, true);
