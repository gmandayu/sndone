...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasLogbookPenerimaanPenyaluranEdit
    /// </summary>
    public static SubAktivitasLogbookPenerimaanPenyaluranEdit subAktivitasLogbookPenerimaanPenyaluranEdit
    {
        get => HttpData.Get<SubAktivitasLogbookPenerimaanPenyaluranEdit>("subAktivitasLogbookPenerimaanPenyaluranEdit")!;
        set => HttpData["subAktivitasLogbookPenerimaanPenyaluranEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasLogbookPenerimaanPenyaluran
    /// </summary>
    public class SubAktivitasLogbookPenerimaanPenyaluranEdit : SubAktivitasLogbookPenerimaanPenyaluranEditBase
    {
        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasLogbookPenerimaanPenyaluranEditBase : SubAktivitasLogbookPenerimaanPenyaluran
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasLogbookPenerimaanPenyaluranEdit";
        ...
        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranEditBase(Controller? controller)
        {
            TableName = "SubAktivitasLogbookPenerimaanPenyaluran";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasLogbookPenerimaanPenyaluran)
            if (subAktivitasLogbookPenerimaanPenyaluran == null || subAktivitasLogbookPenerimaanPenyaluran is SubAktivitasLogbookPenerimaanPenyaluran)
                subAktivitasLogbookPenerimaanPenyaluran = this;
        ...
        // Page name
        public string PageName => "SubAktivitasLogbookPenerimaanPenyaluranEdit";
        ...
        public void SetVisibility()
        {
            ...
            NomorKompartment.SetVisibility();
            InLineBlending.SetVisibility();
            NomorTangki2.SetVisibility();
            NoMeter.SetVisibility();
        }

        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranEditBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                ...
                // List / View / Master View page
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasLogbookPenerimaanPenyaluranView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasLogbookPenerimaanPenyaluranList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasLogbookPenerimaanPenyaluranList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasLogbookPenerimaanPenyaluranList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private async Task<IActionResult?> PageRunBeginAsync()
        {
            ...
            // If null, the conditions aren't fulfilled
                    ...
                    await SetupLookupOptions(InLineBlending);
            return null;
        }
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasLogbookPenerimaanPenyaluranEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'LO'
            LO.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(LO, "LO", "x_LO");
            ...
            SetNormalField(Volume, "Volume", "x_Volume");

            // Standard handling for 'BayNumber'
            BayNumber.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(BayNumber, "BayNumber", "x_BayNumber");

            // Standard handling for 'NomorKompartment'
            NomorKompartment.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NomorKompartment, "NomorKompartment", "x_NomorKompartment");

            // Standard handling for 'InLineBlending'
            InLineBlending.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(InLineBlending, "InLineBlending", "x_InLineBlending");

            // Standard handling for 'NomorTangki2'
            NomorTangki2.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NomorTangki2, "NomorTangki2", "x_NomorTangki2");
        ...
        public void RestoreFormValues()
        {
            ...
            BayNumber.CurrentValue = BayNumber.FormValue;
            NomorKompartment.CurrentValue = NomorKompartment.FormValue;
            InLineBlending.CurrentValue = InLineBlending.FormValue;
            NomorTangki2.CurrentValue = NomorTangki2.FormValue;
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            NomorKompartment.SetDbValue(row["NomorKompartment"]);
            InLineBlending.SetDbValue((ConvertToBool(row["InLineBlending"]) ? "1" : "0"));
            NomorTangki2.SetDbValue(row["NomorTangki2"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("NomorKompartment", NomorKompartment.DefaultValue ?? DbNullValue); // DN
            row.Add("InLineBlending", InLineBlending.DefaultValue ?? DbNullValue); // DN
            row.Add("NomorTangki2", NomorTangki2.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // NomorKompartment
            NomorKompartment.RowCssClass = "row";

            // InLineBlending
            InLineBlending.RowCssClass = "row";

            // NomorTangki2
            NomorTangki2.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                // NoMeter
                    ...
                    // awallookupbung
                    // idProses (jaga leading zero)
                    ResolveLookupView(idProses, "IdProses", "string");
                    // akhirlookupbung
                    ...
                    // awallookupbung
                    // idAktifitas (jaga leading zero)
                    ResolveLookupView(idAktifitas, "IdAktivitas", "string");
                    // akhirlookupbung
                    ...
                    // NomorKompartment
                    NomorKompartment.ViewValue = ConvertToString(NomorKompartment.CurrentValue); // DN
                    NomorKompartment.ViewCustomAttributes = "";

                    // InLineBlending
                    if (ConvertToBool(InLineBlending.CurrentValue)) {
                        InLineBlending.ViewValue = InLineBlending.TagCaption(1) != "" ? InLineBlending.TagCaption(1) : "Yes";
                    } else {
                        InLineBlending.ViewValue = InLineBlending.TagCaption(2) != "" ? InLineBlending.TagCaption(2) : "No";
                    }
                    InLineBlending.ViewCustomAttributes = "";

                    // NomorTangki2
                    NomorTangki2.ViewValue = ConvertToString(NomorTangki2.CurrentValue); // DN
                    NomorTangki2.ViewCustomAttributes = "";
                ...
                // NomorKompartment
                NomorKompartment.HrefValue = "";

                // InLineBlending
                InLineBlending.HrefValue = "";

                // NomorTangki2
                NomorTangki2.HrefValue = "";
                ...
                idProses.EditValue = Empty(curVal) ? DbNullValue : idProses.CurrentValue;
                if (!Empty(curVal)) {
                ...
                idAktifitas.EditValue = Empty(curVal2) ? DbNullValue : idAktifitas.CurrentValue;
                if (!Empty(curVal2)) {
                ...
                LO.EditValue = HtmlEncode(LO.CurrentValue);
                ...
                // BayNumber
                BayNumber.SetupEditAttributes();
                if (!BayNumber.Raw)
                    BayNumber.CurrentValue = HtmlDecode(BayNumber.CurrentValue);
                BayNumber.EditValue = HtmlEncode(BayNumber.CurrentValue);
                BayNumber.PlaceHolder = RemoveHtml(BayNumber.Caption);

                // NomorKompartment
                NomorKompartment.SetupEditAttributes();
                if (!NomorKompartment.Raw)
                    NomorKompartment.CurrentValue = HtmlDecode(NomorKompartment.CurrentValue);
                NomorKompartment.EditValue = HtmlEncode(NomorKompartment.CurrentValue);
                NomorKompartment.PlaceHolder = RemoveHtml(NomorKompartment.Caption);

                // InLineBlending
                InLineBlending.EditValue = InLineBlending.Options(false);
                InLineBlending.PlaceHolder = RemoveHtml(InLineBlending.Caption);

                // NomorTangki2
                NomorTangki2.SetupEditAttributes();
                if (!NomorTangki2.Raw)
                    NomorTangki2.CurrentValue = HtmlDecode(NomorTangki2.CurrentValue);
                NomorTangki2.EditValue = HtmlEncode(NomorTangki2.CurrentValue);
                NomorTangki2.PlaceHolder = RemoveHtml(NomorTangki2.Caption);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomProduk() {
            if (Produk.Visible && Produk.Required) {
                if (!Produk.IsDetailKey && Empty(Produk.FormValue)) {
                    Produk.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }

        private void ValidateCustomLO() {
            if (LO.Visible && LO.Required) {
                if (!LO.IsDetailKey && Empty(LO.FormValue)) {
                    LO.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorTangki() {
            if (NomorTangki.Visible && NomorTangki.Required) {
                if (!NomorTangki.IsDetailKey && Empty(NomorTangki.FormValue)) {
                    NomorTangki.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomMeterAwal() {
            if (MeterAwal.Visible && MeterAwal.Required) {
                if (!MeterAwal.IsDetailKey && Empty(MeterAwal.FormValue)) {
                    MeterAwal.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomMeterAkhir() {
            if (MeterAkhir.Visible && MeterAkhir.Required) {
                if (!MeterAkhir.IsDetailKey && Empty(MeterAkhir.FormValue)) {
                    MeterAkhir.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        private void ValidateCustomVolume() {
            if (Volume.Visible && Volume.Required) {
                if (!Volume.IsDetailKey && Empty(Volume.FormValue)) {
                    Volume.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }

        private void ValidateCustomBayNumber() {
            if (BayNumber.Visible && BayNumber.Required) {
                if (!BayNumber.IsDetailKey && Empty(BayNumber.FormValue)) {
                    BayNumber.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }

        private void ValidateCustomNomorKompartment() {
            if (NomorKompartment.Visible && NomorKompartment.Required) {
                if (!NomorKompartment.IsDetailKey && Empty(NomorKompartment.FormValue)) {
                    NomorKompartment.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }

        private void ValidateCustomInLineBlending() {
            if (InLineBlending.Visible && InLineBlending.Required) {
                if (Empty(InLineBlending.FormValue)) {
                    InLineBlending.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }

        private void ValidateCustomNomorTangki2() {
            if (NomorTangki2.Visible && NomorTangki2.Required) {
                if (!NomorTangki2.IsDetailKey && Empty(NomorTangki2.FormValue)) {
                    NomorTangki2.AddErrorMessage(ConvertToString(NoMeter.RequiredErrorMessage).Replace("%s", NoMeter.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomBayNumber();
                ValidateCustomNomorKompartment();
                ValidateCustomInLineBlending();
                ValidateCustomNomorTangki2();
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // LO
            LO.SetDbValue(rsnew, LO.CurrentValue, LO.ReadOnly);
            ...
            // BayNumber
            BayNumber.SetDbValue(rsnew, BayNumber.CurrentValue, BayNumber.ReadOnly);

            // NomorKompartment
            NomorKompartment.SetDbValue(rsnew, NomorKompartment.CurrentValue, NomorKompartment.ReadOnly);

            // InLineBlending
            InLineBlending.SetDbValue(rsnew, ConvertToBool(InLineBlending.CurrentValue), InLineBlending.ReadOnly);

            // NomorTangki2
            NomorTangki2.SetDbValue(rsnew, NomorTangki2.CurrentValue, NomorTangki2.ReadOnly);
        ...
        {
            object? value;
            if (row.TryGetValue("LO", out value)) // LO
                LO.CurrentValue = value;
            ...
            if (row.TryGetValue("BayNumber", out value)) // BayNumber
                BayNumber.CurrentValue = value;
            if (row.TryGetValue("NomorKompartment", out value)) // NomorKompartment
                NomorKompartment.CurrentValue = value;
            if (row.TryGetValue("InLineBlending", out value)) // InLineBlending
                InLineBlending.CurrentValue = value;
            if (row.TryGetValue("NomorTangki2", out value)) // NomorTangki2
                NomorTangki2.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasLogbookPenerimaanPenyaluranList")), "", TableVar, true);
            string pageId = "edit";
