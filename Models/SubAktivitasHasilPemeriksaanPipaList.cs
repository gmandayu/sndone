...
public partial class SnDOne {
    ...
    public class SubAktivitasHasilPemeriksaanPipaListBase : SubAktivitasHasilPemeriksaanPipa
    {
        ...
        // Constructor
        public SubAktivitasHasilPemeriksaanPipaListBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasHasilPemeriksaanPipaListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field NoBatch
            if (filter.TryGetValue("x_NoBatch", out sv)) {
                RestoreFieldFilter(NoBatch, filter, "NoBatch", sv);
            }
            ...
            // Field Nama_Kegiatan
            if (filter.TryGetValue("x_Nama_Kegiatan", out sv)) {
                RestoreFieldFilter(Nama_Kegiatan, filter, "Nama_Kegiatan", sv);
            }
            ...
            // Field Density
            if (filter.TryGetValue("x_Density", out sv)) {
                RestoreFieldFilter(Density, filter, "Density", sv);
            }

            // Field Temperature
            if (filter.TryGetValue("x_Temperature", out sv)) {
                RestoreFieldFilter(Temperature, filter, "Temperature", sv);
            }

            // Field Pressure
            if (filter.TryGetValue("x_Pressure", out sv)) {
                RestoreFieldFilter(Pressure, filter, "Pressure", sv);
            }

            // Field VolumeDischarge
            if (filter.TryGetValue("x_VolumeDischarge", out sv)) {
                RestoreFieldFilter(VolumeDischarge, filter, "VolumeDischarge", sv);
            }
            ...
            if (filter.TryGetValue("x_SubAktivitasHasilPemeriksaanId", out sv)) {
                RestoreFieldFilter(SubAktivitasHasilPemeriksaanId, filter, "SubAktivitasHasilPemeriksaanId", sv);
            }

            // Field TotalTerima
            if (filter.TryGetValue("x_TotalTerima", out sv)) {
                RestoreFieldFilter(TotalTerima, filter, "TotalTerima", sv);
            }
