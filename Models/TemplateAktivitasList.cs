...
public partial class SnDOne {
    ...
    public class TemplateAktivitasListBase : TemplateAktivitas
    {
        ...
        // Constructor
        public TemplateAktivitasListBase()
        {
        ...
        // Constructor
        public TemplateAktivitasListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public void ShowFilterList()
        {
            ...
            // Field IdTemplateProses
            filter = QueryBuilderWhere("IdTemplateProses");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdTemplateProses, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdTemplateProses.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field UrutanAktivitas
            filter = QueryBuilderWhere("UrutanAktivitas");
            if (Empty(filter))
                BuildSearchSql(ref filter, UrutanAktivitas, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + UrutanAktivitas.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field NamaAktivitas
            filter = QueryBuilderWhere("NamaAktivitas");
            if (Empty(filter))
                BuildSearchSql(ref filter, NamaAktivitas, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NamaAktivitas.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field IdPIC
            filter = QueryBuilderWhere("IdPIC");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdPIC, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdPIC.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field URL
            filter = QueryBuilderWhere("URL");
            if (Empty(filter))
                BuildSearchSql(ref filter, _URL, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + _URL.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        protected void LoadSearchValues() {
            ...
            // IdTemplateProses
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdTemplateProses[]"))
            ...
            // UrutanAktivitas
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_UrutanAktivitas[]"))
