...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasDataGerbongKetelRtwAdd
    /// </summary>
    public static SubAktivitasDataGerbongKetelRtwAdd subAktivitasDataGerbongKetelRtwAdd
    {
        get => HttpData.Get<SubAktivitasDataGerbongKetelRtwAdd>("subAktivitasDataGerbongKetelRtwAdd")!;
        set => HttpData["subAktivitasDataGerbongKetelRtwAdd"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasDataGerbongKetelRTW
    /// </summary>
    public class SubAktivitasDataGerbongKetelRtwAdd : SubAktivitasDataGerbongKetelRtwAddBase
    {
        // Constructor
        public SubAktivitasDataGerbongKetelRtwAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasDataGerbongKetelRtwAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasDataGerbongKetelRtwAddBase : SubAktivitasDataGerbongKetelRtw
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasDataGerbongKetelRtwAdd";
        ...
        // Constructor
        public SubAktivitasDataGerbongKetelRtwAddBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasDataGerbongKetelRtwAdd";
        ...
        // Constructor
        public SubAktivitasDataGerbongKetelRtwAddBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private IActionResult? HandleCopyAction(Dictionary<string, object>? rsold)
        {
            if (rsold == null) {
                ...
                return Terminate("SubAktivitasDataGerbongKetelRtwList");
            }
            return null;
        }
        ...
        private string DetermineReturnUrl()
        {
            string returnUrl = "";
            returnUrl = ReturnUrl;
            if (GetPageName(returnUrl) == "SubAktivitasDataGerbongKetelRtwList")
                returnUrl = AddMasterUrl(ListUrl);
            else if (GetPageName(returnUrl) == "SubAktivitasDataGerbongKetelRtwView")
                returnUrl = ViewUrl;
            return returnUrl;
        }
        ...
        private void HandleAjaxActions(ref string returnUrl)
        {
            if (IsModal && UseAjaxActions) {
                ...
                if (GetPageName(returnUrl) != "SubAktivitasDataGerbongKetelRtwList") {
                    ...
                    returnUrl = "SubAktivitasDataGerbongKetelRtwList";
                }
            }
        }
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasDataGerbongKetelRtwAdd?.PageRender();
                    }
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'TanggalTiba'
            TanggalTiba.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(TanggalTiba, "TanggalTiba", "x_TanggalTiba", true, validate, true);

            // Standard handling for 'Nominasi'
            Nominasi.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Nominasi, "Nominasi", "x_Nominasi");
            ...
            // Standard handling for 'DetailGerbong'
            DetailGerbong.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(DetailGerbong, "DetailGerbong", "x_DetailGerbong");

            // Standard handling for 'DetailQuantity'
            DetailQuantity.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(DetailQuantity, "DetailQuantity", "x_DetailQuantity");
        }
        #pragma warning restore 1998
        ...
        public void RestoreFormValues()
        {
            ...
            TanggalTiba.CurrentValue = TanggalTiba.FormValue;
            TanggalTiba.CurrentValue = UnformatDateTime(TanggalTiba.CurrentValue, TanggalTiba.FormatPattern);
            Nominasi.CurrentValue = Nominasi.FormValue;
            ...
            DetailGerbong.CurrentValue = DetailGerbong.FormValue;
            DetailQuantity.CurrentValue = DetailQuantity.FormValue;
        }
        ...
        public async Task RenderRow()
        {
            ...
            // TanggalTiba
            TanggalTiba.RowCssClass = "row";

            // Nominasi
            Nominasi.RowCssClass = "row";
            ...
            // DetailGerbong
            DetailGerbong.RowCssClass = "row";

            // DetailQuantity
            DetailQuantity.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                if (!JenisProduk.Raw)
                    JenisProduk.CurrentValue = HtmlDecode(JenisProduk.CurrentValue);
                JenisProduk.EditValue = HtmlEncode(JenisProduk.CurrentValue);

                // awallookupbung
                string curVal4 = ConvertToString(JenisProduk.CurrentValue);
                JenisProduk.EditValue = Empty(curVal4) ? DbNullValue : HtmlEncode(JenisProduk.CurrentValue);
                if (!Empty(curVal4)) {
                    if (JenisProduk.Lookup != null && IsDictionary(JenisProduk.Lookup?.Options) && JenisProduk.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        JenisProduk.EditValue = JenisProduk.LookupCacheOption(curVal4);
                    } else { // Lookup from database // DN
                ...
                if (!Nominasi.Raw)
                    Nominasi.CurrentValue = HtmlDecode(Nominasi.CurrentValue);
                Nominasi.EditValue = HtmlEncode(Nominasi.CurrentValue);
                Nominasi.PlaceHolder = RemoveHtml(Nominasi.Caption);
                ...
                // DetailGerbong
                DetailGerbong.SetupEditAttributes();
                if (!DetailGerbong.Raw)
                    DetailGerbong.CurrentValue = HtmlDecode(DetailGerbong.CurrentValue);
                DetailGerbong.EditValue = HtmlEncode(DetailGerbong.CurrentValue);
                DetailGerbong.PlaceHolder = RemoveHtml(DetailGerbong.Caption);

                // DetailQuantity
                DetailQuantity.SetupEditAttributes();
                if (!DetailQuantity.Raw)
                    DetailQuantity.CurrentValue = HtmlDecode(DetailQuantity.CurrentValue);
                DetailQuantity.EditValue = HtmlEncode(DetailQuantity.CurrentValue);
                DetailQuantity.PlaceHolder = RemoveHtml(DetailQuantity.Caption);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        private void ValidateCustomShipment() {
            if (Shipment.Visible && Shipment.Required) {
                if (!Shipment.IsDetailKey && Empty(Shipment.FormValue)) {
                    Shipment.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        private void ValidateCustomJenisProduk() {
            if (JenisProduk.Visible && JenisProduk.Required) {
                if (!JenisProduk.IsDetailKey && Empty(JenisProduk.FormValue)) {
                    JenisProduk.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }

        private void ValidateCustomTanggalTiba() {
            if (TanggalTiba.Visible && TanggalTiba.Required) {
                if (!TanggalTiba.IsDetailKey && Empty(TanggalTiba.FormValue)) {
                    TanggalTiba.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }

        private void ValidateCustomNominasi() {
            if (Nominasi.Visible && Nominasi.Required) {
                if (!Nominasi.IsDetailKey && Empty(Nominasi.FormValue)) {
                    Nominasi.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        private void ValidateCustomuserInput() {
            if (userInput.Visible && userInput.Required) {
                if (!userInput.IsDetailKey && Empty(userInput.FormValue)) {
                    userInput.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        private void ValidateCustometlDate() {
            if (etlDate.Visible && etlDate.Required) {
                if (!etlDate.IsDetailKey && Empty(etlDate.FormValue)) {
                    etlDate.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }

        private void ValidateCustomDetailGerbong() {
            if (DetailGerbong.Visible && DetailGerbong.Required) {
                if (!DetailGerbong.IsDetailKey && Empty(DetailGerbong.FormValue)) {
                    DetailGerbong.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }

        private void ValidateCustomDetailQuantity() {
            if (DetailQuantity.Visible && DetailQuantity.Required) {
                if (!DetailQuantity.IsDetailKey && Empty(DetailQuantity.FormValue)) {
                    DetailQuantity.AddErrorMessage(ConvertToString(DetailQuantity.RequiredErrorMessage).Replace("%s", DetailQuantity.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomTanggalTiba();
                if (!CheckDate(TanggalTiba.FormValue, TanggalTiba.FormatPattern)) {
                    TanggalTiba.AddErrorMessage(TanggalTiba.GetErrorMessage(false));
                }
                ValidateCustomNominasi();
                ...
                ValidateCustomDetailGerbong();
                ValidateCustomDetailQuantity();
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // Shipment
                Shipment.SetDbValue(rsnew, Shipment.CurrentValue);

                // JenisProduk
                JenisProduk.SetDbValue(rsnew, JenisProduk.CurrentValue);

                // TanggalTiba
                TanggalTiba.SetDbValue(rsnew, ConvertToDateTime(TanggalTiba.CurrentValue, TanggalTiba.FormatPattern));

                // Nominasi
                Nominasi.SetDbValue(rsnew, Nominasi.CurrentValue);
                ...
                // DetailGerbong
                DetailGerbong.SetDbValue(rsnew, DetailGerbong.CurrentValue);

                // DetailQuantity
                DetailQuantity.SetDbValue(rsnew, DetailQuantity.CurrentValue);
                return rsnew;
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("Shipment", out value)) // Shipment
                Shipment.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("JenisProduk", out value)) // JenisProduk
                JenisProduk.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("TanggalTiba", out value)) // TanggalTiba
                TanggalTiba.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("Nominasi", out value)) // Nominasi
                Nominasi.SetFormValue(ConvertToString(value));
            ...
            if (row.TryGetValue("DetailGerbong", out value)) // DetailGerbong
                DetailGerbong.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("DetailQuantity", out value)) // DetailQuantity
                DetailQuantity.SetFormValue(ConvertToString(value));
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasDataGerbongKetelRtwList")), "", TableVar, true);
