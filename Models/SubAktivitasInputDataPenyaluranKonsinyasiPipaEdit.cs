...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasInputDataPenyaluranKonsinyasiPipaEdit
    /// </summary>
    public static SubAktivitasInputDataPenyaluranKonsinyasiPipaEdit subAktivitasInputDataPenyaluranKonsinyasiPipaEdit
    {
        get => HttpData.Get<SubAktivitasInputDataPenyaluranKonsinyasiPipaEdit>("subAktivitasInputDataPenyaluranKonsinyasiPipaEdit")!;
        set => HttpData["subAktivitasInputDataPenyaluranKonsinyasiPipaEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasInputDataPenyaluranKonsinyasiPipa
    /// </summary>
    public class SubAktivitasInputDataPenyaluranKonsinyasiPipaEdit : SubAktivitasInputDataPenyaluranKonsinyasiPipaEditBase
    {
        // Constructor
        public SubAktivitasInputDataPenyaluranKonsinyasiPipaEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasInputDataPenyaluranKonsinyasiPipaEdit() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasInputDataPenyaluranKonsinyasiPipaEditBase : SubAktivitasInputDataPenyaluranKonsinyasiPipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasInputDataPenyaluranKonsinyasiPipaEdit";
        ...
        // Constructor
        public SubAktivitasInputDataPenyaluranKonsinyasiPipaEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasInputDataPenyaluranKonsinyasiPipaEdit";
        ...
        // Constructor
        public SubAktivitasInputDataPenyaluranKonsinyasiPipaEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasInputDataPenyaluranKonsinyasiPipaList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasInputDataPenyaluranKonsinyasiPipaList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasInputDataPenyaluranKonsinyasiPipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasInputDataPenyaluranKonsinyasiPipaEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'NoUrut'
            NoUrut.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NoUrut, "NoUrut", "x_NoUrut");
            if (!id.IsDetailKey) {
        ...
        public void RestoreFormValues()
        {
            ...
            NoUrut.CurrentValue = NoUrut.FormValue;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                NoUrut.EditValue = HtmlEncode(NoUrut.CurrentValue);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(NoUrut.RequiredErrorMessage).Replace("%s", NoUrut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(NoUrut.RequiredErrorMessage).Replace("%s", NoUrut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(NoUrut.RequiredErrorMessage).Replace("%s", NoUrut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(NoUrut.RequiredErrorMessage).Replace("%s", NoUrut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(NoUrut.RequiredErrorMessage).Replace("%s", NoUrut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(NoUrut.RequiredErrorMessage).Replace("%s", NoUrut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomProduk() {
            if (Produk.Visible && Produk.Required) {
                if (!Produk.IsDetailKey && Empty(Produk.FormValue)) {
                    Produk.AddErrorMessage(ConvertToString(NoUrut.RequiredErrorMessage).Replace("%s", NoUrut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNominasi() {
            if (Nominasi.Visible && Nominasi.Required) {
                if (!Nominasi.IsDetailKey && Empty(Nominasi.FormValue)) {
                    Nominasi.AddErrorMessage(ConvertToString(NoUrut.RequiredErrorMessage).Replace("%s", NoUrut.Caption));
                }
            }
        }

        private void ValidateCustomNoUrut() {
            if (NoUrut.Visible && NoUrut.Required) {
                if (!NoUrut.IsDetailKey && Empty(NoUrut.FormValue)) {
                    NoUrut.AddErrorMessage(ConvertToString(NoUrut.RequiredErrorMessage).Replace("%s", NoUrut.Caption));
                }
            }
        }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // NoUrut
            NoUrut.SetDbValue(rsnew, NoUrut.CurrentValue, NoUrut.ReadOnly);
            return rsnew;
        }
        ...
        {
            object? value;
            if (row.TryGetValue("NoUrut", out value)) // NoUrut
                NoUrut.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasInputDataPenyaluranKonsinyasiPipaList")), "", TableVar, true);
            string pageId = "edit";
