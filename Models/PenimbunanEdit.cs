...
public partial class SnDOne {
    ...
    public class PenimbunanEditBase : Penimbunan
    {
        ...
        // Constructor
        public PenimbunanEditBase(Controller? controller)
        {
        ...
        // Constructor
        public PenimbunanEditBase() : this(null) { }
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdPenimbunan", out rv)) {
                    IdPenimbunan.FormValue = UrlDecode(rv);
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'NomorPenimbunan'
            NomorPenimbunan.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NomorPenimbunan, "NomorPenimbunan", "x_NomorPenimbunan");
            ...
            if (!IdPenimbunan.IsDetailKey) {
                SetNormalField(IdPenimbunan, "IdPenimbunan", "x_IdPenimbunan");
            }
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomNomorPenimbunan() {
            if (NomorPenimbunan.Visible && NomorPenimbunan.Required) {
                if (!NomorPenimbunan.IsDetailKey && Empty(NomorPenimbunan.FormValue)) {
                    NomorPenimbunan.AddErrorMessage(ConvertToString(LookupJenisPlant.RequiredErrorMessage).Replace("%s", LookupJenisPlant.Caption));
                }
            }
        }
