...
public partial class SnDOne {
    ...
    public class MasterDermagaListBase : MasterDermaga
    {
        ...
        // Constructor
        public MasterDermagaListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public void ShowFilterList()
        {
            ...
            // Field Region
            filter = QueryBuilderWhere("Region");
            if (Empty(filter))
                BuildSearchSql(ref filter, Region, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Region.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Id_Plant
            filter = QueryBuilderWhere("Id_Plant");
            if (Empty(filter))
                BuildSearchSql(ref filter, Id_Plant, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Id_Plant.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field TBBM
            filter = QueryBuilderWhere("TBBM");
            if (Empty(filter))
                BuildSearchSql(ref filter, TBBM, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TBBM.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Equipment_ID
            filter = QueryBuilderWhere("Equipment_ID");
            if (Empty(filter))
                BuildSearchSql(ref filter, Equipment_ID, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Equipment_ID.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Fungsi
            filter = QueryBuilderWhere("Fungsi");
            if (Empty(filter))
                BuildSearchSql(ref filter, Fungsi, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Fungsi.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Kapasitas_DWT
            filter = QueryBuilderWhere("Kapasitas_DWT");
            if (Empty(filter))
                BuildSearchSql(ref filter, Kapasitas_DWT, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Kapasitas_DWT.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Kategori_Kapasitas
            filter = QueryBuilderWhere("Kategori_Kapasitas");
            if (Empty(filter))
                BuildSearchSql(ref filter, Kategori_Kapasitas, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Kategori_Kapasitas.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Jenis_Sartam
            filter = QueryBuilderWhere("Jenis_Sartam");
            if (Empty(filter))
                BuildSearchSql(ref filter, Jenis_Sartam, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Jenis_Sartam.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Type_Sartam
            filter = QueryBuilderWhere("Type_Sartam");
            if (Empty(filter))
                BuildSearchSql(ref filter, Type_Sartam, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Type_Sartam.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Tahun_Pembuatan
            filter = QueryBuilderWhere("Tahun_Pembuatan");
            if (Empty(filter))
                BuildSearchSql(ref filter, Tahun_Pembuatan, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Tahun_Pembuatan.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Kategori_Port
            filter = QueryBuilderWhere("Kategori_Port");
            if (Empty(filter))
                BuildSearchSql(ref filter, Kategori_Port, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Kategori_Port.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected void LoadSearchValues() {
            ...
            // Id_Plant
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Id_Plant[]"))
            ...
            // Equipment_ID
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Equipment_ID[]"))
            ...
            // Kapasitas_DWT
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Kapasitas_DWT[]"))
            ...
            // Kategori_Kapasitas
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Kategori_Kapasitas[]"))
            ...
            // Jenis_Sartam
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Jenis_Sartam[]"))
            ...
            // Type_Sartam
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Type_Sartam[]"))
            ...
            // Tahun_Pembuatan
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Tahun_Pembuatan[]"))
            ...
            // Kategori_Port
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Kategori_Port[]"))
