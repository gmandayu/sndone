...
public partial class SnDOne {
    ...
    /// masterUserList
    /// </summary>
    public static MasterUserList masterUserList
    {
        get => HttpData.Get<MasterUserList>("masterUserList")!;
        set => HttpData["masterUserList"] = value;
    }

    /// <summary>
    /// Page class for MasterUser
    /// </summary>
    public class MasterUserList : MasterUserListBase
    {
        // Constructor
        public MasterUserList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterUserList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class MasterUserListBase : MasterUser
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterUserList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMasterUserlist";
        ...
        // Constructor
        public MasterUserListBase(Controller? controller)
        {
        ...
        if (controller != null)
            ...
            MultiDeleteUrl = "MasterUserDelete";
            MultiUpdateUrl = "MasterUserUpdate";
        ...
        // Page name
        public string PageName => "MasterUserList";
        ...
        // Constructor
        public MasterUserListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(IdIdaman.AdvancedSearch.ToJson())); // Field IdIdaman
            filters.Merge(JObject.Parse(exception.AdvancedSearch.ToJson())); // Field exception 
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field IdIdaman
            if (filter.TryGetValue("x_IdIdaman", out sv)) {
                RestoreFieldFilter(IdIdaman, filter, "IdIdaman", sv);
            }

            // Field exception 
            if (filter.TryGetValue("x_exception", out sv)) {
                RestoreFieldFilter(exception, filter, "exception", sv);
            }
        }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, _UserLevel, def, true); // _UserLevel
            BuildSearchSql(ref where, Rule, def, true); // Rule
            ...
            BuildSearchSql(ref where, StatusAktif, def, true); // StatusAktif
            ...
            BuildSearchSql(ref where, IdIdaman, def, false); // IdIdaman
            BuildSearchSql(ref where, exception, def, true); // exception

            // Set up search command
            if (!def && !Empty(where) && (new[] { "", "reset", "resetall" }).Contains(Command))
                Command = "search";
            if (!def && Command == "search") {
                IdIdaman.AdvancedSearch.Save(); // IdIdaman
                exception.AdvancedSearch.Save(); // exception 
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("Email", _Email, true, config);
            filterList += ProcessFieldFilter("Username", _Username, true, config);
            filterList += ProcessFieldFilter("NamaLengkap", NamaLengkap, true, config);
            filterList += ProcessFieldFilter("UserLevel", _UserLevel, true, config);
            filterList += ProcessFieldFilter("Rule", Rule, true, config);
            filterList += ProcessFieldFilter("IdPosition", IdPosition, true, config);
            filterList += ProcessFieldFilter("Region", Region, true, config);
            filterList += ProcessFieldFilter("Plant", Plant, true, config);
            filterList += ProcessFieldFilter("StatusAktif", StatusAktif, true, config);
            ...
            filterList += ProcessFieldFilter("IdIdaman", IdIdaman, false, config);
            filterList += ProcessFieldFilter("exception ", exception, true, config);
            return filterList;
        }
        ...
        protected bool CheckSearchParms() {
            ...
            if (NamaLengkap.AdvancedSearch.IssetSession)
                return true;
            if (_UserLevel.AdvancedSearch.IssetSession)
                return true;
            if (Rule.AdvancedSearch.IssetSession)
                return true;
            ...
            if (Plant.AdvancedSearch.IssetSession)
                return true;
            if (StatusAktif.AdvancedSearch.IssetSession)
                return true;
            ...
            if (TanggalDiperbarui.AdvancedSearch.IssetSession)
                return true;
            if (IdIdaman.AdvancedSearch.IssetSession)
                return true;
            if (exception.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            _UserLevel.AdvancedSearch.UnsetSession();
            Rule.AdvancedSearch.UnsetSession();
            IdPosition.AdvancedSearch.UnsetSession();
            Region.AdvancedSearch.UnsetSession();
            Plant.AdvancedSearch.UnsetSession();
            StatusAktif.AdvancedSearch.UnsetSession();
            ...
            IdIdaman.AdvancedSearch.UnsetSession();
            exception.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            _UserLevel.AdvancedSearch.Load();
            Rule.AdvancedSearch.Load();
            IdPosition.AdvancedSearch.Load();
            Region.AdvancedSearch.Load();
            Plant.AdvancedSearch.Load();
            StatusAktif.AdvancedSearch.Load();
            ...
            IdIdaman.AdvancedSearch.Load();
            exception.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(_Username, ctrl); // Username
                UpdateSort(NamaLengkap, ctrl); // NamaLengkap
                UpdateSort(_UserLevel, ctrl); // UserLevel
                UpdateSort(Rule, ctrl); // Rule
                UpdateSort(IdPosition, ctrl); // IdPosition
                UpdateSort(Region, ctrl); // Region
                UpdateSort(Plant, ctrl); // Plant
                UpdateSort(StatusAktif, ctrl); // StatusAktif
                ...
                UpdateSort(IdIdaman, ctrl); // IdIdaman
                UpdateSort(exception, ctrl); // exception 
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MasterUser"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MasterUser"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MasterUser"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMasterUserlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMasterUserlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MasterUser"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMasterUserlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("UserLevel")); // DN
                CreateColumnOption(option.Add("Rule")); // DN
                CreateColumnOption(option.Add("IdPosition")); // DN
                CreateColumnOption(option.Add("Region")); // DN
                CreateColumnOption(option.Add("Plant")); // DN
                CreateColumnOption(option.Add("StatusAktif")); // DN
                ...
                CreateColumnOption(option.Add("IdIdaman")); // DN
                CreateColumnOption(option.Add("exception ")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMasterUsersrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMasterUsersrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMasterUserlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        {
            string userlist = String.Join(", ", rows?.Select(r => ConvertToString(r[Config.LoginUsernameFieldName])) ?? new string[0]);
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(masterUserList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(masterUserList.RowCount) + "_MasterUser");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(masterUserList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && masterUserList.RowType == RowType.Add || IsEdit && masterUserList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldSearchValues()
        {
            ...
            LoadField_UserLevelSearchValues();
            LoadFieldRuleSearchValues();
            LoadFieldIdPositionSearchValues();
            LoadFieldRegionSearchValues();
            LoadFieldPlantSearchValues();
            LoadFieldStatusAktifSearchValues();
            ...
            LoadFieldIdIdamanSearchValues();
            LoadFieldexceptionSearchValues();
        }
        ...
        {
            if (!IsAddOrEdit && Query.ContainsKey("x__Email[]")) {
        ...
        {
            if (!IsAddOrEdit && Query.ContainsKey("x__Username[]")) {
        ...
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaLengkap[]")) {
        ...
        private void LoadField_UserLevelSearchValues()
        {
            // _UserLevel
            LoadPrimarySearchValue_UserLevel();
            LoadSearchOperator_UserLevel();
        }

        private void LoadPrimarySearchValue_UserLevel()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x__UserLevel[]")) {
                _UserLevel.AdvancedSearch.SearchValue = Get("x__UserLevel[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                _UserLevel.AdvancedSearch.SearchValue = Get("_UserLevel"); // Default Value // DN
            }
            if (!Empty(_UserLevel.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperator_UserLevel()
        {
            if (Query.ContainsKey("z__UserLevel"))
                _UserLevel.AdvancedSearch.SearchOperator = Get("z__UserLevel", Config.FilterOptionSeparator);
        }

        private void LoadFieldRuleSearchValues()
        {
            // Rule
            LoadPrimarySearchValueRule();
            LoadSearchOperatorRule();
        }

        private void LoadPrimarySearchValueRule()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Rule[]")) {
                Rule.AdvancedSearch.SearchValue = Get("x_Rule[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Rule.AdvancedSearch.SearchValue = Get("Rule"); // Default Value // DN
            }
            if (!Empty(Rule.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorRule()
        {
            if (Query.ContainsKey("z_Rule"))
                Rule.AdvancedSearch.SearchOperator = Get("z_Rule", Config.FilterOptionSeparator);
        }
        ...
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdPosition[]")) {
        ...
        private void LoadFieldStatusAktifSearchValues()
        {
            // StatusAktif
            LoadPrimarySearchValueStatusAktif();
            LoadSearchOperatorStatusAktif();
        }

        private void LoadPrimarySearchValueStatusAktif()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_StatusAktif[]")) {
                StatusAktif.AdvancedSearch.SearchValue = Get("x_StatusAktif[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                StatusAktif.AdvancedSearch.SearchValue = Get("StatusAktif"); // Default Value // DN
            }
            if (!Empty(StatusAktif.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorStatusAktif()
        {
            if (Query.ContainsKey("z_StatusAktif"))
                StatusAktif.AdvancedSearch.SearchOperator = Get("z_StatusAktif", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldIdIdamanSearchValues()
        {
            // IdIdaman
            LoadPrimarySearchValueIdIdaman();
            LoadSearchOperatorIdIdaman();
        }

        private void LoadPrimarySearchValueIdIdaman()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdIdaman")) {
                IdIdaman.AdvancedSearch.SearchValue = Get("x_IdIdaman");
            }
            else if (!IsAddOrEdit) {
                IdIdaman.AdvancedSearch.SearchValue = Get("IdIdaman"); // Default Value // DN
            }
            if (!Empty(IdIdaman.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIdIdaman()
        {
            if (Query.ContainsKey("z_IdIdaman"))
                IdIdaman.AdvancedSearch.SearchOperator = Get("z_IdIdaman");
        }

        private void LoadFieldexceptionSearchValues()
        {
            // exception
            LoadPrimarySearchValueexception();
            LoadSearchOperatorexception();
        }

        private void LoadPrimarySearchValueexception()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_exception[]")) {
                exception.AdvancedSearch.SearchValue = Get("x_exception[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                exception.AdvancedSearch.SearchValue = Get("exception"); // Default Value // DN
            }
            if (!Empty(exception.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorexception()
        {
            if (Query.ContainsKey("z_exception"))
                exception.AdvancedSearch.SearchOperator = Get("z_exception", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (_Username.UseFilter && !Empty(_Username.AdvancedSearch.SearchValue)) {
                    _Username.EditValue = ConvertToString(_Username.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // UserLevel
                if (_UserLevel.UseFilter && !Empty(_UserLevel.AdvancedSearch.SearchValue)) {
                    _UserLevel.EditValue = ConvertToString(_UserLevel.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // Rule
                if (Rule.UseFilter && !Empty(Rule.AdvancedSearch.SearchValue)) {
                    Rule.EditValue = ConvertToString(Rule.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // StatusAktif
                if (StatusAktif.UseFilter && !Empty(StatusAktif.AdvancedSearch.SearchValue)) {
                    StatusAktif.EditValue = ConvertToString(StatusAktif.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                if (!IdIdaman.Raw)
                    IdIdaman.AdvancedSearch.SearchValue = HtmlDecode(IdIdaman.AdvancedSearch.SearchValue);
                IdIdaman.EditValue = HtmlEncode(IdIdaman.AdvancedSearch.SearchValue);
                IdIdaman.PlaceHolder = RemoveHtml(IdIdaman.Caption);

                // exception 
                if (exception.UseFilter && !Empty(exception.AdvancedSearch.SearchValue)) {
                    exception.EditValue = ConvertToString(exception.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMasterUsersrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
