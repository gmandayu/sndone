...
public partial class SnDOne {
    ...
    /// </summary>
    [MaybeNull]
    public static MasterTools masterTools
    {
        get => HttpData.Resolve<MasterTools>("MasterTools");
        set => HttpData["MasterTools"] = value;
    }

    /// <summary>
    /// Table class for MasterTools
    /// </summary>
    public class MasterTools : DbTable
    {
        ...
        public readonly DbField<SqlDbType> NamaTools;
        ...
        public MasterTools()
        {
            ...
            Name = "MasterTools";
            Type = "TABLE";
            UpdateTable = "dbo.MasterTools"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            IdTools = new(this, "x_IdTools", 3, SqlDbType.Int) {
                ...
                CustomMessage = Language.FieldPhrase("MasterTools", "IdTools", "CustomMsg"),
                IsUpload = false
            };
            IdTools.Raw = true;
            IdTools.Lookup = new Lookup<DbField>(IdTools, "MasterTools", true, "IdTools", new List<string> {"IdTools", "", "", ""}, "", "", new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, false, "", "", "");
            Fields.Add("IdTools", IdTools);

            // NamaTools
            NamaTools = new(this, "x_NamaTools", 200, SqlDbType.VarChar) {
                Name = "NamaTools",
                Expression = "[NamaTools]",
                UseBasicSearch = true,
                BasicSearchExpression = "[NamaTools]",
                DateTimeFormat = -1,
                VirtualExpression = "[NamaTools]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("MasterTools", "NamaTools", "CustomMsg"),
                IsUpload = false
            };
            NamaTools.Lookup = new Lookup<DbField>(NamaTools, "MasterTools", true, "NamaTools", new List<string> {"NamaTools", "", "", ""}, "", "", new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, false, "", "", "");
            Fields.Add("NamaTools", NamaTools);
        ...
        public async Task<int> InsertAsync(object data)
        {
            ...
            if (data is IDictionary<string, object> d)
                ...
                throw new ArgumentException("Data must be of anonymous or Dictionary<string, object> or Entities.MasterTool type", "data");
            row = row.Where(kvp => {
            ...
            try {
                ...
                if (row.ContainsKey("IdTools"))
                    row["IdTools"] = lastInsertedId;
                else
                    row.Add("IdTools", lastInsertedId);
                IdTools.SetDbValue(lastInsertedId);
                result = 1;
            } catch (Exception e) {
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                NamaTools.DbValue = row["NamaTools"]; // Set DB value only
            } catch {}
        }
        ...
        private string _sqlKeyFilter => "[IdTools] = @IdTools@";
        ...
        public string GetModalCaption(string pageName)
        {
            return pageName switch {
                "MasterToolsView" => Language.Phrase("View"),
                "MasterToolsEdit" => Language.Phrase("Edit"),
                "MasterToolsAdd" => Language.Phrase("Add"),
                _ => String.Empty
            };
        }
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "MasterToolsView",
                Config.ApiAddAction => "MasterToolsAdd",
                Config.ApiEditAction => "MasterToolsEdit",
                Config.ApiDeleteAction => "MasterToolsDelete",
                Config.ApiListAction => "MasterToolsList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "MasterToolsList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("MasterToolsView", parm);
            else
                url = KeyUrl("MasterToolsView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "MasterToolsAdd";
        ...
        {
            string url = "";
            url = KeyUrl("MasterToolsEdit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("MasterToolsList", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("MasterToolsAdd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("MasterToolsList", "action=copy"))); // DN
        ...
        public string GetDeleteUrl(string parm = "")
        {
            return UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
                ...
                : AppPath(KeyUrl("MasterToolsDelete", parm)); // DN
        }
        ...
        {
            string json = "";
            json += "\"IdTools\":" + ConvertToJson(IdTools.CurrentValue, "number", true);
            json = "{" + json + "}";
            if (htmlEncode)
        ...
        {
            if (!IsNull(IdTools.CurrentValue)) {
                url += "/" + IdTools.CurrentValue;
            } else {
        ...
        public List<string> GetRecordKeys()
        {
            ...
            if (Post("key_m[]", out sv) || Get("key_m[]", out sv)) { // DN
                ...
                if (RouteValues.TryGetValue("IdTools", out object? v0)) { // IdTools // DN
                    key = UrlDecode(v0); // DN
        ...
        public string GetFilterFromRecordKeys(bool setCurrent = true)
        {
            ...
            foreach (var keys in recordKeys) {
                if (!Empty(keyFilter))
                    keyFilter += " OR ";
                if (setCurrent)
                    IdTools.CurrentValue = keys;
                else
                    IdTools.OldValue = keys;
        ...
        {
            string pageName = "MasterToolsList";
        ...
        public async Task RenderEditRow()
        {
            ...
            // NamaTools
            NamaTools.SetupEditAttributes();
            if (!NamaTools.Raw)
                NamaTools.CurrentValue = HtmlDecode(NamaTools.CurrentValue);
            NamaTools.EditValue = NamaTools.CurrentValue;
            NamaTools.PlaceHolder = RemoveHtml(NamaTools.Caption);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(NamaTools);
                        } else {
                            await doc.ExportField(IdTools);
                            await doc.ExportField(NamaTools);
                        }
