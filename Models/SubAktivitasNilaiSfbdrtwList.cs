...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiSfbdrtwList
    /// </summary>
    public static SubAktivitasNilaiSfbdrtwList subAktivitasNilaiSfbdrtwList
    {
        get => HttpData.Get<SubAktivitasNilaiSfbdrtwList>("subAktivitasNilaiSfbdrtwList")!;
        set => HttpData["subAktivitasNilaiSfbdrtwList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiSFBDRTW
    /// </summary>
    public class SubAktivitasNilaiSfbdrtwList : SubAktivitasNilaiSfbdrtwListBase
    {
        // Constructor
        public SubAktivitasNilaiSfbdrtwList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiSfbdrtwList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiSfbdrtwListBase : SubAktivitasNilaiSfbdrtw
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiSfbdrtwList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasNilaiSFBDRTWlist";
        ...
        // Constructor
        public SubAktivitasNilaiSfbdrtwListBase()
        {
            TableName = "SubAktivitasNilaiSFBDRTW";

            // CSS class name as context
            TableVar = "SubAktivitasNilaiSFBDRTW";
            ...
            // Table object (subAktivitasNilaiSfbdrtw)
            if (subAktivitasNilaiSfbdrtw == null || subAktivitasNilaiSfbdrtw is SubAktivitasNilaiSfbdrtw)
                subAktivitasNilaiSfbdrtw = this;

            // Initialize URLs
            AddUrl = "SubAktivitasNilaiSfbdrtwAdd";
            ...
            MultiDeleteUrl = "SubAktivitasNilaiSfbdrtwDelete";
            MultiUpdateUrl = "SubAktivitasNilaiSfbdrtwUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiSfbdrtwList";
        ...
        public void SetVisibility()
        {
            ...
            SFBD_KL_Obs.SetVisibility();
            SFBD_KL_15.SetVisibility();
            SFBD_Barrels.SetVisibility();
            SFBD_LT.SetVisibility();
            SFBD_MT.SetVisibility();
        ...
        // Constructor
        public SubAktivitasNilaiSfbdrtwListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasNilaiSfbdrtwView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasNilaiSFBDRTWgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasNilaiSfbdrtwList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(SFBD_KL_Obs.AdvancedSearch.ToJson())); // Field SFBD_KL_Obs
            filters.Merge(JObject.Parse(SFBD_KL_15.AdvancedSearch.ToJson())); // Field SFBD_KL_15
            filters.Merge(JObject.Parse(SFBD_Barrels.AdvancedSearch.ToJson())); // Field SFBD_Barrels
            filters.Merge(JObject.Parse(SFBD_LT.AdvancedSearch.ToJson())); // Field SFBD_LT
            filters.Merge(JObject.Parse(SFBD_MT.AdvancedSearch.ToJson())); // Field SFBD_MT
        ...
        protected bool RestoreFilterList() {
            ...
            // Field SFBD_KL_Obs
            if (filter?.TryGetValue("x_SFBD_KL_Obs", out sv) ?? false) {
                SFBD_KL_Obs.AdvancedSearch.SearchValue = sv;
                SFBD_KL_Obs.AdvancedSearch.SearchOperator = filter["z_SFBD_KL_Obs"];
                SFBD_KL_Obs.AdvancedSearch.SearchCondition = filter["v_SFBD_KL_Obs"];
                SFBD_KL_Obs.AdvancedSearch.SearchValue2 = filter["y_SFBD_KL_Obs"];
                SFBD_KL_Obs.AdvancedSearch.SearchOperator2 = filter["w_SFBD_KL_Obs"];
                SFBD_KL_Obs.AdvancedSearch.Save();
            }

            // Field SFBD_KL_15
            if (filter?.TryGetValue("x_SFBD_KL_15", out sv) ?? false) {
                SFBD_KL_15.AdvancedSearch.SearchValue = sv;
                SFBD_KL_15.AdvancedSearch.SearchOperator = filter["z_SFBD_KL_15"];
                SFBD_KL_15.AdvancedSearch.SearchCondition = filter["v_SFBD_KL_15"];
                SFBD_KL_15.AdvancedSearch.SearchValue2 = filter["y_SFBD_KL_15"];
                SFBD_KL_15.AdvancedSearch.SearchOperator2 = filter["w_SFBD_KL_15"];
                SFBD_KL_15.AdvancedSearch.Save();
            }

            // Field SFBD_Barrels
            if (filter?.TryGetValue("x_SFBD_Barrels", out sv) ?? false) {
                SFBD_Barrels.AdvancedSearch.SearchValue = sv;
                SFBD_Barrels.AdvancedSearch.SearchOperator = filter["z_SFBD_Barrels"];
                SFBD_Barrels.AdvancedSearch.SearchCondition = filter["v_SFBD_Barrels"];
                SFBD_Barrels.AdvancedSearch.SearchValue2 = filter["y_SFBD_Barrels"];
                SFBD_Barrels.AdvancedSearch.SearchOperator2 = filter["w_SFBD_Barrels"];
                SFBD_Barrels.AdvancedSearch.Save();
            }

            // Field SFBD_LT
            if (filter?.TryGetValue("x_SFBD_LT", out sv) ?? false) {
                SFBD_LT.AdvancedSearch.SearchValue = sv;
                SFBD_LT.AdvancedSearch.SearchOperator = filter["z_SFBD_LT"];
                SFBD_LT.AdvancedSearch.SearchCondition = filter["v_SFBD_LT"];
                SFBD_LT.AdvancedSearch.SearchValue2 = filter["y_SFBD_LT"];
                SFBD_LT.AdvancedSearch.SearchOperator2 = filter["w_SFBD_LT"];
                SFBD_LT.AdvancedSearch.Save();
            }

            // Field SFBD_MT
            if (filter?.TryGetValue("x_SFBD_MT", out sv) ?? false) {
                SFBD_MT.AdvancedSearch.SearchValue = sv;
                SFBD_MT.AdvancedSearch.SearchOperator = filter["z_SFBD_MT"];
                SFBD_MT.AdvancedSearch.SearchCondition = filter["v_SFBD_MT"];
                SFBD_MT.AdvancedSearch.SearchValue2 = filter["y_SFBD_MT"];
                SFBD_MT.AdvancedSearch.SearchOperator2 = filter["w_SFBD_MT"];
                SFBD_MT.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(SFBD_KL_Obs);
            searchFlds.Add(SFBD_KL_15);
            searchFlds.Add(SFBD_Barrels);
            searchFlds.Add(SFBD_LT);
            searchFlds.Add(SFBD_MT);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(SFBD_KL_Obs, ctrl); // SFBD_KL_Obs
                UpdateSort(SFBD_KL_15, ctrl); // SFBD_KL_15
                UpdateSort(SFBD_Barrels, ctrl); // SFBD_Barrels
                UpdateSort(SFBD_LT, ctrl); // SFBD_LT
                UpdateSort(SFBD_MT, ctrl); // SFBD_MT
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""SubAktivitasNilaiSFBDRTW"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasNilaiSFBDRTW"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""SubAktivitasNilaiSFBDRTW"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiSFBDRTWlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiSFBDRTWlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""SubAktivitasNilaiSFBDRTW"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fSubAktivitasNilaiSFBDRTWlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("SFBD_KL_Obs")); // DN
                CreateColumnOption(option.Add("SFBD_KL_15")); // DN
                CreateColumnOption(option.Add("SFBD_Barrels")); // DN
                CreateColumnOption(option.Add("SFBD_LT")); // DN
                CreateColumnOption(option.Add("SFBD_MT")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasNilaiSFBDRTWsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasNilaiSFBDRTWsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiSFBDRTWlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasNilaiSFBDRTW");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasNilaiSfbdrtwList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasNilaiSfbdrtwList.RowCount) + "_SubAktivitasNilaiSFBDRTW");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasNilaiSfbdrtwList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasNilaiSfbdrtwList.RowType == RowType.Add || IsEdit && subAktivitasNilaiSfbdrtwList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            SFBD_KL_Obs.SetDbValue(row["SFBD_KL_Obs"]);
            SFBD_KL_15.SetDbValue(row["SFBD_KL_15"]);
            SFBD_Barrels.SetDbValue(row["SFBD_Barrels"]);
            SFBD_LT.SetDbValue(row["SFBD_LT"]);
            SFBD_MT.SetDbValue(row["SFBD_MT"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("SFBD_KL_Obs", SFBD_KL_Obs.DefaultValue ?? DbNullValue); // DN
            row.Add("SFBD_KL_15", SFBD_KL_15.DefaultValue ?? DbNullValue); // DN
            row.Add("SFBD_Barrels", SFBD_Barrels.DefaultValue ?? DbNullValue); // DN
            row.Add("SFBD_LT", SFBD_LT.DefaultValue ?? DbNullValue); // DN
            row.Add("SFBD_MT", SFBD_MT.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // SFBD_KL_Obs
                SFBD_KL_Obs.ViewValue = ConvertToString(SFBD_KL_Obs.CurrentValue); // DN
                SFBD_KL_Obs.ViewCustomAttributes = "";

                // SFBD_KL_15
                SFBD_KL_15.ViewValue = ConvertToString(SFBD_KL_15.CurrentValue); // DN
                SFBD_KL_15.ViewCustomAttributes = "";

                // SFBD_Barrels
                SFBD_Barrels.ViewValue = ConvertToString(SFBD_Barrels.CurrentValue); // DN
                SFBD_Barrels.ViewCustomAttributes = "";

                // SFBD_LT
                SFBD_LT.ViewValue = ConvertToString(SFBD_LT.CurrentValue); // DN
                SFBD_LT.ViewCustomAttributes = "";

                // SFBD_MT
                SFBD_MT.ViewValue = ConvertToString(SFBD_MT.CurrentValue); // DN
                SFBD_MT.ViewCustomAttributes = "";
                ...
                // SFBD_KL_Obs
                SFBD_KL_Obs.HrefValue = "";
                SFBD_KL_Obs.TooltipValue = "";

                // SFBD_KL_15
                SFBD_KL_15.HrefValue = "";
                SFBD_KL_15.TooltipValue = "";

                // SFBD_Barrels
                SFBD_Barrels.HrefValue = "";
                SFBD_Barrels.TooltipValue = "";

                // SFBD_LT
                SFBD_LT.HrefValue = "";
                SFBD_LT.TooltipValue = "";

                // SFBD_MT
                SFBD_MT.HrefValue = "";
                SFBD_MT.TooltipValue = "";
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fSubAktivitasNilaiSFBDRTWlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fSubAktivitasNilaiSFBDRTWlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fSubAktivitasNilaiSFBDRTWlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasNilaiSFBDRTWsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
