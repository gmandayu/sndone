...
public partial class SnDOne {
    /// <summary>
    /// detailMasterPipaList
    /// </summary>
    public static DetailMasterPipaList detailMasterPipaList
    {
        get => HttpData.Get<DetailMasterPipaList>("detailMasterPipaList")!;
        set => HttpData["detailMasterPipaList"] = value;
    }

    /// <summary>
    /// Page class for DetailMasterPipa
    /// </summary>
    public class DetailMasterPipaList : DetailMasterPipaListBase
    {
        // Constructor
        public DetailMasterPipaList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public DetailMasterPipaList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class DetailMasterPipaListBase : DetailMasterPipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "detailMasterPipaList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fDetailMasterPipalist";
        ...
        // Constructor
        public DetailMasterPipaListBase()
        {
            TableName = "DetailMasterPipa";

            // CSS class name as context
            TableVar = "DetailMasterPipa";
            ...
            // Table object (detailMasterPipa)
            if (detailMasterPipa == null || detailMasterPipa is DetailMasterPipa)
                detailMasterPipa = this;

            // Initialize URLs
            AddUrl = "DetailMasterPipaAdd";
            ...
            MultiDeleteUrl = "DetailMasterPipaDelete";
            MultiUpdateUrl = "DetailMasterPipaUpdate";
        ...
        // Page name
        public string PageName => "DetailMasterPipaList";
        ...
        // Constructor
        public DetailMasterPipaListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "DetailMasterPipaView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fDetailMasterPipagrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                detailMasterPipaList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public void ShowFilterList()
        {
            ...
            // Field idPlantTujuan
            filter = QueryBuilderWhere("idPlantTujuan");
            if (Empty(filter))
                BuildSearchSql(ref filter, idPlantTujuan, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + idPlantTujuan.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Panjang
            filter = QueryBuilderWhere("Panjang");
            if (Empty(filter))
                BuildSearchSql(ref filter, Panjang, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Panjang.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Diameter
            filter = QueryBuilderWhere("Diameter");
            if (Empty(filter))
                BuildSearchSql(ref filter, Diameter, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Diameter.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(idPlantTujuan, ctrl); // idPlantTujuan
                UpdateSort(Panjang, ctrl); // Panjang
                UpdateSort(Diameter, ctrl); // Diameter
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""DetailMasterPipa"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""DetailMasterPipa"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""DetailMasterPipa"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fDetailMasterPipalist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fDetailMasterPipalist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""DetailMasterPipa"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fDetailMasterPipalist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("idPlantTujuan")); // DN
                CreateColumnOption(option.Add("Panjang")); // DN
                CreateColumnOption(option.Add("Diameter")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fDetailMasterPipasrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fDetailMasterPipasrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fDetailMasterPipalist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_DetailMasterPipa");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(detailMasterPipaList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(detailMasterPipaList.RowCount) + "_DetailMasterPipa");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(detailMasterPipaList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && detailMasterPipaList.RowType == RowType.Add || IsEdit && detailMasterPipaList.RowType == RowType.Edit) // Inline-Add/Edit row
            ...
            if (RowType == RowType.View) {
                ...
                if (!Empty(curVal)) {
                    if (idPipa.Lookup != null && IsDictionary(idPipa.Lookup?.Options) && idPipa.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        ...
                        string filterWrk = SearchFilter(idPipa.Lookup?.GetTable()?.Fields["id"].SearchExpression, "=", idPipa.CurrentValue, idPipa.Lookup?.GetTable()?.Fields["id"].SearchDataType, "");
                        string? sqlWrk = idPipa.Lookup?.GetSql(false, filterWrk, null, this, true, true);
                ...
                // Panjang
                Panjang.ViewValue = ConvertToString(Panjang.CurrentValue); // DN
                Panjang.ViewCustomAttributes = "";

                // Diameter
                Diameter.ViewValue = ConvertToString(Diameter.CurrentValue); // DN
                ...
                // idPlantTujuan
                if (idPlantTujuan.UseFilter && !Empty(idPlantTujuan.AdvancedSearch.SearchValue)) {
                    idPlantTujuan.EditValue = ConvertToString(idPlantTujuan.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // Panjang
                if (Panjang.UseFilter && !Empty(Panjang.AdvancedSearch.SearchValue)) {
                    Panjang.EditValue = ConvertToString(Panjang.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // Diameter
                if (Diameter.UseFilter && !Empty(Diameter.AdvancedSearch.SearchValue)) {
                    Diameter.EditValue = ConvertToString(Diameter.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fDetailMasterPipasrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
