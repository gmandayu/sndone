...
public partial class SnDOne {
    ...
    public class MasterRegionEditBase : MasterRegion
    {
        ...
        // Constructor
        public MasterRegionEditBase(Controller? controller)
        {
        ...
        // Constructor
        public MasterRegionEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdRegion", out rv)) {
                    IdRegion.FormValue = UrlDecode(rv);
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdRegion", out rv)) {
                        IdRegion.QueryValue = UrlDecode(rv);
                        loadByQuery = true;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("MasterRegionList");
            }
            return null;
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'RegionCode'
            RegionCode.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(RegionCode, "RegionCode", "x_RegionCode");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomIdRegion() {
            if (IdRegion.Visible && IdRegion.Required) {
                if (!IdRegion.IsDetailKey && Empty(IdRegion.FormValue)) {
                    IdRegion.AddErrorMessage(ConvertToString(RegionCode.RequiredErrorMessage).Replace("%s", RegionCode.Caption));
                }
            }
        }
        ...
        private void ValidateCustomRegion() {
            if (Region.Visible && Region.Required) {
                if (!Region.IsDetailKey && Empty(Region.FormValue)) {
                    Region.AddErrorMessage(ConvertToString(RegionCode.RequiredErrorMessage).Replace("%s", RegionCode.Caption));
                }
            }
        }

        private void ValidateCustomRegionCode() {
