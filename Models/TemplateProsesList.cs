...
public partial class SnDOne {
    ...
    /// templateProsesList
    /// </summary>
    public static TemplateProsesList templateProsesList
    {
        get => HttpData.Get<TemplateProsesList>("templateProsesList")!;
        set => HttpData["templateProsesList"] = value;
    }

    /// <summary>
    /// Page class for TemplateProses
    /// </summary>
    public class TemplateProsesList : TemplateProsesListBase
    {
        // Constructor
        public TemplateProsesList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TemplateProsesList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TemplateProsesListBase : TemplateProses
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "templateProsesList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fTemplateProseslist";
        ...
        // Constructor
        public TemplateProsesListBase(Controller? controller)
        {
        ...
        if (controller != null)
            ...
            // Initialize URLs
            AddUrl = "TemplateProsesAdd?" + Config.TableShowDetail + "=";
            ...
            MultiDeleteUrl = "TemplateProsesDelete";
            MultiUpdateUrl = "TemplateProsesUpdate";
        ...
        // Page name
        public string PageName => "TemplateProsesList";
        ...
        // Constructor
        public TemplateProsesListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, UrutanProses, def, true); // UrutanProses
            BuildSearchSql(ref where, NamaProses, def, true); // NamaProses
            BuildSearchSql(ref where, IdPIC, def, true); // IdPIC
            BuildSearchSql(ref where, IdTools, def, true); // IdTools
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("UrutanProses", UrutanProses, true, config);
            filterList += ProcessFieldFilter("NamaProses", NamaProses, true, config);
            filterList += ProcessFieldFilter("IdPIC", IdPIC, true, config);
            filterList += ProcessFieldFilter("IdTools", IdTools, true, config);
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(UrutanProses);
            searchFlds.Add(NamaProses);
            searchFlds.Add(IdPIC);
            searchFlds.Add(IdTools);
        ...
        protected bool CheckSearchParms() {
            ...
            if (IdTemplate.AdvancedSearch.IssetSession)
                return true;
            if (UrutanProses.AdvancedSearch.IssetSession)
                return true;
            if (NamaProses.AdvancedSearch.IssetSession)
                return true;
            if (IdPIC.AdvancedSearch.IssetSession)
                return true;
            if (IdTools.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            UrutanProses.AdvancedSearch.UnsetSession();
            NamaProses.AdvancedSearch.UnsetSession();
            IdPIC.AdvancedSearch.UnsetSession();
            IdTools.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            UrutanProses.AdvancedSearch.Load();
            NamaProses.AdvancedSearch.Load();
            IdPIC.AdvancedSearch.Load();
            IdTools.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(UrutanProses, ctrl); // UrutanProses
                UpdateSort(NamaProses, ctrl); // NamaProses
                UpdateSort(IdPIC, ctrl); // IdPIC
                UpdateSort(IdTools, ctrl); // IdTools
        ...
        protected async Task SetupListOptions() {
            ...
            item = ListOptions.Add("detail_TemplateAktivitas");
            item.CssClass = "text-nowrap";
        ...
        public async Task RenderListOptions()
        {
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fTemplateProseslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTemplateProseslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            // "detail_TemplateAktivitas"
            listOption = ListOptions["detail_TemplateAktivitas"];
            isVisible = Security.AllowList(CurrentProjectID + "TemplateAktivitas");
            if (isVisible) {
                string caption, title, url;
                var body = Language.Phrase("DetailLink") + Language.TablePhrase("TemplateAktivitas", "TblCaption");
                body = "<a class=\"btn btn-default ew-row-link ew-detail" + (ListOptions.UseDropDownButton ? " dropdown-toggle" : "") + "\" data-action=\"list\" href=\"" + HtmlEncode(AppPath("TemplateAktivitasList?" + Config.TableShowMaster + "=TemplateProses&" + ForeignKeyUrl("fk_IdTemplateProses", IdTemplateProses.CurrentValue) + "")) + "\">" + body + "</a>";
                string links = "";
                ...
                if (detailPage?.DetailView && Security.CanView && Security.AllowView(CurrentProjectID + "TemplateProses") ?? false) {
                    ...
                    url = GetViewUrl(Config.TableShowDetail + "=TemplateAktivitas");
                ...
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "TemplateProses") ?? false) {
                    ...
                    url = GetEditUrl(Config.TableShowDetail + "=TemplateAktivitas");
                ...
                if (detailPage?.DetailAdd && Security.CanAdd && Security.AllowAdd(CurrentProjectID + "TemplateProses") ?? false) {
                    ...
                    url = GetCopyUrl(Config.TableShowDetail + "=TemplateAktivitas");
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdTemplateProses.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            item = option.Add("detailadd_TemplateAktivitas");
            templateAktivitas = Resolve("TemplateAktivitas")!;
            if (templateAktivitas != null) {
                caption = Language.Phrase("Add") + "&nbsp;" + Caption + "/" + templateAktivitas.Caption;
                title = Language.Phrase("Add", true) + "&nbsp;" + Caption + "/" + templateAktivitas.Caption;
                item.Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" + title + "\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(GetAddUrl(Config.TableShowDetail + "=TemplateAktivitas"))) + "\">" + caption + "</a>";
                item.Visible = (templateAktivitas.DetailAdd && Security.AllowAdd(CurrentProjectID + "TemplateProses") && Security.CanAdd);
                if (item.Visible) {
                    if (detailTableLink != "")
                        detailTableLink += ",";
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fTemplateProseslist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("UrutanProses")); // DN
                CreateColumnOption(option.Add("NamaProses")); // DN
                CreateColumnOption(option.Add("IdPIC")); // DN
                CreateColumnOption(option.Add("IdTools")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fTemplateProsessrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fTemplateProsessrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fTemplateProseslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(templateProsesList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(templateProsesList.RowCount) + "_TemplateProses");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(templateProsesList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && templateProsesList.RowType == RowType.Add || IsEdit && templateProsesList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldSearchValues()
        {
            ...
            LoadFieldUrutanProsesSearchValues();
            LoadFieldNamaProsesSearchValues();
            LoadFieldIdPICSearchValues();
            LoadFieldIdToolsSearchValues();
        ...
        private void LoadFieldUrutanProsesSearchValues()
        {
            // UrutanProses
            LoadPrimarySearchValueUrutanProses();
            LoadSearchOperatorUrutanProses();
        }

        private void LoadPrimarySearchValueUrutanProses()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UrutanProses[]")) {
                UrutanProses.AdvancedSearch.SearchValue = Get("x_UrutanProses[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                UrutanProses.AdvancedSearch.SearchValue = Get("UrutanProses"); // Default Value // DN
            }
            if (!Empty(UrutanProses.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorUrutanProses()
        {
            if (Query.ContainsKey("z_UrutanProses"))
                UrutanProses.AdvancedSearch.SearchOperator = Get("z_UrutanProses", Config.FilterOptionSeparator);
        }

        private void LoadFieldNamaProsesSearchValues()
        {
            // NamaProses
            LoadPrimarySearchValueNamaProses();
            LoadSearchOperatorNamaProses();
        }

        private void LoadPrimarySearchValueNamaProses()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaProses[]")) {
                NamaProses.AdvancedSearch.SearchValue = Get("x_NamaProses[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NamaProses.AdvancedSearch.SearchValue = Get("NamaProses"); // Default Value // DN
            }
            if (!Empty(NamaProses.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNamaProses()
        {
            if (Query.ContainsKey("z_NamaProses"))
                NamaProses.AdvancedSearch.SearchOperator = Get("z_NamaProses", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldIdToolsSearchValues()
        {
            // IdTools
            LoadPrimarySearchValueIdTools();
            LoadSearchOperatorIdTools();
        }

        private void LoadPrimarySearchValueIdTools()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdTools[]")) {
                IdTools.AdvancedSearch.SearchValue = Get("x_IdTools[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                IdTools.AdvancedSearch.SearchValue = Get("IdTools"); // Default Value // DN
            }
            if (!Empty(IdTools.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIdTools()
        {
            if (Query.ContainsKey("z_IdTools"))
                IdTools.AdvancedSearch.SearchOperator = Get("z_IdTools", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // UrutanProses
                if (UrutanProses.UseFilter && !Empty(UrutanProses.AdvancedSearch.SearchValue)) {
                    UrutanProses.EditValue = ConvertToString(UrutanProses.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // NamaProses
                if (NamaProses.UseFilter && !Empty(NamaProses.AdvancedSearch.SearchValue)) {
                    NamaProses.EditValue = ConvertToString(NamaProses.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // IdTools
                if (IdTools.UseFilter && !Empty(IdTools.AdvancedSearch.SearchValue)) {
                    IdTools.EditValue = ConvertToString(IdTools.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fTemplateProsessrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
        ...
        public async Task ExportData(dynamic? doc)
        {
            ...
            if (Config.ExportMasterRecord && !Empty(MasterFilterFromSession) && CurrentMasterTable == "MasterTemplate") {
                masterTemplate = new MasterTemplateList();
                if (masterTemplate != null) {
                    var c = await GetConnection2Async(masterTemplate.DbId); // Note: Use new connection for master record // DN
                    using var rsmaster = await c.ExecuteReaderAsync(masterTemplate.GetSql(DbMasterFilter)); // Load master record
                    if (rsmaster?.HasRows ?? false) { // DN
                        exportStyle = doc.Style;
                        doc.SetStyle("v"); // Change to vertical
                        if (!IsExport("csv") || Config.ExportMasterRecordForCsv) {
                            doc.Table = masterTemplate;
                            await masterTemplate.ExportDocument(doc, rsmaster, 1, 1);
                            doc.ExportEmptyRow();
                            doc.Table = this;
                        }
