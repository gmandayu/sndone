...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasFormInputDataMobilTangkiEdit
    /// </summary>
    public static SubAktivitasFormInputDataMobilTangkiEdit subAktivitasFormInputDataMobilTangkiEdit
    {
        get => HttpData.Get<SubAktivitasFormInputDataMobilTangkiEdit>("subAktivitasFormInputDataMobilTangkiEdit")!;
        set => HttpData["subAktivitasFormInputDataMobilTangkiEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasFormInputDataMobilTangki
    /// </summary>
    public class SubAktivitasFormInputDataMobilTangkiEdit : SubAktivitasFormInputDataMobilTangkiEditBase
    {
        // Constructor
        public SubAktivitasFormInputDataMobilTangkiEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasFormInputDataMobilTangkiEdit() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasFormInputDataMobilTangkiEditBase : SubAktivitasFormInputDataMobilTangki
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasFormInputDataMobilTangkiEdit";
        ...
        // Constructor
        public SubAktivitasFormInputDataMobilTangkiEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasFormInputDataMobilTangkiEdit";
        ...
        // Constructor
        public SubAktivitasFormInputDataMobilTangkiEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasFormInputDataMobilTangkiList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasFormInputDataMobilTangkiList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasFormInputDataMobilTangkiList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasFormInputDataMobilTangkiEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomJenisProduk() {
            if (JenisProduk.Visible && JenisProduk.Required) {
                if (!JenisProduk.IsDetailKey && Empty(JenisProduk.FormValue)) {
                    JenisProduk.AddErrorMessage(ConvertToString(Nominasi.RequiredErrorMessage).Replace("%s", Nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTanggal() {
            if (Tanggal.Visible && Tanggal.Required) {
                if (!Tanggal.IsDetailKey && Empty(Tanggal.FormValue)) {
                    Tanggal.AddErrorMessage(ConvertToString(Nominasi.RequiredErrorMessage).Replace("%s", Nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorPolisi() {
            if (NomorPolisi.Visible && NomorPolisi.Required) {
                if (!NomorPolisi.IsDetailKey && Empty(NomorPolisi.FormValue)) {
                    NomorPolisi.AddErrorMessage(ConvertToString(Nominasi.RequiredErrorMessage).Replace("%s", Nominasi.Caption));
                }
            }
        }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // NomorPolisi
            NomorPolisi.SetDbValue(rsnew, NomorPolisi.CurrentValue, NomorPolisi.ReadOnly);
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("NomorPolisi", out value)) // NomorPolisi
                NomorPolisi.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasFormInputDataMobilTangkiList")), "", TableVar, true);
            string pageId = "edit";
