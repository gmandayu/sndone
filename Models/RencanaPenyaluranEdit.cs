...
public partial class SnDOne {
    /// <summary>
    /// rencanaPenyaluranEdit
    /// </summary>
    public static RencanaPenyaluranEdit rencanaPenyaluranEdit
    {
        get => HttpData.Get<RencanaPenyaluranEdit>("rencanaPenyaluranEdit")!;
        set => HttpData["rencanaPenyaluranEdit"] = value;
    }

    /// <summary>
    /// Page class for RencanaPenyaluran
    /// </summary>
    public class RencanaPenyaluranEdit : RencanaPenyaluranEditBase
    {
        // Constructor
        public RencanaPenyaluranEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public RencanaPenyaluranEdit() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class RencanaPenyaluranEditBase : RencanaPenyaluran
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "rencanaPenyaluranEdit";
        ...
        // Constructor
        public RencanaPenyaluranEditBase()
        {
        ...
        // Page name
        public string PageName => "RencanaPenyaluranEdit";
        ...
        // Constructor
        public RencanaPenyaluranEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            SetVisibility();
            NomorRencanaPenyaluran.Required = false;
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdRencanaPenyaluran", out rv)) { // DN
                    IdRencanaPenyaluran.FormValue = UrlDecode(rv); // DN
                    IdRencanaPenyaluran.OldValue = IdRencanaPenyaluran.FormValue;
                } else if (CurrentForm.HasValue("x_IdRencanaPenyaluran")) {
                    IdRencanaPenyaluran.FormValue = CurrentForm.GetValue("x_IdRencanaPenyaluran");
                    IdRencanaPenyaluran.OldValue = IdRencanaPenyaluran.FormValue;
                } else if (!Empty(keyValues)) {
                    IdRencanaPenyaluran.OldValue = ConvertToString(keyValues[0]);
                } else {
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdRencanaPenyaluran", out rv)) { // DN
                        IdRencanaPenyaluran.QueryValue = UrlDecode(rv); // DN
                        loadByQuery = true;
                    } else if (Get("IdRencanaPenyaluran", out sv)) {
                        IdRencanaPenyaluran.QueryValue = sv.ToString();
                        loadByQuery = true;
                    } else {
                        IdRencanaPenyaluran.CurrentValue = DbNullValue;
                    }
                }
        ...
        if (postBack) {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                IdRencanaPenyaluran.FormValue = ConvertToString(keyValues[0]);
            }
        }
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("RencanaPenyaluranList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "RencanaPenyaluranList") {
                                ...
                                returnUrl = "RencanaPenyaluranList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                rencanaPenyaluranEdit?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NomorRencanaPenyaluran' before field var 'x_NomorRencanaPenyaluran'
            val = CurrentForm.HasValue("NomorRencanaPenyaluran") ? CurrentForm.GetValue("NomorRencanaPenyaluran") : CurrentForm.GetValue("x_NomorRencanaPenyaluran");
            if (!NomorRencanaPenyaluran.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NomorRencanaPenyaluran") && !CurrentForm.HasValue("x_NomorRencanaPenyaluran")) // DN
                    NomorRencanaPenyaluran.Visible = false; // Disable update for API request
                else
                    NomorRencanaPenyaluran.SetFormValue(val);
            }
            ...
            // Check field name 'IdRencanaPenyaluran' before field var 'x_IdRencanaPenyaluran'
            val = CurrentForm.HasValue("IdRencanaPenyaluran") ? CurrentForm.GetValue("IdRencanaPenyaluran") : CurrentForm.GetValue("x_IdRencanaPenyaluran");
            if (!IdRencanaPenyaluran.IsDetailKey)
                IdRencanaPenyaluran.SetFormValue(val);
        }
        #pragma warning restore 1998
        ...
        {
            IdRencanaPenyaluran.CurrentValue = IdRencanaPenyaluran.FormValue;
            NomorRencanaPenyaluran.CurrentValue = NomorRencanaPenyaluran.FormValue;
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                if (NomorRencanaPenyaluran.Visible && NomorRencanaPenyaluran.Required) {
                    if (!NomorRencanaPenyaluran.IsDetailKey && Empty(NomorRencanaPenyaluran.FormValue)) {
                        NomorRencanaPenyaluran.AddErrorMessage(ConvertToString(NomorRencanaPenyaluran.RequiredErrorMessage).Replace("%s", NomorRencanaPenyaluran.Caption));
                    }
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // LookupPlant
            LookupPlant.SetDbValue(rsnew, LookupPlant.CurrentValue, LookupPlant.ReadOnly);
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("LookupPlant", out value)) // LookupPlant
                LookupPlant.CurrentValue = value;
