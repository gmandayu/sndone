    ...
    public abstract class AbstractExport : AbstractExportBase
    {
        ...
        protected virtual void AdjustPageBreak(ref dynamic doc)
        {
            ...
            if (body != null) {
                ...
                //int cnt = elements.Length; // Accessing elements.Length directly may cause error: 'object' does not contain a definition for 'Length'
                int cnt = 0;
                ...
                foreach (var element in elements) {
                    ...
                    if (i == 0) { // First element, remove page break before content
                        ...
                        if (UseInlineStyles) {
                            ...
                            classes = pageBreak && !avoid ? AppendClass(classes, "break-before-page") : RemoveClass(classes, "break-before-page");
                            classes = RemoveClass(classes, "break-after-page");
                        }
                    } else {
                        string siblingStyle = element.PreviousSibling is AngleSharp.Dom.IElement sibling ? sibling.GetAttribute("style") ?? "" : "";
                        Dictionary<string, string> siblingStyles = siblingStyle.Split(';', StringSplitOptions.RemoveEmptyEntries)
                            .Select(p => p.Split(':'))
                            .Where(p => p.Length == 2)
                            .ToDictionary(p => p[0].Trim(), p => p[1].Trim());
                        if (siblingStyles.TryGetValue("page-break-after", out string? v) && v == "always") // Config.PageBreakHtml
                            avoid = true;
                        if (UseInlineStyles) {
                            if (pageBreak && !avoid)
                                styles["page-break-before"] = "always";
                    ...
                    avoid = ContainsClass(classes, "break-after-avoid");
                    i++;
                }
            }
        }
