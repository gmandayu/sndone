...
public partial class SnDOne {
    ...
    /// masterRegionList
    /// </summary>
    public static MasterRegionList masterRegionList
    {
        get => HttpData.Get<MasterRegionList>("masterRegionList")!;
        set => HttpData["masterRegionList"] = value;
    }

    /// <summary>
    /// Page class for MasterRegion
    /// </summary>
    public class MasterRegionList : MasterRegionListBase
    {
        // Constructor
        public MasterRegionList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterRegionList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterRegionListBase : MasterRegion
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterRegionList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMasterRegionlist";
        ...
        // Constructor
        public MasterRegionListBase(Controller? controller)
        {
            TableName = "MasterRegion";

            // CSS class name as context
            TableVar = "MasterRegion";
        ...
        if (controller != null)
            ...
            // Table object (masterRegion)
            if (masterRegion == null || masterRegion is MasterRegion)
                masterRegion = this;

            // Initialize URLs
            AddUrl = "MasterRegionAdd";
            ...
            MultiDeleteUrl = "MasterRegionDelete";
            MultiUpdateUrl = "MasterRegionUpdate";
        ...
        // Page name
        public string PageName => "MasterRegionList";
        ...
        {
            IdRegion.SetVisibility();
            Region.SetVisibility();
            RegionCode.SetVisibility();
        }

        // Constructor
        public MasterRegionListBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                var result = new Dictionary<string, string> { { "url", GetUrl(url) }, { "modal", "1" } }; // modal=1
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "MasterRegionView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
...
protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("IdRegion") ? dict["IdRegion"] : IdRegion.CurrentValue));
            return key;
        }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(RegionCode.AdvancedSearch.ToJson())); // Field RegionCode
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field RegionCode
            if (filter.TryGetValue("x_RegionCode", out sv)) {
                RestoreFieldFilter(RegionCode, filter, "RegionCode", sv);
            }
        }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, IdRegion, def, true); // IdRegion
            BuildSearchSql(ref where, Region, def, true); // Region
            BuildSearchSql(ref where, RegionCode, def, false); // RegionCode

            // Set up search command
            if (!def && !Empty(where) && (new[] { "", "reset", "resetall" }).Contains(Command))
                Command = "search";
            if (!def && Command == "search") {
                RegionCode.AdvancedSearch.Save(); // RegionCode
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("IdRegion", IdRegion, true, config);
            filterList += ProcessFieldFilter("Region", Region, true, config);
            filterList += ProcessFieldFilter("RegionCode", RegionCode, false, config);
            return filterList;
        }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(IdRegion);
            searchFlds.Add(Region);
            searchFlds.Add(RegionCode);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (IdRegion.AdvancedSearch.IssetSession)
                return true;
            if (Region.AdvancedSearch.IssetSession)
                return true;
            if (RegionCode.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            IdRegion.AdvancedSearch.UnsetSession();
            Region.AdvancedSearch.UnsetSession();
            RegionCode.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            IdRegion.AdvancedSearch.Load();
            Region.AdvancedSearch.Load();
            RegionCode.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(IdRegion, ctrl); // IdRegion
                UpdateSort(Region, ctrl); // Region
                UpdateSort(RegionCode, ctrl); // RegionCode
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MasterRegion"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MasterRegion"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MasterRegion"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMasterRegionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMasterRegionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdRegion.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MasterRegion"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMasterRegionlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("IdRegion")); // DN
                CreateColumnOption(option.Add("Region")); // DN
                CreateColumnOption(option.Add("RegionCode")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMasterRegionsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMasterRegionsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMasterRegionlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_MasterRegion");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(masterRegionList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(masterRegionList.RowCount) + "_MasterRegion");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(masterRegionList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && masterRegionList.RowType == RowType.Add || IsEdit && masterRegionList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadFieldIdRegionSearchValues();
            LoadFieldRegionSearchValues();
            LoadFieldRegionCodeSearchValues();
        }

        private void LoadFieldIdRegionSearchValues()
        {
            // IdRegion
            LoadPrimarySearchValueIdRegion();
            LoadSearchOperatorIdRegion();
        }

        private void LoadPrimarySearchValueIdRegion()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdRegion[]")) {
                IdRegion.AdvancedSearch.SearchValue = Get("x_IdRegion[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                IdRegion.AdvancedSearch.SearchValue = Get("IdRegion"); // Default Value // DN
            }
            if (!Empty(IdRegion.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIdRegion()
        {
            if (Query.ContainsKey("z_IdRegion"))
                IdRegion.AdvancedSearch.SearchOperator = Get("z_IdRegion", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldRegionCodeSearchValues()
        {
            // RegionCode
            LoadPrimarySearchValueRegionCode();
            LoadSearchOperatorRegionCode();
        }

        private void LoadPrimarySearchValueRegionCode()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_RegionCode")) {
                RegionCode.AdvancedSearch.SearchValue = Get("x_RegionCode");
            }
            else if (!IsAddOrEdit) {
                RegionCode.AdvancedSearch.SearchValue = Get("RegionCode"); // Default Value // DN
            }
            if (!Empty(RegionCode.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorRegionCode()
        {
            if (Query.ContainsKey("z_RegionCode"))
                RegionCode.AdvancedSearch.SearchOperator = Get("z_RegionCode");
        }
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            RowSelected(row);
            IdRegion.SetDbValue(row["IdRegion"]);
            Region.SetDbValue(row["Region"]);
            RegionCode.SetDbValue(row["RegionCode"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            var row = new Dictionary<string, object>();
            row.Add("IdRegion", IdRegion.DefaultValue ?? DbNullValue); // DN
            row.Add("Region", Region.DefaultValue ?? DbNullValue); // DN
            row.Add("RegionCode", RegionCode.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // RegionCode
                    ...
                    // IdRegion
                    IdRegion.ViewValue = IdRegion.CurrentValue;
                    IdRegion.ViewCustomAttributes = "";
                    ...
                    // RegionCode
                    RegionCode.ViewValue = ConvertToString(RegionCode.CurrentValue); // DN
                    RegionCode.ViewCustomAttributes = "";

                // IdRegion
                IdRegion.HrefValue = "";
                IdRegion.TooltipValue = "";
                ...
                // RegionCode
                RegionCode.HrefValue = "";
                RegionCode.TooltipValue = "";
            } else if (RowType == RowType.Search) {
                // IdRegion
                if (IdRegion.UseFilter && !Empty(IdRegion.AdvancedSearch.SearchValue)) {
                    IdRegion.EditValue = ConvertToString(IdRegion.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // RegionCode
                RegionCode.SetupEditAttributes();
                if (!RegionCode.Raw)
                    RegionCode.AdvancedSearch.SearchValue = HtmlDecode(RegionCode.AdvancedSearch.SearchValue);
                RegionCode.EditValue = HtmlEncode(RegionCode.AdvancedSearch.SearchValue);
                RegionCode.PlaceHolder = RemoveHtml(RegionCode.Caption);
            }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMasterRegionsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
