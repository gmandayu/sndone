...
public partial class SnDOne {
    ...
    public class FormInputControlRutinSecurityListBase : FormInputControlRutinSecurity
    {
        ...
        // Constructor
        public FormInputControlRutinSecurityListBase(Controller? controller)
        {
        ...
        // Constructor
        public FormInputControlRutinSecurityListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field NamaSecurity
            if (filter.TryGetValue("x_NamaSecurity", out sv)) {
                RestoreFieldFilter(NamaSecurity, filter, "NamaSecurity", sv);
            }
            ...
            // Field Lokasi
            if (filter.TryGetValue("x_Lokasi", out sv)) {
                RestoreFieldFilter(Lokasi, filter, "Lokasi", sv);
            }

            // Field DownloadDokumen
            if (filter.TryGetValue("x_DownloadDokumen", out sv)) {
                RestoreFieldFilter(DownloadDokumen, filter, "DownloadDokumen", sv);
            }

            // Field UploadFoto
            if (filter.TryGetValue("x_UploadFoto", out sv)) {
                RestoreFieldFilter(UploadFoto, filter, "UploadFoto", sv);
            }

            // Field Ketidaksesuaiaan
            if (filter.TryGetValue("x_Ketidaksesuaiaan", out sv)) {
                RestoreFieldFilter(Ketidaksesuaiaan, filter, "Ketidaksesuaiaan", sv);
            }

            // Field Keterangan
            if (filter.TryGetValue("x_Keterangan", out sv)) {
                RestoreFieldFilter(Keterangan, filter, "Keterangan", sv);
            }

            // Field IdPosition
            if (filter.TryGetValue("x_IdPosition", out sv)) {
                RestoreFieldFilter(IdPosition, filter, "IdPosition", sv);
            }
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("NamaSecurity", NamaSecurity, true, config);
            filterList += ProcessFieldFilter("Tanggal", Tanggal, true, config);
            filterList += ProcessFieldFilter("Lokasi", Lokasi, true, config);
            filterList += ProcessFieldFilter("DownloadDokumen", DownloadDokumen, false, config);
            filterList += ProcessFieldFilter("Ketidaksesuaiaan", Ketidaksesuaiaan, false, config);
            ...
            filterList += ProcessFieldFilter("UserInput", UserInput, true, config);
            ...
            filterList += ProcessFieldFilter("LastUpdatedDate", LastUpdatedDate, false, config);
            return filterList;
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldSearchValues()
        {
            ...
            LoadFieldNamaSecuritySearchValues();
            LoadFieldTanggalSearchValues();
            LoadFieldLokasiSearchValues();
            LoadFieldDownloadDokumenSearchValues();
            LoadFieldUploadFotoSearchValues();
            LoadFieldKetidaksesuaiaanSearchValues();
            LoadFieldKeteranganSearchValues();
            LoadFieldIdPositionSearchValues();
            LoadFieldUserInputSearchValues();
        ...
        private void LoadFieldNamaSecuritySearchValues()
        {
            // NamaSecurity
            LoadPrimarySearchValueNamaSecurity();
            LoadSearchOperatorNamaSecurity();
        }
        private void LoadPrimarySearchValueNamaSecurity()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaSecurity[]")) {
        ...
        private void LoadSearchOperatorNamaSecurity()
        {
        ...
        private void LoadFieldLokasiSearchValues()
        {
            // Lokasi
            LoadPrimarySearchValueLokasi();
            LoadSearchOperatorLokasi();
        }
        private void LoadPrimarySearchValueLokasi()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Lokasi[]")) {
        ...
        private void LoadSearchOperatorLokasi()
        {
        ...
        private void LoadPrimarySearchValueDownloadDokumen()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_DownloadDokumen")) {
                DownloadDokumen.AdvancedSearch.SearchValue = Get("x_DownloadDokumen");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorDownloadDokumen()
        {
            if (Query.ContainsKey("z_DownloadDokumen"))
                DownloadDokumen.AdvancedSearch.SearchOperator = Get("z_DownloadDokumen");
        }
        private void LoadFieldUploadFotoSearchValues()
        {
            // UploadFoto
            LoadPrimarySearchValueUploadFoto();
            LoadSearchOperatorUploadFoto();
        }
        private void LoadPrimarySearchValueUploadFoto()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UploadFoto")) {
                UploadFoto.AdvancedSearch.SearchValue = Get("x_UploadFoto");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorUploadFoto()
        {
            if (Query.ContainsKey("z_UploadFoto"))
                UploadFoto.AdvancedSearch.SearchOperator = Get("z_UploadFoto");
        }
        private void LoadFieldKetidaksesuaiaanSearchValues()
        {
            // Ketidaksesuaiaan
            LoadPrimarySearchValueKetidaksesuaiaan();
            LoadSearchOperatorKetidaksesuaiaan();
        }
        private void LoadPrimarySearchValueKetidaksesuaiaan()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Ketidaksesuaiaan")) {
                Ketidaksesuaiaan.AdvancedSearch.SearchValue = Get("x_Ketidaksesuaiaan");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorKetidaksesuaiaan()
        {
            if (Query.ContainsKey("z_Ketidaksesuaiaan"))
                Ketidaksesuaiaan.AdvancedSearch.SearchOperator = Get("z_Ketidaksesuaiaan");
        }
        ...
        private void LoadFieldUserInputSearchValues()
        {
            // UserInput
            LoadPrimarySearchValueUserInput();
            LoadSearchOperatorUserInput();
        }
        private void LoadPrimarySearchValueUserInput()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UserInput[]")) {
        ...
        private void LoadSearchOperatorUserInput()
        {
        ...
        private void LoadPrimarySearchValueLastUpdatedDate()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_LastUpdatedDate")) {
                LastUpdatedDate.AdvancedSearch.SearchValue = Get("x_LastUpdatedDate");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorLastUpdatedDate()
        {
            if (Query.ContainsKey("z_LastUpdatedDate"))
                LastUpdatedDate.AdvancedSearch.SearchOperator = Get("z_LastUpdatedDate");
        }
