...
public partial class SnDOne {
    ...
    public class TemplateProsesGridBase : TemplateProses
    {
        ...
        // Constructor
        public TemplateProsesGridBase(Controller? controller)
        {
        ...
        // Constructor
        public TemplateProsesGridBase() : this(null) { }
        ...
        private void ValidateCustomIdTemplate() {
            if (IdTemplate.Visible && IdTemplate.Required) {
                if (!IdTemplate.IsDetailKey && Empty(IdTemplate.FormValue)) {
                    IdTemplate.AddErrorMessage(ConvertToString(TanggalDiperbarui.RequiredErrorMessage).Replace("%s", TanggalDiperbarui.Caption));
                }
            }
        }
        ...
        private void ValidateCustomUrutanProses() {
            if (UrutanProses.Visible && UrutanProses.Required) {
                if (!UrutanProses.IsDetailKey && Empty(UrutanProses.FormValue)) {
                    UrutanProses.AddErrorMessage(ConvertToString(TanggalDiperbarui.RequiredErrorMessage).Replace("%s", TanggalDiperbarui.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNamaProses() {
            if (NamaProses.Visible && NamaProses.Required) {
                if (!NamaProses.IsDetailKey && Empty(NamaProses.FormValue)) {
                    NamaProses.AddErrorMessage(ConvertToString(TanggalDiperbarui.RequiredErrorMessage).Replace("%s", TanggalDiperbarui.Caption));
                }
            }
        }
        ...
        private void ValidateCustomIdPIC() {
            if (IdPIC.Visible && IdPIC.Required) {
                if (!IdPIC.IsDetailKey && Empty(IdPIC.FormValue)) {
                    IdPIC.AddErrorMessage(ConvertToString(TanggalDiperbarui.RequiredErrorMessage).Replace("%s", TanggalDiperbarui.Caption));
                }
            }
        }
        ...
        private void ValidateCustomIdTools() {
            if (IdTools.Visible && IdTools.Required) {
                if (!IdTools.IsDetailKey && Empty(IdTools.FormValue)) {
                    IdTools.AddErrorMessage(ConvertToString(TanggalDiperbarui.RequiredErrorMessage).Replace("%s", TanggalDiperbarui.Caption));
                }
            }
        }
        ...
        private void ValidateCustomKeterangan() {
            if (Keterangan.Visible && Keterangan.Required) {
                if (!Keterangan.IsDetailKey && Empty(Keterangan.FormValue)) {
                    Keterangan.AddErrorMessage(ConvertToString(TanggalDiperbarui.RequiredErrorMessage).Replace("%s", TanggalDiperbarui.Caption));
                }
            }
        }
        ...
        private void ValidateCustomDibuatOleh() {
            if (DibuatOleh.Visible && DibuatOleh.Required) {
                if (!DibuatOleh.IsDetailKey && Empty(DibuatOleh.FormValue)) {
                    DibuatOleh.AddErrorMessage(ConvertToString(TanggalDiperbarui.RequiredErrorMessage).Replace("%s", TanggalDiperbarui.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTanggalDibuat() {
            if (TanggalDibuat.Visible && TanggalDibuat.Required) {
                if (!TanggalDibuat.IsDetailKey && Empty(TanggalDibuat.FormValue)) {
                    TanggalDibuat.AddErrorMessage(ConvertToString(TanggalDiperbarui.RequiredErrorMessage).Replace("%s", TanggalDiperbarui.Caption));
                }
            }
        }
        ...
        private void ValidateCustomDiperbaruiOleh() {
            if (DiperbaruiOleh.Visible && DiperbaruiOleh.Required) {
                if (!DiperbaruiOleh.IsDetailKey && Empty(DiperbaruiOleh.FormValue)) {
                    DiperbaruiOleh.AddErrorMessage(ConvertToString(TanggalDiperbarui.RequiredErrorMessage).Replace("%s", TanggalDiperbarui.Caption));
                }
            }
        }
