...
public partial class SnDOne {
    ...
    public class SubAktivitasLogbookPenerimaanPenyaluranEditBase : SubAktivitasLogbookPenerimaanPenyaluran
    {
        ...
        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranEditBase()
        {
        ...
        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasLogbookPenerimaanPenyaluranList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasLogbookPenerimaanPenyaluranList") {
                                ...
                                returnUrl = "SubAktivitasLogbookPenerimaanPenyaluranList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'LO' before field var 'x_LO'
            val = CurrentForm.HasValue("LO") ? CurrentForm.GetValue("LO") : CurrentForm.GetValue("x_LO");
            if (!LO.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("LO") && !CurrentForm.HasValue("x_LO")) // DN
                    LO.Visible = false; // Disable update for API request
                else
                    LO.SetFormValue(val);
            }
            ...
            // Check field name 'BayNumber' before field var 'x_BayNumber'
            val = CurrentForm.HasValue("BayNumber") ? CurrentForm.GetValue("BayNumber") : CurrentForm.GetValue("x_BayNumber");
            if (!BayNumber.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("BayNumber") && !CurrentForm.HasValue("x_BayNumber")) // DN
                    BayNumber.Visible = false; // Disable update for API request
                else
                    BayNumber.SetFormValue(val);
            }

            // Check field name 'NomorKompartment' before field var 'x_NomorKompartment'
            val = CurrentForm.HasValue("NomorKompartment") ? CurrentForm.GetValue("NomorKompartment") : CurrentForm.GetValue("x_NomorKompartment");
            if (!NomorKompartment.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NomorKompartment") && !CurrentForm.HasValue("x_NomorKompartment")) // DN
                    NomorKompartment.Visible = false; // Disable update for API request
                else
                    NomorKompartment.SetFormValue(val);
            }

            // Check field name 'InLineBlending' before field var 'x_InLineBlending'
            val = CurrentForm.HasValue("InLineBlending") ? CurrentForm.GetValue("InLineBlending") : CurrentForm.GetValue("x_InLineBlending");
            if (!InLineBlending.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("InLineBlending") && !CurrentForm.HasValue("x_InLineBlending")) // DN
                    InLineBlending.Visible = false; // Disable update for API request
                else
                    InLineBlending.SetFormValue(val);
            }

            // Check field name 'NomorTangki2' before field var 'x_NomorTangki2'
            val = CurrentForm.HasValue("NomorTangki2") ? CurrentForm.GetValue("NomorTangki2") : CurrentForm.GetValue("x_NomorTangki2");
            if (!NomorTangki2.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NomorTangki2") && !CurrentForm.HasValue("x_NomorTangki2")) // DN
                    NomorTangki2.Visible = false; // Disable update for API request
                else
                    NomorTangki2.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (LO.Visible && LO.Required) {
                    if (!LO.IsDetailKey && Empty(LO.FormValue)) {
                        LO.AddErrorMessage(ConvertToString(LO.RequiredErrorMessage).Replace("%s", LO.Caption));
                    }
                }
                ...
                if (BayNumber.Visible && BayNumber.Required) {
                    if (!BayNumber.IsDetailKey && Empty(BayNumber.FormValue)) {
                        BayNumber.AddErrorMessage(ConvertToString(BayNumber.RequiredErrorMessage).Replace("%s", BayNumber.Caption));
                    }
                }
                if (NomorKompartment.Visible && NomorKompartment.Required) {
                    if (!NomorKompartment.IsDetailKey && Empty(NomorKompartment.FormValue)) {
                        NomorKompartment.AddErrorMessage(ConvertToString(NomorKompartment.RequiredErrorMessage).Replace("%s", NomorKompartment.Caption));
                    }
                }
                if (InLineBlending.Visible && InLineBlending.Required) {
                    if (Empty(InLineBlending.FormValue)) {
                        InLineBlending.AddErrorMessage(ConvertToString(InLineBlending.RequiredErrorMessage).Replace("%s", InLineBlending.Caption));
                    }
                }
                if (NomorTangki2.Visible && NomorTangki2.Required) {
                    if (!NomorTangki2.IsDetailKey && Empty(NomorTangki2.FormValue)) {
                        NomorTangki2.AddErrorMessage(ConvertToString(NomorTangki2.RequiredErrorMessage).Replace("%s", NomorTangki2.Caption));
                    }
                }
