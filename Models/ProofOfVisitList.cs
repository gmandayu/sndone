...
public partial class SnDOne {
    ...
    /// proofOfVisitList
    /// </summary>
    public static ProofOfVisitList proofOfVisitList
    {
        get => HttpData.Get<ProofOfVisitList>("proofOfVisitList")!;
        set => HttpData["proofOfVisitList"] = value;
    }

    /// <summary>
    /// Page class for ProofOfVisit
    /// </summary>
    public class ProofOfVisitList : ProofOfVisitListBase
    {
        // Constructor
        public ProofOfVisitList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ProofOfVisitList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class ProofOfVisitListBase : ProofOfVisit
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "proofOfVisitList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fProofOfVisitlist";
        ...
        // Constructor
        public ProofOfVisitListBase(Controller? controller)
        {
            TableName = "ProofOfVisit";
        ...
        if (controller != null)
            ...
            // Table object (proofOfVisit)
            if (proofOfVisit == null || proofOfVisit is ProofOfVisit)
                proofOfVisit = this;

            // Initialize URLs
            AddUrl = "ProofOfVisitAdd";
            ...
            MultiDeleteUrl = "ProofOfVisitDelete";
            MultiUpdateUrl = "ProofOfVisitUpdate";
        ...
        // Page name
        public string PageName => "ProofOfVisitList";
        ...
        {
            IdProofOfVisit.Visible = false;
            NomorProofOfVisit.Visible = false;
            LinkRedirect.SetVisibility();
            LookupPlant.Visible = false;
            Plant.SetVisibility();
            JumlahKetidaksesuaiaan.SetVisibility();
            TotalPengisian.SetVisibility();
        ...
        // Constructor
        public ProofOfVisitListBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                var result = new Dictionary<string, string> { { "url", GetUrl(url) }, { "modal", "1" } }; // modal=1
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "ProofOfVisitView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
...
protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("IdProofOfVisit") ? dict["IdProofOfVisit"] : IdProofOfVisit.CurrentValue));
            return key;
        }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, TotalPengisian, def, true); // TotalPengisian
            BuildSearchSql(ref where, Catatan, def, true); // Catatan
            BuildSearchSql(ref where, DibuatOleh, def, true); // DibuatOleh
            BuildSearchSql(ref where, TanggalDibuat, def, false); // TanggalDibuat
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("TotalPengisian", TotalPengisian, true, config);
            filterList += ProcessFieldFilter("Catatan", Catatan, true, config);
            filterList += ProcessFieldFilter("DibuatOleh", DibuatOleh, true, config);
            filterList += ProcessFieldFilter("TanggalDibuat", TanggalDibuat, false, config);
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(JumlahKetidaksesuaiaan);
            searchFlds.Add(TotalPengisian);
        ...
        protected bool CheckSearchParms() {
            ...
            if (Plant.AdvancedSearch.IssetSession)
                return true;
            if (TotalPengisian.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            TotalPengisian.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            TotalPengisian.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(TotalPengisian, ctrl); // TotalPengisian
                UpdateSort(Catatan, ctrl); // Catatan
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""ProofOfVisit"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""ProofOfVisit"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""ProofOfVisit"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fProofOfVisitlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fProofOfVisitlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdProofOfVisit.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""ProofOfVisit"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fProofOfVisitlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("TotalPengisian")); // DN
                CreateColumnOption(option.Add("Catatan")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fProofOfVisitsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fProofOfVisitsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fProofOfVisitlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_ProofOfVisit");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(proofOfVisitList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(proofOfVisitList.RowCount) + "_ProofOfVisit");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(proofOfVisitList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && proofOfVisitList.RowType == RowType.Add || IsEdit && proofOfVisitList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldSearchValues()
        {
            ...
            LoadFieldTotalPengisianSearchValues();
        ...
        private void LoadFieldTotalPengisianSearchValues()
        {
            // TotalPengisian
            LoadPrimarySearchValueTotalPengisian();
            LoadSearchOperatorTotalPengisian();
        }

        private void LoadPrimarySearchValueTotalPengisian()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TotalPengisian[]")) {
                TotalPengisian.AdvancedSearch.SearchValue = Get("x_TotalPengisian[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                TotalPengisian.AdvancedSearch.SearchValue = Get("TotalPengisian"); // Default Value // DN
            }
            if (!Empty(TotalPengisian.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTotalPengisian()
        {
            if (Query.ContainsKey("z_TotalPengisian"))
                TotalPengisian.AdvancedSearch.SearchOperator = Get("z_TotalPengisian", Config.FilterOptionSeparator);
        }
        ...
        private void LoadPrimarySearchValueTanggalDibuat()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TanggalDibuat")) {
                TanggalDibuat.AdvancedSearch.SearchValue = Get("x_TanggalDibuat");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorTanggalDibuat()
        {
            if (Query.ContainsKey("z_TanggalDibuat"))
                TanggalDibuat.AdvancedSearch.SearchOperator = Get("z_TanggalDibuat");
        }

        private void LoadSecondarySearchValuesTanggalDibuat()
        {
            if (Query.ContainsKey("v_TanggalDibuat"))
                TanggalDibuat.AdvancedSearch.SearchCondition = Get("v_TanggalDibuat");
            if (Query.ContainsKey("y_TanggalDibuat"))
                TanggalDibuat.AdvancedSearch.SearchValue2 = Get("y_TanggalDibuat");
            if (!Empty(TanggalDibuat.AdvancedSearch.SearchValue2) && Command == "")
                Command = "search";
            if (Query.ContainsKey("w_TanggalDibuat"))
                TanggalDibuat.AdvancedSearch.SearchOperator2 = Get("w_TanggalDibuat");
        }
        ...
        protected Dictionary<string, object> NewRow() {
            var row = new Dictionary<string, object>();
            row.Add("IdProofOfVisit", IdProofOfVisit.DefaultValue ?? DbNullValue); // DN
            row.Add("NomorProofOfVisit", NomorProofOfVisit.DefaultValue ?? DbNullValue); // DN
            ...
            row.Add("TotalPengisian", TotalPengisian.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (!JumlahKetidaksesuaiaan.Raw)
                    JumlahKetidaksesuaiaan.AdvancedSearch.SearchValue = HtmlDecode(JumlahKetidaksesuaiaan.AdvancedSearch.SearchValue);
                JumlahKetidaksesuaiaan.EditValue = HtmlEncode(JumlahKetidaksesuaiaan.AdvancedSearch.SearchValue);
                JumlahKetidaksesuaiaan.PlaceHolder = RemoveHtml(JumlahKetidaksesuaiaan.Caption);

                // TotalPengisian
                if (TotalPengisian.UseFilter && !Empty(TotalPengisian.AdvancedSearch.SearchValue)) {
                    TotalPengisian.EditValue = ConvertToString(TotalPengisian.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // Catatan
                if (Catatan.UseFilter && !Empty(Catatan.AdvancedSearch.SearchValue)) {
                    Catatan.EditValue = ConvertToString(Catatan.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fProofOfVisitsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
