...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasDataGerbongKetelRtwList
    /// </summary>
    public static SubAktivitasDataGerbongKetelRtwList subAktivitasDataGerbongKetelRtwList
    {
        get => HttpData.Get<SubAktivitasDataGerbongKetelRtwList>("subAktivitasDataGerbongKetelRtwList")!;
        set => HttpData["subAktivitasDataGerbongKetelRtwList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasDataGerbongKetelRTW
    /// </summary>
    public class SubAktivitasDataGerbongKetelRtwList : SubAktivitasDataGerbongKetelRtwListBase
    {
        // Constructor
        public SubAktivitasDataGerbongKetelRtwList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasDataGerbongKetelRtwList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasDataGerbongKetelRtwListBase : SubAktivitasDataGerbongKetelRtw
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasDataGerbongKetelRtwList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasDataGerbongKetelRTWlist";
        ...
        // Constructor
        public SubAktivitasDataGerbongKetelRtwListBase()
        {
            TableName = "SubAktivitasDataGerbongKetelRTW";

            // CSS class name as context
            TableVar = "SubAktivitasDataGerbongKetelRTW";
            ...
            // Table object (subAktivitasDataGerbongKetelRtw)
            if (subAktivitasDataGerbongKetelRtw == null || subAktivitasDataGerbongKetelRtw is SubAktivitasDataGerbongKetelRtw)
                subAktivitasDataGerbongKetelRtw = this;

            // Initialize URLs
            AddUrl = "SubAktivitasDataGerbongKetelRtwAdd";
            ...
            MultiDeleteUrl = "SubAktivitasDataGerbongKetelRtwDelete";
            MultiUpdateUrl = "SubAktivitasDataGerbongKetelRtwUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasDataGerbongKetelRtwList";
        ...
        public void SetVisibility()
        {
            ...
            TanggalTiba.SetVisibility();
            Nominasi.SetVisibility();
            userInput.SetVisibility();
            etlDate.SetVisibility();
            DetailGerbong.SetVisibility();
            DetailQuantity.SetVisibility();
        }

        // Constructor
        public SubAktivitasDataGerbongKetelRtwListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasDataGerbongKetelRtwView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasDataGerbongKetelRTWgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasDataGerbongKetelRtwList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Shipment.AdvancedSearch.ToJson())); // Field Shipment
            filters.Merge(JObject.Parse(JenisProduk.AdvancedSearch.ToJson())); // Field JenisProduk
            filters.Merge(JObject.Parse(TanggalTiba.AdvancedSearch.ToJson())); // Field TanggalTiba
            filters.Merge(JObject.Parse(Nominasi.AdvancedSearch.ToJson())); // Field Nominasi
            ...
            filters.Merge(JObject.Parse(DetailGerbong.AdvancedSearch.ToJson())); // Field DetailGerbong
            filters.Merge(JObject.Parse(DetailQuantity.AdvancedSearch.ToJson())); // Field DetailQuantity
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Shipment
            if (filter?.TryGetValue("x_Shipment", out sv) ?? false) {
                Shipment.AdvancedSearch.SearchValue = sv;
                Shipment.AdvancedSearch.SearchOperator = filter["z_Shipment"];
                Shipment.AdvancedSearch.SearchCondition = filter["v_Shipment"];
                Shipment.AdvancedSearch.SearchValue2 = filter["y_Shipment"];
                Shipment.AdvancedSearch.SearchOperator2 = filter["w_Shipment"];
                Shipment.AdvancedSearch.Save();
            }

            // Field JenisProduk
            if (filter?.TryGetValue("x_JenisProduk", out sv) ?? false) {
                JenisProduk.AdvancedSearch.SearchValue = sv;
                JenisProduk.AdvancedSearch.SearchOperator = filter["z_JenisProduk"];
                JenisProduk.AdvancedSearch.SearchCondition = filter["v_JenisProduk"];
                JenisProduk.AdvancedSearch.SearchValue2 = filter["y_JenisProduk"];
                JenisProduk.AdvancedSearch.SearchOperator2 = filter["w_JenisProduk"];
                JenisProduk.AdvancedSearch.Save();
            }

            // Field TanggalTiba
            if (filter?.TryGetValue("x_TanggalTiba", out sv) ?? false) {
                TanggalTiba.AdvancedSearch.SearchValue = sv;
                TanggalTiba.AdvancedSearch.SearchOperator = filter["z_TanggalTiba"];
                TanggalTiba.AdvancedSearch.SearchCondition = filter["v_TanggalTiba"];
                TanggalTiba.AdvancedSearch.SearchValue2 = filter["y_TanggalTiba"];
                TanggalTiba.AdvancedSearch.SearchOperator2 = filter["w_TanggalTiba"];
                TanggalTiba.AdvancedSearch.Save();
            }

            // Field Nominasi
            if (filter?.TryGetValue("x_Nominasi", out sv) ?? false) {
                Nominasi.AdvancedSearch.SearchValue = sv;
                Nominasi.AdvancedSearch.SearchOperator = filter["z_Nominasi"];
                Nominasi.AdvancedSearch.SearchCondition = filter["v_Nominasi"];
                Nominasi.AdvancedSearch.SearchValue2 = filter["y_Nominasi"];
                Nominasi.AdvancedSearch.SearchOperator2 = filter["w_Nominasi"];
                Nominasi.AdvancedSearch.Save();
            }
            ...
            // Field DetailGerbong
            if (filter?.TryGetValue("x_DetailGerbong", out sv) ?? false) {
                DetailGerbong.AdvancedSearch.SearchValue = sv;
                DetailGerbong.AdvancedSearch.SearchOperator = filter["z_DetailGerbong"];
                DetailGerbong.AdvancedSearch.SearchCondition = filter["v_DetailGerbong"];
                DetailGerbong.AdvancedSearch.SearchValue2 = filter["y_DetailGerbong"];
                DetailGerbong.AdvancedSearch.SearchOperator2 = filter["w_DetailGerbong"];
                DetailGerbong.AdvancedSearch.Save();
            }

            // Field DetailQuantity
            if (filter?.TryGetValue("x_DetailQuantity", out sv) ?? false) {
                DetailQuantity.AdvancedSearch.SearchValue = sv;
                DetailQuantity.AdvancedSearch.SearchOperator = filter["z_DetailQuantity"];
                DetailQuantity.AdvancedSearch.SearchCondition = filter["v_DetailQuantity"];
                DetailQuantity.AdvancedSearch.SearchValue2 = filter["y_DetailQuantity"];
                DetailQuantity.AdvancedSearch.SearchOperator2 = filter["w_DetailQuantity"];
                DetailQuantity.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(JenisProduk);
            searchFlds.Add(Nominasi);
            searchFlds.Add(userInput);
            searchFlds.Add(DetailGerbong);
            searchFlds.Add(DetailQuantity);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Shipment, ctrl); // Shipment
                UpdateSort(JenisProduk, ctrl); // JenisProduk
                UpdateSort(TanggalTiba, ctrl); // TanggalTiba
                UpdateSort(Nominasi, ctrl); // Nominasi
                UpdateSort(userInput, ctrl); // userInput
                UpdateSort(etlDate, ctrl); // etlDate
                UpdateSort(DetailGerbong, ctrl); // DetailGerbong
                UpdateSort(DetailQuantity, ctrl); // DetailQuantity
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""SubAktivitasDataGerbongKetelRTW"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasDataGerbongKetelRTW"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""SubAktivitasDataGerbongKetelRTW"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasDataGerbongKetelRTWlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasDataGerbongKetelRTWlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""SubAktivitasDataGerbongKetelRTW"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fSubAktivitasDataGerbongKetelRTWlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Shipment")); // DN
                CreateColumnOption(option.Add("JenisProduk")); // DN
                CreateColumnOption(option.Add("TanggalTiba")); // DN
                CreateColumnOption(option.Add("Nominasi")); // DN
                CreateColumnOption(option.Add("userInput")); // DN
                CreateColumnOption(option.Add("etlDate")); // DN
                CreateColumnOption(option.Add("DetailGerbong")); // DN
                CreateColumnOption(option.Add("DetailQuantity")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasDataGerbongKetelRTWsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasDataGerbongKetelRTWsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasDataGerbongKetelRTWlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasDataGerbongKetelRTW");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasDataGerbongKetelRtwList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasDataGerbongKetelRtwList.RowCount) + "_SubAktivitasDataGerbongKetelRTW");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasDataGerbongKetelRtwList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasDataGerbongKetelRtwList.RowType == RowType.Add || IsEdit && subAktivitasDataGerbongKetelRtwList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            TanggalTiba.SetDbValue(row["TanggalTiba"]);
            Nominasi.SetDbValue(row["Nominasi"]);
            userInput.SetDbValue(row["userInput"]);
            etlDate.SetDbValue(row["etlDate"]);
            DetailGerbong.SetDbValue(row["DetailGerbong"]);
            DetailQuantity.SetDbValue(row["DetailQuantity"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("TanggalTiba", TanggalTiba.DefaultValue ?? DbNullValue); // DN
            row.Add("Nominasi", Nominasi.DefaultValue ?? DbNullValue); // DN
            row.Add("userInput", userInput.DefaultValue ?? DbNullValue); // DN
            row.Add("etlDate", etlDate.DefaultValue ?? DbNullValue); // DN
            row.Add("DetailGerbong", DetailGerbong.DefaultValue ?? DbNullValue); // DN
            row.Add("DetailQuantity", DetailQuantity.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // JenisProduk
                JenisProduk.ViewValue = ConvertToString(JenisProduk.CurrentValue); // DN
                string curVal4 = ConvertToString(JenisProduk.CurrentValue);
                if (!Empty(curVal4)) {
                    if (JenisProduk.Lookup != null && IsDictionary(JenisProduk.Lookup?.Options) && JenisProduk.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        JenisProduk.ViewValue = JenisProduk.LookupCacheOption(curVal4);
                    } else { // Lookup from database // DN
                        string filterWrk4 = SearchFilter(JenisProduk.Lookup?.GetTable()?.Fields["NoProduk"].SearchExpression, "=", JenisProduk.CurrentValue, JenisProduk.Lookup?.GetTable()?.Fields["NoProduk"].SearchDataType, "");
                        string? sqlWrk4 = JenisProduk.Lookup?.GetSql(false, filterWrk4, null, this, true, true);
                        ...
                        if (rswrk4?.Count > 0 && JenisProduk.Lookup != null) { // Lookup values found
                            var listwrk = JenisProduk.Lookup?.RenderViewRow(rswrk4[0]);
                ...
                // TanggalTiba
                TanggalTiba.ViewValue = TanggalTiba.CurrentValue;
                TanggalTiba.ViewValue = FormatDateTime(TanggalTiba.ViewValue, TanggalTiba.FormatPattern);
                TanggalTiba.ViewCustomAttributes = "";

                // Nominasi
                Nominasi.ViewValue = ConvertToString(Nominasi.CurrentValue); // DN
                Nominasi.ViewCustomAttributes = "";
                ...
                // DetailGerbong
                DetailGerbong.ViewValue = ConvertToString(DetailGerbong.CurrentValue); // DN
                DetailGerbong.ViewCustomAttributes = "";

                // DetailQuantity
                DetailQuantity.ViewValue = ConvertToString(DetailQuantity.CurrentValue); // DN
                DetailQuantity.ViewCustomAttributes = "";
                ...
                // TanggalTiba
                TanggalTiba.HrefValue = "";
                TanggalTiba.TooltipValue = "";

                // Nominasi
                Nominasi.HrefValue = "";
                Nominasi.TooltipValue = "";
                ...
                // DetailGerbong
                DetailGerbong.HrefValue = "";
                DetailGerbong.TooltipValue = "";

                // DetailQuantity
                DetailQuantity.HrefValue = "";
                DetailQuantity.TooltipValue = "";
            }
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fSubAktivitasDataGerbongKetelRTWlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fSubAktivitasDataGerbongKetelRTWlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fSubAktivitasDataGerbongKetelRTWlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
