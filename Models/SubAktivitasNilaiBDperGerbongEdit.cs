...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiBDperGerbongEdit
    /// </summary>
    public static SubAktivitasNilaiBDperGerbongEdit subAktivitasNilaiBDperGerbongEdit
    {
        get => HttpData.Get<SubAktivitasNilaiBDperGerbongEdit>("subAktivitasNilaiBDperGerbongEdit")!;
        set => HttpData["subAktivitasNilaiBDperGerbongEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiBDperGerbong
    /// </summary>
    public class SubAktivitasNilaiBDperGerbongEdit : SubAktivitasNilaiBDperGerbongEditBase
    {
        // Constructor
        public SubAktivitasNilaiBDperGerbongEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiBDperGerbongEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiBDperGerbongEditBase : SubAktivitasNilaiBDperGerbong
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiBDperGerbongEdit";
        ...
        // Constructor
        public SubAktivitasNilaiBDperGerbongEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiBDperGerbongEdit";
        ...
        // Constructor
        public SubAktivitasNilaiBDperGerbongEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasNilaiBDperGerbongList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "SubAktivitasNilaiBDperGerbongList") {
                        ...
                        returnUrl = "SubAktivitasNilaiBDperGerbongList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasNilaiBDperGerbongEdit?.PageRender();
                    }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorGerbongKertel() {
            if (NomorGerbongKertel.Visible && NomorGerbongKertel.Required) {
                if (!NomorGerbongKertel.IsDetailKey && Empty(NomorGerbongKertel.FormValue)) {
                    NomorGerbongKertel.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomKL_Obs() {
            if (KL_Obs.Visible && KL_Obs.Required) {
                if (!KL_Obs.IsDetailKey && Empty(KL_Obs.FormValue)) {
                    KL_Obs.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomKL_15() {
            if (KL_15.Visible && KL_15.Required) {
                if (!KL_15.IsDetailKey && Empty(KL_15.FormValue)) {
                    KL_15.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBarrels() {
            if (Barrels.Visible && Barrels.Required) {
                if (!Barrels.IsDetailKey && Empty(Barrels.FormValue)) {
                    Barrels.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLT() {
            if (LT.Visible && LT.Required) {
                if (!LT.IsDetailKey && Empty(LT.FormValue)) {
                    LT.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomMT() {
            if (MT.Visible && MT.Required) {
                if (!MT.IsDetailKey && Empty(MT.FormValue)) {
                    MT.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomuserInput() {
            if (userInput.Visible && userInput.Required) {
                if (!userInput.IsDetailKey && Empty(userInput.FormValue)) {
                    userInput.AddErrorMessage(ConvertToString(Produk.RequiredErrorMessage).Replace("%s", Produk.Caption));
                }
            }
        }
