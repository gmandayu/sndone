...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasFormInputNilaiAktualDischargeEdit
    /// </summary>
    public static SubAktivitasFormInputNilaiAktualDischargeEdit subAktivitasFormInputNilaiAktualDischargeEdit
    {
        get => HttpData.Get<SubAktivitasFormInputNilaiAktualDischargeEdit>("subAktivitasFormInputNilaiAktualDischargeEdit")!;
        set => HttpData["subAktivitasFormInputNilaiAktualDischargeEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasFormInputNilaiAktualDischarge
    /// </summary>
    public class SubAktivitasFormInputNilaiAktualDischargeEdit : SubAktivitasFormInputNilaiAktualDischargeEditBase
    {
        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeEdit() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasFormInputNilaiAktualDischargeEditBase : SubAktivitasFormInputNilaiAktualDischarge
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasFormInputNilaiAktualDischargeEdit";
        ...
        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasFormInputNilaiAktualDischargeEdit";
        ...
        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasFormInputNilaiAktualDischargeList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasFormInputNilaiAktualDischargeList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasFormInputNilaiAktualDischargeList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasFormInputNilaiAktualDischargeEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
            ...
            if (RowType == RowType.View) {
                ...
                if (Tujuan.Lookup != null && IsDictionary(Tujuan.Lookup?.Options) && Tujuan.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    ...
                    if (curVal5 == "") {
                        ...
                        filterWrk5 = SearchFilter(Tujuan.Lookup?.GetTable()?.Fields["IdPlant"].SearchExpression, "=", Tujuan.CurrentValue, Tujuan.Lookup?.GetTable()?.Fields["IdPlant"].SearchDataType, "");
                    }
                    string? sqlWrk5 = Tujuan.Lookup?.GetSql(true, filterWrk5, null, this, false, true);
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomKL_Obs() {
            if (KL_Obs.Visible && KL_Obs.Required) {
                if (!KL_Obs.IsDetailKey && Empty(KL_Obs.FormValue)) {
                    KL_Obs.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomKL_15() {
            if (KL_15.Visible && KL_15.Required) {
                if (!KL_15.IsDetailKey && Empty(KL_15.FormValue)) {
                    KL_15.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBarrels() {
            if (Barrels.Visible && Barrels.Required) {
                if (!Barrels.IsDetailKey && Empty(Barrels.FormValue)) {
                    Barrels.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLT() {
            if (LT.Visible && LT.Required) {
                if (!LT.IsDetailKey && Empty(LT.FormValue)) {
                    LT.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomMT() {
            if (MT.Visible && MT.Required) {
                if (!MT.IsDetailKey && Empty(MT.FormValue)) {
                    MT.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomApakahAdaROB() {
            if (ApakahAdaROB.Visible && ApakahAdaROB.Required) {
                if (Empty(ApakahAdaROB.FormValue)) {
                    ApakahAdaROB.AddErrorMessage(ConvertToString(Tujuan.RequiredErrorMessage).Replace("%s", Tujuan.Caption));
                }
            }
        }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // ApakahAdaROB
            ApakahAdaROB.SetDbValue(rsnew, ConvertToBool(ApakahAdaROB.CurrentValue), ApakahAdaROB.ReadOnly);
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("ApakahAdaROB", out value)) // ApakahAdaROB
                ApakahAdaROB.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasFormInputNilaiAktualDischargeList")), "", TableVar, true);
            string pageId = "edit";
