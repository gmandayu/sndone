...
public partial class SnDOne {
    ...
    /// aktivitasList
    /// </summary>
    public static AktivitasList aktivitasList
    {
        get => HttpData.Get<AktivitasList>("aktivitasList")!;
        set => HttpData["aktivitasList"] = value;
    }

    /// <summary>
    /// Page class for Aktivitas
    /// </summary>
    public class AktivitasList : AktivitasListBase
    {
        // Constructor
        public AktivitasList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public AktivitasList() : base()
        {
        }
        ...
        public override void PageLoad() {
            //Log("Page Load");
            IsNominationTankReceivingLineOpen.Visible = false;
            IsNonNominationReceivingLineClosedAndSealed.Visible = false;
            IsTankEmptyAndDry.Visible = false;
            IsDocumentationComplete.Visible = false;
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class AktivitasListBase : Aktivitas
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "aktivitasList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fAktivitaslist";
        ...
        // Constructor
        public AktivitasListBase(Controller? controller)
        {
            TableName = "Aktivitas";
        ...
        if (controller != null)
            ...
            // Initialize URLs
            AddUrl = "AktivitasAdd?" + Config.TableShowDetail + "=";
            ...
            MultiDeleteUrl = "AktivitasDelete";
            MultiUpdateUrl = "AktivitasUpdate";
        ...
        // Page name
        public string PageName => "AktivitasList";
        ...
        public void SetVisibility()
        {
            ...
            sandar_nama_kapal.Visible = false;
            sandar_tgl_tiba.Visible = false;
            sandar_nominasi.Visible = false;
            Produk.Visible = false;
            Shipment.Visible = false;
            Nama_Proses.Visible = false;
            IdDokumen.Visible = false;
            PathFile.Visible = false;
            TanggalUploadDok.Visible = false;
            UserUploadDok.Visible = false;
            NamaDokumen.Visible = false;
            Keterangan.SetVisibility();
            IdRefAnak.Visible = false;
            TableAnak.Visible = false;
        ...
        // Constructor
        public AktivitasListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(TanggalMulai.AdvancedSearch.ToJson())); // Field TanggalMulai
            filters.Merge(JObject.Parse(TanggalSelesai.AdvancedSearch.ToJson())); // Field TanggalSelesai
            ...
            filters.Merge(JObject.Parse(sandar_tgl_tiba.AdvancedSearch.ToJson())); // Field sandar_tgl_tiba
            filters.Merge(JObject.Parse(TanggalUploadDok.AdvancedSearch.ToJson())); // Field TanggalUploadDok
            filters.Merge(JObject.Parse(IsNominationTankReceivingLineOpen.AdvancedSearch.ToJson())); // Field IsNominationTankReceivingLineOpen
            filters.Merge(JObject.Parse(IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.ToJson())); // Field IsNonNominationReceivingLineClosedAndSealed
            filters.Merge(JObject.Parse(IsTankEmptyAndDry.AdvancedSearch.ToJson())); // Field IsTankEmptyAndDry
            filters.Merge(JObject.Parse(IsDocumentationComplete.AdvancedSearch.ToJson())); // Field IsDocumentationComplete
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field TanggalMulai
            if (filter.TryGetValue("x_TanggalMulai", out sv)) {
                RestoreFieldFilter(TanggalMulai, filter, "TanggalMulai", sv);
            }

            // Field TanggalSelesai
            if (filter.TryGetValue("x_TanggalSelesai", out sv)) {
                RestoreFieldFilter(TanggalSelesai, filter, "TanggalSelesai", sv);
            }
            ...
            // Field sandar_tgl_tiba
            if (filter.TryGetValue("x_sandar_tgl_tiba", out sv)) {
                RestoreFieldFilter(sandar_tgl_tiba, filter, "sandar_tgl_tiba", sv);
            }

            // Field TanggalUploadDok
            if (filter.TryGetValue("x_TanggalUploadDok", out sv)) {
                RestoreFieldFilter(TanggalUploadDok, filter, "TanggalUploadDok", sv);
            }

            // Field IsNominationTankReceivingLineOpen
            if (filter.TryGetValue("x_IsNominationTankReceivingLineOpen", out sv)) {
                RestoreFieldFilter(IsNominationTankReceivingLineOpen, filter, "IsNominationTankReceivingLineOpen", sv);
            }

            // Field IsNonNominationReceivingLineClosedAndSealed
            if (filter.TryGetValue("x_IsNonNominationReceivingLineClosedAndSealed", out sv)) {
                RestoreFieldFilter(IsNonNominationReceivingLineClosedAndSealed, filter, "IsNonNominationReceivingLineClosedAndSealed", sv);
            }

            // Field IsTankEmptyAndDry
            if (filter.TryGetValue("x_IsTankEmptyAndDry", out sv)) {
                RestoreFieldFilter(IsTankEmptyAndDry, filter, "IsTankEmptyAndDry", sv);
            }

            // Field IsDocumentationComplete
            if (filter.TryGetValue("x_IsDocumentationComplete", out sv)) {
                RestoreFieldFilter(IsDocumentationComplete, filter, "IsDocumentationComplete", sv);
            }
        }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, No_Referensi, def, true); // No_Referensi
            BuildSearchSql(ref where, IdProses, def, true); // IdProses
            BuildSearchSql(ref where, IdTemplateAktivitas, def, true); // IdTemplateAktivitas
            BuildSearchSql(ref where, Aktivitas2, def, true); // Aktivitas2
            BuildSearchSql(ref where, NamaAktivitas, def, true); // NamaAktivitas
            BuildSearchSql(ref where, PIC, def, true); // PIC
            BuildSearchSql(ref where, TanggalMulai, def, true); // TanggalMulai
            BuildSearchSql(ref where, TanggalSelesai, def, true); // TanggalSelesai
            BuildSearchSql(ref where, StatusAktivitas, def, true); // StatusAktivitas
            ...
            BuildSearchSql(ref where, TipeAktivitas, def, true); // TipeAktivitas
            BuildSearchSql(ref where, sandar_nama_kapal, def, true); // sandar_nama_kapal
            BuildSearchSql(ref where, sandar_tgl_tiba, def, true); // sandar_tgl_tiba
            BuildSearchSql(ref where, sandar_nominasi, def, true); // sandar_nominasi
            BuildSearchSql(ref where, Produk, def, true); // Produk
            BuildSearchSql(ref where, Shipment, def, true); // Shipment
            BuildSearchSql(ref where, Nama_Proses, def, true); // Nama_Proses
            BuildSearchSql(ref where, IdDokumen, def, true); // IdDokumen
            BuildSearchSql(ref where, PathFile, def, true); // PathFile
            BuildSearchSql(ref where, TanggalUploadDok, def, true); // TanggalUploadDok
            BuildSearchSql(ref where, UserUploadDok, def, true); // UserUploadDok
            BuildSearchSql(ref where, NamaDokumen, def, true); // NamaDokumen
            BuildSearchSql(ref where, Keterangan, def, true); // Keterangan
            BuildSearchSql(ref where, IdRefAnak, def, true); // IdRefAnak
            BuildSearchSql(ref where, TableAnak, def, true); // TableAnak
            BuildSearchSql(ref where, TipeProses, def, true); // TipeProses
            BuildSearchSql(ref where, Urutan, def, true); // Urutan
            BuildSearchSql(ref where, IsNominationTankReceivingLineOpen, def, false); // IsNominationTankReceivingLineOpen
            BuildSearchSql(ref where, IsNonNominationReceivingLineClosedAndSealed, def, false); // IsNonNominationReceivingLineClosedAndSealed
            BuildSearchSql(ref where, IsTankEmptyAndDry, def, false); // IsTankEmptyAndDry
            BuildSearchSql(ref where, IsDocumentationComplete, def, false); // IsDocumentationComplete

            // Set up search command
            if (!def && !Empty(where) && (new[] { "", "reset", "resetall" }).Contains(Command))
                Command = "search";
            if (!def && Command == "search") {
                TanggalMulai.AdvancedSearch.Save(); // TanggalMulai
                TanggalSelesai.AdvancedSearch.Save(); // TanggalSelesai
                TanggalDibuat.AdvancedSearch.Save(); // TanggalDibuat
                TanggalDiperbarui.AdvancedSearch.Save(); // TanggalDiperbarui
                sandar_tgl_tiba.AdvancedSearch.Save(); // sandar_tgl_tiba
                TanggalUploadDok.AdvancedSearch.Save(); // TanggalUploadDok
                IsNominationTankReceivingLineOpen.AdvancedSearch.Save(); // IsNominationTankReceivingLineOpen
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.Save(); // IsNonNominationReceivingLineClosedAndSealed
                IsTankEmptyAndDry.AdvancedSearch.Save(); // IsTankEmptyAndDry
                IsDocumentationComplete.AdvancedSearch.Save(); // IsDocumentationComplete
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("No_Referensi", No_Referensi, true, config);
            filterList += ProcessFieldFilter("IdProses", IdProses, true, config);
            filterList += ProcessFieldFilter("Aktivitas", Aktivitas2, true, config);
            filterList += ProcessFieldFilter("PIC", PIC, true, config);
            filterList += ProcessFieldFilter("TanggalMulai", TanggalMulai, true, config);
            filterList += ProcessFieldFilter("TanggalSelesai", TanggalSelesai, true, config);
            filterList += ProcessFieldFilter("StatusAktivitas", StatusAktivitas, true, config);
            ...
            filterList += ProcessFieldFilter("TipeAktivitas", TipeAktivitas, true, config);
            ...
            filterList += ProcessFieldFilter("Urutan", Urutan, true, config);
            filterList += ProcessFieldFilter("IsNominationTankReceivingLineOpen", IsNominationTankReceivingLineOpen, false, config);
            filterList += ProcessFieldFilter("IsNonNominationReceivingLineClosedAndSealed", IsNonNominationReceivingLineClosedAndSealed, false, config);
            filterList += ProcessFieldFilter("IsTankEmptyAndDry", IsTankEmptyAndDry, false, config);
            filterList += ProcessFieldFilter("IsDocumentationComplete", IsDocumentationComplete, false, config);
            return filterList;
        }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(No_Referensi);
            searchFlds.Add(IdProses);
            searchFlds.Add(IdTemplateAktivitas);
            searchFlds.Add(Aktivitas2);
            searchFlds.Add(NamaAktivitas);
            searchFlds.Add(PIC);
            searchFlds.Add(TanggalMulai);
            searchFlds.Add(TanggalSelesai);
            searchFlds.Add(StatusAktivitas);
            searchFlds.Add(DibuatOleh);
            searchFlds.Add(TanggalDibuat);
            searchFlds.Add(DiperbaruiOleh);
            searchFlds.Add(TanggalDiperbarui);
            searchFlds.Add(TipeAktivitas);
            searchFlds.Add(sandar_nama_kapal);
            searchFlds.Add(sandar_tgl_tiba);
            searchFlds.Add(sandar_nominasi);
            searchFlds.Add(Produk);
            searchFlds.Add(Shipment);
            searchFlds.Add(Nama_Proses);
            searchFlds.Add(IdDokumen);
            searchFlds.Add(PathFile);
            searchFlds.Add(TanggalUploadDok);
            searchFlds.Add(UserUploadDok);
            searchFlds.Add(NamaDokumen);
            searchFlds.Add(Keterangan);
            searchFlds.Add(IdRefAnak);
            searchFlds.Add(TableAnak);
            searchFlds.Add(TipeProses);
            searchFlds.Add(Urutan);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (No_Referensi.AdvancedSearch.IssetSession)
                return true;
            if (IdProses.AdvancedSearch.IssetSession)
                return true;
            if (IdTemplateAktivitas.AdvancedSearch.IssetSession)
                return true;
            if (Aktivitas2.AdvancedSearch.IssetSession)
                return true;
            if (NamaAktivitas.AdvancedSearch.IssetSession)
                return true;
            if (PIC.AdvancedSearch.IssetSession)
                return true;
            if (TanggalMulai.AdvancedSearch.IssetSession)
                return true;
            if (TanggalSelesai.AdvancedSearch.IssetSession)
                return true;
            if (StatusAktivitas.AdvancedSearch.IssetSession)
                return true;
            ...
            if (TanggalDiperbarui.AdvancedSearch.IssetSession)
                return true;
            if (TipeAktivitas.AdvancedSearch.IssetSession)
                return true;
            if (sandar_nama_kapal.AdvancedSearch.IssetSession)
                return true;
            if (sandar_tgl_tiba.AdvancedSearch.IssetSession)
                return true;
            if (sandar_nominasi.AdvancedSearch.IssetSession)
                return true;
            if (Produk.AdvancedSearch.IssetSession)
                return true;
            if (Shipment.AdvancedSearch.IssetSession)
                return true;
            if (Nama_Proses.AdvancedSearch.IssetSession)
                return true;
            if (IdDokumen.AdvancedSearch.IssetSession)
                return true;
            if (PathFile.AdvancedSearch.IssetSession)
                return true;
            if (TanggalUploadDok.AdvancedSearch.IssetSession)
                return true;
            if (UserUploadDok.AdvancedSearch.IssetSession)
                return true;
            if (NamaDokumen.AdvancedSearch.IssetSession)
                return true;
            if (Keterangan.AdvancedSearch.IssetSession)
                return true;
            if (IdRefAnak.AdvancedSearch.IssetSession)
                return true;
            if (TableAnak.AdvancedSearch.IssetSession)
                return true;
            if (TipeProses.AdvancedSearch.IssetSession)
                return true;
            if (Urutan.AdvancedSearch.IssetSession)
                return true;
            if (IsNominationTankReceivingLineOpen.AdvancedSearch.IssetSession)
                return true;
            if (IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.IssetSession)
                return true;
            if (IsTankEmptyAndDry.AdvancedSearch.IssetSession)
                return true;
            if (IsDocumentationComplete.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            No_Referensi.AdvancedSearch.UnsetSession();
            IdProses.AdvancedSearch.UnsetSession();
            IdTemplateAktivitas.AdvancedSearch.UnsetSession();
            Aktivitas2.AdvancedSearch.UnsetSession();
            NamaAktivitas.AdvancedSearch.UnsetSession();
            PIC.AdvancedSearch.UnsetSession();
            TanggalMulai.AdvancedSearch.UnsetSession();
            TanggalSelesai.AdvancedSearch.UnsetSession();
            StatusAktivitas.AdvancedSearch.UnsetSession();
            ...
            TipeAktivitas.AdvancedSearch.UnsetSession();
            sandar_nama_kapal.AdvancedSearch.UnsetSession();
            sandar_tgl_tiba.AdvancedSearch.UnsetSession();
            sandar_nominasi.AdvancedSearch.UnsetSession();
            Produk.AdvancedSearch.UnsetSession();
            Shipment.AdvancedSearch.UnsetSession();
            Nama_Proses.AdvancedSearch.UnsetSession();
            IdDokumen.AdvancedSearch.UnsetSession();
            PathFile.AdvancedSearch.UnsetSession();
            TanggalUploadDok.AdvancedSearch.UnsetSession();
            UserUploadDok.AdvancedSearch.UnsetSession();
            NamaDokumen.AdvancedSearch.UnsetSession();
            Keterangan.AdvancedSearch.UnsetSession();
            IdRefAnak.AdvancedSearch.UnsetSession();
            TableAnak.AdvancedSearch.UnsetSession();
            TipeProses.AdvancedSearch.UnsetSession();
            Urutan.AdvancedSearch.UnsetSession();
            IsNominationTankReceivingLineOpen.AdvancedSearch.UnsetSession();
            IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.UnsetSession();
            IsTankEmptyAndDry.AdvancedSearch.UnsetSession();
            IsDocumentationComplete.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            No_Referensi.AdvancedSearch.Load();
            IdProses.AdvancedSearch.Load();
            IdTemplateAktivitas.AdvancedSearch.Load();
            Aktivitas2.AdvancedSearch.Load();
            NamaAktivitas.AdvancedSearch.Load();
            PIC.AdvancedSearch.Load();
            TanggalMulai.AdvancedSearch.Load();
            TanggalSelesai.AdvancedSearch.Load();
            StatusAktivitas.AdvancedSearch.Load();
            ...
            TipeAktivitas.AdvancedSearch.Load();
            sandar_nama_kapal.AdvancedSearch.Load();
            sandar_tgl_tiba.AdvancedSearch.Load();
            sandar_nominasi.AdvancedSearch.Load();
            Produk.AdvancedSearch.Load();
            Shipment.AdvancedSearch.Load();
            Nama_Proses.AdvancedSearch.Load();
            IdDokumen.AdvancedSearch.Load();
            PathFile.AdvancedSearch.Load();
            TanggalUploadDok.AdvancedSearch.Load();
            UserUploadDok.AdvancedSearch.Load();
            NamaDokumen.AdvancedSearch.Load();
            Keterangan.AdvancedSearch.Load();
            IdRefAnak.AdvancedSearch.Load();
            TableAnak.AdvancedSearch.Load();
            TipeProses.AdvancedSearch.Load();
            Urutan.AdvancedSearch.Load();
            IsNominationTankReceivingLineOpen.AdvancedSearch.Load();
            IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.Load();
            IsTankEmptyAndDry.AdvancedSearch.Load();
            IsDocumentationComplete.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = Urutan.Expression + " ASC" + ", " + IdAktivitas.Expression + " ASC"; // Set up default sort
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(No_Referensi, ctrl); // No_Referensi
                UpdateSort(IdProses, ctrl); // IdProses
                UpdateSort(Aktivitas2, ctrl); // Aktivitas
                UpdateSort(PIC, ctrl); // PIC
                UpdateSort(TanggalMulai, ctrl); // TanggalMulai
                UpdateSort(TanggalSelesai, ctrl); // TanggalSelesai
                UpdateSort(StatusAktivitas, ctrl); // StatusAktivitas
                ...
                UpdateSort(TipeAktivitas, ctrl); // TipeAktivitas
                UpdateSort(Keterangan, ctrl); // Keterangan
                UpdateSort(TipeProses, ctrl); // TipeProses
                UpdateSort(Urutan, ctrl); // Urutan
                UpdateSort(IsNominationTankReceivingLineOpen, ctrl); // IsNominationTankReceivingLineOpen
                UpdateSort(IsNonNominationReceivingLineClosedAndSealed, ctrl); // IsNonNominationReceivingLineClosedAndSealed
                UpdateSort(IsTankEmptyAndDry, ctrl); // IsTankEmptyAndDry
                UpdateSort(IsDocumentationComplete, ctrl); // IsDocumentationComplete
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    No_Referensi.Sort = "";
                    IdProses.Sort = "";
                    IdTemplateAktivitas.Sort = "";
                    Aktivitas2.Sort = "";
                    NamaAktivitas.Sort = "";
                    PIC.Sort = "";
                    TanggalMulai.Sort = "";
                    TanggalSelesai.Sort = "";
                    StatusAktivitas.Sort = "";
                    DibuatOleh.Sort = "";
                    TanggalDibuat.Sort = "";
                    DiperbaruiOleh.Sort = "";
                    TanggalDiperbarui.Sort = "";
                    TipeAktivitas.Sort = "";
                    sandar_nama_kapal.Sort = "";
                    sandar_tgl_tiba.Sort = "";
                    sandar_nominasi.Sort = "";
                    Produk.Sort = "";
                    Shipment.Sort = "";
                    Nama_Proses.Sort = "";
                    IdDokumen.Sort = "";
                    PathFile.Sort = "";
                    TanggalUploadDok.Sort = "";
                    UserUploadDok.Sort = "";
                    NamaDokumen.Sort = "";
                    Keterangan.Sort = "";
                    IdRefAnak.Sort = "";
                    ...
                    TipeProses.Sort = "";
                    Urutan.Sort = "";
                    IsNominationTankReceivingLineOpen.Sort = "";
                    IsNonNominationReceivingLineClosedAndSealed.Sort = "";
                    IsTankEmptyAndDry.Sort = "";
                    IsDocumentationComplete.Sort = "";
                }
        ...
        protected async Task SetupListOptions() {
            ...
            // "detail_AktivitasDokumen"
            item = ListOptions.Add("detail_AktivitasDokumen");
            item.CssClass = "text-nowrap";
            item.Visible = Security.AllowList(CurrentProjectID + "AktivitasDokumen");
            item.OnLeft = true;
            ...
            if (ShowMultipleDetails) {
                ...
                item.OnLeft = true;
                item.ShowInButtonGroup = false;
                ListOptions.HideDetailItems();
            }

            // Set up detail pages
            var _pages = new SubPages();
            _pages.Add("AktivitasDokumen");
            DetailPages = _pages;
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""Aktivitas"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fAktivitaslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fAktivitaslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            // "detail_AktivitasDokumen"
            listOption = ListOptions["detail_AktivitasDokumen"];
            isVisible = Security.AllowList(CurrentProjectID + "AktivitasDokumen");
            if (isVisible) {
                string caption, title, url;
                var body = Language.Phrase("DetailLink") + Language.TablePhrase("AktivitasDokumen", "TblCaption");
                body = "<a class=\"btn btn-default ew-row-link ew-detail" + (ListOptions.UseDropDownButton ? " dropdown-toggle" : "") + "\" data-action=\"list\" href=\"" + HtmlEncode(AppPath("AktivitasDokumenList?" + Config.TableShowMaster + "=Aktivitas&" + ForeignKeyUrl("fk_IdAktivitas", IdAktivitas.CurrentValue) + "")) + "\">" + body + "</a>";
                string links = "";
                detailPage = Resolve("AktivitasDokumenGrid")!;
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "Aktivitas") ?? false) {
                    caption = Language.Phrase("MasterDetailEditLink", null);
                    title = Language.Phrase("MasterDetailEditLink", true);
                    url = GetEditUrl(Config.TableShowDetail + "=AktivitasDokumen");
                    ...
                    if (!Empty(detailEditTblVar))
                        detailEditTblVar += ",";
                    detailEditTblVar += "AktivitasDokumen";
                }
                if (!Empty(links)) {
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdAktivitas.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("No_Referensi")); // DN
                CreateColumnOption(option.Add("IdProses")); // DN
                CreateColumnOption(option.Add("Aktivitas")); // DN
                CreateColumnOption(option.Add("PIC")); // DN
                CreateColumnOption(option.Add("TanggalMulai")); // DN
                CreateColumnOption(option.Add("TanggalSelesai")); // DN
                CreateColumnOption(option.Add("StatusAktivitas")); // DN
                ...
                CreateColumnOption(option.Add("TipeAktivitas")); // DN
                CreateColumnOption(option.Add("Keterangan")); // DN
                CreateColumnOption(option.Add("TipeProses")); // DN
                CreateColumnOption(option.Add("Urutan")); // DN
                CreateColumnOption(option.Add("IsNominationTankReceivingLineOpen")); // DN
                CreateColumnOption(option.Add("IsNonNominationReceivingLineClosedAndSealed")); // DN
                CreateColumnOption(option.Add("IsTankEmptyAndDry")); // DN
                CreateColumnOption(option.Add("IsDocumentationComplete")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fAktivitassrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fAktivitassrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fAktivitaslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(aktivitasList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(aktivitasList.RowCount) + "_Aktivitas");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(aktivitasList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && aktivitasList.RowType == RowType.Add || IsEdit && aktivitasList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadFieldNo_ReferensiSearchValues();
            LoadFieldIdProsesSearchValues();
            LoadFieldIdTemplateAktivitasSearchValues();
            LoadFieldAktivitas2SearchValues();
            LoadFieldNamaAktivitasSearchValues();
            LoadFieldPICSearchValues();
            LoadFieldTanggalMulaiSearchValues();
            LoadFieldTanggalSelesaiSearchValues();
            LoadFieldStatusAktivitasSearchValues();
            ...
            LoadFieldTipeAktivitasSearchValues();
            LoadFieldsandar_nama_kapalSearchValues();
            LoadFieldsandar_tgl_tibaSearchValues();
            LoadFieldsandar_nominasiSearchValues();
            LoadFieldProdukSearchValues();
            LoadFieldShipmentSearchValues();
            LoadFieldNama_ProsesSearchValues();
            LoadFieldIdDokumenSearchValues();
            LoadFieldPathFileSearchValues();
            LoadFieldTanggalUploadDokSearchValues();
            LoadFieldUserUploadDokSearchValues();
            LoadFieldNamaDokumenSearchValues();
            LoadFieldKeteranganSearchValues();
            LoadFieldIdRefAnakSearchValues();
            LoadFieldTableAnakSearchValues();
            LoadFieldTipeProsesSearchValues();
            LoadFieldUrutanSearchValues();
            LoadFieldIsNominationTankReceivingLineOpenSearchValues();
            LoadFieldIsNonNominationReceivingLineClosedAndSealedSearchValues();
            LoadFieldIsTankEmptyAndDrySearchValues();
            LoadFieldIsDocumentationCompleteSearchValues();
        }

        private void LoadFieldNo_ReferensiSearchValues()
        {
            // No_Referensi
            LoadPrimarySearchValueNo_Referensi();
            LoadSearchOperatorNo_Referensi();
        }

        private void LoadPrimarySearchValueNo_Referensi()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_No_Referensi[]")) {
                No_Referensi.AdvancedSearch.SearchValue = Get("x_No_Referensi[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                No_Referensi.AdvancedSearch.SearchValue = Get("No_Referensi"); // Default Value // DN
            }
            if (!Empty(No_Referensi.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNo_Referensi()
        {
            if (Query.ContainsKey("z_No_Referensi"))
                No_Referensi.AdvancedSearch.SearchOperator = Get("z_No_Referensi", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldIdTemplateAktivitasSearchValues()
        {
            // IdTemplateAktivitas
            LoadPrimarySearchValueIdTemplateAktivitas();
            LoadSearchOperatorIdTemplateAktivitas();
        }

        private void LoadPrimarySearchValueIdTemplateAktivitas()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdTemplateAktivitas[]")) {
                IdTemplateAktivitas.AdvancedSearch.SearchValue = Get("x_IdTemplateAktivitas[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                IdTemplateAktivitas.AdvancedSearch.SearchValue = Get("IdTemplateAktivitas"); // Default Value // DN
            }
            if (!Empty(IdTemplateAktivitas.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIdTemplateAktivitas()
        {
            if (Query.ContainsKey("z_IdTemplateAktivitas"))
                IdTemplateAktivitas.AdvancedSearch.SearchOperator = Get("z_IdTemplateAktivitas", Config.FilterOptionSeparator);
        }

        private void LoadFieldAktivitas2SearchValues()
        {
            // Aktivitas2
            LoadPrimarySearchValueAktivitas2();
            LoadSearchOperatorAktivitas2();
        }

        private void LoadPrimarySearchValueAktivitas2()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Aktivitas2[]")) {
                Aktivitas2.AdvancedSearch.SearchValue = Get("x_Aktivitas2[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Aktivitas2.AdvancedSearch.SearchValue = Get("Aktivitas2"); // Default Value // DN
            }
            if (!Empty(Aktivitas2.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorAktivitas2()
        {
            if (Query.ContainsKey("z_Aktivitas2"))
                Aktivitas2.AdvancedSearch.SearchOperator = Get("z_Aktivitas2", Config.FilterOptionSeparator);
        }

        private void LoadFieldNamaAktivitasSearchValues()
        {
            // NamaAktivitas
            LoadPrimarySearchValueNamaAktivitas();
            LoadSearchOperatorNamaAktivitas();
        }

        private void LoadPrimarySearchValueNamaAktivitas()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaAktivitas[]")) {
                NamaAktivitas.AdvancedSearch.SearchValue = Get("x_NamaAktivitas[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NamaAktivitas.AdvancedSearch.SearchValue = Get("NamaAktivitas"); // Default Value // DN
            }
            if (!Empty(NamaAktivitas.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNamaAktivitas()
        {
            if (Query.ContainsKey("z_NamaAktivitas"))
                NamaAktivitas.AdvancedSearch.SearchOperator = Get("z_NamaAktivitas", Config.FilterOptionSeparator);
        }

        private void LoadFieldPICSearchValues()
        {
            // PIC
            LoadPrimarySearchValuePIC();
            LoadSearchOperatorPIC();
        }

        private void LoadPrimarySearchValuePIC()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_PIC[]")) {
                PIC.AdvancedSearch.SearchValue = Get("x_PIC[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                PIC.AdvancedSearch.SearchValue = Get("PIC"); // Default Value // DN
            }
            if (!Empty(PIC.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorPIC()
        {
            if (Query.ContainsKey("z_PIC"))
                PIC.AdvancedSearch.SearchOperator = Get("z_PIC", Config.FilterOptionSeparator);
        }

        private void LoadFieldTanggalMulaiSearchValues()
        {
            // TanggalMulai
            LoadPrimarySearchValueTanggalMulai();
            LoadSearchOperatorTanggalMulai();
        }

        private void LoadPrimarySearchValueTanggalMulai()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TanggalMulai[]")) {
                TanggalMulai.AdvancedSearch.SearchValue = Get("x_TanggalMulai[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                TanggalMulai.AdvancedSearch.SearchValue = Get("TanggalMulai"); // Default Value // DN
            }
            if (!Empty(TanggalMulai.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTanggalMulai()
        {
            if (Query.ContainsKey("z_TanggalMulai"))
                TanggalMulai.AdvancedSearch.SearchOperator = Get("z_TanggalMulai", Config.FilterOptionSeparator);
        }

        private void LoadFieldTanggalSelesaiSearchValues()
        {
            // TanggalSelesai
            LoadPrimarySearchValueTanggalSelesai();
            LoadSearchOperatorTanggalSelesai();
        }

        private void LoadPrimarySearchValueTanggalSelesai()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TanggalSelesai[]")) {
                TanggalSelesai.AdvancedSearch.SearchValue = Get("x_TanggalSelesai[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                TanggalSelesai.AdvancedSearch.SearchValue = Get("TanggalSelesai"); // Default Value // DN
            }
            if (!Empty(TanggalSelesai.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTanggalSelesai()
        {
            if (Query.ContainsKey("z_TanggalSelesai"))
                TanggalSelesai.AdvancedSearch.SearchOperator = Get("z_TanggalSelesai", Config.FilterOptionSeparator);
        }

        private void LoadFieldStatusAktivitasSearchValues()
        {
            // StatusAktivitas
            LoadPrimarySearchValueStatusAktivitas();
            LoadSearchOperatorStatusAktivitas();
        }

        private void LoadPrimarySearchValueStatusAktivitas()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_StatusAktivitas[]")) {
                StatusAktivitas.AdvancedSearch.SearchValue = Get("x_StatusAktivitas[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                StatusAktivitas.AdvancedSearch.SearchValue = Get("StatusAktivitas"); // Default Value // DN
            }
            if (!Empty(StatusAktivitas.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorStatusAktivitas()
        {
            if (Query.ContainsKey("z_StatusAktivitas"))
                StatusAktivitas.AdvancedSearch.SearchOperator = Get("z_StatusAktivitas", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldTipeAktivitasSearchValues()
        {
            // TipeAktivitas
            LoadPrimarySearchValueTipeAktivitas();
            LoadSearchOperatorTipeAktivitas();
        }

        private void LoadPrimarySearchValueTipeAktivitas()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TipeAktivitas[]")) {
                TipeAktivitas.AdvancedSearch.SearchValue = Get("x_TipeAktivitas[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                TipeAktivitas.AdvancedSearch.SearchValue = Get("TipeAktivitas"); // Default Value // DN
            }
            if (!Empty(TipeAktivitas.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTipeAktivitas()
        {
            if (Query.ContainsKey("z_TipeAktivitas"))
                TipeAktivitas.AdvancedSearch.SearchOperator = Get("z_TipeAktivitas", Config.FilterOptionSeparator);
        }

        private void LoadFieldsandar_nama_kapalSearchValues()
        {
            // sandar_nama_kapal
            LoadPrimarySearchValuesandar_nama_kapal();
            LoadSearchOperatorsandar_nama_kapal();
        }

        private void LoadPrimarySearchValuesandar_nama_kapal()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_sandar_nama_kapal[]")) {
                sandar_nama_kapal.AdvancedSearch.SearchValue = Get("x_sandar_nama_kapal[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                sandar_nama_kapal.AdvancedSearch.SearchValue = Get("sandar_nama_kapal"); // Default Value // DN
            }
            if (!Empty(sandar_nama_kapal.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorsandar_nama_kapal()
        {
            if (Query.ContainsKey("z_sandar_nama_kapal"))
                sandar_nama_kapal.AdvancedSearch.SearchOperator = Get("z_sandar_nama_kapal", Config.FilterOptionSeparator);
        }

        private void LoadFieldsandar_tgl_tibaSearchValues()
        {
            // sandar_tgl_tiba
            LoadPrimarySearchValuesandar_tgl_tiba();
            LoadSearchOperatorsandar_tgl_tiba();
        }

        private void LoadPrimarySearchValuesandar_tgl_tiba()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_sandar_tgl_tiba[]")) {
                sandar_tgl_tiba.AdvancedSearch.SearchValue = Get("x_sandar_tgl_tiba[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                sandar_tgl_tiba.AdvancedSearch.SearchValue = Get("sandar_tgl_tiba"); // Default Value // DN
            }
            if (!Empty(sandar_tgl_tiba.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorsandar_tgl_tiba()
        {
            if (Query.ContainsKey("z_sandar_tgl_tiba"))
                sandar_tgl_tiba.AdvancedSearch.SearchOperator = Get("z_sandar_tgl_tiba", Config.FilterOptionSeparator);
        }

        private void LoadFieldsandar_nominasiSearchValues()
        {
            // sandar_nominasi
            LoadPrimarySearchValuesandar_nominasi();
            LoadSearchOperatorsandar_nominasi();
        }

        private void LoadPrimarySearchValuesandar_nominasi()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_sandar_nominasi[]")) {
                sandar_nominasi.AdvancedSearch.SearchValue = Get("x_sandar_nominasi[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                sandar_nominasi.AdvancedSearch.SearchValue = Get("sandar_nominasi"); // Default Value // DN
            }
            if (!Empty(sandar_nominasi.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorsandar_nominasi()
        {
            if (Query.ContainsKey("z_sandar_nominasi"))
                sandar_nominasi.AdvancedSearch.SearchOperator = Get("z_sandar_nominasi", Config.FilterOptionSeparator);
        }

        private void LoadFieldProdukSearchValues()
        {
            // Produk
            LoadPrimarySearchValueProduk();
            LoadSearchOperatorProduk();
        }

        private void LoadPrimarySearchValueProduk()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Produk[]")) {
                Produk.AdvancedSearch.SearchValue = Get("x_Produk[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Produk.AdvancedSearch.SearchValue = Get("Produk"); // Default Value // DN
            }
            if (!Empty(Produk.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorProduk()
        {
            if (Query.ContainsKey("z_Produk"))
                Produk.AdvancedSearch.SearchOperator = Get("z_Produk", Config.FilterOptionSeparator);
        }

        private void LoadFieldShipmentSearchValues()
        {
            // Shipment
            LoadPrimarySearchValueShipment();
            LoadSearchOperatorShipment();
        }

        private void LoadPrimarySearchValueShipment()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Shipment[]")) {
                Shipment.AdvancedSearch.SearchValue = Get("x_Shipment[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Shipment.AdvancedSearch.SearchValue = Get("Shipment"); // Default Value // DN
            }
            if (!Empty(Shipment.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorShipment()
        {
            if (Query.ContainsKey("z_Shipment"))
                Shipment.AdvancedSearch.SearchOperator = Get("z_Shipment", Config.FilterOptionSeparator);
        }

        private void LoadFieldNama_ProsesSearchValues()
        {
            // Nama_Proses
            LoadPrimarySearchValueNama_Proses();
            LoadSearchOperatorNama_Proses();
        }

        private void LoadPrimarySearchValueNama_Proses()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Nama_Proses[]")) {
                Nama_Proses.AdvancedSearch.SearchValue = Get("x_Nama_Proses[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Nama_Proses.AdvancedSearch.SearchValue = Get("Nama_Proses"); // Default Value // DN
            }
            if (!Empty(Nama_Proses.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNama_Proses()
        {
            if (Query.ContainsKey("z_Nama_Proses"))
                Nama_Proses.AdvancedSearch.SearchOperator = Get("z_Nama_Proses", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldTanggalUploadDokSearchValues()
        {
            // TanggalUploadDok
            LoadPrimarySearchValueTanggalUploadDok();
            LoadSearchOperatorTanggalUploadDok();
        }

        private void LoadPrimarySearchValueTanggalUploadDok()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TanggalUploadDok[]")) {
                TanggalUploadDok.AdvancedSearch.SearchValue = Get("x_TanggalUploadDok[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                TanggalUploadDok.AdvancedSearch.SearchValue = Get("TanggalUploadDok"); // Default Value // DN
            }
            if (!Empty(TanggalUploadDok.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTanggalUploadDok()
        {
            if (Query.ContainsKey("z_TanggalUploadDok"))
                TanggalUploadDok.AdvancedSearch.SearchOperator = Get("z_TanggalUploadDok", Config.FilterOptionSeparator);
        }

        private void LoadFieldUserUploadDokSearchValues()
        {
            // UserUploadDok
            LoadPrimarySearchValueUserUploadDok();
            LoadSearchOperatorUserUploadDok();
        }

        private void LoadPrimarySearchValueUserUploadDok()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UserUploadDok[]")) {
                UserUploadDok.AdvancedSearch.SearchValue = Get("x_UserUploadDok[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                UserUploadDok.AdvancedSearch.SearchValue = Get("UserUploadDok"); // Default Value // DN
            }
            if (!Empty(UserUploadDok.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorUserUploadDok()
        {
            if (Query.ContainsKey("z_UserUploadDok"))
                UserUploadDok.AdvancedSearch.SearchOperator = Get("z_UserUploadDok", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldIdRefAnakSearchValues()
        {
            // IdRefAnak
            LoadPrimarySearchValueIdRefAnak();
            LoadSearchOperatorIdRefAnak();
        }

        private void LoadPrimarySearchValueIdRefAnak()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdRefAnak[]")) {
                IdRefAnak.AdvancedSearch.SearchValue = Get("x_IdRefAnak[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                IdRefAnak.AdvancedSearch.SearchValue = Get("IdRefAnak"); // Default Value // DN
            }
            if (!Empty(IdRefAnak.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIdRefAnak()
        {
            if (Query.ContainsKey("z_IdRefAnak"))
                IdRefAnak.AdvancedSearch.SearchOperator = Get("z_IdRefAnak", Config.FilterOptionSeparator);
        }

        private void LoadFieldTableAnakSearchValues()
        {
            // TableAnak
            LoadPrimarySearchValueTableAnak();
            LoadSearchOperatorTableAnak();
        }

        private void LoadPrimarySearchValueTableAnak()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TableAnak[]")) {
                TableAnak.AdvancedSearch.SearchValue = Get("x_TableAnak[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                TableAnak.AdvancedSearch.SearchValue = Get("TableAnak"); // Default Value // DN
            }
            if (!Empty(TableAnak.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTableAnak()
        {
            if (Query.ContainsKey("z_TableAnak"))
                TableAnak.AdvancedSearch.SearchOperator = Get("z_TableAnak", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldUrutanSearchValues()
        {
            // Urutan
            LoadPrimarySearchValueUrutan();
            LoadSearchOperatorUrutan();
        }

        private void LoadPrimarySearchValueUrutan()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Urutan[]")) {
                Urutan.AdvancedSearch.SearchValue = Get("x_Urutan[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Urutan.AdvancedSearch.SearchValue = Get("Urutan"); // Default Value // DN
            }
            if (!Empty(Urutan.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorUrutan()
        {
            if (Query.ContainsKey("z_Urutan"))
                Urutan.AdvancedSearch.SearchOperator = Get("z_Urutan", Config.FilterOptionSeparator);
        }

        private void LoadFieldIsNominationTankReceivingLineOpenSearchValues()
        {
            // IsNominationTankReceivingLineOpen
            LoadPrimarySearchValueIsNominationTankReceivingLineOpen();
            LoadSearchOperatorIsNominationTankReceivingLineOpen();
        }

        private void LoadPrimarySearchValueIsNominationTankReceivingLineOpen()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IsNominationTankReceivingLineOpen")) {
                IsNominationTankReceivingLineOpen.AdvancedSearch.SearchValue = Get("x_IsNominationTankReceivingLineOpen");
            }
            else if (!IsAddOrEdit) {
                IsNominationTankReceivingLineOpen.AdvancedSearch.SearchValue = Get("IsNominationTankReceivingLineOpen"); // Default Value // DN
            }
            if (!Empty(IsNominationTankReceivingLineOpen.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIsNominationTankReceivingLineOpen()
        {
            if (Query.ContainsKey("z_IsNominationTankReceivingLineOpen"))
                IsNominationTankReceivingLineOpen.AdvancedSearch.SearchOperator = Get("z_IsNominationTankReceivingLineOpen");
        }

        private void LoadFieldIsNonNominationReceivingLineClosedAndSealedSearchValues()
        {
            // IsNonNominationReceivingLineClosedAndSealed
            LoadPrimarySearchValueIsNonNominationReceivingLineClosedAndSealed();
            LoadSearchOperatorIsNonNominationReceivingLineClosedAndSealed();
        }

        private void LoadPrimarySearchValueIsNonNominationReceivingLineClosedAndSealed()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IsNonNominationReceivingLineClosedAndSealed")) {
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchValue = Get("x_IsNonNominationReceivingLineClosedAndSealed");
            }
            else if (!IsAddOrEdit) {
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchValue = Get("IsNonNominationReceivingLineClosedAndSealed"); // Default Value // DN
            }
            if (!Empty(IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIsNonNominationReceivingLineClosedAndSealed()
        {
            if (Query.ContainsKey("z_IsNonNominationReceivingLineClosedAndSealed"))
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchOperator = Get("z_IsNonNominationReceivingLineClosedAndSealed");
        }

        private void LoadFieldIsTankEmptyAndDrySearchValues()
        {
            // IsTankEmptyAndDry
            LoadPrimarySearchValueIsTankEmptyAndDry();
            LoadSearchOperatorIsTankEmptyAndDry();
        }

        private void LoadPrimarySearchValueIsTankEmptyAndDry()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IsTankEmptyAndDry")) {
                IsTankEmptyAndDry.AdvancedSearch.SearchValue = Get("x_IsTankEmptyAndDry");
            }
            else if (!IsAddOrEdit) {
                IsTankEmptyAndDry.AdvancedSearch.SearchValue = Get("IsTankEmptyAndDry"); // Default Value // DN
            }
            if (!Empty(IsTankEmptyAndDry.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIsTankEmptyAndDry()
        {
            if (Query.ContainsKey("z_IsTankEmptyAndDry"))
                IsTankEmptyAndDry.AdvancedSearch.SearchOperator = Get("z_IsTankEmptyAndDry");
        }

        private void LoadFieldIsDocumentationCompleteSearchValues()
        {
            // IsDocumentationComplete
            LoadPrimarySearchValueIsDocumentationComplete();
            LoadSearchOperatorIsDocumentationComplete();
        }

        private void LoadPrimarySearchValueIsDocumentationComplete()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IsDocumentationComplete")) {
                IsDocumentationComplete.AdvancedSearch.SearchValue = Get("x_IsDocumentationComplete");
            }
            else if (!IsAddOrEdit) {
                IsDocumentationComplete.AdvancedSearch.SearchValue = Get("IsDocumentationComplete"); // Default Value // DN
            }
            if (!Empty(IsDocumentationComplete.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIsDocumentationComplete()
        {
            if (Query.ContainsKey("z_IsDocumentationComplete"))
                IsDocumentationComplete.AdvancedSearch.SearchOperator = Get("z_IsDocumentationComplete");
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // No_Referensi
                if (No_Referensi.UseFilter && !Empty(No_Referensi.AdvancedSearch.SearchValue)) {
                    No_Referensi.EditValue = ConvertToString(No_Referensi.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // Aktivitas
                if (Aktivitas2.UseFilter && !Empty(Aktivitas2.AdvancedSearch.SearchValue)) {
                    Aktivitas2.EditValue = ConvertToString(Aktivitas2.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // PIC
                if (PIC.UseFilter && !Empty(PIC.AdvancedSearch.SearchValue)) {
                    PIC.EditValue = ConvertToString(PIC.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // TanggalMulai
                if (TanggalMulai.UseFilter && !Empty(TanggalMulai.AdvancedSearch.SearchValue)) {
                    TanggalMulai.EditValue = ConvertToString(TanggalMulai.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // TanggalSelesai
                if (TanggalSelesai.UseFilter && !Empty(TanggalSelesai.AdvancedSearch.SearchValue)) {
                    TanggalSelesai.EditValue = ConvertToString(TanggalSelesai.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // StatusAktivitas
                if (StatusAktivitas.UseFilter && !Empty(StatusAktivitas.AdvancedSearch.SearchValue)) {
                    StatusAktivitas.EditValue = ConvertToString(StatusAktivitas.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // TipeAktivitas
                if (TipeAktivitas.UseFilter && !Empty(TipeAktivitas.AdvancedSearch.SearchValue)) {
                    TipeAktivitas.EditValue = ConvertToString(TipeAktivitas.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // Urutan
                if (Urutan.UseFilter && !Empty(Urutan.AdvancedSearch.SearchValue)) {
                    Urutan.EditValue = ConvertToString(Urutan.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fAktivitassrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
