...
public partial class SnDOne {
    ...
    public class ExportEmail : AbstractExport
    {
        ...
        public override void AddImage(string imageFile, string breakType = "")
        {
            ...
            string html = "<div class=\"" + classes + "\">" + GetFileImgTag([imageFile]) + "</div>";
            ...
            foreach (var img in doc.QuerySelectorAll("img")) {
                ...
                if (src.StartsWith("data:") && src.Contains(";base64,")) { // Data URL
                    if (inline) // Inline (No change required if disposition is "attachment")
                        img.SetAttribute("src", await TempImage(DataFromBase64Url(src), Config.EmailClass != "AzureEmail")); // Create temp image as cid or base64 URL
                } else if (FileExists(src)) { // Not embedded image
                    if (inline) // Inline
                        img.SetAttribute("src", await TempImage(await FileReadAllBytesAsync(src), Config.EmailClass != "AzureEmail")); // Create temp image as cid or base64 URL
                    else // Attachment
