...
public partial class SnDOne {
    ...
    public class SubAktivitasNilaiBLdanAlrtwEditBase : SubAktivitasNilaiBLdanAlrtw
    {
        ...
        // Constructor
        public SubAktivitasNilaiBLdanAlrtwEditBase()
        {
        ...
        // Constructor
        public SubAktivitasNilaiBLdanAlrtwEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasNilaiBLdanAlrtwList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasNilaiBLdanAlrtwList") {
                                ...
                                returnUrl = "SubAktivitasNilaiBLdanAlrtwList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'BL_KL_Obs' before field var 'x_BL_KL_Obs'
            val = CurrentForm.HasValue("BL_KL_Obs") ? CurrentForm.GetValue("BL_KL_Obs") : CurrentForm.GetValue("x_BL_KL_Obs");
            if (!BL_KL_Obs.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("BL_KL_Obs") && !CurrentForm.HasValue("x_BL_KL_Obs")) // DN
                    BL_KL_Obs.Visible = false; // Disable update for API request
                else
                    BL_KL_Obs.SetFormValue(val);
            }

            // Check field name 'BL_KL_15' before field var 'x_BL_KL_15'
            val = CurrentForm.HasValue("BL_KL_15") ? CurrentForm.GetValue("BL_KL_15") : CurrentForm.GetValue("x_BL_KL_15");
            if (!BL_KL_15.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("BL_KL_15") && !CurrentForm.HasValue("x_BL_KL_15")) // DN
                    BL_KL_15.Visible = false; // Disable update for API request
                else
                    BL_KL_15.SetFormValue(val);
            }
            ...
            // Check field name 'AL_KL_Obs' before field var 'x_AL_KL_Obs'
            val = CurrentForm.HasValue("AL_KL_Obs") ? CurrentForm.GetValue("AL_KL_Obs") : CurrentForm.GetValue("x_AL_KL_Obs");
            if (!AL_KL_Obs.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AL_KL_Obs") && !CurrentForm.HasValue("x_AL_KL_Obs")) // DN
                    AL_KL_Obs.Visible = false; // Disable update for API request
                else
                    AL_KL_Obs.SetFormValue(val);
            }

            // Check field name 'AL_KL_15' before field var 'x_AL_KL_15'
            val = CurrentForm.HasValue("AL_KL_15") ? CurrentForm.GetValue("AL_KL_15") : CurrentForm.GetValue("x_AL_KL_15");
            if (!AL_KL_15.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AL_KL_15") && !CurrentForm.HasValue("x_AL_KL_15")) // DN
                    AL_KL_15.Visible = false; // Disable update for API request
                else
                    AL_KL_15.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (BL_KL_Obs.Visible && BL_KL_Obs.Required) {
                    if (!BL_KL_Obs.IsDetailKey && Empty(BL_KL_Obs.FormValue)) {
                        BL_KL_Obs.AddErrorMessage(ConvertToString(BL_KL_Obs.RequiredErrorMessage).Replace("%s", BL_KL_Obs.Caption));
                    }
                }
                if (BL_KL_15.Visible && BL_KL_15.Required) {
                    if (!BL_KL_15.IsDetailKey && Empty(BL_KL_15.FormValue)) {
                        BL_KL_15.AddErrorMessage(ConvertToString(BL_KL_15.RequiredErrorMessage).Replace("%s", BL_KL_15.Caption));
                    }
                }
                ...
                if (AL_KL_Obs.Visible && AL_KL_Obs.Required) {
                    if (!AL_KL_Obs.IsDetailKey && Empty(AL_KL_Obs.FormValue)) {
                        AL_KL_Obs.AddErrorMessage(ConvertToString(AL_KL_Obs.RequiredErrorMessage).Replace("%s", AL_KL_Obs.Caption));
                    }
                }
                if (AL_KL_15.Visible && AL_KL_15.Required) {
                    if (!AL_KL_15.IsDetailKey && Empty(AL_KL_15.FormValue)) {
                        AL_KL_15.AddErrorMessage(ConvertToString(AL_KL_15.RequiredErrorMessage).Replace("%s", AL_KL_15.Caption));
                    }
                }
