...
public partial class SnDOne {
    ...
    public class MwtOnlineDetailEditBase : MwtOnlineDetail
    {
        ...
        // Constructor
        public MwtOnlineDetailEditBase(Controller? controller)
        {
        ...
        // Constructor
        public MwtOnlineDetailEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("MwtOnlineDetailList");
            }
            return null;
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'Penjelasan'
            Penjelasan.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Penjelasan, "Penjelasan", "x_Penjelasan");

            // Standard handling for 'UploadEvidence'
            UploadEvidence.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(UploadEvidence, "UploadEvidence", "x_UploadEvidence");

            // Standard handling for 'CreatedDate'
            CreatedDate.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(CreatedDate, "CreatedDate", "x_CreatedDate", true, validate, true);
            ...
            // Standard handling for 'FileType'
            FileType.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(FileType, "FileType", "x_FileType");

            // Standard handling for 'OriginalFileName'
            OriginalFileName.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(OriginalFileName, "OriginalFileName", "x_OriginalFileName");

            // Standard handling for 'Flag'
            Flag.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Flag, "Flag", "x_Flag");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomId() {
            if (Id.Visible && Id.Required) {
                if (!Id.IsDetailKey && Empty(Id.FormValue)) {
                    Id.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }

        private void ValidateCustomPenjelasan() {
            if (Penjelasan.Visible && Penjelasan.Required) {
                if (!Penjelasan.IsDetailKey && Empty(Penjelasan.FormValue)) {
                    Penjelasan.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }

        private void ValidateCustomUploadEvidence() {
            if (UploadEvidence.Visible && UploadEvidence.Required) {
                if (!UploadEvidence.IsDetailKey && Empty(UploadEvidence.FormValue)) {
                    UploadEvidence.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }

        private void ValidateCustomCreatedDate() {
            if (CreatedDate.Visible && CreatedDate.Required) {
                if (!CreatedDate.IsDetailKey && Empty(CreatedDate.FormValue)) {
                    CreatedDate.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }
        ...
        private void ValidateCustomCreatedBy() {
            if (CreatedBy.Visible && CreatedBy.Required) {
                if (!CreatedBy.IsDetailKey && Empty(CreatedBy.FormValue)) {
                    CreatedBy.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedDate() {
            if (LastUpdatedDate.Visible && LastUpdatedDate.Required) {
                if (!LastUpdatedDate.IsDetailKey && Empty(LastUpdatedDate.FormValue)) {
                    LastUpdatedDate.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusEmail() {
            if (StatusEmail.Visible && StatusEmail.Required) {
                if (!StatusEmail.IsDetailKey && Empty(StatusEmail.FormValue)) {
                    StatusEmail.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }

        private void ValidateCustomFileType() {
            if (FileType.Visible && FileType.Required) {
                if (!FileType.IsDetailKey && Empty(FileType.FormValue)) {
                    FileType.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }

        private void ValidateCustomOriginalFileName() {
            if (OriginalFileName.Visible && OriginalFileName.Required) {
                if (!OriginalFileName.IsDetailKey && Empty(OriginalFileName.FormValue)) {
                    OriginalFileName.AddErrorMessage(ConvertToString(Flag.RequiredErrorMessage).Replace("%s", Flag.Caption));
                }
            }
        }

        private void ValidateCustomFlag() {
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomPenjelasan();
                ValidateCustomUploadEvidence();
                ValidateCustomCreatedDate();
                ...
                ValidateCustomFileType();
                ValidateCustomOriginalFileName();
                ValidateCustomFlag();
