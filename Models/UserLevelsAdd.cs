...
public partial class SnDOne {
    ...
    public class UserLevelsAddBase : UserLevels
    {
        ...
        // Constructor
        public UserLevelsAddBase(Controller? controller)
        {
        ...
        // Constructor
        public UserLevelsAddBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'UserLevelName'
            UserLevelName.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(UserLevelName, "UserLevelName", "x_UserLevelName");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomUserLevelID() {
            if (UserLevelID.Visible && UserLevelID.Required) {
                if (!UserLevelID.IsDetailKey && Empty(UserLevelID.FormValue)) {
                    UserLevelID.AddErrorMessage(ConvertToString(UserLevelName.RequiredErrorMessage).Replace("%s", UserLevelName.Caption));
                }
            }
        }

        private void ValidateCustomUserLevelName() {
            if (UserLevelName.Visible && UserLevelName.Required) {
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            } else if (ConvertToInt(UserLevelID.CurrentValue) < AdvancedSecurity.AnonymousUserLevelId) {
                FailureMessage = Language.Phrase("UserLevelIDIncorrect");
            } else if (ConvertToInt(UserLevelID.CurrentValue) == AdvancedSecurity.DefaultUserLevelId && !SameText(UserLevelName.CurrentValue, "default")) {
                FailureMessage = Language.Phrase("UserLevelDefaultName");
            } else if (ConvertToInt(UserLevelID.CurrentValue) == AdvancedSecurity.AdminUserLevelId && !SameText(UserLevelName.CurrentValue, "administrator")) {
                FailureMessage = Language.Phrase("UserLevelAdministratorName");
            } else if (ConvertToInt(UserLevelID.CurrentValue) == AdvancedSecurity.AnonymousUserLevelId && !SameText(UserLevelName.CurrentValue, "anonymous")) {
                FailureMessage = Language.Phrase("UserLevelAnonymousName");
            } else if (ConvertToInt(UserLevelID.CurrentValue) > AdvancedSecurity.DefaultUserLevelId && (SameText(UserLevelName.CurrentValue, "administrator") || SameText(UserLevelName.CurrentValue, "default") || SameText(UserLevelName.CurrentValue, "anonymous"))) {
            ...
            if (priv > 0) {
                ...
                string[] columns = [Config.UserLevelPrivTableNameField, Config.UserLevelPrivUserLevelIdField, Config.UserLevelPrivPrivField];
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("UserLevelName", out value)) // UserLevelName
                UserLevelName.SetFormValue(ConvertToString(value));
        }
