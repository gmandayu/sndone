...
public partial class SnDOne {
    ...
    public class SubAktivitasTotalPenyaluranLpgEditBase : SubAktivitasTotalPenyaluranLpg
    {
        ...
        // Constructor
        public SubAktivitasTotalPenyaluranLpgEditBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasTotalPenyaluranLpgEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasTotalPenyaluranLpgList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "SubAktivitasTotalPenyaluranLpgList") {
                        ...
                        returnUrl = "SubAktivitasTotalPenyaluranLpgList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void ValidateCustomTotalPenyaluran() {
            if (TotalPenyaluran.Visible && TotalPenyaluran.Required) {
                if (!TotalPenyaluran.IsDetailKey && Empty(TotalPenyaluran.FormValue)) {
                    TotalPenyaluran.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStokAkhir() {
            if (StokAkhir.Visible && StokAkhir.Required) {
                if (!StokAkhir.IsDetailKey && Empty(StokAkhir.FormValue)) {
                    StokAkhir.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
