...
public partial class SnDOne {
    ...
    /// mwtOnlineAdd
    /// </summary>
    public static MwtOnlineAdd mwtOnlineAdd
    {
        get => HttpData.Get<MwtOnlineAdd>("mwtOnlineAdd")!;
        set => HttpData["mwtOnlineAdd"] = value;
    }

    /// <summary>
    /// Page class for MWTOnline
    /// </summary>
    public class MwtOnlineAdd : MwtOnlineAddBase
    {
        // Constructor
        public MwtOnlineAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MwtOnlineAdd() : base()
        {
        }
        ...
        public override void PageRedirecting(ref string url) {
            ...
            if (!Empty(Id.CurrentValue)) {
                url = "MwtOnlineEdit/" + Id.CurrentValue;
            }
            Console.WriteLine($"INI URLNYA {url}");
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MwtOnlineAddBase : MwtOnline
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mwtOnlineAdd";
        ...
        // Page name
        public string PageName => "MwtOnlineAdd";
        ...
        public void SetVisibility()
        {
            ...
            CompletedBy.Visible = false;
            CompletedDate.Visible = false;
        }

        // Constructor
        public MwtOnlineAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("MwtOnlineList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        else if (GetPageName(returnUrl) == "MwtOnlineView")
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'KeteranganFile' before field var 'x_KeteranganFile'
            val = CurrentForm.HasValue("KeteranganFile") ? CurrentForm.GetValue("KeteranganFile") : CurrentForm.GetValue("x_KeteranganFile");
            if (!KeteranganFile.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("KeteranganFile") && !CurrentForm.HasValue("x_KeteranganFile")) // DN
                    KeteranganFile.Visible = false; // Disable update for API request
                else
                    KeteranganFile.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            KeteranganFile.CurrentValue = KeteranganFile.FormValue;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                TaskList.EditValue = TaskList.CurrentValue; // DN
                TaskList.PlaceHolder = RemoveHtml(TaskList.Caption);
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                if (KeteranganFile.Visible && KeteranganFile.Required) {
                    if (!KeteranganFile.IsDetailKey && Empty(KeteranganFile.FormValue)) {
                        KeteranganFile.AddErrorMessage(ConvertToString(KeteranganFile.RequiredErrorMessage).Replace("%s", KeteranganFile.Caption));
                    }
                }
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // LinkRedirect
                LinkRedirect.SetDbValue(rsnew, LinkRedirect.CurrentValue);

                // LookupPlant
                LookupPlant.SetDbValue(rsnew, LookupPlant.CurrentValue);

                // TaskList
                TaskList.SetDbValue(rsnew, TaskList.CurrentValue);

                // KeteranganFile
                KeteranganFile.SetDbValue(rsnew, KeteranganFile.CurrentValue);
                return rsnew;
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("LinkRedirect", out value)) // LinkRedirect
                LinkRedirect.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("LookupPlant", out value)) // LookupPlant
                LookupPlant.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("TaskList", out value)) // TaskList
                TaskList.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("KeteranganFile", out value)) // KeteranganFile
                KeteranganFile.SetFormValue(ConvertToString(value));
        }
