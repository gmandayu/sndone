...
public partial class SnDOne {
    ...
    public class SubAkivitasNilaiAktualDischargePipaEditBase : SubAkivitasNilaiAktualDischargePipa
    {
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaEditBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAkivitasNilaiAktualDischargePipaList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "SubAkivitasNilaiAktualDischargePipaList") {
                        ...
                        returnUrl = "SubAkivitasNilaiAktualDischargePipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'AktualDischarge_klobs'
            AktualDischarge_klobs.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AktualDischarge_klobs, "AktualDischarge_klobs", "x_AktualDischarge_klobs");

            // Standard handling for 'AktualDischarge_kl15'
            AktualDischarge_kl15.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AktualDischarge_kl15, "AktualDischarge_kl15", "x_AktualDischarge_kl15");

            // Standard handling for 'AktualDischarge_barrels'
            AktualDischarge_barrels.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AktualDischarge_barrels, "AktualDischarge_barrels", "x_AktualDischarge_barrels");

            // Standard handling for 'AktualDischarge_lt'
            AktualDischarge_lt.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AktualDischarge_lt, "AktualDischarge_lt", "x_AktualDischarge_lt");

            // Standard handling for 'AktualDischarge_mt'
            AktualDischarge_mt.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AktualDischarge_mt, "AktualDischarge_mt", "x_AktualDischarge_mt");
        ...
        private void ValidateCustomAktualDischarge_klobs() {
            if (AktualDischarge_klobs.Visible && AktualDischarge_klobs.Required) {
                if (!AktualDischarge_klobs.IsDetailKey && Empty(AktualDischarge_klobs.FormValue)) {
                    AktualDischarge_klobs.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }

        private void ValidateCustomAktualDischarge_kl15() {
            if (AktualDischarge_kl15.Visible && AktualDischarge_kl15.Required) {
                if (!AktualDischarge_kl15.IsDetailKey && Empty(AktualDischarge_kl15.FormValue)) {
                    AktualDischarge_kl15.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }

        private void ValidateCustomAktualDischarge_barrels() {
            if (AktualDischarge_barrels.Visible && AktualDischarge_barrels.Required) {
                if (!AktualDischarge_barrels.IsDetailKey && Empty(AktualDischarge_barrels.FormValue)) {
                    AktualDischarge_barrels.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }

        private void ValidateCustomAktualDischarge_lt() {
            if (AktualDischarge_lt.Visible && AktualDischarge_lt.Required) {
                if (!AktualDischarge_lt.IsDetailKey && Empty(AktualDischarge_lt.FormValue)) {
                    AktualDischarge_lt.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }

        private void ValidateCustomAktualDischarge_mt() {
            if (AktualDischarge_mt.Visible && AktualDischarge_mt.Required) {
                if (!AktualDischarge_mt.IsDetailKey && Empty(AktualDischarge_mt.FormValue)) {
                    AktualDischarge_mt.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomAktualDischarge_klobs();
                ValidateCustomAktualDischarge_kl15();
                ValidateCustomAktualDischarge_barrels();
                ValidateCustomAktualDischarge_lt();
                ValidateCustomAktualDischarge_mt();
