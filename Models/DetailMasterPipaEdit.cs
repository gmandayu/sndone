...
public partial class SnDOne {
    ...
    public class DetailMasterPipaEditBase : DetailMasterPipa
    {
        ...
        // Constructor
        public DetailMasterPipaEditBase(Controller? controller)
        {
        ...
        // Constructor
        public DetailMasterPipaEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("DetailMasterPipaList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "DetailMasterPipaList") {
                        ...
                        returnUrl = "DetailMasterPipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'idPlantTujuan'
            idPlantTujuan.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(idPlantTujuan, "idPlantTujuan", "x_idPlantTujuan");

            // Standard handling for 'Panjang'
            Panjang.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Panjang, "Panjang", "x_Panjang");

            // Standard handling for 'Diameter'
            Diameter.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Diameter, "Diameter", "x_Diameter");
        ...
        private void ValidateCustomidPipa() {
            if (idPipa.Visible && idPipa.Required) {
                if (!idPipa.IsDetailKey && Empty(idPipa.FormValue)) {
                    idPipa.AddErrorMessage(ConvertToString(Volume.RequiredErrorMessage).Replace("%s", Volume.Caption));
                }
            }
        }

        private void ValidateCustomidPlantTujuan() {
            if (idPlantTujuan.Visible && idPlantTujuan.Required) {
                if (!idPlantTujuan.IsDetailKey && Empty(idPlantTujuan.FormValue)) {
                    idPlantTujuan.AddErrorMessage(ConvertToString(Volume.RequiredErrorMessage).Replace("%s", Volume.Caption));
                }
            }
        }

        private void ValidateCustomPanjang() {
            if (Panjang.Visible && Panjang.Required) {
                if (!Panjang.IsDetailKey && Empty(Panjang.FormValue)) {
                    Panjang.AddErrorMessage(ConvertToString(Volume.RequiredErrorMessage).Replace("%s", Volume.Caption));
                }
            }
        }

        private void ValidateCustomDiameter() {
            if (Diameter.Visible && Diameter.Required) {
                if (!Diameter.IsDetailKey && Empty(Diameter.FormValue)) {
                    Diameter.AddErrorMessage(ConvertToString(Volume.RequiredErrorMessage).Replace("%s", Volume.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomidPlantTujuan();
                ValidateCustomPanjang();
                ValidateCustomDiameter();
