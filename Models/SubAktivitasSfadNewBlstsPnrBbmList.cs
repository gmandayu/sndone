...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasSfadNewBlstsPnrBbmList
    /// </summary>
    public static SubAktivitasSfadNewBlstsPnrBbmList subAktivitasSfadNewBlstsPnrBbmList
    {
        get => HttpData.Get<SubAktivitasSfadNewBlstsPnrBbmList>("subAktivitasSfadNewBlstsPnrBbmList")!;
        set => HttpData["subAktivitasSfadNewBlstsPnrBbmList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasSFADNewBLSTSPnrBBM
    /// </summary>
    public class SubAktivitasSfadNewBlstsPnrBbmList : SubAktivitasSfadNewBlstsPnrBbmListBase
    {
        // Constructor
        public SubAktivitasSfadNewBlstsPnrBbmList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasSfadNewBlstsPnrBbmList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasSfadNewBlstsPnrBbmListBase : SubAktivitasSfadNewBlstsPnrBbm
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasSfadNewBlstsPnrBbmList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasSFADNewBLSTSPnrBBMlist";
        ...
        // Constructor
        public SubAktivitasSfadNewBlstsPnrBbmListBase(Controller? controller)
        {
        ...
        if (controller != null)
            ...
            // Initialize URLs
            AddUrl = "SubAktivitasSfadNewBlstsPnrBbmAdd";
            ...
            MultiDeleteUrl = "SubAktivitasSfadNewBlstsPnrBbmDelete";
            MultiUpdateUrl = "SubAktivitasSfadNewBlstsPnrBbmUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasSfadNewBlstsPnrBbmList";
        ...
        // Constructor
        public SubAktivitasSfadNewBlstsPnrBbmListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void PrepareInitialSettings()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasSFADNewBLSTSPnrBBMgrid";
        ...
        private void PageRunEnd()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasSfadNewBlstsPnrBbmList?.PageRender();
            }
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasSFADNewBLSTSPnrBBM"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLSTSPnrBBMlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLSTSPnrBBMlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasSFADNewBLSTSPnrBBMsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasSFADNewBLSTSPnrBBMsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLSTSPnrBBMlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_SubAktivitasSFADNewBLSTSPnrBBM");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasSfadNewBlstsPnrBbmList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasSfadNewBlstsPnrBbmList.RowCount) + "_SubAktivitasSFADNewBLSTSPnrBBM");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasSfadNewBlstsPnrBbmList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasSfadNewBlstsPnrBbmList.RowType == RowType.Add || IsEdit && subAktivitasSfadNewBlstsPnrBbmList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // etlDate
                    ...
                    // awallookupbung
                    // Tujuan
                    ResolveLookupView(Tujuan, "IdPlant", "number");
                    // akhirlookupbung
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasSFADNewBLSTSPnrBBMsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
