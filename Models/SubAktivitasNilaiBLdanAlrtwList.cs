...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiBLdanAlrtwList
    /// </summary>
    public static SubAktivitasNilaiBLdanAlrtwList subAktivitasNilaiBLdanAlrtwList
    {
        get => HttpData.Get<SubAktivitasNilaiBLdanAlrtwList>("subAktivitasNilaiBLdanAlrtwList")!;
        set => HttpData["subAktivitasNilaiBLdanAlrtwList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiBLdanALRTW
    /// </summary>
    public class SubAktivitasNilaiBLdanAlrtwList : SubAktivitasNilaiBLdanAlrtwListBase
    {
        // Constructor
        public SubAktivitasNilaiBLdanAlrtwList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiBLdanAlrtwList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiBLdanAlrtwListBase : SubAktivitasNilaiBLdanAlrtw
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiBLdanAlrtwList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasNilaiBLdanALRTWlist";
        ...
        // Constructor
        public SubAktivitasNilaiBLdanAlrtwListBase()
        {
            TableName = "SubAktivitasNilaiBLdanALRTW";
            ...
            // Table object (subAktivitasNilaiBLdanAlrtw)
            if (subAktivitasNilaiBLdanAlrtw == null || subAktivitasNilaiBLdanAlrtw is SubAktivitasNilaiBLdanAlrtw)
                subAktivitasNilaiBLdanAlrtw = this;

            // Initialize URLs
            AddUrl = "SubAktivitasNilaiBLdanAlrtwAdd";
            ...
            MultiDeleteUrl = "SubAktivitasNilaiBLdanAlrtwDelete";
            MultiUpdateUrl = "SubAktivitasNilaiBLdanAlrtwUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiBLdanAlrtwList";
        ...
        public void SetVisibility()
        {
            ...
            BL_KL_Obs.SetVisibility();
            BL_KL_15.SetVisibility();
            BL_Barrels.SetVisibility();
            BL_LT.SetVisibility();
            BL_MT.SetVisibility();
            AL_KL_Obs.SetVisibility();
            AL_KL_15.SetVisibility();
        ...
        // Constructor
        public SubAktivitasNilaiBLdanAlrtwListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasNilaiBLdanAlrtwView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasNilaiBLdanALRTWgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasNilaiBLdanAlrtwList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(BL_KL_Obs.AdvancedSearch.ToJson())); // Field BL_KL_Obs
            filters.Merge(JObject.Parse(BL_KL_15.AdvancedSearch.ToJson())); // Field BL_KL_15
            ...
            filters.Merge(JObject.Parse(AL_KL_Obs.AdvancedSearch.ToJson())); // Field AL_KL_Obs
            filters.Merge(JObject.Parse(AL_KL_15.AdvancedSearch.ToJson())); // Field AL_KL_15
        ...
        protected bool RestoreFilterList() {
            ...
            // Field BL_KL_Obs
            if (filter?.TryGetValue("x_BL_KL_Obs", out sv) ?? false) {
                BL_KL_Obs.AdvancedSearch.SearchValue = sv;
                BL_KL_Obs.AdvancedSearch.SearchOperator = filter["z_BL_KL_Obs"];
                BL_KL_Obs.AdvancedSearch.SearchCondition = filter["v_BL_KL_Obs"];
                BL_KL_Obs.AdvancedSearch.SearchValue2 = filter["y_BL_KL_Obs"];
                BL_KL_Obs.AdvancedSearch.SearchOperator2 = filter["w_BL_KL_Obs"];
                BL_KL_Obs.AdvancedSearch.Save();
            }

            // Field BL_KL_15
            if (filter?.TryGetValue("x_BL_KL_15", out sv) ?? false) {
                BL_KL_15.AdvancedSearch.SearchValue = sv;
                BL_KL_15.AdvancedSearch.SearchOperator = filter["z_BL_KL_15"];
                BL_KL_15.AdvancedSearch.SearchCondition = filter["v_BL_KL_15"];
                BL_KL_15.AdvancedSearch.SearchValue2 = filter["y_BL_KL_15"];
                BL_KL_15.AdvancedSearch.SearchOperator2 = filter["w_BL_KL_15"];
                BL_KL_15.AdvancedSearch.Save();
            }
            ...
            // Field AL_KL_Obs
            if (filter?.TryGetValue("x_AL_KL_Obs", out sv) ?? false) {
                AL_KL_Obs.AdvancedSearch.SearchValue = sv;
                AL_KL_Obs.AdvancedSearch.SearchOperator = filter["z_AL_KL_Obs"];
                AL_KL_Obs.AdvancedSearch.SearchCondition = filter["v_AL_KL_Obs"];
                AL_KL_Obs.AdvancedSearch.SearchValue2 = filter["y_AL_KL_Obs"];
                AL_KL_Obs.AdvancedSearch.SearchOperator2 = filter["w_AL_KL_Obs"];
                AL_KL_Obs.AdvancedSearch.Save();
            }

            // Field AL_KL_15
            if (filter?.TryGetValue("x_AL_KL_15", out sv) ?? false) {
                AL_KL_15.AdvancedSearch.SearchValue = sv;
                AL_KL_15.AdvancedSearch.SearchOperator = filter["z_AL_KL_15"];
                AL_KL_15.AdvancedSearch.SearchCondition = filter["v_AL_KL_15"];
                AL_KL_15.AdvancedSearch.SearchValue2 = filter["y_AL_KL_15"];
                AL_KL_15.AdvancedSearch.SearchOperator2 = filter["w_AL_KL_15"];
                AL_KL_15.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(BL_KL_Obs);
            searchFlds.Add(BL_KL_15);
            searchFlds.Add(BL_Barrels);
            searchFlds.Add(BL_LT);
            searchFlds.Add(BL_MT);
            searchFlds.Add(AL_KL_Obs);
            searchFlds.Add(AL_KL_15);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(BL_KL_Obs, ctrl); // BL_KL_Obs
                UpdateSort(BL_KL_15, ctrl); // BL_KL_15
                UpdateSort(BL_Barrels, ctrl); // BL_Barrels
                UpdateSort(BL_LT, ctrl); // BL_LT
                UpdateSort(BL_MT, ctrl); // BL_MT
                UpdateSort(AL_KL_Obs, ctrl); // AL_KL_Obs
                UpdateSort(AL_KL_15, ctrl); // AL_KL_15
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasNilaiBLdanALRTW"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiBLdanALRTWlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiBLdanALRTWlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("BL_KL_Obs")); // DN
                CreateColumnOption(option.Add("BL_KL_15")); // DN
                CreateColumnOption(option.Add("BL_Barrels")); // DN
                CreateColumnOption(option.Add("BL_LT")); // DN
                CreateColumnOption(option.Add("BL_MT")); // DN
                CreateColumnOption(option.Add("AL_KL_Obs")); // DN
                CreateColumnOption(option.Add("AL_KL_15")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasNilaiBLdanALRTWsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasNilaiBLdanALRTWsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiBLdanALRTWlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasNilaiBLdanALRTW");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasNilaiBLdanAlrtwList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasNilaiBLdanAlrtwList.RowCount) + "_SubAktivitasNilaiBLdanALRTW");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasNilaiBLdanAlrtwList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasNilaiBLdanAlrtwList.RowType == RowType.Add || IsEdit && subAktivitasNilaiBLdanAlrtwList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("BL_KL_Obs", BL_KL_Obs.DefaultValue ?? DbNullValue); // DN
            row.Add("BL_KL_15", BL_KL_15.DefaultValue ?? DbNullValue); // DN
            ...
            row.Add("AL_KL_Obs", AL_KL_Obs.DefaultValue ?? DbNullValue); // DN
            row.Add("AL_KL_15", AL_KL_15.DefaultValue ?? DbNullValue); // DN
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fSubAktivitasNilaiBLdanALRTWlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fSubAktivitasNilaiBLdanALRTWlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fSubAktivitasNilaiBLdanALRTWlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasNilaiBLdanALRTWsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
