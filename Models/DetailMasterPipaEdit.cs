...
public partial class SnDOne {
    /// <summary>
    /// detailMasterPipaEdit
    /// </summary>
    public static DetailMasterPipaEdit detailMasterPipaEdit
    {
        get => HttpData.Get<DetailMasterPipaEdit>("detailMasterPipaEdit")!;
        set => HttpData["detailMasterPipaEdit"] = value;
    }

    /// <summary>
    /// Page class for DetailMasterPipa
    /// </summary>
    public class DetailMasterPipaEdit : DetailMasterPipaEditBase
    {
        // Constructor
        public DetailMasterPipaEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public DetailMasterPipaEdit() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class DetailMasterPipaEditBase : DetailMasterPipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "detailMasterPipaEdit";
        ...
        // Constructor
        public DetailMasterPipaEditBase()
        {
        ...
        // Page name
        public string PageName => "DetailMasterPipaEdit";
        ...
        // Constructor
        public DetailMasterPipaEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("DetailMasterPipaList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = ReturnUrl;
                    if (GetPageName(returnUrl) == "DetailMasterPipaList")
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "DetailMasterPipaList") {
                                ...
                                returnUrl = "DetailMasterPipaList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                detailMasterPipaEdit?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'idPlantTujuan' before field var 'x_idPlantTujuan'
            val = CurrentForm.HasValue("idPlantTujuan") ? CurrentForm.GetValue("idPlantTujuan") : CurrentForm.GetValue("x_idPlantTujuan");
            if (!idPlantTujuan.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("idPlantTujuan") && !CurrentForm.HasValue("x_idPlantTujuan")) // DN
                    idPlantTujuan.Visible = false; // Disable update for API request
                else
                    idPlantTujuan.SetFormValue(val);
            }

            // Check field name 'Panjang' before field var 'x_Panjang'
            val = CurrentForm.HasValue("Panjang") ? CurrentForm.GetValue("Panjang") : CurrentForm.GetValue("x_Panjang");
            if (!Panjang.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Panjang") && !CurrentForm.HasValue("x_Panjang")) // DN
                    Panjang.Visible = false; // Disable update for API request
                else
                    Panjang.SetFormValue(val);
            }

            // Check field name 'Diameter' before field var 'x_Diameter'
            val = CurrentForm.HasValue("Diameter") ? CurrentForm.GetValue("Diameter") : CurrentForm.GetValue("x_Diameter");
            if (!Diameter.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Diameter") && !CurrentForm.HasValue("x_Diameter")) // DN
                    Diameter.Visible = false; // Disable update for API request
                else
                    Diameter.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            idPlantTujuan.CurrentValue = idPlantTujuan.FormValue;
            Panjang.CurrentValue = Panjang.FormValue;
            Diameter.CurrentValue = Diameter.FormValue;
            ...
            if (RowType == RowType.View) {
                ...
                if (idPipa.Lookup != null && IsDictionary(idPipa.Lookup?.Options) && idPipa.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    ...
                    if (curVal == "") {
                        ...
                        filterWrk = SearchFilter(idPipa.Lookup?.GetTable()?.Fields["id"].SearchExpression, "=", idPipa.CurrentValue, idPipa.Lookup?.GetTable()?.Fields["id"].SearchDataType, "");
                    }
                    string? sqlWrk = idPipa.Lookup?.GetSql(true, filterWrk, null, this, false, true);
                    List<Dictionary<string, object>>? rswrk = sqlWrk != null ? Connection.GetRows(sqlWrk) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                    idPipa.EditValue = rswrk;
                }
                ...
                string curVal2 = ConvertToString(idPlantTujuan.CurrentValue);
                if (idPlantTujuan.Lookup != null && IsDictionary(idPlantTujuan.Lookup?.Options) && idPlantTujuan.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    idPlantTujuan.EditValue = idPlantTujuan.Lookup?.Options.Values.ToList();
                } else { // Lookup from database
                    string filterWrk2 = "";
                    if (curVal2 == "") {
                        filterWrk2 = "0=1";
                    } else {
                        filterWrk2 = SearchFilter(idPlantTujuan.Lookup?.GetTable()?.Fields["IdPlant"].SearchExpression, "=", idPlantTujuan.CurrentValue, idPlantTujuan.Lookup?.GetTable()?.Fields["IdPlant"].SearchDataType, "");
                    }
                    string? sqlWrk2 = idPlantTujuan.Lookup?.GetSql(true, filterWrk2, null, this, false, true);
                    List<Dictionary<string, object>>? rswrk2 = sqlWrk2 != null ? Connection.GetRows(sqlWrk2) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                    idPlantTujuan.EditValue = rswrk2;
                }
                ...
                Panjang.EditValue = HtmlEncode(Panjang.CurrentValue);
                ...
                Diameter.EditValue = HtmlEncode(Diameter.CurrentValue);
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                if (idPlantTujuan.Visible && idPlantTujuan.Required) {
                    if (!idPlantTujuan.IsDetailKey && Empty(idPlantTujuan.FormValue)) {
                        idPlantTujuan.AddErrorMessage(ConvertToString(idPlantTujuan.RequiredErrorMessage).Replace("%s", idPlantTujuan.Caption));
                    }
                }
                if (Panjang.Visible && Panjang.Required) {
                    if (!Panjang.IsDetailKey && Empty(Panjang.FormValue)) {
                        Panjang.AddErrorMessage(ConvertToString(Panjang.RequiredErrorMessage).Replace("%s", Panjang.Caption));
                    }
                }
                if (Diameter.Visible && Diameter.Required) {
                    if (!Diameter.IsDetailKey && Empty(Diameter.FormValue)) {
                        Diameter.AddErrorMessage(ConvertToString(Diameter.RequiredErrorMessage).Replace("%s", Diameter.Caption));
                    }
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // idPipa
            idPipa.SetDbValue(rsnew, idPipa.CurrentValue, idPipa.ReadOnly);

            // idPlantTujuan
            idPlantTujuan.SetDbValue(rsnew, idPlantTujuan.CurrentValue, idPlantTujuan.ReadOnly);

            // Panjang
            Panjang.SetDbValue(rsnew, Panjang.CurrentValue, Panjang.ReadOnly);

            // Diameter
            Diameter.SetDbValue(rsnew, Diameter.CurrentValue, Diameter.ReadOnly);
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("idPipa", out value)) // idPipa
                idPipa.CurrentValue = value;
            if (row.TryGetValue("idPlantTujuan", out value)) // idPlantTujuan
                idPlantTujuan.CurrentValue = value;
            if (row.TryGetValue("Panjang", out value)) // Panjang
                Panjang.CurrentValue = value;
            if (row.TryGetValue("Diameter", out value)) // Diameter
                Diameter.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("DetailMasterPipaList")), "", TableVar, true);
            string pageId = "edit";
