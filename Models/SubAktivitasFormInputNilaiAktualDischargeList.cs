...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasFormInputNilaiAktualDischargeList
    /// </summary>
    public static SubAktivitasFormInputNilaiAktualDischargeList subAktivitasFormInputNilaiAktualDischargeList
    {
        get => HttpData.Get<SubAktivitasFormInputNilaiAktualDischargeList>("subAktivitasFormInputNilaiAktualDischargeList")!;
        set => HttpData["subAktivitasFormInputNilaiAktualDischargeList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasFormInputNilaiAktualDischarge
    /// </summary>
    public class SubAktivitasFormInputNilaiAktualDischargeList : SubAktivitasFormInputNilaiAktualDischargeListBase
    {
        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasFormInputNilaiAktualDischargeListBase : SubAktivitasFormInputNilaiAktualDischarge
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasFormInputNilaiAktualDischargeList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasFormInputNilaiAktualDischargelist";
        ...
        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeListBase()
        {
            TableName = "SubAktivitasFormInputNilaiAktualDischarge";

            // CSS class name as context
            TableVar = "SubAktivitasFormInputNilaiAktualDischarge";
            ...
            // Table object (subAktivitasFormInputNilaiAktualDischarge)
            if (subAktivitasFormInputNilaiAktualDischarge == null || subAktivitasFormInputNilaiAktualDischarge is SubAktivitasFormInputNilaiAktualDischarge)
                subAktivitasFormInputNilaiAktualDischarge = this;

            // Initialize URLs
            AddUrl = "SubAktivitasFormInputNilaiAktualDischargeAdd";
            ...
            MultiDeleteUrl = "SubAktivitasFormInputNilaiAktualDischargeDelete";
            MultiUpdateUrl = "SubAktivitasFormInputNilaiAktualDischargeUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasFormInputNilaiAktualDischargeList";
        ...
        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasFormInputNilaiAktualDischargeView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasFormInputNilaiAktualDischargegrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasFormInputNilaiAktualDischargeList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(KL_Obs.AdvancedSearch.ToJson())); // Field KL_Obs
            filters.Merge(JObject.Parse(KL_15.AdvancedSearch.ToJson())); // Field KL_15
            filters.Merge(JObject.Parse(Barrels.AdvancedSearch.ToJson())); // Field Barrels
            filters.Merge(JObject.Parse(LT.AdvancedSearch.ToJson())); // Field LT
            filters.Merge(JObject.Parse(MT.AdvancedSearch.ToJson())); // Field MT
        ...
        protected bool RestoreFilterList() {
            ...
            // Field KL_Obs
            if (filter?.TryGetValue("x_KL_Obs", out sv) ?? false) {
                KL_Obs.AdvancedSearch.SearchValue = sv;
                KL_Obs.AdvancedSearch.SearchOperator = filter["z_KL_Obs"];
                KL_Obs.AdvancedSearch.SearchCondition = filter["v_KL_Obs"];
                KL_Obs.AdvancedSearch.SearchValue2 = filter["y_KL_Obs"];
                KL_Obs.AdvancedSearch.SearchOperator2 = filter["w_KL_Obs"];
                KL_Obs.AdvancedSearch.Save();
            }

            // Field KL_15
            if (filter?.TryGetValue("x_KL_15", out sv) ?? false) {
                KL_15.AdvancedSearch.SearchValue = sv;
                KL_15.AdvancedSearch.SearchOperator = filter["z_KL_15"];
                KL_15.AdvancedSearch.SearchCondition = filter["v_KL_15"];
                KL_15.AdvancedSearch.SearchValue2 = filter["y_KL_15"];
                KL_15.AdvancedSearch.SearchOperator2 = filter["w_KL_15"];
                KL_15.AdvancedSearch.Save();
            }

            // Field Barrels
            if (filter?.TryGetValue("x_Barrels", out sv) ?? false) {
                Barrels.AdvancedSearch.SearchValue = sv;
                Barrels.AdvancedSearch.SearchOperator = filter["z_Barrels"];
                Barrels.AdvancedSearch.SearchCondition = filter["v_Barrels"];
                Barrels.AdvancedSearch.SearchValue2 = filter["y_Barrels"];
                Barrels.AdvancedSearch.SearchOperator2 = filter["w_Barrels"];
                Barrels.AdvancedSearch.Save();
            }

            // Field LT
            if (filter?.TryGetValue("x_LT", out sv) ?? false) {
                LT.AdvancedSearch.SearchValue = sv;
                LT.AdvancedSearch.SearchOperator = filter["z_LT"];
                LT.AdvancedSearch.SearchCondition = filter["v_LT"];
                LT.AdvancedSearch.SearchValue2 = filter["y_LT"];
                LT.AdvancedSearch.SearchOperator2 = filter["w_LT"];
                LT.AdvancedSearch.Save();
            }

            // Field MT
            if (filter?.TryGetValue("x_MT", out sv) ?? false) {
                MT.AdvancedSearch.SearchValue = sv;
                MT.AdvancedSearch.SearchOperator = filter["z_MT"];
                MT.AdvancedSearch.SearchCondition = filter["v_MT"];
                MT.AdvancedSearch.SearchValue2 = filter["y_MT"];
                MT.AdvancedSearch.SearchOperator2 = filter["w_MT"];
                MT.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(KL_Obs);
            searchFlds.Add(KL_15);
            searchFlds.Add(Barrels);
            searchFlds.Add(LT);
            searchFlds.Add(MT);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(KL_Obs, ctrl); // KL_Obs
                UpdateSort(KL_15, ctrl); // KL_15
                UpdateSort(Barrels, ctrl); // Barrels
                UpdateSort(LT, ctrl); // LT
                UpdateSort(MT, ctrl); // MT
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""SubAktivitasFormInputNilaiAktualDischarge"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasFormInputNilaiAktualDischarge"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""SubAktivitasFormInputNilaiAktualDischarge"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasFormInputNilaiAktualDischargelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasFormInputNilaiAktualDischargelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""SubAktivitasFormInputNilaiAktualDischarge"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fSubAktivitasFormInputNilaiAktualDischargelist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("KL_Obs")); // DN
                CreateColumnOption(option.Add("KL_15")); // DN
                CreateColumnOption(option.Add("Barrels")); // DN
                CreateColumnOption(option.Add("LT")); // DN
                CreateColumnOption(option.Add("MT")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasFormInputNilaiAktualDischargesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasFormInputNilaiAktualDischargesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasFormInputNilaiAktualDischargelist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasFormInputNilaiAktualDischarge");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasFormInputNilaiAktualDischargeList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasFormInputNilaiAktualDischargeList.RowCount) + "_SubAktivitasFormInputNilaiAktualDischarge");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasFormInputNilaiAktualDischargeList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasFormInputNilaiAktualDischargeList.RowType == RowType.Add || IsEdit && subAktivitasFormInputNilaiAktualDischargeList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // Tujuan
                string curVal5 = ConvertToString(Tujuan.CurrentValue);
                if (!Empty(curVal5)) {
                    if (Tujuan.Lookup != null && IsDictionary(Tujuan.Lookup?.Options) && Tujuan.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        Tujuan.ViewValue = Tujuan.LookupCacheOption(curVal5);
                    } else { // Lookup from database // DN
                        string filterWrk5 = SearchFilter(Tujuan.Lookup?.GetTable()?.Fields["IdPlant"].SearchExpression, "=", Tujuan.CurrentValue, Tujuan.Lookup?.GetTable()?.Fields["IdPlant"].SearchDataType, "");
                        string? sqlWrk5 = Tujuan.Lookup?.GetSql(false, filterWrk5, null, this, true, true);
                        ...
                        if (rswrk5?.Count > 0 && Tujuan.Lookup != null) { // Lookup values found
                            var listwrk = Tujuan.Lookup?.RenderViewRow(rswrk5[0]);
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fSubAktivitasFormInputNilaiAktualDischargelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fSubAktivitasFormInputNilaiAktualDischargelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fSubAktivitasFormInputNilaiAktualDischargelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasFormInputNilaiAktualDischargesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
