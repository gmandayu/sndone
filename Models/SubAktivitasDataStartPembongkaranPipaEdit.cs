...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasDataStartPembongkaranPipaEdit
    /// </summary>
    public static SubAktivitasDataStartPembongkaranPipaEdit subAktivitasDataStartPembongkaranPipaEdit
    {
        get => HttpData.Get<SubAktivitasDataStartPembongkaranPipaEdit>("subAktivitasDataStartPembongkaranPipaEdit")!;
        set => HttpData["subAktivitasDataStartPembongkaranPipaEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasDataStartPembongkaranPipa
    /// </summary>
    public class SubAktivitasDataStartPembongkaranPipaEdit : SubAktivitasDataStartPembongkaranPipaEditBase
    {
        // Constructor
        public SubAktivitasDataStartPembongkaranPipaEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasDataStartPembongkaranPipaEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasDataStartPembongkaranPipaEditBase : SubAktivitasDataStartPembongkaranPipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasDataStartPembongkaranPipaEdit";
        ...
        // Constructor
        public SubAktivitasDataStartPembongkaranPipaEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasDataStartPembongkaranPipaEdit";
        ...
        // Constructor
        public SubAktivitasDataStartPembongkaranPipaEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasDataStartPembongkaranPipaList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasDataStartPembongkaranPipaList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasDataStartPembongkaranPipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasDataStartPembongkaranPipaEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'tglStartBongkar'
            tglStartBongkar.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(tglStartBongkar, "tglStartBongkar", "x_tglStartBongkar", true, validate, true);
            if (!id.IsDetailKey) {
        ...
        public void RestoreFormValues()
        {
            ...
            tglStartBongkar.CurrentValue = tglStartBongkar.FormValue;
            tglStartBongkar.CurrentValue = UnformatDateTime(tglStartBongkar.CurrentValue, tglStartBongkar.FormatPattern);
        }
        ...
        public async Task RenderRow()
        {
            ...
            // tglStartBongkar
            tglStartBongkar.RowCssClass = "row";
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(tglStartBongkar.RequiredErrorMessage).Replace("%s", tglStartBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(tglStartBongkar.RequiredErrorMessage).Replace("%s", tglStartBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(tglStartBongkar.RequiredErrorMessage).Replace("%s", tglStartBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(tglStartBongkar.RequiredErrorMessage).Replace("%s", tglStartBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(tglStartBongkar.RequiredErrorMessage).Replace("%s", tglStartBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(tglStartBongkar.RequiredErrorMessage).Replace("%s", tglStartBongkar.Caption));
                }
            }
        }

        private void ValidateCustomtglStartBongkar() {
            if (tglStartBongkar.Visible && tglStartBongkar.Required) {
                if (!tglStartBongkar.IsDetailKey && Empty(tglStartBongkar.FormValue)) {
                    tglStartBongkar.AddErrorMessage(ConvertToString(tglStartBongkar.RequiredErrorMessage).Replace("%s", tglStartBongkar.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomtglStartBongkar();
                if (!CheckDate(tglStartBongkar.FormValue, tglStartBongkar.FormatPattern)) {
                    tglStartBongkar.AddErrorMessage(tglStartBongkar.GetErrorMessage(false));
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // tglStartBongkar
            tglStartBongkar.SetDbValue(rsnew, ConvertToDateTime(tglStartBongkar.CurrentValue, tglStartBongkar.FormatPattern), tglStartBongkar.ReadOnly);
            return rsnew;
        }
        ...
        {
            object? value;
            if (row.TryGetValue("tglStartBongkar", out value)) // tglStartBongkar
                tglStartBongkar.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasDataStartPembongkaranPipaList")), "", TableVar, true);
            string pageId = "edit";
