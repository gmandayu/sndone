...
public partial class SnDOne {
    ...
    /// setUserModAdd
    /// </summary>
    public static SetUserModAdd setUserModAdd
    {
        get => HttpData.Get<SetUserModAdd>("setUserModAdd")!;
        set => HttpData["setUserModAdd"] = value;
    }

    /// <summary>
    /// Page class for SetUserMOD
    /// </summary>
    public class SetUserModAdd : SetUserModAddBase
    {
        // Constructor
        public SetUserModAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SetUserModAdd() : base()
        {
        }
        ...
        public override void PageLoad() {
            ...
            try
            {
                // Baca Rule sebagai NVARCHAR → string
                int role = 0;
                int.TryParse(Convert.ToString(CurrentUserInfo("Rule")), out role);
                bool isAllowed = role == 0 || role == 10;
                if (isAllowed)
                    return; // boleh lanjut render

                // Tidak diizinkan
                var msg = "Anda tidak memiliki akses untuk menambah Set User MOD. " +
            ...
            {
                Log("Guard SetUserMODAdd.Page_Load error: " + ex.Message);
                var listUrl = GetUrl("SetUserMODList");
    ...
    /// Page base class
    /// </summary>
    public class SetUserModAddBase : SetUserMod
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "setUserModAdd";
        ...
        // Page name
        public string PageName => "SetUserModAdd";
        ...
        // Constructor
        public SetUserModAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("SetUserModList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        else if (GetPageName(returnUrl) == "SetUserModView")
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SetUserModList") {
                                ...
                                returnUrl = "SetUserModList"; // Return list page content
                            }
                        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'LookupUserMod' before field var 'x_LookupUserMod'
            val = CurrentForm.HasValue("LookupUserMod") ? CurrentForm.GetValue("LookupUserMod") : CurrentForm.GetValue("x_LookupUserMod");
            if (!LookupUserMod.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("LookupUserMod") && !CurrentForm.HasValue("x_LookupUserMod")) // DN
                    LookupUserMod.Visible = false; // Disable update for API request
                else
                    LookupUserMod.SetFormValue(val);
            }

            // Check field name 'UserMOD' before field var 'x_UserMOD'
            val = CurrentForm.HasValue("UserMOD") ? CurrentForm.GetValue("UserMOD") : CurrentForm.GetValue("x_UserMOD");
            if (!UserMOD.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("UserMOD") && !CurrentForm.HasValue("x_UserMOD")) // DN
                    UserMOD.Visible = false; // Disable update for API request
                else
                    UserMOD.SetFormValue(val);
            }
                ...
                if (!Empty(curVal4)) {
                    if (UserMOD.Lookup != null && IsDictionary(UserMOD.Lookup?.Options) && UserMOD.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        ...
                        if (rswrk4?.Count > 0 && UserMOD.Lookup != null) { // Lookup values found
                            ...
                            UserMOD.EditValue = HtmlEncode(UserMOD.CurrentValue);
                        }
                    }
                } else {
                    UserMOD.EditValue = DbNullValue;
                }
            ...
            bool validateForm = true;
                ...
                if (LookupUserMod.Visible && LookupUserMod.Required) {
                    if (!LookupUserMod.IsDetailKey && Empty(LookupUserMod.FormValue)) {
                        LookupUserMod.AddErrorMessage(ConvertToString(LookupUserMod.RequiredErrorMessage).Replace("%s", LookupUserMod.Caption));
                    }
                }
                if (UserMOD.Visible && UserMOD.Required) {
                    if (!UserMOD.IsDetailKey && Empty(UserMOD.FormValue)) {
                        UserMOD.AddErrorMessage(ConvertToString(UserMOD.RequiredErrorMessage).Replace("%s", UserMOD.Caption));
                    }
                }
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // LookupUserMod
                LookupUserMod.SetDbValue(rsnew, LookupUserMod.CurrentValue);

                // UserMOD
                UserMOD.SetDbValue(rsnew, UserMOD.CurrentValue);
                return rsnew;
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("LookupUserMod", out value)) // LookupUserMod
                LookupUserMod.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("UserMOD", out value)) // UserMOD
                UserMOD.SetFormValue(ConvertToString(value));
        }
