...
public partial class SnDOne {
    ...
    /// masterTemplateEdit
    /// </summary>
    public static MasterTemplateEdit masterTemplateEdit
    {
        get => HttpData.Get<MasterTemplateEdit>("masterTemplateEdit")!;
        set => HttpData["masterTemplateEdit"] = value;
    }

    /// <summary>
    /// Page class for MasterTemplate
    /// </summary>
    public class MasterTemplateEdit : MasterTemplateEditBase
    {
        // Constructor
        public MasterTemplateEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterTemplateEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterTemplateEditBase : MasterTemplate
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterTemplateEdit";
        ...
        // Page name
        public string PageName => "MasterTemplateEdit";
        ...
        // Constructor
        public MasterTemplateEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdTemplate", out rv)) { // DN
                    IdTemplate.FormValue = UrlDecode(rv); // DN
                    IdTemplate.OldValue = IdTemplate.FormValue;
                } else if (CurrentForm.HasValue("x_IdTemplate")) {
                    IdTemplate.FormValue = CurrentForm.GetValue("x_IdTemplate");
                    IdTemplate.OldValue = IdTemplate.FormValue;
                } else if (!Empty(keyValues)) {
                    IdTemplate.OldValue = ConvertToString(keyValues[0]);
                } else {
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdTemplate", out rv)) { // DN
                        IdTemplate.QueryValue = UrlDecode(rv); // DN
                        loadByQuery = true;
                    } else if (Get("IdTemplate", out sv)) {
                        IdTemplate.QueryValue = sv.ToString();
                        loadByQuery = true;
                    } else {
                        IdTemplate.CurrentValue = DbNullValue;
                    }
                }
        ...
        if (postBack) {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                IdTemplate.FormValue = ConvertToString(keyValues[0]);
            }
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("MasterTemplateList"); // No matching record, return to list
                        }
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "MasterTemplateList") {
                                ...
                                returnUrl = "MasterTemplateList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NamaTemplate' before field var 'x_NamaTemplate'
            val = CurrentForm.HasValue("NamaTemplate") ? CurrentForm.GetValue("NamaTemplate") : CurrentForm.GetValue("x_NamaTemplate");
            if (!NamaTemplate.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NamaTemplate") && !CurrentForm.HasValue("x_NamaTemplate")) // DN
                    NamaTemplate.Visible = false; // Disable update for API request
                else
                    NamaTemplate.SetFormValue(val);
            }
            ...
            // Check field name 'JenisProses' before field var 'x_JenisProses'
            val = CurrentForm.HasValue("JenisProses") ? CurrentForm.GetValue("JenisProses") : CurrentForm.GetValue("x_JenisProses");
            if (!JenisProses.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("JenisProses") && !CurrentForm.HasValue("x_JenisProses")) // DN
                    JenisProses.Visible = false; // Disable update for API request
                else
                    JenisProses.SetFormValue(val);
            }
            ...
            // Check field name 'TipePenimbunan' before field var 'x_TipePenimbunan'
            val = CurrentForm.HasValue("TipePenimbunan") ? CurrentForm.GetValue("TipePenimbunan") : CurrentForm.GetValue("x_TipePenimbunan");
            if (!TipePenimbunan.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TipePenimbunan") && !CurrentForm.HasValue("x_TipePenimbunan")) // DN
                    TipePenimbunan.Visible = false; // Disable update for API request
                else
                    TipePenimbunan.SetFormValue(val);
            }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                if (NamaTemplate.Visible && NamaTemplate.Required) {
                    if (!NamaTemplate.IsDetailKey && Empty(NamaTemplate.FormValue)) {
                        NamaTemplate.AddErrorMessage(ConvertToString(NamaTemplate.RequiredErrorMessage).Replace("%s", NamaTemplate.Caption));
                    }
                }
                ...
                if (JenisProses.Visible && JenisProses.Required) {
                    if (!JenisProses.IsDetailKey && Empty(JenisProses.FormValue)) {
                        JenisProses.AddErrorMessage(ConvertToString(JenisProses.RequiredErrorMessage).Replace("%s", JenisProses.Caption));
                    }
                }
                ...
                if (TipePenimbunan.Visible && TipePenimbunan.Required) {
                    if (!TipePenimbunan.IsDetailKey && Empty(TipePenimbunan.FormValue)) {
                        TipePenimbunan.AddErrorMessage(ConvertToString(TipePenimbunan.RequiredErrorMessage).Replace("%s", TipePenimbunan.Caption));
                    }
                }
            ...
            if (detailTblVar.Contains("TemplateProses") && templateProses?.DetailEdit == true) {
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            if (updateRow) {
                try {
                    ...
                    var detailTblVar = CurrentDetailTables;
                    if (detailTblVar.Contains("TemplateProses") && templateProses?.DetailEdit == true && result) {
                        templateProsesGrid = Resolve("TemplateProsesGrid")!; // Get detail page object
                        if (templateProsesGrid != null) {
                            Security.LoadCurrentUserLevel(ProjectID + "TemplateProses"); // Load user level of detail table
                            result = await templateProsesGrid.GridUpdate();
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // NamaTemplate
            NamaTemplate.SetDbValue(rsnew, NamaTemplate.CurrentValue, NamaTemplate.ReadOnly);

            // IdModa
            IdModa.SetDbValue(rsnew, IdModa.CurrentValue, IdModa.ReadOnly);

            // JenisProses
            JenisProses.SetDbValue(rsnew, JenisProses.CurrentValue, JenisProses.ReadOnly);

            // Keterangan
            Keterangan.SetDbValue(rsnew, Keterangan.CurrentValue, Keterangan.ReadOnly);

            // TipePenimbunan
            TipePenimbunan.SetDbValue(rsnew, TipePenimbunan.CurrentValue, TipePenimbunan.ReadOnly);

            // TipePenyaluran
            TipePenyaluran.SetDbValue(rsnew, TipePenyaluran.CurrentValue, TipePenyaluran.ReadOnly);

            // KategoriPenyaluran
            KategoriPenyaluran.SetDbValue(rsnew, KategoriPenyaluran.CurrentValue, KategoriPenyaluran.ReadOnly);
        ...
        protected void SetupDetailParms() {
            ...
            if (!Empty(detailTblVar)) {
                ...
                if (detailTblVars.Contains("TemplateProses")) {
                    ...
                    if (templateProsesGrid?.DetailEdit ?? false) {
                        templateProsesGrid.CurrentMode = "edit";
                        templateProsesGrid.CurrentAction = "gridedit";
