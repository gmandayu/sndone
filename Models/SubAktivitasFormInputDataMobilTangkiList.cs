...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasFormInputDataMobilTangkiList
    /// </summary>
    public static SubAktivitasFormInputDataMobilTangkiList subAktivitasFormInputDataMobilTangkiList
    {
        get => HttpData.Get<SubAktivitasFormInputDataMobilTangkiList>("subAktivitasFormInputDataMobilTangkiList")!;
        set => HttpData["subAktivitasFormInputDataMobilTangkiList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasFormInputDataMobilTangki
    /// </summary>
    public class SubAktivitasFormInputDataMobilTangkiList : SubAktivitasFormInputDataMobilTangkiListBase
    {
        // Constructor
        public SubAktivitasFormInputDataMobilTangkiList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasFormInputDataMobilTangkiList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasFormInputDataMobilTangkiListBase : SubAktivitasFormInputDataMobilTangki
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasFormInputDataMobilTangkiList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasFormInputDataMobilTangkilist";
        ...
        // Constructor
        public SubAktivitasFormInputDataMobilTangkiListBase()
        {
            TableName = "SubAktivitasFormInputDataMobilTangki";

            // CSS class name as context
            TableVar = "SubAktivitasFormInputDataMobilTangki";
            ...
            // Table object (subAktivitasFormInputDataMobilTangki)
            if (subAktivitasFormInputDataMobilTangki == null || subAktivitasFormInputDataMobilTangki is SubAktivitasFormInputDataMobilTangki)
                subAktivitasFormInputDataMobilTangki = this;

            // Initialize URLs
            AddUrl = "SubAktivitasFormInputDataMobilTangkiAdd";
            ...
            MultiDeleteUrl = "SubAktivitasFormInputDataMobilTangkiDelete";
            MultiUpdateUrl = "SubAktivitasFormInputDataMobilTangkiUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasFormInputDataMobilTangkiList";
        ...
        // Constructor
        public SubAktivitasFormInputDataMobilTangkiListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasFormInputDataMobilTangkiView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasFormInputDataMobilTangkigrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasFormInputDataMobilTangkiList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(NomorPolisi.AdvancedSearch.ToJson())); // Field NomorPolisi
        ...
        protected bool RestoreFilterList() {
            ...
            // Field NomorPolisi
            if (filter?.TryGetValue("x_NomorPolisi", out sv) ?? false) {
                NomorPolisi.AdvancedSearch.SearchValue = sv;
                NomorPolisi.AdvancedSearch.SearchOperator = filter["z_NomorPolisi"];
                NomorPolisi.AdvancedSearch.SearchCondition = filter["v_NomorPolisi"];
                NomorPolisi.AdvancedSearch.SearchValue2 = filter["y_NomorPolisi"];
                NomorPolisi.AdvancedSearch.SearchOperator2 = filter["w_NomorPolisi"];
                NomorPolisi.AdvancedSearch.Save();
            }
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""SubAktivitasFormInputDataMobilTangki"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasFormInputDataMobilTangki"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""SubAktivitasFormInputDataMobilTangki"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasFormInputDataMobilTangkilist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasFormInputDataMobilTangkilist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""SubAktivitasFormInputDataMobilTangki"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fSubAktivitasFormInputDataMobilTangkilist""" +
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasFormInputDataMobilTangkisrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasFormInputDataMobilTangkisrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasFormInputDataMobilTangkilist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasFormInputDataMobilTangki");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasFormInputDataMobilTangkiList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasFormInputDataMobilTangkiList.RowCount) + "_SubAktivitasFormInputDataMobilTangki");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasFormInputDataMobilTangkiList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasFormInputDataMobilTangkiList.RowType == RowType.Add || IsEdit && subAktivitasFormInputDataMobilTangkiList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasFormInputDataMobilTangkisrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
