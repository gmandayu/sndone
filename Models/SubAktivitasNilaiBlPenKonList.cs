...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiBlPenKonList
    /// </summary>
    public static SubAktivitasNilaiBlPenKonList subAktivitasNilaiBlPenKonList
    {
        get => HttpData.Get<SubAktivitasNilaiBlPenKonList>("subAktivitasNilaiBlPenKonList")!;
        set => HttpData["subAktivitasNilaiBlPenKonList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiBLPenKon
    /// </summary>
    public class SubAktivitasNilaiBlPenKonList : SubAktivitasNilaiBlPenKonListBase
    {
        // Constructor
        public SubAktivitasNilaiBlPenKonList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiBlPenKonList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiBlPenKonListBase : SubAktivitasNilaiBlPenKon
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiBlPenKonList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasNilaiBLPenKonlist";
        ...
        // Constructor
        public SubAktivitasNilaiBlPenKonListBase()
        {
            TableName = "SubAktivitasNilaiBLPenKon";
            ...
            // Table object (subAktivitasNilaiBlPenKon)
            if (subAktivitasNilaiBlPenKon == null || subAktivitasNilaiBlPenKon is SubAktivitasNilaiBlPenKon)
                subAktivitasNilaiBlPenKon = this;

            // Initialize URLs
            AddUrl = "SubAktivitasNilaiBlPenKonAdd";
            ...
            MultiDeleteUrl = "SubAktivitasNilaiBlPenKonDelete";
            MultiUpdateUrl = "SubAktivitasNilaiBlPenKonUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiBlPenKonList";
        ...
        // Constructor
        public SubAktivitasNilaiBlPenKonListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasNilaiBlPenKonView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasNilaiBLPenKongrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasNilaiBlPenKonList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasNilaiBLPenKon"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiBLPenKonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiBLPenKonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasNilaiBLPenKonsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasNilaiBLPenKonsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiBLPenKonlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasNilaiBLPenKon");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasNilaiBlPenKonList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasNilaiBlPenKonList.RowCount) + "_SubAktivitasNilaiBLPenKon");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasNilaiBlPenKonList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasNilaiBlPenKonList.RowType == RowType.Add || IsEdit && subAktivitasNilaiBlPenKonList.RowType == RowType.Edit) // Inline-Add/Edit row
