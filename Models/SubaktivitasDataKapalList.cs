...
public partial class SnDOne {
    /// <summary>
    /// subaktivitasDataKapalList
    /// </summary>
    public static SubaktivitasDataKapalList subaktivitasDataKapalList
    {
        get => HttpData.Get<SubaktivitasDataKapalList>("subaktivitasDataKapalList")!;
        set => HttpData["subaktivitasDataKapalList"] = value;
    }

    /// <summary>
    /// Page class for SubaktivitasDataKapal
    /// </summary>
    public class SubaktivitasDataKapalList : SubaktivitasDataKapalListBase
    {
        // Constructor
        public SubaktivitasDataKapalList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubaktivitasDataKapalList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubaktivitasDataKapalListBase : SubaktivitasDataKapal
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subaktivitasDataKapalList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubaktivitasDataKapallist";
        ...
        // Constructor
        public SubaktivitasDataKapalListBase()
        {
            TableName = "SubaktivitasDataKapal";

            // CSS class name as context
            TableVar = "SubaktivitasDataKapal";
            ...
            // Table object (subaktivitasDataKapal)
            if (subaktivitasDataKapal == null || subaktivitasDataKapal is SubaktivitasDataKapal)
                subaktivitasDataKapal = this;

            // Initialize URLs
            AddUrl = "SubaktivitasDataKapalAdd";
            ...
            MultiDeleteUrl = "SubaktivitasDataKapalDelete";
            MultiUpdateUrl = "SubaktivitasDataKapalUpdate";
        ...
        // Page name
        public string PageName => "SubaktivitasDataKapalList";
        ...
        // Constructor
        public SubaktivitasDataKapalListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubaktivitasDataKapalView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubaktivitasDataKapalgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subaktivitasDataKapalList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubaktivitasDataKapal"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubaktivitasDataKapallist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubaktivitasDataKapallist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubaktivitasDataKapalsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubaktivitasDataKapalsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubaktivitasDataKapallist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubaktivitasDataKapal");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subaktivitasDataKapalList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subaktivitasDataKapalList.RowCount) + "_SubaktivitasDataKapal");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subaktivitasDataKapalList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subaktivitasDataKapalList.RowType == RowType.Add || IsEdit && subaktivitasDataKapalList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubaktivitasDataKapalsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
