...
public partial class SnDOne {
    ...
    public class GoodHouseKeepingEditBase : GoodHouseKeeping
    {
        ...
        // Constructor
        public GoodHouseKeepingEditBase(Controller? controller)
        {
        ...
        // Constructor
        public GoodHouseKeepingEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdGoodHouseKeeping", out rv)) {
                    IdGoodHouseKeeping.FormValue = UrlDecode(rv);
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdGoodHouseKeeping", out rv)) {
                        IdGoodHouseKeeping.QueryValue = UrlDecode(rv);
                        loadByQuery = true;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("GoodHouseKeepingList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "GoodHouseKeepingList") {
                        ...
                        returnUrl = "GoodHouseKeepingList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            if (!IdGoodHouseKeeping.IsDetailKey) {
                SetNormalField(IdGoodHouseKeeping, "IdGoodHouseKeeping", "x_IdGoodHouseKeeping");
            }

            // Standard handling for 'NomorGoodHouseKeeping'
            NomorGoodHouseKeeping.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NomorGoodHouseKeeping, "NomorGoodHouseKeeping", "x_NomorGoodHouseKeeping");
        ...
        private void ValidateCustomIdGoodHouseKeeping() {
            if (IdGoodHouseKeeping.Visible && IdGoodHouseKeeping.Required) {
                if (!IdGoodHouseKeeping.IsDetailKey && Empty(IdGoodHouseKeeping.FormValue)) {
                    IdGoodHouseKeeping.AddErrorMessage(ConvertToString(Catatan.RequiredErrorMessage).Replace("%s", Catatan.Caption));
                }
            }
        }

        private void ValidateCustomNomorGoodHouseKeeping() {
            if (NomorGoodHouseKeeping.Visible && NomorGoodHouseKeeping.Required) {
                if (!NomorGoodHouseKeeping.IsDetailKey && Empty(NomorGoodHouseKeeping.FormValue)) {
                    NomorGoodHouseKeeping.AddErrorMessage(ConvertToString(Catatan.RequiredErrorMessage).Replace("%s", Catatan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomPlant() {
            if (Plant.Visible && Plant.Required) {
                if (!Plant.IsDetailKey && Empty(Plant.FormValue)) {
                    Plant.AddErrorMessage(ConvertToString(Catatan.RequiredErrorMessage).Replace("%s", Catatan.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ValidateCustomIdGoodHouseKeeping();
                ValidateCustomNomorGoodHouseKeeping();
                ValidateCustomPlant();
