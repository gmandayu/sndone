...
public partial class SnDOne {
    ...
    public class VSampleOperasionalListBase : VSampleOperasional
    {
        ...
        // Constructor
        public VSampleOperasionalListBase(Controller? controller)
        {
        ...
        // Constructor
        public VSampleOperasionalListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field RedirectLink
            if (filter.TryGetValue("x_RedirectLink", out sv)) {
                RestoreFieldFilter(RedirectLink, filter, "RedirectLink", sv);
            }

            // Field Proses
            if (filter.TryGetValue("x_Proses2", out sv)) {
                RestoreFieldFilter(Proses2, filter, "Proses2", sv);
            }

            // Field Plant
            if (filter.TryGetValue("x_Plant", out sv)) {
                RestoreFieldFilter(Plant, filter, "Plant", sv);
            }

            // Field Moda
            if (filter.TryGetValue("x_Moda", out sv)) {
                RestoreFieldFilter(Moda, filter, "Moda", sv);
            }
            ...
            // Field NamaDokumen
            if (filter.TryGetValue("x_NamaDokumen", out sv)) {
                RestoreFieldFilter(NamaDokumen, filter, "NamaDokumen", sv);
            }
            ...
            if (filter.TryGetValue("x_IdAktivitasDokumen", out sv)) {
                RestoreFieldFilter(IdAktivitasDokumen, filter, "IdAktivitasDokumen", sv);
            }

            // Field StatusUpload
            if (filter.TryGetValue("x_StatusUpload", out sv)) {
                RestoreFieldFilter(StatusUpload, filter, "StatusUpload", sv);
            }
        }
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("Moda", Moda, true, config);
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldModaSearchValues()
        {
            // Moda
            LoadPrimarySearchValueModa();
            LoadSearchOperatorModa();
        }

        private void LoadPrimarySearchValueModa()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Moda[]")) {
        ...
        private void LoadSearchOperatorModa()
        {
        ...
        private void LoadFieldIdAktivitasDokumenSearchValues()
        {
            // IdAktivitasDokumen
            LoadPrimarySearchValueIdAktivitasDokumen();
            LoadSearchOperatorIdAktivitasDokumen();
        }

        private void LoadPrimarySearchValueIdAktivitasDokumen()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdAktivitasDokumen")) {
                IdAktivitasDokumen.AdvancedSearch.SearchValue = Get("x_IdAktivitasDokumen");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorIdAktivitasDokumen()
        {
            if (Query.ContainsKey("z_IdAktivitasDokumen"))
                IdAktivitasDokumen.AdvancedSearch.SearchOperator = Get("z_IdAktivitasDokumen");
        }
        ...
        private void LoadPrimarySearchValueStatusUpload()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_StatusUpload")) {
                StatusUpload.AdvancedSearch.SearchValue = Get("x_StatusUpload");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorStatusUpload()
        {
            if (Query.ContainsKey("z_StatusUpload"))
                StatusUpload.AdvancedSearch.SearchOperator = Get("z_StatusUpload");
        }
