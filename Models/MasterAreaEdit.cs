...
public partial class SnDOne {
    ...
    /// masterAreaEdit
    /// </summary>
    public static MasterAreaEdit masterAreaEdit
    {
        get => HttpData.Get<MasterAreaEdit>("masterAreaEdit")!;
        set => HttpData["masterAreaEdit"] = value;
    }

    /// <summary>
    /// Page class for MasterArea
    /// </summary>
    public class MasterAreaEdit : MasterAreaEditBase
    {
        // Constructor
        public MasterAreaEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterAreaEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterAreaEditBase : MasterArea
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterAreaEdit";
        ...
        // Constructor
        public MasterAreaEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "MasterAreaEdit";
        ...
        // Constructor
        public MasterAreaEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("MasterAreaList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "MasterAreaList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "MasterAreaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'Area'
            Area.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Area, "Area", "x_Area");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomID() {
            if (ID.Visible && ID.Required) {
                if (!ID.IsDetailKey && Empty(ID.FormValue)) {
                    ID.AddErrorMessage(ConvertToString(Area.RequiredErrorMessage).Replace("%s", Area.Caption));
                }
            }
        }

        private void ValidateCustomArea() {
            if (Area.Visible && Area.Required) {
        ...
        {
            object? value;
            if (row.TryGetValue("Area", out value)) // Area
                Area.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MasterAreaList")), "", TableVar, true);
            string pageId = "edit";
