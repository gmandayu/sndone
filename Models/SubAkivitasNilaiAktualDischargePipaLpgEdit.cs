...
public partial class SnDOne {
    /// <summary>
    /// subAkivitasNilaiAktualDischargePipaLpgEdit
    /// </summary>
    public static SubAkivitasNilaiAktualDischargePipaLpgEdit subAkivitasNilaiAktualDischargePipaLpgEdit
    {
        get => HttpData.Get<SubAkivitasNilaiAktualDischargePipaLpgEdit>("subAkivitasNilaiAktualDischargePipaLpgEdit")!;
        set => HttpData["subAkivitasNilaiAktualDischargePipaLpgEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAkivitasNilaiAktualDischargePipaLPG
    /// </summary>
    public class SubAkivitasNilaiAktualDischargePipaLpgEdit : SubAkivitasNilaiAktualDischargePipaLpgEditBase
    {
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaLpgEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAkivitasNilaiAktualDischargePipaLpgEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAkivitasNilaiAktualDischargePipaLpgEditBase : SubAkivitasNilaiAktualDischargePipaLpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAkivitasNilaiAktualDischargePipaLpgEdit";
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaLpgEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAkivitasNilaiAktualDischargePipaLpgEdit";
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaLpgEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAkivitasNilaiAktualDischargePipaLpgList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAkivitasNilaiAktualDischargePipaLpgList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAkivitasNilaiAktualDischargePipaLpgList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAkivitasNilaiAktualDischargePipaLpgEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(AktualDischarge_mt.RequiredErrorMessage).Replace("%s", AktualDischarge_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(AktualDischarge_mt.RequiredErrorMessage).Replace("%s", AktualDischarge_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(AktualDischarge_mt.RequiredErrorMessage).Replace("%s", AktualDischarge_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(AktualDischarge_mt.RequiredErrorMessage).Replace("%s", AktualDischarge_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(AktualDischarge_mt.RequiredErrorMessage).Replace("%s", AktualDischarge_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(AktualDischarge_mt.RequiredErrorMessage).Replace("%s", AktualDischarge_mt.Caption));
                }
            }
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAkivitasNilaiAktualDischargePipaLpgList")), "", TableVar, true);
            string pageId = "edit";
