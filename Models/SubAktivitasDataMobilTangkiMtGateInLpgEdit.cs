...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasDataMobilTangkiMtGateInLpgEdit
    /// </summary>
    public static SubAktivitasDataMobilTangkiMtGateInLpgEdit subAktivitasDataMobilTangkiMtGateInLpgEdit
    {
        get => HttpData.Get<SubAktivitasDataMobilTangkiMtGateInLpgEdit>("subAktivitasDataMobilTangkiMtGateInLpgEdit")!;
        set => HttpData["subAktivitasDataMobilTangkiMtGateInLpgEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasDataMobilTangkiMTGateInLPG
    /// </summary>
    public class SubAktivitasDataMobilTangkiMtGateInLpgEdit : SubAktivitasDataMobilTangkiMtGateInLpgEditBase
    {
        // Constructor
        public SubAktivitasDataMobilTangkiMtGateInLpgEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasDataMobilTangkiMtGateInLpgEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasDataMobilTangkiMtGateInLpgEditBase : SubAktivitasDataMobilTangkiMtGateInLpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasDataMobilTangkiMtGateInLpgEdit";
        ...
        // Constructor
        public SubAktivitasDataMobilTangkiMtGateInLpgEditBase(Controller? controller)
        {
            TableName = "SubAktivitasDataMobilTangkiMTGateInLPG";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasDataMobilTangkiMtGateInLpg)
            if (subAktivitasDataMobilTangkiMtGateInLpg == null || subAktivitasDataMobilTangkiMtGateInLpg is SubAktivitasDataMobilTangkiMtGateInLpg)
                subAktivitasDataMobilTangkiMtGateInLpg = this;
        ...
        // Page name
        public string PageName => "SubAktivitasDataMobilTangkiMtGateInLpgEdit";
        ...
        // Constructor
        public SubAktivitasDataMobilTangkiMtGateInLpgEditBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                ...
                // List / View / Master View page
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasDataMobilTangkiMtGateInLpgView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasDataMobilTangkiMtGateInLpgList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasDataMobilTangkiMtGateInLpgList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasDataMobilTangkiMtGateInLpgList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasDataMobilTangkiMtGateInLpgEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'AMT'
            AMT.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AMT, "AMT", "x_AMT");

            // Standard handling for 'WaktuGateIn'
            WaktuGateIn.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(WaktuGateIn, "WaktuGateIn", "x_WaktuGateIn", true, validate, true);
        ...
        public void RestoreFormValues()
        {
            ...
            AMT.CurrentValue = AMT.FormValue;
            WaktuGateIn.CurrentValue = WaktuGateIn.FormValue;
            WaktuGateIn.CurrentValue = UnformatDateTime(WaktuGateIn.CurrentValue, WaktuGateIn.FormatPattern);
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            AMT.SetDbValue(row["AMT"]);
            WaktuGateIn.SetDbValue(row["WaktuGateIn"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("AMT", AMT.DefaultValue ?? DbNullValue); // DN
            row.Add("WaktuGateIn", WaktuGateIn.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // TimbanganAwal
                    ...
                    // AMT
                    AMT.ViewValue = ConvertToString(AMT.CurrentValue); // DN
                    AMT.ViewCustomAttributes = "";

                    // WaktuGateIn
                    WaktuGateIn.ViewValue = WaktuGateIn.CurrentValue;
                    WaktuGateIn.ViewValue = FormatDateTime(WaktuGateIn.ViewValue, WaktuGateIn.FormatPattern);
                    WaktuGateIn.ViewCustomAttributes = "";
                ...
                // AMT
                AMT.SetupEditAttributes();
                if (!AMT.Raw)
                    AMT.CurrentValue = HtmlDecode(AMT.CurrentValue);
                AMT.EditValue = HtmlEncode(AMT.CurrentValue);
                AMT.PlaceHolder = RemoveHtml(AMT.Caption);

                // WaktuGateIn
                WaktuGateIn.SetupEditAttributes();
                WaktuGateIn.EditValue = FormatDateTime(WaktuGateIn.CurrentValue, WaktuGateIn.FormatPattern);
                WaktuGateIn.PlaceHolder = RemoveHtml(WaktuGateIn.Caption);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorPolisi() {
            if (NomorPolisi.Visible && NomorPolisi.Required) {
                if (!NomorPolisi.IsDetailKey && Empty(NomorPolisi.FormValue)) {
                    NomorPolisi.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                }
            }
        }

        private void ValidateCustomAMT() {
            if (AMT.Visible && AMT.Required) {
                if (!AMT.IsDetailKey && Empty(AMT.FormValue)) {
                    AMT.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                }
            }
        }

        private void ValidateCustomWaktuGateIn() {
            if (WaktuGateIn.Visible && WaktuGateIn.Required) {
                if (!WaktuGateIn.IsDetailKey && Empty(WaktuGateIn.FormValue)) {
                    WaktuGateIn.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomAMT();
                ValidateCustomWaktuGateIn();
                if (!CheckDate(WaktuGateIn.FormValue, WaktuGateIn.FormatPattern)) {
                    WaktuGateIn.AddErrorMessage(WaktuGateIn.GetErrorMessage(false));
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // AMT
            AMT.SetDbValue(rsnew, AMT.CurrentValue, AMT.ReadOnly);

            // WaktuGateIn
            WaktuGateIn.SetDbValue(rsnew, ConvertToDateTime(WaktuGateIn.CurrentValue, WaktuGateIn.FormatPattern), WaktuGateIn.ReadOnly);
        ...
        {
            object? value;
            if (row.TryGetValue("AMT", out value)) // AMT
                AMT.CurrentValue = value;
            if (row.TryGetValue("WaktuGateIn", out value)) // WaktuGateIn
                WaktuGateIn.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasDataMobilTangkiMtGateInLpgList")), "", TableVar, true);
            string pageId = "edit";
