...
public partial class SnDOne {
    ...
    public class MwtOnlineAddBase : MwtOnline
    {
        ...
        // Constructor
        public MwtOnlineAddBase(Controller? controller)
        {
            TableName = "MWTOnline";
        ...
        // Constructor
        public MwtOnlineAddBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'KeteranganFile'
            KeteranganFile.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(KeteranganFile, "KeteranganFile", "x_KeteranganFile");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomLinkRedirect() {
            if (LinkRedirect.Visible && LinkRedirect.Required) {
                if (!LinkRedirect.IsDetailKey && Empty(LinkRedirect.FormValue)) {
                    LinkRedirect.AddErrorMessage(ConvertToString(KeteranganFile.RequiredErrorMessage).Replace("%s", KeteranganFile.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLookupPlant() {
            if (LookupPlant.Visible && LookupPlant.Required) {
                if (!LookupPlant.IsDetailKey && Empty(LookupPlant.FormValue)) {
                    LookupPlant.AddErrorMessage(ConvertToString(KeteranganFile.RequiredErrorMessage).Replace("%s", KeteranganFile.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTaskList() {
            if (TaskList.Visible && TaskList.Required) {
                if (!TaskList.IsDetailKey && Empty(TaskList.FormValue)) {
                    TaskList.AddErrorMessage(ConvertToString(KeteranganFile.RequiredErrorMessage).Replace("%s", KeteranganFile.Caption));
                }
            }
        }

        private void ValidateCustomKeteranganFile() {
