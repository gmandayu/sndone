...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasLogbookPenerimaanPenyaluranList
    /// </summary>
    public static SubAktivitasLogbookPenerimaanPenyaluranList subAktivitasLogbookPenerimaanPenyaluranList
    {
        get => HttpData.Get<SubAktivitasLogbookPenerimaanPenyaluranList>("subAktivitasLogbookPenerimaanPenyaluranList")!;
        set => HttpData["subAktivitasLogbookPenerimaanPenyaluranList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasLogbookPenerimaanPenyaluran
    /// </summary>
    public class SubAktivitasLogbookPenerimaanPenyaluranList : SubAktivitasLogbookPenerimaanPenyaluranListBase
    {
        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasLogbookPenerimaanPenyaluranListBase : SubAktivitasLogbookPenerimaanPenyaluran
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasLogbookPenerimaanPenyaluranList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasLogbookPenerimaanPenyaluranlist";
        ...
        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranListBase()
        {
            ...
            // Initialize URLs
            AddUrl = "SubAktivitasLogbookPenerimaanPenyaluranAdd";
            ...
            MultiDeleteUrl = "SubAktivitasLogbookPenerimaanPenyaluranDelete";
            MultiUpdateUrl = "SubAktivitasLogbookPenerimaanPenyaluranUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasLogbookPenerimaanPenyaluranList";
        ...
        // Constructor
        public SubAktivitasLogbookPenerimaanPenyaluranListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasLogbookPenerimaanPenyalurangrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasLogbookPenerimaanPenyaluranList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(InLineBlending.AdvancedSearch.ToJson())); // Field InLineBlending
            filters.Merge(JObject.Parse(NomorTangki2.AdvancedSearch.ToJson())); // Field NomorTangki2
        ...
        protected bool RestoreFilterList() {
            ...
            // Field InLineBlending
            if (filter?.TryGetValue("x_InLineBlending", out sv) ?? false) {
                InLineBlending.AdvancedSearch.SearchValue = sv;
                InLineBlending.AdvancedSearch.SearchOperator = filter["z_InLineBlending"];
                InLineBlending.AdvancedSearch.SearchCondition = filter["v_InLineBlending"];
                InLineBlending.AdvancedSearch.SearchValue2 = filter["y_InLineBlending"];
                InLineBlending.AdvancedSearch.SearchOperator2 = filter["w_InLineBlending"];
                InLineBlending.AdvancedSearch.Save();
            }

            // Field NomorTangki2
            if (filter?.TryGetValue("x_NomorTangki2", out sv) ?? false) {
                NomorTangki2.AdvancedSearch.SearchValue = sv;
                NomorTangki2.AdvancedSearch.SearchOperator = filter["z_NomorTangki2"];
                NomorTangki2.AdvancedSearch.SearchCondition = filter["v_NomorTangki2"];
                NomorTangki2.AdvancedSearch.SearchValue2 = filter["y_NomorTangki2"];
                NomorTangki2.AdvancedSearch.SearchOperator2 = filter["w_NomorTangki2"];
                NomorTangki2.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NomorKompartment);
            searchFlds.Add(NomorTangki2);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(NomorKompartment, ctrl); // NomorKompartment
                UpdateSort(InLineBlending, ctrl); // InLineBlending
                UpdateSort(NomorTangki2, ctrl); // NomorTangki2
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasLogbookPenerimaanPenyaluran"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasLogbookPenerimaanPenyaluranlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasLogbookPenerimaanPenyaluranlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("NomorKompartment")); // DN
                CreateColumnOption(option.Add("InLineBlending")); // DN
                CreateColumnOption(option.Add("NomorTangki2")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasLogbookPenerimaanPenyaluransrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasLogbookPenerimaanPenyaluransrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasLogbookPenerimaanPenyaluranlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasLogbookPenerimaanPenyaluran");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasLogbookPenerimaanPenyaluranList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasLogbookPenerimaanPenyaluranList.RowCount) + "_SubAktivitasLogbookPenerimaanPenyaluran");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasLogbookPenerimaanPenyaluranList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasLogbookPenerimaanPenyaluranList.RowType == RowType.Add || IsEdit && subAktivitasLogbookPenerimaanPenyaluranList.RowType == RowType.Edit) // Inline-Add/Edit row
