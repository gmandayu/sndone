...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasFormInputBLnSfalrtwEdit
    /// </summary>
    public static SubAktivitasFormInputBLnSfalrtwEdit subAktivitasFormInputBLnSfalrtwEdit
    {
        get => HttpData.Get<SubAktivitasFormInputBLnSfalrtwEdit>("subAktivitasFormInputBLnSfalrtwEdit")!;
        set => HttpData["subAktivitasFormInputBLnSfalrtwEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasFormInputBLnSFALRTW
    /// </summary>
    public class SubAktivitasFormInputBLnSfalrtwEdit : SubAktivitasFormInputBLnSfalrtwEditBase
    {
        // Constructor
        public SubAktivitasFormInputBLnSfalrtwEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasFormInputBLnSfalrtwEdit() : base()
        {
        }
        ...
        public override void PageRender() {
            ...
            string sql = @"
                SELECT
                    ...
                    SA.AL_MT
                FROM Penerimaan P
                LEFT JOIN Penyaluran N ON P.IdPenyaluran = N.IdPenyaluran
                LEFT JOIN SubAktivitasNilaiBLdanALRTW SA ON N.NomorPenyaluran = SA.NoReferensi
                WHERE P.NomorPenerimaan = @ref;";
            var param = new Dictionary<string, object> { { "@ref", noRef } };
            var data = DatabaseQueryHelper.ExecuteSelectSingle<(
                string BL_KL_Obs, 
                string BL_KL_15, 
                string BL_Barrels, 
                string BL_LT, 
                ...
                string AL_LT, 
                string AL_MT
            )>(query: sql, parameters: param, noReferensi: noRef, mapFunc: r => 
                {
            ...
            if (EqualityComparer<(
                ...
                string, string, string, string, string
            )>.Default.Equals(data, default))
                return;
            void FillIfEmpty(DbField field, string val)
            {
                var cur = Convert.ToString(field.CurrentValue);
                if (string.IsNullOrWhiteSpace(cur))
                    field.EditValue = val;
            }
            FillIfEmpty(BL_KLObs, data.BL_KL_Obs);
            FillIfEmpty(BL_KL15, data.BL_KL_15);
            FillIfEmpty(BL_Barrels, data.BL_Barrels);
            FillIfEmpty(BL_LT, data.BL_LT);
            FillIfEmpty(BL_MT, data.BL_MT);
            FillIfEmpty(SFAL_KLObs, data.AL_KL_Obs);
            FillIfEmpty(SFAL_KL15, data.AL_KL_15);
            FillIfEmpty(SFAL_Barrels, data.AL_Barrels);
            FillIfEmpty(SFAL_LT, data.AL_LT);
            FillIfEmpty(SFAL_MT, data.AL_MT);
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasFormInputBLnSfalrtwEditBase : SubAktivitasFormInputBLnSfalrtw
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasFormInputBLnSfalrtwEdit";
        ...
        // Constructor
        public SubAktivitasFormInputBLnSfalrtwEditBase()
        {
        ...
        // Page name
        public string PageName => "SubAktivitasFormInputBLnSfalrtwEdit";
        ...
        // Constructor
        public SubAktivitasFormInputBLnSfalrtwEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasFormInputBLnSfalrtwList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasFormInputBLnSfalrtwList") {
                                ...
                                returnUrl = "SubAktivitasFormInputBLnSfalrtwList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasFormInputBLnSfalrtwEdit?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
