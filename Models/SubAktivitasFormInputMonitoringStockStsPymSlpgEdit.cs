...
public partial class SnDOne {
    ...
    public class SubAktivitasFormInputMonitoringStockStsPymSlpgEditBase : SubAktivitasFormInputMonitoringStockStsPymSlpg
    {
        ...
        // Constructor
        public SubAktivitasFormInputMonitoringStockStsPymSlpgEditBase()
        {
        ...
        // Constructor
        public SubAktivitasFormInputMonitoringStockStsPymSlpgEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasFormInputMonitoringStockStsPymSlpgList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasFormInputMonitoringStockStsPymSlpgList") {
                                ...
                                returnUrl = "SubAktivitasFormInputMonitoringStockStsPymSlpgList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NomorTangki' before field var 'x_NomorTangki'
            val = CurrentForm.HasValue("NomorTangki") ? CurrentForm.GetValue("NomorTangki") : CurrentForm.GetValue("x_NomorTangki");
            if (!NomorTangki.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NomorTangki") && !CurrentForm.HasValue("x_NomorTangki")) // DN
                    NomorTangki.Visible = false; // Disable update for API request
                else
                    NomorTangki.SetFormValue(val);
            }
            ...
            // Check field name 'QuantityAwal' before field var 'x_QuantityAwal'
            val = CurrentForm.HasValue("QuantityAwal") ? CurrentForm.GetValue("QuantityAwal") : CurrentForm.GetValue("x_QuantityAwal");
            if (!QuantityAwal.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("QuantityAwal") && !CurrentForm.HasValue("x_QuantityAwal")) // DN
                    QuantityAwal.Visible = false; // Disable update for API request
                else
                    QuantityAwal.SetFormValue(val);
            }

            // Check field name 'QuantityAkhir' before field var 'x_QuantityAkhir'
            val = CurrentForm.HasValue("QuantityAkhir") ? CurrentForm.GetValue("QuantityAkhir") : CurrentForm.GetValue("x_QuantityAkhir");
            if (!QuantityAkhir.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("QuantityAkhir") && !CurrentForm.HasValue("x_QuantityAkhir")) // DN
                    QuantityAkhir.Visible = false; // Disable update for API request
                else
                    QuantityAkhir.SetFormValue(val);
            }
            ...
            // Check field name 'TanggalQuantityAwal' before field var 'x_TanggalQuantityAwal'
            val = CurrentForm.HasValue("TanggalQuantityAwal") ? CurrentForm.GetValue("TanggalQuantityAwal") : CurrentForm.GetValue("x_TanggalQuantityAwal");
            if (!TanggalQuantityAwal.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TanggalQuantityAwal") && !CurrentForm.HasValue("x_TanggalQuantityAwal")) // DN
                    TanggalQuantityAwal.Visible = false; // Disable update for API request
                else
                    TanggalQuantityAwal.SetFormValue(val, true, validate);
                TanggalQuantityAwal.CurrentValue = UnformatDateTime(TanggalQuantityAwal.CurrentValue, TanggalQuantityAwal.FormatPattern);
            }

            // Check field name 'TanggalQuantityAkhir' before field var 'x_TanggalQuantityAkhir'
            val = CurrentForm.HasValue("TanggalQuantityAkhir") ? CurrentForm.GetValue("TanggalQuantityAkhir") : CurrentForm.GetValue("x_TanggalQuantityAkhir");
            if (!TanggalQuantityAkhir.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TanggalQuantityAkhir") && !CurrentForm.HasValue("x_TanggalQuantityAkhir")) // DN
                    TanggalQuantityAkhir.Visible = false; // Disable update for API request
                else
                    TanggalQuantityAkhir.SetFormValue(val, true, validate);
            ...
            bool validateForm = true;
                ...
                if (NomorTangki.Visible && NomorTangki.Required) {
                    if (!NomorTangki.IsDetailKey && Empty(NomorTangki.FormValue)) {
                        NomorTangki.AddErrorMessage(ConvertToString(NomorTangki.RequiredErrorMessage).Replace("%s", NomorTangki.Caption));
                    }
                }
                ...
                if (QuantityAwal.Visible && QuantityAwal.Required) {
                    if (!QuantityAwal.IsDetailKey && Empty(QuantityAwal.FormValue)) {
                        QuantityAwal.AddErrorMessage(ConvertToString(QuantityAwal.RequiredErrorMessage).Replace("%s", QuantityAwal.Caption));
                    }
                }
                if (QuantityAkhir.Visible && QuantityAkhir.Required) {
                    if (!QuantityAkhir.IsDetailKey && Empty(QuantityAkhir.FormValue)) {
                        QuantityAkhir.AddErrorMessage(ConvertToString(QuantityAkhir.RequiredErrorMessage).Replace("%s", QuantityAkhir.Caption));
                    }
                }
                ...
                if (TanggalQuantityAwal.Visible && TanggalQuantityAwal.Required) {
                    if (!TanggalQuantityAwal.IsDetailKey && Empty(TanggalQuantityAwal.FormValue)) {
                        TanggalQuantityAwal.AddErrorMessage(ConvertToString(TanggalQuantityAwal.RequiredErrorMessage).Replace("%s", TanggalQuantityAwal.Caption));
                    }
                }
