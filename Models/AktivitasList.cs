...
public partial class SnDOne {
    ...
    public class AktivitasListBase : Aktivitas
    {
        ...
        // Constructor
        public AktivitasListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected bool RestoreFilterList() {
            ...
            // Field TanggalMulai
            if (filter?.TryGetValue("x_TanggalMulai", out sv) ?? false) {
                TanggalMulai.AdvancedSearch.SearchValue = sv;
                TanggalMulai.AdvancedSearch.SearchOperator = filter["z_TanggalMulai"];
                TanggalMulai.AdvancedSearch.SearchCondition = filter["v_TanggalMulai"];
                TanggalMulai.AdvancedSearch.SearchValue2 = filter["y_TanggalMulai"];
                TanggalMulai.AdvancedSearch.SearchOperator2 = filter["w_TanggalMulai"];
                TanggalMulai.AdvancedSearch.Save();
            }

            // Field TanggalSelesai
            if (filter?.TryGetValue("x_TanggalSelesai", out sv) ?? false) {
                TanggalSelesai.AdvancedSearch.SearchValue = sv;
                TanggalSelesai.AdvancedSearch.SearchOperator = filter["z_TanggalSelesai"];
                TanggalSelesai.AdvancedSearch.SearchCondition = filter["v_TanggalSelesai"];
                TanggalSelesai.AdvancedSearch.SearchValue2 = filter["y_TanggalSelesai"];
                TanggalSelesai.AdvancedSearch.SearchOperator2 = filter["w_TanggalSelesai"];
                TanggalSelesai.AdvancedSearch.Save();
            }
            ...
            if (filter?.TryGetValue("x_sandar_tgl_tiba", out sv) ?? false) {
                sandar_tgl_tiba.AdvancedSearch.SearchValue = sv;
                sandar_tgl_tiba.AdvancedSearch.SearchOperator = filter["z_sandar_tgl_tiba"];
                sandar_tgl_tiba.AdvancedSearch.SearchCondition = filter["v_sandar_tgl_tiba"];
                sandar_tgl_tiba.AdvancedSearch.SearchValue2 = filter["y_sandar_tgl_tiba"];
                sandar_tgl_tiba.AdvancedSearch.SearchOperator2 = filter["w_sandar_tgl_tiba"];
                sandar_tgl_tiba.AdvancedSearch.Save();
            }

            // Field TanggalUploadDok
            if (filter?.TryGetValue("x_TanggalUploadDok", out sv) ?? false) {
                TanggalUploadDok.AdvancedSearch.SearchValue = sv;
                TanggalUploadDok.AdvancedSearch.SearchOperator = filter["z_TanggalUploadDok"];
                TanggalUploadDok.AdvancedSearch.SearchCondition = filter["v_TanggalUploadDok"];
                TanggalUploadDok.AdvancedSearch.SearchValue2 = filter["y_TanggalUploadDok"];
                TanggalUploadDok.AdvancedSearch.SearchOperator2 = filter["w_TanggalUploadDok"];
                TanggalUploadDok.AdvancedSearch.Save();
            }

            // Field IsNominationTankReceivingLineOpen
            if (filter?.TryGetValue("x_IsNominationTankReceivingLineOpen", out sv) ?? false) {
                IsNominationTankReceivingLineOpen.AdvancedSearch.SearchValue = sv;
                IsNominationTankReceivingLineOpen.AdvancedSearch.SearchOperator = filter["z_IsNominationTankReceivingLineOpen"];
                IsNominationTankReceivingLineOpen.AdvancedSearch.SearchCondition = filter["v_IsNominationTankReceivingLineOpen"];
                IsNominationTankReceivingLineOpen.AdvancedSearch.SearchValue2 = filter["y_IsNominationTankReceivingLineOpen"];
                IsNominationTankReceivingLineOpen.AdvancedSearch.SearchOperator2 = filter["w_IsNominationTankReceivingLineOpen"];
                IsNominationTankReceivingLineOpen.AdvancedSearch.Save();
            }

            // Field IsNonNominationReceivingLineClosedAndSealed
            if (filter?.TryGetValue("x_IsNonNominationReceivingLineClosedAndSealed", out sv) ?? false) {
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchValue = sv;
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchOperator = filter["z_IsNonNominationReceivingLineClosedAndSealed"];
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchCondition = filter["v_IsNonNominationReceivingLineClosedAndSealed"];
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchValue2 = filter["y_IsNonNominationReceivingLineClosedAndSealed"];
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchOperator2 = filter["w_IsNonNominationReceivingLineClosedAndSealed"];
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.Save();
            }

            // Field IsTankEmptyAndDry
            if (filter?.TryGetValue("x_IsTankEmptyAndDry", out sv) ?? false) {
                IsTankEmptyAndDry.AdvancedSearch.SearchValue = sv;
                IsTankEmptyAndDry.AdvancedSearch.SearchOperator = filter["z_IsTankEmptyAndDry"];
                IsTankEmptyAndDry.AdvancedSearch.SearchCondition = filter["v_IsTankEmptyAndDry"];
                IsTankEmptyAndDry.AdvancedSearch.SearchValue2 = filter["y_IsTankEmptyAndDry"];
                IsTankEmptyAndDry.AdvancedSearch.SearchOperator2 = filter["w_IsTankEmptyAndDry"];
                IsTankEmptyAndDry.AdvancedSearch.Save();
            }

            // Field IsDocumentationComplete
            if (filter?.TryGetValue("x_IsDocumentationComplete", out sv) ?? false) {
                IsDocumentationComplete.AdvancedSearch.SearchValue = sv;
                IsDocumentationComplete.AdvancedSearch.SearchOperator = filter["z_IsDocumentationComplete"];
                IsDocumentationComplete.AdvancedSearch.SearchCondition = filter["v_IsDocumentationComplete"];
                IsDocumentationComplete.AdvancedSearch.SearchValue2 = filter["y_IsDocumentationComplete"];
                IsDocumentationComplete.AdvancedSearch.SearchOperator2 = filter["w_IsDocumentationComplete"];
                IsDocumentationComplete.AdvancedSearch.Save();
            }
        ...
        public void ShowFilterList()
        {
            ...
            // Field No_Referensi
            filter = QueryBuilderWhere("No_Referensi");
            if (Empty(filter))
                BuildSearchSql(ref filter, No_Referensi, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + No_Referensi.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field IdProses
            filter = QueryBuilderWhere("IdProses");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdProses, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdProses.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Aktivitas
            filter = QueryBuilderWhere("Aktivitas");
            if (Empty(filter))
                BuildSearchSql(ref filter, Aktivitas2, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Aktivitas2.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field PIC
            filter = QueryBuilderWhere("PIC");
            if (Empty(filter))
                BuildSearchSql(ref filter, PIC, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + PIC.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field TanggalMulai
            filter = QueryBuilderWhere("TanggalMulai");
            if (Empty(filter))
                BuildSearchSql(ref filter, TanggalMulai, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TanggalMulai.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field TanggalSelesai
            filter = QueryBuilderWhere("TanggalSelesai");
            if (Empty(filter))
                BuildSearchSql(ref filter, TanggalSelesai, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TanggalSelesai.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field StatusAktivitas
            filter = QueryBuilderWhere("StatusAktivitas");
            if (Empty(filter))
                BuildSearchSql(ref filter, StatusAktivitas, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + StatusAktivitas.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field TipeAktivitas
            filter = QueryBuilderWhere("TipeAktivitas");
            if (Empty(filter))
                BuildSearchSql(ref filter, TipeAktivitas, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TipeAktivitas.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field TipeProses
            filter = QueryBuilderWhere("TipeProses");
            if (Empty(filter))
                BuildSearchSql(ref filter, TipeProses, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TipeProses.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Urutan
            filter = QueryBuilderWhere("Urutan");
            if (Empty(filter))
                BuildSearchSql(ref filter, Urutan, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Urutan.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field IsNominationTankReceivingLineOpen
            filter = QueryBuilderWhere("IsNominationTankReceivingLineOpen");
            if (Empty(filter))
                BuildSearchSql(ref filter, IsNominationTankReceivingLineOpen, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IsNominationTankReceivingLineOpen.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field IsNonNominationReceivingLineClosedAndSealed
            filter = QueryBuilderWhere("IsNonNominationReceivingLineClosedAndSealed");
            if (Empty(filter))
                BuildSearchSql(ref filter, IsNonNominationReceivingLineClosedAndSealed, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IsNonNominationReceivingLineClosedAndSealed.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field IsTankEmptyAndDry
            filter = QueryBuilderWhere("IsTankEmptyAndDry");
            if (Empty(filter))
                BuildSearchSql(ref filter, IsTankEmptyAndDry, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IsTankEmptyAndDry.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field IsDocumentationComplete
            filter = QueryBuilderWhere("IsDocumentationComplete");
            if (Empty(filter))
                BuildSearchSql(ref filter, IsDocumentationComplete, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IsDocumentationComplete.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        protected void LoadSearchValues() {
            ...
            // No_Referensi
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_No_Referensi[]"))
            ...
            // IdProses
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdProses[]"))
            ...
            // IdTemplateAktivitas
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdTemplateAktivitas[]"))
            ...
            // Aktivitas2
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Aktivitas2[]"))
            ...
            // NamaAktivitas
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaAktivitas[]"))
            ...
            // TanggalMulai
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TanggalMulai[]"))
            ...
            // TanggalSelesai
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TanggalSelesai[]"))
            ...
            // StatusAktivitas
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_StatusAktivitas[]"))
            ...
            // TipeAktivitas
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TipeAktivitas[]"))
            ...
            // sandar_nama_kapal
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_sandar_nama_kapal[]"))
            ...
            // sandar_tgl_tiba
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_sandar_tgl_tiba[]"))
            ...
            // sandar_nominasi
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_sandar_nominasi[]"))
            ...
            // Shipment
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Shipment[]"))
            ...
            // Nama_Proses
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Nama_Proses[]"))
            ...
            // IdDokumen
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdDokumen[]"))
            ...
            // PathFile
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_PathFile[]"))
            ...
            // TanggalUploadDok
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TanggalUploadDok[]"))
            ...
            // UserUploadDok
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_UserUploadDok[]"))
            ...
            // NamaDokumen
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaDokumen[]"))
            ...
            // IdRefAnak
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdRefAnak[]"))
            ...
            // TableAnak
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TableAnak[]"))
            ...
            // TipeProses
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TipeProses[]"))
            ...
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IsNominationTankReceivingLineOpen"))
                    IsNominationTankReceivingLineOpen.AdvancedSearch.SearchValue = Get("x_IsNominationTankReceivingLineOpen", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z_IsNominationTankReceivingLineOpen"))
                IsNominationTankReceivingLineOpen.AdvancedSearch.SearchOperator = Get("z_IsNominationTankReceivingLineOpen", Config.FilterOptionSeparator);

            // IsNonNominationReceivingLineClosedAndSealed
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IsNonNominationReceivingLineClosedAndSealed"))
                    IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchValue = Get("x_IsNonNominationReceivingLineClosedAndSealed", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z_IsNonNominationReceivingLineClosedAndSealed"))
                IsNonNominationReceivingLineClosedAndSealed.AdvancedSearch.SearchOperator = Get("z_IsNonNominationReceivingLineClosedAndSealed", Config.FilterOptionSeparator);

            // IsTankEmptyAndDry
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IsTankEmptyAndDry"))
                    IsTankEmptyAndDry.AdvancedSearch.SearchValue = Get("x_IsTankEmptyAndDry", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z_IsTankEmptyAndDry"))
                IsTankEmptyAndDry.AdvancedSearch.SearchOperator = Get("z_IsTankEmptyAndDry", Config.FilterOptionSeparator);

            // IsDocumentationComplete
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IsDocumentationComplete"))
                    IsDocumentationComplete.AdvancedSearch.SearchValue = Get("x_IsDocumentationComplete", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z_IsDocumentationComplete"))
                IsDocumentationComplete.AdvancedSearch.SearchOperator = Get("z_IsDocumentationComplete", Config.FilterOptionSeparator);
        }
