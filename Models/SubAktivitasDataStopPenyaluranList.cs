...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasDataStopPenyaluranList
    /// </summary>
    public static SubAktivitasDataStopPenyaluranList subAktivitasDataStopPenyaluranList
    {
        get => HttpData.Get<SubAktivitasDataStopPenyaluranList>("subAktivitasDataStopPenyaluranList")!;
        set => HttpData["subAktivitasDataStopPenyaluranList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasDataStopPenyaluran
    /// </summary>
    public class SubAktivitasDataStopPenyaluranList : SubAktivitasDataStopPenyaluranListBase
    {
        // Constructor
        public SubAktivitasDataStopPenyaluranList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasDataStopPenyaluranList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasDataStopPenyaluranListBase : SubAktivitasDataStopPenyaluran
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasDataStopPenyaluranList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasDataStopPenyaluranlist";
        ...
        // Constructor
        public SubAktivitasDataStopPenyaluranListBase()
        {
            TableName = "SubAktivitasDataStopPenyaluran";

            // CSS class name as context
            TableVar = "SubAktivitasDataStopPenyaluran";
            ...
            // Table object (subAktivitasDataStopPenyaluran)
            if (subAktivitasDataStopPenyaluran == null || subAktivitasDataStopPenyaluran is SubAktivitasDataStopPenyaluran)
                subAktivitasDataStopPenyaluran = this;

            // Initialize URLs
            AddUrl = "SubAktivitasDataStopPenyaluranAdd";
            ...
            MultiDeleteUrl = "SubAktivitasDataStopPenyaluranDelete";
            MultiUpdateUrl = "SubAktivitasDataStopPenyaluranUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasDataStopPenyaluranList";
        ...
        // Constructor
        public SubAktivitasDataStopPenyaluranListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasDataStopPenyaluranView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasDataStopPenyalurangrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasDataStopPenyaluranList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasDataStopPenyaluran"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasDataStopPenyaluranlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasDataStopPenyaluranlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasDataStopPenyaluransrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasDataStopPenyaluransrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasDataStopPenyaluranlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasDataStopPenyaluran");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasDataStopPenyaluranList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasDataStopPenyaluranList.RowCount) + "_SubAktivitasDataStopPenyaluran");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasDataStopPenyaluranList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasDataStopPenyaluranList.RowType == RowType.Add || IsEdit && subAktivitasDataStopPenyaluranList.RowType == RowType.Edit) // Inline-Add/Edit row
