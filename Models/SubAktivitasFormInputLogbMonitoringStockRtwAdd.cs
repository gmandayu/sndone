...
public partial class SnDOne {
    ...
    public class SubAktivitasFormInputLogbMonitoringStockRtwAddBase : SubAktivitasFormInputLogbMonitoringStockRtw
    {
        ...
        // Constructor
        public SubAktivitasFormInputLogbMonitoringStockRtwAddBase()
        {
        ...
        // Constructor
        public SubAktivitasFormInputLogbMonitoringStockRtwAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("SubAktivitasFormInputLogbMonitoringStockRtwList"); // No matching record, return to List page // DN
                    }
                    break;
                    ...
                    if (res) { // Add successful
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasFormInputLogbMonitoringStockRtwList") {
                                ...
                                returnUrl = "SubAktivitasFormInputLogbMonitoringStockRtwList"; // Return list page content
                            }
                        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NoTangki' before field var 'x_NoTangki'
            val = CurrentForm.HasValue("NoTangki") ? CurrentForm.GetValue("NoTangki") : CurrentForm.GetValue("x_NoTangki");
            if (!NoTangki.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NoTangki") && !CurrentForm.HasValue("x_NoTangki")) // DN
                    NoTangki.Visible = false; // Disable update for API request
                else
                    NoTangki.SetFormValue(val, true, validate);
            }
            ...
            // Check field name 'LevelTangki' before field var 'x_LevelTangki'
            val = CurrentForm.HasValue("LevelTangki") ? CurrentForm.GetValue("LevelTangki") : CurrentForm.GetValue("x_LevelTangki");
            if (!LevelTangki.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("LevelTangki") && !CurrentForm.HasValue("x_LevelTangki")) // DN
                    LevelTangki.Visible = false; // Disable update for API request
                else
                    LevelTangki.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (NoTangki.Visible && NoTangki.Required) {
                    if (!NoTangki.IsDetailKey && Empty(NoTangki.FormValue)) {
                        NoTangki.AddErrorMessage(ConvertToString(NoTangki.RequiredErrorMessage).Replace("%s", NoTangki.Caption));
                    }
                }
                ...
                if (LevelTangki.Visible && LevelTangki.Required) {
                    if (!LevelTangki.IsDetailKey && Empty(LevelTangki.FormValue)) {
                        LevelTangki.AddErrorMessage(ConvertToString(LevelTangki.RequiredErrorMessage).Replace("%s", LevelTangki.Caption));
                    }
                }
