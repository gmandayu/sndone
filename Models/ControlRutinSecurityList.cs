...
public partial class SnDOne {
    /// <summary>
    /// controlRutinSecurityList
    /// </summary>
    public static ControlRutinSecurityList controlRutinSecurityList
    {
        get => HttpData.Get<ControlRutinSecurityList>("controlRutinSecurityList")!;
        set => HttpData["controlRutinSecurityList"] = value;
    }

    /// <summary>
    /// Page class for ControlRutinSecurity
    /// </summary>
    public class ControlRutinSecurityList : ControlRutinSecurityListBase
    {
        // Constructor
        public ControlRutinSecurityList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ControlRutinSecurityList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class ControlRutinSecurityListBase : ControlRutinSecurity
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "controlRutinSecurityList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fControlRutinSecuritylist";
        ...
        // Constructor
        public ControlRutinSecurityListBase()
        {
            TableName = "ControlRutinSecurity";

            // CSS class name as context
            TableVar = "ControlRutinSecurity";
            ...
            // Table object (controlRutinSecurity)
            if (controlRutinSecurity == null || controlRutinSecurity is ControlRutinSecurity)
                controlRutinSecurity = this;

            // Initialize URLs
            AddUrl = "ControlRutinSecurityAdd";
            ...
            MultiDeleteUrl = "ControlRutinSecurityDelete";
            MultiUpdateUrl = "ControlRutinSecurityUpdate";
        ...
        // Page name
        public string PageName => "ControlRutinSecurityList";
        ...
        {
            IdCRS.Visible = false;
            NomorCRS.Visible = false;
            LinkRedirect.SetVisibility();
            LookupPlant.Visible = false;
            Plant.SetVisibility();
            JumlahKetidaksesuaiaan.SetVisibility();
        ...
        // Constructor
        public ControlRutinSecurityListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "ControlRutinSecurityView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("IdCRS") ? dict["IdCRS"] : IdCRS.CurrentValue));
            return key;
        }
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fControlRutinSecuritygrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                controlRutinSecurityList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(IdCRS.AdvancedSearch.ToJson())); // Field IdCRS
            filters.Merge(JObject.Parse(NomorCRS.AdvancedSearch.ToJson())); // Field NomorCRS
            ...
            filters.Merge(JObject.Parse(JumlahKetidaksesuaiaan.AdvancedSearch.ToJson())); // Field JumlahKetidaksesuaiaan
            filters.Merge(JObject.Parse(Catatan.AdvancedSearch.ToJson())); // Field Catatan
            filters.Merge(JObject.Parse(DibuatOleh.AdvancedSearch.ToJson())); // Field DibuatOleh
            filters.Merge(JObject.Parse(TanggalDibuat.AdvancedSearch.ToJson())); // Field TanggalDibuat
            filters.Merge(JObject.Parse(DiperbaruiOleh.AdvancedSearch.ToJson())); // Field DiperbaruiOleh
        ...
        protected bool RestoreFilterList() {
            ...
            // Field IdCRS
            if (filter?.TryGetValue("x_IdCRS", out sv) ?? false) {
                IdCRS.AdvancedSearch.SearchValue = sv;
                IdCRS.AdvancedSearch.SearchOperator = filter["z_IdCRS"];
                IdCRS.AdvancedSearch.SearchCondition = filter["v_IdCRS"];
                IdCRS.AdvancedSearch.SearchValue2 = filter["y_IdCRS"];
                IdCRS.AdvancedSearch.SearchOperator2 = filter["w_IdCRS"];
                IdCRS.AdvancedSearch.Save();
            }

            // Field NomorCRS
            if (filter?.TryGetValue("x_NomorCRS", out sv) ?? false) {
                NomorCRS.AdvancedSearch.SearchValue = sv;
                NomorCRS.AdvancedSearch.SearchOperator = filter["z_NomorCRS"];
                NomorCRS.AdvancedSearch.SearchCondition = filter["v_NomorCRS"];
                NomorCRS.AdvancedSearch.SearchValue2 = filter["y_NomorCRS"];
                NomorCRS.AdvancedSearch.SearchOperator2 = filter["w_NomorCRS"];
                NomorCRS.AdvancedSearch.Save();
            }
            ...
            // Field JumlahKetidaksesuaiaan
            if (filter?.TryGetValue("x_JumlahKetidaksesuaiaan", out sv) ?? false) {
                JumlahKetidaksesuaiaan.AdvancedSearch.SearchValue = sv;
                JumlahKetidaksesuaiaan.AdvancedSearch.SearchOperator = filter["z_JumlahKetidaksesuaiaan"];
                JumlahKetidaksesuaiaan.AdvancedSearch.SearchCondition = filter["v_JumlahKetidaksesuaiaan"];
                JumlahKetidaksesuaiaan.AdvancedSearch.SearchValue2 = filter["y_JumlahKetidaksesuaiaan"];
                JumlahKetidaksesuaiaan.AdvancedSearch.SearchOperator2 = filter["w_JumlahKetidaksesuaiaan"];
                JumlahKetidaksesuaiaan.AdvancedSearch.Save();
            }

            // Field Catatan
            if (filter?.TryGetValue("x_Catatan", out sv) ?? false) {
                Catatan.AdvancedSearch.SearchValue = sv;
                Catatan.AdvancedSearch.SearchOperator = filter["z_Catatan"];
                Catatan.AdvancedSearch.SearchCondition = filter["v_Catatan"];
                Catatan.AdvancedSearch.SearchValue2 = filter["y_Catatan"];
                Catatan.AdvancedSearch.SearchOperator2 = filter["w_Catatan"];
                Catatan.AdvancedSearch.Save();
            }

            // Field DibuatOleh
            if (filter?.TryGetValue("x_DibuatOleh", out sv) ?? false) {
                DibuatOleh.AdvancedSearch.SearchValue = sv;
                DibuatOleh.AdvancedSearch.SearchOperator = filter["z_DibuatOleh"];
                DibuatOleh.AdvancedSearch.SearchCondition = filter["v_DibuatOleh"];
                DibuatOleh.AdvancedSearch.SearchValue2 = filter["y_DibuatOleh"];
                DibuatOleh.AdvancedSearch.SearchOperator2 = filter["w_DibuatOleh"];
                DibuatOleh.AdvancedSearch.Save();
            }
            ...
            if (filter?.TryGetValue("x_DiperbaruiOleh", out sv) ?? false) {
                DiperbaruiOleh.AdvancedSearch.SearchValue = sv;
                DiperbaruiOleh.AdvancedSearch.SearchOperator = filter["z_DiperbaruiOleh"];
                DiperbaruiOleh.AdvancedSearch.SearchCondition = filter["v_DiperbaruiOleh"];
                DiperbaruiOleh.AdvancedSearch.SearchValue2 = filter["y_DiperbaruiOleh"];
                DiperbaruiOleh.AdvancedSearch.SearchOperator2 = filter["w_DiperbaruiOleh"];
                DiperbaruiOleh.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(JumlahKetidaksesuaiaan, ctrl); // JumlahKetidaksesuaiaan
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""ControlRutinSecurity"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""ControlRutinSecurity"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""ControlRutinSecurity"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fControlRutinSecuritylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fControlRutinSecuritylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdCRS.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""ControlRutinSecurity"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fControlRutinSecuritylist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("JumlahKetidaksesuaiaan")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fControlRutinSecuritysrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fControlRutinSecuritysrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fControlRutinSecuritylist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_ControlRutinSecurity");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(controlRutinSecurityList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(controlRutinSecurityList.RowCount) + "_ControlRutinSecurity");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(controlRutinSecurityList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && controlRutinSecurityList.RowType == RowType.Add || IsEdit && controlRutinSecurityList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            RowSelected(row);
            IdCRS.SetDbValue(row["IdCRS"]);
            NomorCRS.SetDbValue(row["NomorCRS"]);
        ...
        protected Dictionary<string, object> NewRow() {
            var row = new Dictionary<string, object>();
            row.Add("IdCRS", IdCRS.DefaultValue ?? DbNullValue); // DN
            row.Add("NomorCRS", NomorCRS.DefaultValue ?? DbNullValue); // DN
            ...
            row.Add("JumlahKetidaksesuaiaan", JumlahKetidaksesuaiaan.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                // IdCRS
                IdCRS.ViewValue = IdCRS.CurrentValue;
                IdCRS.ViewCustomAttributes = "";

                // NomorCRS
                NomorCRS.ViewValue = ConvertToString(NomorCRS.CurrentValue); // DN
                NomorCRS.ViewCustomAttributes = "";
                ...
                // JumlahKetidaksesuaiaan
                JumlahKetidaksesuaiaan.ViewValue = JumlahKetidaksesuaiaan.CurrentValue;
                JumlahKetidaksesuaiaan.ViewValue = FormatNumber(JumlahKetidaksesuaiaan.ViewValue, JumlahKetidaksesuaiaan.FormatPattern);
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fControlRutinSecuritysrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
