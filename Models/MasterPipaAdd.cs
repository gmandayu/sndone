...
public partial class SnDOne {
    ...
    /// masterPipaAdd
    /// </summary>
    public static MasterPipaAdd masterPipaAdd
    {
        get => HttpData.Get<MasterPipaAdd>("masterPipaAdd")!;
        set => HttpData["masterPipaAdd"] = value;
    }

    /// <summary>
    /// Page class for MasterPipa
    /// </summary>
    public class MasterPipaAdd : MasterPipaAddBase
    {
        // Constructor
        public MasterPipaAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterPipaAdd() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class MasterPipaAddBase : MasterPipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterPipaAdd";
        ...
        // Constructor
        public MasterPipaAddBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "MasterPipaAdd";
        ...
        // Constructor
        public MasterPipaAddBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private IActionResult? HandleCopyAction(Dictionary<string, object>? rsold)
        {
            if (rsold == null) {
                ...
                return Terminate("MasterPipaList");
            }
            return null;
        }
        ...
        private string DetermineReturnUrl()
        {
            ...
            else if (GetPageName(returnUrl) == "MasterPipaView")
                returnUrl = ViewUrl;
            return returnUrl;
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'KeteranganPipa'
            KeteranganPipa.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(KeteranganPipa, "KeteranganPipa", "x_KeteranganPipa");

            // Standard handling for 'idPlantAsal'
            idPlantAsal.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(idPlantAsal, "idPlantAsal", "x_idPlantAsal");
            ...
            SetNormalField(Flowrate, "Flowrate", "x_Flowrate", true, validate, false);

            // Standard handling for 'BiayaProject'
            BiayaProject.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(BiayaProject, "BiayaProject", "x_BiayaProject", true, validate, false);

            // Standard handling for 'PlantAsal'
            PlantAsal.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(PlantAsal, "PlantAsal", "x_PlantAsal");

            // Standard handling for 'NamaPlantAsal'
            NamaPlantAsal.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NamaPlantAsal, "NamaPlantAsal", "x_NamaPlantAsal");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomKeteranganPipa() {
            if (KeteranganPipa.Visible && KeteranganPipa.Required) {
                if (!KeteranganPipa.IsDetailKey && Empty(KeteranganPipa.FormValue)) {
                    KeteranganPipa.AddErrorMessage(ConvertToString(NamaPlantAsal.RequiredErrorMessage).Replace("%s", NamaPlantAsal.Caption));
                }
            }
        }

        private void ValidateCustomidPlantAsal() {
            if (idPlantAsal.Visible && idPlantAsal.Required) {
                if (!idPlantAsal.IsDetailKey && Empty(idPlantAsal.FormValue)) {
                    idPlantAsal.AddErrorMessage(ConvertToString(NamaPlantAsal.RequiredErrorMessage).Replace("%s", NamaPlantAsal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomFlowrate() {
            if (Flowrate.Visible && Flowrate.Required) {
                if (!Flowrate.IsDetailKey && Empty(Flowrate.FormValue)) {
                    Flowrate.AddErrorMessage(ConvertToString(NamaPlantAsal.RequiredErrorMessage).Replace("%s", NamaPlantAsal.Caption));
                }
            }
        }

        private void ValidateCustomBiayaProject() {
            if (BiayaProject.Visible && BiayaProject.Required) {
                if (!BiayaProject.IsDetailKey && Empty(BiayaProject.FormValue)) {
                    BiayaProject.AddErrorMessage(ConvertToString(NamaPlantAsal.RequiredErrorMessage).Replace("%s", NamaPlantAsal.Caption));
                }
            }
        }

        private void ValidateCustomPlantAsal() {
            if (PlantAsal.Visible && PlantAsal.Required) {
                if (!PlantAsal.IsDetailKey && Empty(PlantAsal.FormValue)) {
                    PlantAsal.AddErrorMessage(ConvertToString(NamaPlantAsal.RequiredErrorMessage).Replace("%s", NamaPlantAsal.Caption));
                }
            }
        }

        private void ValidateCustomNamaPlantAsal() {
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // KeteranganPipa
                KeteranganPipa.SetDbValue(rsnew, KeteranganPipa.CurrentValue);

                // idPlantAsal
                idPlantAsal.SetDbValue(rsnew, idPlantAsal.CurrentValue);

                // Flowrate
                Flowrate.SetDbValue(rsnew, Flowrate.CurrentValue);

                // BiayaProject
                BiayaProject.SetDbValue(rsnew, BiayaProject.CurrentValue);

                // PlantAsal
                PlantAsal.SetDbValue(rsnew, PlantAsal.CurrentValue);

                // NamaPlantAsal
                NamaPlantAsal.SetDbValue(rsnew, NamaPlantAsal.CurrentValue);
                return rsnew;
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("KeteranganPipa", out value)) // KeteranganPipa
                KeteranganPipa.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("idPlantAsal", out value)) // idPlantAsal
                idPlantAsal.SetFormValue(ConvertToString(value));
            ...
            if (row.TryGetValue("BiayaProject", out value)) // BiayaProject
                BiayaProject.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("PlantAsal", out value)) // PlantAsal
                PlantAsal.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("NamaPlantAsal", out value)) // NamaPlantAsal
                NamaPlantAsal.SetFormValue(ConvertToString(value));
        }
