...
public partial class SnDOne {
    ...
    /// closingStockView
    /// </summary>
    public static ClosingStockView closingStockView
    {
        get => HttpData.Get<ClosingStockView>("closingStockView")!;
        set => HttpData["closingStockView"] = value;
    }

    /// <summary>
    /// Page class for ClosingStock
    /// </summary>
    public class ClosingStockView : ClosingStockViewBase
    {
        // Constructor
        public ClosingStockView(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ClosingStockView() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class ClosingStockViewBase : ClosingStock
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "closingStockView";
        ...
        public ClosingStockViewBase()
        {
            ...
            if (IsApi()) {
                ...
                if (keys.Length > 0)
                    RecordKeys["IdClosingStock"] = keys[0];
            } else {
                RecordKeys["IdClosingStock"] = RouteValues.TryGetValue("IdClosingStock", out obj) && obj != null ? UrlDecode(obj) : Get("IdClosingStock"); // DN
            }
        ...
        // Page name
        public string PageName => "ClosingStockView";
        ...
        // Constructor
        public ClosingStockViewBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (RouteValues.TryGetValue("IdClosingStock", out v) && !Empty(v)) { // DN
                IdClosingStock.QueryValue = UrlDecode(v); // DN
                RecordKeys["IdClosingStock"] = IdClosingStock.QueryValue;
            } else if (Get("IdClosingStock", out sv)) {
                IdClosingStock.QueryValue = sv.ToString();
                RecordKeys["IdClosingStock"] = IdClosingStock.QueryValue;
            } else if (IsApi() && !Empty(keyValues)) {
                IdClosingStock.QueryValue = ConvertToString(keyValues[0]);
                RecordKeys["IdClosingStock"] = IdClosingStock.QueryValue;
            } else if (!loadCurrentRecord) {
                returnUrl = "ClosingStockList"; // Return to list
            }
                ...
                case "show": // Get a record to display
                        ...
                        if (!res) { // Load record based on key
                            ...
                            if (IsApi()) {
                                ...
                                return Terminate("ClosingStockList"); // Return to list page
                            }
                        }
                    break;
            }
        ...
        public async Task ExportData(dynamic? doc, string[] keys)
        {
            ...
            StartRecord = 1;
            if (keys.Length >= 1) {
                IdClosingStock.OldValue = keys[0];
