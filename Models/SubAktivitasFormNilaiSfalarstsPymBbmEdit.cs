...
public partial class SnDOne {
    ...
    public class SubAktivitasFormNilaiSfalarstsPymBbmEditBase : SubAktivitasFormNilaiSfalarstsPymBbm
    {
        ...
        // Constructor
        public SubAktivitasFormNilaiSfalarstsPymBbmEditBase()
        {
        ...
        // Constructor
        public SubAktivitasFormNilaiSfalarstsPymBbmEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasFormNilaiSfalarstsPymBbmList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasFormNilaiSfalarstsPymBbmList") {
                                ...
                                returnUrl = "SubAktivitasFormNilaiSfalarstsPymBbmList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'AR_klobs' before field var 'x_AR_klobs'
            val = CurrentForm.HasValue("AR_klobs") ? CurrentForm.GetValue("AR_klobs") : CurrentForm.GetValue("x_AR_klobs");
            if (!AR_klobs.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AR_klobs") && !CurrentForm.HasValue("x_AR_klobs")) // DN
                    AR_klobs.Visible = false; // Disable update for API request
                else
                    AR_klobs.SetFormValue(val);
            }

            // Check field name 'AR_kl15' before field var 'x_AR_kl15'
            val = CurrentForm.HasValue("AR_kl15") ? CurrentForm.GetValue("AR_kl15") : CurrentForm.GetValue("x_AR_kl15");
            if (!AR_kl15.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AR_kl15") && !CurrentForm.HasValue("x_AR_kl15")) // DN
                    AR_kl15.Visible = false; // Disable update for API request
                else
                    AR_kl15.SetFormValue(val);
            }

            // Check field name 'AR_barrels' before field var 'x_AR_barrels'
            val = CurrentForm.HasValue("AR_barrels") ? CurrentForm.GetValue("AR_barrels") : CurrentForm.GetValue("x_AR_barrels");
            if (!AR_barrels.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AR_barrels") && !CurrentForm.HasValue("x_AR_barrels")) // DN
                    AR_barrels.Visible = false; // Disable update for API request
                else
                    AR_barrels.SetFormValue(val);
            }

            // Check field name 'AR_lt' before field var 'x_AR_lt'
            val = CurrentForm.HasValue("AR_lt") ? CurrentForm.GetValue("AR_lt") : CurrentForm.GetValue("x_AR_lt");
            if (!AR_lt.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AR_lt") && !CurrentForm.HasValue("x_AR_lt")) // DN
                    AR_lt.Visible = false; // Disable update for API request
                else
                    AR_lt.SetFormValue(val);
            }

            // Check field name 'AR_mt' before field var 'x_AR_mt'
            val = CurrentForm.HasValue("AR_mt") ? CurrentForm.GetValue("AR_mt") : CurrentForm.GetValue("x_AR_mt");
            if (!AR_mt.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AR_mt") && !CurrentForm.HasValue("x_AR_mt")) // DN
                    AR_mt.Visible = false; // Disable update for API request
                else
                    AR_mt.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (AR_klobs.Visible && AR_klobs.Required) {
                    if (!AR_klobs.IsDetailKey && Empty(AR_klobs.FormValue)) {
                        AR_klobs.AddErrorMessage(ConvertToString(AR_klobs.RequiredErrorMessage).Replace("%s", AR_klobs.Caption));
                    }
                }
                if (AR_kl15.Visible && AR_kl15.Required) {
                    if (!AR_kl15.IsDetailKey && Empty(AR_kl15.FormValue)) {
                        AR_kl15.AddErrorMessage(ConvertToString(AR_kl15.RequiredErrorMessage).Replace("%s", AR_kl15.Caption));
                    }
                }
                if (AR_barrels.Visible && AR_barrels.Required) {
                    if (!AR_barrels.IsDetailKey && Empty(AR_barrels.FormValue)) {
                        AR_barrels.AddErrorMessage(ConvertToString(AR_barrels.RequiredErrorMessage).Replace("%s", AR_barrels.Caption));
                    }
                }
                if (AR_lt.Visible && AR_lt.Required) {
                    if (!AR_lt.IsDetailKey && Empty(AR_lt.FormValue)) {
                        AR_lt.AddErrorMessage(ConvertToString(AR_lt.RequiredErrorMessage).Replace("%s", AR_lt.Caption));
                    }
                }
