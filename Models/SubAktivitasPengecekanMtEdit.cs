...
public partial class SnDOne {
    ...
    public class SubAktivitasPengecekanMtEditBase : SubAktivitasPengecekanMt
    {
        ...
        // Constructor
        public SubAktivitasPengecekanMtEditBase()
        {
        ...
        // Constructor
        public SubAktivitasPengecekanMtEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasPengecekanMtList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasPengecekanMtList") {
                                ...
                                returnUrl = "SubAktivitasPengecekanMtList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'JamPemeriksaan' before field var 'x_JamPemeriksaan'
            val = CurrentForm.HasValue("JamPemeriksaan") ? CurrentForm.GetValue("JamPemeriksaan") : CurrentForm.GetValue("x_JamPemeriksaan");
            if (!JamPemeriksaan.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("JamPemeriksaan") && !CurrentForm.HasValue("x_JamPemeriksaan")) // DN
                    JamPemeriksaan.Visible = false; // Disable update for API request
                else
                    JamPemeriksaan.SetFormValue(val, true, validate);
                JamPemeriksaan.CurrentValue = UnformatDateTime(JamPemeriksaan.CurrentValue, JamPemeriksaan.FormatPattern);
            }

            // Check field name 'ExTangki' before field var 'x_ExTangki'
            val = CurrentForm.HasValue("ExTangki") ? CurrentForm.GetValue("ExTangki") : CurrentForm.GetValue("x_ExTangki");
            if (!ExTangki.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("ExTangki") && !CurrentForm.HasValue("x_ExTangki")) // DN
                    ExTangki.Visible = false; // Disable update for API request
                else
                    ExTangki.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (JamPemeriksaan.Visible && JamPemeriksaan.Required) {
                    if (!JamPemeriksaan.IsDetailKey && Empty(JamPemeriksaan.FormValue)) {
                        JamPemeriksaan.AddErrorMessage(ConvertToString(JamPemeriksaan.RequiredErrorMessage).Replace("%s", JamPemeriksaan.Caption));
                    }
                }
                ...
                if (ExTangki.Visible && ExTangki.Required) {
                    if (!ExTangki.IsDetailKey && Empty(ExTangki.FormValue)) {
                        ExTangki.AddErrorMessage(ConvertToString(ExTangki.RequiredErrorMessage).Replace("%s", ExTangki.Caption));
                    }
                }
