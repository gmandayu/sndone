...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiEstimasisFlowratePipaEdit
    /// </summary>
    public static SubAktivitasNilaiEstimasisFlowratePipaEdit subAktivitasNilaiEstimasisFlowratePipaEdit
    {
        get => HttpData.Get<SubAktivitasNilaiEstimasisFlowratePipaEdit>("subAktivitasNilaiEstimasisFlowratePipaEdit")!;
        set => HttpData["subAktivitasNilaiEstimasisFlowratePipaEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiEstimasisFlowratePipa
    /// </summary>
    public class SubAktivitasNilaiEstimasisFlowratePipaEdit : SubAktivitasNilaiEstimasisFlowratePipaEditBase
    {
        // Constructor
        public SubAktivitasNilaiEstimasisFlowratePipaEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiEstimasisFlowratePipaEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiEstimasisFlowratePipaEditBase : SubAktivitasNilaiEstimasisFlowratePipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiEstimasisFlowratePipaEdit";
        ...
        // Constructor
        public SubAktivitasNilaiEstimasisFlowratePipaEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiEstimasisFlowratePipaEdit";
        ...
        // Constructor
        public SubAktivitasNilaiEstimasisFlowratePipaEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasNilaiEstimasisFlowratePipaList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasNilaiEstimasisFlowratePipaList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasNilaiEstimasisFlowratePipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasNilaiEstimasisFlowratePipaEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomNominasi() {
            if (Nominasi.Visible && Nominasi.Required) {
                if (!Nominasi.IsDetailKey && Empty(Nominasi.FormValue)) {
                    Nominasi.AddErrorMessage(ConvertToString(ETC.RequiredErrorMessage).Replace("%s", ETC.Caption));
                }
            }
        }
        ...
        private void ValidateCustomPressure() {
            if (Pressure.Visible && Pressure.Required) {
                if (!Pressure.IsDetailKey && Empty(Pressure.FormValue)) {
                    Pressure.AddErrorMessage(ConvertToString(ETC.RequiredErrorMessage).Replace("%s", ETC.Caption));
                }
            }
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasNilaiEstimasisFlowratePipaList")), "", TableVar, true);
            string pageId = "edit";
