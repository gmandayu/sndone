...
public partial class SnDOne {
    ...
    /// masterTemplateList
    /// </summary>
    public static MasterTemplateList masterTemplateList
    {
        get => HttpData.Get<MasterTemplateList>("masterTemplateList")!;
        set => HttpData["masterTemplateList"] = value;
    }

    /// <summary>
    /// Page class for MasterTemplate
    /// </summary>
    public class MasterTemplateList : MasterTemplateListBase
    {
        // Constructor
        public MasterTemplateList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterTemplateList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterTemplateListBase : MasterTemplate
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterTemplateList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMasterTemplatelist";
        ...
        // Constructor
        public MasterTemplateListBase()
        {
            TableName = "MasterTemplate";

            // CSS class name as context
            TableVar = "MasterTemplate";
            ...
            // Initialize URLs
            AddUrl = "MasterTemplateAdd?" + Config.TableShowDetail + "=";
            ...
            MultiDeleteUrl = "MasterTemplateDelete";
            MultiUpdateUrl = "MasterTemplateUpdate";
        ...
        // Page name
        public string PageName => "MasterTemplateList";
        ...
        // Constructor
        public MasterTemplateListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, NamaTemplate, def, true); // NamaTemplate
            BuildSearchSql(ref where, IdModa, def, true); // IdModa
            BuildSearchSql(ref where, JenisProses, def, true); // JenisProses
            ...
            BuildSearchSql(ref where, TipePenimbunan, def, true); // TipePenimbunan
        ...
        public void ShowFilterList()
        {
            ...
            // Field NamaTemplate
            filter = QueryBuilderWhere("NamaTemplate");
            if (Empty(filter))
                BuildSearchSql(ref filter, NamaTemplate, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NamaTemplate.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field JenisProses
            filter = QueryBuilderWhere("JenisProses");
            if (Empty(filter))
                BuildSearchSql(ref filter, JenisProses, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + JenisProses.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field TipePenimbunan
            filter = QueryBuilderWhere("TipePenimbunan");
            if (Empty(filter))
                BuildSearchSql(ref filter, TipePenimbunan, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TipePenimbunan.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NamaTemplate);
            searchFlds.Add(IdModa);
            searchFlds.Add(JenisProses);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (NamaTemplate.AdvancedSearch.IssetSession)
                return true;
            if (IdModa.AdvancedSearch.IssetSession)
                return true;
            if (JenisProses.AdvancedSearch.IssetSession)
                return true;
            ...
            if (TanggalDiperbarui.AdvancedSearch.IssetSession)
                return true;
            if (TipePenimbunan.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            NamaTemplate.AdvancedSearch.UnsetSession();
            IdModa.AdvancedSearch.UnsetSession();
            JenisProses.AdvancedSearch.UnsetSession();
            ...
            TipePenimbunan.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            NamaTemplate.AdvancedSearch.Load();
            IdModa.AdvancedSearch.Load();
            JenisProses.AdvancedSearch.Load();
            ...
            TipePenimbunan.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(NamaTemplate, ctrl); // NamaTemplate
                UpdateSort(IdModa, ctrl); // IdModa
                UpdateSort(JenisProses, ctrl); // JenisProses
                ...
                UpdateSort(TipePenimbunan, ctrl); // TipePenimbunan
        ...
        protected async Task SetupListOptions() {
            ...
            // "detail_TemplateProses"
            item = ListOptions.Add("detail_TemplateProses");
            item.CssClass = "text-nowrap";
            item.Visible = Security.AllowList(CurrentProjectID + "TemplateProses");
            item.OnLeft = true;
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MasterTemplate"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MasterTemplate"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MasterTemplate"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMasterTemplatelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMasterTemplatelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            // "detail_TemplateProses"
            listOption = ListOptions["detail_TemplateProses"];
            isVisible = Security.AllowList(CurrentProjectID + "TemplateProses");
            if (isVisible) {
                string caption, title, url;
                var body = Language.Phrase("DetailLink") + Language.TablePhrase("TemplateProses", "TblCaption");
                body = "<a class=\"btn btn-default ew-row-link ew-detail" + (ListOptions.UseDropDownButton ? " dropdown-toggle" : "") + "\" data-action=\"list\" href=\"" + HtmlEncode(AppPath("TemplateProsesList?" + Config.TableShowMaster + "=MasterTemplate&" + ForeignKeyUrl("fk_IdTemplate", IdTemplate.CurrentValue) + "")) + "\">" + body + "</a>";
                string links = "";
                detailPage = Resolve("TemplateProsesGrid")!;
                if (detailPage?.DetailView && Security.CanView && Security.AllowView(CurrentProjectID + "MasterTemplate") ?? false) {
                    caption = Language.Phrase("MasterDetailViewLink", null);
                    title = Language.Phrase("MasterDetailViewLink", true);
                    url = GetViewUrl(Config.TableShowDetail + "=TemplateProses");
                    ...
                    if (!Empty(detailViewTblVar))
                        detailViewTblVar += ",";
                    detailViewTblVar += "TemplateProses";
                }
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "MasterTemplate") ?? false) {
                    caption = Language.Phrase("MasterDetailEditLink", null);
                    title = Language.Phrase("MasterDetailEditLink", true);
                    url = GetEditUrl(Config.TableShowDetail + "=TemplateProses");
                    ...
                    if (!Empty(detailEditTblVar))
                        detailEditTblVar += ",";
                    detailEditTblVar += "TemplateProses";
                }
                if (detailPage?.DetailAdd && Security.CanAdd && Security.AllowAdd(CurrentProjectID + "MasterTemplate") ?? false) {
                    caption = Language.Phrase("MasterDetailCopyLink", null);
                    title = Language.Phrase("MasterDetailCopyLink", true);
                    url = GetCopyUrl(Config.TableShowDetail + "=TemplateProses");
                    ...
                    if (!Empty(detailCopyTblVar))
                        detailCopyTblVar += ",";
                    detailCopyTblVar += "TemplateProses";
                }
                if (!Empty(links)) {
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdTemplate.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MasterTemplate"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item = option.Add("detailadd_TemplateProses");
            templateProses = Resolve("TemplateProses")!;
            if (templateProses != null) {
                caption = Language.Phrase("Add") + "&nbsp;" + Caption + "/" + templateProses.Caption;
                title = Language.Phrase("Add", true) + "&nbsp;" + Caption + "/" + templateProses.Caption;
                item.Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" + title + "\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(GetAddUrl(Config.TableShowDetail + "=TemplateProses"))) + "\">" + caption + "</a>";
                item.Visible = (templateProses.DetailAdd && Security.AllowAdd(CurrentProjectID + "MasterTemplate") && Security.CanAdd);
                if (item.Visible) {
                    if (detailTableLink != "")
                        detailTableLink += ",";
                    detailTableLink += "TemplateProses";
                }
            }
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMasterTemplatelist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("NamaTemplate")); // DN
                CreateColumnOption(option.Add("IdModa")); // DN
                CreateColumnOption(option.Add("JenisProses")); // DN
                ...
                CreateColumnOption(option.Add("TipePenimbunan")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMasterTemplatesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMasterTemplatesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMasterTemplatelist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MasterTemplate");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(masterTemplateList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(masterTemplateList.RowCount) + "_MasterTemplate");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(masterTemplateList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && masterTemplateList.RowType == RowType.Add || IsEdit && masterTemplateList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // NamaTemplate
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaTemplate[]"))
                    NamaTemplate.AdvancedSearch.SearchValue = Get("x_NamaTemplate[]", Config.FilterOptionSeparator);
                else
                    NamaTemplate.AdvancedSearch.SearchValue = Get("NamaTemplate"); // Default Value // DN
            if (!Empty(NamaTemplate.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_NamaTemplate"))
                NamaTemplate.AdvancedSearch.SearchOperator = Get("z_NamaTemplate", Config.FilterOptionSeparator);
            ...
            // JenisProses
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_JenisProses[]"))
                    JenisProses.AdvancedSearch.SearchValue = Get("x_JenisProses[]", Config.FilterOptionSeparator);
                else
                    JenisProses.AdvancedSearch.SearchValue = Get("JenisProses"); // Default Value // DN
            if (!Empty(JenisProses.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_JenisProses"))
                JenisProses.AdvancedSearch.SearchOperator = Get("z_JenisProses", Config.FilterOptionSeparator);
            ...
            // TipePenimbunan
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TipePenimbunan[]"))
                    TipePenimbunan.AdvancedSearch.SearchValue = Get("x_TipePenimbunan[]", Config.FilterOptionSeparator);
                else
                    TipePenimbunan.AdvancedSearch.SearchValue = Get("TipePenimbunan"); // Default Value // DN
            if (!Empty(TipePenimbunan.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_TipePenimbunan"))
                TipePenimbunan.AdvancedSearch.SearchOperator = Get("z_TipePenimbunan", Config.FilterOptionSeparator);
            ...
            if (RowType == RowType.View) {
                ...
                if (!Empty(curVal)) {
                    if (IdModa.Lookup != null && IsDictionary(IdModa.Lookup?.Options) && IdModa.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        ...
                        string filterWrk = SearchFilter(IdModa.Lookup?.GetTable()?.Fields["IdModa"].SearchExpression, "=", IdModa.CurrentValue, IdModa.Lookup?.GetTable()?.Fields["IdModa"].SearchDataType, "");
                        string? sqlWrk = IdModa.Lookup?.GetSql(false, filterWrk, null, this, true, true);
                ...
                // NamaTemplate
                if (NamaTemplate.UseFilter && !Empty(NamaTemplate.AdvancedSearch.SearchValue)) {
                    NamaTemplate.EditValue = ConvertToString(NamaTemplate.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // JenisProses
                if (JenisProses.UseFilter && !Empty(JenisProses.AdvancedSearch.SearchValue)) {
                    JenisProses.EditValue = ConvertToString(JenisProses.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // TipePenimbunan
                if (TipePenimbunan.UseFilter && !Empty(TipePenimbunan.AdvancedSearch.SearchValue)) {
                    TipePenimbunan.EditValue = ConvertToString(TipePenimbunan.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMasterTemplatesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
