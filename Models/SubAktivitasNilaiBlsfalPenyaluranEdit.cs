...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiBlsfalPenyaluranEdit
    /// </summary>
    public static SubAktivitasNilaiBlsfalPenyaluranEdit subAktivitasNilaiBlsfalPenyaluranEdit
    {
        get => HttpData.Get<SubAktivitasNilaiBlsfalPenyaluranEdit>("subAktivitasNilaiBlsfalPenyaluranEdit")!;
        set => HttpData["subAktivitasNilaiBlsfalPenyaluranEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiBLSFALPenyaluran
    /// </summary>
    public class SubAktivitasNilaiBlsfalPenyaluranEdit : SubAktivitasNilaiBlsfalPenyaluranEditBase
    {
        // Constructor
        public SubAktivitasNilaiBlsfalPenyaluranEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiBlsfalPenyaluranEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiBlsfalPenyaluranEditBase : SubAktivitasNilaiBlsfalPenyaluran
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiBlsfalPenyaluranEdit";
        ...
        // Constructor
        public SubAktivitasNilaiBlsfalPenyaluranEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiBlsfalPenyaluranEdit";
        ...
        // Constructor
        public SubAktivitasNilaiBlsfalPenyaluranEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasNilaiBlsfalPenyaluranList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasNilaiBlsfalPenyaluranList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasNilaiBlsfalPenyaluranList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasNilaiBlsfalPenyaluranEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomBL_KLObs() {
            if (BL_KLObs.Visible && BL_KLObs.Required) {
                if (!BL_KLObs.IsDetailKey && Empty(BL_KLObs.FormValue)) {
                    BL_KLObs.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_KL15() {
            if (BL_KL15.Visible && BL_KL15.Required) {
                if (!BL_KL15.IsDetailKey && Empty(BL_KL15.FormValue)) {
                    BL_KL15.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_Barrels() {
            if (BL_Barrels.Visible && BL_Barrels.Required) {
                if (!BL_Barrels.IsDetailKey && Empty(BL_Barrels.FormValue)) {
                    BL_Barrels.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_LT() {
            if (BL_LT.Visible && BL_LT.Required) {
                if (!BL_LT.IsDetailKey && Empty(BL_LT.FormValue)) {
                    BL_LT.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_MT() {
            if (BL_MT.Visible && BL_MT.Required) {
                if (!BL_MT.IsDetailKey && Empty(BL_MT.FormValue)) {
                    BL_MT.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_KLObs() {
            if (SFAL_KLObs.Visible && SFAL_KLObs.Required) {
                if (!SFAL_KLObs.IsDetailKey && Empty(SFAL_KLObs.FormValue)) {
                    SFAL_KLObs.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_KL15() {
            if (SFAL_KL15.Visible && SFAL_KL15.Required) {
                if (!SFAL_KL15.IsDetailKey && Empty(SFAL_KL15.FormValue)) {
                    SFAL_KL15.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_Barrels() {
            if (SFAL_Barrels.Visible && SFAL_Barrels.Required) {
                if (!SFAL_Barrels.IsDetailKey && Empty(SFAL_Barrels.FormValue)) {
                    SFAL_Barrels.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_LT() {
            if (SFAL_LT.Visible && SFAL_LT.Required) {
                if (!SFAL_LT.IsDetailKey && Empty(SFAL_LT.FormValue)) {
                    SFAL_LT.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_MT() {
            if (SFAL_MT.Visible && SFAL_MT.Required) {
                if (!SFAL_MT.IsDetailKey && Empty(SFAL_MT.FormValue)) {
                    SFAL_MT.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasNilaiBlsfalPenyaluranList")), "", TableVar, true);
            string pageId = "edit";
