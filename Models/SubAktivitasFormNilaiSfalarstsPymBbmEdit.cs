...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasFormNilaiSfalarstsPymBbmEdit
    /// </summary>
    public static SubAktivitasFormNilaiSfalarstsPymBbmEdit subAktivitasFormNilaiSfalarstsPymBbmEdit
    {
        get => HttpData.Get<SubAktivitasFormNilaiSfalarstsPymBbmEdit>("subAktivitasFormNilaiSfalarstsPymBbmEdit")!;
        set => HttpData["subAktivitasFormNilaiSfalarstsPymBbmEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasFormNilaiSFALARSTSPymBBM
    /// </summary>
    public class SubAktivitasFormNilaiSfalarstsPymBbmEdit : SubAktivitasFormNilaiSfalarstsPymBbmEditBase
    {
        // Constructor
        public SubAktivitasFormNilaiSfalarstsPymBbmEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasFormNilaiSfalarstsPymBbmEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasFormNilaiSfalarstsPymBbmEditBase : SubAktivitasFormNilaiSfalarstsPymBbm
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasFormNilaiSfalarstsPymBbmEdit";
        ...
        // Constructor
        public SubAktivitasFormNilaiSfalarstsPymBbmEditBase(Controller? controller)
        {
            TableName = "SubAktivitasFormNilaiSFALARSTSPymBBM";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasFormNilaiSfalarstsPymBbm)
            if (subAktivitasFormNilaiSfalarstsPymBbm == null || subAktivitasFormNilaiSfalarstsPymBbm is SubAktivitasFormNilaiSfalarstsPymBbm)
                subAktivitasFormNilaiSfalarstsPymBbm = this;
        ...
        // Page name
        public string PageName => "SubAktivitasFormNilaiSfalarstsPymBbmEdit";
        ...
        public void SetVisibility()
        {
            ...
            AR_klobs.SetVisibility();
            AR_kl15.SetVisibility();
            AR_barrels.SetVisibility();
            AR_lt.SetVisibility();
            AR_mt.SetVisibility();
        ...
        // Constructor
        public SubAktivitasFormNilaiSfalarstsPymBbmEditBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                ...
                // List / View / Master View page
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasFormNilaiSfalarstsPymBbmView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasFormNilaiSfalarstsPymBbmList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasFormNilaiSfalarstsPymBbmList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasFormNilaiSfalarstsPymBbmList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasFormNilaiSfalarstsPymBbmEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'AR_klobs'
            AR_klobs.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AR_klobs, "AR_klobs", "x_AR_klobs");

            // Standard handling for 'AR_kl15'
            AR_kl15.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AR_kl15, "AR_kl15", "x_AR_kl15");

            // Standard handling for 'AR_barrels'
            AR_barrels.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AR_barrels, "AR_barrels", "x_AR_barrels");

            // Standard handling for 'AR_lt'
            AR_lt.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AR_lt, "AR_lt", "x_AR_lt");

            // Standard handling for 'AR_mt'
            AR_mt.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AR_mt, "AR_mt", "x_AR_mt");
            if (!id.IsDetailKey) {
        ...
        public void RestoreFormValues()
        {
            ...
            AR_klobs.CurrentValue = AR_klobs.FormValue;
            AR_kl15.CurrentValue = AR_kl15.FormValue;
            AR_barrels.CurrentValue = AR_barrels.FormValue;
            AR_lt.CurrentValue = AR_lt.FormValue;
            AR_mt.CurrentValue = AR_mt.FormValue;
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("AR_klobs", AR_klobs.DefaultValue ?? DbNullValue); // DN
            row.Add("AR_kl15", AR_kl15.DefaultValue ?? DbNullValue); // DN
            row.Add("AR_barrels", AR_barrels.DefaultValue ?? DbNullValue); // DN
            row.Add("AR_lt", AR_lt.DefaultValue ?? DbNullValue); // DN
            row.Add("AR_mt", AR_mt.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // AR_klobs
            AR_klobs.RowCssClass = "row";

            // AR_kl15
            AR_kl15.RowCssClass = "row";

            // AR_barrels
            AR_barrels.RowCssClass = "row";

            // AR_lt
            AR_lt.RowCssClass = "row";

            // AR_mt
            AR_mt.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                AR_klobs.EditValue = HtmlEncode(AR_klobs.CurrentValue);
                ...
                AR_kl15.EditValue = HtmlEncode(AR_kl15.CurrentValue);
                ...
                AR_barrels.EditValue = HtmlEncode(AR_barrels.CurrentValue);
                ...
                AR_lt.EditValue = HtmlEncode(AR_lt.CurrentValue);
                ...
                AR_mt.EditValue = HtmlEncode(AR_mt.CurrentValue);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_klobs() {
            if (SFAL_klobs.Visible && SFAL_klobs.Required) {
                if (!SFAL_klobs.IsDetailKey && Empty(SFAL_klobs.FormValue)) {
                    SFAL_klobs.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_kl15() {
            if (SFAL_kl15.Visible && SFAL_kl15.Required) {
                if (!SFAL_kl15.IsDetailKey && Empty(SFAL_kl15.FormValue)) {
                    SFAL_kl15.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_barrels() {
            if (SFAL_barrels.Visible && SFAL_barrels.Required) {
                if (!SFAL_barrels.IsDetailKey && Empty(SFAL_barrels.FormValue)) {
                    SFAL_barrels.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_lt() {
            if (SFAL_lt.Visible && SFAL_lt.Required) {
                if (!SFAL_lt.IsDetailKey && Empty(SFAL_lt.FormValue)) {
                    SFAL_lt.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_mt() {
            if (SFAL_mt.Visible && SFAL_mt.Required) {
                if (!SFAL_mt.IsDetailKey && Empty(SFAL_mt.FormValue)) {
                    SFAL_mt.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }

        private void ValidateCustomAR_klobs() {
            if (AR_klobs.Visible && AR_klobs.Required) {
                if (!AR_klobs.IsDetailKey && Empty(AR_klobs.FormValue)) {
                    AR_klobs.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }

        private void ValidateCustomAR_kl15() {
            if (AR_kl15.Visible && AR_kl15.Required) {
                if (!AR_kl15.IsDetailKey && Empty(AR_kl15.FormValue)) {
                    AR_kl15.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }

        private void ValidateCustomAR_barrels() {
            if (AR_barrels.Visible && AR_barrels.Required) {
                if (!AR_barrels.IsDetailKey && Empty(AR_barrels.FormValue)) {
                    AR_barrels.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }

        private void ValidateCustomAR_lt() {
            if (AR_lt.Visible && AR_lt.Required) {
                if (!AR_lt.IsDetailKey && Empty(AR_lt.FormValue)) {
                    AR_lt.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }

        private void ValidateCustomAR_mt() {
            if (AR_mt.Visible && AR_mt.Required) {
                if (!AR_mt.IsDetailKey && Empty(AR_mt.FormValue)) {
                    AR_mt.AddErrorMessage(ConvertToString(AR_mt.RequiredErrorMessage).Replace("%s", AR_mt.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomAR_klobs();
                ValidateCustomAR_kl15();
                ValidateCustomAR_barrels();
                ValidateCustomAR_lt();
                ValidateCustomAR_mt();
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // AR_klobs
            AR_klobs.SetDbValue(rsnew, AR_klobs.CurrentValue, AR_klobs.ReadOnly);

            // AR_kl15
            AR_kl15.SetDbValue(rsnew, AR_kl15.CurrentValue, AR_kl15.ReadOnly);

            // AR_barrels
            AR_barrels.SetDbValue(rsnew, AR_barrels.CurrentValue, AR_barrels.ReadOnly);

            // AR_lt
            AR_lt.SetDbValue(rsnew, AR_lt.CurrentValue, AR_lt.ReadOnly);

            // AR_mt
            AR_mt.SetDbValue(rsnew, AR_mt.CurrentValue, AR_mt.ReadOnly);
            return rsnew;
        }
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("AR_klobs", out value)) // AR_klobs
                AR_klobs.CurrentValue = value;
            if (row.TryGetValue("AR_kl15", out value)) // AR_kl15
                AR_kl15.CurrentValue = value;
            if (row.TryGetValue("AR_barrels", out value)) // AR_barrels
                AR_barrels.CurrentValue = value;
            if (row.TryGetValue("AR_lt", out value)) // AR_lt
                AR_lt.CurrentValue = value;
            if (row.TryGetValue("AR_mt", out value)) // AR_mt
                AR_mt.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasFormNilaiSfalarstsPymBbmList")), "", TableVar, true);
            string pageId = "edit";
