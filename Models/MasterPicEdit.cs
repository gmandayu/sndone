...
public partial class SnDOne {
    ...
    public class MasterPicEditBase : MasterPic
    {
        ...
        // Constructor
        public MasterPicEditBase(Controller? controller)
        {
            TableName = "MasterPIC";
        ...
        // Constructor
        public MasterPicEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdPIC", out rv)) {
                    IdPIC.FormValue = UrlDecode(rv);
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("MasterPicList");
            }
            return null;
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'NamaPIC'
            NamaPIC.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NamaPIC, "NamaPIC", "x_NamaPIC");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomIdPIC() {
            if (IdPIC.Visible && IdPIC.Required) {
                if (!IdPIC.IsDetailKey && Empty(IdPIC.FormValue)) {
                    IdPIC.AddErrorMessage(ConvertToString(NamaPIC.RequiredErrorMessage).Replace("%s", NamaPIC.Caption));
                }
            }
        }

        private void ValidateCustomNamaPIC() {
