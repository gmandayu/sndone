...
public partial class SnDOne {
    ...
    public class SubAktivitasInputDataKapalStsPyrLpgEditBase : SubAktivitasInputDataKapalStsPyrLpg
    {
        ...
        // Constructor
        public SubAktivitasInputDataKapalStsPyrLpgEditBase()
        {
        ...
        // Constructor
        public SubAktivitasInputDataKapalStsPyrLpgEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasInputDataKapalStsPyrLpgList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasInputDataKapalStsPyrLpgList") {
                                ...
                                returnUrl = "SubAktivitasInputDataKapalStsPyrLpgList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'VoyageNumber' before field var 'x_VoyageNumber'
            val = CurrentForm.HasValue("VoyageNumber") ? CurrentForm.GetValue("VoyageNumber") : CurrentForm.GetValue("x_VoyageNumber");
            if (!VoyageNumber.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("VoyageNumber") && !CurrentForm.HasValue("x_VoyageNumber")) // DN
                    VoyageNumber.Visible = false; // Disable update for API request
                else
                    VoyageNumber.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (VoyageNumber.Visible && VoyageNumber.Required) {
                    if (!VoyageNumber.IsDetailKey && Empty(VoyageNumber.FormValue)) {
                        VoyageNumber.AddErrorMessage(ConvertToString(VoyageNumber.RequiredErrorMessage).Replace("%s", VoyageNumber.Caption));
                    }
                }
