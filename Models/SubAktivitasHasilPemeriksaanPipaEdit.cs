...
public partial class SnDOne {
    ...
    public class SubAktivitasHasilPemeriksaanPipaEditBase : SubAktivitasHasilPemeriksaanPipa
    {
        ...
        // Constructor
        public SubAktivitasHasilPemeriksaanPipaEditBase()
        {
        ...
        // Constructor
        public SubAktivitasHasilPemeriksaanPipaEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasHasilPemeriksaanPipaList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasHasilPemeriksaanPipaList") {
                                ...
                                returnUrl = "SubAktivitasHasilPemeriksaanPipaList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NoBatch' before field var 'x_NoBatch'
            val = CurrentForm.HasValue("NoBatch") ? CurrentForm.GetValue("NoBatch") : CurrentForm.GetValue("x_NoBatch");
            if (!NoBatch.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NoBatch") && !CurrentForm.HasValue("x_NoBatch")) // DN
                    NoBatch.Visible = false; // Disable update for API request
                else
                    NoBatch.SetFormValue(val);
            }
            ...
            // Check field name 'VolumeDischarge' before field var 'x_VolumeDischarge'
            val = CurrentForm.HasValue("VolumeDischarge") ? CurrentForm.GetValue("VolumeDischarge") : CurrentForm.GetValue("x_VolumeDischarge");
            if (!VolumeDischarge.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("VolumeDischarge") && !CurrentForm.HasValue("x_VolumeDischarge")) // DN
                    VolumeDischarge.Visible = false; // Disable update for API request
                else
                    VolumeDischarge.SetFormValue(val);
            }
            ...
            // Check field name 'TotalTerima' before field var 'x_TotalTerima'
            val = CurrentForm.HasValue("TotalTerima") ? CurrentForm.GetValue("TotalTerima") : CurrentForm.GetValue("x_TotalTerima");
            if (!TotalTerima.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TotalTerima") && !CurrentForm.HasValue("x_TotalTerima")) // DN
                    TotalTerima.Visible = false; // Disable update for API request
                else
                    TotalTerima.SetFormValue(val);
            }
            ...
            if (!ETC.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("ETC") && !CurrentForm.HasValue("x_ETC")) // DN
                    ...
                    ETC.SetFormValue(val, true, validate);
            ...
            if (!ETA.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("ETA") && !CurrentForm.HasValue("x_ETA")) // DN
                    ...
                    ETA.SetFormValue(val, true, validate);
            ...
            if (!ETA2.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("ETA2") && !CurrentForm.HasValue("x_ETA2")) // DN
                    ...
                    ETA2.SetFormValue(val, true, validate);
            ...
            if (!ETA3.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("ETA3") && !CurrentForm.HasValue("x_ETA3")) // DN
                    ...
                    ETA3.SetFormValue(val, true, validate);
            ...
            bool validateForm = true;
                ...
                if (NoBatch.Visible && NoBatch.Required) {
                    if (!NoBatch.IsDetailKey && Empty(NoBatch.FormValue)) {
                        NoBatch.AddErrorMessage(ConvertToString(NoBatch.RequiredErrorMessage).Replace("%s", NoBatch.Caption));
                    }
                }
                ...
                if (VolumeDischarge.Visible && VolumeDischarge.Required) {
                    if (!VolumeDischarge.IsDetailKey && Empty(VolumeDischarge.FormValue)) {
                        VolumeDischarge.AddErrorMessage(ConvertToString(VolumeDischarge.RequiredErrorMessage).Replace("%s", VolumeDischarge.Caption));
                    }
                }
                ...
                if (TotalTerima.Visible && TotalTerima.Required) {
                    if (!TotalTerima.IsDetailKey && Empty(TotalTerima.FormValue)) {
                        TotalTerima.AddErrorMessage(ConvertToString(TotalTerima.RequiredErrorMessage).Replace("%s", TotalTerima.Caption));
                    }
                }
