...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiAgreementPenKonList
    /// </summary>
    public static SubAktivitasNilaiAgreementPenKonList subAktivitasNilaiAgreementPenKonList
    {
        get => HttpData.Get<SubAktivitasNilaiAgreementPenKonList>("subAktivitasNilaiAgreementPenKonList")!;
        set => HttpData["subAktivitasNilaiAgreementPenKonList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiAgreementPenKon
    /// </summary>
    public class SubAktivitasNilaiAgreementPenKonList : SubAktivitasNilaiAgreementPenKonListBase
    {
        // Constructor
        public SubAktivitasNilaiAgreementPenKonList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiAgreementPenKonList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiAgreementPenKonListBase : SubAktivitasNilaiAgreementPenKon
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiAgreementPenKonList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasNilaiAgreementPenKonlist";
        ...
        // Constructor
        public SubAktivitasNilaiAgreementPenKonListBase()
        {
            ...
            // Initialize URLs
            AddUrl = "SubAktivitasNilaiAgreementPenKonAdd";
            ...
            MultiDeleteUrl = "SubAktivitasNilaiAgreementPenKonDelete";
            MultiUpdateUrl = "SubAktivitasNilaiAgreementPenKonUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiAgreementPenKonList";
        ...
        // Constructor
        public SubAktivitasNilaiAgreementPenKonListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasNilaiAgreementPenKongrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasNilaiAgreementPenKonList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasNilaiAgreementPenKon"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiAgreementPenKonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiAgreementPenKonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasNilaiAgreementPenKonsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasNilaiAgreementPenKonsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasNilaiAgreementPenKonlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasNilaiAgreementPenKon");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasNilaiAgreementPenKonList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasNilaiAgreementPenKonList.RowCount) + "_SubAktivitasNilaiAgreementPenKon");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasNilaiAgreementPenKonList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasNilaiAgreementPenKonList.RowType == RowType.Add || IsEdit && subAktivitasNilaiAgreementPenKonList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasNilaiAgreementPenKonsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
