...
public partial class SnDOne {
    ...
    public class SubAktivitasFormInputNilaiAktualDischargeListBase : SubAktivitasFormInputNilaiAktualDischarge
    {
        ...
        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeListBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasFormInputNilaiAktualDischargeListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field KL_Obs
            if (filter.TryGetValue("x_KL_Obs", out sv)) {
                RestoreFieldFilter(KL_Obs, filter, "KL_Obs", sv);
            }

            // Field KL_15
            if (filter.TryGetValue("x_KL_15", out sv)) {
                RestoreFieldFilter(KL_15, filter, "KL_15", sv);
            }

            // Field Barrels
            if (filter.TryGetValue("x_Barrels", out sv)) {
                RestoreFieldFilter(Barrels, filter, "Barrels", sv);
            }

            // Field LT
            if (filter.TryGetValue("x_LT", out sv)) {
                RestoreFieldFilter(LT, filter, "LT", sv);
            }

            // Field MT
            if (filter.TryGetValue("x_MT", out sv)) {
                RestoreFieldFilter(MT, filter, "MT", sv);
            }
