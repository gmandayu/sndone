...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasTotalPenyaluranEdit
    /// </summary>
    public static SubAktivitasTotalPenyaluranEdit subAktivitasTotalPenyaluranEdit
    {
        get => HttpData.Get<SubAktivitasTotalPenyaluranEdit>("subAktivitasTotalPenyaluranEdit")!;
        set => HttpData["subAktivitasTotalPenyaluranEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasTotalPenyaluran
    /// </summary>
    public class SubAktivitasTotalPenyaluranEdit : SubAktivitasTotalPenyaluranEditBase
    {
        // Constructor
        public SubAktivitasTotalPenyaluranEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasTotalPenyaluranEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasTotalPenyaluranEditBase : SubAktivitasTotalPenyaluran
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasTotalPenyaluranEdit";
        ...
        // Constructor
        public SubAktivitasTotalPenyaluranEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasTotalPenyaluranEdit";
        ...
        // Constructor
        public SubAktivitasTotalPenyaluranEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasTotalPenyaluranList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasTotalPenyaluranList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasTotalPenyaluranList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasTotalPenyaluranEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'TotalPenyaluran'
            TotalPenyaluran.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(TotalPenyaluran, "TotalPenyaluran", "x_TotalPenyaluran");

            // Standard handling for 'StokAkhir'
            StokAkhir.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(StokAkhir, "StokAkhir", "x_StokAkhir");
            if (!id.IsDetailKey) {
        ...
        public void RestoreFormValues()
        {
            ...
            TotalPenyaluran.CurrentValue = TotalPenyaluran.FormValue;
            StokAkhir.CurrentValue = StokAkhir.FormValue;
        }
        ...
        public async Task RenderRow()
        {
            ...
            // TotalPenyaluran
            TotalPenyaluran.RowCssClass = "row";

            // StokAkhir
            StokAkhir.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                TotalPenyaluran.EditValue = HtmlEncode(TotalPenyaluran.CurrentValue);
                ...
                StokAkhir.EditValue = HtmlEncode(StokAkhir.CurrentValue);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(StokAkhir.RequiredErrorMessage).Replace("%s", StokAkhir.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(StokAkhir.RequiredErrorMessage).Replace("%s", StokAkhir.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(StokAkhir.RequiredErrorMessage).Replace("%s", StokAkhir.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(StokAkhir.RequiredErrorMessage).Replace("%s", StokAkhir.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(StokAkhir.RequiredErrorMessage).Replace("%s", StokAkhir.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(StokAkhir.RequiredErrorMessage).Replace("%s", StokAkhir.Caption));
                }
            }
        }

        private void ValidateCustomTotalPenyaluran() {
            if (TotalPenyaluran.Visible && TotalPenyaluran.Required) {
                if (!TotalPenyaluran.IsDetailKey && Empty(TotalPenyaluran.FormValue)) {
                    TotalPenyaluran.AddErrorMessage(ConvertToString(StokAkhir.RequiredErrorMessage).Replace("%s", StokAkhir.Caption));
                }
            }
        }

        private void ValidateCustomStokAkhir() {
            if (StokAkhir.Visible && StokAkhir.Required) {
                if (!StokAkhir.IsDetailKey && Empty(StokAkhir.FormValue)) {
                    StokAkhir.AddErrorMessage(ConvertToString(StokAkhir.RequiredErrorMessage).Replace("%s", StokAkhir.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomTotalPenyaluran();
                ValidateCustomStokAkhir();
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // TotalPenyaluran
            TotalPenyaluran.SetDbValue(rsnew, TotalPenyaluran.CurrentValue, TotalPenyaluran.ReadOnly);

            // StokAkhir
            StokAkhir.SetDbValue(rsnew, StokAkhir.CurrentValue, StokAkhir.ReadOnly);
            return rsnew;
        }
        ...
        {
            object? value;
            if (row.TryGetValue("TotalPenyaluran", out value)) // TotalPenyaluran
                TotalPenyaluran.CurrentValue = value;
            if (row.TryGetValue("StokAkhir", out value)) // StokAkhir
                StokAkhir.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasTotalPenyaluranList")), "", TableVar, true);
            string pageId = "edit";
