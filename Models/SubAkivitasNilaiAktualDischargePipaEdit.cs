...
public partial class SnDOne {
    /// <summary>
    /// subAkivitasNilaiAktualDischargePipaEdit
    /// </summary>
    public static SubAkivitasNilaiAktualDischargePipaEdit subAkivitasNilaiAktualDischargePipaEdit
    {
        get => HttpData.Get<SubAkivitasNilaiAktualDischargePipaEdit>("subAkivitasNilaiAktualDischargePipaEdit")!;
        set => HttpData["subAkivitasNilaiAktualDischargePipaEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAkivitasNilaiAktualDischargePipa
    /// </summary>
    public class SubAkivitasNilaiAktualDischargePipaEdit : SubAkivitasNilaiAktualDischargePipaEditBase
    {
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAkivitasNilaiAktualDischargePipaEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAkivitasNilaiAktualDischargePipaEditBase : SubAkivitasNilaiAktualDischargePipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAkivitasNilaiAktualDischargePipaEdit";
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaEditBase()
        {
        ...
        // Page name
        public string PageName => "SubAkivitasNilaiAktualDischargePipaEdit";
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAkivitasNilaiAktualDischargePipaList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = ReturnUrl;
                    if (GetPageName(returnUrl) == "SubAkivitasNilaiAktualDischargePipaList")
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAkivitasNilaiAktualDischargePipaList") {
                                ...
                                returnUrl = "SubAkivitasNilaiAktualDischargePipaList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAkivitasNilaiAktualDischargePipaEdit?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'AktualDischarge_klobs' before field var 'x_AktualDischarge_klobs'
            val = CurrentForm.HasValue("AktualDischarge_klobs") ? CurrentForm.GetValue("AktualDischarge_klobs") : CurrentForm.GetValue("x_AktualDischarge_klobs");
            if (!AktualDischarge_klobs.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AktualDischarge_klobs") && !CurrentForm.HasValue("x_AktualDischarge_klobs")) // DN
                    AktualDischarge_klobs.Visible = false; // Disable update for API request
                else
                    AktualDischarge_klobs.SetFormValue(val);
            }

            // Check field name 'AktualDischarge_kl15' before field var 'x_AktualDischarge_kl15'
            val = CurrentForm.HasValue("AktualDischarge_kl15") ? CurrentForm.GetValue("AktualDischarge_kl15") : CurrentForm.GetValue("x_AktualDischarge_kl15");
            if (!AktualDischarge_kl15.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AktualDischarge_kl15") && !CurrentForm.HasValue("x_AktualDischarge_kl15")) // DN
                    AktualDischarge_kl15.Visible = false; // Disable update for API request
                else
                    AktualDischarge_kl15.SetFormValue(val);
            }

            // Check field name 'AktualDischarge_barrels' before field var 'x_AktualDischarge_barrels'
            val = CurrentForm.HasValue("AktualDischarge_barrels") ? CurrentForm.GetValue("AktualDischarge_barrels") : CurrentForm.GetValue("x_AktualDischarge_barrels");
            if (!AktualDischarge_barrels.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AktualDischarge_barrels") && !CurrentForm.HasValue("x_AktualDischarge_barrels")) // DN
                    AktualDischarge_barrels.Visible = false; // Disable update for API request
                else
                    AktualDischarge_barrels.SetFormValue(val);
            }

            // Check field name 'AktualDischarge_lt' before field var 'x_AktualDischarge_lt'
            val = CurrentForm.HasValue("AktualDischarge_lt") ? CurrentForm.GetValue("AktualDischarge_lt") : CurrentForm.GetValue("x_AktualDischarge_lt");
            if (!AktualDischarge_lt.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AktualDischarge_lt") && !CurrentForm.HasValue("x_AktualDischarge_lt")) // DN
                    AktualDischarge_lt.Visible = false; // Disable update for API request
                else
                    AktualDischarge_lt.SetFormValue(val);
            }

            // Check field name 'AktualDischarge_mt' before field var 'x_AktualDischarge_mt'
            val = CurrentForm.HasValue("AktualDischarge_mt") ? CurrentForm.GetValue("AktualDischarge_mt") : CurrentForm.GetValue("x_AktualDischarge_mt");
            if (!AktualDischarge_mt.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AktualDischarge_mt") && !CurrentForm.HasValue("x_AktualDischarge_mt")) // DN
                    AktualDischarge_mt.Visible = false; // Disable update for API request
                else
                    AktualDischarge_mt.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            AktualDischarge_klobs.CurrentValue = AktualDischarge_klobs.FormValue;
            AktualDischarge_kl15.CurrentValue = AktualDischarge_kl15.FormValue;
            AktualDischarge_barrels.CurrentValue = AktualDischarge_barrels.FormValue;
            AktualDischarge_lt.CurrentValue = AktualDischarge_lt.FormValue;
            AktualDischarge_mt.CurrentValue = AktualDischarge_mt.FormValue;
        ...
        public async Task RenderRow()
        {
            ...
            AktualDischarge_klobs.RowCssClass = "row";

            // AktualDischarge_kl15
            AktualDischarge_kl15.RowCssClass = "row";

            // AktualDischarge_barrels
            AktualDischarge_barrels.RowCssClass = "row";

            // AktualDischarge_lt
            AktualDischarge_lt.RowCssClass = "row";

            // AktualDischarge_mt
            AktualDischarge_mt.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                // AktualDischarge_klobs
                AktualDischarge_klobs.SetupEditAttributes();
                if (!AktualDischarge_klobs.Raw)
                    AktualDischarge_klobs.CurrentValue = HtmlDecode(AktualDischarge_klobs.CurrentValue);
                AktualDischarge_klobs.EditValue = HtmlEncode(AktualDischarge_klobs.CurrentValue);
                AktualDischarge_klobs.PlaceHolder = RemoveHtml(AktualDischarge_klobs.Caption);

                // AktualDischarge_kl15
                AktualDischarge_kl15.SetupEditAttributes();
                if (!AktualDischarge_kl15.Raw)
                    AktualDischarge_kl15.CurrentValue = HtmlDecode(AktualDischarge_kl15.CurrentValue);
                AktualDischarge_kl15.EditValue = HtmlEncode(AktualDischarge_kl15.CurrentValue);
                AktualDischarge_kl15.PlaceHolder = RemoveHtml(AktualDischarge_kl15.Caption);

                // AktualDischarge_barrels
                AktualDischarge_barrels.SetupEditAttributes();
                if (!AktualDischarge_barrels.Raw)
                    AktualDischarge_barrels.CurrentValue = HtmlDecode(AktualDischarge_barrels.CurrentValue);
                AktualDischarge_barrels.EditValue = HtmlEncode(AktualDischarge_barrels.CurrentValue);
                AktualDischarge_barrels.PlaceHolder = RemoveHtml(AktualDischarge_barrels.Caption);

                // AktualDischarge_lt
                AktualDischarge_lt.SetupEditAttributes();
                if (!AktualDischarge_lt.Raw)
                    AktualDischarge_lt.CurrentValue = HtmlDecode(AktualDischarge_lt.CurrentValue);
                AktualDischarge_lt.EditValue = HtmlEncode(AktualDischarge_lt.CurrentValue);
                AktualDischarge_lt.PlaceHolder = RemoveHtml(AktualDischarge_lt.Caption);

                // AktualDischarge_mt
                AktualDischarge_mt.SetupEditAttributes();
                if (!AktualDischarge_mt.Raw)
                    AktualDischarge_mt.CurrentValue = HtmlDecode(AktualDischarge_mt.CurrentValue);
                AktualDischarge_mt.EditValue = HtmlEncode(AktualDischarge_mt.CurrentValue);
                AktualDischarge_mt.PlaceHolder = RemoveHtml(AktualDischarge_mt.Caption);
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                if (AktualDischarge_klobs.Visible && AktualDischarge_klobs.Required) {
                    if (!AktualDischarge_klobs.IsDetailKey && Empty(AktualDischarge_klobs.FormValue)) {
                        AktualDischarge_klobs.AddErrorMessage(ConvertToString(AktualDischarge_klobs.RequiredErrorMessage).Replace("%s", AktualDischarge_klobs.Caption));
                    }
                }
                if (AktualDischarge_kl15.Visible && AktualDischarge_kl15.Required) {
                    if (!AktualDischarge_kl15.IsDetailKey && Empty(AktualDischarge_kl15.FormValue)) {
                        AktualDischarge_kl15.AddErrorMessage(ConvertToString(AktualDischarge_kl15.RequiredErrorMessage).Replace("%s", AktualDischarge_kl15.Caption));
                    }
                }
                if (AktualDischarge_barrels.Visible && AktualDischarge_barrels.Required) {
                    if (!AktualDischarge_barrels.IsDetailKey && Empty(AktualDischarge_barrels.FormValue)) {
                        AktualDischarge_barrels.AddErrorMessage(ConvertToString(AktualDischarge_barrels.RequiredErrorMessage).Replace("%s", AktualDischarge_barrels.Caption));
                    }
                }
                if (AktualDischarge_lt.Visible && AktualDischarge_lt.Required) {
                    if (!AktualDischarge_lt.IsDetailKey && Empty(AktualDischarge_lt.FormValue)) {
                        AktualDischarge_lt.AddErrorMessage(ConvertToString(AktualDischarge_lt.RequiredErrorMessage).Replace("%s", AktualDischarge_lt.Caption));
                    }
                }
                if (AktualDischarge_mt.Visible && AktualDischarge_mt.Required) {
                    if (!AktualDischarge_mt.IsDetailKey && Empty(AktualDischarge_mt.FormValue)) {
                        AktualDischarge_mt.AddErrorMessage(ConvertToString(AktualDischarge_mt.RequiredErrorMessage).Replace("%s", AktualDischarge_mt.Caption));
                    }
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // AktualDischarge_klobs
            AktualDischarge_klobs.SetDbValue(rsnew, AktualDischarge_klobs.CurrentValue, AktualDischarge_klobs.ReadOnly);

            // AktualDischarge_kl15
            AktualDischarge_kl15.SetDbValue(rsnew, AktualDischarge_kl15.CurrentValue, AktualDischarge_kl15.ReadOnly);

            // AktualDischarge_barrels
            AktualDischarge_barrels.SetDbValue(rsnew, AktualDischarge_barrels.CurrentValue, AktualDischarge_barrels.ReadOnly);

            // AktualDischarge_lt
            AktualDischarge_lt.SetDbValue(rsnew, AktualDischarge_lt.CurrentValue, AktualDischarge_lt.ReadOnly);

            // AktualDischarge_mt
            AktualDischarge_mt.SetDbValue(rsnew, AktualDischarge_mt.CurrentValue, AktualDischarge_mt.ReadOnly);
            return rsnew;
        }
        ...
        {
            object? value;
            if (row.TryGetValue("AktualDischarge_klobs", out value)) // AktualDischarge_klobs
                AktualDischarge_klobs.CurrentValue = value;
            if (row.TryGetValue("AktualDischarge_kl15", out value)) // AktualDischarge_kl15
                AktualDischarge_kl15.CurrentValue = value;
            if (row.TryGetValue("AktualDischarge_barrels", out value)) // AktualDischarge_barrels
                AktualDischarge_barrels.CurrentValue = value;
            if (row.TryGetValue("AktualDischarge_lt", out value)) // AktualDischarge_lt
                AktualDischarge_lt.CurrentValue = value;
            if (row.TryGetValue("AktualDischarge_mt", out value)) // AktualDischarge_mt
                AktualDischarge_mt.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAkivitasNilaiAktualDischargePipaList")), "", TableVar, true);
            string pageId = "edit";
