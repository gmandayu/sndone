...
public partial class SnDOne {
    ...
    public class SubAktivitasDataMobilTangkiMtGateInLpgEditBase : SubAktivitasDataMobilTangkiMtGateInLpg
    {
        ...
        // Constructor
        public SubAktivitasDataMobilTangkiMtGateInLpgEditBase()
        {
        ...
        // Constructor
        public SubAktivitasDataMobilTangkiMtGateInLpgEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasDataMobilTangkiMtGateInLpgList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasDataMobilTangkiMtGateInLpgList") {
                                ...
                                returnUrl = "SubAktivitasDataMobilTangkiMtGateInLpgList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'AMT' before field var 'x_AMT'
            val = CurrentForm.HasValue("AMT") ? CurrentForm.GetValue("AMT") : CurrentForm.GetValue("x_AMT");
            if (!AMT.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AMT") && !CurrentForm.HasValue("x_AMT")) // DN
                    AMT.Visible = false; // Disable update for API request
                else
                    AMT.SetFormValue(val);
            }

            // Check field name 'WaktuGateIn' before field var 'x_WaktuGateIn'
            val = CurrentForm.HasValue("WaktuGateIn") ? CurrentForm.GetValue("WaktuGateIn") : CurrentForm.GetValue("x_WaktuGateIn");
            if (!WaktuGateIn.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("WaktuGateIn") && !CurrentForm.HasValue("x_WaktuGateIn")) // DN
                    WaktuGateIn.Visible = false; // Disable update for API request
                else
                    WaktuGateIn.SetFormValue(val, true, validate);
            ...
            bool validateForm = true;
                ...
                if (AMT.Visible && AMT.Required) {
                    if (!AMT.IsDetailKey && Empty(AMT.FormValue)) {
                        AMT.AddErrorMessage(ConvertToString(AMT.RequiredErrorMessage).Replace("%s", AMT.Caption));
                    }
                }
                if (WaktuGateIn.Visible && WaktuGateIn.Required) {
                    if (!WaktuGateIn.IsDetailKey && Empty(WaktuGateIn.FormValue)) {
                        WaktuGateIn.AddErrorMessage(ConvertToString(WaktuGateIn.RequiredErrorMessage).Replace("%s", WaktuGateIn.Caption));
                    }
                }
