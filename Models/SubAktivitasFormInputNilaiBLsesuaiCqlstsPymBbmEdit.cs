...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit
    /// </summary>
    public static SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit
    {
        get => HttpData.Get<SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit>("subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit")!;
        set => HttpData["subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasFormInputNilaiBLsesuaiCQLSTSPymBBM
    /// </summary>
    public class SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit : SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEditBase
    {
        // Constructor
        public SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEditBase : SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbm
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit";
        ...
        // Constructor
        public SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEditBase(Controller? controller)
        {
            TableName = "SubAktivitasFormInputNilaiBLsesuaiCQLSTSPymBBM";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbm)
            if (subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbm == null || subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbm is SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbm)
                subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbm = this;
        ...
        // Page name
        public string PageName => "SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit";
        ...
        public void SetVisibility()
        {
            ...
            SFAL_KL_Obs.SetVisibility();
            SFAL_KL_15.SetVisibility();
        ...
        // Constructor
        public SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEditBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                ...
                // List / View / Master View page
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'KL_Obs'
            KL_Obs.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(KL_Obs, "KL_Obs", "x_KL_Obs");

            // Standard handling for 'KL_15'
            KL_15.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(KL_15, "KL_15", "x_KL_15");

            // Standard handling for 'Barrels'
            Barrels.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Barrels, "Barrels", "x_Barrels");

            // Standard handling for 'LT'
            LT.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(LT, "LT", "x_LT");

            // Standard handling for 'MT'
            MT.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(MT, "MT", "x_MT");

            // Standard handling for 'SFAL_KL_Obs'
            SFAL_KL_Obs.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(SFAL_KL_Obs, "SFAL_KL_Obs", "x_SFAL_KL_Obs");

            // Standard handling for 'SFAL_KL_15'
            SFAL_KL_15.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(SFAL_KL_15, "SFAL_KL_15", "x_SFAL_KL_15");
        ...
        public void RestoreFormValues()
        {
            ...
            SFAL_KL_Obs.CurrentValue = SFAL_KL_Obs.FormValue;
            SFAL_KL_15.CurrentValue = SFAL_KL_15.FormValue;
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("SFAL_KL_Obs", SFAL_KL_Obs.DefaultValue ?? DbNullValue); // DN
            row.Add("SFAL_KL_15", SFAL_KL_15.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // SFAL_KL_Obs
            SFAL_KL_Obs.RowCssClass = "row";

            // SFAL_KL_15
            SFAL_KL_15.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                SFAL_KL_Obs.EditValue = HtmlEncode(SFAL_KL_Obs.CurrentValue);
                ...
                SFAL_KL_15.EditValue = HtmlEncode(SFAL_KL_15.CurrentValue);
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }

        private void ValidateCustomKL_Obs() {
            if (KL_Obs.Visible && KL_Obs.Required) {
                if (!KL_Obs.IsDetailKey && Empty(KL_Obs.FormValue)) {
                    KL_Obs.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }

        private void ValidateCustomKL_15() {
            if (KL_15.Visible && KL_15.Required) {
                if (!KL_15.IsDetailKey && Empty(KL_15.FormValue)) {
                    KL_15.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }

        private void ValidateCustomBarrels() {
            if (Barrels.Visible && Barrels.Required) {
                if (!Barrels.IsDetailKey && Empty(Barrels.FormValue)) {
                    Barrels.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }

        private void ValidateCustomLT() {
            if (LT.Visible && LT.Required) {
                if (!LT.IsDetailKey && Empty(LT.FormValue)) {
                    LT.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }

        private void ValidateCustomMT() {
            if (MT.Visible && MT.Required) {
                if (!MT.IsDetailKey && Empty(MT.FormValue)) {
                    MT.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }

        private void ValidateCustomSFAL_KL_Obs() {
            if (SFAL_KL_Obs.Visible && SFAL_KL_Obs.Required) {
                if (!SFAL_KL_Obs.IsDetailKey && Empty(SFAL_KL_Obs.FormValue)) {
                    SFAL_KL_Obs.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }

        private void ValidateCustomSFAL_KL_15() {
            if (SFAL_KL_15.Visible && SFAL_KL_15.Required) {
                if (!SFAL_KL_15.IsDetailKey && Empty(SFAL_KL_15.FormValue)) {
                    SFAL_KL_15.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_Barrels() {
            if (SFAL_Barrels.Visible && SFAL_Barrels.Required) {
                if (!SFAL_Barrels.IsDetailKey && Empty(SFAL_Barrels.FormValue)) {
                    SFAL_Barrels.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_LT() {
            if (SFAL_LT.Visible && SFAL_LT.Required) {
                if (!SFAL_LT.IsDetailKey && Empty(SFAL_LT.FormValue)) {
                    SFAL_LT.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomKL_Obs();
                ValidateCustomKL_15();
                ValidateCustomBarrels();
                ValidateCustomLT();
                ValidateCustomMT();
                ValidateCustomSFAL_KL_Obs();
                ValidateCustomSFAL_KL_15();
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // KL_Obs
            KL_Obs.SetDbValue(rsnew, KL_Obs.CurrentValue, KL_Obs.ReadOnly);

            // KL_15
            KL_15.SetDbValue(rsnew, KL_15.CurrentValue, KL_15.ReadOnly);

            // Barrels
            Barrels.SetDbValue(rsnew, Barrels.CurrentValue, Barrels.ReadOnly);

            // LT
            LT.SetDbValue(rsnew, LT.CurrentValue, LT.ReadOnly);

            // MT
            MT.SetDbValue(rsnew, MT.CurrentValue, MT.ReadOnly);

            // SFAL_KL_Obs
            SFAL_KL_Obs.SetDbValue(rsnew, SFAL_KL_Obs.CurrentValue, SFAL_KL_Obs.ReadOnly);

            // SFAL_KL_15
            SFAL_KL_15.SetDbValue(rsnew, SFAL_KL_15.CurrentValue, SFAL_KL_15.ReadOnly);
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("KL_Obs", out value)) // KL_Obs
                KL_Obs.CurrentValue = value;
            if (row.TryGetValue("KL_15", out value)) // KL_15
                KL_15.CurrentValue = value;
            if (row.TryGetValue("Barrels", out value)) // Barrels
                Barrels.CurrentValue = value;
            if (row.TryGetValue("LT", out value)) // LT
                LT.CurrentValue = value;
            if (row.TryGetValue("MT", out value)) // MT
                MT.CurrentValue = value;
            if (row.TryGetValue("SFAL_KL_Obs", out value)) // SFAL_KL_Obs
                SFAL_KL_Obs.CurrentValue = value;
            if (row.TryGetValue("SFAL_KL_15", out value)) // SFAL_KL_15
                SFAL_KL_15.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmList")), "", TableVar, true);
            string pageId = "edit";
