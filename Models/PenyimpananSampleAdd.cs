...
public partial class SnDOne {
    /// <summary>
    /// penyimpananSampleAdd
    /// </summary>
    public static PenyimpananSampleAdd penyimpananSampleAdd
    {
        get => HttpData.Get<PenyimpananSampleAdd>("penyimpananSampleAdd")!;
        set => HttpData["penyimpananSampleAdd"] = value;
    }

    /// <summary>
    /// Page class for PenyimpananSample
    /// </summary>
    public class PenyimpananSampleAdd : PenyimpananSampleAddBase
    {
        // Constructor
        public PenyimpananSampleAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public PenyimpananSampleAdd() : base()
        {
        }
        ...
        public override void PageRender() {
            ...
            if (CurrentPageID() != "add")
                return;
            if (Empty(ExpiredEst.EditValue))
                ExpiredEst.EditValue = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
            if(Empty(Tanggal.EditValue))
                Tanggal.EditValue = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class PenyimpananSampleAddBase : PenyimpananSample
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "penyimpananSampleAdd";
        ...
        // Constructor
        public PenyimpananSampleAddBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "PenyimpananSampleAdd";
        ...
        {
            id.Visible = false;
            NomorPenyimpananSample.Visible = false;
            ...
            TanggalDimusnahkan.Visible = false;
            LokasiPemusnahan.Visible = false;
        ...
        // Constructor
        public PenyimpananSampleAddBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private string DetermineReturnUrl()
        {
            ...
            else if (GetPageName(returnUrl) == "PenyimpananSampleView")
                returnUrl = ViewUrl;
            return returnUrl;
        }
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        penyimpananSampleAdd?.PageRender();
                    }
        }
        ...
        protected async Task LoadFormValues() {
            ...
            SetNormalField(ExpiredEst, "ExpiredEst", "x_ExpiredEst", true, validate, true);
        }
        #pragma warning restore 1998
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                JenisSample.EditValue = JenisSample.Options(true);
                JenisSample.PlaceHolder = RemoveHtml(JenisSample.Caption);
                ...
                if (IdPlant.Lookup != null && IsDictionary(IdPlant.Lookup?.Options) && IdPlant.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    ...
                    if (curVal2 == "") {
                        ...
                        filterWrk2 = SearchFilter(IdPlant.Lookup?.GetTable()?.Fields["IdPlant"].SearchExpression, "=", IdPlant.CurrentValue, IdPlant.Lookup?.GetTable()?.Fields["IdPlant"].SearchDataType, "");
                    }
                    string? sqlWrk2 = IdPlant.Lookup?.GetSql(true, filterWrk2, null, this, false, true);
                    List<Dictionary<string, object>>? rswrk2 = sqlWrk2 != null ? Connection.GetRows(sqlWrk2) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                    IdPlant.EditValue = rswrk2;
                }
                ...
                if (!NamaMasterSample.Raw)
                    NamaMasterSample.CurrentValue = HtmlDecode(NamaMasterSample.CurrentValue);
                NamaMasterSample.EditValue = HtmlEncode(NamaMasterSample.CurrentValue);
                ...
                Foto.EditValue = Foto.CurrentValue; // DN
                Foto.PlaceHolder = RemoveHtml(Foto.Caption);

                // ExpiredEst
                ExpiredEst.SetupEditAttributes();
                ExpiredEst.EditValue = FormatDateTime(ExpiredEst.CurrentValue, ExpiredEst.FormatPattern);
        ...
        private void ValidateCustomJenisSample() {
            if (JenisSample.Visible && JenisSample.Required) {
                if (!JenisSample.IsDetailKey && Empty(JenisSample.FormValue)) {
                    JenisSample.AddErrorMessage(ConvertToString(ExpiredEst.RequiredErrorMessage).Replace("%s", ExpiredEst.Caption));
                }
            }
        }
        ...
        private void ValidateCustomIdPlant() {
            if (IdPlant.Visible && IdPlant.Required) {
                if (!IdPlant.IsDetailKey && Empty(IdPlant.FormValue)) {
                    IdPlant.AddErrorMessage(ConvertToString(ExpiredEst.RequiredErrorMessage).Replace("%s", ExpiredEst.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTanggal() {
            if (Tanggal.Visible && Tanggal.Required) {
                if (!Tanggal.IsDetailKey && Empty(Tanggal.FormValue)) {
                    Tanggal.AddErrorMessage(ConvertToString(ExpiredEst.RequiredErrorMessage).Replace("%s", ExpiredEst.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNamaMasterSample() {
            if (NamaMasterSample.Visible && NamaMasterSample.Required) {
                if (!NamaMasterSample.IsDetailKey && Empty(NamaMasterSample.FormValue)) {
                    NamaMasterSample.AddErrorMessage(ConvertToString(ExpiredEst.RequiredErrorMessage).Replace("%s", ExpiredEst.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorSegel() {
            if (NomorSegel.Visible && NomorSegel.Required) {
                if (!NomorSegel.IsDetailKey && Empty(NomorSegel.FormValue)) {
                    NomorSegel.AddErrorMessage(ConvertToString(ExpiredEst.RequiredErrorMessage).Replace("%s", ExpiredEst.Caption));
                }
            }
        }
        ...
        private void ValidateCustomFoto() {
            if (Foto.Visible && Foto.Required) {
                if (!Foto.IsDetailKey && Empty(Foto.FormValue)) {
                    Foto.AddErrorMessage(ConvertToString(ExpiredEst.RequiredErrorMessage).Replace("%s", ExpiredEst.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                if (!CheckDate(ExpiredEst.FormValue, ExpiredEst.FormatPattern)) {
                    ExpiredEst.AddErrorMessage(ExpiredEst.GetErrorMessage(false));
                }
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // JenisSample
                JenisSample.SetDbValue(rsnew, JenisSample.CurrentValue);
                ...
                // NamaMasterSample
                NamaMasterSample.SetDbValue(rsnew, NamaMasterSample.CurrentValue);

                // NomorSegel
                NomorSegel.SetDbValue(rsnew, NomorSegel.CurrentValue);

                // Foto
                Foto.SetDbValue(rsnew, Foto.CurrentValue);

                // ExpiredEst
                ExpiredEst.SetDbValue(rsnew, ConvertToDateTime(ExpiredEst.CurrentValue, ExpiredEst.FormatPattern));
                return rsnew;
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
        ...
        {
            object? value;
            if (row.TryGetValue("JenisSample", out value)) // JenisSample
                JenisSample.SetFormValue(ConvertToString(value));
            ...
            if (row.TryGetValue("NamaMasterSample", out value)) // NamaMasterSample
                NamaMasterSample.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("NomorSegel", out value)) // NomorSegel
                NomorSegel.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("Foto", out value)) // Foto
                Foto.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("ExpiredEst", out value)) // ExpiredEst
                ExpiredEst.SetFormValue(ConvertToString(value));
        }
