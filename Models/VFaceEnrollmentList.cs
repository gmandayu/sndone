...
public partial class SnDOne {
    ...
    /// vFaceEnrollmentList
    /// </summary>
    public static VFaceEnrollmentList vFaceEnrollmentList
    {
        get => HttpData.Get<VFaceEnrollmentList>("vFaceEnrollmentList")!;
        set => HttpData["vFaceEnrollmentList"] = value;
    }

    /// <summary>
    /// Page class for VFaceEnrollment
    /// </summary>
    public class VFaceEnrollmentList : VFaceEnrollmentListBase
    {
        // Constructor
        public VFaceEnrollmentList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public VFaceEnrollmentList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class VFaceEnrollmentListBase : VFaceEnrollment
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "vFaceEnrollmentList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fVFaceEnrollmentlist";
        ...
        // Constructor
        public VFaceEnrollmentListBase()
        {
            ...
            MultiDeleteUrl = "VFaceEnrollmentDelete";
            MultiUpdateUrl = "VFaceEnrollmentUpdate";
        ...
        // Page name
        public string PageName => "VFaceEnrollmentList";
        ...
        // Constructor
        public VFaceEnrollmentListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                vFaceEnrollmentList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(IdUser.AdvancedSearch.ToJson())); // Field IdUser
            ...
            filters.Merge(JObject.Parse(NamaLengkap.AdvancedSearch.ToJson())); // Field NamaLengkap
            filters.Merge(JObject.Parse(IdPosition.AdvancedSearch.ToJson())); // Field IdPosition
            filters.Merge(JObject.Parse(Jabatan.AdvancedSearch.ToJson())); // Field Jabatan
            filters.Merge(JObject.Parse(Face.AdvancedSearch.ToJson())); // Field Face
            filters.Merge(JObject.Parse(TanggalInputFace.AdvancedSearch.ToJson())); // Field TanggalInputFace
            filters.Merge(JObject.Parse(UserInputFace.AdvancedSearch.ToJson())); // Field UserInputFace
            filters.Merge(JObject.Parse(AzurePersonId.AdvancedSearch.ToJson())); // Field AzurePersonId
        ...
        protected bool RestoreFilterList() {
            ...
            // Field IdUser
            if (filter?.TryGetValue("x_IdUser", out sv) ?? false) {
                IdUser.AdvancedSearch.SearchValue = sv;
                IdUser.AdvancedSearch.SearchOperator = filter["z_IdUser"];
                IdUser.AdvancedSearch.SearchCondition = filter["v_IdUser"];
                IdUser.AdvancedSearch.SearchValue2 = filter["y_IdUser"];
                IdUser.AdvancedSearch.SearchOperator2 = filter["w_IdUser"];
                IdUser.AdvancedSearch.Save();
            }
            ...
            // Field NamaLengkap
            if (filter?.TryGetValue("x_NamaLengkap", out sv) ?? false) {
                NamaLengkap.AdvancedSearch.SearchValue = sv;
                NamaLengkap.AdvancedSearch.SearchOperator = filter["z_NamaLengkap"];
                NamaLengkap.AdvancedSearch.SearchCondition = filter["v_NamaLengkap"];
                NamaLengkap.AdvancedSearch.SearchValue2 = filter["y_NamaLengkap"];
                NamaLengkap.AdvancedSearch.SearchOperator2 = filter["w_NamaLengkap"];
                NamaLengkap.AdvancedSearch.Save();
            }
            ...
            // Field Jabatan
            if (filter?.TryGetValue("x_Jabatan", out sv) ?? false) {
                Jabatan.AdvancedSearch.SearchValue = sv;
                Jabatan.AdvancedSearch.SearchOperator = filter["z_Jabatan"];
                Jabatan.AdvancedSearch.SearchCondition = filter["v_Jabatan"];
                Jabatan.AdvancedSearch.SearchValue2 = filter["y_Jabatan"];
                Jabatan.AdvancedSearch.SearchOperator2 = filter["w_Jabatan"];
                Jabatan.AdvancedSearch.Save();
            }

            // Field Face
            if (filter?.TryGetValue("x_Face", out sv) ?? false) {
                Face.AdvancedSearch.SearchValue = sv;
                Face.AdvancedSearch.SearchOperator = filter["z_Face"];
                Face.AdvancedSearch.SearchCondition = filter["v_Face"];
                Face.AdvancedSearch.SearchValue2 = filter["y_Face"];
                Face.AdvancedSearch.SearchOperator2 = filter["w_Face"];
                Face.AdvancedSearch.Save();
            }

            // Field TanggalInputFace
            if (filter?.TryGetValue("x_TanggalInputFace", out sv) ?? false) {
                TanggalInputFace.AdvancedSearch.SearchValue = sv;
                TanggalInputFace.AdvancedSearch.SearchOperator = filter["z_TanggalInputFace"];
                TanggalInputFace.AdvancedSearch.SearchCondition = filter["v_TanggalInputFace"];
                TanggalInputFace.AdvancedSearch.SearchValue2 = filter["y_TanggalInputFace"];
                TanggalInputFace.AdvancedSearch.SearchOperator2 = filter["w_TanggalInputFace"];
                TanggalInputFace.AdvancedSearch.Save();
            }

            // Field UserInputFace
            if (filter?.TryGetValue("x_UserInputFace", out sv) ?? false) {
                UserInputFace.AdvancedSearch.SearchValue = sv;
                UserInputFace.AdvancedSearch.SearchOperator = filter["z_UserInputFace"];
                UserInputFace.AdvancedSearch.SearchCondition = filter["v_UserInputFace"];
                UserInputFace.AdvancedSearch.SearchValue2 = filter["y_UserInputFace"];
                UserInputFace.AdvancedSearch.SearchOperator2 = filter["w_UserInputFace"];
                UserInputFace.AdvancedSearch.Save();
            }

            // Field AzurePersonId
            if (filter?.TryGetValue("x_AzurePersonId", out sv) ?? false) {
                AzurePersonId.AdvancedSearch.SearchValue = sv;
                AzurePersonId.AdvancedSearch.SearchOperator = filter["z_AzurePersonId"];
                AzurePersonId.AdvancedSearch.SearchCondition = filter["v_AzurePersonId"];
                AzurePersonId.AdvancedSearch.SearchValue2 = filter["y_AzurePersonId"];
                AzurePersonId.AdvancedSearch.SearchOperator2 = filter["w_AzurePersonId"];
                AzurePersonId.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, IdUser, def, false); // IdUser
            ...
            BuildSearchSql(ref where, NamaLengkap, def, true); // NamaLengkap
            BuildSearchSql(ref where, IdPosition, def, true); // IdPosition
            BuildSearchSql(ref where, Jabatan, def, true); // Jabatan
            BuildSearchSql(ref where, Face, def, true); // Face
            BuildSearchSql(ref where, TanggalInputFace, def, true); // TanggalInputFace
            BuildSearchSql(ref where, UserInputFace, def, true); // UserInputFace
            BuildSearchSql(ref where, AzurePersonId, def, false); // AzurePersonId

            // Set up search command
            if (!def && !Empty(where) && (new[] { "", "reset", "resetall" }).Contains(Command))
                Command = "search";
            if (!def && Command == "search") {
                IdUser.AdvancedSearch.Save(); // IdUser
                _Username.AdvancedSearch.Save(); // Username
                _Email.AdvancedSearch.Save(); // Email
                NamaLengkap.AdvancedSearch.Save(); // NamaLengkap
                IdPosition.AdvancedSearch.Save(); // IdPosition
                Jabatan.AdvancedSearch.Save(); // Jabatan
                Face.AdvancedSearch.Save(); // Face
                TanggalInputFace.AdvancedSearch.Save(); // TanggalInputFace
                UserInputFace.AdvancedSearch.Save(); // UserInputFace
                AzurePersonId.AdvancedSearch.Save(); // AzurePersonId
        ...
        public void ShowFilterList()
        {
            ...
            // Field NamaLengkap
            filter = QueryBuilderWhere("NamaLengkap");
            if (Empty(filter))
                BuildSearchSql(ref filter, NamaLengkap, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NamaLengkap.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field Face
            filter = QueryBuilderWhere("Face");
            if (Empty(filter))
                BuildSearchSql(ref filter, Face, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Face.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field TanggalInputFace
            filter = QueryBuilderWhere("TanggalInputFace");
            if (Empty(filter))
                BuildSearchSql(ref filter, TanggalInputFace, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TanggalInputFace.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field UserInputFace
            filter = QueryBuilderWhere("UserInputFace");
            if (Empty(filter))
                BuildSearchSql(ref filter, UserInputFace, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + UserInputFace.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field AzurePersonId
            filter = QueryBuilderWhere("AzurePersonId");
            if (Empty(filter))
                BuildSearchSql(ref filter, AzurePersonId, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + AzurePersonId.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NamaLengkap);
            searchFlds.Add(DownloadFace);
            searchFlds.Add(IdPosition);
            searchFlds.Add(Jabatan);
            searchFlds.Add(Face);
            searchFlds.Add(TanggalInputFace);
            searchFlds.Add(UserInputFace);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (IdUser.AdvancedSearch.IssetSession)
                return true;
            ...
            if (_Email.AdvancedSearch.IssetSession)
                return true;
            if (NamaLengkap.AdvancedSearch.IssetSession)
                return true;
            if (IdPosition.AdvancedSearch.IssetSession)
                return true;
            if (Jabatan.AdvancedSearch.IssetSession)
                return true;
            if (Face.AdvancedSearch.IssetSession)
                return true;
            if (TanggalInputFace.AdvancedSearch.IssetSession)
                return true;
            if (UserInputFace.AdvancedSearch.IssetSession)
                return true;
            if (AzurePersonId.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            IdUser.AdvancedSearch.UnsetSession();
            _Username.AdvancedSearch.UnsetSession();
            LinkRedirect.AdvancedSearch.UnsetSession();
            _Email.AdvancedSearch.UnsetSession();
            NamaLengkap.AdvancedSearch.UnsetSession();
            IdPosition.AdvancedSearch.UnsetSession();
            Jabatan.AdvancedSearch.UnsetSession();
            Face.AdvancedSearch.UnsetSession();
            TanggalInputFace.AdvancedSearch.UnsetSession();
            UserInputFace.AdvancedSearch.UnsetSession();
            AzurePersonId.AdvancedSearch.UnsetSession();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(NamaLengkap, ctrl); // NamaLengkap
                UpdateSort(DownloadFace, ctrl); // DownloadFace
                UpdateSort(IdPosition, ctrl); // IdPosition
                UpdateSort(Jabatan, ctrl); // Jabatan
                UpdateSort(Face, ctrl); // Face
                UpdateSort(TanggalInputFace, ctrl); // TanggalInputFace
                UpdateSort(UserInputFace, ctrl); // UserInputFace
                UpdateSort(AzurePersonId, ctrl); // AzurePersonId
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""VFaceEnrollment"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fVFaceEnrollmentlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fVFaceEnrollmentlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdUser.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("NamaLengkap")); // DN
                CreateColumnOption(option.Add("DownloadFace")); // DN
                CreateColumnOption(option.Add("IdPosition")); // DN
                CreateColumnOption(option.Add("Jabatan")); // DN
                CreateColumnOption(option.Add("Face")); // DN
                CreateColumnOption(option.Add("TanggalInputFace")); // DN
                CreateColumnOption(option.Add("UserInputFace")); // DN
                CreateColumnOption(option.Add("AzurePersonId")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fVFaceEnrollmentsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fVFaceEnrollmentsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fVFaceEnrollmentlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_VFaceEnrollment");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(vFaceEnrollmentList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(vFaceEnrollmentList.RowCount) + "_VFaceEnrollment");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(vFaceEnrollmentList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && vFaceEnrollmentList.RowType == RowType.Add || IsEdit && vFaceEnrollmentList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // IdUser
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdUser"))
                    IdUser.AdvancedSearch.SearchValue = Get("x_IdUser");
                else
                    IdUser.AdvancedSearch.SearchValue = Get("IdUser"); // Default Value // DN
            if (!Empty(IdUser.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_IdUser"))
                IdUser.AdvancedSearch.SearchOperator = Get("z_IdUser");

            // _Username
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x__Username[]"))
                    _Username.AdvancedSearch.SearchValue = Get("x__Username[]", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z__Username"))
                _Username.AdvancedSearch.SearchOperator = Get("z__Username", Config.FilterOptionSeparator);
            ...
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x__Email[]"))
                    _Email.AdvancedSearch.SearchValue = Get("x__Email[]", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z__Email"))
                _Email.AdvancedSearch.SearchOperator = Get("z__Email", Config.FilterOptionSeparator);

            // NamaLengkap
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaLengkap[]"))
                    NamaLengkap.AdvancedSearch.SearchValue = Get("x_NamaLengkap[]", Config.FilterOptionSeparator);
                else
                    NamaLengkap.AdvancedSearch.SearchValue = Get("NamaLengkap"); // Default Value // DN
            if (!Empty(NamaLengkap.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_NamaLengkap"))
                NamaLengkap.AdvancedSearch.SearchOperator = Get("z_NamaLengkap", Config.FilterOptionSeparator);
            ...
            // Face
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Face[]"))
                    Face.AdvancedSearch.SearchValue = Get("x_Face[]", Config.FilterOptionSeparator);
                else
                    Face.AdvancedSearch.SearchValue = Get("Face"); // Default Value // DN
            if (!Empty(Face.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Face"))
                Face.AdvancedSearch.SearchOperator = Get("z_Face", Config.FilterOptionSeparator);

            // TanggalInputFace
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TanggalInputFace[]"))
                    TanggalInputFace.AdvancedSearch.SearchValue = Get("x_TanggalInputFace[]", Config.FilterOptionSeparator);
                else
                    TanggalInputFace.AdvancedSearch.SearchValue = Get("TanggalInputFace"); // Default Value // DN
            if (!Empty(TanggalInputFace.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_TanggalInputFace"))
                TanggalInputFace.AdvancedSearch.SearchOperator = Get("z_TanggalInputFace", Config.FilterOptionSeparator);

            // UserInputFace
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_UserInputFace[]"))
                    UserInputFace.AdvancedSearch.SearchValue = Get("x_UserInputFace[]", Config.FilterOptionSeparator);
                else
                    UserInputFace.AdvancedSearch.SearchValue = Get("UserInputFace"); // Default Value // DN
            if (!Empty(UserInputFace.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_UserInputFace"))
                UserInputFace.AdvancedSearch.SearchOperator = Get("z_UserInputFace", Config.FilterOptionSeparator);

            // AzurePersonId
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_AzurePersonId"))
                    AzurePersonId.AdvancedSearch.SearchValue = Get("x_AzurePersonId", Config.FilterOptionSeparator);
                else
                    AzurePersonId.AdvancedSearch.SearchValue = Get("AzurePersonId"); // Default Value // DN
            if (!Empty(AzurePersonId.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_AzurePersonId"))
                AzurePersonId.AdvancedSearch.SearchOperator = Get("z_AzurePersonId", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (_Email.UseFilter && !Empty(_Email.AdvancedSearch.SearchValue)) {
                    _Email.EditValue = ConvertToString(_Email.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // NamaLengkap
                if (NamaLengkap.UseFilter && !Empty(NamaLengkap.AdvancedSearch.SearchValue)) {
                    NamaLengkap.EditValue = ConvertToString(NamaLengkap.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // DownloadFace
                DownloadFace.SetupEditAttributes();
                DownloadFace.EditValue = DownloadFace.AdvancedSearch.SearchValue; // DN
                DownloadFace.PlaceHolder = RemoveHtml(DownloadFace.Caption);
                ...
                // Face
                if (Face.UseFilter && !Empty(Face.AdvancedSearch.SearchValue)) {
                    Face.EditValue = ConvertToString(Face.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // TanggalInputFace
                if (TanggalInputFace.UseFilter && !Empty(TanggalInputFace.AdvancedSearch.SearchValue)) {
                    TanggalInputFace.EditValue = ConvertToString(TanggalInputFace.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // UserInputFace
                if (UserInputFace.UseFilter && !Empty(UserInputFace.AdvancedSearch.SearchValue)) {
                    UserInputFace.EditValue = ConvertToString(UserInputFace.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fVFaceEnrollmentsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"VFaceEnrollment\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("VFaceEnrollmentSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("VFaceEnrollmentSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
