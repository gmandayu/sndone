...
public partial class SnDOne {
    /// <summary>
    /// subAkivitasNilaiAktualDischargePipaLpgList
    /// </summary>
    public static SubAkivitasNilaiAktualDischargePipaLpgList subAkivitasNilaiAktualDischargePipaLpgList
    {
        get => HttpData.Get<SubAkivitasNilaiAktualDischargePipaLpgList>("subAkivitasNilaiAktualDischargePipaLpgList")!;
        set => HttpData["subAkivitasNilaiAktualDischargePipaLpgList"] = value;
    }

    /// <summary>
    /// Page class for SubAkivitasNilaiAktualDischargePipaLPG
    /// </summary>
    public class SubAkivitasNilaiAktualDischargePipaLpgList : SubAkivitasNilaiAktualDischargePipaLpgListBase
    {
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaLpgList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAkivitasNilaiAktualDischargePipaLpgList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAkivitasNilaiAktualDischargePipaLpgListBase : SubAkivitasNilaiAktualDischargePipaLpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAkivitasNilaiAktualDischargePipaLpgList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAkivitasNilaiAktualDischargePipaLPGlist";
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaLpgListBase()
        {
            TableName = "SubAkivitasNilaiAktualDischargePipaLPG";

            // CSS class name as context
            TableVar = "SubAkivitasNilaiAktualDischargePipaLPG";
            ...
            // Table object (subAkivitasNilaiAktualDischargePipaLpg)
            if (subAkivitasNilaiAktualDischargePipaLpg == null || subAkivitasNilaiAktualDischargePipaLpg is SubAkivitasNilaiAktualDischargePipaLpg)
                subAkivitasNilaiAktualDischargePipaLpg = this;

            // Initialize URLs
            AddUrl = "SubAkivitasNilaiAktualDischargePipaLpgAdd";
            ...
            MultiDeleteUrl = "SubAkivitasNilaiAktualDischargePipaLpgDelete";
            MultiUpdateUrl = "SubAkivitasNilaiAktualDischargePipaLpgUpdate";
        ...
        // Page name
        public string PageName => "SubAkivitasNilaiAktualDischargePipaLpgList";
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaLpgListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAkivitasNilaiAktualDischargePipaLpgView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAkivitasNilaiAktualDischargePipaLPGgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAkivitasNilaiAktualDischargePipaLpgList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAkivitasNilaiAktualDischargePipaLPG"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAkivitasNilaiAktualDischargePipaLPGlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAkivitasNilaiAktualDischargePipaLPGlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAkivitasNilaiAktualDischargePipaLPGsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAkivitasNilaiAktualDischargePipaLPGsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAkivitasNilaiAktualDischargePipaLPGlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAkivitasNilaiAktualDischargePipaLPG");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAkivitasNilaiAktualDischargePipaLpgList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAkivitasNilaiAktualDischargePipaLpgList.RowCount) + "_SubAkivitasNilaiAktualDischargePipaLPG");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAkivitasNilaiAktualDischargePipaLpgList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAkivitasNilaiAktualDischargePipaLpgList.RowType == RowType.Add || IsEdit && subAkivitasNilaiAktualDischargePipaLpgList.RowType == RowType.Edit) // Inline-Add/Edit row
