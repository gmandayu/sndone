...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasCatatanKhususEdit
    /// </summary>
    public static SubAktivitasCatatanKhususEdit subAktivitasCatatanKhususEdit
    {
        get => HttpData.Get<SubAktivitasCatatanKhususEdit>("subAktivitasCatatanKhususEdit")!;
        set => HttpData["subAktivitasCatatanKhususEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasCatatanKhusus
    /// </summary>
    public class SubAktivitasCatatanKhususEdit : SubAktivitasCatatanKhususEditBase
    {
        // Constructor
        public SubAktivitasCatatanKhususEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasCatatanKhususEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasCatatanKhususEditBase : SubAktivitasCatatanKhusus
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasCatatanKhususEdit";
        ...
        // Constructor
        public SubAktivitasCatatanKhususEditBase()
        {
        ...
        // Page name
        public string PageName => "SubAktivitasCatatanKhususEdit";
        ...
        // Constructor
        public SubAktivitasCatatanKhususEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasCatatanKhususList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = ReturnUrl;
                    if (GetPageName(returnUrl) == "SubAktivitasCatatanKhususList")
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasCatatanKhususList") {
                                ...
                                returnUrl = "SubAktivitasCatatanKhususList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasCatatanKhususEdit?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'Kontaminasi' before field var 'x_Kontaminasi'
            val = CurrentForm.HasValue("Kontaminasi") ? CurrentForm.GetValue("Kontaminasi") : CurrentForm.GetValue("x_Kontaminasi");
            if (!Kontaminasi.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Kontaminasi") && !CurrentForm.HasValue("x_Kontaminasi")) // DN
                    Kontaminasi.Visible = false; // Disable update for API request
                else
                    Kontaminasi.SetFormValue(val);
            }

            // Check field name 'Insident' before field var 'x_Insident'
            val = CurrentForm.HasValue("Insident") ? CurrentForm.GetValue("Insident") : CurrentForm.GetValue("x_Insident");
            if (!Insident.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Insident") && !CurrentForm.HasValue("x_Insident")) // DN
                    Insident.Visible = false; // Disable update for API request
                else
                    Insident.SetFormValue(val);
            }

            // Check field name 'CuacaBuruk' before field var 'x_CuacaBuruk'
            val = CurrentForm.HasValue("CuacaBuruk") ? CurrentForm.GetValue("CuacaBuruk") : CurrentForm.GetValue("x_CuacaBuruk");
            if (!CuacaBuruk.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("CuacaBuruk") && !CurrentForm.HasValue("x_CuacaBuruk")) // DN
                    CuacaBuruk.Visible = false; // Disable update for API request
                else
                    CuacaBuruk.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            Kontaminasi.CurrentValue = Kontaminasi.FormValue;
            Insident.CurrentValue = Insident.FormValue;
            CuacaBuruk.CurrentValue = CuacaBuruk.FormValue;
        ...
        public async Task RenderRow()
        {
            ...
            // Kontaminasi
            Kontaminasi.RowCssClass = "row";

            // Insident
            Insident.RowCssClass = "row";

            // CuacaBuruk
            CuacaBuruk.RowCssClass = "row";
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                if (Kontaminasi.Visible && Kontaminasi.Required) {
                    if (!Kontaminasi.IsDetailKey && Empty(Kontaminasi.FormValue)) {
                        Kontaminasi.AddErrorMessage(ConvertToString(Kontaminasi.RequiredErrorMessage).Replace("%s", Kontaminasi.Caption));
                    }
                }
                if (Insident.Visible && Insident.Required) {
                    if (!Insident.IsDetailKey && Empty(Insident.FormValue)) {
                        Insident.AddErrorMessage(ConvertToString(Insident.RequiredErrorMessage).Replace("%s", Insident.Caption));
                    }
                }
                if (CuacaBuruk.Visible && CuacaBuruk.Required) {
                    if (!CuacaBuruk.IsDetailKey && Empty(CuacaBuruk.FormValue)) {
                        CuacaBuruk.AddErrorMessage(ConvertToString(CuacaBuruk.RequiredErrorMessage).Replace("%s", CuacaBuruk.Caption));
                    }
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // Kontaminasi
            Kontaminasi.SetDbValue(rsnew, Kontaminasi.CurrentValue, Kontaminasi.ReadOnly);

            // Insident
            Insident.SetDbValue(rsnew, Insident.CurrentValue, Insident.ReadOnly);

            // CuacaBuruk
            CuacaBuruk.SetDbValue(rsnew, CuacaBuruk.CurrentValue, CuacaBuruk.ReadOnly);
        ...
        {
            object? value;
            if (row.TryGetValue("Kontaminasi", out value)) // Kontaminasi
                Kontaminasi.CurrentValue = value;
            if (row.TryGetValue("Insident", out value)) // Insident
                Insident.CurrentValue = value;
            if (row.TryGetValue("CuacaBuruk", out value)) // CuacaBuruk
                CuacaBuruk.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasCatatanKhususList")), "", TableVar, true);
            string pageId = "edit";
