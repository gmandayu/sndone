...
public partial class SnDOne {
    ...
    /// masterProdukList
    /// </summary>
    public static MasterProdukList masterProdukList
    {
        get => HttpData.Get<MasterProdukList>("masterProdukList")!;
        set => HttpData["masterProdukList"] = value;
    }

    /// <summary>
    /// Page class for MasterProduk
    /// </summary>
    public class MasterProdukList : MasterProdukListBase
    {
        // Constructor
        public MasterProdukList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterProdukList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterProdukListBase : MasterProduk
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterProdukList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMasterProduklist";
        ...
        // Constructor
        public MasterProdukListBase(Controller? controller)
        {
            TableName = "MasterProduk";

            // CSS class name as context
            TableVar = "MasterProduk";
        ...
        if (controller != null)
            ...
            MultiDeleteUrl = "MasterProdukDelete";
            MultiUpdateUrl = "MasterProdukUpdate";
        ...
        // Page name
        public string PageName => "MasterProdukList";
        ...
        // Constructor
        public MasterProdukListBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, NoProduk, def, true); // NoProduk
            BuildSearchSql(ref where, NamaProduk, def, true); // NamaProduk
            BuildSearchSql(ref where, TipeProduk, def, true); // TipeProduk
            BuildSearchSql(ref where, DensityMin, def, true); // DensityMin
            BuildSearchSql(ref where, DensityMax, def, true); // DensityMax
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("NoProduk", NoProduk, true, config);
            filterList += ProcessFieldFilter("NamaProduk", NamaProduk, true, config);
            filterList += ProcessFieldFilter("TipeProduk", TipeProduk, true, config);
            filterList += ProcessFieldFilter("DensityMin", DensityMin, true, config);
            filterList += ProcessFieldFilter("DensityMax", DensityMax, true, config);
            return filterList;
        }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NoProduk);
            searchFlds.Add(NamaProduk);
            searchFlds.Add(TipeProduk);
            searchFlds.Add(DensityMin);
            searchFlds.Add(DensityMax);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (NoProduk.AdvancedSearch.IssetSession)
                return true;
            if (NamaProduk.AdvancedSearch.IssetSession)
                return true;
            if (TipeProduk.AdvancedSearch.IssetSession)
                return true;
            if (DensityMin.AdvancedSearch.IssetSession)
                return true;
            if (DensityMax.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            NoProduk.AdvancedSearch.UnsetSession();
            NamaProduk.AdvancedSearch.UnsetSession();
            TipeProduk.AdvancedSearch.UnsetSession();
            DensityMin.AdvancedSearch.UnsetSession();
            DensityMax.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            NoProduk.AdvancedSearch.Load();
            NamaProduk.AdvancedSearch.Load();
            TipeProduk.AdvancedSearch.Load();
            DensityMin.AdvancedSearch.Load();
            DensityMax.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(NoProduk, ctrl); // NoProduk
                UpdateSort(NamaProduk, ctrl); // NamaProduk
                UpdateSort(TipeProduk, ctrl); // TipeProduk
                UpdateSort(DensityMin, ctrl); // DensityMin
                UpdateSort(DensityMax, ctrl); // DensityMax
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MasterProduk"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MasterProduk"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MasterProduk"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMasterProduklist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMasterProduklist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(NoProduk.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MasterProduk"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMasterProduklist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("NoProduk")); // DN
                CreateColumnOption(option.Add("NamaProduk")); // DN
                CreateColumnOption(option.Add("TipeProduk")); // DN
                CreateColumnOption(option.Add("DensityMin")); // DN
                CreateColumnOption(option.Add("DensityMax")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMasterProduksrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMasterProduksrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMasterProduklist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_MasterProduk");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(masterProdukList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(masterProdukList.RowCount) + "_MasterProduk");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(masterProdukList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && masterProdukList.RowType == RowType.Add || IsEdit && masterProdukList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadFieldNoProdukSearchValues();
            LoadFieldNamaProdukSearchValues();
            LoadFieldTipeProdukSearchValues();
            LoadFieldDensityMinSearchValues();
            LoadFieldDensityMaxSearchValues();
        }

        private void LoadFieldNoProdukSearchValues()
        {
            // NoProduk
            LoadPrimarySearchValueNoProduk();
            LoadSearchOperatorNoProduk();
        }

        private void LoadPrimarySearchValueNoProduk()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NoProduk[]")) {
                NoProduk.AdvancedSearch.SearchValue = Get("x_NoProduk[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NoProduk.AdvancedSearch.SearchValue = Get("NoProduk"); // Default Value // DN
            }
            if (!Empty(NoProduk.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNoProduk()
        {
            if (Query.ContainsKey("z_NoProduk"))
                NoProduk.AdvancedSearch.SearchOperator = Get("z_NoProduk", Config.FilterOptionSeparator);
        }

        private void LoadFieldNamaProdukSearchValues()
        {
            // NamaProduk
            LoadPrimarySearchValueNamaProduk();
            LoadSearchOperatorNamaProduk();
        }

        private void LoadPrimarySearchValueNamaProduk()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaProduk[]")) {
                NamaProduk.AdvancedSearch.SearchValue = Get("x_NamaProduk[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NamaProduk.AdvancedSearch.SearchValue = Get("NamaProduk"); // Default Value // DN
            }
            if (!Empty(NamaProduk.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNamaProduk()
        {
            if (Query.ContainsKey("z_NamaProduk"))
                NamaProduk.AdvancedSearch.SearchOperator = Get("z_NamaProduk", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldDensityMinSearchValues()
        {
            // DensityMin
            LoadPrimarySearchValueDensityMin();
            LoadSearchOperatorDensityMin();
        }

        private void LoadPrimarySearchValueDensityMin()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_DensityMin[]")) {
                DensityMin.AdvancedSearch.SearchValue = Get("x_DensityMin[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                DensityMin.AdvancedSearch.SearchValue = Get("DensityMin"); // Default Value // DN
            }
            if (!Empty(DensityMin.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorDensityMin()
        {
            if (Query.ContainsKey("z_DensityMin"))
                DensityMin.AdvancedSearch.SearchOperator = Get("z_DensityMin", Config.FilterOptionSeparator);
        }

        private void LoadFieldDensityMaxSearchValues()
        {
            // DensityMax
            LoadPrimarySearchValueDensityMax();
            LoadSearchOperatorDensityMax();
        }

        private void LoadPrimarySearchValueDensityMax()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_DensityMax[]")) {
                DensityMax.AdvancedSearch.SearchValue = Get("x_DensityMax[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                DensityMax.AdvancedSearch.SearchValue = Get("DensityMax"); // Default Value // DN
            }
            if (!Empty(DensityMax.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorDensityMax()
        {
            if (Query.ContainsKey("z_DensityMax"))
                DensityMax.AdvancedSearch.SearchOperator = Get("z_DensityMax", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // NoProduk
                NoProduk.HrefValue = "";
                NoProduk.TooltipValue = "";

                // NamaProduk
                NamaProduk.HrefValue = "";
                NamaProduk.TooltipValue = "";
                ...
                DensityMin.TooltipValue = "";

                // DensityMax
                DensityMax.HrefValue = "";
                DensityMax.TooltipValue = "";
            } else if (RowType == RowType.Search) {
                // NoProduk
                if (NoProduk.UseFilter && !Empty(NoProduk.AdvancedSearch.SearchValue)) {
                    NoProduk.EditValue = ConvertToString(NoProduk.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // NamaProduk
                if (NamaProduk.UseFilter && !Empty(NamaProduk.AdvancedSearch.SearchValue)) {
                    NamaProduk.EditValue = ConvertToString(NamaProduk.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // DensityMin
                if (DensityMin.UseFilter && !Empty(DensityMin.AdvancedSearch.SearchValue)) {
                    DensityMin.EditValue = ConvertToString(DensityMin.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // DensityMax
                if (DensityMax.UseFilter && !Empty(DensityMax.AdvancedSearch.SearchValue)) {
                    DensityMax.EditValue = ConvertToString(DensityMax.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMasterProduksrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
