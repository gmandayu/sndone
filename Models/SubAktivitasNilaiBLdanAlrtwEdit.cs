...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiBLdanAlrtwEdit
    /// </summary>
    public static SubAktivitasNilaiBLdanAlrtwEdit subAktivitasNilaiBLdanAlrtwEdit
    {
        get => HttpData.Get<SubAktivitasNilaiBLdanAlrtwEdit>("subAktivitasNilaiBLdanAlrtwEdit")!;
        set => HttpData["subAktivitasNilaiBLdanAlrtwEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiBLdanALRTW
    /// </summary>
    public class SubAktivitasNilaiBLdanAlrtwEdit : SubAktivitasNilaiBLdanAlrtwEditBase
    {
        // Constructor
        public SubAktivitasNilaiBLdanAlrtwEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiBLdanAlrtwEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiBLdanAlrtwEditBase : SubAktivitasNilaiBLdanAlrtw
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiBLdanAlrtwEdit";
        ...
        // Constructor
        public SubAktivitasNilaiBLdanAlrtwEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiBLdanAlrtwEdit";
        ...
        // Constructor
        public SubAktivitasNilaiBLdanAlrtwEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasNilaiBLdanAlrtwList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasNilaiBLdanAlrtwList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasNilaiBLdanAlrtwList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasNilaiBLdanAlrtwEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'BL_KL_Obs'
            BL_KL_Obs.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(BL_KL_Obs, "BL_KL_Obs", "x_BL_KL_Obs");

            // Standard handling for 'BL_KL_15'
            BL_KL_15.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(BL_KL_15, "BL_KL_15", "x_BL_KL_15");
            ...
            // Standard handling for 'AL_KL_Obs'
            AL_KL_Obs.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AL_KL_Obs, "AL_KL_Obs", "x_AL_KL_Obs");

            // Standard handling for 'AL_KL_15'
            AL_KL_15.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AL_KL_15, "AL_KL_15", "x_AL_KL_15");

            // Standard handling for 'AL_Barrels'
            AL_Barrels.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AL_Barrels, "AL_Barrels", "x_AL_Barrels");

            // Standard handling for 'AL_LT'
            AL_LT.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AL_LT, "AL_LT", "x_AL_LT");

            // Standard handling for 'AL_MT'
            AL_MT.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(AL_MT, "AL_MT", "x_AL_MT");
        ...
        public void RestoreFormValues()
        {
            ...
            BL_KL_Obs.CurrentValue = BL_KL_Obs.FormValue;
            BL_KL_15.CurrentValue = BL_KL_15.FormValue;
            BL_Barrels.CurrentValue = BL_Barrels.FormValue;
            BL_LT.CurrentValue = BL_LT.FormValue;
            BL_MT.CurrentValue = BL_MT.FormValue;
            AL_KL_Obs.CurrentValue = AL_KL_Obs.FormValue;
            AL_KL_15.CurrentValue = AL_KL_15.FormValue;
        ...
        public async Task RenderRow()
        {
            ...
            // BL_KL_Obs
            BL_KL_Obs.RowCssClass = "row";

            // BL_KL_15
            BL_KL_15.RowCssClass = "row";
            ...
            // AL_KL_Obs
            AL_KL_Obs.RowCssClass = "row";

            // AL_KL_15
            AL_KL_15.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                BL_KL_Obs.EditValue = HtmlEncode(BL_KL_Obs.CurrentValue);
                ...
                BL_KL_15.EditValue = HtmlEncode(BL_KL_15.CurrentValue);
                ...
                AL_KL_Obs.EditValue = HtmlEncode(AL_KL_Obs.CurrentValue);
                ...
                AL_KL_15.EditValue = HtmlEncode(AL_KL_15.CurrentValue);
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }

        private void ValidateCustomBL_KL_Obs() {
            if (BL_KL_Obs.Visible && BL_KL_Obs.Required) {
                if (!BL_KL_Obs.IsDetailKey && Empty(BL_KL_Obs.FormValue)) {
                    BL_KL_Obs.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }

        private void ValidateCustomBL_KL_15() {
            if (BL_KL_15.Visible && BL_KL_15.Required) {
                if (!BL_KL_15.IsDetailKey && Empty(BL_KL_15.FormValue)) {
                    BL_KL_15.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_Barrels() {
            if (BL_Barrels.Visible && BL_Barrels.Required) {
                if (!BL_Barrels.IsDetailKey && Empty(BL_Barrels.FormValue)) {
                    BL_Barrels.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_LT() {
            if (BL_LT.Visible && BL_LT.Required) {
                if (!BL_LT.IsDetailKey && Empty(BL_LT.FormValue)) {
                    BL_LT.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_MT() {
            if (BL_MT.Visible && BL_MT.Required) {
                if (!BL_MT.IsDetailKey && Empty(BL_MT.FormValue)) {
                    BL_MT.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }

        private void ValidateCustomAL_KL_Obs() {
            if (AL_KL_Obs.Visible && AL_KL_Obs.Required) {
                if (!AL_KL_Obs.IsDetailKey && Empty(AL_KL_Obs.FormValue)) {
                    AL_KL_Obs.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }

        private void ValidateCustomAL_KL_15() {
            if (AL_KL_15.Visible && AL_KL_15.Required) {
                if (!AL_KL_15.IsDetailKey && Empty(AL_KL_15.FormValue)) {
                    AL_KL_15.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }

        private void ValidateCustomAL_Barrels() {
            if (AL_Barrels.Visible && AL_Barrels.Required) {
                if (!AL_Barrels.IsDetailKey && Empty(AL_Barrels.FormValue)) {
                    AL_Barrels.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }

        private void ValidateCustomAL_LT() {
            if (AL_LT.Visible && AL_LT.Required) {
                if (!AL_LT.IsDetailKey && Empty(AL_LT.FormValue)) {
                    AL_LT.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }

        private void ValidateCustomAL_MT() {
            if (AL_MT.Visible && AL_MT.Required) {
                if (!AL_MT.IsDetailKey && Empty(AL_MT.FormValue)) {
                    AL_MT.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomBL_KL_Obs();
                ValidateCustomBL_KL_15();
                ValidateCustomBL_Barrels();
                ValidateCustomBL_LT();
                ValidateCustomBL_MT();
                ValidateCustomAL_KL_Obs();
                ValidateCustomAL_KL_15();
                ValidateCustomAL_Barrels();
                ValidateCustomAL_LT();
                ValidateCustomAL_MT();
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // BL_KL_Obs
            BL_KL_Obs.SetDbValue(rsnew, BL_KL_Obs.CurrentValue, BL_KL_Obs.ReadOnly);

            // BL_KL_15
            BL_KL_15.SetDbValue(rsnew, BL_KL_15.CurrentValue, BL_KL_15.ReadOnly);
            ...
            // AL_KL_Obs
            AL_KL_Obs.SetDbValue(rsnew, AL_KL_Obs.CurrentValue, AL_KL_Obs.ReadOnly);

            // AL_KL_15
            AL_KL_15.SetDbValue(rsnew, AL_KL_15.CurrentValue, AL_KL_15.ReadOnly);
        ...
        {
            object? value;
            if (row.TryGetValue("BL_KL_Obs", out value)) // BL_KL_Obs
                BL_KL_Obs.CurrentValue = value;
            if (row.TryGetValue("BL_KL_15", out value)) // BL_KL_15
                BL_KL_15.CurrentValue = value;
            ...
            if (row.TryGetValue("AL_KL_Obs", out value)) // AL_KL_Obs
                AL_KL_Obs.CurrentValue = value;
            if (row.TryGetValue("AL_KL_15", out value)) // AL_KL_15
                AL_KL_15.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasNilaiBLdanAlrtwList")), "", TableVar, true);
            string pageId = "edit";
