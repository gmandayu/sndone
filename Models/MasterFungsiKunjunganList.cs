...
public partial class SnDOne {
    /// <summary>
    /// masterFungsiKunjunganList
    /// </summary>
    public static MasterFungsiKunjunganList masterFungsiKunjunganList
    {
        get => HttpData.Get<MasterFungsiKunjunganList>("masterFungsiKunjunganList")!;
        set => HttpData["masterFungsiKunjunganList"] = value;
    }

    /// <summary>
    /// Page class for MasterFungsiKunjungan
    /// </summary>
    public class MasterFungsiKunjunganList : MasterFungsiKunjunganListBase
    {
        // Constructor
        public MasterFungsiKunjunganList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterFungsiKunjunganList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterFungsiKunjunganListBase : MasterFungsiKunjungan
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterFungsiKunjunganList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMasterFungsiKunjunganlist";
        ...
        // Constructor
        public MasterFungsiKunjunganListBase()
        {
            TableName = "MasterFungsiKunjungan";

            // CSS class name as context
            TableVar = "MasterFungsiKunjungan";
            ...
            // Table object (masterFungsiKunjungan)
            if (masterFungsiKunjungan == null || masterFungsiKunjungan is MasterFungsiKunjungan)
                masterFungsiKunjungan = this;

            // Initialize URLs
            AddUrl = "MasterFungsiKunjunganAdd";
            ...
            MultiDeleteUrl = "MasterFungsiKunjunganDelete";
            MultiUpdateUrl = "MasterFungsiKunjunganUpdate";
        ...
        // Page name
        public string PageName => "MasterFungsiKunjunganList";
        ...
        {
            ID.SetVisibility();
            FungsiKunjungan.SetVisibility();
        }

        // Constructor
        public MasterFungsiKunjunganListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "MasterFungsiKunjunganView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fMasterFungsiKunjungangrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                masterFungsiKunjunganList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(FungsiKunjungan);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(FungsiKunjungan, ctrl); // FungsiKunjungan
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MasterFungsiKunjungan"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MasterFungsiKunjungan"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MasterFungsiKunjungan"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMasterFungsiKunjunganlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMasterFungsiKunjunganlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MasterFungsiKunjungan"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMasterFungsiKunjunganlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("FungsiKunjungan")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMasterFungsiKunjungansrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMasterFungsiKunjungansrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMasterFungsiKunjunganlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MasterFungsiKunjungan");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(masterFungsiKunjunganList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(masterFungsiKunjunganList.RowCount) + "_MasterFungsiKunjungan");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(masterFungsiKunjunganList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && masterFungsiKunjunganList.RowType == RowType.Add || IsEdit && masterFungsiKunjunganList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            FungsiKunjungan.SetDbValue(row["FungsiKunjungan"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("FungsiKunjungan", FungsiKunjungan.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // FungsiKunjungan
                FungsiKunjungan.ViewValue = ConvertToString(FungsiKunjungan.CurrentValue); // DN
                FungsiKunjungan.ViewCustomAttributes = "";
                ...
                // FungsiKunjungan
                FungsiKunjungan.HrefValue = "";
                FungsiKunjungan.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMasterFungsiKunjungansrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
