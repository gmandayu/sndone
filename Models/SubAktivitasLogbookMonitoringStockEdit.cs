...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasLogbookMonitoringStockEdit
    /// </summary>
    public static SubAktivitasLogbookMonitoringStockEdit subAktivitasLogbookMonitoringStockEdit
    {
        get => HttpData.Get<SubAktivitasLogbookMonitoringStockEdit>("subAktivitasLogbookMonitoringStockEdit")!;
        set => HttpData["subAktivitasLogbookMonitoringStockEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasLogbookMonitoringStock
    /// </summary>
    public class SubAktivitasLogbookMonitoringStockEdit : SubAktivitasLogbookMonitoringStockEditBase
    {
        // Constructor
        public SubAktivitasLogbookMonitoringStockEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasLogbookMonitoringStockEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasLogbookMonitoringStockEditBase : SubAktivitasLogbookMonitoringStock
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasLogbookMonitoringStockEdit";
        ...
        // Constructor
        public SubAktivitasLogbookMonitoringStockEditBase(Controller? controller)
        {
            TableName = "SubAktivitasLogbookMonitoringStock";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasLogbookMonitoringStock)
            if (subAktivitasLogbookMonitoringStock == null || subAktivitasLogbookMonitoringStock is SubAktivitasLogbookMonitoringStock)
                subAktivitasLogbookMonitoringStock = this;
        ...
        // Page name
        public string PageName => "SubAktivitasLogbookMonitoringStockEdit";
        ...
        // Constructor
        public SubAktivitasLogbookMonitoringStockEditBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                ...
                // List / View / Master View page
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasLogbookMonitoringStockView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasLogbookMonitoringStockList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasLogbookMonitoringStockList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasLogbookMonitoringStockList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasLogbookMonitoringStockEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'Nomor_Tangki'
            Nomor_Tangki.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Nomor_Tangki, "Nomor_Tangki", "x_Nomor_Tangki");
            ...
            // Standard handling for 'Level_Tangki'
            Level_Tangki.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Level_Tangki, "Level_Tangki", "x_Level_Tangki");

            // Standard handling for 'Volume_KLObs'
            Volume_KLObs.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Volume_KLObs, "Volume_KLObs", "x_Volume_KLObs");
        ...
        public void RestoreFormValues()
        {
            ...
            Nomor_Tangki.CurrentValue = Nomor_Tangki.FormValue;
            ...
            Level_Tangki.CurrentValue = Level_Tangki.FormValue;
            Volume_KLObs.CurrentValue = Volume_KLObs.FormValue;
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            Volume_KLObs.SetDbValue(row["Volume_KLObs"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Volume_KLObs", Volume_KLObs.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // Level_Tangki
            Level_Tangki.RowCssClass = "row";

            // Volume_KLObs
            Volume_KLObs.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                // IdPlant
                    ...
                    // Volume_KLObs
                    Volume_KLObs.ViewValue = ConvertToString(Volume_KLObs.CurrentValue); // DN
                    Volume_KLObs.ViewCustomAttributes = "";
                ...
                // Nomor_Tangki
                Nomor_Tangki.SetupEditAttributes();
                if (!Nomor_Tangki.Raw)
                    Nomor_Tangki.CurrentValue = HtmlDecode(Nomor_Tangki.CurrentValue);
                Nomor_Tangki.EditValue = HtmlEncode(Nomor_Tangki.CurrentValue);
                Nomor_Tangki.PlaceHolder = RemoveHtml(Nomor_Tangki.Caption);
                ...
                StatusAktivitas.EditValue = Empty(curVal) ? DbNullValue : FormatNumber(StatusAktivitas.CurrentValue, StatusAktivitas.FormatPattern);
                if (!Empty(curVal)) {
                    if (StatusAktivitas.Lookup != null && IsDictionary(StatusAktivitas.Lookup?.Options) && StatusAktivitas.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        StatusAktivitas.EditValue = StatusAktivitas.LookupCacheOption(curVal);
                ...
                // Level_Tangki
                Level_Tangki.SetupEditAttributes();
                if (!Level_Tangki.Raw)
                    Level_Tangki.CurrentValue = HtmlDecode(Level_Tangki.CurrentValue);
                Level_Tangki.EditValue = HtmlEncode(Level_Tangki.CurrentValue);
                Level_Tangki.PlaceHolder = RemoveHtml(Level_Tangki.Caption);

                // Volume_KLObs
                Volume_KLObs.SetupEditAttributes();
                if (!Volume_KLObs.Raw)
                    Volume_KLObs.CurrentValue = HtmlDecode(Volume_KLObs.CurrentValue);
                Volume_KLObs.EditValue = HtmlEncode(Volume_KLObs.CurrentValue);
                Volume_KLObs.PlaceHolder = RemoveHtml(Volume_KLObs.Caption);
        ...
        private void ValidateCustomid() {
            if (id.Visible && id.Required) {
                if (!id.IsDetailKey && Empty(id.FormValue)) {
                    id.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }

        private void ValidateCustomNomor_Tangki() {
            if (Nomor_Tangki.Visible && Nomor_Tangki.Required) {
                if (!Nomor_Tangki.IsDetailKey && Empty(Nomor_Tangki.FormValue)) {
                    Nomor_Tangki.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustomProduk() {
            if (Produk.Visible && Produk.Required) {
                if (!Produk.IsDetailKey && Empty(Produk.FormValue)) {
                    Produk.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustomAktivitas2() {
            if (Aktivitas2.Visible && Aktivitas2.Required) {
                if (!Aktivitas2.IsDetailKey && Empty(Aktivitas2.FormValue)) {
                    Aktivitas2.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }

        private void ValidateCustomLevel_Tangki() {
            if (Level_Tangki.Visible && Level_Tangki.Required) {
                if (!Level_Tangki.IsDetailKey && Empty(Level_Tangki.FormValue)) {
                    Level_Tangki.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }

        private void ValidateCustomVolume_KLObs() {
            if (Volume_KLObs.Visible && Volume_KLObs.Required) {
                if (!Volume_KLObs.IsDetailKey && Empty(Volume_KLObs.FormValue)) {
                    Volume_KLObs.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustomuserInput() {
            if (userInput.Visible && userInput.Required) {
                if (!userInput.IsDetailKey && Empty(userInput.FormValue)) {
                    userInput.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustometlDate() {
            if (etlDate.Visible && etlDate.Required) {
                if (!etlDate.IsDetailKey && Empty(etlDate.FormValue)) {
                    etlDate.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomNomor_Tangki();
                ValidateCustomProduk();
                ValidateCustomStatusAktivitas();
                ValidateCustomAktivitas2();
                ValidateCustomLevel_Tangki();
                ValidateCustomVolume_KLObs();
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // Nomor_Tangki
            Nomor_Tangki.SetDbValue(rsnew, Nomor_Tangki.CurrentValue, Nomor_Tangki.ReadOnly);
            ...
            // Level_Tangki
            Level_Tangki.SetDbValue(rsnew, Level_Tangki.CurrentValue, Level_Tangki.ReadOnly);

            // Volume_KLObs
            Volume_KLObs.SetDbValue(rsnew, Volume_KLObs.CurrentValue, Volume_KLObs.ReadOnly);
            ...
            // IdPlant
            IdPlant.SetDbValue(rsnew, IdPlant.CurrentValue, IdPlant.ReadOnly);
            return rsnew;
        }
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("Nomor_Tangki", out value)) // Nomor_Tangki
                Nomor_Tangki.CurrentValue = value;
            ...
            if (row.TryGetValue("Level_Tangki", out value)) // Level_Tangki
                Level_Tangki.CurrentValue = value;
            if (row.TryGetValue("Volume_KLObs", out value)) // Volume_KLObs
                Volume_KLObs.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasLogbookMonitoringStockList")), "", TableVar, true);
            string pageId = "edit";
