...
public partial class SnDOne {
    ...
    /// mwtOnlineList
    /// </summary>
    public static MwtOnlineList mwtOnlineList
    {
        get => HttpData.Get<MwtOnlineList>("mwtOnlineList")!;
        set => HttpData["mwtOnlineList"] = value;
    }

    /// <summary>
    /// Page class for MWTOnline
    /// </summary>
    public class MwtOnlineList : MwtOnlineListBase
    {
        // Constructor
        public MwtOnlineList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MwtOnlineList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class MwtOnlineListBase : MwtOnline
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mwtOnlineList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMWTOnlinelist";
        ...
        public MwtOnlineListBase()
        {
            ...
            MultiDeleteUrl = "MwtOnlineDelete";
            MultiUpdateUrl = "MwtOnlineUpdate";
        ...
        // Page name
        public string PageName => "MwtOnlineList";
        ...
        // Constructor
        public MwtOnlineListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(NoMWTOnline.AdvancedSearch.ToJson())); // Field NoMWTOnline
            filters.Merge(JObject.Parse(LinkRedirect.AdvancedSearch.ToJson())); // Field LinkRedirect
            filters.Merge(JObject.Parse(LookupPlant.AdvancedSearch.ToJson())); // Field LookupPlant
            filters.Merge(JObject.Parse(IdPlant.AdvancedSearch.ToJson())); // Field IdPlant
            filters.Merge(JObject.Parse(TaskList.AdvancedSearch.ToJson())); // Field TaskList
            filters.Merge(JObject.Parse(Status.AdvancedSearch.ToJson())); // Field Status
            filters.Merge(JObject.Parse(_File.AdvancedSearch.ToJson())); // Field File
            filters.Merge(JObject.Parse(DownloadFile.AdvancedSearch.ToJson())); // Field DownloadFile
            filters.Merge(JObject.Parse(StatusEmail.AdvancedSearch.ToJson())); // Field StatusEmail
            filters.Merge(JObject.Parse(CreatedBy.AdvancedSearch.ToJson())); // Field CreatedBy
            filters.Merge(JObject.Parse(CreatedDate.AdvancedSearch.ToJson())); // Field CreatedDate
            ...
            filters.Merge(JObject.Parse(CompletedBy.AdvancedSearch.ToJson())); // Field CompletedBy
            filters.Merge(JObject.Parse(CompletedDate.AdvancedSearch.ToJson())); // Field CompletedDate
        ...
        protected bool RestoreFilterList() {
            ...
            // Field NoMWTOnline
            if (filter?.TryGetValue("x_NoMWTOnline", out sv) ?? false) {
                NoMWTOnline.AdvancedSearch.SearchValue = sv;
                NoMWTOnline.AdvancedSearch.SearchOperator = filter["z_NoMWTOnline"];
                NoMWTOnline.AdvancedSearch.SearchCondition = filter["v_NoMWTOnline"];
                NoMWTOnline.AdvancedSearch.SearchValue2 = filter["y_NoMWTOnline"];
                NoMWTOnline.AdvancedSearch.SearchOperator2 = filter["w_NoMWTOnline"];
                NoMWTOnline.AdvancedSearch.Save();
            }

            // Field LinkRedirect
            if (filter?.TryGetValue("x_LinkRedirect", out sv) ?? false) {
                LinkRedirect.AdvancedSearch.SearchValue = sv;
                LinkRedirect.AdvancedSearch.SearchOperator = filter["z_LinkRedirect"];
                LinkRedirect.AdvancedSearch.SearchCondition = filter["v_LinkRedirect"];
                LinkRedirect.AdvancedSearch.SearchValue2 = filter["y_LinkRedirect"];
                LinkRedirect.AdvancedSearch.SearchOperator2 = filter["w_LinkRedirect"];
                LinkRedirect.AdvancedSearch.Save();
            }

            // Field LookupPlant
            if (filter?.TryGetValue("x_LookupPlant", out sv) ?? false) {
                LookupPlant.AdvancedSearch.SearchValue = sv;
                LookupPlant.AdvancedSearch.SearchOperator = filter["z_LookupPlant"];
                LookupPlant.AdvancedSearch.SearchCondition = filter["v_LookupPlant"];
                LookupPlant.AdvancedSearch.SearchValue2 = filter["y_LookupPlant"];
                LookupPlant.AdvancedSearch.SearchOperator2 = filter["w_LookupPlant"];
                LookupPlant.AdvancedSearch.Save();
            }
            ...
            // Field TaskList
            if (filter?.TryGetValue("x_TaskList", out sv) ?? false) {
                TaskList.AdvancedSearch.SearchValue = sv;
                TaskList.AdvancedSearch.SearchOperator = filter["z_TaskList"];
                TaskList.AdvancedSearch.SearchCondition = filter["v_TaskList"];
                TaskList.AdvancedSearch.SearchValue2 = filter["y_TaskList"];
                TaskList.AdvancedSearch.SearchOperator2 = filter["w_TaskList"];
                TaskList.AdvancedSearch.Save();
            }
            ...
            // Field File
            if (filter?.TryGetValue("x__File", out sv) ?? false) {
                _File.AdvancedSearch.SearchValue = sv;
                _File.AdvancedSearch.SearchOperator = filter["z__File"];
                _File.AdvancedSearch.SearchCondition = filter["v__File"];
                _File.AdvancedSearch.SearchValue2 = filter["y__File"];
                _File.AdvancedSearch.SearchOperator2 = filter["w__File"];
                _File.AdvancedSearch.Save();
            }

            // Field DownloadFile
            if (filter?.TryGetValue("x_DownloadFile", out sv) ?? false) {
                DownloadFile.AdvancedSearch.SearchValue = sv;
                DownloadFile.AdvancedSearch.SearchOperator = filter["z_DownloadFile"];
                DownloadFile.AdvancedSearch.SearchCondition = filter["v_DownloadFile"];
                DownloadFile.AdvancedSearch.SearchValue2 = filter["y_DownloadFile"];
                DownloadFile.AdvancedSearch.SearchOperator2 = filter["w_DownloadFile"];
                DownloadFile.AdvancedSearch.Save();
            }

            // Field StatusEmail
            if (filter?.TryGetValue("x_StatusEmail", out sv) ?? false) {
                StatusEmail.AdvancedSearch.SearchValue = sv;
                StatusEmail.AdvancedSearch.SearchOperator = filter["z_StatusEmail"];
                StatusEmail.AdvancedSearch.SearchCondition = filter["v_StatusEmail"];
                StatusEmail.AdvancedSearch.SearchValue2 = filter["y_StatusEmail"];
                StatusEmail.AdvancedSearch.SearchOperator2 = filter["w_StatusEmail"];
                StatusEmail.AdvancedSearch.Save();
            }
            ...
            // Field CreatedDate
            if (filter?.TryGetValue("x_CreatedDate", out sv) ?? false) {
                CreatedDate.AdvancedSearch.SearchValue = sv;
            ...
            // Field CompletedBy
            if (filter?.TryGetValue("x_CompletedBy", out sv) ?? false) {
                CompletedBy.AdvancedSearch.SearchValue = sv;
                CompletedBy.AdvancedSearch.SearchOperator = filter["z_CompletedBy"];
                CompletedBy.AdvancedSearch.SearchCondition = filter["v_CompletedBy"];
                CompletedBy.AdvancedSearch.SearchValue2 = filter["y_CompletedBy"];
                CompletedBy.AdvancedSearch.SearchOperator2 = filter["w_CompletedBy"];
                CompletedBy.AdvancedSearch.Save();
            }

            // Field CompletedDate
            if (filter?.TryGetValue("x_CompletedDate", out sv) ?? false) {
                CompletedDate.AdvancedSearch.SearchValue = sv;
                CompletedDate.AdvancedSearch.SearchOperator = filter["z_CompletedDate"];
                CompletedDate.AdvancedSearch.SearchCondition = filter["v_CompletedDate"];
                CompletedDate.AdvancedSearch.SearchValue2 = filter["y_CompletedDate"];
                CompletedDate.AdvancedSearch.SearchOperator2 = filter["w_CompletedDate"];
                CompletedDate.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, NoMWTOnline, def, true); // NoMWTOnline
            BuildSearchSql(ref where, LinkRedirect, def, true); // LinkRedirect
            BuildSearchSql(ref where, LookupPlant, def, true); // LookupPlant
            BuildSearchSql(ref where, IdPlant, def, true); // IdPlant
            BuildSearchSql(ref where, TaskList, def, true); // TaskList
            BuildSearchSql(ref where, Status, def, true); // Status
            BuildSearchSql(ref where, _File, def, true); // _File
            BuildSearchSql(ref where, DownloadFile, def, false); // DownloadFile
            BuildSearchSql(ref where, StatusEmail, def, true); // StatusEmail
            BuildSearchSql(ref where, CreatedBy, def, true); // CreatedBy
            BuildSearchSql(ref where, CreatedDate, def, true); // CreatedDate
            ...
            BuildSearchSql(ref where, CompletedBy, def, true); // CompletedBy
            BuildSearchSql(ref where, CompletedDate, def, true); // CompletedDate
            ...
            if (!def && Command == "search") {
                ...
                NoMWTOnline.AdvancedSearch.Save(); // NoMWTOnline
                LinkRedirect.AdvancedSearch.Save(); // LinkRedirect
                LookupPlant.AdvancedSearch.Save(); // LookupPlant
                IdPlant.AdvancedSearch.Save(); // IdPlant
                TaskList.AdvancedSearch.Save(); // TaskList
                Status.AdvancedSearch.Save(); // Status
                _File.AdvancedSearch.Save(); // File
                DownloadFile.AdvancedSearch.Save(); // DownloadFile
                StatusEmail.AdvancedSearch.Save(); // StatusEmail
                ...
                CompletedBy.AdvancedSearch.Save(); // CompletedBy
                CompletedDate.AdvancedSearch.Save(); // CompletedDate
        ...
        public void ShowFilterList()
        {
            ...
            // Field DownloadFile
            filter = QueryBuilderWhere("DownloadFile");
            if (Empty(filter))
                BuildSearchSql(ref filter, DownloadFile, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DownloadFile.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field CreatedDate
            filter = QueryBuilderWhere("CreatedDate");
            if (Empty(filter))
                BuildSearchSql(ref filter, CreatedDate, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + CreatedDate.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field CompletedBy
            filter = QueryBuilderWhere("CompletedBy");
            if (Empty(filter))
                BuildSearchSql(ref filter, CompletedBy, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + CompletedBy.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field CompletedDate
            filter = QueryBuilderWhere("CompletedDate");
            if (Empty(filter))
                BuildSearchSql(ref filter, CompletedDate, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + CompletedDate.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NoMWTOnline);
            searchFlds.Add(LinkRedirect);
            searchFlds.Add(LookupPlant);
            searchFlds.Add(TaskList);
            searchFlds.Add(Status);
            searchFlds.Add(_File);
            searchFlds.Add(DownloadFile);
            searchFlds.Add(StatusEmail);
            searchFlds.Add(CreatedBy);
            searchFlds.Add(LastUpdatedBy);
            searchFlds.Add(CompletedBy);
        ...
        protected bool CheckSearchParms() {
            ...
            if (Id.AdvancedSearch.IssetSession)
                return true;
            if (NoMWTOnline.AdvancedSearch.IssetSession)
                return true;
            if (LinkRedirect.AdvancedSearch.IssetSession)
                return true;
            if (LookupPlant.AdvancedSearch.IssetSession)
                return true;
            if (IdPlant.AdvancedSearch.IssetSession)
                return true;
            if (TaskList.AdvancedSearch.IssetSession)
                return true;
            if (Status.AdvancedSearch.IssetSession)
                return true;
            if (_File.AdvancedSearch.IssetSession)
                return true;
            if (DownloadFile.AdvancedSearch.IssetSession)
                return true;
            if (StatusEmail.AdvancedSearch.IssetSession)
                return true;
            ...
            if (LastUpdatedDate.AdvancedSearch.IssetSession)
                return true;
            if (CompletedBy.AdvancedSearch.IssetSession)
                return true;
            if (CompletedDate.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            NoMWTOnline.AdvancedSearch.UnsetSession();
            LinkRedirect.AdvancedSearch.UnsetSession();
            LookupPlant.AdvancedSearch.UnsetSession();
            IdPlant.AdvancedSearch.UnsetSession();
            TaskList.AdvancedSearch.UnsetSession();
            Status.AdvancedSearch.UnsetSession();
            _File.AdvancedSearch.UnsetSession();
            DownloadFile.AdvancedSearch.UnsetSession();
            StatusEmail.AdvancedSearch.UnsetSession();
            ...
            CompletedBy.AdvancedSearch.UnsetSession();
            CompletedDate.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            NoMWTOnline.AdvancedSearch.Load();
            LinkRedirect.AdvancedSearch.Load();
            LookupPlant.AdvancedSearch.Load();
            IdPlant.AdvancedSearch.Load();
            TaskList.AdvancedSearch.Load();
            Status.AdvancedSearch.Load();
            _File.AdvancedSearch.Load();
            DownloadFile.AdvancedSearch.Load();
            StatusEmail.AdvancedSearch.Load();
            ...
            CompletedBy.AdvancedSearch.Load();
            CompletedDate.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = CreatedDate.Expression + " DESC"; // Set up default sort
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(DownloadFile, ctrl); // DownloadFile
                UpdateSort(CreatedBy, ctrl); // CreatedBy
                UpdateSort(CreatedDate, ctrl); // CreatedDate
                UpdateSort(LastUpdatedBy, ctrl); // LastUpdatedBy
                UpdateSort(LastUpdatedDate, ctrl); // LastUpdatedDate
                UpdateSort(CompletedBy, ctrl); // CompletedBy
                UpdateSort(CompletedDate, ctrl); // CompletedDate
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MWTOnline"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMWTOnlinelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMWTOnlinelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MWTOnline"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMWTOnlinelist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("DownloadFile")); // DN
                CreateColumnOption(option.Add("CreatedBy")); // DN
                CreateColumnOption(option.Add("CreatedDate")); // DN
                CreateColumnOption(option.Add("LastUpdatedBy")); // DN
                CreateColumnOption(option.Add("LastUpdatedDate")); // DN
                CreateColumnOption(option.Add("CompletedBy")); // DN
                CreateColumnOption(option.Add("CompletedDate")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMWTOnlinesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMWTOnlinesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMWTOnlinelist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mwtOnlineList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mwtOnlineList.RowCount) + "_MWTOnline");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mwtOnlineList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mwtOnlineList.RowType == RowType.Add || IsEdit && mwtOnlineList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // NoMWTOnline
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NoMWTOnline[]"))
                    NoMWTOnline.AdvancedSearch.SearchValue = Get("x_NoMWTOnline[]", Config.FilterOptionSeparator);
                else
                    NoMWTOnline.AdvancedSearch.SearchValue = Get("NoMWTOnline"); // Default Value // DN
            if (!Empty(NoMWTOnline.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_NoMWTOnline"))
                NoMWTOnline.AdvancedSearch.SearchOperator = Get("z_NoMWTOnline", Config.FilterOptionSeparator);
            ...
            // LookupPlant
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_LookupPlant[]"))
                    LookupPlant.AdvancedSearch.SearchValue = Get("x_LookupPlant[]", Config.FilterOptionSeparator);
                else
                    LookupPlant.AdvancedSearch.SearchValue = Get("LookupPlant"); // Default Value // DN
            if (!Empty(LookupPlant.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_LookupPlant"))
                LookupPlant.AdvancedSearch.SearchOperator = Get("z_LookupPlant", Config.FilterOptionSeparator);
            ...
            // TaskList
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TaskList[]"))
                    TaskList.AdvancedSearch.SearchValue = Get("x_TaskList[]", Config.FilterOptionSeparator);
                else
                    TaskList.AdvancedSearch.SearchValue = Get("TaskList"); // Default Value // DN
            if (!Empty(TaskList.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_TaskList"))
                TaskList.AdvancedSearch.SearchOperator = Get("z_TaskList", Config.FilterOptionSeparator);
            ...
            // _File
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x__File[]"))
                    _File.AdvancedSearch.SearchValue = Get("x__File[]", Config.FilterOptionSeparator);
                else
                    _File.AdvancedSearch.SearchValue = Get("_File"); // Default Value // DN
            if (!Empty(_File.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z__File"))
                _File.AdvancedSearch.SearchOperator = Get("z__File", Config.FilterOptionSeparator);

            // DownloadFile
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DownloadFile"))
                    DownloadFile.AdvancedSearch.SearchValue = Get("x_DownloadFile", Config.FilterOptionSeparator);
                else
                    DownloadFile.AdvancedSearch.SearchValue = Get("DownloadFile"); // Default Value // DN
            if (!Empty(DownloadFile.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_DownloadFile"))
                DownloadFile.AdvancedSearch.SearchOperator = Get("z_DownloadFile", Config.FilterOptionSeparator);

            // StatusEmail
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_StatusEmail[]"))
                    StatusEmail.AdvancedSearch.SearchValue = Get("x_StatusEmail[]", Config.FilterOptionSeparator);
                else
                    StatusEmail.AdvancedSearch.SearchValue = Get("StatusEmail"); // Default Value // DN
            if (!Empty(StatusEmail.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_StatusEmail"))
                StatusEmail.AdvancedSearch.SearchOperator = Get("z_StatusEmail", Config.FilterOptionSeparator);
            ...
            // CreatedDate
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_CreatedDate[]"))
                    CreatedDate.AdvancedSearch.SearchValue = Get("x_CreatedDate[]", Config.FilterOptionSeparator);
                else
                    CreatedDate.AdvancedSearch.SearchValue = Get("CreatedDate"); // Default Value // DN
            if (!Empty(CreatedDate.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_CreatedDate"))
                CreatedDate.AdvancedSearch.SearchOperator = Get("z_CreatedDate", Config.FilterOptionSeparator);
            ...
            // CompletedBy
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_CompletedBy[]"))
                    CompletedBy.AdvancedSearch.SearchValue = Get("x_CompletedBy[]", Config.FilterOptionSeparator);
                else
                    CompletedBy.AdvancedSearch.SearchValue = Get("CompletedBy"); // Default Value // DN
            if (!Empty(CompletedBy.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_CompletedBy"))
                CompletedBy.AdvancedSearch.SearchOperator = Get("z_CompletedBy", Config.FilterOptionSeparator);

            // CompletedDate
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_CompletedDate[]"))
                    CompletedDate.AdvancedSearch.SearchValue = Get("x_CompletedDate[]", Config.FilterOptionSeparator);
                else
                    CompletedDate.AdvancedSearch.SearchValue = Get("CompletedDate"); // Default Value // DN
            if (!Empty(CompletedDate.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_CompletedDate"))
                CompletedDate.AdvancedSearch.SearchOperator = Get("z_CompletedDate", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            // KeteranganFile
            KeteranganFile.CellCssStyle = "white-space: nowrap;";
            ...
            if (RowType == RowType.View) {
                ...
                if (!DownloadFile.Raw)
                    DownloadFile.AdvancedSearch.SearchValue = HtmlDecode(DownloadFile.AdvancedSearch.SearchValue);
                DownloadFile.EditValue = HtmlEncode(DownloadFile.AdvancedSearch.SearchValue);
                ...
                // CreatedDate
                if (CreatedDate.UseFilter && !Empty(CreatedDate.AdvancedSearch.SearchValue)) {
                    CreatedDate.EditValue = ConvertToString(CreatedDate.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // CompletedBy
                if (CompletedBy.UseFilter && !Empty(CompletedBy.AdvancedSearch.SearchValue)) {
                    CompletedBy.EditValue = ConvertToString(CompletedBy.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // CompletedDate
                if (CompletedDate.UseFilter && !Empty(CompletedDate.AdvancedSearch.SearchValue)) {
                    CompletedDate.EditValue = ConvertToString(CompletedDate.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMWTOnlinesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
