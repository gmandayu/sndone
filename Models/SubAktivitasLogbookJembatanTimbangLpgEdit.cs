...
public partial class SnDOne {
    ...
    public class SubAktivitasLogbookJembatanTimbangLpgEditBase : SubAktivitasLogbookJembatanTimbangLpg
    {
        ...
        // Constructor
        public SubAktivitasLogbookJembatanTimbangLpgEditBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasLogbookJembatanTimbangLpgEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasLogbookJembatanTimbangLpgList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "SubAktivitasLogbookJembatanTimbangLpgList") {
                        ...
                        returnUrl = "SubAktivitasLogbookJembatanTimbangLpgList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'SPA'
            SPA.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(SPA, "SPA", "x_SPA");

            // Standard handling for 'TimbanganAwal'
            TimbanganAwal.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(TimbanganAwal, "TimbanganAwal", "x_TimbanganAwal");

            // Standard handling for 'TimbanganAkhir'
            TimbanganAkhir.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(TimbanganAkhir, "TimbanganAkhir", "x_TimbanganAkhir");

            // Standard handling for 'NetWight'
            NetWight.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NetWight, "NetWight", "x_NetWight");

            // Standard handling for 'PresentaseLevelRotoGauge'
            PresentaseLevelRotoGauge.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(PresentaseLevelRotoGauge, "PresentaseLevelRotoGauge", "x_PresentaseLevelRotoGauge");
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorTangki() {
            if (NomorTangki.Visible && NomorTangki.Required) {
                if (!NomorTangki.IsDetailKey && Empty(NomorTangki.FormValue)) {
                    NomorTangki.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomProduk() {
            if (Produk.Visible && Produk.Required) {
                if (!Produk.IsDetailKey && Empty(Produk.FormValue)) {
                    Produk.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }

        private void ValidateCustomSPA() {
            if (SPA.Visible && SPA.Required) {
                if (!SPA.IsDetailKey && Empty(SPA.FormValue)) {
                    SPA.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }

        private void ValidateCustomTimbanganAwal() {
            if (TimbanganAwal.Visible && TimbanganAwal.Required) {
                if (!TimbanganAwal.IsDetailKey && Empty(TimbanganAwal.FormValue)) {
                    TimbanganAwal.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }

        private void ValidateCustomTimbanganAkhir() {
            if (TimbanganAkhir.Visible && TimbanganAkhir.Required) {
                if (!TimbanganAkhir.IsDetailKey && Empty(TimbanganAkhir.FormValue)) {
                    TimbanganAkhir.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }

        private void ValidateCustomNetWight() {
            if (NetWight.Visible && NetWight.Required) {
                if (!NetWight.IsDetailKey && Empty(NetWight.FormValue)) {
                    NetWight.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }

        private void ValidateCustomPresentaseLevelRotoGauge() {
            if (PresentaseLevelRotoGauge.Visible && PresentaseLevelRotoGauge.Required) {
                if (!PresentaseLevelRotoGauge.IsDetailKey && Empty(PresentaseLevelRotoGauge.FormValue)) {
                    PresentaseLevelRotoGauge.AddErrorMessage(ConvertToString(lastUpdatedDate.RequiredErrorMessage).Replace("%s", lastUpdatedDate.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomSPA();
                ValidateCustomTimbanganAwal();
                ValidateCustomTimbanganAkhir();
                ValidateCustomNetWight();
                ValidateCustomPresentaseLevelRotoGauge();
