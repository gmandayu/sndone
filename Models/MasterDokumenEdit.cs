...
public partial class SnDOne {
    ...
    /// masterDokumenEdit
    /// </summary>
    public static MasterDokumenEdit masterDokumenEdit
    {
        get => HttpData.Get<MasterDokumenEdit>("masterDokumenEdit")!;
        set => HttpData["masterDokumenEdit"] = value;
    }

    /// <summary>
    /// Page class for MasterDokumen
    /// </summary>
    public class MasterDokumenEdit : MasterDokumenEditBase
    {
        // Constructor
        public MasterDokumenEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterDokumenEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterDokumenEditBase : MasterDokumen
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterDokumenEdit";
        ...
        // Constructor
        public MasterDokumenEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "MasterDokumenEdit";
        ...
        // Constructor
        public MasterDokumenEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdDokumen", out rv)) {
                    IdDokumen.FormValue = UrlDecode(rv);
                    IdDokumen.OldValue = IdDokumen.FormValue;
                } else if (CurrentForm.HasValue("x_IdDokumen")) {
                    IdDokumen.FormValue = CurrentForm.GetValue("x_IdDokumen");
                    IdDokumen.OldValue = IdDokumen.FormValue;
                } else if (!Empty(keyValues)) {
                    IdDokumen.OldValue = ConvertToString(keyValues[0]);
                } else {
                    loaded = false;
                }
                if (loaded)
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdDokumen", out rv)) {
                        IdDokumen.QueryValue = UrlDecode(rv);
                        loadByQuery = true;
                    } else if (Get("IdDokumen", out sv)) {
                        IdDokumen.QueryValue = sv.ToString();
                        loadByQuery = true;
                    } else {
                        IdDokumen.CurrentValue = DbNullValue;
                    }
                }
                if (IsShow) {
                    loaded = await LoadRow();
        ...
        private async Task HandleApiFormKeys()
        {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                IdDokumen.FormValue = ConvertToString(keyValues[0]);
            }
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "MasterDokumenList") {
        ...
        private void ValidateCustomIdDokumen() {
            if (IdDokumen.Visible && IdDokumen.Required) {
                if (!IdDokumen.IsDetailKey && Empty(IdDokumen.FormValue)) {
                    IdDokumen.AddErrorMessage(ConvertToString(NamaDokumen.RequiredErrorMessage).Replace("%s", NamaDokumen.Caption));
                }
            }
        }
