...
public partial class SnDOne {
    ...
    /// </summary>
    public abstract class AspNetMakerPage : IAspNetMakerPage, IQueryFactory, IDisposable
    {
        // Private properties
        ...
        private Lazy<ListOptions> _headerOptions;

        private Lazy<ListOptions> _footerOptions;
        ...
        public AspNetMakerPage()
        {
            ...
            // Header options
            _headerOptions = new Lazy<ListOptions>(() => new ListOptions { TagClassName = "ew-header-option" });

            // Footer options
            _footerOptions = new Lazy<ListOptions>(() => new ListOptions { TagClassName = "ew-footer-option" });
        ...
        // Connection
        public virtual IDatabaseConnection Connection => GetConnection(DbId);
        #pragma warning restore 618

        // Get query factory
        public QueryFactory GetQueryFactory(bool main = false) => Connection.GetQueryFactory(main);

        // Get query builder
        public QueryBuilder GetQueryBuilder(bool main = false)
        {
            var query = GetQueryFactory(main).Query() as XQuery;
            string tbl = UpdateTable ?? TableName;
            return !Empty(tbl) ? query!.From(tbl) : query!;
        }
        ...
        // Header options
        public ListOptions HeaderOptions => _headerOptions.Value;

        // Footer options
        public ListOptions FooterOptions => _footerOptions.Value;
