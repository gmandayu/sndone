...
public partial class SnDOne {
    ...
    public class ProofOfVisitEditBase : ProofOfVisit
    {
        ...
        // Constructor
        public ProofOfVisitEditBase(Controller? controller)
        {
        ...
        // Constructor
        public ProofOfVisitEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdProofOfVisit", out rv)) {
                    IdProofOfVisit.FormValue = UrlDecode(rv);
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdProofOfVisit", out rv)) {
                        IdProofOfVisit.QueryValue = UrlDecode(rv);
                        loadByQuery = true;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("ProofOfVisitList");
            }
            return null;
        }
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            if (!IdProofOfVisit.IsDetailKey) {
                SetNormalField(IdProofOfVisit, "IdProofOfVisit", "x_IdProofOfVisit");
            }

            // Standard handling for 'NomorProofOfVisit'
            NomorProofOfVisit.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NomorProofOfVisit, "NomorProofOfVisit", "x_NomorProofOfVisit");
        ...
        private void ValidateCustomIdProofOfVisit() {
            if (IdProofOfVisit.Visible && IdProofOfVisit.Required) {
                if (!IdProofOfVisit.IsDetailKey && Empty(IdProofOfVisit.FormValue)) {
                    IdProofOfVisit.AddErrorMessage(ConvertToString(Catatan.RequiredErrorMessage).Replace("%s", Catatan.Caption));
                }
            }
        }

        private void ValidateCustomNomorProofOfVisit() {
            if (NomorProofOfVisit.Visible && NomorProofOfVisit.Required) {
                if (!NomorProofOfVisit.IsDetailKey && Empty(NomorProofOfVisit.FormValue)) {
                    NomorProofOfVisit.AddErrorMessage(ConvertToString(Catatan.RequiredErrorMessage).Replace("%s", Catatan.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ValidateCustomIdProofOfVisit();
                ValidateCustomNomorProofOfVisit();
                ValidateCustomPlant();
