...
public partial class SnDOne {
    ...
    public class SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEditBase : SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbm
    {
        ...
        // Constructor
        public SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEditBase()
        {
        ...
        // Constructor
        public SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmList") {
                                ...
                                returnUrl = "SubAktivitasFormInputNilaiBLsesuaiCqlstsPymBbmList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'SFAL_KL_Obs' before field var 'x_SFAL_KL_Obs'
            val = CurrentForm.HasValue("SFAL_KL_Obs") ? CurrentForm.GetValue("SFAL_KL_Obs") : CurrentForm.GetValue("x_SFAL_KL_Obs");
            if (!SFAL_KL_Obs.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("SFAL_KL_Obs") && !CurrentForm.HasValue("x_SFAL_KL_Obs")) // DN
                    SFAL_KL_Obs.Visible = false; // Disable update for API request
                else
                    SFAL_KL_Obs.SetFormValue(val);
            }

            // Check field name 'SFAL_KL_15' before field var 'x_SFAL_KL_15'
            val = CurrentForm.HasValue("SFAL_KL_15") ? CurrentForm.GetValue("SFAL_KL_15") : CurrentForm.GetValue("x_SFAL_KL_15");
            if (!SFAL_KL_15.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("SFAL_KL_15") && !CurrentForm.HasValue("x_SFAL_KL_15")) // DN
                    SFAL_KL_15.Visible = false; // Disable update for API request
                else
                    SFAL_KL_15.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (SFAL_KL_Obs.Visible && SFAL_KL_Obs.Required) {
                    if (!SFAL_KL_Obs.IsDetailKey && Empty(SFAL_KL_Obs.FormValue)) {
                        SFAL_KL_Obs.AddErrorMessage(ConvertToString(SFAL_KL_Obs.RequiredErrorMessage).Replace("%s", SFAL_KL_Obs.Caption));
                    }
                }
                if (SFAL_KL_15.Visible && SFAL_KL_15.Required) {
                    if (!SFAL_KL_15.IsDetailKey && Empty(SFAL_KL_15.FormValue)) {
                        SFAL_KL_15.AddErrorMessage(ConvertToString(SFAL_KL_15.RequiredErrorMessage).Replace("%s", SFAL_KL_15.Caption));
                    }
                }
