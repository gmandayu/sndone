...
public partial class SnDOne {
    ...
    public class AktivitasDokumenGridBase : AktivitasDokumen
    {
        ...
        // Constructor
        public AktivitasDokumenGridBase()
        {
        ...
        protected async Task LoadFormValues() {
            ...
            if (!IdAktivitas.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("IdAktivitas") && !CurrentForm.HasValue("x_IdAktivitas")) // DN
                    IdAktivitas.Visible = false; // Disable update for API request
                else
            ...
            // Check field name 'UploadDokumen' before field var 'x_UploadDokumen'
            val = CurrentForm.HasValue("UploadDokumen") ? CurrentForm.GetValue("UploadDokumen") : CurrentForm.GetValue("x_UploadDokumen");
            if (!UploadDokumen.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("UploadDokumen") && !CurrentForm.HasValue("x_UploadDokumen")) // DN
                    UploadDokumen.Visible = false; // Disable update for API request
                else
                    UploadDokumen.SetFormValue(val);
            }
            if (CurrentForm.HasValue("o_UploadDokumen"))
                UploadDokumen.OldValue = CurrentForm.GetValue("o_UploadDokumen");

            // Check field name 'DownloadDokumen' before field var 'x_DownloadDokumen'
            val = CurrentForm.HasValue("DownloadDokumen") ? CurrentForm.GetValue("DownloadDokumen") : CurrentForm.GetValue("x_DownloadDokumen");
            if (!DownloadDokumen.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("DownloadDokumen") && !CurrentForm.HasValue("x_DownloadDokumen")) // DN
                    DownloadDokumen.Visible = false; // Disable update for API request
                else
                    DownloadDokumen.SetFormValue(val);
            }
            if (CurrentForm.HasValue("o_DownloadDokumen"))
                DownloadDokumen.OldValue = CurrentForm.GetValue("o_DownloadDokumen");

            // Check field name 'DiunggahOleh' before field var 'x_DiunggahOleh'
            val = CurrentForm.HasValue("DiunggahOleh") ? CurrentForm.GetValue("DiunggahOleh") : CurrentForm.GetValue("x_DiunggahOleh");
            if (!DiunggahOleh.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("DiunggahOleh") && !CurrentForm.HasValue("x_DiunggahOleh")) // DN
                    DiunggahOleh.Visible = false; // Disable update for API request
                else
                    DiunggahOleh.SetFormValue(val);
            }
            if (CurrentForm.HasValue("o_DiunggahOleh"))
                DiunggahOleh.OldValue = CurrentForm.GetValue("o_DiunggahOleh");

            // Check field name 'TanggalUpload' before field var 'x_TanggalUpload'
            val = CurrentForm.HasValue("TanggalUpload") ? CurrentForm.GetValue("TanggalUpload") : CurrentForm.GetValue("x_TanggalUpload");
            if (!TanggalUpload.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TanggalUpload") && !CurrentForm.HasValue("x_TanggalUpload")) // DN
                    TanggalUpload.Visible = false; // Disable update for API request
                else
                    TanggalUpload.SetFormValue(val, true, validate);
            ...
            // Check field name 'IdAktivitasDokumen' before field var 'x_IdAktivitasDokumen'
            val = CurrentForm.HasValue("IdAktivitasDokumen") ? CurrentForm.GetValue("IdAktivitasDokumen") : CurrentForm.GetValue("x_IdAktivitasDokumen");
            if (!IdAktivitasDokumen.IsDetailKey && !IsGridAdd && !IsAdd)
                IdAktivitasDokumen.SetFormValue(val);
        }
        #pragma warning restore 1998
            ...
            bool validateForm = true;
                ...
                if (IdAktivitas.Visible && IdAktivitas.Required) {
                    if (!IdAktivitas.IsDetailKey && Empty(IdAktivitas.FormValue)) {
                        IdAktivitas.AddErrorMessage(ConvertToString(IdAktivitas.RequiredErrorMessage).Replace("%s", IdAktivitas.Caption));
                    }
                }
                ...
                if (UploadDokumen.Visible && UploadDokumen.Required) {
                    if (!UploadDokumen.IsDetailKey && Empty(UploadDokumen.FormValue)) {
                        UploadDokumen.AddErrorMessage(ConvertToString(UploadDokumen.RequiredErrorMessage).Replace("%s", UploadDokumen.Caption));
                    }
                }
                if (DownloadDokumen.Visible && DownloadDokumen.Required) {
                    if (!DownloadDokumen.IsDetailKey && Empty(DownloadDokumen.FormValue)) {
                        DownloadDokumen.AddErrorMessage(ConvertToString(DownloadDokumen.RequiredErrorMessage).Replace("%s", DownloadDokumen.Caption));
                    }
                }
                if (DiunggahOleh.Visible && DiunggahOleh.Required) {
                    if (!DiunggahOleh.IsDetailKey && Empty(DiunggahOleh.FormValue)) {
                        DiunggahOleh.AddErrorMessage(ConvertToString(DiunggahOleh.RequiredErrorMessage).Replace("%s", DiunggahOleh.Caption));
                    }
                }
                if (TanggalUpload.Visible && TanggalUpload.Required) {
                    if (!TanggalUpload.IsDetailKey && Empty(TanggalUpload.FormValue)) {
                        TanggalUpload.AddErrorMessage(ConvertToString(TanggalUpload.RequiredErrorMessage).Replace("%s", TanggalUpload.Caption));
                    }
                }
