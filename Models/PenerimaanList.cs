...
public partial class SnDOne {
    ...
    /// penerimaanList
    /// </summary>
    public static PenerimaanList penerimaanList
    {
        get => HttpData.Get<PenerimaanList>("penerimaanList")!;
        set => HttpData["penerimaanList"] = value;
    }

    /// <summary>
    /// Page class for Penerimaan
    /// </summary>
    public class PenerimaanList : PenerimaanListBase
    {
        // Constructor
        public PenerimaanList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public PenerimaanList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class PenerimaanListBase : Penerimaan
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "penerimaanList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fPenerimaanlist";
        ...
        // Constructor
        public PenerimaanListBase(Controller? controller)
        {
        ...
        if (controller != null)
            ...
            MultiDeleteUrl = "PenerimaanDelete";
            MultiUpdateUrl = "PenerimaanUpdate";
        ...
        // Page name
        public string PageName => "PenerimaanList";
        ...
        // Constructor
        public PenerimaanListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(TanggalSandar.AdvancedSearch.ToJson())); // Field TanggalSandar
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, NomorPenerimaan, def, true); // NomorPenerimaan
            BuildSearchSql(ref where, Proses2, def, true); // Proses2
            BuildSearchSql(ref where, NomorVoyage, def, true); // NomorVoyage
            BuildSearchSql(ref where, NomorPengiriman, def, true); // NomorPengiriman
            ...
            BuildSearchSql(ref where, ModaTransportasi, def, true); // ModaTransportasi
            BuildSearchSql(ref where, NamaKapal, def, true); // NamaKapal
            BuildSearchSql(ref where, Refinery, def, true); // Refinery
            BuildSearchSql(ref where, TanggalSandar, def, true); // TanggalSandar
            BuildSearchSql(ref where, IdDermaga, def, true); // IdDermaga
            ...
            if (!def && Command == "search") {
                TanggalSandar.AdvancedSearch.Save(); // TanggalSandar
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("Proses", Proses2, true, config);
            filterList += ProcessFieldFilter("NomorVoyage", NomorVoyage, true, config);
            filterList += ProcessFieldFilter("NomorPengiriman", NomorPengiriman, true, config);
            ...
            filterList += ProcessFieldFilter("ModaTransportasi", ModaTransportasi, true, config);
            filterList += ProcessFieldFilter("NamaKapal", NamaKapal, true, config);
            filterList += ProcessFieldFilter("Refinery", Refinery, true, config);
            filterList += ProcessFieldFilter("TanggalSandar", TanggalSandar, true, config);
            filterList += ProcessFieldFilter("IdDermaga", IdDermaga, true, config);
            ...
            filterList += ProcessFieldFilter("IdTemplate", IdTemplate, true, config);
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NomorPenerimaan);
            searchFlds.Add(Proses2);
            searchFlds.Add(NomorVoyage);
            searchFlds.Add(NomorPengiriman);
            searchFlds.Add(IdPlant);
            searchFlds.Add(TipeProdukSTS);
            searchFlds.Add(KodeProduk);
            searchFlds.Add(ModaTransportasi);
            searchFlds.Add(NamaKapal);
            searchFlds.Add(TanggalSandar);
            searchFlds.Add(IdDermaga);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (NomorPenerimaan.AdvancedSearch.IssetSession)
                return true;
            if (Proses2.AdvancedSearch.IssetSession)
                return true;
            if (NomorVoyage.AdvancedSearch.IssetSession)
                return true;
            if (NomorPengiriman.AdvancedSearch.IssetSession)
                return true;
            ...
            if (KodeProduk.AdvancedSearch.IssetSession)
                return true;
            if (ModaTransportasi.AdvancedSearch.IssetSession)
                return true;
            if (NamaKapal.AdvancedSearch.IssetSession)
                return true;
            if (Refinery.AdvancedSearch.IssetSession)
                return true;
            if (TanggalSandar.AdvancedSearch.IssetSession)
                return true;
            if (IdDermaga.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            NomorPenerimaan.AdvancedSearch.UnsetSession();
            Proses2.AdvancedSearch.UnsetSession();
            NomorVoyage.AdvancedSearch.UnsetSession();
            NomorPengiriman.AdvancedSearch.UnsetSession();
            IdPlant.AdvancedSearch.UnsetSession();
            TipeProdukSTS.AdvancedSearch.UnsetSession();
            KodeProduk.AdvancedSearch.UnsetSession();
            ModaTransportasi.AdvancedSearch.UnsetSession();
            NamaKapal.AdvancedSearch.UnsetSession();
            Refinery.AdvancedSearch.UnsetSession();
            TanggalSandar.AdvancedSearch.UnsetSession();
            IdDermaga.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            NomorPenerimaan.AdvancedSearch.Load();
            Proses2.AdvancedSearch.Load();
            NomorVoyage.AdvancedSearch.Load();
            NomorPengiriman.AdvancedSearch.Load();
            IdPlant.AdvancedSearch.Load();
            TipeProdukSTS.AdvancedSearch.Load();
            KodeProduk.AdvancedSearch.Load();
            ModaTransportasi.AdvancedSearch.Load();
            NamaKapal.AdvancedSearch.Load();
            Refinery.AdvancedSearch.Load();
            TanggalSandar.AdvancedSearch.Load();
            IdDermaga.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = TanggalSandar.Expression + " DESC"; // Set up default sort
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Proses2, ctrl); // Proses
                UpdateSort(NomorVoyage, ctrl); // NomorVoyage
                UpdateSort(NomorPengiriman, ctrl); // NomorPengiriman
                UpdateSort(IdPlant, ctrl); // IdPlant
                UpdateSort(TipeProdukSTS, ctrl); // TipeProdukSTS
                UpdateSort(KodeProduk, ctrl); // KodeProduk
                UpdateSort(ModaTransportasi, ctrl); // ModaTransportasi
                UpdateSort(NamaKapal, ctrl); // NamaKapal
                UpdateSort(Refinery, ctrl); // Refinery
                UpdateSort(PenyaluranLookup, ctrl); // PenyaluranLookup
                UpdateSort(TanggalSandar, ctrl); // TanggalSandar
                UpdateSort(IdDermaga, ctrl); // IdDermaga
                UpdateSort(StatusProses, ctrl); // StatusProses
                UpdateSort(PersentaseProgress, ctrl); // PersentaseProgress
                UpdateSort(IdTemplate, ctrl); // IdTemplate
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    IdPenerimaan.Sort = "";
                    NomorPenerimaan.Sort = "";
                    Proses2.Sort = "";
                    NomorVoyage.Sort = "";
                    NomorPengiriman.Sort = "";
                    LookupPlant.Sort = "";
                    IdPlant.Sort = "";
                    TipeProdukSTS.Sort = "";
                    KodeProduk.Sort = "";
                    ModaTransportasi.Sort = "";
                    NamaKapal.Sort = "";
                    DetailRTW.Sort = "";
                    Refinery.Sort = "";
                    IdPenyaluran.Sort = "";
                    PenyaluranLookup.Sort = "";
                    TanggalSandar.Sort = "";
                    IdDermaga.Sort = "";
                    StatusProses.Sort = "";
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""Penerimaan"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""Penerimaan"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""Penerimaan"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fPenerimaanlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fPenerimaanlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdPenerimaan.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""Penerimaan"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fPenerimaanlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Proses")); // DN
                CreateColumnOption(option.Add("NomorVoyage")); // DN
                CreateColumnOption(option.Add("NomorPengiriman")); // DN
                ...
                CreateColumnOption(option.Add("ModaTransportasi")); // DN
                CreateColumnOption(option.Add("NamaKapal")); // DN
                CreateColumnOption(option.Add("Refinery")); // DN
                CreateColumnOption(option.Add("PenyaluranLookup")); // DN
                CreateColumnOption(option.Add("TanggalSandar")); // DN
                CreateColumnOption(option.Add("IdDermaga")); // DN
                CreateColumnOption(option.Add("StatusProses")); // DN
                CreateColumnOption(option.Add("PersentaseProgress")); // DN
                CreateColumnOption(option.Add("IdTemplate")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fPenerimaansrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fPenerimaansrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fPenerimaanlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(penerimaanList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(penerimaanList.RowCount) + "_Penerimaan");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(penerimaanList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && penerimaanList.RowType == RowType.Add || IsEdit && penerimaanList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadFieldNomorPenerimaanSearchValues();
            LoadFieldProses2SearchValues();
            LoadFieldNomorVoyageSearchValues();
            LoadFieldNomorPengirimanSearchValues();
            LoadFieldIdPlantSearchValues();
            LoadFieldTipeProdukSTSSearchValues();
            LoadFieldKodeProdukSearchValues();
            LoadFieldModaTransportasiSearchValues();
            LoadFieldNamaKapalSearchValues();
            LoadFieldRefinerySearchValues();
            LoadFieldTanggalSandarSearchValues();
            LoadFieldIdDermagaSearchValues();
        ...
        private void LoadFieldNomorPenerimaanSearchValues()
        {
            // NomorPenerimaan
            LoadPrimarySearchValueNomorPenerimaan();
            LoadSearchOperatorNomorPenerimaan();
        }

        private void LoadPrimarySearchValueNomorPenerimaan()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NomorPenerimaan[]")) {
                NomorPenerimaan.AdvancedSearch.SearchValue = Get("x_NomorPenerimaan[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NomorPenerimaan.AdvancedSearch.SearchValue = Get("NomorPenerimaan"); // Default Value // DN
            }
            if (!Empty(NomorPenerimaan.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNomorPenerimaan()
        {
            if (Query.ContainsKey("z_NomorPenerimaan"))
                NomorPenerimaan.AdvancedSearch.SearchOperator = Get("z_NomorPenerimaan", Config.FilterOptionSeparator);
        }

        private void LoadFieldProses2SearchValues()
        {
            // Proses2
            LoadPrimarySearchValueProses2();
            LoadSearchOperatorProses2();
        }

        private void LoadPrimarySearchValueProses2()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Proses2[]")) {
                Proses2.AdvancedSearch.SearchValue = Get("x_Proses2[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Proses2.AdvancedSearch.SearchValue = Get("Proses2"); // Default Value // DN
            }
            if (!Empty(Proses2.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorProses2()
        {
            if (Query.ContainsKey("z_Proses2"))
                Proses2.AdvancedSearch.SearchOperator = Get("z_Proses2", Config.FilterOptionSeparator);
        }

        private void LoadFieldNomorVoyageSearchValues()
        {
            // NomorVoyage
            LoadPrimarySearchValueNomorVoyage();
            LoadSearchOperatorNomorVoyage();
        }

        private void LoadPrimarySearchValueNomorVoyage()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NomorVoyage[]")) {
                NomorVoyage.AdvancedSearch.SearchValue = Get("x_NomorVoyage[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NomorVoyage.AdvancedSearch.SearchValue = Get("NomorVoyage"); // Default Value // DN
            }
            if (!Empty(NomorVoyage.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNomorVoyage()
        {
            if (Query.ContainsKey("z_NomorVoyage"))
                NomorVoyage.AdvancedSearch.SearchOperator = Get("z_NomorVoyage", Config.FilterOptionSeparator);
        }

        private void LoadFieldNomorPengirimanSearchValues()
        {
            // NomorPengiriman
            LoadPrimarySearchValueNomorPengiriman();
            LoadSearchOperatorNomorPengiriman();
        }

        private void LoadPrimarySearchValueNomorPengiriman()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NomorPengiriman[]")) {
                NomorPengiriman.AdvancedSearch.SearchValue = Get("x_NomorPengiriman[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NomorPengiriman.AdvancedSearch.SearchValue = Get("NomorPengiriman"); // Default Value // DN
            }
            if (!Empty(NomorPengiriman.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNomorPengiriman()
        {
            if (Query.ContainsKey("z_NomorPengiriman"))
                NomorPengiriman.AdvancedSearch.SearchOperator = Get("z_NomorPengiriman", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldModaTransportasiSearchValues()
        {
            // ModaTransportasi
            LoadPrimarySearchValueModaTransportasi();
            LoadSearchOperatorModaTransportasi();
        }

        private void LoadPrimarySearchValueModaTransportasi()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_ModaTransportasi[]")) {
                ModaTransportasi.AdvancedSearch.SearchValue = Get("x_ModaTransportasi[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                ModaTransportasi.AdvancedSearch.SearchValue = Get("ModaTransportasi"); // Default Value // DN
            }
            if (!Empty(ModaTransportasi.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorModaTransportasi()
        {
            if (Query.ContainsKey("z_ModaTransportasi"))
                ModaTransportasi.AdvancedSearch.SearchOperator = Get("z_ModaTransportasi", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldRefinerySearchValues()
        {
            // Refinery
            LoadPrimarySearchValueRefinery();
            LoadSearchOperatorRefinery();
        }

        private void LoadPrimarySearchValueRefinery()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Refinery[]")) {
                Refinery.AdvancedSearch.SearchValue = Get("x_Refinery[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Refinery.AdvancedSearch.SearchValue = Get("Refinery"); // Default Value // DN
            }
            if (!Empty(Refinery.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorRefinery()
        {
            if (Query.ContainsKey("z_Refinery"))
                Refinery.AdvancedSearch.SearchOperator = Get("z_Refinery", Config.FilterOptionSeparator);
        }
        ...
        private void LoadSecondarySearchValuesTanggalSandar()
        {
            ...
            if (Query.ContainsKey("y_TanggalSandar"))
                TanggalSandar.AdvancedSearch.SearchValue2 = Get("y_TanggalSandar", Config.FilterOptionSeparator);
            if (!Empty(TanggalSandar.AdvancedSearch.SearchValue2) && Command == "")
                Command = "search";
        ...
        private void LoadFieldIdDermagaSearchValues()
        {
            // IdDermaga
            LoadPrimarySearchValueIdDermaga();
            LoadSearchOperatorIdDermaga();
        }

        private void LoadPrimarySearchValueIdDermaga()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdDermaga[]")) {
                IdDermaga.AdvancedSearch.SearchValue = Get("x_IdDermaga[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                IdDermaga.AdvancedSearch.SearchValue = Get("IdDermaga"); // Default Value // DN
            }
            if (!Empty(IdDermaga.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIdDermaga()
        {
            if (Query.ContainsKey("z_IdDermaga"))
                IdDermaga.AdvancedSearch.SearchOperator = Get("z_IdDermaga", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // Proses
                if (Proses2.UseFilter && !Empty(Proses2.AdvancedSearch.SearchValue)) {
                    Proses2.EditValue = ConvertToString(Proses2.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // NomorVoyage
                if (NomorVoyage.UseFilter && !Empty(NomorVoyage.AdvancedSearch.SearchValue)) {
                    NomorVoyage.EditValue = ConvertToString(NomorVoyage.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // NomorPengiriman
                if (NomorPengiriman.UseFilter && !Empty(NomorPengiriman.AdvancedSearch.SearchValue)) {
                    NomorPengiriman.EditValue = ConvertToString(NomorPengiriman.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // ModaTransportasi
                if (ModaTransportasi.UseFilter && !Empty(ModaTransportasi.AdvancedSearch.SearchValue)) {
                    ModaTransportasi.EditValue = ConvertToString(ModaTransportasi.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // NamaKapal
                if (NamaKapal.UseFilter && !Empty(NamaKapal.AdvancedSearch.SearchValue)) {
                    NamaKapal.EditValue = ConvertToString(NamaKapal.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // Refinery
                if (Refinery.UseFilter && !Empty(Refinery.AdvancedSearch.SearchValue)) {
                    Refinery.EditValue = ConvertToString(Refinery.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // PenyaluranLookup
                PenyaluranLookup.SetupEditAttributes();
                PenyaluranLookup.EditValue = PenyaluranLookup.AdvancedSearch.SearchValue;
                PenyaluranLookup.PlaceHolder = RemoveHtml(PenyaluranLookup.Caption);

                // TanggalSandar
                if (TanggalSandar.UseFilter && !Empty(TanggalSandar.AdvancedSearch.SearchValue)) {
                    TanggalSandar.EditValue = ConvertToString(TanggalSandar.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // IdDermaga
                if (IdDermaga.UseFilter && !Empty(IdDermaga.AdvancedSearch.SearchValue)) {
                    IdDermaga.EditValue = ConvertToString(IdDermaga.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // IdTemplate
                if (IdTemplate.UseFilter && !Empty(IdTemplate.AdvancedSearch.SearchValue)) {
                    IdTemplate.EditValue = ConvertToString(IdTemplate.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fPenerimaansrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"Penerimaan\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("PenerimaanSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("PenerimaanSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
