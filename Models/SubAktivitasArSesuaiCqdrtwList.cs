...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasArSesuaiCqdrtwList
    /// </summary>
    public static SubAktivitasArSesuaiCqdrtwList subAktivitasArSesuaiCqdrtwList
    {
        get => HttpData.Get<SubAktivitasArSesuaiCqdrtwList>("subAktivitasArSesuaiCqdrtwList")!;
        set => HttpData["subAktivitasArSesuaiCqdrtwList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasARSesuaiCQDRTW
    /// </summary>
    public class SubAktivitasArSesuaiCqdrtwList : SubAktivitasArSesuaiCqdrtwListBase
    {
        // Constructor
        public SubAktivitasArSesuaiCqdrtwList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasArSesuaiCqdrtwList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasArSesuaiCqdrtwListBase : SubAktivitasArSesuaiCqdrtw
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasArSesuaiCqdrtwList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasARSesuaiCQDRTWlist";
        ...
        // Constructor
        public SubAktivitasArSesuaiCqdrtwListBase(Controller? controller)
        {
            TableName = "SubAktivitasARSesuaiCQDRTW";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasArSesuaiCqdrtw)
            if (subAktivitasArSesuaiCqdrtw == null || subAktivitasArSesuaiCqdrtw is SubAktivitasArSesuaiCqdrtw)
                subAktivitasArSesuaiCqdrtw = this;

            // Initialize URLs
            AddUrl = "SubAktivitasArSesuaiCqdrtwAdd";
            ...
            MultiDeleteUrl = "SubAktivitasArSesuaiCqdrtwDelete";
            MultiUpdateUrl = "SubAktivitasArSesuaiCqdrtwUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasArSesuaiCqdrtwList";
        ...
        // Constructor
        public SubAktivitasArSesuaiCqdrtwListBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                var result = new Dictionary<string, string> { { "url", GetUrl(url) }, { "modal", "1" } }; // modal=1
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasArSesuaiCqdrtwView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void PrepareInitialSettings()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasARSesuaiCQDRTWgrid";
        ...
        private void PageRunEnd()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasArSesuaiCqdrtwList?.PageRender();
            }
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasARSesuaiCQDRTW"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasARSesuaiCQDRTWlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasARSesuaiCQDRTWlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasARSesuaiCQDRTWsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasARSesuaiCQDRTWsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasARSesuaiCQDRTWlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_SubAktivitasARSesuaiCQDRTW");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasArSesuaiCqdrtwList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasArSesuaiCqdrtwList.RowCount) + "_SubAktivitasARSesuaiCQDRTW");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasArSesuaiCqdrtwList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasArSesuaiCqdrtwList.RowType == RowType.Add || IsEdit && subAktivitasArSesuaiCqdrtwList.RowType == RowType.Edit) // Inline-Add/Edit row
