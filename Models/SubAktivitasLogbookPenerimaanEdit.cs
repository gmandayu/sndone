...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasLogbookPenerimaanEdit
    /// </summary>
    public static SubAktivitasLogbookPenerimaanEdit subAktivitasLogbookPenerimaanEdit
    {
        get => HttpData.Get<SubAktivitasLogbookPenerimaanEdit>("subAktivitasLogbookPenerimaanEdit")!;
        set => HttpData["subAktivitasLogbookPenerimaanEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasLogbookPenerimaan
    /// </summary>
    public class SubAktivitasLogbookPenerimaanEdit : SubAktivitasLogbookPenerimaanEditBase
    {
        // Constructor
        public SubAktivitasLogbookPenerimaanEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasLogbookPenerimaanEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasLogbookPenerimaanEditBase : SubAktivitasLogbookPenerimaan
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasLogbookPenerimaanEdit";
        ...
        // Constructor
        public SubAktivitasLogbookPenerimaanEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasLogbookPenerimaanEdit";
        ...
        // Constructor
        public SubAktivitasLogbookPenerimaanEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasLogbookPenerimaanList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasLogbookPenerimaanList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasLogbookPenerimaanList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasLogbookPenerimaanEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomNama_Kapal() {
            if (Nama_Kapal.Visible && Nama_Kapal.Required) {
                if (!Nama_Kapal.IsDetailKey && Empty(Nama_Kapal.FormValue)) {
                    Nama_Kapal.AddErrorMessage(ConvertToString(IsQualityActive.RequiredErrorMessage).Replace("%s", IsQualityActive.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTanggalJam() {
            if (TanggalJam.Visible && TanggalJam.Required) {
                if (!TanggalJam.IsDetailKey && Empty(TanggalJam.FormValue)) {
                    TanggalJam.AddErrorMessage(ConvertToString(IsQualityActive.RequiredErrorMessage).Replace("%s", IsQualityActive.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNama_Kegiatan() {
            if (Nama_Kegiatan.Visible && Nama_Kegiatan.Required) {
                if (!Nama_Kegiatan.IsDetailKey && Empty(Nama_Kegiatan.FormValue)) {
                    Nama_Kegiatan.AddErrorMessage(ConvertToString(IsQualityActive.RequiredErrorMessage).Replace("%s", IsQualityActive.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(IsQualityActive.RequiredErrorMessage).Replace("%s", IsQualityActive.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTemperatur() {
            if (Temperatur.Visible && Temperatur.Required) {
                if (!Temperatur.IsDetailKey && Empty(Temperatur.FormValue)) {
                    Temperatur.AddErrorMessage(ConvertToString(IsQualityActive.RequiredErrorMessage).Replace("%s", IsQualityActive.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSubAktivitasHasilPemeriksaanId() {
            if (SubAktivitasHasilPemeriksaanId.Visible && SubAktivitasHasilPemeriksaanId.Required) {
                if (!SubAktivitasHasilPemeriksaanId.IsDetailKey && Empty(SubAktivitasHasilPemeriksaanId.FormValue)) {
                    SubAktivitasHasilPemeriksaanId.AddErrorMessage(ConvertToString(IsQualityActive.RequiredErrorMessage).Replace("%s", IsQualityActive.Caption));
                }
            }
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasLogbookPenerimaanList")), "", TableVar, true);
            string pageId = "edit";
