...
public partial class SnDOne {
    ...
    public class SetUserModListBase : SetUserMod
    {
        ...
        // Constructor
        public SetUserModListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected bool RestoreFilterList() {
            ...
            // Field LookupUserMod
            if (filter?.TryGetValue("x_LookupUserMod", out sv) ?? false) {
                LookupUserMod.AdvancedSearch.SearchValue = sv;
                LookupUserMod.AdvancedSearch.SearchOperator = filter["z_LookupUserMod"];
                LookupUserMod.AdvancedSearch.SearchCondition = filter["v_LookupUserMod"];
                LookupUserMod.AdvancedSearch.SearchValue2 = filter["y_LookupUserMod"];
                LookupUserMod.AdvancedSearch.SearchOperator2 = filter["w_LookupUserMod"];
                LookupUserMod.AdvancedSearch.Save();
            }
        ...
        public void ShowFilterList()
        {
            ...
            // Field UserMOD
            filter = QueryBuilderWhere("UserMOD");
            if (Empty(filter))
                BuildSearchSql(ref filter, UserMOD, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + UserMOD.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected void LoadSearchValues() {
            ...
            // LookupUserMod
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_LookupUserMod[]"))
            ...
            // UserMOD
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_UserMOD[]"))
