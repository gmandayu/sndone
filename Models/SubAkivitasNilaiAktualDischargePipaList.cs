...
public partial class SnDOne {
    ...
    public class SubAkivitasNilaiAktualDischargePipaListBase : SubAkivitasNilaiAktualDischargePipa
    {
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaListBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAkivitasNilaiAktualDischargePipaListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            if (filter.TryGetValue("x_AktualDischarge_klobs", out sv)) {
                RestoreFieldFilter(AktualDischarge_klobs, filter, "AktualDischarge_klobs", sv);
            }

            // Field AktualDischarge_kl15
            if (filter.TryGetValue("x_AktualDischarge_kl15", out sv)) {
                RestoreFieldFilter(AktualDischarge_kl15, filter, "AktualDischarge_kl15", sv);
            }

            // Field AktualDischarge_barrels
            if (filter.TryGetValue("x_AktualDischarge_barrels", out sv)) {
                RestoreFieldFilter(AktualDischarge_barrels, filter, "AktualDischarge_barrels", sv);
            }

            // Field AktualDischarge_lt
            if (filter.TryGetValue("x_AktualDischarge_lt", out sv)) {
                RestoreFieldFilter(AktualDischarge_lt, filter, "AktualDischarge_lt", sv);
            }

            // Field AktualDischarge_mt
            if (filter.TryGetValue("x_AktualDischarge_mt", out sv)) {
                RestoreFieldFilter(AktualDischarge_mt, filter, "AktualDischarge_mt", sv);
            }
