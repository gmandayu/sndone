...
public partial class SnDOne {
    ...
    public class PenerimaanEditBase : Penerimaan
    {
        ...
        // Constructor
        public PenerimaanEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdPenerimaan", out rv)) { // DN
                    IdPenerimaan.FormValue = UrlDecode(rv); // DN
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("PenerimaanList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = "PenerimaanList";
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NomorPenerimaan' before field var 'x_NomorPenerimaan'
            val = CurrentForm.HasValue("NomorPenerimaan") ? CurrentForm.GetValue("NomorPenerimaan") : CurrentForm.GetValue("x_NomorPenerimaan");
            if (!NomorPenerimaan.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NomorPenerimaan") && !CurrentForm.HasValue("x_NomorPenerimaan")) // DN
                    NomorPenerimaan.Visible = false; // Disable update for API request
                else
                    NomorPenerimaan.SetFormValue(val);
            }
            ...
            // Check field name 'PenyaluranLookup' before field var 'x_PenyaluranLookup'
            val = CurrentForm.HasValue("PenyaluranLookup") ? CurrentForm.GetValue("PenyaluranLookup") : CurrentForm.GetValue("x_PenyaluranLookup");
            if (!PenyaluranLookup.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("PenyaluranLookup") && !CurrentForm.HasValue("x_PenyaluranLookup")) // DN
                    PenyaluranLookup.Visible = false; // Disable update for API request
                else
                    PenyaluranLookup.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                if (NomorPenerimaan.Visible && NomorPenerimaan.Required) {
                    if (!NomorPenerimaan.IsDetailKey && Empty(NomorPenerimaan.FormValue)) {
                        NomorPenerimaan.AddErrorMessage(ConvertToString(NomorPenerimaan.RequiredErrorMessage).Replace("%s", NomorPenerimaan.Caption));
                    }
                }
                ...
                if (PenyaluranLookup.Visible && PenyaluranLookup.Required) {
                    if (!PenyaluranLookup.IsDetailKey && Empty(PenyaluranLookup.FormValue)) {
                        PenyaluranLookup.AddErrorMessage(ConvertToString(PenyaluranLookup.RequiredErrorMessage).Replace("%s", PenyaluranLookup.Caption));
                    }
                }
