...
public partial class SnDOne {
    /// <summary>
    /// penimbunanPenyaluranSearch
    /// </summary>
    public static PenimbunanPenyaluranSearch penimbunanPenyaluranSearch
    {
        get => HttpData.Get<PenimbunanPenyaluranSearch>("penimbunanPenyaluranSearch")!;
        set => HttpData["penimbunanPenyaluranSearch"] = value;
    }

    /// <summary>
    /// Page class for PenimbunanPenyaluran
    /// </summary>
    public class PenimbunanPenyaluranSearch : PenimbunanPenyaluranSearchBase
    {
        // Constructor
        public PenimbunanPenyaluranSearch(Controller controller) : base(controller)
        {
        }

        // Constructor
        public PenimbunanPenyaluranSearch() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class PenimbunanPenyaluranSearchBase : PenimbunanPenyaluran
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "penimbunanPenyaluranSearch";
        ...
        // Constructor
        public PenimbunanPenyaluranSearchBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "PenimbunanPenyaluranSearch";
        ...
        // Constructor
        public PenimbunanPenyaluranSearchBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsSearch) {
                ...
                if (!Empty(srchStr)) {
                    srchStr = "PenimbunanPenyaluranList?" + srchStr;
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        penimbunanPenyaluranSearch?.PageRender();
                    }
        }
        ...
        protected string BuildAdvancedSearch() {
            string srchUrl = "";
            BuildSearchUrl(ref srchUrl, IdModa); // IdModa
            BuildSearchUrl(ref srchUrl, LookupNoPenyaluran); // LookupNoPenyaluran
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            if (!IsAddOrEdit && Form.ContainsKey("x_LookupNoPenyaluran")) {
                LookupNoPenyaluran.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_LookupNoPenyaluran");
            }
        }

        private void LoadSearchOperatorLookupNoPenyaluran()
        {
            if (Form.ContainsKey("z_LookupNoPenyaluran"))
                LookupNoPenyaluran.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_LookupNoPenyaluran");
        }
        ...
        {
            if (!IsAddOrEdit && Form.ContainsKey("x_NoPenyaluran")) {
                NoPenyaluran.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_NoPenyaluran", Config.FilterOptionSeparator);
            }
        }

        private void LoadSearchOperatorNoPenyaluran()
        {
            if (Form.ContainsKey("z_NoPenyaluran"))
                NoPenyaluran.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_NoPenyaluran", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                string curVal6 = ConvertToString(IdModa.AdvancedSearch.SearchValue);
                if (IdModa.Lookup != null && IsDictionary(IdModa.Lookup?.Options) && IdModa.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    ...
                    if (curVal6 == "") {
                        ...
                        filterWrk6 = SearchFilter(IdModa.Lookup?.GetTable()?.Fields["IdModa"].SearchExpression, "=", IdModa.AdvancedSearch.SearchValue, IdModa.Lookup?.GetTable()?.Fields["IdModa"].SearchDataType, "");
                    }
