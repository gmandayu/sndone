...
public partial class SnDOne {
    ...
    public class SubaktivitasNilaiLoadedFigurePipaEditBase : SubaktivitasNilaiLoadedFigurePipa
    {
        ...
        // Constructor
        public SubaktivitasNilaiLoadedFigurePipaEditBase()
        {
        ...
        // Constructor
        public SubaktivitasNilaiLoadedFigurePipaEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubaktivitasNilaiLoadedFigurePipaList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubaktivitasNilaiLoadedFigurePipaList") {
                                ...
                                returnUrl = "SubaktivitasNilaiLoadedFigurePipaList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'klobs' before field var 'x_klobs'
            val = CurrentForm.HasValue("klobs") ? CurrentForm.GetValue("klobs") : CurrentForm.GetValue("x_klobs");
            if (!klobs.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("klobs") && !CurrentForm.HasValue("x_klobs")) // DN
                    klobs.Visible = false; // Disable update for API request
                else
                    klobs.SetFormValue(val);
            }

            // Check field name 'kl15' before field var 'x_kl15'
            val = CurrentForm.HasValue("kl15") ? CurrentForm.GetValue("kl15") : CurrentForm.GetValue("x_kl15");
            if (!kl15.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("kl15") && !CurrentForm.HasValue("x_kl15")) // DN
                    kl15.Visible = false; // Disable update for API request
                else
                    kl15.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (klobs.Visible && klobs.Required) {
                    if (!klobs.IsDetailKey && Empty(klobs.FormValue)) {
                        klobs.AddErrorMessage(ConvertToString(klobs.RequiredErrorMessage).Replace("%s", klobs.Caption));
                    }
                }
                if (kl15.Visible && kl15.Required) {
                    if (!kl15.IsDetailKey && Empty(kl15.FormValue)) {
                        kl15.AddErrorMessage(ConvertToString(kl15.RequiredErrorMessage).Replace("%s", kl15.Caption));
                    }
                }
