...
public partial class SnDOne {
    /// <summary>
    /// templateAktivitasDokumenList
    /// </summary>
    public static TemplateAktivitasDokumenList templateAktivitasDokumenList
    {
        get => HttpData.Get<TemplateAktivitasDokumenList>("templateAktivitasDokumenList")!;
        set => HttpData["templateAktivitasDokumenList"] = value;
    }

    /// <summary>
    /// Page class for TemplateAktivitasDokumen
    /// </summary>
    public class TemplateAktivitasDokumenList : TemplateAktivitasDokumenListBase
    {
        // Constructor
        public TemplateAktivitasDokumenList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TemplateAktivitasDokumenList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TemplateAktivitasDokumenListBase : TemplateAktivitasDokumen
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "templateAktivitasDokumenList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fTemplateAktivitasDokumenlist";
        ...
        // Constructor
        public TemplateAktivitasDokumenListBase()
        {
            ...
            // Table object (templateAktivitasDokumen)
            if (templateAktivitasDokumen == null || templateAktivitasDokumen is TemplateAktivitasDokumen)
                templateAktivitasDokumen = this;
            ...
            MultiDeleteUrl = "TemplateAktivitasDokumenDelete";
            MultiUpdateUrl = "TemplateAktivitasDokumenUpdate";
        ...
        // Page name
        public string PageName => "TemplateAktivitasDokumenList";
        ...
        // Constructor
        public TemplateAktivitasDokumenListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "TemplateAktivitasDokumenView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                templateAktivitasDokumenList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public void ShowFilterList()
        {
            ...
            // Field IdTemplateAktivitas
            filter = QueryBuilderWhere("IdTemplateAktivitas");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdTemplateAktivitas, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdTemplateAktivitas.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field IdDokumen
            filter = QueryBuilderWhere("IdDokumen");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdDokumen, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdDokumen.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field WajibUpload
            filter = QueryBuilderWhere("WajibUpload");
            if (Empty(filter))
                BuildSearchSql(ref filter, WajibUpload, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + WajibUpload.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(IdTemplateAktivitas, ctrl); // IdTemplateAktivitas
                UpdateSort(IdDokumen, ctrl); // IdDokumen
        ...
        public async Task RenderListOptions()
        {
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fTemplateAktivitasDokumenlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTemplateAktivitasDokumenlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdTemplateAktivitasDokumen.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fTemplateAktivitasDokumenlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("IdTemplateAktivitas")); // DN
                CreateColumnOption(option.Add("IdDokumen")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fTemplateAktivitasDokumensrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fTemplateAktivitasDokumensrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fTemplateAktivitasDokumenlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(templateAktivitasDokumenList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(templateAktivitasDokumenList.RowCount) + "_TemplateAktivitasDokumen");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(templateAktivitasDokumenList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && templateAktivitasDokumenList.RowType == RowType.Add || IsEdit && templateAktivitasDokumenList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // WajibUpload
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_WajibUpload[]"))
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // IdTemplateAktivitas
                if (IdTemplateAktivitas.UseFilter && !Empty(IdTemplateAktivitas.AdvancedSearch.SearchValue)) {
                    IdTemplateAktivitas.EditValue = ConvertToString(IdTemplateAktivitas.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // IdDokumen
                if (IdDokumen.UseFilter && !Empty(IdDokumen.AdvancedSearch.SearchValue)) {
                    IdDokumen.EditValue = ConvertToString(IdDokumen.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fTemplateAktivitasDokumenlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fTemplateAktivitasDokumenlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fTemplateAktivitasDokumenlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fTemplateAktivitasDokumensrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
        ...
        public async Task ExportData(dynamic? doc)
        {
            ...
            if (Config.ExportMasterRecord && !Empty(MasterFilterFromSession) && CurrentMasterTable == "TemplateAktivitas") {
                templateAktivitas = new TemplateAktivitasList();
                if (templateAktivitas != null) {
                    var c = await GetConnection2Async(templateAktivitas.DbId); // Note: Use new connection for master record // DN
                    using var rsmaster = await c.ExecuteReaderAsync(templateAktivitas.GetSql(DbMasterFilter)); // Load master record
                    if (rsmaster?.HasRows ?? false) { // DN
                        exportStyle = doc.Style;
                        doc.SetStyle("v"); // Change to vertical
                        if (!IsExport("csv") || Config.ExportMasterRecordForCsv) {
                            doc.Table = templateAktivitas;
                            await templateAktivitas.ExportDocument(doc, rsmaster, 1, 1);
                            doc.ExportEmptyRow();
                            doc.Table = this;
                        }
        ...
        protected void SetupMasterParms() {
            ...
            if (Query.TryGetValue(Config.TableShowMaster, out masterTblVar) || Query.TryGetValue(Config.TableMaster, out masterTblVar)) { // Do not use Get()
                ...
                if (masterTblVar == "TemplateAktivitas") {
                    validMaster = true;
                    if (templateAktivitas != null && (Get("fk_IdTemplateAktivitas", out fk) || Get("IdTemplateAktivitas", out fk))) {
                        templateAktivitas.IdTemplateAktivitas.QueryValue = fk;
                        IdTemplateAktivitas.QueryValue = templateAktivitas.IdTemplateAktivitas.QueryValue;
                        IdTemplateAktivitas.SessionValue = IdTemplateAktivitas.QueryValue;
                        foreignKeys.Add("IdTemplateAktivitas", fk);
                        if (!IsNumeric(IdTemplateAktivitas.QueryValue))
                            validMaster = false;
                    } else {
                        validMaster = false;
                    }
                ...
                if (masterTblVar == "TemplateAktivitas") {
                    validMaster = true;
                    if (templateAktivitas != null && (Post("fk_IdTemplateAktivitas", out fk) || Post("IdTemplateAktivitas", out fk))) {
                        templateAktivitas.IdTemplateAktivitas.FormValue = fk;
                        IdTemplateAktivitas.FormValue = templateAktivitas.IdTemplateAktivitas.FormValue;
                        IdTemplateAktivitas.SessionValue = IdTemplateAktivitas.FormValue;
                        foreignKeys.Add("IdTemplateAktivitas", fk);
                        if (!IsNumeric(IdTemplateAktivitas.FormValue))
                            validMaster = false;
                    } else {
                        validMaster = false;
                    }
            ...
            if (validMaster) {
                ...
                if (masterTblVar != "TemplateAktivitas") {
                    if (!foreignKeys.ContainsKey("IdTemplateAktivitas")) // Not current foreign key
                        IdTemplateAktivitas.SessionValue = "";
                }
            }
