...
public partial class SnDOne {
    ...
    public class ClosingStockListBase : ClosingStock
    {
        ...
        // Constructor
        public ClosingStockListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected bool RestoreFilterList() {
            ...
            if (filter?.TryGetValue("x_TanggalClosingStock", out sv) ?? false) {
                TanggalClosingStock.AdvancedSearch.SearchValue = sv;
                TanggalClosingStock.AdvancedSearch.SearchOperator = filter["z_TanggalClosingStock"];
                TanggalClosingStock.AdvancedSearch.SearchCondition = filter["v_TanggalClosingStock"];
                TanggalClosingStock.AdvancedSearch.SearchValue2 = filter["y_TanggalClosingStock"];
                TanggalClosingStock.AdvancedSearch.SearchOperator2 = filter["w_TanggalClosingStock"];
                TanggalClosingStock.AdvancedSearch.Save();
            }
        ...
        public void ShowFilterList()
        {
            ...
            // Field TanggalClosingStock
            filter = QueryBuilderWhere("TanggalClosingStock");
            if (Empty(filter))
                BuildSearchSql(ref filter, TanggalClosingStock, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TanggalClosingStock.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Produk
            filter = QueryBuilderWhere("Produk");
            if (Empty(filter))
                BuildSearchSql(ref filter, Produk, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Produk.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected void LoadSearchValues() {
            ...
            // NomorClosingStock
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NomorClosingStock[]"))
            ...
            // TanggalClosingStock
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TanggalClosingStock"))
                    TanggalClosingStock.AdvancedSearch.SearchValue = Get("x_TanggalClosingStock", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z_TanggalClosingStock"))
                TanggalClosingStock.AdvancedSearch.SearchOperator = Get("z_TanggalClosingStock", Config.FilterOptionSeparator);
