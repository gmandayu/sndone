...
public partial class SnDOne {
    ...
    public class SubAktivitasFormInputBLnSfalrtwListBase : SubAktivitasFormInputBLnSfalrtw
    {
        ...
        // Constructor
        public SubAktivitasFormInputBLnSfalrtwListBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasFormInputBLnSfalrtwListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field BL_KLObs
            if (filter.TryGetValue("x_BL_KLObs", out sv)) {
                RestoreFieldFilter(BL_KLObs, filter, "BL_KLObs", sv);
            }

            // Field BL_KL15
            if (filter.TryGetValue("x_BL_KL15", out sv)) {
                RestoreFieldFilter(BL_KL15, filter, "BL_KL15", sv);
            }

            // Field BL_Barrels
            if (filter.TryGetValue("x_BL_Barrels", out sv)) {
                RestoreFieldFilter(BL_Barrels, filter, "BL_Barrels", sv);
            }

            // Field BL_LT
            if (filter.TryGetValue("x_BL_LT", out sv)) {
                RestoreFieldFilter(BL_LT, filter, "BL_LT", sv);
            }

            // Field BL_MT
            if (filter.TryGetValue("x_BL_MT", out sv)) {
                RestoreFieldFilter(BL_MT, filter, "BL_MT", sv);
            }

            // Field SFAL_KLObs
            if (filter.TryGetValue("x_SFAL_KLObs", out sv)) {
                RestoreFieldFilter(SFAL_KLObs, filter, "SFAL_KLObs", sv);
            }

            // Field SFAL_KL15
            if (filter.TryGetValue("x_SFAL_KL15", out sv)) {
                RestoreFieldFilter(SFAL_KL15, filter, "SFAL_KL15", sv);
            }

            // Field SFAL_Barrels
            if (filter.TryGetValue("x_SFAL_Barrels", out sv)) {
                RestoreFieldFilter(SFAL_Barrels, filter, "SFAL_Barrels", sv);
            }

            // Field SFAL_LT
            if (filter.TryGetValue("x_SFAL_LT", out sv)) {
                RestoreFieldFilter(SFAL_LT, filter, "SFAL_LT", sv);
            }

            // Field SFAL_MT
            if (filter.TryGetValue("x_SFAL_MT", out sv)) {
                RestoreFieldFilter(SFAL_MT, filter, "SFAL_MT", sv);
            }
