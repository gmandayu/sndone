...
public partial class SnDOne {
    ...
    public class SubAktivitasNilaiSfbdrtwListBase : SubAktivitasNilaiSfbdrtw
    {
        ...
        // Constructor
        public SubAktivitasNilaiSfbdrtwListBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasNilaiSfbdrtwListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field SFBD_KL_Obs
            if (filter.TryGetValue("x_SFBD_KL_Obs", out sv)) {
                RestoreFieldFilter(SFBD_KL_Obs, filter, "SFBD_KL_Obs", sv);
            }

            // Field SFBD_KL_15
            if (filter.TryGetValue("x_SFBD_KL_15", out sv)) {
                RestoreFieldFilter(SFBD_KL_15, filter, "SFBD_KL_15", sv);
            }

            // Field SFBD_Barrels
            if (filter.TryGetValue("x_SFBD_Barrels", out sv)) {
                RestoreFieldFilter(SFBD_Barrels, filter, "SFBD_Barrels", sv);
            }

            // Field SFBD_LT
            if (filter.TryGetValue("x_SFBD_LT", out sv)) {
                RestoreFieldFilter(SFBD_LT, filter, "SFBD_LT", sv);
            }

            // Field SFBD_MT
            if (filter.TryGetValue("x_SFBD_MT", out sv)) {
                RestoreFieldFilter(SFBD_MT, filter, "SFBD_MT", sv);
            }
