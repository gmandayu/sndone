...
public partial class SnDOne {
    /// <summary>
    /// formInputControlRutinSecurityList
    /// </summary>
    public static FormInputControlRutinSecurityList formInputControlRutinSecurityList
    {
        get => HttpData.Get<FormInputControlRutinSecurityList>("formInputControlRutinSecurityList")!;
        set => HttpData["formInputControlRutinSecurityList"] = value;
    }

    /// <summary>
    /// Page class for FormInputControlRutinSecurity
    /// </summary>
    public class FormInputControlRutinSecurityList : FormInputControlRutinSecurityListBase
    {
        // Constructor
        public FormInputControlRutinSecurityList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public FormInputControlRutinSecurityList() : base()
        {
        }
        ...
        public override void PageDataRendering(ref string header) {
            ...
            header += @"
                <style>
                /* khusus page LIST FormInputProofOfVisit */
                .ew-master-div,
                table.ew-master-table { display: none !important; }
                </style>";
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class FormInputControlRutinSecurityListBase : FormInputControlRutinSecurity
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "formInputControlRutinSecurityList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fFormInputControlRutinSecuritylist";
        ...
        // Constructor
        public FormInputControlRutinSecurityListBase()
        {
            TableName = "FormInputControlRutinSecurity";

            // CSS class name as context
            TableVar = "FormInputControlRutinSecurity";
            ...
            // Initialize URLs
            AddUrl = "FormInputControlRutinSecurityAdd";
            ...
            MultiDeleteUrl = "FormInputControlRutinSecurityDelete";
            MultiUpdateUrl = "FormInputControlRutinSecurityUpdate";
        ...
        // Page name
        public string PageName => "FormInputControlRutinSecurityList";
        ...
        // Constructor
        public FormInputControlRutinSecurityListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fFormInputControlRutinSecuritygrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                formInputControlRutinSecurityList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(NamaSecurity.AdvancedSearch.ToJson())); // Field NamaSecurity
            filters.Merge(JObject.Parse(Tanggal.AdvancedSearch.ToJson())); // Field Tanggal
            filters.Merge(JObject.Parse(Lokasi.AdvancedSearch.ToJson())); // Field Lokasi
            filters.Merge(JObject.Parse(DownloadDokumen.AdvancedSearch.ToJson())); // Field DownloadDokumen
            filters.Merge(JObject.Parse(UploadFoto.AdvancedSearch.ToJson())); // Field UploadFoto
            filters.Merge(JObject.Parse(Ketidaksesuaiaan.AdvancedSearch.ToJson())); // Field Ketidaksesuaiaan
            filters.Merge(JObject.Parse(Keterangan.AdvancedSearch.ToJson())); // Field Keterangan
            filters.Merge(JObject.Parse(IdPosition.AdvancedSearch.ToJson())); // Field IdPosition
        ...
        protected bool RestoreFilterList() {
            ...
            // Field NamaSecurity
            if (filter?.TryGetValue("x_NamaSecurity", out sv) ?? false) {
                NamaSecurity.AdvancedSearch.SearchValue = sv;
                NamaSecurity.AdvancedSearch.SearchOperator = filter["z_NamaSecurity"];
                NamaSecurity.AdvancedSearch.SearchCondition = filter["v_NamaSecurity"];
                NamaSecurity.AdvancedSearch.SearchValue2 = filter["y_NamaSecurity"];
                NamaSecurity.AdvancedSearch.SearchOperator2 = filter["w_NamaSecurity"];
                NamaSecurity.AdvancedSearch.Save();
            }
            ...
            // Field Lokasi
            if (filter?.TryGetValue("x_Lokasi", out sv) ?? false) {
                Lokasi.AdvancedSearch.SearchValue = sv;
                Lokasi.AdvancedSearch.SearchOperator = filter["z_Lokasi"];
                Lokasi.AdvancedSearch.SearchCondition = filter["v_Lokasi"];
                Lokasi.AdvancedSearch.SearchValue2 = filter["y_Lokasi"];
                Lokasi.AdvancedSearch.SearchOperator2 = filter["w_Lokasi"];
                Lokasi.AdvancedSearch.Save();
            }

            // Field DownloadDokumen
            if (filter?.TryGetValue("x_DownloadDokumen", out sv) ?? false) {
                DownloadDokumen.AdvancedSearch.SearchValue = sv;
                DownloadDokumen.AdvancedSearch.SearchOperator = filter["z_DownloadDokumen"];
                DownloadDokumen.AdvancedSearch.SearchCondition = filter["v_DownloadDokumen"];
                DownloadDokumen.AdvancedSearch.SearchValue2 = filter["y_DownloadDokumen"];
                DownloadDokumen.AdvancedSearch.SearchOperator2 = filter["w_DownloadDokumen"];
                DownloadDokumen.AdvancedSearch.Save();
            }

            // Field UploadFoto
            if (filter?.TryGetValue("x_UploadFoto", out sv) ?? false) {
                UploadFoto.AdvancedSearch.SearchValue = sv;
                UploadFoto.AdvancedSearch.SearchOperator = filter["z_UploadFoto"];
                UploadFoto.AdvancedSearch.SearchCondition = filter["v_UploadFoto"];
                UploadFoto.AdvancedSearch.SearchValue2 = filter["y_UploadFoto"];
                UploadFoto.AdvancedSearch.SearchOperator2 = filter["w_UploadFoto"];
                UploadFoto.AdvancedSearch.Save();
            }

            // Field Ketidaksesuaiaan
            if (filter?.TryGetValue("x_Ketidaksesuaiaan", out sv) ?? false) {
                Ketidaksesuaiaan.AdvancedSearch.SearchValue = sv;
                Ketidaksesuaiaan.AdvancedSearch.SearchOperator = filter["z_Ketidaksesuaiaan"];
                Ketidaksesuaiaan.AdvancedSearch.SearchCondition = filter["v_Ketidaksesuaiaan"];
                Ketidaksesuaiaan.AdvancedSearch.SearchValue2 = filter["y_Ketidaksesuaiaan"];
                Ketidaksesuaiaan.AdvancedSearch.SearchOperator2 = filter["w_Ketidaksesuaiaan"];
                Ketidaksesuaiaan.AdvancedSearch.Save();
            }

            // Field Keterangan
            if (filter?.TryGetValue("x_Keterangan", out sv) ?? false) {
                Keterangan.AdvancedSearch.SearchValue = sv;
                Keterangan.AdvancedSearch.SearchOperator = filter["z_Keterangan"];
                Keterangan.AdvancedSearch.SearchCondition = filter["v_Keterangan"];
                Keterangan.AdvancedSearch.SearchValue2 = filter["y_Keterangan"];
                Keterangan.AdvancedSearch.SearchOperator2 = filter["w_Keterangan"];
                Keterangan.AdvancedSearch.Save();
            }

            // Field IdPosition
            if (filter?.TryGetValue("x_IdPosition", out sv) ?? false) {
                IdPosition.AdvancedSearch.SearchValue = sv;
                IdPosition.AdvancedSearch.SearchOperator = filter["z_IdPosition"];
                IdPosition.AdvancedSearch.SearchCondition = filter["v_IdPosition"];
                IdPosition.AdvancedSearch.SearchValue2 = filter["y_IdPosition"];
                IdPosition.AdvancedSearch.SearchOperator2 = filter["w_IdPosition"];
                IdPosition.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, NamaSecurity, def, true); // NamaSecurity
            BuildSearchSql(ref where, Tanggal, def, true); // Tanggal
            BuildSearchSql(ref where, Lokasi, def, true); // Lokasi
            BuildSearchSql(ref where, DownloadDokumen, def, false); // DownloadDokumen
            BuildSearchSql(ref where, UploadFoto, def, false); // UploadFoto
            BuildSearchSql(ref where, Ketidaksesuaiaan, def, false); // Ketidaksesuaiaan
            BuildSearchSql(ref where, Keterangan, def, true); // Keterangan
            BuildSearchSql(ref where, IdPosition, def, true); // IdPosition
            BuildSearchSql(ref where, UserInput, def, true); // UserInput
            ...
            if (!def && Command == "search") {
                ...
                NoReferensi.AdvancedSearch.Save(); // NoReferensi
                NamaSecurity.AdvancedSearch.Save(); // NamaSecurity
                Tanggal.AdvancedSearch.Save(); // Tanggal
                Lokasi.AdvancedSearch.Save(); // Lokasi
                DownloadDokumen.AdvancedSearch.Save(); // DownloadDokumen
                UploadFoto.AdvancedSearch.Save(); // UploadFoto
                Ketidaksesuaiaan.AdvancedSearch.Save(); // Ketidaksesuaiaan
                Keterangan.AdvancedSearch.Save(); // Keterangan
                IdPosition.AdvancedSearch.Save(); // IdPosition
                UserInput.AdvancedSearch.Save(); // UserInput
        ...
        public void ShowFilterList()
        {
            ...
            // Field NoReferensi
            filter = QueryBuilderWhere("NoReferensi");
            if (Empty(filter))
                BuildSearchSql(ref filter, NoReferensi, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NoReferensi.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field NamaSecurity
            filter = QueryBuilderWhere("NamaSecurity");
            if (Empty(filter))
                BuildSearchSql(ref filter, NamaSecurity, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NamaSecurity.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field Lokasi
            filter = QueryBuilderWhere("Lokasi");
            if (Empty(filter))
                BuildSearchSql(ref filter, Lokasi, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Lokasi.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field DownloadDokumen
            filter = QueryBuilderWhere("DownloadDokumen");
            if (Empty(filter))
                BuildSearchSql(ref filter, DownloadDokumen, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DownloadDokumen.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Ketidaksesuaiaan
            filter = QueryBuilderWhere("Ketidaksesuaiaan");
            if (Empty(filter))
                BuildSearchSql(ref filter, Ketidaksesuaiaan, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Ketidaksesuaiaan.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field IdPosition
            filter = QueryBuilderWhere("IdPosition");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdPosition, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdPosition.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field UserInput
            filter = QueryBuilderWhere("UserInput");
            if (Empty(filter))
                BuildSearchSql(ref filter, UserInput, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + UserInput.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NamaSecurity);
            searchFlds.Add(Lokasi);
            searchFlds.Add(DownloadDokumen);
            searchFlds.Add(UploadFoto);
            searchFlds.Add(Ketidaksesuaiaan);
        ...
        protected bool CheckSearchParms() {
            ...
            if (NoReferensi.AdvancedSearch.IssetSession)
                return true;
            if (NamaSecurity.AdvancedSearch.IssetSession)
                return true;
            if (Tanggal.AdvancedSearch.IssetSession)
                return true;
            if (Lokasi.AdvancedSearch.IssetSession)
                return true;
            if (DownloadDokumen.AdvancedSearch.IssetSession)
                return true;
            if (UploadFoto.AdvancedSearch.IssetSession)
                return true;
            if (Ketidaksesuaiaan.AdvancedSearch.IssetSession)
                return true;
            if (Keterangan.AdvancedSearch.IssetSession)
                return true;
            if (IdPosition.AdvancedSearch.IssetSession)
                return true;
            if (UserInput.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            NamaSecurity.AdvancedSearch.UnsetSession();
            Tanggal.AdvancedSearch.UnsetSession();
            Lokasi.AdvancedSearch.UnsetSession();
            DownloadDokumen.AdvancedSearch.UnsetSession();
            UploadFoto.AdvancedSearch.UnsetSession();
            Ketidaksesuaiaan.AdvancedSearch.UnsetSession();
            Keterangan.AdvancedSearch.UnsetSession();
            IdPosition.AdvancedSearch.UnsetSession();
            UserInput.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            NamaSecurity.AdvancedSearch.Load();
            Tanggal.AdvancedSearch.Load();
            Lokasi.AdvancedSearch.Load();
            DownloadDokumen.AdvancedSearch.Load();
            UploadFoto.AdvancedSearch.Load();
            Ketidaksesuaiaan.AdvancedSearch.Load();
            Keterangan.AdvancedSearch.Load();
            IdPosition.AdvancedSearch.Load();
            UserInput.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(NamaSecurity, ctrl); // NamaSecurity
                UpdateSort(Tanggal, ctrl); // Tanggal
                UpdateSort(Lokasi, ctrl); // Lokasi
                UpdateSort(DownloadDokumen, ctrl); // DownloadDokumen
                UpdateSort(Ketidaksesuaiaan, ctrl); // Ketidaksesuaiaan
                UpdateSort(Keterangan, ctrl); // Keterangan
                UpdateSort(IdPosition, ctrl); // IdPosition
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""FormInputControlRutinSecurity"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""FormInputControlRutinSecurity"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""FormInputControlRutinSecurity"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fFormInputControlRutinSecuritylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fFormInputControlRutinSecuritylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""FormInputControlRutinSecurity"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fFormInputControlRutinSecuritylist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("NamaSecurity")); // DN
                CreateColumnOption(option.Add("Tanggal")); // DN
                CreateColumnOption(option.Add("Lokasi")); // DN
                CreateColumnOption(option.Add("DownloadDokumen")); // DN
                CreateColumnOption(option.Add("Ketidaksesuaiaan")); // DN
                CreateColumnOption(option.Add("Keterangan")); // DN
                CreateColumnOption(option.Add("IdPosition")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fFormInputControlRutinSecuritysrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fFormInputControlRutinSecuritysrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fFormInputControlRutinSecuritylist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_FormInputControlRutinSecurity");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(formInputControlRutinSecurityList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(formInputControlRutinSecurityList.RowCount) + "_FormInputControlRutinSecurity");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(formInputControlRutinSecurityList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && formInputControlRutinSecurityList.RowType == RowType.Add || IsEdit && formInputControlRutinSecurityList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // NoReferensi
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NoReferensi[]"))
            ...
            // NamaSecurity
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaSecurity[]"))
                    NamaSecurity.AdvancedSearch.SearchValue = Get("x_NamaSecurity[]", Config.FilterOptionSeparator);
                else
                    NamaSecurity.AdvancedSearch.SearchValue = Get("NamaSecurity"); // Default Value // DN
            if (!Empty(NamaSecurity.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_NamaSecurity"))
                NamaSecurity.AdvancedSearch.SearchOperator = Get("z_NamaSecurity", Config.FilterOptionSeparator);
            ...
            // Lokasi
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Lokasi[]"))
                    Lokasi.AdvancedSearch.SearchValue = Get("x_Lokasi[]", Config.FilterOptionSeparator);
                else
                    Lokasi.AdvancedSearch.SearchValue = Get("Lokasi"); // Default Value // DN
            if (!Empty(Lokasi.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Lokasi"))
                Lokasi.AdvancedSearch.SearchOperator = Get("z_Lokasi", Config.FilterOptionSeparator);

            // DownloadDokumen
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DownloadDokumen"))
            ...
            // UploadFoto
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_UploadFoto"))
                    UploadFoto.AdvancedSearch.SearchValue = Get("x_UploadFoto", Config.FilterOptionSeparator);
                else
                    UploadFoto.AdvancedSearch.SearchValue = Get("UploadFoto"); // Default Value // DN
            if (!Empty(UploadFoto.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_UploadFoto"))
                UploadFoto.AdvancedSearch.SearchOperator = Get("z_UploadFoto", Config.FilterOptionSeparator);

            // Ketidaksesuaiaan
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Ketidaksesuaiaan"))
                    Ketidaksesuaiaan.AdvancedSearch.SearchValue = Get("x_Ketidaksesuaiaan", Config.FilterOptionSeparator);
                else
                    Ketidaksesuaiaan.AdvancedSearch.SearchValue = Get("Ketidaksesuaiaan"); // Default Value // DN
            if (!Empty(Ketidaksesuaiaan.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Ketidaksesuaiaan"))
                Ketidaksesuaiaan.AdvancedSearch.SearchOperator = Get("z_Ketidaksesuaiaan", Config.FilterOptionSeparator);
            ...
            // IdPosition
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdPosition[]"))
                    IdPosition.AdvancedSearch.SearchValue = Get("x_IdPosition[]", Config.FilterOptionSeparator);
                else
                    IdPosition.AdvancedSearch.SearchValue = Get("IdPosition"); // Default Value // DN
            if (!Empty(IdPosition.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_IdPosition"))
                IdPosition.AdvancedSearch.SearchOperator = Get("z_IdPosition", Config.FilterOptionSeparator);

            // UserInput
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_UserInput[]"))
                    UserInput.AdvancedSearch.SearchValue = Get("x_UserInput[]", Config.FilterOptionSeparator);
                else
                    UserInput.AdvancedSearch.SearchValue = Get("UserInput"); // Default Value // DN
            if (!Empty(UserInput.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_UserInput"))
                UserInput.AdvancedSearch.SearchOperator = Get("z_UserInput", Config.FilterOptionSeparator);
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // NamaSecurity
                if (NamaSecurity.UseFilter && !Empty(NamaSecurity.AdvancedSearch.SearchValue)) {
                    NamaSecurity.EditValue = ConvertToString(NamaSecurity.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // Lokasi
                if (Lokasi.UseFilter && !Empty(Lokasi.AdvancedSearch.SearchValue)) {
                    Lokasi.EditValue = ConvertToString(Lokasi.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // DownloadDokumen
                DownloadDokumen.SetupEditAttributes();
                DownloadDokumen.EditValue = DownloadDokumen.AdvancedSearch.SearchValue; // DN
                ...
                // IdPosition
                if (IdPosition.UseFilter && !Empty(IdPosition.AdvancedSearch.SearchValue)) {
                    IdPosition.EditValue = ConvertToString(IdPosition.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // UserInput
                if (UserInput.UseFilter && !Empty(UserInput.AdvancedSearch.SearchValue)) {
                    UserInput.EditValue = ConvertToString(UserInput.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fFormInputControlRutinSecuritylist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fFormInputControlRutinSecuritylist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fFormInputControlRutinSecuritylist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fFormInputControlRutinSecuritysrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
