...
public partial class SnDOne {
    ...
    /// masterPositionEdit
    /// </summary>
    public static MasterPositionEdit masterPositionEdit
    {
        get => HttpData.Get<MasterPositionEdit>("masterPositionEdit")!;
        set => HttpData["masterPositionEdit"] = value;
    }

    /// <summary>
    /// Page class for MasterPosition
    /// </summary>
    public class MasterPositionEdit : MasterPositionEditBase
    {
        // Constructor
        public MasterPositionEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterPositionEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterPositionEditBase : MasterPosition
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterPositionEdit";
        ...
        // Constructor
        public MasterPositionEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "MasterPositionEdit";
        ...
        // Constructor
        public MasterPositionEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdPosition", out rv)) {
                    IdPosition.FormValue = UrlDecode(rv);
                    IdPosition.OldValue = IdPosition.FormValue;
                } else if (CurrentForm.HasValue("x_IdPosition")) {
                    IdPosition.FormValue = CurrentForm.GetValue("x_IdPosition");
                    IdPosition.OldValue = IdPosition.FormValue;
                } else if (!Empty(keyValues)) {
                    IdPosition.OldValue = ConvertToString(keyValues[0]);
                } else {
                    loaded = false;
                }
                if (loaded)
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdPosition", out rv)) {
                        IdPosition.QueryValue = UrlDecode(rv);
                        loadByQuery = true;
                    } else if (Get("IdPosition", out sv)) {
                        IdPosition.QueryValue = sv.ToString();
                        loadByQuery = true;
                    } else {
                        IdPosition.CurrentValue = DbNullValue;
                    }
                }
                if (IsShow) {
                    loaded = await LoadRow();
        ...
        private async Task HandleApiFormKeys()
        {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                IdPosition.FormValue = ConvertToString(keyValues[0]);
            }
        }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("MasterPositionList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "MasterPositionList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "MasterPositionList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            if (CurrentForm.HasValue("o_IdPosition"))
                IdPosition.OldValue = CurrentForm.GetValue("o_IdPosition");

            // Standard handling for 'NamaPosition'
            NamaPosition.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NamaPosition, "NamaPosition", "x_NamaPosition");

            // Standard handling for 'Location'
            Location.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Location, "Location", "x_Location");
            ...
            // Standard handling for 'Role'
            Role.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Role, "Role", "x_Role");
        }
        #pragma warning restore 1998
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                NamaPosition.EditValue = HtmlEncode(NamaPosition.CurrentValue);
                ...
                if (Role.Lookup != null && IsDictionary(Role.Lookup?.Options) && Role.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    Role.EditValue = Role.Lookup?.Options.Values.ToList();
                } else { // Lookup from database
                    string filterWrk2 = "";
                    if (curVal2 == "") {
                        filterWrk2 = "0=1";
                    } else {
                        filterWrk2 = SearchFilter(Role.Lookup?.GetTable()?.Fields["IdPIC"].SearchExpression, "=", Role.CurrentValue, Role.Lookup?.GetTable()?.Fields["IdPIC"].SearchDataType, "");
                    }
                    string? sqlWrk2 = Role.Lookup?.GetSql(true, filterWrk2, null, this, false, true);
                    List<Dictionary<string, object>>? rswrk2 = sqlWrk2 != null ? Connection.GetRows(sqlWrk2) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                    Role.EditValue = rswrk2;
                }
        ...
        private void ValidateCustomIdPosition() {
            if (IdPosition.Visible && IdPosition.Required) {
                if (!IdPosition.IsDetailKey && Empty(IdPosition.FormValue)) {
                    IdPosition.AddErrorMessage(ConvertToString(Role.RequiredErrorMessage).Replace("%s", Role.Caption));
                }
            }
        }

        private void ValidateCustomNamaPosition() {
            if (NamaPosition.Visible && NamaPosition.Required) {
                if (!NamaPosition.IsDetailKey && Empty(NamaPosition.FormValue)) {
                    NamaPosition.AddErrorMessage(ConvertToString(Role.RequiredErrorMessage).Replace("%s", Role.Caption));
                }
            }
        }

        private void ValidateCustomLocation() {
            if (Location.Visible && Location.Required) {
                if (!Location.IsDetailKey && Empty(Location.FormValue)) {
                    Location.AddErrorMessage(ConvertToString(Role.RequiredErrorMessage).Replace("%s", Role.Caption));
                }
            }
        }
        ...
        private void ValidateCustom_UserLevel() {
            if (_UserLevel.Visible && _UserLevel.Required) {
                if (!_UserLevel.IsDetailKey && Empty(_UserLevel.FormValue)) {
                    _UserLevel.AddErrorMessage(ConvertToString(Role.RequiredErrorMessage).Replace("%s", Role.Caption));
                }
            }
        }

        private void ValidateCustomRole() {
            if (Role.Visible && Role.Required) {
                if (!Role.IsDetailKey && Empty(Role.FormValue)) {
                    Role.AddErrorMessage(ConvertToString(Role.RequiredErrorMessage).Replace("%s", Role.Caption));
                }
            }
        }
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // Check field with unique index (IdPosition)
            if (!Empty(IdPosition.CurrentValue)) {
                string filterChk = "([IdPosition] = " + AdjustSql(IdPosition.CurrentValue, DbId) + ")";
                filterChk = filterChk + " AND NOT (" + filter + ")";
                try {
                    if (await GetQueryBuilder().WhereRaw(filterChk).ExistsAsync()) {
                        var idxErrMsg = Language.Phrase("DupIndex").Replace("%f", IdPosition.Caption);
                        idxErrMsg = idxErrMsg.Replace("%v", ConvertToString(IdPosition.CurrentValue));
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // NamaPosition
            NamaPosition.SetDbValue(rsnew, NamaPosition.CurrentValue, NamaPosition.ReadOnly);
            ...
            // Role
            Role.SetDbValue(rsnew, Role.CurrentValue, Role.ReadOnly);
            return rsnew;
        }
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("NamaPosition", out value)) // NamaPosition
                NamaPosition.CurrentValue = value;
            if (row.TryGetValue("Location", out value)) // Location
                Location.CurrentValue = value;
            if (row.TryGetValue("UserLevel", out value)) // UserLevel
                _UserLevel.CurrentValue = value;
            if (row.TryGetValue("Role", out value)) // Role
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MasterPositionList")), "", TableVar, true);
            string pageId = "edit";
