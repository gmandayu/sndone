...
public partial class SnDOne {
    /// <summary>
    /// subaktivitasInputLogbookPenyaluranPipaEdit
    /// </summary>
    public static SubaktivitasInputLogbookPenyaluranPipaEdit subaktivitasInputLogbookPenyaluranPipaEdit
    {
        get => HttpData.Get<SubaktivitasInputLogbookPenyaluranPipaEdit>("subaktivitasInputLogbookPenyaluranPipaEdit")!;
        set => HttpData["subaktivitasInputLogbookPenyaluranPipaEdit"] = value;
    }

    /// <summary>
    /// Page class for SubaktivitasInputLogbookPenyaluranPipa
    /// </summary>
    public class SubaktivitasInputLogbookPenyaluranPipaEdit : SubaktivitasInputLogbookPenyaluranPipaEditBase
    {
        // Constructor
        public SubaktivitasInputLogbookPenyaluranPipaEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubaktivitasInputLogbookPenyaluranPipaEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubaktivitasInputLogbookPenyaluranPipaEditBase : SubaktivitasInputLogbookPenyaluranPipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subaktivitasInputLogbookPenyaluranPipaEdit";
        ...
        // Constructor
        public SubaktivitasInputLogbookPenyaluranPipaEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubaktivitasInputLogbookPenyaluranPipaEdit";
        ...
        // Constructor
        public SubaktivitasInputLogbookPenyaluranPipaEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubaktivitasInputLogbookPenyaluranPipaList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubaktivitasInputLogbookPenyaluranPipaList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubaktivitasInputLogbookPenyaluranPipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subaktivitasInputLogbookPenyaluranPipaEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomId() {
            if (Id.Visible && Id.Required) {
                if (!Id.IsDetailKey && Empty(Id.FormValue)) {
                    Id.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomIdAktifitas() {
            if (IdAktifitas.Visible && IdAktifitas.Required) {
                if (!IdAktifitas.IsDetailKey && Empty(IdAktifitas.FormValue)) {
                    IdAktifitas.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorTangki() {
            if (NomorTangki.Visible && NomorTangki.Required) {
                if (!NomorTangki.IsDetailKey && Empty(NomorTangki.FormValue)) {
                    NomorTangki.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLO() {
            if (LO.Visible && LO.Required) {
                if (!LO.IsDetailKey && Empty(LO.FormValue)) {
                    LO.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomProduk() {
            if (Produk.Visible && Produk.Required) {
                if (!Produk.IsDetailKey && Empty(Produk.FormValue)) {
                    Produk.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomMeterAwal() {
            if (MeterAwal.Visible && MeterAwal.Required) {
                if (!MeterAwal.IsDetailKey && Empty(MeterAwal.FormValue)) {
                    MeterAwal.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomQuantity() {
            if (Quantity.Visible && Quantity.Required) {
                if (!Quantity.IsDetailKey && Empty(Quantity.FormValue)) {
                    Quantity.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomuserInput() {
            if (userInput.Visible && userInput.Required) {
                if (!userInput.IsDetailKey && Empty(userInput.FormValue)) {
                    userInput.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustometlDate() {
            if (etlDate.Visible && etlDate.Required) {
                if (!etlDate.IsDetailKey && Empty(etlDate.FormValue)) {
                    etlDate.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(Tanggal.RequiredErrorMessage).Replace("%s", Tanggal.Caption));
                }
            }
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubaktivitasInputLogbookPenyaluranPipaList")), "", TableVar, true);
            string pageId = "edit";
