...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasDataMobilTangkiMtGateOutEdit
    /// </summary>
    public static SubAktivitasDataMobilTangkiMtGateOutEdit subAktivitasDataMobilTangkiMtGateOutEdit
    {
        get => HttpData.Get<SubAktivitasDataMobilTangkiMtGateOutEdit>("subAktivitasDataMobilTangkiMtGateOutEdit")!;
        set => HttpData["subAktivitasDataMobilTangkiMtGateOutEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasDataMobilTangkiMTGateOut
    /// </summary>
    public class SubAktivitasDataMobilTangkiMtGateOutEdit : SubAktivitasDataMobilTangkiMtGateOutEditBase
    {
        // Constructor
        public SubAktivitasDataMobilTangkiMtGateOutEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasDataMobilTangkiMtGateOutEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasDataMobilTangkiMtGateOutEditBase : SubAktivitasDataMobilTangkiMtGateOut
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasDataMobilTangkiMtGateOutEdit";
        ...
        // Constructor
        public SubAktivitasDataMobilTangkiMtGateOutEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasDataMobilTangkiMtGateOutEdit";
        ...
        // Constructor
        public SubAktivitasDataMobilTangkiMtGateOutEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasDataMobilTangkiMtGateOutList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasDataMobilTangkiMtGateOutList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasDataMobilTangkiMtGateOutList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasDataMobilTangkiMtGateOutEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'WaktuGateOut'
            WaktuGateOut.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(WaktuGateOut, "WaktuGateOut", "x_WaktuGateOut", true, validate, true);
            if (!id.IsDetailKey) {
        ...
        public void RestoreFormValues()
        {
            ...
            WaktuGateOut.CurrentValue = WaktuGateOut.FormValue;
            WaktuGateOut.CurrentValue = UnformatDateTime(WaktuGateOut.CurrentValue, WaktuGateOut.FormatPattern);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                AMT.EditValue = ConvertToString(AMT.CurrentValue); // DN
                AMT.ViewCustomAttributes = "";

                // NomorSegel
                NomorSegel.SetupEditAttributes();
                if (!NomorSegel.Raw)
                    NomorSegel.CurrentValue = HtmlDecode(NomorSegel.CurrentValue);
                NomorSegel.EditValue = HtmlEncode(NomorSegel.CurrentValue);
                NomorSegel.PlaceHolder = RemoveHtml(NomorSegel.Caption);

                // WaktuGateOut
                WaktuGateOut.SetupEditAttributes();
                WaktuGateOut.EditValue = FormatDateTime(WaktuGateOut.CurrentValue, WaktuGateOut.FormatPattern);
                WaktuGateOut.PlaceHolder = RemoveHtml(WaktuGateOut.Caption);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorPolisi() {
            if (NomorPolisi.Visible && NomorPolisi.Required) {
                if (!NomorPolisi.IsDetailKey && Empty(NomorPolisi.FormValue)) {
                    NomorPolisi.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomAMT() {
            if (AMT.Visible && AMT.Required) {
                if (!AMT.IsDetailKey && Empty(AMT.FormValue)) {
                    AMT.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorSegel() {
            if (NomorSegel.Visible && NomorSegel.Required) {
                if (!NomorSegel.IsDetailKey && Empty(NomorSegel.FormValue)) {
                    NomorSegel.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }

        private void ValidateCustomWaktuGateOut() {
            if (WaktuGateOut.Visible && WaktuGateOut.Required) {
                if (!WaktuGateOut.IsDetailKey && Empty(WaktuGateOut.FormValue)) {
                    WaktuGateOut.AddErrorMessage(ConvertToString(WaktuGateOut.RequiredErrorMessage).Replace("%s", WaktuGateOut.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomWaktuGateOut();
                if (!CheckDate(WaktuGateOut.FormValue, WaktuGateOut.FormatPattern)) {
                    WaktuGateOut.AddErrorMessage(WaktuGateOut.GetErrorMessage(false));
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // NomorSegel
            NomorSegel.SetDbValue(rsnew, NomorSegel.CurrentValue, NomorSegel.ReadOnly);

            // WaktuGateOut
            WaktuGateOut.SetDbValue(rsnew, ConvertToDateTime(WaktuGateOut.CurrentValue, WaktuGateOut.FormatPattern), WaktuGateOut.ReadOnly);
            return rsnew;
        }
        ...
        {
            object? value;
            if (row.TryGetValue("NomorSegel", out value)) // NomorSegel
                NomorSegel.CurrentValue = value;
            if (row.TryGetValue("WaktuGateOut", out value)) // WaktuGateOut
                WaktuGateOut.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasDataMobilTangkiMtGateOutList")), "", TableVar, true);
            string pageId = "edit";
