...
public partial class SnDOne {
    ...
    /// mappingPositionAdd
    /// </summary>
    public static MappingPositionAdd mappingPositionAdd
    {
        get => HttpData.Get<MappingPositionAdd>("mappingPositionAdd")!;
        set => HttpData["mappingPositionAdd"] = value;
    }

    /// <summary>
    /// Page class for MappingPosition
    /// </summary>
    public class MappingPositionAdd : MappingPositionAddBase
    {
        // Constructor
        public MappingPositionAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MappingPositionAdd() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class MappingPositionAddBase : MappingPosition
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mappingPositionAdd";
        ...
        // Constructor
        public MappingPositionAddBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "MappingPositionAdd";
        ...
        // Constructor
        public MappingPositionAddBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private IActionResult? HandleCopyAction(Dictionary<string, object>? rsold)
        {
            if (rsold == null) {
                ...
                return Terminate("MappingPositionList");
            }
            return null;
        }
        ...
        private string DetermineReturnUrl()
        {
            string returnUrl = "";
            returnUrl = ReturnUrl;
            if (GetPageName(returnUrl) == "MappingPositionList")
                returnUrl = AddMasterUrl(ListUrl);
            else if (GetPageName(returnUrl) == "MappingPositionView")
                returnUrl = ViewUrl;
            return returnUrl;
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'IdRegion'
            IdRegion.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(IdRegion, "IdRegion", "x_IdRegion");
        ...
        public void RestoreFormValues()
        {
            ...
            IdRegion.CurrentValue = IdRegion.FormValue;
        ...
        public async Task RenderRow()
        {
            ...
            // IdRegion
            IdRegion.RowCssClass = "row";

            // NamaRegion
            NamaRegion.RowCssClass = "row";

            // IdPlant
            IdPlant.RowCssClass = "row";

            // NamaPlant
            NamaPlant.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                if (IdPosition.Lookup != null && IsDictionary(IdPosition.Lookup?.Options) && IdPosition.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    IdPosition.EditValue = IdPosition.Lookup?.Options.Values.ToList();
                } else { // Lookup from database
                    string filterWrk = "";
                    if (curVal == "") {
                        filterWrk = "0=1";
                    } else {
                        filterWrk = SearchFilter(IdPosition.Lookup?.GetTable()?.Fields["IdPosition"].SearchExpression, "=", IdPosition.CurrentValue, IdPosition.Lookup?.GetTable()?.Fields["IdPosition"].SearchDataType, "");
                    }
                    string? sqlWrk = IdPosition.Lookup?.GetSql(true, filterWrk, null, this, false, true);
                    List<Dictionary<string, object>>? rswrk = sqlWrk != null ? Connection.GetRows(sqlWrk) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                    IdPosition.EditValue = rswrk;
                }
                ...
                if (IdRegion.Lookup != null && IsDictionary(IdRegion.Lookup?.Options) && IdRegion.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    IdRegion.EditValue = IdRegion.Lookup?.Options.Values.ToList();
                } else { // Lookup from database
                    string filterWrk2 = "";
                    if (curVal2 == "") {
                        filterWrk2 = "0=1";
                    } else {
                        filterWrk2 = SearchFilter(IdRegion.Lookup?.GetTable()?.Fields["IdRegion"].SearchExpression, "=", IdRegion.CurrentValue, IdRegion.Lookup?.GetTable()?.Fields["IdRegion"].SearchDataType, "");
                    }
                    string? sqlWrk2 = IdRegion.Lookup?.GetSql(true, filterWrk2, null, this, false, true);
                    List<Dictionary<string, object>>? rswrk2 = sqlWrk2 != null ? Connection.GetRows(sqlWrk2) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                    IdRegion.EditValue = rswrk2;
                }
                ...
                if (IdPlant.Lookup != null && IsDictionary(IdPlant.Lookup?.Options) && IdPlant.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    IdPlant.EditValue = IdPlant.Lookup?.Options.Values.ToList();
                } else { // Lookup from database
                    string filterWrk3 = "";
                    if (curVal3 == "") {
                        filterWrk3 = "0=1";
                    } else {
                        filterWrk3 = SearchFilter(IdPlant.Lookup?.GetTable()?.Fields["IdPlant"].SearchExpression, "=", IdPlant.CurrentValue, IdPlant.Lookup?.GetTable()?.Fields["IdPlant"].SearchDataType, "");
                    }
                    string? sqlWrk3 = IdPlant.Lookup?.GetSql(true, filterWrk3, null, this, false, true);
        ...
        private void ValidateCustomIdPosition() {
            if (IdPosition.Visible && IdPosition.Required) {
                if (!IdPosition.IsDetailKey && Empty(IdPosition.FormValue)) {
                    IdPosition.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }

        private void ValidateCustomIdRegion() {
            if (IdRegion.Visible && IdRegion.Required) {
                if (!IdRegion.IsDetailKey && Empty(IdRegion.FormValue)) {
                    IdRegion.AddErrorMessage(ConvertToString(IdPlant.RequiredErrorMessage).Replace("%s", IdPlant.Caption));
                }
            }
        }
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // IdRegion
                IdRegion.SetDbValue(rsnew, IdRegion.CurrentValue);
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("IdRegion", out value)) // IdRegion
                IdRegion.SetFormValue(ConvertToString(value));
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MappingPositionList")), "", TableVar, true);
