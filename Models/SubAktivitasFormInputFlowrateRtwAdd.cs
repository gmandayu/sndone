...
public partial class SnDOne {
    ...
    public class SubAktivitasFormInputFlowrateRtwAddBase : SubAktivitasFormInputFlowrateRtw
    {
        ...
        // Constructor
        public SubAktivitasFormInputFlowrateRtwAddBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasFormInputFlowrateRtwAddBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private IActionResult? HandleCopyAction(Dictionary<string, object>? rsold)
        {
            if (rsold == null) {
                ...
                return Terminate("SubAktivitasFormInputFlowrateRtwList");
            }
            return null;
        }
        ...
        private void HandleAjaxActions(ref string returnUrl)
        {
            if (IsModal && UseAjaxActions) {
                ...
                if (GetPageName(returnUrl) != "SubAktivitasFormInputFlowrateRtwList") {
                    ...
                    returnUrl = "SubAktivitasFormInputFlowrateRtwList";
                }
            }
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'StrippingTime'
            StrippingTime.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(StrippingTime, "StrippingTime", "x_StrippingTime");

            // Standard handling for 'Pressuren'
            Pressuren.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Pressuren, "Pressuren", "x_Pressuren");

            // Standard handling for 'LamaPembongkaran'
            LamaPembongkaran.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(LamaPembongkaran, "LamaPembongkaran", "x_LamaPembongkaran");
        ...
        private void ValidateCustomFlowrate() {
            if (Flowrate.Visible && Flowrate.Required) {
                if (!Flowrate.IsDetailKey && Empty(Flowrate.FormValue)) {
                    Flowrate.AddErrorMessage(ConvertToString(etlDate.RequiredErrorMessage).Replace("%s", etlDate.Caption));
                }
            }
        }

        private void ValidateCustomStrippingTime() {
            if (StrippingTime.Visible && StrippingTime.Required) {
                if (!StrippingTime.IsDetailKey && Empty(StrippingTime.FormValue)) {
                    StrippingTime.AddErrorMessage(ConvertToString(etlDate.RequiredErrorMessage).Replace("%s", etlDate.Caption));
                }
            }
        }

        private void ValidateCustomPressuren() {
            if (Pressuren.Visible && Pressuren.Required) {
                if (!Pressuren.IsDetailKey && Empty(Pressuren.FormValue)) {
                    Pressuren.AddErrorMessage(ConvertToString(etlDate.RequiredErrorMessage).Replace("%s", etlDate.Caption));
                }
            }
        }

        private void ValidateCustomLamaPembongkaran() {
            if (LamaPembongkaran.Visible && LamaPembongkaran.Required) {
                if (!LamaPembongkaran.IsDetailKey && Empty(LamaPembongkaran.FormValue)) {
                    LamaPembongkaran.AddErrorMessage(ConvertToString(etlDate.RequiredErrorMessage).Replace("%s", etlDate.Caption));
                }
            }
        }
        ...
        private void ValidateCustomETC() {
            if (ETC.Visible && ETC.Required) {
                if (!ETC.IsDetailKey && Empty(ETC.FormValue)) {
                    ETC.AddErrorMessage(ConvertToString(etlDate.RequiredErrorMessage).Replace("%s", etlDate.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomStrippingTime();
                ValidateCustomPressuren();
                ValidateCustomLamaPembongkaran();
                ValidateCustomETC();
