...
public partial class SnDOne {
    /// <summary>
    /// templateAktivitasList
    /// </summary>
    public static TemplateAktivitasList templateAktivitasList
    {
        get => HttpData.Get<TemplateAktivitasList>("templateAktivitasList")!;
        set => HttpData["templateAktivitasList"] = value;
    }

    /// <summary>
    /// Page class for TemplateAktivitas
    /// </summary>
    public class TemplateAktivitasList : TemplateAktivitasListBase
    {
        // Constructor
        public TemplateAktivitasList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TemplateAktivitasList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TemplateAktivitasListBase : TemplateAktivitas
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "templateAktivitasList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fTemplateAktivitaslist";
        ...
        // Constructor
        public TemplateAktivitasListBase(Controller? controller)
        {
            TableName = "TemplateAktivitas";

            // CSS class name as context
            TableVar = "TemplateAktivitas";
        ...
        if (controller != null)
            ...
            // Initialize URLs
            AddUrl = "TemplateAktivitasAdd?" + Config.TableShowDetail + "=";
            ...
            MultiDeleteUrl = "TemplateAktivitasDelete";
            MultiUpdateUrl = "TemplateAktivitasUpdate";
        ...
        // Page name
        public string PageName => "TemplateAktivitasList";
        ...
        // Constructor
        public TemplateAktivitasListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void PrepareInitialSettings()
        {
            ...
            if (IsModal)
                FormName = "fTemplateAktivitasgrid";
        ...
        private async Task<IActionResult?> LoadMasterRecords()
        {
            ...
            // Load master record
            if (CurrentMode != "add" && !Empty(MasterFilterFromSession) && CurrentMasterTable == "TemplateProses") {
                templateProses = Resolve("TemplateProses")!;
                if (templateProses != null) {
                    var rowMaster = await templateProses.Connection.GetRowAsync(templateProses.GetSql(DbMasterFilter));
                    MasterRecordExists = rowMaster != null;
                    if (!MasterRecordExists) {
                        FailureMessage = Language.Phrase("NoRecord"); // Set no record found
                        return Terminate("TemplateProsesList"); // Return to master page
                    } else {
                        templateProses.LoadListRowValues(rowMaster);
                    }
                    templateProses.RowType = RowType.Master; // Master row
                    await templateProses.RenderListRow(); // Note: Do it outside "using" // DN
                }
            }
            return null;
        }
        ...
        private void PageRunEnd()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                templateAktivitasList?.PageRender();
            }
        }
        #pragma warning restore 219
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, IdTemplateProses, def, true); // IdTemplateProses
            BuildSearchSql(ref where, UrutanAktivitas, def, true); // UrutanAktivitas
            BuildSearchSql(ref where, NamaAktivitas, def, true); // NamaAktivitas
            BuildSearchSql(ref where, IdPIC, def, true); // IdPIC
            ...
            BuildSearchSql(ref where, _URL, def, true); // _URL
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("IdTemplateProses", IdTemplateProses, true, config);
            filterList += ProcessFieldFilter("UrutanAktivitas", UrutanAktivitas, true, config);
            filterList += ProcessFieldFilter("NamaAktivitas", NamaAktivitas, true, config);
            filterList += ProcessFieldFilter("IdPIC", IdPIC, true, config);
            ...
            filterList += ProcessFieldFilter("URL", _URL, true, config);
            return filterList;
        }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(IdTemplateProses);
            searchFlds.Add(UrutanAktivitas);
            searchFlds.Add(NamaAktivitas);
            searchFlds.Add(IdPIC);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (IdTemplateProses.AdvancedSearch.IssetSession)
                return true;
            if (UrutanAktivitas.AdvancedSearch.IssetSession)
                return true;
            if (NamaAktivitas.AdvancedSearch.IssetSession)
                return true;
            if (IdPIC.AdvancedSearch.IssetSession)
                return true;
            ...
            if (TipeAktivitas.AdvancedSearch.IssetSession)
                return true;
            if (_URL.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            IdTemplateProses.AdvancedSearch.UnsetSession();
            UrutanAktivitas.AdvancedSearch.UnsetSession();
            NamaAktivitas.AdvancedSearch.UnsetSession();
            IdPIC.AdvancedSearch.UnsetSession();
            ...
            _URL.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            IdTemplateProses.AdvancedSearch.Load();
            UrutanAktivitas.AdvancedSearch.Load();
            NamaAktivitas.AdvancedSearch.Load();
            IdPIC.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(IdTemplateProses, ctrl); // IdTemplateProses
                UpdateSort(UrutanAktivitas, ctrl); // UrutanAktivitas
                UpdateSort(NamaAktivitas, ctrl); // NamaAktivitas
                UpdateSort(IdPIC, ctrl); // IdPIC
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    IdTemplateProses.Sort = "";
                    UrutanAktivitas.Sort = "";
                    NamaAktivitas.Sort = "";
                    IdPIC.Sort = "";
                    Keterangan.Sort = "";
                    DibuatOleh.Sort = "";
        ...
        protected async Task SetupListOptions() {
            ...
            item = ListOptions.Add("detail_TemplateAktivitasDokumen");
            item.CssClass = "text-nowrap";
            ...
            _pages.Add("TemplateAktivitasDokumen");
            DetailPages = _pages;
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""TemplateAktivitas"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""TemplateAktivitas"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""TemplateAktivitas"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fTemplateAktivitaslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTemplateAktivitaslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            // "detail_TemplateAktivitasDokumen"
            listOption = ListOptions["detail_TemplateAktivitasDokumen"];
            isVisible = Security.AllowList(CurrentProjectID + "TemplateAktivitasDokumen");
            if (isVisible) {
                string caption, title, url;
                var body = Language.Phrase("DetailLink") + Language.TablePhrase("TemplateAktivitasDokumen", "TblCaption");
                body = "<a class=\"btn btn-default ew-row-link ew-detail" + (ListOptions.UseDropDownButton ? " dropdown-toggle" : "") + "\" data-action=\"list\" href=\"" + HtmlEncode(AppPath("TemplateAktivitasDokumenList?" + Config.TableShowMaster + "=TemplateAktivitas&" + ForeignKeyUrl("fk_IdTemplateAktivitas", IdTemplateAktivitas.CurrentValue) + "")) + "\">" + body + "</a>";
                string links = "";
                ...
                if (detailPage?.DetailView && Security.CanView && Security.AllowView(CurrentProjectID + "TemplateAktivitas") ?? false) {
                    ...
                    url = GetViewUrl(Config.TableShowDetail + "=TemplateAktivitasDokumen");
                ...
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "TemplateAktivitas") ?? false) {
                    ...
                    url = GetEditUrl(Config.TableShowDetail + "=TemplateAktivitasDokumen");
                ...
                if (detailPage?.DetailAdd && Security.CanAdd && Security.AllowAdd(CurrentProjectID + "TemplateAktivitas") ?? false) {
                    ...
                    url = GetCopyUrl(Config.TableShowDetail + "=TemplateAktivitasDokumen");
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdTemplateAktivitas.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""TemplateAktivitas"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item = option.Add("detailadd_TemplateAktivitasDokumen");
            templateAktivitasDokumen = Resolve("TemplateAktivitasDokumen")!;
            if (templateAktivitasDokumen != null) {
                caption = Language.Phrase("Add") + "&nbsp;" + Caption + "/" + templateAktivitasDokumen.Caption;
                title = Language.Phrase("Add", true) + "&nbsp;" + Caption + "/" + templateAktivitasDokumen.Caption;
                item.Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" + title + "\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(GetAddUrl(Config.TableShowDetail + "=TemplateAktivitasDokumen"))) + "\">" + caption + "</a>";
                item.Visible = (templateAktivitasDokumen.DetailAdd && Security.AllowAdd(CurrentProjectID + "TemplateAktivitas") && Security.CanAdd);
                if (item.Visible) {
                    if (detailTableLink != "")
                        detailTableLink += ",";
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fTemplateAktivitaslist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("IdTemplateProses")); // DN
                CreateColumnOption(option.Add("UrutanAktivitas")); // DN
                CreateColumnOption(option.Add("NamaAktivitas")); // DN
                CreateColumnOption(option.Add("IdPIC")); // DN
                ...
                CreateColumnOption(option.Add("URL")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fTemplateAktivitassrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fTemplateAktivitassrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fTemplateAktivitaslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_TemplateAktivitas");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(templateAktivitasList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(templateAktivitasList.RowCount) + "_TemplateAktivitas");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(templateAktivitasList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && templateAktivitasList.RowType == RowType.Add || IsEdit && templateAktivitasList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadFieldIdTemplateProsesSearchValues();
            LoadFieldUrutanAktivitasSearchValues();
            LoadFieldNamaAktivitasSearchValues();
            LoadFieldIdPICSearchValues();
            ...
            LoadField_URLSearchValues();
        }

        private void LoadFieldIdTemplateProsesSearchValues()
        {
            // IdTemplateProses
            LoadPrimarySearchValueIdTemplateProses();
            LoadSearchOperatorIdTemplateProses();
        }

        private void LoadPrimarySearchValueIdTemplateProses()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdTemplateProses[]")) {
                IdTemplateProses.AdvancedSearch.SearchValue = Get("x_IdTemplateProses[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                IdTemplateProses.AdvancedSearch.SearchValue = Get("IdTemplateProses"); // Default Value // DN
            }
            if (!Empty(IdTemplateProses.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIdTemplateProses()
        {
            if (Query.ContainsKey("z_IdTemplateProses"))
                IdTemplateProses.AdvancedSearch.SearchOperator = Get("z_IdTemplateProses", Config.FilterOptionSeparator);
        }

        private void LoadFieldUrutanAktivitasSearchValues()
        {
            // UrutanAktivitas
            LoadPrimarySearchValueUrutanAktivitas();
            LoadSearchOperatorUrutanAktivitas();
        }

        private void LoadPrimarySearchValueUrutanAktivitas()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UrutanAktivitas[]")) {
                UrutanAktivitas.AdvancedSearch.SearchValue = Get("x_UrutanAktivitas[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                UrutanAktivitas.AdvancedSearch.SearchValue = Get("UrutanAktivitas"); // Default Value // DN
            }
            if (!Empty(UrutanAktivitas.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorUrutanAktivitas()
        {
            if (Query.ContainsKey("z_UrutanAktivitas"))
                UrutanAktivitas.AdvancedSearch.SearchOperator = Get("z_UrutanAktivitas", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldIdPICSearchValues()
        {
            // IdPIC
            LoadPrimarySearchValueIdPIC();
            LoadSearchOperatorIdPIC();
        }

        private void LoadPrimarySearchValueIdPIC()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdPIC[]")) {
                IdPIC.AdvancedSearch.SearchValue = Get("x_IdPIC[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                IdPIC.AdvancedSearch.SearchValue = Get("IdPIC"); // Default Value // DN
            }
            if (!Empty(IdPIC.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorIdPIC()
        {
            if (Query.ContainsKey("z_IdPIC"))
                IdPIC.AdvancedSearch.SearchOperator = Get("z_IdPIC", Config.FilterOptionSeparator);
        }
        ...
        private void LoadField_URLSearchValues()
        {
            // _URL
            LoadPrimarySearchValue_URL();
            LoadSearchOperator_URL();
        }

        private void LoadPrimarySearchValue_URL()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x__URL[]")) {
                _URL.AdvancedSearch.SearchValue = Get("x__URL[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                _URL.AdvancedSearch.SearchValue = Get("_URL"); // Default Value // DN
            }
            if (!Empty(_URL.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperator_URL()
        {
            if (Query.ContainsKey("z__URL"))
                _URL.AdvancedSearch.SearchOperator = Get("z__URL", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // URL
                    ...
                    // awallookupbung
                    // IdTemplateProses
                    ResolveLookupView(IdTemplateProses, "IdTemplateProses", "number");
                    // akhirlookupbung
                    ...
                    // awallookupbung
                    // IdPIC (jaga leading zero)
                    ResolveLookupView(IdPIC, "IdPIC", "string");
                    // akhirlookupbung
                ...
                // IdTemplateProses
                if (IdTemplateProses.UseFilter && !Empty(IdTemplateProses.AdvancedSearch.SearchValue)) {
                    IdTemplateProses.EditValue = ConvertToString(IdTemplateProses.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // UrutanAktivitas
                if (UrutanAktivitas.UseFilter && !Empty(UrutanAktivitas.AdvancedSearch.SearchValue)) {
                    UrutanAktivitas.EditValue = ConvertToString(UrutanAktivitas.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // NamaAktivitas
                if (NamaAktivitas.UseFilter && !Empty(NamaAktivitas.AdvancedSearch.SearchValue)) {
                    NamaAktivitas.EditValue = ConvertToString(NamaAktivitas.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // IdPIC
                if (IdPIC.UseFilter && !Empty(IdPIC.AdvancedSearch.SearchValue)) {
                    IdPIC.EditValue = ConvertToString(IdPIC.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // URL
                if (_URL.UseFilter && !Empty(_URL.AdvancedSearch.SearchValue)) {
                    _URL.EditValue = ConvertToString(_URL.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fTemplateAktivitassrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
        ...
        public async Task ExportData(dynamic? doc)
        {
            ...
            if (Config.ExportMasterRecord && !Empty(MasterFilterFromSession) && CurrentMasterTable == "TemplateProses") {
                templateProses = new TemplateProsesList();
                if (templateProses != null) {
                    var c = await GetConnection2Async(templateProses.DbId); // Note: Use new connection for master record // DN
                    using var rsmaster = await c.ExecuteReaderAsync(templateProses.GetSql(DbMasterFilter)); // Load master record
                    if (rsmaster?.HasRows ?? false) { // DN
                        exportStyle = doc.Style;
                        doc.SetStyle("v"); // Change to vertical
                        if (!IsExport("csv") || Config.ExportMasterRecordForCsv) {
                            doc.Table = templateProses;
                            await templateProses.ExportDocument(doc, rsmaster, 1, 1);
                            doc.ExportEmptyRow();
                            doc.Table = this;
                        }
