...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasTotalPenyaluranLpgEdit
    /// </summary>
    public static SubAktivitasTotalPenyaluranLpgEdit subAktivitasTotalPenyaluranLpgEdit
    {
        get => HttpData.Get<SubAktivitasTotalPenyaluranLpgEdit>("subAktivitasTotalPenyaluranLpgEdit")!;
        set => HttpData["subAktivitasTotalPenyaluranLpgEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasTotalPenyaluranLPG
    /// </summary>
    public class SubAktivitasTotalPenyaluranLpgEdit : SubAktivitasTotalPenyaluranLpgEditBase
    {
        // Constructor
        public SubAktivitasTotalPenyaluranLpgEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasTotalPenyaluranLpgEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasTotalPenyaluranLpgEditBase : SubAktivitasTotalPenyaluranLpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasTotalPenyaluranLpgEdit";
        ...
        // Constructor
        public SubAktivitasTotalPenyaluranLpgEditBase()
        {
        ...
        // Page name
        public string PageName => "SubAktivitasTotalPenyaluranLpgEdit";
        ...
        // Constructor
        public SubAktivitasTotalPenyaluranLpgEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasTotalPenyaluranLpgList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = ReturnUrl;
                    if (GetPageName(returnUrl) == "SubAktivitasTotalPenyaluranLpgList")
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasTotalPenyaluranLpgList") {
                                ...
                                returnUrl = "SubAktivitasTotalPenyaluranLpgList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasTotalPenyaluranLpgEdit?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'TotalPenyaluran' before field var 'x_TotalPenyaluran'
            val = CurrentForm.HasValue("TotalPenyaluran") ? CurrentForm.GetValue("TotalPenyaluran") : CurrentForm.GetValue("x_TotalPenyaluran");
            if (!TotalPenyaluran.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TotalPenyaluran") && !CurrentForm.HasValue("x_TotalPenyaluran")) // DN
                    TotalPenyaluran.Visible = false; // Disable update for API request
                else
                    TotalPenyaluran.SetFormValue(val);
            }

            // Check field name 'StokAkhir' before field var 'x_StokAkhir'
            val = CurrentForm.HasValue("StokAkhir") ? CurrentForm.GetValue("StokAkhir") : CurrentForm.GetValue("x_StokAkhir");
            if (!StokAkhir.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("StokAkhir") && !CurrentForm.HasValue("x_StokAkhir")) // DN
                    StokAkhir.Visible = false; // Disable update for API request
                else
                    StokAkhir.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (TotalPenyaluran.Visible && TotalPenyaluran.Required) {
                    if (!TotalPenyaluran.IsDetailKey && Empty(TotalPenyaluran.FormValue)) {
                        TotalPenyaluran.AddErrorMessage(ConvertToString(TotalPenyaluran.RequiredErrorMessage).Replace("%s", TotalPenyaluran.Caption));
                    }
                }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasTotalPenyaluranLpgList")), "", TableVar, true);
            string pageId = "edit";
