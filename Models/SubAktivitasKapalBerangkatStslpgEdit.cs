...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasKapalBerangkatStslpgEdit
    /// </summary>
    public static SubAktivitasKapalBerangkatStslpgEdit subAktivitasKapalBerangkatStslpgEdit
    {
        get => HttpData.Get<SubAktivitasKapalBerangkatStslpgEdit>("subAktivitasKapalBerangkatStslpgEdit")!;
        set => HttpData["subAktivitasKapalBerangkatStslpgEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasKapalBerangkatSTSLPG
    /// </summary>
    public class SubAktivitasKapalBerangkatStslpgEdit : SubAktivitasKapalBerangkatStslpgEditBase
    {
        // Constructor
        public SubAktivitasKapalBerangkatStslpgEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasKapalBerangkatStslpgEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasKapalBerangkatStslpgEditBase : SubAktivitasKapalBerangkatStslpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasKapalBerangkatStslpgEdit";
        ...
        // Constructor
        public SubAktivitasKapalBerangkatStslpgEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasKapalBerangkatStslpgEdit";
        ...
        // Constructor
        public SubAktivitasKapalBerangkatStslpgEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasKapalBerangkatStslpgList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasKapalBerangkatStslpgList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasKapalBerangkatStslpgList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasKapalBerangkatStslpgEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'tglBerangkat'
            tglBerangkat.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(tglBerangkat, "tglBerangkat", "x_tglBerangkat", true, validate, true);
            if (!id.IsDetailKey) {
        ...
        public void RestoreFormValues()
        {
            ...
            tglBerangkat.CurrentValue = tglBerangkat.FormValue;
            tglBerangkat.CurrentValue = UnformatDateTime(tglBerangkat.CurrentValue, tglBerangkat.FormatPattern);
        }
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(tglBerangkat.RequiredErrorMessage).Replace("%s", tglBerangkat.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(tglBerangkat.RequiredErrorMessage).Replace("%s", tglBerangkat.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(tglBerangkat.RequiredErrorMessage).Replace("%s", tglBerangkat.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(tglBerangkat.RequiredErrorMessage).Replace("%s", tglBerangkat.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(tglBerangkat.RequiredErrorMessage).Replace("%s", tglBerangkat.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(tglBerangkat.RequiredErrorMessage).Replace("%s", tglBerangkat.Caption));
                }
            }
        }

        private void ValidateCustomtglBerangkat() {
            if (tglBerangkat.Visible && tglBerangkat.Required) {
                if (!tglBerangkat.IsDetailKey && Empty(tglBerangkat.FormValue)) {
                    tglBerangkat.AddErrorMessage(ConvertToString(tglBerangkat.RequiredErrorMessage).Replace("%s", tglBerangkat.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomtglBerangkat();
                if (!CheckDate(tglBerangkat.FormValue, tglBerangkat.FormatPattern)) {
                    tglBerangkat.AddErrorMessage(tglBerangkat.GetErrorMessage(false));
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // tglBerangkat
            tglBerangkat.SetDbValue(rsnew, ConvertToDateTime(tglBerangkat.CurrentValue, tglBerangkat.FormatPattern), tglBerangkat.ReadOnly);
            return rsnew;
        }
        ...
        {
            object? value;
            if (row.TryGetValue("tglBerangkat", out value)) // tglBerangkat
                tglBerangkat.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasKapalBerangkatStslpgList")), "", TableVar, true);
            string pageId = "edit";
