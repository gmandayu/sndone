...
public partial class SnDOne {
    /// <summary>
    /// subaktivitasKapalBerangkatStsPnrBbmList
    /// </summary>
    public static SubaktivitasKapalBerangkatStsPnrBbmList subaktivitasKapalBerangkatStsPnrBbmList
    {
        get => HttpData.Get<SubaktivitasKapalBerangkatStsPnrBbmList>("subaktivitasKapalBerangkatStsPnrBbmList")!;
        set => HttpData["subaktivitasKapalBerangkatStsPnrBbmList"] = value;
    }

    /// <summary>
    /// Page class for SubaktivitasKapalBerangkatSTSPnrBBM
    /// </summary>
    public class SubaktivitasKapalBerangkatStsPnrBbmList : SubaktivitasKapalBerangkatStsPnrBbmListBase
    {
        // Constructor
        public SubaktivitasKapalBerangkatStsPnrBbmList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubaktivitasKapalBerangkatStsPnrBbmList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubaktivitasKapalBerangkatStsPnrBbmListBase : SubaktivitasKapalBerangkatStsPnrBbm
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subaktivitasKapalBerangkatStsPnrBbmList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubaktivitasKapalBerangkatSTSPnrBBMlist";
        ...
        // Constructor
        public SubaktivitasKapalBerangkatStsPnrBbmListBase()
        {
            TableName = "SubaktivitasKapalBerangkatSTSPnrBBM";

            // CSS class name as context
            TableVar = "SubaktivitasKapalBerangkatSTSPnrBBM";
            ...
            // Table object (subaktivitasKapalBerangkatStsPnrBbm)
            if (subaktivitasKapalBerangkatStsPnrBbm == null || subaktivitasKapalBerangkatStsPnrBbm is SubaktivitasKapalBerangkatStsPnrBbm)
                subaktivitasKapalBerangkatStsPnrBbm = this;

            // Initialize URLs
            AddUrl = "SubaktivitasKapalBerangkatStsPnrBbmAdd";
            ...
            MultiDeleteUrl = "SubaktivitasKapalBerangkatStsPnrBbmDelete";
            MultiUpdateUrl = "SubaktivitasKapalBerangkatStsPnrBbmUpdate";
        ...
        // Page name
        public string PageName => "SubaktivitasKapalBerangkatStsPnrBbmList";
        ...
        public void SetVisibility()
        {
            ...
            tglBerangkat.SetVisibility();
            userInput.SetVisibility();
            etlDate.SetVisibility();
        }

        // Constructor
        public SubaktivitasKapalBerangkatStsPnrBbmListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubaktivitasKapalBerangkatStsPnrBbmView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubaktivitasKapalBerangkatSTSPnrBBMgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subaktivitasKapalBerangkatStsPnrBbmList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(tglBerangkat.AdvancedSearch.ToJson())); // Field tglBerangkat
        ...
        protected bool RestoreFilterList() {
            ...
            // Field tglBerangkat
            if (filter?.TryGetValue("x_tglBerangkat", out sv) ?? false) {
                tglBerangkat.AdvancedSearch.SearchValue = sv;
                tglBerangkat.AdvancedSearch.SearchOperator = filter["z_tglBerangkat"];
                tglBerangkat.AdvancedSearch.SearchCondition = filter["v_tglBerangkat"];
                tglBerangkat.AdvancedSearch.SearchValue2 = filter["y_tglBerangkat"];
                tglBerangkat.AdvancedSearch.SearchOperator2 = filter["w_tglBerangkat"];
                tglBerangkat.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(tglBerangkat, ctrl); // tglBerangkat
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubaktivitasKapalBerangkatSTSPnrBBM"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubaktivitasKapalBerangkatSTSPnrBBMlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubaktivitasKapalBerangkatSTSPnrBBMlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("tglBerangkat")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubaktivitasKapalBerangkatSTSPnrBBMsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubaktivitasKapalBerangkatSTSPnrBBMsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubaktivitasKapalBerangkatSTSPnrBBMlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubaktivitasKapalBerangkatSTSPnrBBM");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subaktivitasKapalBerangkatStsPnrBbmList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subaktivitasKapalBerangkatStsPnrBbmList.RowCount) + "_SubaktivitasKapalBerangkatSTSPnrBBM");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subaktivitasKapalBerangkatStsPnrBbmList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subaktivitasKapalBerangkatStsPnrBbmList.RowType == RowType.Add || IsEdit && subaktivitasKapalBerangkatStsPnrBbmList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            tglBerangkat.SetDbValue(row["tglBerangkat"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("tglBerangkat", tglBerangkat.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // tglBerangkat
                tglBerangkat.ViewValue = tglBerangkat.CurrentValue;
                tglBerangkat.ViewValue = FormatDateTime(tglBerangkat.ViewValue, tglBerangkat.FormatPattern);
                tglBerangkat.ViewCustomAttributes = "";
                ...
                // tglBerangkat
                tglBerangkat.HrefValue = "";
                tglBerangkat.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubaktivitasKapalBerangkatSTSPnrBBMsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
