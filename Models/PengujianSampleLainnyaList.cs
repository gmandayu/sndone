...
public partial class SnDOne {
    /// <summary>
    /// pengujianSampleLainnyaList
    /// </summary>
    public static PengujianSampleLainnyaList pengujianSampleLainnyaList
    {
        get => HttpData.Get<PengujianSampleLainnyaList>("pengujianSampleLainnyaList")!;
        set => HttpData["pengujianSampleLainnyaList"] = value;
    }

    /// <summary>
    /// Page class for PengujianSampleLainnya
    /// </summary>
    public class PengujianSampleLainnyaList : PengujianSampleLainnyaListBase
    {
        // Constructor
        public PengujianSampleLainnyaList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public PengujianSampleLainnyaList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class PengujianSampleLainnyaListBase : PengujianSampleLainnya
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "pengujianSampleLainnyaList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fPengujianSampleLainnyalist";
        ...
        // Constructor
        public PengujianSampleLainnyaListBase()
        {
            TableName = "PengujianSampleLainnya";
            ...
            // Table object (pengujianSampleLainnya)
            if (pengujianSampleLainnya == null || pengujianSampleLainnya is PengujianSampleLainnya)
                pengujianSampleLainnya = this;

            // Initialize URLs
            AddUrl = "PengujianSampleLainnyaAdd";
            ...
            MultiDeleteUrl = "PengujianSampleLainnyaDelete";
            MultiUpdateUrl = "PengujianSampleLainnyaUpdate";
        ...
        // Page name
        public string PageName => "PengujianSampleLainnyaList";
        ...
        {
            id.Visible = false;
            NomorPengujianSampelLainnya.SetVisibility();
            idPlant.SetVisibility();
            Tanggal.SetVisibility();
            Produk.SetVisibility();
            UploadHasil.Visible = false;
            CF_UploadHasil.SetVisibility();
        ...
        // Constructor
        public PengujianSampleLainnyaListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "PengujianSampleLainnyaView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fPengujianSampleLainnyagrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                pengujianSampleLainnyaList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(NomorPengujianSampelLainnya.AdvancedSearch.ToJson())); // Field NomorPengujianSampelLainnya
        ...
        protected bool RestoreFilterList() {
            ...
            // Field NomorPengujianSampelLainnya
            if (filter?.TryGetValue("x_NomorPengujianSampelLainnya", out sv) ?? false) {
                NomorPengujianSampelLainnya.AdvancedSearch.SearchValue = sv;
                NomorPengujianSampelLainnya.AdvancedSearch.SearchOperator = filter["z_NomorPengujianSampelLainnya"];
                NomorPengujianSampelLainnya.AdvancedSearch.SearchCondition = filter["v_NomorPengujianSampelLainnya"];
                NomorPengujianSampelLainnya.AdvancedSearch.SearchValue2 = filter["y_NomorPengujianSampelLainnya"];
                NomorPengujianSampelLainnya.AdvancedSearch.SearchOperator2 = filter["w_NomorPengujianSampelLainnya"];
                NomorPengujianSampelLainnya.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, NomorPengujianSampelLainnya, def, true); // NomorPengujianSampelLainnya
            ...
            BuildSearchSql(ref where, Keterangan, def, false); // Keterangan
            ...
            if (!def && Command == "search") {
                NomorPengujianSampelLainnya.AdvancedSearch.Save(); // NomorPengujianSampelLainnya
        ...
        public void ShowFilterList()
        {
            ...
            // Field NomorPengujianSampelLainnya
            filter = QueryBuilderWhere("NomorPengujianSampelLainnya");
            if (Empty(filter))
                BuildSearchSql(ref filter, NomorPengujianSampelLainnya, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NomorPengujianSampelLainnya.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Produk, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Keterangan, false, false);
            if (!Empty(filter))
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NomorPengujianSampelLainnya);
            searchFlds.Add(Produk);
            searchFlds.Add(UploadHasil);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (NomorPengujianSampelLainnya.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            NomorPengujianSampelLainnya.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            NomorPengujianSampelLainnya.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(NomorPengujianSampelLainnya, ctrl); // NomorPengujianSampelLainnya
                UpdateSort(idPlant, ctrl); // idPlant
                UpdateSort(Tanggal, ctrl); // Tanggal
                UpdateSort(Produk, ctrl); // Produk
                UpdateSort(CF_UploadHasil, ctrl); // CF_UploadHasil
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    id.Sort = "";
                    NomorPengujianSampelLainnya.Sort = "";
                    idPlant.Sort = "";
                    Tanggal.Sort = "";
                    Produk.Sort = "";
                    UploadHasil.Sort = "";
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""PengujianSampleLainnya"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""PengujianSampleLainnya"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""PengujianSampleLainnya"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fPengujianSampleLainnyalist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fPengujianSampleLainnyalist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""PengujianSampleLainnya"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fPengujianSampleLainnyalist""" +
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fPengujianSampleLainnyasrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fPengujianSampleLainnyasrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fPengujianSampleLainnyalist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_PengujianSampleLainnya");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(pengujianSampleLainnyaList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(pengujianSampleLainnyaList.RowCount) + "_PengujianSampleLainnya");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(pengujianSampleLainnyaList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && pengujianSampleLainnyaList.RowType == RowType.Add || IsEdit && pengujianSampleLainnyaList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NomorPengujianSampelLainnya[]"))
                    NomorPengujianSampelLainnya.AdvancedSearch.SearchValue = Get("x_NomorPengujianSampelLainnya[]", Config.FilterOptionSeparator);
                else
                    NomorPengujianSampelLainnya.AdvancedSearch.SearchValue = Get("NomorPengujianSampelLainnya"); // Default Value // DN
            if (!Empty(NomorPengujianSampelLainnya.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_NomorPengujianSampelLainnya"))
                NomorPengujianSampelLainnya.AdvancedSearch.SearchOperator = Get("z_NomorPengujianSampelLainnya", Config.FilterOptionSeparator);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("NomorPengujianSampelLainnya", NomorPengujianSampelLainnya.DefaultValue ?? DbNullValue); // DN
            ...
            row.Add("UploadHasil", UploadHasil.DefaultValue ?? DbNullValue); // DN
            row.Add("CF_UploadHasil", CF_UploadHasil.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (!Empty(curVal2)) {
                    if (Produk.Lookup != null && IsDictionary(Produk.Lookup?.Options) && Produk.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        ...
                        if (rswrk2?.Count > 0 && Produk.Lookup != null) { // Lookup values found
                            ...
                            Produk.ViewValue = Produk.CurrentValue;
                        }
                    }
                } else {
                ...
                // NomorPengujianSampelLainnya
                if (NomorPengujianSampelLainnya.UseFilter && !Empty(NomorPengujianSampelLainnya.AdvancedSearch.SearchValue)) {
                    NomorPengujianSampelLainnya.EditValue = ConvertToString(NomorPengujianSampelLainnya.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // Produk
                if (Produk.UseFilter && !Empty(Produk.AdvancedSearch.SearchValue)) {
                    Produk.EditValue = ConvertToString(Produk.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // CF_UploadHasil
                CF_UploadHasil.SetupEditAttributes();
                CF_UploadHasil.EditValue = CF_UploadHasil.AdvancedSearch.SearchValue; // DN
                CF_UploadHasil.PlaceHolder = RemoveHtml(CF_UploadHasil.Caption);

                // Keterangan
                Keterangan.SetupEditAttributes();
                Keterangan.EditValue = Keterangan.AdvancedSearch.SearchValue; // DN
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fPengujianSampleLainnyasrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
