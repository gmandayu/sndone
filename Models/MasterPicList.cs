...
public partial class SnDOne {
    ...
    /// masterPicList
    /// </summary>
    public static MasterPicList masterPicList
    {
        get => HttpData.Get<MasterPicList>("masterPicList")!;
        set => HttpData["masterPicList"] = value;
    }

    /// <summary>
    /// Page class for MasterPIC
    /// </summary>
    public class MasterPicList : MasterPicListBase
    {
        // Constructor
        public MasterPicList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterPicList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterPicListBase : MasterPic
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterPicList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMasterPIClist";
        ...
        // Constructor
        public MasterPicListBase(Controller? controller)
        {
            TableName = "MasterPIC";
        ...
        if (controller != null)
            ...
            // Table object (masterPic)
            if (masterPic == null || masterPic is MasterPic)
                masterPic = this;
            ...
            MultiDeleteUrl = "MasterPicDelete";
            MultiUpdateUrl = "MasterPicUpdate";
        ...
        // Page name
        public string PageName => "MasterPicList";
        ...
        {
            IdPIC.SetVisibility();
            NamaPIC.SetVisibility();
        }

        // Constructor
        public MasterPicListBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                var result = new Dictionary<string, string> { { "url", GetUrl(url) }, { "modal", "1" } }; // modal=1
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "MasterPicView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
...
protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("IdPIC") ? dict["IdPIC"] : IdPIC.CurrentValue));
            return key;
        }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, NamaPIC, def, true); // NamaPIC
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("NamaPIC", NamaPIC, true, config);
            return filterList;
        }
        ...
        protected bool CheckSearchParms() {
            ...
            if (IdPIC.AdvancedSearch.IssetSession)
                return true;
            if (NamaPIC.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            NamaPIC.AdvancedSearch.UnsetSession();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(NamaPIC, ctrl); // NamaPIC
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MasterPIC"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MasterPIC"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MasterPIC"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMasterPIClist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMasterPIClist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdPIC.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MasterPIC"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMasterPIClist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("NamaPIC")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMasterPICsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMasterPICsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMasterPIClist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(masterPicList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(masterPicList.RowCount) + "_MasterPIC");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(masterPicList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && masterPicList.RowType == RowType.Add || IsEdit && masterPicList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldNamaPICSearchValues()
        {
            // NamaPIC
            LoadPrimarySearchValueNamaPIC();
            LoadSearchOperatorNamaPIC();
        }

        private void LoadPrimarySearchValueNamaPIC()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaPIC[]")) {
                NamaPIC.AdvancedSearch.SearchValue = Get("x_NamaPIC[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NamaPIC.AdvancedSearch.SearchValue = Get("NamaPIC"); // Default Value // DN
            }
            if (!Empty(NamaPIC.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNamaPIC()
        {
            if (Query.ContainsKey("z_NamaPIC"))
                NamaPIC.AdvancedSearch.SearchOperator = Get("z_NamaPIC", Config.FilterOptionSeparator);
        }
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            NamaPIC.SetDbValue(row["NamaPIC"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("NamaPIC", NamaPIC.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                // NamaPIC
                    ...
                    // NamaPIC
                    NamaPIC.ViewValue = ConvertToString(NamaPIC.CurrentValue); // DN
                    NamaPIC.ViewCustomAttributes = "";
                ...
                // NamaPIC
                if (NamaPIC.UseFilter && !Empty(NamaPIC.AdvancedSearch.SearchValue)) {
                    NamaPIC.EditValue = ConvertToString(NamaPIC.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMasterPICsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
