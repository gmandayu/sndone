...
public partial class SnDOne {
    ...
    public class SetPjsListBase : SetPjs
    {
        ...
        // Constructor
        public SetPjsListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected bool RestoreFilterList() {
            ...
            if (filter?.TryGetValue("x_DownloadSuratTugas", out sv) ?? false) {
                DownloadSuratTugas.AdvancedSearch.SearchValue = sv;
                DownloadSuratTugas.AdvancedSearch.SearchOperator = filter["z_DownloadSuratTugas"];
                DownloadSuratTugas.AdvancedSearch.SearchCondition = filter["v_DownloadSuratTugas"];
                DownloadSuratTugas.AdvancedSearch.SearchValue2 = filter["y_DownloadSuratTugas"];
                DownloadSuratTugas.AdvancedSearch.SearchOperator2 = filter["w_DownloadSuratTugas"];
                DownloadSuratTugas.AdvancedSearch.Save();
            }

            // Field Remaks
            if (filter?.TryGetValue("x_Remaks", out sv) ?? false) {
                Remaks.AdvancedSearch.SearchValue = sv;
                Remaks.AdvancedSearch.SearchOperator = filter["z_Remaks"];
                Remaks.AdvancedSearch.SearchCondition = filter["v_Remaks"];
                Remaks.AdvancedSearch.SearchValue2 = filter["y_Remaks"];
                Remaks.AdvancedSearch.SearchOperator2 = filter["w_Remaks"];
                Remaks.AdvancedSearch.Save();
            }
        ...
        public void ShowFilterList()
        {
            ...
            // Field RedirectLink
            filter = QueryBuilderWhere("RedirectLink");
            if (Empty(filter))
                BuildSearchSql(ref filter, RedirectLink, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + RedirectLink.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            filter = QueryBuilderWhere("OrganizationLevel");
            if (Empty(filter))
                BuildSearchSql(ref filter, OrganizationLevel, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + OrganizationLevel.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field PosisiAwal
            filter = QueryBuilderWhere("PosisiAwal");
            if (Empty(filter))
                BuildSearchSql(ref filter, PosisiAwal, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + PosisiAwal.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field PosisiPJS
            filter = QueryBuilderWhere("PosisiPJS");
            if (Empty(filter))
                BuildSearchSql(ref filter, PosisiPJS, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + PosisiPJS.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field DownloadSuratTugas
            filter = QueryBuilderWhere("DownloadSuratTugas");
            if (Empty(filter))
                BuildSearchSql(ref filter, DownloadSuratTugas, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DownloadSuratTugas.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Remaks
            filter = QueryBuilderWhere("Remaks");
            if (Empty(filter))
                BuildSearchSql(ref filter, Remaks, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Remaks.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field DiperbaharuiOleh
            filter = QueryBuilderWhere("DiperbaharuiOleh");
            if (Empty(filter))
                BuildSearchSql(ref filter, DiperbaharuiOleh, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DiperbaharuiOleh.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field DiperbaharuiTanggal
            filter = QueryBuilderWhere("DiperbaharuiTanggal");
            if (Empty(filter))
                BuildSearchSql(ref filter, DiperbaharuiTanggal, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DiperbaharuiTanggal.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        protected void LoadSearchValues() {
            ...
            // RedirectLink
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_RedirectLink[]"))
            ...
            // OrganizationLevel
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_OrganizationLevel[]"))
            ...
            // PosisiAwal
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_PosisiAwal[]"))
            ...
            // PosisiPJS
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_PosisiPJS[]"))
            ...
            // DownloadSuratTugas
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DownloadSuratTugas"))
                    DownloadSuratTugas.AdvancedSearch.SearchValue = Get("x_DownloadSuratTugas", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z_DownloadSuratTugas"))
                DownloadSuratTugas.AdvancedSearch.SearchOperator = Get("z_DownloadSuratTugas", Config.FilterOptionSeparator);

            // Remaks
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Remaks[]"))
            ...
            // DiperbaharuiOleh
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DiperbaharuiOleh[]"))
            ...
            // DiperbaharuiTanggal
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DiperbaharuiTanggal[]"))
