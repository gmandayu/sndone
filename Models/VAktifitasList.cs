...
public partial class SnDOne {
    ...
    /// vAktifitasList
    /// </summary>
    public static VAktifitasList vAktifitasList
    {
        get => HttpData.Get<VAktifitasList>("vAktifitasList")!;
        set => HttpData["vAktifitasList"] = value;
    }

    /// <summary>
    /// Page class for v_aktifitas
    /// </summary>
    public class VAktifitasList : VAktifitasListBase
    {
        // Constructor
        public VAktifitasList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public VAktifitasList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class VAktifitasListBase : VAktifitas
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "vAktifitasList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fv_aktifitaslist";
        ...
        // Constructor
        public VAktifitasListBase()
        {
            TableName = "v_aktifitas";

            // CSS class name as context
            TableVar = "v_aktifitas";
            ...
            // Table object (vAktifitas)
            if (vAktifitas == null || vAktifitas is VAktifitas)
                vAktifitas = this;

            // Initialize URLs
            AddUrl = "VAktifitasAdd";
            ...
            MultiDeleteUrl = "VAktifitasDelete";
            MultiUpdateUrl = "VAktifitasUpdate";
        ...
        // Page name
        public string PageName => "VAktifitasList";
        ...
        // Constructor
        public VAktifitasListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "VAktifitasView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fv_aktifitaslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fv_aktifitaslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fv_aktifitassrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fv_aktifitassrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fv_aktifitaslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_v_aktifitas");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(vAktifitasList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(vAktifitasList.RowCount) + "_v_aktifitas");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(vAktifitasList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && vAktifitasList.RowType == RowType.Add || IsEdit && vAktifitasList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // NamaAktivitas
                NamaAktivitas.ViewValue = NamaAktivitas.CurrentValue;
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fv_aktifitassrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
