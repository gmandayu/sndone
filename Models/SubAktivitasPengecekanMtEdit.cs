...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasPengecekanMtEdit
    /// </summary>
    public static SubAktivitasPengecekanMtEdit subAktivitasPengecekanMtEdit
    {
        get => HttpData.Get<SubAktivitasPengecekanMtEdit>("subAktivitasPengecekanMtEdit")!;
        set => HttpData["subAktivitasPengecekanMtEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasPengecekanMT
    /// </summary>
    public class SubAktivitasPengecekanMtEdit : SubAktivitasPengecekanMtEditBase
    {
        // Constructor
        public SubAktivitasPengecekanMtEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasPengecekanMtEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasPengecekanMtEditBase : SubAktivitasPengecekanMt
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasPengecekanMtEdit";
        ...
        // Constructor
        public SubAktivitasPengecekanMtEditBase(Controller? controller)
        {
            TableName = "SubAktivitasPengecekanMT";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasPengecekanMt)
            if (subAktivitasPengecekanMt == null || subAktivitasPengecekanMt is SubAktivitasPengecekanMt)
                subAktivitasPengecekanMt = this;
        ...
        // Page name
        public string PageName => "SubAktivitasPengecekanMtEdit";
        ...
        public void SetVisibility()
        {
            ...
            JamPemeriksaan.SetVisibility();
            ExTangki.SetVisibility();
        ...
        // Constructor
        public SubAktivitasPengecekanMtEditBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                ...
                // List / View / Master View page
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasPengecekanMtView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasPengecekanMtList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasPengecekanMtList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasPengecekanMtList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasPengecekanMtEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'JamPemeriksaan'
            JamPemeriksaan.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(JamPemeriksaan, "JamPemeriksaan", "x_JamPemeriksaan", true, validate, true);

            // Standard handling for 'ExTangki'
            ExTangki.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(ExTangki, "ExTangki", "x_ExTangki");
        ...
        public void RestoreFormValues()
        {
            ...
            JamPemeriksaan.CurrentValue = JamPemeriksaan.FormValue;
            JamPemeriksaan.CurrentValue = UnformatDateTime(JamPemeriksaan.CurrentValue, JamPemeriksaan.FormatPattern);
            ExTangki.CurrentValue = ExTangki.FormValue;
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("JamPemeriksaan", JamPemeriksaan.DefaultValue ?? DbNullValue); // DN
            row.Add("ExTangki", ExTangki.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // JamPemeriksaan
            JamPemeriksaan.RowCssClass = "row";

            // ExTangki
            ExTangki.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                ExTangki.EditValue = HtmlEncode(ExTangki.CurrentValue);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomProduk() {
            if (Produk.Visible && Produk.Required) {
                if (!Produk.IsDetailKey && Empty(Produk.FormValue)) {
                    Produk.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNomorPolisi() {
            if (NomorPolisi.Visible && NomorPolisi.Required) {
                if (!NomorPolisi.IsDetailKey && Empty(NomorPolisi.FormValue)) {
                    NomorPolisi.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }

        private void ValidateCustomJamPemeriksaan() {
            if (JamPemeriksaan.Visible && JamPemeriksaan.Required) {
                if (!JamPemeriksaan.IsDetailKey && Empty(JamPemeriksaan.FormValue)) {
                    JamPemeriksaan.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }

        private void ValidateCustomExTangki() {
            if (ExTangki.Visible && ExTangki.Required) {
                if (!ExTangki.IsDetailKey && Empty(ExTangki.FormValue)) {
                    ExTangki.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomJamPemeriksaan();
                if (!CheckDate(JamPemeriksaan.FormValue, JamPemeriksaan.FormatPattern)) {
                    JamPemeriksaan.AddErrorMessage(JamPemeriksaan.GetErrorMessage(false));
                }
                ValidateCustomExTangki();
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // JamPemeriksaan
            JamPemeriksaan.SetDbValue(rsnew, ConvertToDateTime(JamPemeriksaan.CurrentValue, JamPemeriksaan.FormatPattern), JamPemeriksaan.ReadOnly);

            // ExTangki
            ExTangki.SetDbValue(rsnew, ExTangki.CurrentValue, ExTangki.ReadOnly);
        ...
        {
            object? value;
            if (row.TryGetValue("JamPemeriksaan", out value)) // JamPemeriksaan
                JamPemeriksaan.CurrentValue = value;
            if (row.TryGetValue("ExTangki", out value)) // ExTangki
                ExTangki.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasPengecekanMtList")), "", TableVar, true);
            string pageId = "edit";
