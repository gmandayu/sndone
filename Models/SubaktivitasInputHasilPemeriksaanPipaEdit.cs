...
public partial class SnDOne {
    ...
    public class SubaktivitasInputHasilPemeriksaanPipaEditBase : SubaktivitasInputHasilPemeriksaanPipa
    {
        ...
        // Constructor
        public SubaktivitasInputHasilPemeriksaanPipaEditBase(Controller? controller)
        {
        ...
        // Constructor
        public SubaktivitasInputHasilPemeriksaanPipaEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubaktivitasInputHasilPemeriksaanPipaList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "SubaktivitasInputHasilPemeriksaanPipaList") {
                        ...
                        returnUrl = "SubaktivitasInputHasilPemeriksaanPipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'QuantityDischarge'
            QuantityDischarge.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(QuantityDischarge, "QuantityDischarge", "x_QuantityDischarge");
        ...
        private void ValidateCustomNamaKegiatan() {
            if (NamaKegiatan.Visible && NamaKegiatan.Required) {
                if (!NamaKegiatan.IsDetailKey && Empty(NamaKegiatan.FormValue)) {
                    NamaKegiatan.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomDensity() {
            if (Density.Visible && Density.Required) {
                if (!Density.IsDetailKey && Empty(Density.FormValue)) {
                    Density.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTemperature() {
            if (Temperature.Visible && Temperature.Required) {
                if (!Temperature.IsDetailKey && Empty(Temperature.FormValue)) {
                    Temperature.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomPressure() {
            if (Pressure.Visible && Pressure.Required) {
                if (!Pressure.IsDetailKey && Empty(Pressure.FormValue)) {
                    Pressure.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomKeterangan() {
            if (Keterangan.Visible && Keterangan.Required) {
                if (!Keterangan.IsDetailKey && Empty(Keterangan.FormValue)) {
                    Keterangan.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }

        private void ValidateCustomQuantityDischarge() {
            if (QuantityDischarge.Visible && QuantityDischarge.Required) {
                if (!QuantityDischarge.IsDetailKey && Empty(QuantityDischarge.FormValue)) {
                    QuantityDischarge.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomQuantityDischarge();
