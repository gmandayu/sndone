...
public partial class SnDOne {
    ...
    public class MasterProdukListBase : MasterProduk
    {
        ...
        // Constructor
        public MasterProdukListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public void ShowFilterList()
        {
            ...
            // Field NoProduk
            filter = QueryBuilderWhere("NoProduk");
            if (Empty(filter))
                BuildSearchSql(ref filter, NoProduk, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NoProduk.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field NamaProduk
            filter = QueryBuilderWhere("NamaProduk");
            if (Empty(filter))
                BuildSearchSql(ref filter, NamaProduk, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NamaProduk.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field DensityMin
            filter = QueryBuilderWhere("DensityMin");
            if (Empty(filter))
                BuildSearchSql(ref filter, DensityMin, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DensityMin.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field DensityMax
            filter = QueryBuilderWhere("DensityMax");
            if (Empty(filter))
                BuildSearchSql(ref filter, DensityMax, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DensityMax.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        protected void LoadSearchValues() {
            ...
            // NoProduk
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NoProduk[]"))
            ...
            // NamaProduk
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaProduk[]"))
            ...
            // DensityMin
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DensityMin[]"))
            ...
            // DensityMax
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DensityMax[]"))
