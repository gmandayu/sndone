...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiAgreementPenKonEdit
    /// </summary>
    public static SubAktivitasNilaiAgreementPenKonEdit subAktivitasNilaiAgreementPenKonEdit
    {
        get => HttpData.Get<SubAktivitasNilaiAgreementPenKonEdit>("subAktivitasNilaiAgreementPenKonEdit")!;
        set => HttpData["subAktivitasNilaiAgreementPenKonEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiAgreementPenKon
    /// </summary>
    public class SubAktivitasNilaiAgreementPenKonEdit : SubAktivitasNilaiAgreementPenKonEditBase
    {
        // Constructor
        public SubAktivitasNilaiAgreementPenKonEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiAgreementPenKonEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiAgreementPenKonEditBase : SubAktivitasNilaiAgreementPenKon
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiAgreementPenKonEdit";
        ...
        // Constructor
        public SubAktivitasNilaiAgreementPenKonEditBase(Controller? controller)
        {
            TableName = "SubAktivitasNilaiAgreementPenKon";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasNilaiAgreementPenKon)
            if (subAktivitasNilaiAgreementPenKon == null || subAktivitasNilaiAgreementPenKon is SubAktivitasNilaiAgreementPenKon)
                subAktivitasNilaiAgreementPenKon = this;
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiAgreementPenKonEdit";
        ...
        // Constructor
        public SubAktivitasNilaiAgreementPenKonEditBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                ...
                // List / View / Master View page
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasNilaiAgreementPenKonView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasNilaiAgreementPenKonList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasNilaiAgreementPenKonList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasNilaiAgreementPenKonList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasNilaiAgreementPenKonEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        private void ValidateCustomFlowrate() {
            if (Flowrate.Visible && Flowrate.Required) {
                if (!Flowrate.IsDetailKey && Empty(Flowrate.FormValue)) {
                    Flowrate.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        private void ValidateCustomPressure() {
            if (Pressure.Visible && Pressure.Required) {
                if (!Pressure.IsDetailKey && Empty(Pressure.FormValue)) {
                    Pressure.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStrippingTime() {
            if (StrippingTime.Visible && StrippingTime.Required) {
                if (!StrippingTime.IsDetailKey && Empty(StrippingTime.FormValue)) {
                    StrippingTime.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        private void ValidateCustomETC() {
            if (ETC.Visible && ETC.Required) {
                if (!ETC.IsDetailKey && Empty(ETC.FormValue)) {
                    ETC.AddErrorMessage(ConvertToString(LamaPembongkaran.RequiredErrorMessage).Replace("%s", LamaPembongkaran.Caption));
                }
            }
        }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // LamaPembongkaran
            LamaPembongkaran.SetDbValue(rsnew, LamaPembongkaran.CurrentValue, LamaPembongkaran.ReadOnly);
            return rsnew;
        }
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("LamaPembongkaran", out value)) // LamaPembongkaran
                LamaPembongkaran.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasNilaiAgreementPenKonList")), "", TableVar, true);
            string pageId = "edit";
