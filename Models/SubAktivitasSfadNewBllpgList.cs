...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasSfadNewBllpgList
    /// </summary>
    public static SubAktivitasSfadNewBllpgList subAktivitasSfadNewBllpgList
    {
        get => HttpData.Get<SubAktivitasSfadNewBllpgList>("subAktivitasSfadNewBllpgList")!;
        set => HttpData["subAktivitasSfadNewBllpgList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasSFADNewBLLPG
    /// </summary>
    public class SubAktivitasSfadNewBllpgList : SubAktivitasSfadNewBllpgListBase
    {
        // Constructor
        public SubAktivitasSfadNewBllpgList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasSfadNewBllpgList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasSfadNewBllpgListBase : SubAktivitasSfadNewBllpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasSfadNewBllpgList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasSFADNewBLLPGlist";
        ...
        // Constructor
        public SubAktivitasSfadNewBllpgListBase()
        {
            TableName = "SubAktivitasSFADNewBLLPG";
            ...
            // Table object (subAktivitasSfadNewBllpg)
            if (subAktivitasSfadNewBllpg == null || subAktivitasSfadNewBllpg is SubAktivitasSfadNewBllpg)
                subAktivitasSfadNewBllpg = this;

            // Initialize URLs
            AddUrl = "SubAktivitasSfadNewBllpgAdd";
            ...
            MultiDeleteUrl = "SubAktivitasSfadNewBllpgDelete";
            MultiUpdateUrl = "SubAktivitasSfadNewBllpgUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasSfadNewBllpgList";
        ...
        // Constructor
        public SubAktivitasSfadNewBllpgListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasSfadNewBllpgView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasSFADNewBLLPGgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasSfadNewBllpgList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasSFADNewBLLPG"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLLPGlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLLPGlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasSFADNewBLLPGsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasSFADNewBLLPGsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasSFADNewBLLPGlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasSFADNewBLLPG");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasSfadNewBllpgList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasSfadNewBllpgList.RowCount) + "_SubAktivitasSFADNewBLLPG");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasSfadNewBllpgList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasSfadNewBllpgList.RowType == RowType.Add || IsEdit && subAktivitasSfadNewBllpgList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasSFADNewBLLPGsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
