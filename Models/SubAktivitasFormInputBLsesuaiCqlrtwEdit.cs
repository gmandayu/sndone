...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasFormInputBLsesuaiCqlrtwEdit
    /// </summary>
    public static SubAktivitasFormInputBLsesuaiCqlrtwEdit subAktivitasFormInputBLsesuaiCqlrtwEdit
    {
        get => HttpData.Get<SubAktivitasFormInputBLsesuaiCqlrtwEdit>("subAktivitasFormInputBLsesuaiCqlrtwEdit")!;
        set => HttpData["subAktivitasFormInputBLsesuaiCqlrtwEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasFormInputBLsesuaiCQLRTW
    /// </summary>
    public class SubAktivitasFormInputBLsesuaiCqlrtwEdit : SubAktivitasFormInputBLsesuaiCqlrtwEditBase
    {
        // Constructor
        public SubAktivitasFormInputBLsesuaiCqlrtwEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasFormInputBLsesuaiCqlrtwEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasFormInputBLsesuaiCqlrtwEditBase : SubAktivitasFormInputBLsesuaiCqlrtw
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasFormInputBLsesuaiCqlrtwEdit";
        ...
        // Constructor
        public SubAktivitasFormInputBLsesuaiCqlrtwEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasFormInputBLsesuaiCqlrtwEdit";
        ...
        // Constructor
        public SubAktivitasFormInputBLsesuaiCqlrtwEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasFormInputBLsesuaiCqlrtwList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasFormInputBLsesuaiCqlrtwList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasFormInputBLsesuaiCqlrtwList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasFormInputBLsesuaiCqlrtwEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'ML'
            ML.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(ML, "ML", "x_ML");
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                ML.EditValue = HtmlEncode(ML.CurrentValue);
        ...
        private void ValidateCustomKL_Obs() {
            if (KL_Obs.Visible && KL_Obs.Required) {
                if (!KL_Obs.IsDetailKey && Empty(KL_Obs.FormValue)) {
                    KL_Obs.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        private void ValidateCustomKL_15() {
            if (KL_15.Visible && KL_15.Required) {
                if (!KL_15.IsDetailKey && Empty(KL_15.FormValue)) {
                    KL_15.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        private void ValidateCustomML() {
            if (ML.Visible && ML.Required) {
                if (!ML.IsDetailKey && Empty(ML.FormValue)) {
                    ML.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // ML
            ML.SetDbValue(rsnew, ML.CurrentValue, ML.ReadOnly);
            return rsnew;
        }
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("ML", out value)) // ML
                ML.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasFormInputBLsesuaiCqlrtwList")), "", TableVar, true);
            string pageId = "edit";
