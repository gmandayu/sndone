...
public partial class SnDOne {
    /// <summary>
    /// controlRutinSecurityAdd
    /// </summary>
    public static ControlRutinSecurityAdd controlRutinSecurityAdd
    {
        get => HttpData.Get<ControlRutinSecurityAdd>("controlRutinSecurityAdd")!;
        set => HttpData["controlRutinSecurityAdd"] = value;
    }

    /// <summary>
    /// Page class for ControlRutinSecurity
    /// </summary>
    public class ControlRutinSecurityAdd : ControlRutinSecurityAddBase
    {
        // Constructor
        public ControlRutinSecurityAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ControlRutinSecurityAdd() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class ControlRutinSecurityAddBase : ControlRutinSecurity
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "controlRutinSecurityAdd";
        ...
        // Constructor
        public ControlRutinSecurityAddBase()
        {
        ...
        // Page name
        public string PageName => "ControlRutinSecurityAdd";
        ...
        // Constructor
        public ControlRutinSecurityAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdCRS", out rv)) { // DN
                    IdCRS.QueryValue = UrlDecode(rv); // DN
                } else if (Get("IdCRS", out sv)) {
                    IdCRS.QueryValue = sv.ToString();
                }
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("ControlRutinSecurityList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        string returnUrl = "";
                        returnUrl = ReturnUrl;
                        if (GetPageName(returnUrl) == "ControlRutinSecurityList")
                            returnUrl = AddMasterUrl(ListUrl); // List page, return to List page with correct master key if necessary
                        else if (GetPageName(returnUrl) == "ControlRutinSecurityView")
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "ControlRutinSecurityList") {
                                ...
                                returnUrl = "ControlRutinSecurityList"; // Return list page content
                            }
                        }
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                controlRutinSecurityAdd?.PageRender();
            }
            return PageResult();
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NomorCRS' before field var 'x_NomorCRS'
            val = CurrentForm.HasValue("NomorCRS") ? CurrentForm.GetValue("NomorCRS") : CurrentForm.GetValue("x_NomorCRS");
            if (!NomorCRS.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NomorCRS") && !CurrentForm.HasValue("x_NomorCRS")) // DN
                    NomorCRS.Visible = false; // Disable update for API request
                else
                    NomorCRS.SetFormValue(val);
            }
            ...
            if (!Plant.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Plant") && !CurrentForm.HasValue("x_Plant")) // DN
                    ...
                    Plant.SetFormValue(val, true, validate);
            }
            ...
            // Check field name 'IdCRS' before field var 'x_IdCRS'
            val = CurrentForm.HasValue("IdCRS") ? CurrentForm.GetValue("IdCRS") : CurrentForm.GetValue("x_IdCRS");
        }
        #pragma warning restore 1998
        ...
        {
            NomorCRS.CurrentValue = NomorCRS.FormValue;
        ...
        public async Task RenderRow()
        {
            ...
            // IdCRS
            IdCRS.RowCssClass = "row";

            // NomorCRS
            NomorCRS.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                if (!NomorCRS.Raw)
                    NomorCRS.CurrentValue = HtmlDecode(NomorCRS.CurrentValue);
                NomorCRS.EditValue = HtmlEncode(NomorCRS.CurrentValue);
                NomorCRS.PlaceHolder = RemoveHtml(NomorCRS.Caption);
                ...
                if (!Empty(curVal2)) {
                    if (Plant.Lookup != null && IsDictionary(Plant.Lookup?.Options) && Plant.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        ...
                        if (rswrk2?.Count > 0 && Plant.Lookup != null) { // Lookup values found
                            ...
                            Plant.EditValue = HtmlEncode(FormatNumber(Plant.CurrentValue, Plant.FormatPattern));
                        }
                    }
                } else {
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                if (NomorCRS.Visible && NomorCRS.Required) {
                    if (!NomorCRS.IsDetailKey && Empty(NomorCRS.FormValue)) {
                        NomorCRS.AddErrorMessage(ConvertToString(NomorCRS.RequiredErrorMessage).Replace("%s", NomorCRS.Caption));
                    }
                }
                ...
                if (!CheckInteger(Plant.FormValue)) {
                    Plant.AddErrorMessage(Plant.GetErrorMessage(false));
                }
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            if (insertRow) {
                try {
                    ...
                    rsnew["IdCRS"] = IdCRS.CurrentValue!;
                } catch (Exception e) {
                    if (Config.Debug)
                        throw;
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // NomorCRS
                NomorCRS.SetDbValue(rsnew, NomorCRS.CurrentValue);
        ...
        {
            object? value;
            if (row.TryGetValue("NomorCRS", out value)) // NomorCRS
                NomorCRS.SetFormValue(ConvertToString(value));
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("ControlRutinSecurityList")), "", TableVar, true);
