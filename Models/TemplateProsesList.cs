...
public partial class SnDOne {
    ...
    public class TemplateProsesListBase : TemplateProses
    {
        ...
        // Constructor
        public TemplateProsesListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public void ShowFilterList()
        {
            ...
            // Field UrutanProses
            filter = QueryBuilderWhere("UrutanProses");
            if (Empty(filter))
                BuildSearchSql(ref filter, UrutanProses, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + UrutanProses.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field NamaProses
            filter = QueryBuilderWhere("NamaProses");
            if (Empty(filter))
                BuildSearchSql(ref filter, NamaProses, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NamaProses.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field IdTools
            filter = QueryBuilderWhere("IdTools");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdTools, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdTools.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected void LoadSearchValues() {
            ...
            // UrutanProses
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_UrutanProses[]"))
            ...
            // NamaProses
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaProses[]"))
            ...
            // IdTools
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdTools[]"))
