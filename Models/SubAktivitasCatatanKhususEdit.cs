...
public partial class SnDOne {
    ...
    public class SubAktivitasCatatanKhususEditBase : SubAktivitasCatatanKhusus
    {
        ...
        // Constructor
        public SubAktivitasCatatanKhususEditBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasCatatanKhususEditBase() : this(null) { }
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasCatatanKhususList");
            }
            return null;
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "SubAktivitasCatatanKhususList") {
                        ...
                        returnUrl = "SubAktivitasCatatanKhususList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'Kontaminasi'
            Kontaminasi.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Kontaminasi, "Kontaminasi", "x_Kontaminasi");

            // Standard handling for 'Insident'
            Insident.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Insident, "Insident", "x_Insident");

            // Standard handling for 'CuacaBuruk'
            CuacaBuruk.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(CuacaBuruk, "CuacaBuruk", "x_CuacaBuruk");
        ...
        private void ValidateCustomKontaminasi() {
            if (Kontaminasi.Visible && Kontaminasi.Required) {
                if (!Kontaminasi.IsDetailKey && Empty(Kontaminasi.FormValue)) {
                    Kontaminasi.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }

        private void ValidateCustomInsident() {
            if (Insident.Visible && Insident.Required) {
                if (!Insident.IsDetailKey && Empty(Insident.FormValue)) {
                    Insident.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }

        private void ValidateCustomCuacaBuruk() {
            if (CuacaBuruk.Visible && CuacaBuruk.Required) {
                if (!CuacaBuruk.IsDetailKey && Empty(CuacaBuruk.FormValue)) {
                    CuacaBuruk.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        private void ValidateCustomKeterangan() {
            if (Keterangan.Visible && Keterangan.Required) {
                if (!Keterangan.IsDetailKey && Empty(Keterangan.FormValue)) {
                    Keterangan.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomKontaminasi();
                ValidateCustomInsident();
                ValidateCustomCuacaBuruk();
