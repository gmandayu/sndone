...
public partial class SnDOne {
    /// <summary>
    /// subaktivitasInputLogbookPenyaluranPipaBackupAdd
    /// </summary>
    public static SubaktivitasInputLogbookPenyaluranPipaBackupAdd subaktivitasInputLogbookPenyaluranPipaBackupAdd
    {
        get => HttpData.Get<SubaktivitasInputLogbookPenyaluranPipaBackupAdd>("subaktivitasInputLogbookPenyaluranPipaBackupAdd")!;
        set => HttpData["subaktivitasInputLogbookPenyaluranPipaBackupAdd"] = value;
    }

    /// <summary>
    /// Page class for SubaktivitasInputLogbookPenyaluranPipa_Backup
    /// </summary>
    public class SubaktivitasInputLogbookPenyaluranPipaBackupAdd : SubaktivitasInputLogbookPenyaluranPipaBackupAddBase
    {
        // Constructor
        public SubaktivitasInputLogbookPenyaluranPipaBackupAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubaktivitasInputLogbookPenyaluranPipaBackupAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubaktivitasInputLogbookPenyaluranPipaBackupAddBase : SubaktivitasInputLogbookPenyaluranPipaBackup
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subaktivitasInputLogbookPenyaluranPipaBackupAdd";
        ...
        // Constructor
        public SubaktivitasInputLogbookPenyaluranPipaBackupAddBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubaktivitasInputLogbookPenyaluranPipaBackupAdd";
        ...
        // Constructor
        public SubaktivitasInputLogbookPenyaluranPipaBackupAddBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private IActionResult? HandleCopyAction(Dictionary<string, object>? rsold)
        {
            if (rsold == null) {
                ...
                return Terminate("SubaktivitasInputLogbookPenyaluranPipaBackupList");
            }
            return null;
        }
        ...
        private string DetermineReturnUrl()
        {
            string returnUrl = "";
            returnUrl = ReturnUrl;
            if (GetPageName(returnUrl) == "SubaktivitasInputLogbookPenyaluranPipaBackupList")
                returnUrl = AddMasterUrl(ListUrl);
            else if (GetPageName(returnUrl) == "SubaktivitasInputLogbookPenyaluranPipaBackupView")
                returnUrl = ViewUrl;
            return returnUrl;
        }
        ...
        private void HandleAjaxActions(ref string returnUrl)
        {
            if (IsModal && UseAjaxActions) {
                ...
                if (GetPageName(returnUrl) != "SubaktivitasInputLogbookPenyaluranPipaBackupList") {
                    ...
                    returnUrl = "SubaktivitasInputLogbookPenyaluranPipaBackupList";
                }
            }
        }
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subaktivitasInputLogbookPenyaluranPipaBackupAdd?.PageRender();
                    }
        }
        ...
        private void ValidateCustomNomorTangki() {
            if (NomorTangki.Visible && NomorTangki.Required) {
                if (!NomorTangki.IsDetailKey && Empty(NomorTangki.FormValue)) {
                    NomorTangki.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLO() {
            if (LO.Visible && LO.Required) {
                if (!LO.IsDetailKey && Empty(LO.FormValue)) {
                    LO.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomProduk() {
            if (Produk.Visible && Produk.Required) {
                if (!Produk.IsDetailKey && Empty(Produk.FormValue)) {
                    Produk.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomMeterAwal() {
            if (MeterAwal.Visible && MeterAwal.Required) {
                if (!MeterAwal.IsDetailKey && Empty(MeterAwal.FormValue)) {
                    MeterAwal.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomMeterAkhir() {
            if (MeterAkhir.Visible && MeterAkhir.Required) {
                if (!MeterAkhir.IsDetailKey && Empty(MeterAkhir.FormValue)) {
                    MeterAkhir.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomQuantity() {
            if (Quantity.Visible && Quantity.Required) {
                if (!Quantity.IsDetailKey && Empty(Quantity.FormValue)) {
                    Quantity.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBayNumber() {
            if (BayNumber.Visible && BayNumber.Required) {
                if (!BayNumber.IsDetailKey && Empty(BayNumber.FormValue)) {
                    BayNumber.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomuserInput() {
            if (userInput.Visible && userInput.Required) {
                if (!userInput.IsDetailKey && Empty(userInput.FormValue)) {
                    userInput.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        private void ValidateCustometlDate() {
            if (etlDate.Visible && etlDate.Required) {
                if (!etlDate.IsDetailKey && Empty(etlDate.FormValue)) {
                    etlDate.AddErrorMessage(ConvertToString(NoReferensi.RequiredErrorMessage).Replace("%s", NoReferensi.Caption));
                }
            }
        }
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // NomorTangki
                NomorTangki.SetDbValue(rsnew, NomorTangki.CurrentValue);

                // LO
                LO.SetDbValue(rsnew, LO.CurrentValue);

                // Produk
                Produk.SetDbValue(rsnew, Produk.CurrentValue);

                // MeterAwal
                MeterAwal.SetDbValue(rsnew, MeterAwal.CurrentValue);

                // MeterAkhir
                MeterAkhir.SetDbValue(rsnew, MeterAkhir.CurrentValue);

                // Quantity
                Quantity.SetDbValue(rsnew, Quantity.CurrentValue);

                // BayNumber
                BayNumber.SetDbValue(rsnew, BayNumber.CurrentValue);
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("NomorTangki", out value)) // NomorTangki
                NomorTangki.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("LO", out value)) // LO
                LO.SetFormValue(ConvertToString(value));
            ...
            if (row.TryGetValue("MeterAwal", out value)) // MeterAwal
                MeterAwal.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("MeterAkhir", out value)) // MeterAkhir
                MeterAkhir.SetFormValue(ConvertToString(value));
            ...
            if (row.TryGetValue("BayNumber", out value)) // BayNumber
                BayNumber.SetFormValue(ConvertToString(value));
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubaktivitasInputLogbookPenyaluranPipaBackupList")), "", TableVar, true);
