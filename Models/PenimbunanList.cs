...
public partial class SnDOne {
    ...
    public class PenimbunanListBase : Penimbunan
    {
        ...
        // Constructor
        public PenimbunanListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected bool RestoreFilterList() {
            ...
            if (filter?.TryGetValue("x_LookupPenerimaan", out sv) ?? false) {
                LookupPenerimaan.AdvancedSearch.SearchValue = sv;
                LookupPenerimaan.AdvancedSearch.SearchOperator = filter["z_LookupPenerimaan"];
                LookupPenerimaan.AdvancedSearch.SearchCondition = filter["v_LookupPenerimaan"];
                LookupPenerimaan.AdvancedSearch.SearchValue2 = filter["y_LookupPenerimaan"];
                LookupPenerimaan.AdvancedSearch.SearchOperator2 = filter["w_LookupPenerimaan"];
                LookupPenerimaan.AdvancedSearch.Save();
            }
        ...
        public void ShowFilterList()
        {
            ...
            // Field IdPenerimaan
            filter = QueryBuilderWhere("IdPenerimaan");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdPenerimaan, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdPenerimaan.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected void LoadSearchValues() {
            ...
            // NomorPenimbunan
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NomorPenimbunan[]"))
            ...
            // LookupPenerimaan
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_LookupPenerimaan"))
                    LookupPenerimaan.AdvancedSearch.SearchValue = Get("x_LookupPenerimaan", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z_LookupPenerimaan"))
                LookupPenerimaan.AdvancedSearch.SearchOperator = Get("z_LookupPenerimaan", Config.FilterOptionSeparator);

            // IdPenerimaan
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdPenerimaan[]"))
