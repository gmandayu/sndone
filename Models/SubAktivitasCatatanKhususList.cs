...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasCatatanKhususList
    /// </summary>
    public static SubAktivitasCatatanKhususList subAktivitasCatatanKhususList
    {
        get => HttpData.Get<SubAktivitasCatatanKhususList>("subAktivitasCatatanKhususList")!;
        set => HttpData["subAktivitasCatatanKhususList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasCatatanKhusus
    /// </summary>
    public class SubAktivitasCatatanKhususList : SubAktivitasCatatanKhususListBase
    {
        // Constructor
        public SubAktivitasCatatanKhususList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasCatatanKhususList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasCatatanKhususListBase : SubAktivitasCatatanKhusus
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasCatatanKhususList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasCatatanKhususlist";
        ...
        // Constructor
        public SubAktivitasCatatanKhususListBase()
        {
            TableName = "SubAktivitasCatatanKhusus";

            // CSS class name as context
            TableVar = "SubAktivitasCatatanKhusus";
            ...
            // Table object (subAktivitasCatatanKhusus)
            if (subAktivitasCatatanKhusus == null || subAktivitasCatatanKhusus is SubAktivitasCatatanKhusus)
                subAktivitasCatatanKhusus = this;

            // Initialize URLs
            AddUrl = "SubAktivitasCatatanKhususAdd";
            ...
            MultiDeleteUrl = "SubAktivitasCatatanKhususDelete";
            MultiUpdateUrl = "SubAktivitasCatatanKhususUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasCatatanKhususList";
        ...
        public void SetVisibility()
        {
            ...
            Kontaminasi.SetVisibility();
            Insident.SetVisibility();
            CuacaBuruk.SetVisibility();
        ...
        // Constructor
        public SubAktivitasCatatanKhususListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasCatatanKhususView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            await SetupLookupOptions(Kontaminasi);
            await SetupLookupOptions(Insident);
            await SetupLookupOptions(CuacaBuruk);
            await SetupLookupOptions(StatusAktivitas);

            // Update form name to avoid conflict
            if (IsModal)
                FormName = "fSubAktivitasCatatanKhususgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasCatatanKhususList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Kontaminasi.AdvancedSearch.ToJson())); // Field Kontaminasi
            filters.Merge(JObject.Parse(Insident.AdvancedSearch.ToJson())); // Field Insident
            filters.Merge(JObject.Parse(CuacaBuruk.AdvancedSearch.ToJson())); // Field CuacaBuruk
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Kontaminasi
            if (filter?.TryGetValue("x_Kontaminasi", out sv) ?? false) {
                Kontaminasi.AdvancedSearch.SearchValue = sv;
                Kontaminasi.AdvancedSearch.SearchOperator = filter["z_Kontaminasi"];
                Kontaminasi.AdvancedSearch.SearchCondition = filter["v_Kontaminasi"];
                Kontaminasi.AdvancedSearch.SearchValue2 = filter["y_Kontaminasi"];
                Kontaminasi.AdvancedSearch.SearchOperator2 = filter["w_Kontaminasi"];
                Kontaminasi.AdvancedSearch.Save();
            }

            // Field Insident
            if (filter?.TryGetValue("x_Insident", out sv) ?? false) {
                Insident.AdvancedSearch.SearchValue = sv;
                Insident.AdvancedSearch.SearchOperator = filter["z_Insident"];
                Insident.AdvancedSearch.SearchCondition = filter["v_Insident"];
                Insident.AdvancedSearch.SearchValue2 = filter["y_Insident"];
                Insident.AdvancedSearch.SearchOperator2 = filter["w_Insident"];
                Insident.AdvancedSearch.Save();
            }

            // Field CuacaBuruk
            if (filter?.TryGetValue("x_CuacaBuruk", out sv) ?? false) {
                CuacaBuruk.AdvancedSearch.SearchValue = sv;
                CuacaBuruk.AdvancedSearch.SearchOperator = filter["z_CuacaBuruk"];
                CuacaBuruk.AdvancedSearch.SearchCondition = filter["v_CuacaBuruk"];
                CuacaBuruk.AdvancedSearch.SearchValue2 = filter["y_CuacaBuruk"];
                CuacaBuruk.AdvancedSearch.SearchOperator2 = filter["w_CuacaBuruk"];
                CuacaBuruk.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Kontaminasi);
            searchFlds.Add(Insident);
            searchFlds.Add(CuacaBuruk);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Kontaminasi, ctrl); // Kontaminasi
                UpdateSort(Insident, ctrl); // Insident
                UpdateSort(CuacaBuruk, ctrl); // CuacaBuruk
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasCatatanKhusus"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasCatatanKhususlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasCatatanKhususlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Kontaminasi")); // DN
                CreateColumnOption(option.Add("Insident")); // DN
                CreateColumnOption(option.Add("CuacaBuruk")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasCatatanKhusussrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasCatatanKhusussrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasCatatanKhususlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasCatatanKhusus");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasCatatanKhususList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasCatatanKhususList.RowCount) + "_SubAktivitasCatatanKhusus");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasCatatanKhususList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasCatatanKhususList.RowType == RowType.Add || IsEdit && subAktivitasCatatanKhususList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            Kontaminasi.SetDbValue(row["Kontaminasi"]);
            Insident.SetDbValue(row["Insident"]);
            CuacaBuruk.SetDbValue(row["CuacaBuruk"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Kontaminasi", Kontaminasi.DefaultValue ?? DbNullValue); // DN
            row.Add("Insident", Insident.DefaultValue ?? DbNullValue); // DN
            row.Add("CuacaBuruk", CuacaBuruk.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // Kontaminasi
                if (!Empty(Kontaminasi.CurrentValue)) {
                    Kontaminasi.ViewValue = Kontaminasi.OptionCaption(ConvertToString(Kontaminasi.CurrentValue));
                } else {
                    Kontaminasi.ViewValue = DbNullValue;
                }
                Kontaminasi.ViewCustomAttributes = "";

                // Insident
                if (!Empty(Insident.CurrentValue)) {
                    Insident.ViewValue = Insident.OptionCaption(ConvertToString(Insident.CurrentValue));
                } else {
                    Insident.ViewValue = DbNullValue;
                }
                Insident.ViewCustomAttributes = "";

                // CuacaBuruk
                if (!Empty(CuacaBuruk.CurrentValue)) {
                    CuacaBuruk.ViewValue = CuacaBuruk.OptionCaption(ConvertToString(CuacaBuruk.CurrentValue));
                } else {
                    CuacaBuruk.ViewValue = DbNullValue;
                }
                CuacaBuruk.ViewCustomAttributes = "";
                ...
                string curVal6 = ConvertToString(StatusAktivitas.CurrentValue);
                if (!Empty(curVal6)) {
                    if (StatusAktivitas.Lookup != null && IsDictionary(StatusAktivitas.Lookup?.Options) && StatusAktivitas.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        StatusAktivitas.ViewValue = StatusAktivitas.LookupCacheOption(curVal6);
                    } else { // Lookup from database // DN
                        string filterWrk6 = SearchFilter(StatusAktivitas.Lookup?.GetTable()?.Fields["IdAktivitas"].SearchExpression, "=", StatusAktivitas.CurrentValue, StatusAktivitas.Lookup?.GetTable()?.Fields["IdAktivitas"].SearchDataType, "");
                        string? sqlWrk6 = StatusAktivitas.Lookup?.GetSql(false, filterWrk6, null, this, true, true);
                        ...
                        if (rswrk6?.Count > 0 && StatusAktivitas.Lookup != null) { // Lookup values found
                            var listwrk = StatusAktivitas.Lookup?.RenderViewRow(rswrk6[0]);
                ...
                // Kontaminasi
                Kontaminasi.HrefValue = "";
                Kontaminasi.TooltipValue = "";

                // Insident
                Insident.HrefValue = "";
                Insident.TooltipValue = "";

                // CuacaBuruk
                CuacaBuruk.HrefValue = "";
                CuacaBuruk.TooltipValue = "";
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fSubAktivitasCatatanKhususlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fSubAktivitasCatatanKhususlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fSubAktivitasCatatanKhususlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasCatatanKhusussrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
