...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasInputLogbookPenerimaanPipaEdit
    /// </summary>
    public static SubAktivitasInputLogbookPenerimaanPipaEdit subAktivitasInputLogbookPenerimaanPipaEdit
    {
        get => HttpData.Get<SubAktivitasInputLogbookPenerimaanPipaEdit>("subAktivitasInputLogbookPenerimaanPipaEdit")!;
        set => HttpData["subAktivitasInputLogbookPenerimaanPipaEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasInputLogbookPenerimaanPipa
    /// </summary>
    public class SubAktivitasInputLogbookPenerimaanPipaEdit : SubAktivitasInputLogbookPenerimaanPipaEditBase
    {
        // Constructor
        public SubAktivitasInputLogbookPenerimaanPipaEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasInputLogbookPenerimaanPipaEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasInputLogbookPenerimaanPipaEditBase : SubAktivitasInputLogbookPenerimaanPipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasInputLogbookPenerimaanPipaEdit";
        ...
        // Constructor
        public SubAktivitasInputLogbookPenerimaanPipaEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasInputLogbookPenerimaanPipaEdit";
        ...
        // Constructor
        public SubAktivitasInputLogbookPenerimaanPipaEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasInputLogbookPenerimaanPipaList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasInputLogbookPenerimaanPipaList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasInputLogbookPenerimaanPipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasInputLogbookPenerimaanPipaEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'NamaBatch'
            NamaBatch.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NamaBatch, "NamaBatch", "x_NamaBatch");

            // Standard handling for 'TanggalJam'
            TanggalJam.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(TanggalJam, "TanggalJam", "x_TanggalJam", true, validate, true);
        ...
        public void RestoreFormValues()
        {
            ...
            NamaBatch.CurrentValue = NamaBatch.FormValue;
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                NamaBatch.EditValue = HtmlEncode(NamaBatch.CurrentValue);
        ...
        private void ValidateCustomNamaBatch() {
            if (NamaBatch.Visible && NamaBatch.Required) {
                if (!NamaBatch.IsDetailKey && Empty(NamaBatch.FormValue)) {
                    NamaBatch.AddErrorMessage(ConvertToString(SubAktivitasHasilPemeriksaanId.RequiredErrorMessage).Replace("%s", SubAktivitasHasilPemeriksaanId.Caption));
                }
            }
        }

        private void ValidateCustomTanggalJam() {
            if (TanggalJam.Visible && TanggalJam.Required) {
                if (!TanggalJam.IsDetailKey && Empty(TanggalJam.FormValue)) {
                    TanggalJam.AddErrorMessage(ConvertToString(SubAktivitasHasilPemeriksaanId.RequiredErrorMessage).Replace("%s", SubAktivitasHasilPemeriksaanId.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNamaKegiatan() {
            if (NamaKegiatan.Visible && NamaKegiatan.Required) {
                if (!NamaKegiatan.IsDetailKey && Empty(NamaKegiatan.FormValue)) {
                    NamaKegiatan.AddErrorMessage(ConvertToString(SubAktivitasHasilPemeriksaanId.RequiredErrorMessage).Replace("%s", SubAktivitasHasilPemeriksaanId.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTemperature() {
            if (Temperature.Visible && Temperature.Required) {
                if (!Temperature.IsDetailKey && Empty(Temperature.FormValue)) {
                    Temperature.AddErrorMessage(ConvertToString(SubAktivitasHasilPemeriksaanId.RequiredErrorMessage).Replace("%s", SubAktivitasHasilPemeriksaanId.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomNamaBatch();
                ValidateCustomTanggalJam();
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // NamaBatch
            NamaBatch.SetDbValue(rsnew, NamaBatch.CurrentValue, NamaBatch.ReadOnly);
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("NamaBatch", out value)) // NamaBatch
                NamaBatch.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasInputLogbookPenerimaanPipaList")), "", TableVar, true);
            string pageId = "edit";
