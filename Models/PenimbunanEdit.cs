...
public partial class SnDOne {
    ...
    /// penimbunanEdit
    /// </summary>
    public static PenimbunanEdit penimbunanEdit
    {
        get => HttpData.Get<PenimbunanEdit>("penimbunanEdit")!;
        set => HttpData["penimbunanEdit"] = value;
    }

    /// <summary>
    /// Page class for Penimbunan
    /// </summary>
    public class PenimbunanEdit : PenimbunanEditBase
    {
        // Constructor
        public PenimbunanEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public PenimbunanEdit() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class PenimbunanEditBase : Penimbunan
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "penimbunanEdit";
        ...
        // Page name
        public string PageName => "PenimbunanEdit";
        ...
        // Constructor
        public PenimbunanEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            SetVisibility();
            NomorPenimbunan.Required = false;
            Plant.Required = false;
            IdPenerimaan.Required = false;
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdPenimbunan", out rv)) { // DN
                    IdPenimbunan.FormValue = UrlDecode(rv); // DN
                    IdPenimbunan.OldValue = IdPenimbunan.FormValue;
                } else if (CurrentForm.HasValue("x_IdPenimbunan")) {
                    IdPenimbunan.FormValue = CurrentForm.GetValue("x_IdPenimbunan");
                    IdPenimbunan.OldValue = IdPenimbunan.FormValue;
                } else if (!Empty(keyValues)) {
                    IdPenimbunan.OldValue = ConvertToString(keyValues[0]);
                } else {
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdPenimbunan", out rv)) { // DN
                        ...
                        IdPenimbunan.CurrentValue = DbNullValue;
                    }
                }
        ...
        if (postBack) {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                IdPenimbunan.FormValue = ConvertToString(keyValues[0]);
            }
        }
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("PenimbunanList"); // No matching record, return to list
                        }
                    break;
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'NomorPenimbunan' before field var 'x_NomorPenimbunan'
            val = CurrentForm.HasValue("NomorPenimbunan") ? CurrentForm.GetValue("NomorPenimbunan") : CurrentForm.GetValue("x_NomorPenimbunan");
            if (!NomorPenimbunan.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NomorPenimbunan") && !CurrentForm.HasValue("x_NomorPenimbunan")) // DN
                    NomorPenimbunan.Visible = false; // Disable update for API request
                else
                    NomorPenimbunan.SetFormValue(val);
            }
            ...
            if (!IdPenimbunan.IsDetailKey)
                IdPenimbunan.SetFormValue(val);
        }
        #pragma warning restore 1998
        ...
        {
            IdPenimbunan.CurrentValue = IdPenimbunan.FormValue;
            NomorPenimbunan.CurrentValue = NomorPenimbunan.FormValue;
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                if (NomorPenimbunan.Visible && NomorPenimbunan.Required) {
                    if (!NomorPenimbunan.IsDetailKey && Empty(NomorPenimbunan.FormValue)) {
                        NomorPenimbunan.AddErrorMessage(ConvertToString(NomorPenimbunan.RequiredErrorMessage).Replace("%s", NomorPenimbunan.Caption));
                    }
                }
