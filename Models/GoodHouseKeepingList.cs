...
public partial class SnDOne {
    /// <summary>
    /// goodHouseKeepingList
    /// </summary>
    public static GoodHouseKeepingList goodHouseKeepingList
    {
        get => HttpData.Get<GoodHouseKeepingList>("goodHouseKeepingList")!;
        set => HttpData["goodHouseKeepingList"] = value;
    }

    /// <summary>
    /// Page class for GoodHouseKeeping
    /// </summary>
    public class GoodHouseKeepingList : GoodHouseKeepingListBase
    {
        // Constructor
        public GoodHouseKeepingList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public GoodHouseKeepingList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class GoodHouseKeepingListBase : GoodHouseKeeping
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "goodHouseKeepingList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fGoodHouseKeepinglist";
        ...
        // Constructor
        public GoodHouseKeepingListBase()
        {
            TableName = "GoodHouseKeeping";
            ...
            // Table object (goodHouseKeeping)
            if (goodHouseKeeping == null || goodHouseKeeping is GoodHouseKeeping)
                goodHouseKeeping = this;

            // Initialize URLs
            AddUrl = "GoodHouseKeepingAdd";
            ...
            MultiDeleteUrl = "GoodHouseKeepingDelete";
            MultiUpdateUrl = "GoodHouseKeepingUpdate";
        ...
        // Page name
        public string PageName => "GoodHouseKeepingList";
        ...
        {
            IdGoodHouseKeeping.Visible = false;
            NomorGoodHouseKeeping.Visible = false;
        ...
        // Constructor
        public GoodHouseKeepingListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "GoodHouseKeepingView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("IdGoodHouseKeeping") ? dict["IdGoodHouseKeeping"] : IdGoodHouseKeeping.CurrentValue));
            return key;
        }
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fGoodHouseKeepinggrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                goodHouseKeepingList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    IdGoodHouseKeeping.Sort = "";
                    NomorGoodHouseKeeping.Sort = "";
                    LinkRedirect.Sort = "";
                    LookupPlant.Sort = "";
                    Plant.Sort = "";
                    Catatan.Sort = "";
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""GoodHouseKeeping"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""GoodHouseKeeping"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""GoodHouseKeeping"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fGoodHouseKeepinglist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fGoodHouseKeepinglist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdGoodHouseKeeping.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""GoodHouseKeeping"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fGoodHouseKeepinglist""" +
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fGoodHouseKeepingsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fGoodHouseKeepingsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fGoodHouseKeepinglist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_GoodHouseKeeping");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(goodHouseKeepingList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(goodHouseKeepingList.RowCount) + "_GoodHouseKeeping");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(goodHouseKeepingList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && goodHouseKeepingList.RowType == RowType.Add || IsEdit && goodHouseKeepingList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected Dictionary<string, object> NewRow() {
            var row = new Dictionary<string, object>();
            row.Add("IdGoodHouseKeeping", IdGoodHouseKeeping.DefaultValue ?? DbNullValue); // DN
            row.Add("NomorGoodHouseKeeping", NomorGoodHouseKeeping.DefaultValue ?? DbNullValue); // DN
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fGoodHouseKeepingsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
