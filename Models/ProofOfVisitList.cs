...
public partial class SnDOne {
    ...
    public class ProofOfVisitListBase : ProofOfVisit
    {
        ...
        // Constructor
        public ProofOfVisitListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public void ShowFilterList()
        {
            ...
            // Field TotalPengisian
            filter = QueryBuilderWhere("TotalPengisian");
            if (Empty(filter))
                BuildSearchSql(ref filter, TotalPengisian, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TotalPengisian.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Catatan
            filter = QueryBuilderWhere("Catatan");
            if (Empty(filter))
                BuildSearchSql(ref filter, Catatan, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Catatan.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, TanggalDibuat, false, false);
            if (!Empty(filter))
        ...
        protected void LoadSearchValues() {
            ...
            // TotalPengisian
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TotalPengisian[]"))
