...
public partial class SnDOne {
    ...
    public class VFaceEnrollmentListBase : VFaceEnrollment
    {
        ...
        // Constructor
        public VFaceEnrollmentListBase(Controller? controller)
        {
        ...
        // Constructor
        public VFaceEnrollmentListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field IdUser
            if (filter.TryGetValue("x_IdUser", out sv)) {
                RestoreFieldFilter(IdUser, filter, "IdUser", sv);
            }

            // Field Username
            if (filter.TryGetValue("x__Username", out sv)) {
                RestoreFieldFilter(_Username, filter, "_Username", sv);
            }

            // Field Email
            if (filter.TryGetValue("x__Email", out sv)) {
                RestoreFieldFilter(_Email, filter, "_Email", sv);
            }

            // Field NamaLengkap
            if (filter.TryGetValue("x_NamaLengkap", out sv)) {
                RestoreFieldFilter(NamaLengkap, filter, "NamaLengkap", sv);
            }
            ...
            // Field Jabatan
            if (filter.TryGetValue("x_Jabatan", out sv)) {
                RestoreFieldFilter(Jabatan, filter, "Jabatan", sv);
            }

            // Field Face
            if (filter.TryGetValue("x_Face", out sv)) {
                RestoreFieldFilter(Face, filter, "Face", sv);
            }

            // Field TanggalInputFace
            if (filter.TryGetValue("x_TanggalInputFace", out sv)) {
                RestoreFieldFilter(TanggalInputFace, filter, "TanggalInputFace", sv);
            }

            // Field UserInputFace
            if (filter.TryGetValue("x_UserInputFace", out sv)) {
                RestoreFieldFilter(UserInputFace, filter, "UserInputFace", sv);
            }

            // Field AzurePersonId
            if (filter.TryGetValue("x_AzurePersonId", out sv)) {
                RestoreFieldFilter(AzurePersonId, filter, "AzurePersonId", sv);
            }
        }
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("Face", Face, true, config);
            filterList += ProcessFieldFilter("TanggalInputFace", TanggalInputFace, true, config);
            filterList += ProcessFieldFilter("UserInputFace", UserInputFace, true, config);
            filterList += ProcessFieldFilter("AzurePersonId", AzurePersonId, false, config);
            return filterList;
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_IdUser")) {
        ...
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Face[]")) {
        ...
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TanggalInputFace[]")) {
        ...
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UserInputFace[]")) {
        ...
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_AzurePersonId")) {
                AzurePersonId.AdvancedSearch.SearchValue = Get("x_AzurePersonId");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorAzurePersonId()
        {
            if (Query.ContainsKey("z_AzurePersonId"))
                AzurePersonId.AdvancedSearch.SearchOperator = Get("z_AzurePersonId");
        }
