...
public partial class SnDOne {
    ...
    /// setPjsList
    /// </summary>
    public static SetPjsList setPjsList
    {
        get => HttpData.Get<SetPjsList>("setPjsList")!;
        set => HttpData["setPjsList"] = value;
    }

    /// <summary>
    /// Page class for SetPjs
    /// </summary>
    public class SetPjsList : SetPjsListBase
    {
        // Constructor
        public SetPjsList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SetPjsList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class SetPjsListBase : SetPjs
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "setPjsList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSetPjslist";
        ...
        // Constructor
        public SetPjsListBase(Controller? controller)
        {
            TableName = "SetPjs";
        ...
        if (controller != null)
            ...
            MultiDeleteUrl = "SetPjsDelete";
            MultiUpdateUrl = "SetPjsUpdate";
        ...
        // Page name
        public string PageName => "SetPjsList";
        ...
        // Constructor
        public SetPjsListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(DownloadSuratTugas.AdvancedSearch.ToJson())); // Field DownloadSuratTugas
            filters.Merge(JObject.Parse(Remaks.AdvancedSearch.ToJson())); // Field Remaks
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field DownloadSuratTugas
            if (filter.TryGetValue("x_DownloadSuratTugas", out sv)) {
                RestoreFieldFilter(DownloadSuratTugas, filter, "DownloadSuratTugas", sv);
            }

            // Field Remaks
            if (filter.TryGetValue("x_Remaks", out sv)) {
                RestoreFieldFilter(Remaks, filter, "Remaks", sv);
            }
        }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, RedirectLink, def, true); // RedirectLink
            BuildSearchSql(ref where, Nama, def, true); // Nama
            BuildSearchSql(ref where, OrganizationLevel, def, true); // OrganizationLevel
            BuildSearchSql(ref where, PosisiAwal, def, true); // PosisiAwal
            BuildSearchSql(ref where, PosisiPJS, def, true); // PosisiPJS
            ...
            BuildSearchSql(ref where, DownloadSuratTugas, def, false); // DownloadSuratTugas
            BuildSearchSql(ref where, Remaks, def, true); // Remaks
            BuildSearchSql(ref where, DibuatOleh, def, true); // DibuatOleh
            BuildSearchSql(ref where, TanggalDibuat, def, true); // TanggalDibuat
            BuildSearchSql(ref where, DiperbaharuiOleh, def, true); // DiperbaharuiOleh
            BuildSearchSql(ref where, DiperbaharuiTanggal, def, true); // DiperbaharuiTanggal

            // Set up search command
            if (!def && !Empty(where) && (new[] { "", "reset", "resetall" }).Contains(Command))
                Command = "search";
            if (!def && Command == "search") {
                DownloadSuratTugas.AdvancedSearch.Save(); // DownloadSuratTugas
                Remaks.AdvancedSearch.Save(); // Remaks
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("RedirectLink", RedirectLink, true, config);
            filterList += ProcessFieldFilter("Nama", Nama, true, config);
            filterList += ProcessFieldFilter("OrganizationLevel", OrganizationLevel, true, config);
            filterList += ProcessFieldFilter("PosisiAwal", PosisiAwal, true, config);
            filterList += ProcessFieldFilter("PosisiPJS", PosisiPJS, true, config);
            ...
            filterList += ProcessFieldFilter("DownloadSuratTugas", DownloadSuratTugas, false, config);
            filterList += ProcessFieldFilter("Remaks", Remaks, true, config);
            ...
            filterList += ProcessFieldFilter("DiperbaharuiOleh", DiperbaharuiOleh, true, config);
            filterList += ProcessFieldFilter("DiperbaharuiTanggal", DiperbaharuiTanggal, true, config);
            return filterList;
        }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(RedirectLink);
            searchFlds.Add(Nama);
            searchFlds.Add(OrganizationLevel);
            searchFlds.Add(PosisiAwal);
            searchFlds.Add(PosisiPJS);
            searchFlds.Add(TanggalMulai);
            searchFlds.Add(TanggalSelesai);
            searchFlds.Add(Status);
            searchFlds.Add(Remaks);
            searchFlds.Add(DibuatOleh);
            searchFlds.Add(TanggalDibuat);
            searchFlds.Add(DiperbaharuiOleh);
            searchFlds.Add(DiperbaharuiTanggal);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (RedirectLink.AdvancedSearch.IssetSession)
                return true;
            if (Nama.AdvancedSearch.IssetSession)
                return true;
            if (OrganizationLevel.AdvancedSearch.IssetSession)
                return true;
            if (PosisiAwal.AdvancedSearch.IssetSession)
                return true;
            if (PosisiPJS.AdvancedSearch.IssetSession)
                return true;
            ...
            if (Status.AdvancedSearch.IssetSession)
                return true;
            if (DownloadSuratTugas.AdvancedSearch.IssetSession)
                return true;
            if (Remaks.AdvancedSearch.IssetSession)
                return true;
            if (DibuatOleh.AdvancedSearch.IssetSession)
                return true;
            if (TanggalDibuat.AdvancedSearch.IssetSession)
                return true;
            if (DiperbaharuiOleh.AdvancedSearch.IssetSession)
                return true;
            if (DiperbaharuiTanggal.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            RedirectLink.AdvancedSearch.UnsetSession();
            Nama.AdvancedSearch.UnsetSession();
            OrganizationLevel.AdvancedSearch.UnsetSession();
            PosisiAwal.AdvancedSearch.UnsetSession();
            PosisiPJS.AdvancedSearch.UnsetSession();
            TanggalMulai.AdvancedSearch.UnsetSession();
            TanggalSelesai.AdvancedSearch.UnsetSession();
            Status.AdvancedSearch.UnsetSession();
            DownloadSuratTugas.AdvancedSearch.UnsetSession();
            Remaks.AdvancedSearch.UnsetSession();
            DibuatOleh.AdvancedSearch.UnsetSession();
            TanggalDibuat.AdvancedSearch.UnsetSession();
            DiperbaharuiOleh.AdvancedSearch.UnsetSession();
            DiperbaharuiTanggal.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            RedirectLink.AdvancedSearch.Load();
            Nama.AdvancedSearch.Load();
            OrganizationLevel.AdvancedSearch.Load();
            PosisiAwal.AdvancedSearch.Load();
            PosisiPJS.AdvancedSearch.Load();
            TanggalMulai.AdvancedSearch.Load();
            TanggalSelesai.AdvancedSearch.Load();
            Status.AdvancedSearch.Load();
            DownloadSuratTugas.AdvancedSearch.Load();
            Remaks.AdvancedSearch.Load();
            DibuatOleh.AdvancedSearch.Load();
            TanggalDibuat.AdvancedSearch.Load();
            DiperbaharuiOleh.AdvancedSearch.Load();
            DiperbaharuiTanggal.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(RedirectLink, ctrl); // RedirectLink
                UpdateSort(Nama, ctrl); // Nama
                UpdateSort(OrganizationLevel, ctrl); // OrganizationLevel
                UpdateSort(PosisiAwal, ctrl); // PosisiAwal
                UpdateSort(PosisiPJS, ctrl); // PosisiPJS
                UpdateSort(TanggalMulai, ctrl); // TanggalMulai
                UpdateSort(TanggalSelesai, ctrl); // TanggalSelesai
                UpdateSort(Status, ctrl); // Status
                UpdateSort(DownloadSuratTugas, ctrl); // DownloadSuratTugas
                UpdateSort(Remaks, ctrl); // Remaks
                UpdateSort(DibuatOleh, ctrl); // DibuatOleh
                UpdateSort(TanggalDibuat, ctrl); // TanggalDibuat
                UpdateSort(DiperbaharuiOleh, ctrl); // DiperbaharuiOleh
                UpdateSort(DiperbaharuiTanggal, ctrl); // DiperbaharuiTanggal
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    TanggalSelesai.Sort = "";
                    Status.Sort = "";
                    DownloadSuratTugas.Sort = "";
                    SuratTugas.Sort = "";
                    Keterangan.Sort = "";
                    Remaks.Sort = "";
                    DibuatOleh.Sort = "";
                    TanggalDibuat.Sort = "";
                    DiperbaharuiOleh.Sort = "";
                    DiperbaharuiTanggal.Sort = "";
                    PlantAwal.Sort = "";
                    RegionAwal.Sort = "";
                }
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""SetPjs"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SetPjs"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""SetPjs"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSetPjslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSetPjslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""SetPjs"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fSetPjslist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("RedirectLink")); // DN
                CreateColumnOption(option.Add("Nama")); // DN
                CreateColumnOption(option.Add("OrganizationLevel")); // DN
                CreateColumnOption(option.Add("PosisiAwal")); // DN
                CreateColumnOption(option.Add("PosisiPJS")); // DN
                ...
                CreateColumnOption(option.Add("DownloadSuratTugas")); // DN
                CreateColumnOption(option.Add("Remaks")); // DN
                CreateColumnOption(option.Add("DibuatOleh")); // DN
                CreateColumnOption(option.Add("TanggalDibuat")); // DN
                CreateColumnOption(option.Add("DiperbaharuiOleh")); // DN
                CreateColumnOption(option.Add("DiperbaharuiTanggal")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSetPjssrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSetPjssrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSetPjslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(setPjsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(setPjsList.RowCount) + "_SetPjs");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(setPjsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && setPjsList.RowType == RowType.Add || IsEdit && setPjsList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadFieldRedirectLinkSearchValues();
            LoadFieldNamaSearchValues();
            LoadFieldOrganizationLevelSearchValues();
            LoadFieldPosisiAwalSearchValues();
            LoadFieldPosisiPJSSearchValues();
            LoadFieldTanggalMulaiSearchValues();
            LoadFieldTanggalSelesaiSearchValues();
            LoadFieldStatusSearchValues();
            LoadFieldDownloadSuratTugasSearchValues();
            LoadFieldRemaksSearchValues();
            LoadFieldDibuatOlehSearchValues();
            LoadFieldTanggalDibuatSearchValues();
            LoadFieldDiperbaharuiOlehSearchValues();
            LoadFieldDiperbaharuiTanggalSearchValues();
        }

        private void LoadFieldRedirectLinkSearchValues()
        {
            // RedirectLink
            LoadPrimarySearchValueRedirectLink();
            LoadSearchOperatorRedirectLink();
        }

        private void LoadPrimarySearchValueRedirectLink()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_RedirectLink[]")) {
                RedirectLink.AdvancedSearch.SearchValue = Get("x_RedirectLink[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                RedirectLink.AdvancedSearch.SearchValue = Get("RedirectLink"); // Default Value // DN
            }
            if (!Empty(RedirectLink.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorRedirectLink()
        {
            if (Query.ContainsKey("z_RedirectLink"))
                RedirectLink.AdvancedSearch.SearchOperator = Get("z_RedirectLink", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldOrganizationLevelSearchValues()
        {
            // OrganizationLevel
            LoadPrimarySearchValueOrganizationLevel();
            LoadSearchOperatorOrganizationLevel();
        }

        private void LoadPrimarySearchValueOrganizationLevel()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_OrganizationLevel[]")) {
                OrganizationLevel.AdvancedSearch.SearchValue = Get("x_OrganizationLevel[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                OrganizationLevel.AdvancedSearch.SearchValue = Get("OrganizationLevel"); // Default Value // DN
            }
            if (!Empty(OrganizationLevel.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorOrganizationLevel()
        {
            if (Query.ContainsKey("z_OrganizationLevel"))
                OrganizationLevel.AdvancedSearch.SearchOperator = Get("z_OrganizationLevel", Config.FilterOptionSeparator);
        }

        private void LoadFieldPosisiAwalSearchValues()
        {
            // PosisiAwal
            LoadPrimarySearchValuePosisiAwal();
            LoadSearchOperatorPosisiAwal();
        }

        private void LoadPrimarySearchValuePosisiAwal()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_PosisiAwal[]")) {
                PosisiAwal.AdvancedSearch.SearchValue = Get("x_PosisiAwal[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                PosisiAwal.AdvancedSearch.SearchValue = Get("PosisiAwal"); // Default Value // DN
            }
            if (!Empty(PosisiAwal.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorPosisiAwal()
        {
            if (Query.ContainsKey("z_PosisiAwal"))
                PosisiAwal.AdvancedSearch.SearchOperator = Get("z_PosisiAwal", Config.FilterOptionSeparator);
        }

        private void LoadFieldPosisiPJSSearchValues()
        {
            // PosisiPJS
            LoadPrimarySearchValuePosisiPJS();
            LoadSearchOperatorPosisiPJS();
        }

        private void LoadPrimarySearchValuePosisiPJS()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_PosisiPJS[]")) {
                PosisiPJS.AdvancedSearch.SearchValue = Get("x_PosisiPJS[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                PosisiPJS.AdvancedSearch.SearchValue = Get("PosisiPJS"); // Default Value // DN
            }
            if (!Empty(PosisiPJS.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorPosisiPJS()
        {
            if (Query.ContainsKey("z_PosisiPJS"))
                PosisiPJS.AdvancedSearch.SearchOperator = Get("z_PosisiPJS", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldDownloadSuratTugasSearchValues()
        {
            // DownloadSuratTugas
            LoadPrimarySearchValueDownloadSuratTugas();
            LoadSearchOperatorDownloadSuratTugas();
        }

        private void LoadPrimarySearchValueDownloadSuratTugas()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_DownloadSuratTugas")) {
                DownloadSuratTugas.AdvancedSearch.SearchValue = Get("x_DownloadSuratTugas");
            }
            else if (!IsAddOrEdit) {
                DownloadSuratTugas.AdvancedSearch.SearchValue = Get("DownloadSuratTugas"); // Default Value // DN
            }
            if (!Empty(DownloadSuratTugas.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorDownloadSuratTugas()
        {
            if (Query.ContainsKey("z_DownloadSuratTugas"))
                DownloadSuratTugas.AdvancedSearch.SearchOperator = Get("z_DownloadSuratTugas");
        }

        private void LoadFieldRemaksSearchValues()
        {
            // Remaks
            LoadPrimarySearchValueRemaks();
            LoadSearchOperatorRemaks();
        }

        private void LoadPrimarySearchValueRemaks()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Remaks[]")) {
                Remaks.AdvancedSearch.SearchValue = Get("x_Remaks[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Remaks.AdvancedSearch.SearchValue = Get("Remaks"); // Default Value // DN
            }
            if (!Empty(Remaks.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorRemaks()
        {
            if (Query.ContainsKey("z_Remaks"))
                Remaks.AdvancedSearch.SearchOperator = Get("z_Remaks", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldDiperbaharuiOlehSearchValues()
        {
            // DiperbaharuiOleh
            LoadPrimarySearchValueDiperbaharuiOleh();
            LoadSearchOperatorDiperbaharuiOleh();
        }

        private void LoadPrimarySearchValueDiperbaharuiOleh()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_DiperbaharuiOleh[]")) {
                DiperbaharuiOleh.AdvancedSearch.SearchValue = Get("x_DiperbaharuiOleh[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                DiperbaharuiOleh.AdvancedSearch.SearchValue = Get("DiperbaharuiOleh"); // Default Value // DN
            }
            if (!Empty(DiperbaharuiOleh.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorDiperbaharuiOleh()
        {
            if (Query.ContainsKey("z_DiperbaharuiOleh"))
                DiperbaharuiOleh.AdvancedSearch.SearchOperator = Get("z_DiperbaharuiOleh", Config.FilterOptionSeparator);
        }

        private void LoadFieldDiperbaharuiTanggalSearchValues()
        {
            // DiperbaharuiTanggal
            LoadPrimarySearchValueDiperbaharuiTanggal();
            LoadSearchOperatorDiperbaharuiTanggal();
        }

        private void LoadPrimarySearchValueDiperbaharuiTanggal()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_DiperbaharuiTanggal[]")) {
                DiperbaharuiTanggal.AdvancedSearch.SearchValue = Get("x_DiperbaharuiTanggal[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                DiperbaharuiTanggal.AdvancedSearch.SearchValue = Get("DiperbaharuiTanggal"); // Default Value // DN
            }
            if (!Empty(DiperbaharuiTanggal.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorDiperbaharuiTanggal()
        {
            if (Query.ContainsKey("z_DiperbaharuiTanggal"))
                DiperbaharuiTanggal.AdvancedSearch.SearchOperator = Get("z_DiperbaharuiTanggal", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // RedirectLink
                if (RedirectLink.UseFilter && !Empty(RedirectLink.AdvancedSearch.SearchValue)) {
                    RedirectLink.EditValue = ConvertToString(RedirectLink.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // OrganizationLevel
                if (OrganizationLevel.UseFilter && !Empty(OrganizationLevel.AdvancedSearch.SearchValue)) {
                    OrganizationLevel.EditValue = ConvertToString(OrganizationLevel.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // PosisiAwal
                if (PosisiAwal.UseFilter && !Empty(PosisiAwal.AdvancedSearch.SearchValue)) {
                    PosisiAwal.EditValue = ConvertToString(PosisiAwal.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // PosisiPJS
                if (PosisiPJS.UseFilter && !Empty(PosisiPJS.AdvancedSearch.SearchValue)) {
                    PosisiPJS.EditValue = ConvertToString(PosisiPJS.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                if (!DownloadSuratTugas.Raw)
                    DownloadSuratTugas.AdvancedSearch.SearchValue = HtmlDecode(DownloadSuratTugas.AdvancedSearch.SearchValue);
                DownloadSuratTugas.EditValue = HtmlEncode(DownloadSuratTugas.AdvancedSearch.SearchValue);
                DownloadSuratTugas.PlaceHolder = RemoveHtml(DownloadSuratTugas.Caption);

                // Remaks
                if (Remaks.UseFilter && !Empty(Remaks.AdvancedSearch.SearchValue)) {
                    Remaks.EditValue = ConvertToString(Remaks.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // DiperbaharuiOleh
                if (DiperbaharuiOleh.UseFilter && !Empty(DiperbaharuiOleh.AdvancedSearch.SearchValue)) {
                    DiperbaharuiOleh.EditValue = ConvertToString(DiperbaharuiOleh.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // DiperbaharuiTanggal
                if (DiperbaharuiTanggal.UseFilter && !Empty(DiperbaharuiTanggal.AdvancedSearch.SearchValue)) {
                    DiperbaharuiTanggal.EditValue = ConvertToString(DiperbaharuiTanggal.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSetPjssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
