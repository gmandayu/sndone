...
public partial class SnDOne {
    ...
    public class UserLevelsListBase : UserLevels
    {
        ...
        // Constructor
        public UserLevelsListBase(Controller? controller)
        {
        ...
        // Constructor
        public UserLevelsListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, UserLevelName, def, true); // UserLevelName
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("UserLevelID", UserLevelID, true, config);
            filterList += ProcessFieldFilter("UserLevelName", UserLevelName, true, config);
            return filterList;
        }
        ...
        protected bool CheckSearchParms() {
            ...
            if (UserLevelID.AdvancedSearch.IssetSession)
                return true;
            if (UserLevelName.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            UserLevelName.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            UserLevelName.AdvancedSearch.Load();
        }
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""UserLevels"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""UserLevels"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fUserLevelslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            if (Security.HasUserLevelID(ConvertToInt(UserLevelID.CurrentValue)) || ConvertToInt(UserLevelID.CurrentValue) < AdvancedSecurity.DefaultUserLevelId && ConvertToInt(UserLevelID.CurrentValue) != AdvancedSecurity.AnonymousUserLevelId) {
                listOption?.SetBody("-");
            } else {
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(UserLevelID.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fUserLevelslist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("UserLevelName")); // DN
            }
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadFieldUserLevelIDSearchValues();
            LoadFieldUserLevelNameSearchValues();
        }

        private void LoadFieldUserLevelIDSearchValues()
        {
            // UserLevelID
            LoadPrimarySearchValueUserLevelID();
            LoadSearchOperatorUserLevelID();
        }

        private void LoadPrimarySearchValueUserLevelID()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UserLevelID[]")) {
                UserLevelID.AdvancedSearch.SearchValue = Get("x_UserLevelID[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorUserLevelID()
        {
            if (Query.ContainsKey("z_UserLevelID"))
                UserLevelID.AdvancedSearch.SearchOperator = Get("z_UserLevelID", Config.FilterOptionSeparator);
        }

        private void LoadFieldUserLevelNameSearchValues()
        {
            // UserLevelName
            LoadPrimarySearchValueUserLevelName();
            LoadSearchOperatorUserLevelName();
        }

        private void LoadPrimarySearchValueUserLevelName()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UserLevelName[]")) {
                UserLevelName.AdvancedSearch.SearchValue = Get("x_UserLevelName[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                UserLevelName.AdvancedSearch.SearchValue = Get("UserLevelName"); // Default Value // DN
            }
            if (!Empty(UserLevelName.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorUserLevelName()
        {
            if (Query.ContainsKey("z_UserLevelName"))
                UserLevelName.AdvancedSearch.SearchOperator = Get("z_UserLevelName", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (UserLevelID.UseFilter && !Empty(UserLevelID.AdvancedSearch.SearchValue)) {
                    UserLevelID.EditValue = ConvertToString(UserLevelID.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // UserLevelName
                if (UserLevelName.UseFilter && !Empty(UserLevelName.AdvancedSearch.SearchValue)) {
                    UserLevelName.EditValue = ConvertToString(UserLevelName.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
