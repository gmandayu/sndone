...
public partial class SnDOne {
    ...
    /// mwtOnlineDetailList
    /// </summary>
    public static MwtOnlineDetailList mwtOnlineDetailList
    {
        get => HttpData.Get<MwtOnlineDetailList>("mwtOnlineDetailList")!;
        set => HttpData["mwtOnlineDetailList"] = value;
    }

    /// <summary>
    /// Page class for MWTOnlineDetail
    /// </summary>
    public class MwtOnlineDetailList : MwtOnlineDetailListBase
    {
        // Constructor
        public MwtOnlineDetailList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MwtOnlineDetailList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MwtOnlineDetailListBase : MwtOnlineDetail
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mwtOnlineDetailList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMWTOnlineDetaillist";
        ...
        // Constructor
        public MwtOnlineDetailListBase()
        {
            TableName = "MWTOnlineDetail";

            // CSS class name as context
            TableVar = "MWTOnlineDetail";
            ...
            MultiDeleteUrl = "MwtOnlineDetailDelete";
            MultiUpdateUrl = "MwtOnlineDetailUpdate";
        ...
        // Page name
        public string PageName => "MwtOnlineDetailList";
        ...
        // Constructor
        public MwtOnlineDetailListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mwtOnlineDetailList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Penjelasan.AdvancedSearch.ToJson())); // Field Penjelasan
            filters.Merge(JObject.Parse(UploadEvidence.AdvancedSearch.ToJson())); // Field UploadEvidence
            ...
            filters.Merge(JObject.Parse(FileType.AdvancedSearch.ToJson())); // Field FileType
            filters.Merge(JObject.Parse(OriginalFileName.AdvancedSearch.ToJson())); // Field OriginalFileName
            filters.Merge(JObject.Parse(Flag.AdvancedSearch.ToJson())); // Field Flag
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Penjelasan
            if (filter?.TryGetValue("x_Penjelasan", out sv) ?? false) {
                Penjelasan.AdvancedSearch.SearchValue = sv;
                Penjelasan.AdvancedSearch.SearchOperator = filter["z_Penjelasan"];
                Penjelasan.AdvancedSearch.SearchCondition = filter["v_Penjelasan"];
                Penjelasan.AdvancedSearch.SearchValue2 = filter["y_Penjelasan"];
                Penjelasan.AdvancedSearch.SearchOperator2 = filter["w_Penjelasan"];
                Penjelasan.AdvancedSearch.Save();
            }

            // Field UploadEvidence
            if (filter?.TryGetValue("x_UploadEvidence", out sv) ?? false) {
                UploadEvidence.AdvancedSearch.SearchValue = sv;
                UploadEvidence.AdvancedSearch.SearchOperator = filter["z_UploadEvidence"];
                UploadEvidence.AdvancedSearch.SearchCondition = filter["v_UploadEvidence"];
                UploadEvidence.AdvancedSearch.SearchValue2 = filter["y_UploadEvidence"];
                UploadEvidence.AdvancedSearch.SearchOperator2 = filter["w_UploadEvidence"];
                UploadEvidence.AdvancedSearch.Save();
            }
            ...
            // Field FileType
            if (filter?.TryGetValue("x_FileType", out sv) ?? false) {
                FileType.AdvancedSearch.SearchValue = sv;
                FileType.AdvancedSearch.SearchOperator = filter["z_FileType"];
                FileType.AdvancedSearch.SearchCondition = filter["v_FileType"];
                FileType.AdvancedSearch.SearchValue2 = filter["y_FileType"];
                FileType.AdvancedSearch.SearchOperator2 = filter["w_FileType"];
                FileType.AdvancedSearch.Save();
            }

            // Field OriginalFileName
            if (filter?.TryGetValue("x_OriginalFileName", out sv) ?? false) {
                OriginalFileName.AdvancedSearch.SearchValue = sv;
                OriginalFileName.AdvancedSearch.SearchOperator = filter["z_OriginalFileName"];
                OriginalFileName.AdvancedSearch.SearchCondition = filter["v_OriginalFileName"];
                OriginalFileName.AdvancedSearch.SearchValue2 = filter["y_OriginalFileName"];
                OriginalFileName.AdvancedSearch.SearchOperator2 = filter["w_OriginalFileName"];
                OriginalFileName.AdvancedSearch.Save();
            }

            // Field Flag
            if (filter?.TryGetValue("x_Flag", out sv) ?? false) {
                Flag.AdvancedSearch.SearchValue = sv;
                Flag.AdvancedSearch.SearchOperator = filter["z_Flag"];
                Flag.AdvancedSearch.SearchCondition = filter["v_Flag"];
                Flag.AdvancedSearch.SearchValue2 = filter["y_Flag"];
                Flag.AdvancedSearch.SearchOperator2 = filter["w_Flag"];
                Flag.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Penjelasan);
            searchFlds.Add(UploadEvidence);
            searchFlds.Add(CreatedBy);
            searchFlds.Add(LastUpdatedBy);
            searchFlds.Add(StatusEmail);
            searchFlds.Add(FileType);
            searchFlds.Add(OriginalFileName);
            searchFlds.Add(Flag);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(UploadEvidence, ctrl); // UploadEvidence
                ...
                UpdateSort(StatusEmail, ctrl); // StatusEmail
                UpdateSort(FileType, ctrl); // FileType
                UpdateSort(OriginalFileName, ctrl); // OriginalFileName
                UpdateSort(Flag, ctrl); // Flag
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MWTOnlineDetail"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMWTOnlineDetaillist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMWTOnlineDetaillist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("UploadEvidence")); // DN
                ...
                CreateColumnOption(option.Add("StatusEmail")); // DN
                CreateColumnOption(option.Add("FileType")); // DN
                CreateColumnOption(option.Add("OriginalFileName")); // DN
                CreateColumnOption(option.Add("Flag")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMWTOnlineDetailsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMWTOnlineDetailsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMWTOnlineDetaillist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MWTOnlineDetail");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mwtOnlineDetailList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mwtOnlineDetailList.RowCount) + "_MWTOnlineDetail");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mwtOnlineDetailList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mwtOnlineDetailList.RowType == RowType.Add || IsEdit && mwtOnlineDetailList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMWTOnlineDetailsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
