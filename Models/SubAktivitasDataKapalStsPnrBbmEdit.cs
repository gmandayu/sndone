...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasDataKapalStsPnrBbmEdit
    /// </summary>
    public static SubAktivitasDataKapalStsPnrBbmEdit subAktivitasDataKapalStsPnrBbmEdit
    {
        get => HttpData.Get<SubAktivitasDataKapalStsPnrBbmEdit>("subAktivitasDataKapalStsPnrBbmEdit")!;
        set => HttpData["subAktivitasDataKapalStsPnrBbmEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasDataKapalSTSPnrBBM
    /// </summary>
    public class SubAktivitasDataKapalStsPnrBbmEdit : SubAktivitasDataKapalStsPnrBbmEditBase
    {
        // Constructor
        public SubAktivitasDataKapalStsPnrBbmEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasDataKapalStsPnrBbmEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasDataKapalStsPnrBbmEditBase : SubAktivitasDataKapalStsPnrBbm
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasDataKapalStsPnrBbmEdit";
        ...
        // Constructor
        public SubAktivitasDataKapalStsPnrBbmEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasDataKapalStsPnrBbmEdit";
        ...
        // Constructor
        public SubAktivitasDataKapalStsPnrBbmEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasDataKapalStsPnrBbmList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasDataKapalStsPnrBbmList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasDataKapalStsPnrBbmList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasDataKapalStsPnrBbmEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'tgltiba'
            tgltiba.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(tgltiba, "tgltiba", "x_tgltiba", true, validate, true);
        ...
        public void RestoreFormValues()
        {
            ...
            tgltiba.CurrentValue = tgltiba.FormValue;
            tgltiba.CurrentValue = UnformatDateTime(tgltiba.CurrentValue, tgltiba.FormatPattern);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedBy() {
            if (lastUpdatedBy.Visible && lastUpdatedBy.Required) {
                if (!lastUpdatedBy.IsDetailKey && Empty(lastUpdatedBy.FormValue)) {
                    lastUpdatedBy.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomnamakapal() {
            if (namakapal.Visible && namakapal.Required) {
                if (!namakapal.IsDetailKey && Empty(namakapal.FormValue)) {
                    namakapal.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomshipment() {
            if (shipment.Visible && shipment.Required) {
                if (!shipment.IsDetailKey && Empty(shipment.FormValue)) {
                    shipment.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }
        ...
        private void ValidateCustomproduk() {
            if (produk.Visible && produk.Required) {
                if (!produk.IsDetailKey && Empty(produk.FormValue)) {
                    produk.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }

        private void ValidateCustomtgltiba() {
            if (tgltiba.Visible && tgltiba.Required) {
                if (!tgltiba.IsDetailKey && Empty(tgltiba.FormValue)) {
                    tgltiba.AddErrorMessage(ConvertToString(nominasi.RequiredErrorMessage).Replace("%s", nominasi.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomtgltiba();
                if (!CheckDate(tgltiba.FormValue, tgltiba.FormatPattern)) {
                    tgltiba.AddErrorMessage(tgltiba.GetErrorMessage(false));
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // tgltiba
            tgltiba.SetDbValue(rsnew, ConvertToDateTime(tgltiba.CurrentValue, tgltiba.FormatPattern), tgltiba.ReadOnly);
        ...
        {
            object? value;
            if (row.TryGetValue("tgltiba", out value)) // tgltiba
                tgltiba.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasDataKapalStsPnrBbmList")), "", TableVar, true);
            string pageId = "edit";
