...
public partial class SnDOne {
    ...
    public class PenyimpananSampleListBase : PenyimpananSample
    {
        ...
        // Constructor
        public PenyimpananSampleListBase(Controller? controller)
        {
        ...
        // Constructor
        public PenyimpananSampleListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            if (filter.TryGetValue("x_NomorPenyimpananSample", out sv)) {
                RestoreFieldFilter(NomorPenyimpananSample, filter, "NomorPenyimpananSample", sv);
            }

            // Field JenisSample
            if (filter.TryGetValue("x_JenisSample", out sv)) {
                RestoreFieldFilter(JenisSample, filter, "JenisSample", sv);
            }
            ...
            if (filter.TryGetValue("x_NamaMasterSample", out sv)) {
                RestoreFieldFilter(NamaMasterSample, filter, "NamaMasterSample", sv);
            }

            // Field NomorSegel
            if (filter.TryGetValue("x_NomorSegel", out sv)) {
                RestoreFieldFilter(NomorSegel, filter, "NomorSegel", sv);
            }
            ...
            // Field Foto
            if (filter.TryGetValue("x_Foto", out sv)) {
                RestoreFieldFilter(Foto, filter, "Foto", sv);
            }

            // Field ExpiredEst
            if (filter.TryGetValue("x_ExpiredEst", out sv)) {
                RestoreFieldFilter(ExpiredEst, filter, "ExpiredEst", sv);
            }

            // Field TanggalDimusnahkan
            if (filter.TryGetValue("x_TanggalDimusnahkan", out sv)) {
                RestoreFieldFilter(TanggalDimusnahkan, filter, "TanggalDimusnahkan", sv);
            }

            // Field LokasiPemusnahan
            if (filter.TryGetValue("x_LokasiPemusnahan", out sv)) {
                RestoreFieldFilter(LokasiPemusnahan, filter, "LokasiPemusnahan", sv);
            }
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("NomorPenyimpananSample", NomorPenyimpananSample, false, config);
            filterList += ProcessFieldFilter("JenisSample", JenisSample, true, config);
            filterList += ProcessFieldFilter("IdPlant", IdPlant, true, config);
            filterList += ProcessFieldFilter("Tanggal", Tanggal, false, config);
            filterList += ProcessFieldFilter("NamaMasterSample", NamaMasterSample, false, config);
            filterList += ProcessFieldFilter("NomorSegel", NomorSegel, false, config);
            filterList += ProcessFieldFilter("Status", Status, true, config);
            filterList += ProcessFieldFilter("ExpiredEst", ExpiredEst, false, config);
            filterList += ProcessFieldFilter("TanggalDimusnahkan", TanggalDimusnahkan, false, config);
            filterList += ProcessFieldFilter("LokasiPemusnahan", LokasiPemusnahan, true, config);
            filterList += ProcessFieldFilter("CreatedBy", CreatedBy, false, config);
            filterList += ProcessFieldFilter("etlDate", etlDate, false, config);
            filterList += ProcessFieldFilter("LastUpdatedBy", LastUpdatedBy, false, config);
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadFieldNomorPenyimpananSampleSearchValues();
            LoadFieldJenisSampleSearchValues();
            LoadFieldIdPlantSearchValues();
            LoadFieldTanggalSearchValues();
            LoadFieldNamaMasterSampleSearchValues();
            LoadFieldNomorSegelSearchValues();
            LoadFieldStatusSearchValues();
            LoadFieldFotoSearchValues();
            LoadFieldExpiredEstSearchValues();
            LoadFieldTanggalDimusnahkanSearchValues();
            LoadFieldLokasiPemusnahanSearchValues();
        ...
        private void LoadFieldNomorPenyimpananSampleSearchValues()
        {
            // NomorPenyimpananSample
            LoadPrimarySearchValueNomorPenyimpananSample();
            LoadSearchOperatorNomorPenyimpananSample();
        }

        private void LoadPrimarySearchValueNomorPenyimpananSample()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NomorPenyimpananSample")) {
        ...
        private void LoadSearchOperatorNomorPenyimpananSample()
        {
        ...
        private void LoadFieldJenisSampleSearchValues()
        {
            // JenisSample
            LoadPrimarySearchValueJenisSample();
            LoadSearchOperatorJenisSample();
        }

        private void LoadPrimarySearchValueJenisSample()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_JenisSample[]")) {
        ...
        private void LoadSearchOperatorJenisSample()
        {
        ...
        private void LoadFieldNamaMasterSampleSearchValues()
        {
            // NamaMasterSample
            LoadPrimarySearchValueNamaMasterSample();
            LoadSearchOperatorNamaMasterSample();
        }

        private void LoadPrimarySearchValueNamaMasterSample()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaMasterSample")) {
                NamaMasterSample.AdvancedSearch.SearchValue = Get("x_NamaMasterSample");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorNamaMasterSample()
        {
            if (Query.ContainsKey("z_NamaMasterSample"))
                NamaMasterSample.AdvancedSearch.SearchOperator = Get("z_NamaMasterSample");
        }

        private void LoadFieldNomorSegelSearchValues()
        {
            // NomorSegel
            LoadPrimarySearchValueNomorSegel();
            LoadSearchOperatorNomorSegel();
        }

        private void LoadPrimarySearchValueNomorSegel()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NomorSegel")) {
                NomorSegel.AdvancedSearch.SearchValue = Get("x_NomorSegel");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorNomorSegel()
        {
            if (Query.ContainsKey("z_NomorSegel"))
                NomorSegel.AdvancedSearch.SearchOperator = Get("z_NomorSegel");
        }
        ...
        private void LoadFieldFotoSearchValues()
        {
            // Foto
            LoadPrimarySearchValueFoto();
            LoadSearchOperatorFoto();
        }

        private void LoadPrimarySearchValueFoto()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Foto")) {
                Foto.AdvancedSearch.SearchValue = Get("x_Foto");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorFoto()
        {
            if (Query.ContainsKey("z_Foto"))
                Foto.AdvancedSearch.SearchOperator = Get("z_Foto");
        }

        private void LoadFieldExpiredEstSearchValues()
        {
            // ExpiredEst
            LoadPrimarySearchValueExpiredEst();
            LoadSearchOperatorExpiredEst();
        }

        private void LoadPrimarySearchValueExpiredEst()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_ExpiredEst")) {
                ExpiredEst.AdvancedSearch.SearchValue = Get("x_ExpiredEst");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorExpiredEst()
        {
            if (Query.ContainsKey("z_ExpiredEst"))
                ExpiredEst.AdvancedSearch.SearchOperator = Get("z_ExpiredEst");
        }

        private void LoadFieldTanggalDimusnahkanSearchValues()
        {
            // TanggalDimusnahkan
            LoadPrimarySearchValueTanggalDimusnahkan();
            LoadSearchOperatorTanggalDimusnahkan();
        }

        private void LoadPrimarySearchValueTanggalDimusnahkan()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TanggalDimusnahkan")) {
                TanggalDimusnahkan.AdvancedSearch.SearchValue = Get("x_TanggalDimusnahkan");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorTanggalDimusnahkan()
        {
            if (Query.ContainsKey("z_TanggalDimusnahkan"))
                TanggalDimusnahkan.AdvancedSearch.SearchOperator = Get("z_TanggalDimusnahkan");
        }

        private void LoadFieldLokasiPemusnahanSearchValues()
        {
            // LokasiPemusnahan
            LoadPrimarySearchValueLokasiPemusnahan();
            LoadSearchOperatorLokasiPemusnahan();
        }

        private void LoadPrimarySearchValueLokasiPemusnahan()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_LokasiPemusnahan[]")) {
        ...
        private void LoadSearchOperatorLokasiPemusnahan()
        {
        ...
        private void LoadPrimarySearchValueetlDate()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_etlDate")) {
                etlDate.AdvancedSearch.SearchValue = Get("x_etlDate");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatoretlDate()
        {
            if (Query.ContainsKey("z_etlDate"))
                etlDate.AdvancedSearch.SearchOperator = Get("z_etlDate");
        }
