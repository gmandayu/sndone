...
public partial class SnDOne {
    ...
    public class MwtOnlineListBase : MwtOnline
    {
        ...
        // Constructor
        public MwtOnlineListBase(Controller? controller)
        {
            TableName = "MWTOnline";
        ...
        // Constructor
        public MwtOnlineListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            // Field NoMWTOnline
            if (filter.TryGetValue("x_NoMWTOnline", out sv)) {
                RestoreFieldFilter(NoMWTOnline, filter, "NoMWTOnline", sv);
            }

            // Field LinkRedirect
            if (filter.TryGetValue("x_LinkRedirect", out sv)) {
                RestoreFieldFilter(LinkRedirect, filter, "LinkRedirect", sv);
            }

            // Field LookupPlant
            if (filter.TryGetValue("x_LookupPlant", out sv)) {
                RestoreFieldFilter(LookupPlant, filter, "LookupPlant", sv);
            }

            // Field IdPlant
            if (filter.TryGetValue("x_IdPlant", out sv)) {
                RestoreFieldFilter(IdPlant, filter, "IdPlant", sv);
            }

            // Field TaskList
            if (filter.TryGetValue("x_TaskList", out sv)) {
                RestoreFieldFilter(TaskList, filter, "TaskList", sv);
            }

            // Field Status
            if (filter.TryGetValue("x_Status", out sv)) {
                RestoreFieldFilter(Status, filter, "Status", sv);
            }

            // Field File
            if (filter.TryGetValue("x__File", out sv)) {
                RestoreFieldFilter(_File, filter, "_File", sv);
            }

            // Field DownloadFile
            if (filter.TryGetValue("x_DownloadFile", out sv)) {
                RestoreFieldFilter(DownloadFile, filter, "DownloadFile", sv);
            }

            // Field StatusEmail
            if (filter.TryGetValue("x_StatusEmail", out sv)) {
                RestoreFieldFilter(StatusEmail, filter, "StatusEmail", sv);
            }
            ...
            // Field CreatedDate
            if (filter.TryGetValue("x_CreatedDate", out sv)) {
                RestoreFieldFilter(CreatedDate, filter, "CreatedDate", sv);
            }
            ...
            // Field CompletedBy
            if (filter.TryGetValue("x_CompletedBy", out sv)) {
                RestoreFieldFilter(CompletedBy, filter, "CompletedBy", sv);
            }

            // Field CompletedDate
            if (filter.TryGetValue("x_CompletedDate", out sv)) {
                RestoreFieldFilter(CompletedDate, filter, "CompletedDate", sv);
            }
        }
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("DownloadFile", DownloadFile, false, config);
            filterList += ProcessFieldFilter("CreatedBy", CreatedBy, true, config);
            filterList += ProcessFieldFilter("CreatedDate", CreatedDate, true, config);
            ...
            filterList += ProcessFieldFilter("CompletedBy", CompletedBy, true, config);
            filterList += ProcessFieldFilter("CompletedDate", CompletedDate, true, config);
            return filterList;
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldSearchValues()
        {
            ...
            LoadFieldNoMWTOnlineSearchValues();
            LoadFieldLinkRedirectSearchValues();
            LoadFieldLookupPlantSearchValues();
            LoadFieldIdPlantSearchValues();
            LoadFieldTaskListSearchValues();
            LoadFieldStatusSearchValues();
            LoadField_FileSearchValues();
            LoadFieldDownloadFileSearchValues();
            LoadFieldStatusEmailSearchValues();
            LoadFieldCreatedBySearchValues();
            LoadFieldCreatedDateSearchValues();
            LoadFieldLastUpdatedBySearchValues();
            LoadFieldLastUpdatedDateSearchValues();
            LoadFieldCompletedBySearchValues();
            LoadFieldCompletedDateSearchValues();
        }
        ...
        private void LoadFieldNoMWTOnlineSearchValues()
        {
            // NoMWTOnline
            LoadPrimarySearchValueNoMWTOnline();
            LoadSearchOperatorNoMWTOnline();
        }

        private void LoadPrimarySearchValueNoMWTOnline()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NoMWTOnline[]")) {
        ...
        private void LoadSearchOperatorNoMWTOnline()
        {
        ...
        private void LoadFieldLookupPlantSearchValues()
        {
            // LookupPlant
            LoadPrimarySearchValueLookupPlant();
            LoadSearchOperatorLookupPlant();
        }

        private void LoadPrimarySearchValueLookupPlant()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_LookupPlant[]")) {
        ...
        private void LoadSearchOperatorLookupPlant()
        {
        ...
        private void LoadFieldTaskListSearchValues()
        {
            // TaskList
            LoadPrimarySearchValueTaskList();
            LoadSearchOperatorTaskList();
        }

        private void LoadPrimarySearchValueTaskList()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TaskList[]")) {
        ...
        private void LoadSearchOperatorTaskList()
        {
        ...
        private void LoadField_FileSearchValues()
        {
            // _File
            LoadPrimarySearchValue_File();
            LoadSearchOperator_File();
        }

        private void LoadPrimarySearchValue_File()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x__File[]")) {
        ...
        private void LoadSearchOperator_File()
        {
            if (Query.ContainsKey("z__File"))
                _File.AdvancedSearch.SearchOperator = Get("z__File", Config.FilterOptionSeparator);
        }

        private void LoadFieldDownloadFileSearchValues()
        {
            // DownloadFile
            LoadPrimarySearchValueDownloadFile();
            LoadSearchOperatorDownloadFile();
        }

        private void LoadPrimarySearchValueDownloadFile()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_DownloadFile")) {
                DownloadFile.AdvancedSearch.SearchValue = Get("x_DownloadFile");
            }
            else if (!IsAddOrEdit) {
        ...
        private void LoadSearchOperatorDownloadFile()
        {
            if (Query.ContainsKey("z_DownloadFile"))
                DownloadFile.AdvancedSearch.SearchOperator = Get("z_DownloadFile");
        }

        private void LoadFieldStatusEmailSearchValues()
        {
            // StatusEmail
            LoadPrimarySearchValueStatusEmail();
            LoadSearchOperatorStatusEmail();
        }

        private void LoadPrimarySearchValueStatusEmail()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_StatusEmail[]")) {
        ...
        private void LoadSearchOperatorStatusEmail()
        {
        ...
        private void LoadFieldCreatedDateSearchValues()
        {
            // CreatedDate
            LoadPrimarySearchValueCreatedDate();
            LoadSearchOperatorCreatedDate();
        }

        private void LoadPrimarySearchValueCreatedDate()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_CreatedDate[]")) {
        ...
        private void LoadSearchOperatorCreatedDate()
        {
        ...
        private void LoadFieldCompletedBySearchValues()
        {
            // CompletedBy
            LoadPrimarySearchValueCompletedBy();
            LoadSearchOperatorCompletedBy();
        }

        private void LoadPrimarySearchValueCompletedBy()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_CompletedBy[]")) {
        ...
        private void LoadSearchOperatorCompletedBy()
        {
        ...
        private void LoadFieldCompletedDateSearchValues()
        {
            // CompletedDate
            LoadPrimarySearchValueCompletedDate();
            LoadSearchOperatorCompletedDate();
        }

        private void LoadPrimarySearchValueCompletedDate()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_CompletedDate[]")) {
        ...
        private void LoadSearchOperatorCompletedDate()
        {
