...
public partial class SnDOne {
    ...
    public class SubAktivitasLogbookMonitoringStockEditBase : SubAktivitasLogbookMonitoringStock
    {
        ...
        // Constructor
        public SubAktivitasLogbookMonitoringStockEditBase()
        {
        ...
        // Constructor
        public SubAktivitasLogbookMonitoringStockEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasLogbookMonitoringStockList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasLogbookMonitoringStockList") {
                                ...
                                returnUrl = "SubAktivitasLogbookMonitoringStockList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'Nomor_Tangki' before field var 'x_Nomor_Tangki'
            val = CurrentForm.HasValue("Nomor_Tangki") ? CurrentForm.GetValue("Nomor_Tangki") : CurrentForm.GetValue("x_Nomor_Tangki");
            if (!Nomor_Tangki.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Nomor_Tangki") && !CurrentForm.HasValue("x_Nomor_Tangki")) // DN
                    Nomor_Tangki.Visible = false; // Disable update for API request
                else
                    Nomor_Tangki.SetFormValue(val);
            }
            ...
            // Check field name 'Level_Tangki' before field var 'x_Level_Tangki'
            val = CurrentForm.HasValue("Level_Tangki") ? CurrentForm.GetValue("Level_Tangki") : CurrentForm.GetValue("x_Level_Tangki");
            if (!Level_Tangki.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Level_Tangki") && !CurrentForm.HasValue("x_Level_Tangki")) // DN
                    Level_Tangki.Visible = false; // Disable update for API request
                else
                    Level_Tangki.SetFormValue(val);
            }

            // Check field name 'Volume_KLObs' before field var 'x_Volume_KLObs'
            val = CurrentForm.HasValue("Volume_KLObs") ? CurrentForm.GetValue("Volume_KLObs") : CurrentForm.GetValue("x_Volume_KLObs");
            if (!Volume_KLObs.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Volume_KLObs") && !CurrentForm.HasValue("x_Volume_KLObs")) // DN
                    Volume_KLObs.Visible = false; // Disable update for API request
                else
                    Volume_KLObs.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (Nomor_Tangki.Visible && Nomor_Tangki.Required) {
                    if (!Nomor_Tangki.IsDetailKey && Empty(Nomor_Tangki.FormValue)) {
                        Nomor_Tangki.AddErrorMessage(ConvertToString(Nomor_Tangki.RequiredErrorMessage).Replace("%s", Nomor_Tangki.Caption));
                    }
                }
                ...
                if (Level_Tangki.Visible && Level_Tangki.Required) {
                    if (!Level_Tangki.IsDetailKey && Empty(Level_Tangki.FormValue)) {
                        Level_Tangki.AddErrorMessage(ConvertToString(Level_Tangki.RequiredErrorMessage).Replace("%s", Level_Tangki.Caption));
                    }
                }
                if (Volume_KLObs.Visible && Volume_KLObs.Required) {
                    if (!Volume_KLObs.IsDetailKey && Empty(Volume_KLObs.FormValue)) {
                        Volume_KLObs.AddErrorMessage(ConvertToString(Volume_KLObs.RequiredErrorMessage).Replace("%s", Volume_KLObs.Caption));
                    }
                }
