...
public partial class SnDOne {
    ...
    /// masterToolsList
    /// </summary>
    public static MasterToolsList masterToolsList
    {
        get => HttpData.Get<MasterToolsList>("masterToolsList")!;
        set => HttpData["masterToolsList"] = value;
    }

    /// <summary>
    /// Page class for MasterTools
    /// </summary>
    public class MasterToolsList : MasterToolsListBase
    {
        // Constructor
        public MasterToolsList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterToolsList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterToolsListBase : MasterTools
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterToolsList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMasterToolslist";
        ...
        // Constructor
        public MasterToolsListBase(Controller? controller)
        {
            TableName = "MasterTools";

            // CSS class name as context
            TableVar = "MasterTools";
        ...
        if (controller != null)
            ...
            // Table object (masterTools)
            if (masterTools == null || masterTools is MasterTools)
                masterTools = this;

            // Initialize URLs
            AddUrl = "MasterToolsAdd";
            ...
            MultiDeleteUrl = "MasterToolsDelete";
            MultiUpdateUrl = "MasterToolsUpdate";
        ...
        // Page name
        public string PageName => "MasterToolsList";
        ...
        public void SetVisibility()
        {
            ...
            NamaTools.SetVisibility();
        }

        // Constructor
        public MasterToolsListBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                var result = new Dictionary<string, string> { { "url", GetUrl(url) }, { "modal", "1" } }; // modal=1
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "MasterToolsView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
...
protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("IdTools") ? dict["IdTools"] : IdTools.CurrentValue));
            return key;
        }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, NamaTools, def, true); // NamaTools
        ...
        private string BuildFieldFilters(FilterConfiguration config)
        {
            ...
            filterList += ProcessFieldFilter("NamaTools", NamaTools, true, config);
            return filterList;
        }
        ...
        protected bool CheckSearchParms() {
            ...
            if (IdTools.AdvancedSearch.IssetSession)
                return true;
            if (NamaTools.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            NamaTools.AdvancedSearch.UnsetSession();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(NamaTools, ctrl); // NamaTools
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MasterTools"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MasterTools"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MasterTools"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMasterToolslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMasterToolslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdTools.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MasterTools"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMasterToolslist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("NamaTools")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMasterToolssrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMasterToolssrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMasterToolslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_MasterTools");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(masterToolsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(masterToolsList.RowCount) + "_MasterTools");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(masterToolsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && masterToolsList.RowType == RowType.Add || IsEdit && masterToolsList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldSearchValues()
        {
            ...
            LoadFieldNamaToolsSearchValues();
        }
        ...
        private void LoadFieldNamaToolsSearchValues()
        {
            // NamaTools
            LoadPrimarySearchValueNamaTools();
            LoadSearchOperatorNamaTools();
        }

        private void LoadPrimarySearchValueNamaTools()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaTools[]")) {
                NamaTools.AdvancedSearch.SearchValue = Get("x_NamaTools[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NamaTools.AdvancedSearch.SearchValue = Get("NamaTools"); // Default Value // DN
            }
            if (!Empty(NamaTools.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNamaTools()
        {
            if (Query.ContainsKey("z_NamaTools"))
                NamaTools.AdvancedSearch.SearchOperator = Get("z_NamaTools", Config.FilterOptionSeparator);
        }
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            NamaTools.SetDbValue(row["NamaTools"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("NamaTools", NamaTools.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                // NamaTools
                    ...
                    // IdTools
                    IdTools.ViewValue = IdTools.CurrentValue;
                    IdTools.ViewCustomAttributes = "";

                    // NamaTools
                    NamaTools.ViewValue = ConvertToString(NamaTools.CurrentValue); // DN
                    NamaTools.ViewCustomAttributes = "";
                ...
                // NamaTools
                if (NamaTools.UseFilter && !Empty(NamaTools.AdvancedSearch.SearchValue)) {
                    NamaTools.EditValue = ConvertToString(NamaTools.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMasterToolssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
