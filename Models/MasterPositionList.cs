...
public partial class SnDOne {
    ...
    /// masterPositionList
    /// </summary>
    public static MasterPositionList masterPositionList
    {
        get => HttpData.Get<MasterPositionList>("masterPositionList")!;
        set => HttpData["masterPositionList"] = value;
    }

    /// <summary>
    /// Page class for MasterPosition
    /// </summary>
    public class MasterPositionList : MasterPositionListBase
    {
        // Constructor
        public MasterPositionList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterPositionList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterPositionListBase : MasterPosition
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterPositionList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMasterPositionlist";
        ...
        // Constructor
        public MasterPositionListBase()
        {
            TableName = "MasterPosition";

            // CSS class name as context
            TableVar = "MasterPosition";
            ...
            // Table object (masterPosition)
            if (masterPosition == null || masterPosition is MasterPosition)
                masterPosition = this;

            // Initialize URLs
            AddUrl = "MasterPositionAdd";
            ...
            MultiDeleteUrl = "MasterPositionDelete";
            MultiUpdateUrl = "MasterPositionUpdate";
        ...
        // Page name
        public string PageName => "MasterPositionList";
        ...
        // Constructor
        public MasterPositionListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "MasterPositionView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("IdPosition") ? dict["IdPosition"] : IdPosition.CurrentValue));
            return key;
        }
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            await SetupLookupOptions(Role);

            // Update form name to avoid conflict
            if (IsModal)
                FormName = "fMasterPositiongrid";
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(NamaPosition.AdvancedSearch.ToJson())); // Field NamaPosition
            filters.Merge(JObject.Parse(Location.AdvancedSearch.ToJson())); // Field Location
            filters.Merge(JObject.Parse(_UserLevel.AdvancedSearch.ToJson())); // Field UserLevel
            filters.Merge(JObject.Parse(Role.AdvancedSearch.ToJson())); // Field Role
        ...
        protected bool RestoreFilterList() {
            ...
            // Field NamaPosition
            if (filter?.TryGetValue("x_NamaPosition", out sv) ?? false) {
                NamaPosition.AdvancedSearch.SearchValue = sv;
                NamaPosition.AdvancedSearch.SearchOperator = filter["z_NamaPosition"];
                NamaPosition.AdvancedSearch.SearchCondition = filter["v_NamaPosition"];
                NamaPosition.AdvancedSearch.SearchValue2 = filter["y_NamaPosition"];
                NamaPosition.AdvancedSearch.SearchOperator2 = filter["w_NamaPosition"];
                NamaPosition.AdvancedSearch.Save();
            }
            ...
            // Field UserLevel
            if (filter?.TryGetValue("x__UserLevel", out sv) ?? false) {
                _UserLevel.AdvancedSearch.SearchValue = sv;
                _UserLevel.AdvancedSearch.SearchOperator = filter["z__UserLevel"];
                _UserLevel.AdvancedSearch.SearchCondition = filter["v__UserLevel"];
                _UserLevel.AdvancedSearch.SearchValue2 = filter["y__UserLevel"];
                _UserLevel.AdvancedSearch.SearchOperator2 = filter["w__UserLevel"];
                _UserLevel.AdvancedSearch.Save();
            }

            // Field Role
            if (filter?.TryGetValue("x_Role", out sv) ?? false) {
                Role.AdvancedSearch.SearchValue = sv;
                Role.AdvancedSearch.SearchOperator = filter["z_Role"];
                Role.AdvancedSearch.SearchCondition = filter["v_Role"];
                Role.AdvancedSearch.SearchValue2 = filter["y_Role"];
                Role.AdvancedSearch.SearchOperator2 = filter["w_Role"];
                Role.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, NamaPosition, def, true); // NamaPosition
            BuildSearchSql(ref where, Location, def, true); // Location
            BuildSearchSql(ref where, _UserLevel, def, true); // _UserLevel
            BuildSearchSql(ref where, Role, def, true); // Role
            ...
            if (!def && Command == "search") {
                ...
                NamaPosition.AdvancedSearch.Save(); // NamaPosition
                Location.AdvancedSearch.Save(); // Location
                _UserLevel.AdvancedSearch.Save(); // UserLevel
                Role.AdvancedSearch.Save(); // Role
        ...
        public void ShowFilterList()
        {
            ...
            // Field NamaPosition
            filter = QueryBuilderWhere("NamaPosition");
            if (Empty(filter))
                BuildSearchSql(ref filter, NamaPosition, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NamaPosition.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Location
            filter = QueryBuilderWhere("Location");
            if (Empty(filter))
                BuildSearchSql(ref filter, Location, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Location.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field Role
            filter = QueryBuilderWhere("Role");
            if (Empty(filter))
                BuildSearchSql(ref filter, Role, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Role.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        protected bool CheckSearchParms() {
            ...
            if (IdPosition.AdvancedSearch.IssetSession)
                return true;
            if (NamaPosition.AdvancedSearch.IssetSession)
                return true;
            if (Location.AdvancedSearch.IssetSession)
                return true;
            if (_UserLevel.AdvancedSearch.IssetSession)
                return true;
            if (Role.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            NamaPosition.AdvancedSearch.UnsetSession();
            Location.AdvancedSearch.UnsetSession();
            _UserLevel.AdvancedSearch.UnsetSession();
            Role.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            NamaPosition.AdvancedSearch.Load();
            Location.AdvancedSearch.Load();
            _UserLevel.AdvancedSearch.Load();
            Role.AdvancedSearch.Load();
        }
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MasterPosition"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MasterPosition"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MasterPosition"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMasterPositionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMasterPositionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdPosition.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MasterPosition"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMasterPositionlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("NamaPosition")); // DN
                CreateColumnOption(option.Add("Location")); // DN
                CreateColumnOption(option.Add("UserLevel")); // DN
                CreateColumnOption(option.Add("Role")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMasterPositionsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMasterPositionsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMasterPositionlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MasterPosition");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(masterPositionList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(masterPositionList.RowCount) + "_MasterPosition");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(masterPositionList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && masterPositionList.RowType == RowType.Add || IsEdit && masterPositionList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // NamaPosition
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaPosition[]"))
                    NamaPosition.AdvancedSearch.SearchValue = Get("x_NamaPosition[]", Config.FilterOptionSeparator);
                else
                    NamaPosition.AdvancedSearch.SearchValue = Get("NamaPosition"); // Default Value // DN
            if (!Empty(NamaPosition.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_NamaPosition"))
                NamaPosition.AdvancedSearch.SearchOperator = Get("z_NamaPosition", Config.FilterOptionSeparator);

            // Location
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Location[]"))
                    Location.AdvancedSearch.SearchValue = Get("x_Location[]", Config.FilterOptionSeparator);
                else
                    Location.AdvancedSearch.SearchValue = Get("Location"); // Default Value // DN
            if (!Empty(Location.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Location"))
                Location.AdvancedSearch.SearchOperator = Get("z_Location", Config.FilterOptionSeparator);
            ...
            // Role
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Role[]"))
                    Role.AdvancedSearch.SearchValue = Get("x_Role[]", Config.FilterOptionSeparator);
                else
                    Role.AdvancedSearch.SearchValue = Get("Role"); // Default Value // DN
            if (!Empty(Role.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Role"))
                Role.AdvancedSearch.SearchOperator = Get("z_Role", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                IdPosition.ViewValue = FormatNumber(IdPosition.ViewValue, IdPosition.FormatPattern);
                IdPosition.ViewCustomAttributes = "";

                // NamaPosition
                NamaPosition.ViewValue = ConvertToString(NamaPosition.CurrentValue); // DN
                NamaPosition.ViewCustomAttributes = "";
                ...
                if (!Empty(curVal2)) {
                    if (Role.Lookup != null && IsDictionary(Role.Lookup?.Options) && Role.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        ...
                        string filterWrk2 = SearchFilter(Role.Lookup?.GetTable()?.Fields["IdPIC"].SearchExpression, "=", Role.CurrentValue, Role.Lookup?.GetTable()?.Fields["IdPIC"].SearchDataType, "");
                        string? sqlWrk2 = Role.Lookup?.GetSql(false, filterWrk2, null, this, true, true);
                ...
                // NamaPosition
                NamaPosition.HrefValue = "";
                NamaPosition.TooltipValue = "";
                ...
                // NamaPosition
                if (NamaPosition.UseFilter && !Empty(NamaPosition.AdvancedSearch.SearchValue)) {
                    NamaPosition.EditValue = ConvertToString(NamaPosition.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // Location
                if (Location.UseFilter && !Empty(Location.AdvancedSearch.SearchValue)) {
                    Location.EditValue = ConvertToString(Location.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // Role
                if (Role.UseFilter && !Empty(Role.AdvancedSearch.SearchValue)) {
                    Role.EditValue = ConvertToString(Role.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMasterPositionsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
