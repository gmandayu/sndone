...
public partial class SnDOne {
    ...
    public class MappingPositionListBase : MappingPosition
    {
        ...
        // Constructor
        public MappingPositionListBase()
        {
        ...
        // Constructor
        public MappingPositionListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected void LoadSearchValues() {
            ...
            // NamaRegion
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaRegion[]"))
            ...
            // NamaPlant
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NamaPlant[]"))
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // IdRegion
                string curVal2 = ConvertToString(IdRegion.CurrentValue);
                if (!Empty(curVal2)) {
                    if (IdRegion.Lookup != null && IsDictionary(IdRegion.Lookup?.Options) && IdRegion.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        IdRegion.ViewValue = IdRegion.LookupCacheOption(curVal2);
                    } else { // Lookup from database // DN
                        string filterWrk2 = SearchFilter(IdRegion.Lookup?.GetTable()?.Fields["IdRegion"].SearchExpression, "=", IdRegion.CurrentValue, IdRegion.Lookup?.GetTable()?.Fields["IdRegion"].SearchDataType, "");
                        string? sqlWrk2 = IdRegion.Lookup?.GetSql(false, filterWrk2, null, this, true, true);
                        List<Dictionary<string, object>>? rswrk2 = sqlWrk2 != null ? Connection.GetRows(sqlWrk2) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                        if (rswrk2?.Count > 0 && IdRegion.Lookup != null) { // Lookup values found
                            var listwrk = IdRegion.Lookup?.RenderViewRow(rswrk2[0]);
                            IdRegion.ViewValue = IdRegion.DisplayValue(listwrk);
                        } else {
                            IdRegion.ViewValue = FormatNumber(IdRegion.CurrentValue, IdRegion.FormatPattern);
                        }
                    }
                } else {
                    IdRegion.ViewValue = DbNullValue;
                }
