...
public partial class SnDOne {
    ...
    public class MasterToolsEditBase : MasterTools
    {
        ...
        // Constructor
        public MasterToolsEditBase(Controller? controller)
        {
        ...
        // Constructor
        public MasterToolsEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("IdTools", out rv)) {
                    IdTools.FormValue = UrlDecode(rv);
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("IdTools", out rv)) {
                        IdTools.QueryValue = UrlDecode(rv);
                        loadByQuery = true;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("MasterToolsList");
            }
            return null;
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'NamaTools'
            NamaTools.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NamaTools, "NamaTools", "x_NamaTools");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomIdTools() {
            if (IdTools.Visible && IdTools.Required) {
                if (!IdTools.IsDetailKey && Empty(IdTools.FormValue)) {
                    IdTools.AddErrorMessage(ConvertToString(NamaTools.RequiredErrorMessage).Replace("%s", NamaTools.Caption));
                }
            }
        }

        private void ValidateCustomNamaTools() {
