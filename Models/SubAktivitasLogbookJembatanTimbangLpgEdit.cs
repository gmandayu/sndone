...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasLogbookJembatanTimbangLpgEdit
    /// </summary>
    public static SubAktivitasLogbookJembatanTimbangLpgEdit subAktivitasLogbookJembatanTimbangLpgEdit
    {
        get => HttpData.Get<SubAktivitasLogbookJembatanTimbangLpgEdit>("subAktivitasLogbookJembatanTimbangLpgEdit")!;
        set => HttpData["subAktivitasLogbookJembatanTimbangLpgEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasLogbookJembatanTimbangLPG
    /// </summary>
    public class SubAktivitasLogbookJembatanTimbangLpgEdit : SubAktivitasLogbookJembatanTimbangLpgEditBase
    {
        // Constructor
        public SubAktivitasLogbookJembatanTimbangLpgEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasLogbookJembatanTimbangLpgEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasLogbookJembatanTimbangLpgEditBase : SubAktivitasLogbookJembatanTimbangLpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasLogbookJembatanTimbangLpgEdit";
        ...
        // Constructor
        public SubAktivitasLogbookJembatanTimbangLpgEditBase()
        {
            TableName = "SubAktivitasLogbookJembatanTimbangLPG";
            ...
            // Table object (subAktivitasLogbookJembatanTimbangLpg)
            if (subAktivitasLogbookJembatanTimbangLpg == null || subAktivitasLogbookJembatanTimbangLpg is SubAktivitasLogbookJembatanTimbangLpg)
                subAktivitasLogbookJembatanTimbangLpg = this;
        ...
        // Page name
        public string PageName => "SubAktivitasLogbookJembatanTimbangLpgEdit";
        ...
        public void SetVisibility()
        {
            ...
            SPA.SetVisibility();
            TimbanganAwal.SetVisibility();
            TimbanganAkhir.SetVisibility();
            NetWight.SetVisibility();
            PresentaseLevelRotoGauge.SetVisibility();
        ...
        // Constructor
        public SubAktivitasLogbookJembatanTimbangLpgEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (SameString(pageName, GetPageName(ListUrl)) ||
                            ...
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "SubAktivitasLogbookJembatanTimbangLpgView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasLogbookJembatanTimbangLpgList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = ReturnUrl;
                    if (GetPageName(returnUrl) == "SubAktivitasLogbookJembatanTimbangLpgList")
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasLogbookJembatanTimbangLpgList") {
                                ...
                                returnUrl = "SubAktivitasLogbookJembatanTimbangLpgList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasLogbookJembatanTimbangLpgEdit?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'SPA' before field var 'x_SPA'
            val = CurrentForm.HasValue("SPA") ? CurrentForm.GetValue("SPA") : CurrentForm.GetValue("x_SPA");
            if (!SPA.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("SPA") && !CurrentForm.HasValue("x_SPA")) // DN
                    SPA.Visible = false; // Disable update for API request
                else
                    SPA.SetFormValue(val);
            }

            // Check field name 'TimbanganAwal' before field var 'x_TimbanganAwal'
            val = CurrentForm.HasValue("TimbanganAwal") ? CurrentForm.GetValue("TimbanganAwal") : CurrentForm.GetValue("x_TimbanganAwal");
            if (!TimbanganAwal.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TimbanganAwal") && !CurrentForm.HasValue("x_TimbanganAwal")) // DN
                    TimbanganAwal.Visible = false; // Disable update for API request
                else
                    TimbanganAwal.SetFormValue(val);
            }

            // Check field name 'TimbanganAkhir' before field var 'x_TimbanganAkhir'
            val = CurrentForm.HasValue("TimbanganAkhir") ? CurrentForm.GetValue("TimbanganAkhir") : CurrentForm.GetValue("x_TimbanganAkhir");
            if (!TimbanganAkhir.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TimbanganAkhir") && !CurrentForm.HasValue("x_TimbanganAkhir")) // DN
                    TimbanganAkhir.Visible = false; // Disable update for API request
                else
                    TimbanganAkhir.SetFormValue(val);
            }

            // Check field name 'NetWight' before field var 'x_NetWight'
            val = CurrentForm.HasValue("NetWight") ? CurrentForm.GetValue("NetWight") : CurrentForm.GetValue("x_NetWight");
            if (!NetWight.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("NetWight") && !CurrentForm.HasValue("x_NetWight")) // DN
                    NetWight.Visible = false; // Disable update for API request
                else
                    NetWight.SetFormValue(val);
            }

            // Check field name 'PresentaseLevelRotoGauge' before field var 'x_PresentaseLevelRotoGauge'
            val = CurrentForm.HasValue("PresentaseLevelRotoGauge") ? CurrentForm.GetValue("PresentaseLevelRotoGauge") : CurrentForm.GetValue("x_PresentaseLevelRotoGauge");
            if (!PresentaseLevelRotoGauge.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("PresentaseLevelRotoGauge") && !CurrentForm.HasValue("x_PresentaseLevelRotoGauge")) // DN
                    PresentaseLevelRotoGauge.Visible = false; // Disable update for API request
                else
                    PresentaseLevelRotoGauge.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            SPA.CurrentValue = SPA.FormValue;
            TimbanganAwal.CurrentValue = TimbanganAwal.FormValue;
            TimbanganAkhir.CurrentValue = TimbanganAkhir.FormValue;
            NetWight.CurrentValue = NetWight.FormValue;
            PresentaseLevelRotoGauge.CurrentValue = PresentaseLevelRotoGauge.FormValue;
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            SPA.SetDbValue(row["SPA"]);
            TimbanganAwal.SetDbValue(row["TimbanganAwal"]);
            TimbanganAkhir.SetDbValue(row["TimbanganAkhir"]);
            NetWight.SetDbValue(row["NetWight"]);
            PresentaseLevelRotoGauge.SetDbValue(row["PresentaseLevelRotoGauge"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("SPA", SPA.DefaultValue ?? DbNullValue); // DN
            row.Add("TimbanganAwal", TimbanganAwal.DefaultValue ?? DbNullValue); // DN
            row.Add("TimbanganAkhir", TimbanganAkhir.DefaultValue ?? DbNullValue); // DN
            row.Add("NetWight", NetWight.DefaultValue ?? DbNullValue); // DN
            row.Add("PresentaseLevelRotoGauge", PresentaseLevelRotoGauge.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // TimbanganAwal
            TimbanganAwal.RowCssClass = "row";

            // TimbanganAkhir
            TimbanganAkhir.RowCssClass = "row";

            // NetWight
            NetWight.RowCssClass = "row";

            // PresentaseLevelRotoGauge
            PresentaseLevelRotoGauge.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                // SPA
                SPA.ViewValue = ConvertToString(SPA.CurrentValue); // DN
                SPA.ViewCustomAttributes = "";

                // TimbanganAwal
                TimbanganAwal.ViewValue = ConvertToString(TimbanganAwal.CurrentValue); // DN
                TimbanganAwal.ViewCustomAttributes = "";

                // TimbanganAkhir
                TimbanganAkhir.ViewValue = ConvertToString(TimbanganAkhir.CurrentValue); // DN
                TimbanganAkhir.ViewCustomAttributes = "";

                // NetWight
                NetWight.ViewValue = ConvertToString(NetWight.CurrentValue); // DN
                NetWight.ViewCustomAttributes = "";

                // PresentaseLevelRotoGauge
                PresentaseLevelRotoGauge.ViewValue = ConvertToString(PresentaseLevelRotoGauge.CurrentValue); // DN
                PresentaseLevelRotoGauge.ViewCustomAttributes = "";
                ...
                // TimbanganAwal
                TimbanganAwal.HrefValue = "";

                // TimbanganAkhir
                TimbanganAkhir.HrefValue = "";

                // NetWight
                NetWight.HrefValue = "";

                // PresentaseLevelRotoGauge
                PresentaseLevelRotoGauge.HrefValue = "";
                ...
                // SPA
                SPA.SetupEditAttributes();
                if (!SPA.Raw)
                    SPA.CurrentValue = HtmlDecode(SPA.CurrentValue);
                SPA.EditValue = HtmlEncode(SPA.CurrentValue);
                SPA.PlaceHolder = RemoveHtml(SPA.Caption);

                // TimbanganAwal
                TimbanganAwal.SetupEditAttributes();
                if (!TimbanganAwal.Raw)
                    TimbanganAwal.CurrentValue = HtmlDecode(TimbanganAwal.CurrentValue);
                TimbanganAwal.EditValue = HtmlEncode(TimbanganAwal.CurrentValue);
                TimbanganAwal.PlaceHolder = RemoveHtml(TimbanganAwal.Caption);

                // TimbanganAkhir
                TimbanganAkhir.SetupEditAttributes();
                if (!TimbanganAkhir.Raw)
                    TimbanganAkhir.CurrentValue = HtmlDecode(TimbanganAkhir.CurrentValue);
                TimbanganAkhir.EditValue = HtmlEncode(TimbanganAkhir.CurrentValue);
                TimbanganAkhir.PlaceHolder = RemoveHtml(TimbanganAkhir.Caption);

                // NetWight
                NetWight.SetupEditAttributes();
                if (!NetWight.Raw)
                    NetWight.CurrentValue = HtmlDecode(NetWight.CurrentValue);
                NetWight.EditValue = HtmlEncode(NetWight.CurrentValue);
                NetWight.PlaceHolder = RemoveHtml(NetWight.Caption);

                // PresentaseLevelRotoGauge
                PresentaseLevelRotoGauge.SetupEditAttributes();
                if (!PresentaseLevelRotoGauge.Raw)
                    PresentaseLevelRotoGauge.CurrentValue = HtmlDecode(PresentaseLevelRotoGauge.CurrentValue);
                PresentaseLevelRotoGauge.EditValue = HtmlEncode(PresentaseLevelRotoGauge.CurrentValue);
                PresentaseLevelRotoGauge.PlaceHolder = RemoveHtml(PresentaseLevelRotoGauge.Caption);
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                if (SPA.Visible && SPA.Required) {
                    if (!SPA.IsDetailKey && Empty(SPA.FormValue)) {
                        SPA.AddErrorMessage(ConvertToString(SPA.RequiredErrorMessage).Replace("%s", SPA.Caption));
                    }
                }
                if (TimbanganAwal.Visible && TimbanganAwal.Required) {
                    if (!TimbanganAwal.IsDetailKey && Empty(TimbanganAwal.FormValue)) {
                        TimbanganAwal.AddErrorMessage(ConvertToString(TimbanganAwal.RequiredErrorMessage).Replace("%s", TimbanganAwal.Caption));
                    }
                }
                if (TimbanganAkhir.Visible && TimbanganAkhir.Required) {
                    if (!TimbanganAkhir.IsDetailKey && Empty(TimbanganAkhir.FormValue)) {
                        TimbanganAkhir.AddErrorMessage(ConvertToString(TimbanganAkhir.RequiredErrorMessage).Replace("%s", TimbanganAkhir.Caption));
                    }
                }
                if (NetWight.Visible && NetWight.Required) {
                    if (!NetWight.IsDetailKey && Empty(NetWight.FormValue)) {
                        NetWight.AddErrorMessage(ConvertToString(NetWight.RequiredErrorMessage).Replace("%s", NetWight.Caption));
                    }
                }
                if (PresentaseLevelRotoGauge.Visible && PresentaseLevelRotoGauge.Required) {
                    if (!PresentaseLevelRotoGauge.IsDetailKey && Empty(PresentaseLevelRotoGauge.FormValue)) {
                        PresentaseLevelRotoGauge.AddErrorMessage(ConvertToString(PresentaseLevelRotoGauge.RequiredErrorMessage).Replace("%s", PresentaseLevelRotoGauge.Caption));
                    }
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // SPA
            SPA.SetDbValue(rsnew, SPA.CurrentValue, SPA.ReadOnly);

            // TimbanganAwal
            TimbanganAwal.SetDbValue(rsnew, TimbanganAwal.CurrentValue, TimbanganAwal.ReadOnly);

            // TimbanganAkhir
            TimbanganAkhir.SetDbValue(rsnew, TimbanganAkhir.CurrentValue, TimbanganAkhir.ReadOnly);

            // NetWight
            NetWight.SetDbValue(rsnew, NetWight.CurrentValue, NetWight.ReadOnly);

            // PresentaseLevelRotoGauge
            PresentaseLevelRotoGauge.SetDbValue(rsnew, PresentaseLevelRotoGauge.CurrentValue, PresentaseLevelRotoGauge.ReadOnly);
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("SPA", out value)) // SPA
                SPA.CurrentValue = value;
            if (row.TryGetValue("TimbanganAwal", out value)) // TimbanganAwal
                TimbanganAwal.CurrentValue = value;
            if (row.TryGetValue("TimbanganAkhir", out value)) // TimbanganAkhir
                TimbanganAkhir.CurrentValue = value;
            if (row.TryGetValue("NetWight", out value)) // NetWight
                NetWight.CurrentValue = value;
            if (row.TryGetValue("PresentaseLevelRotoGauge", out value)) // PresentaseLevelRotoGauge
                PresentaseLevelRotoGauge.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasLogbookJembatanTimbangLpgList")), "", TableVar, true);
            string pageId = "edit";
