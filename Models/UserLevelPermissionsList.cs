...
public partial class SnDOne {
    ...
    public class UserLevelPermissionsListBase : UserLevelPermissions
    {
        ...
        // Constructor
        public UserLevelPermissionsListBase(Controller? controller)
        {
        ...
        // Constructor
        public UserLevelPermissionsListBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, _TableName, def, true); // _TableName
            BuildSearchSql(ref where, Permission, def, true); // Permission
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("TableName", _TableName, true, config);
            filterList += ProcessFieldFilter("Permission", Permission, true, config);
            return filterList;
        }
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (_TableName.AdvancedSearch.IssetSession)
                return true;
            if (Permission.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            _TableName.AdvancedSearch.UnsetSession();
            Permission.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            _TableName.AdvancedSearch.Load();
            Permission.AdvancedSearch.Load();
        }
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""UserLevelPermissions"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""UserLevelPermissions"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""UserLevelPermissions"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fUserLevelPermissionslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelPermissionslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(UserLevelID.CurrentValue + Config.CompositeKeySeparator + _TableName.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""UserLevelPermissions"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fUserLevelPermissionslist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("TableName")); // DN
                CreateColumnOption(option.Add("Permission")); // DN
            }
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelPermissionslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadField_TableNameSearchValues();
            LoadFieldPermissionSearchValues();
        }

        private void LoadField_TableNameSearchValues()
        {
            // _TableName
            LoadPrimarySearchValue_TableName();
            LoadSearchOperator_TableName();
        }

        private void LoadPrimarySearchValue_TableName()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x__TableName[]")) {
                _TableName.AdvancedSearch.SearchValue = Get("x__TableName[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                _TableName.AdvancedSearch.SearchValue = Get("_TableName"); // Default Value // DN
            }
            if (!Empty(_TableName.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperator_TableName()
        {
            if (Query.ContainsKey("z__TableName"))
                _TableName.AdvancedSearch.SearchOperator = Get("z__TableName", Config.FilterOptionSeparator);
        }

        private void LoadFieldPermissionSearchValues()
        {
            // Permission
            LoadPrimarySearchValuePermission();
            LoadSearchOperatorPermission();
        }

        private void LoadPrimarySearchValuePermission()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Permission[]")) {
                Permission.AdvancedSearch.SearchValue = Get("x_Permission[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Permission.AdvancedSearch.SearchValue = Get("Permission"); // Default Value // DN
            }
            if (!Empty(Permission.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorPermission()
        {
            if (Query.ContainsKey("z_Permission"))
                Permission.AdvancedSearch.SearchOperator = Get("z_Permission", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                UserLevelID.EditValue = UserLevelID.AdvancedSearch.SearchValue;
                UserLevelID.PlaceHolder = RemoveHtml(UserLevelID.Caption);

                // TableName
                if (_TableName.UseFilter && !Empty(_TableName.AdvancedSearch.SearchValue)) {
                    _TableName.EditValue = ConvertToString(_TableName.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // Permission
                if (Permission.UseFilter && !Empty(Permission.AdvancedSearch.SearchValue)) {
                    Permission.EditValue = ConvertToString(Permission.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
            }
