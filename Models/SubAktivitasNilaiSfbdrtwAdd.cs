...
public partial class SnDOne {
    ...
    public class SubAktivitasNilaiSfbdrtwAddBase : SubAktivitasNilaiSfbdrtw
    {
        ...
        // Constructor
        public SubAktivitasNilaiSfbdrtwAddBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasNilaiSfbdrtwAddBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private IActionResult? HandleCopyAction(Dictionary<string, object>? rsold)
        {
            if (rsold == null) {
                ...
                return Terminate("SubAktivitasNilaiSfbdrtwList");
            }
            return null;
        }
        ...
        private void HandleAjaxActions(ref string returnUrl)
        {
            if (IsModal && UseAjaxActions) {
                ...
                if (GetPageName(returnUrl) != "SubAktivitasNilaiSfbdrtwList") {
                    ...
                    returnUrl = "SubAktivitasNilaiSfbdrtwList";
                }
            }
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'SFBD_KL_Obs'
            SFBD_KL_Obs.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(SFBD_KL_Obs, "SFBD_KL_Obs", "x_SFBD_KL_Obs");

            // Standard handling for 'SFBD_KL_15'
            SFBD_KL_15.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(SFBD_KL_15, "SFBD_KL_15", "x_SFBD_KL_15");
        ...
        private void ValidateCustomSFBD_KL_Obs() {
            if (SFBD_KL_Obs.Visible && SFBD_KL_Obs.Required) {
                if (!SFBD_KL_Obs.IsDetailKey && Empty(SFBD_KL_Obs.FormValue)) {
                    SFBD_KL_Obs.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }

        private void ValidateCustomSFBD_KL_15() {
            if (SFBD_KL_15.Visible && SFBD_KL_15.Required) {
                if (!SFBD_KL_15.IsDetailKey && Empty(SFBD_KL_15.FormValue)) {
                    SFBD_KL_15.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFBD_Barrels() {
            if (SFBD_Barrels.Visible && SFBD_Barrels.Required) {
                if (!SFBD_Barrels.IsDetailKey && Empty(SFBD_Barrels.FormValue)) {
                    SFBD_Barrels.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFBD_LT() {
            if (SFBD_LT.Visible && SFBD_LT.Required) {
                if (!SFBD_LT.IsDetailKey && Empty(SFBD_LT.FormValue)) {
                    SFBD_LT.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFBD_MT() {
            if (SFBD_MT.Visible && SFBD_MT.Required) {
                if (!SFBD_MT.IsDetailKey && Empty(SFBD_MT.FormValue)) {
                    SFBD_MT.AddErrorMessage(ConvertToString(StatusAktivitas.RequiredErrorMessage).Replace("%s", StatusAktivitas.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomSFBD_KL_Obs();
                ValidateCustomSFBD_KL_15();
