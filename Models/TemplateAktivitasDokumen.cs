...
public partial class SnDOne {
    /// <summary>
    /// templateAktivitasDokumen
    /// </summary>
    [MaybeNull]
    public static TemplateAktivitasDokumen templateAktivitasDokumen
    {
        get => HttpData.Resolve<TemplateAktivitasDokumen>("TemplateAktivitasDokumen");
        set => HttpData["TemplateAktivitasDokumen"] = value;
    }

    /// <summary>
    /// Table class for TemplateAktivitasDokumen
    /// </summary>
    public class TemplateAktivitasDokumen : DbTable
    {
        public override Dictionary<string, string> KeyFields { get; set; } = new() { // DN
            { "IdTemplateAktivitasDokumen", "IdTemplateAktivitasDokumen" },
        };
        ...
        // Constructor
        public TemplateAktivitasDokumen()
        {
            // Language object // DN
            Language = ResolveLanguage();
            TableVar = "TemplateAktivitasDokumen";
            Name = "TemplateAktivitasDokumen";
            Type = "TABLE";
            UpdateTable = "dbo.TemplateAktivitasDokumen"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            IdTemplateAktivitasDokumen = new(this, "x_IdTemplateAktivitasDokumen", 3, SqlDbType.Int) {
                ...
                CustomMessage = Language.FieldPhrase("TemplateAktivitasDokumen", "IdTemplateAktivitasDokumen", "CustomMsg"),
                IsUpload = false
            };
            ...
            IdTemplateAktivitas = new(this, "x_IdTemplateAktivitas", 3, SqlDbType.Int) {
                ...
                CustomMessage = Language.FieldPhrase("TemplateAktivitasDokumen", "IdTemplateAktivitas", "CustomMsg"),
                IsUpload = false
            };
            ...
            IdTemplateAktivitas.GetSelectFilter = () => "TipeAktivitas = 'Dokumen'";
            ...
            IdDokumen = new(this, "x_IdDokumen", 3, SqlDbType.Int) {
                ...
                DefaultErrorMessage = Language.Phrase("IncorrectInteger"),
                SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"],
                CustomMessage = Language.FieldPhrase("TemplateAktivitasDokumen", "IdDokumen", "CustomMsg"),
                IsUpload = false
            };
            IdDokumen.Raw = true;
            ...
            WajibUpload = new(this, "x_WajibUpload", 11, SqlDbType.Bit) {
                ...
                CustomMessage = Language.FieldPhrase("TemplateAktivitasDokumen", "WajibUpload", "CustomMsg"),
                IsUpload = false
            };
            WajibUpload.Raw = true;
            WajibUpload.Lookup = new Lookup<DbField>(WajibUpload, "TemplateAktivitasDokumen", true, "WajibUpload", new List<string> {"WajibUpload", "", "", ""}, "", "", new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, false, "", "", "");
            WajibUpload.UserValues = ["1"];
            WajibUpload.GetDefault = () => true;
        ...
        public string MasterFilterFromSession
        {
            ...
            if (CurrentMasterTable == "TemplateAktivitas") {
                dynamic masterTable = Resolve("TemplateAktivitas")!;
                if (!Empty(IdTemplateAktivitas.SessionValue))
                    masterFilter += "" + KeyFilter(masterTable.IdTemplateAktivitas, IdTemplateAktivitas.SessionValue, masterTable.IdTemplateAktivitas.DataType, masterTable.DbId);
                else
                    return "";
            }
                return masterFilter;
        ...
        public string DetailFilterFromSession
        {
            get { // Detail filter
                ...
                if (CurrentMasterTable == "TemplateAktivitas") {
                    dynamic masterTable = Resolve("TemplateAktivitas")!;
                    if (!Empty(IdTemplateAktivitas.SessionValue))
                        detailFilter += "" + KeyFilter(IdTemplateAktivitas, IdTemplateAktivitas.SessionValue, masterTable.IdTemplateAktivitas.DataType, DbId);
                    else
                        return "";
                }
                return detailFilter;
        ...
        public string? MasterFilter(dynamic? masterTable, Dictionary<string, object?> keys) // DN
        {
            ...
            case "TemplateAktivitas":
                key = keys["IdTemplateAktivitas"] ?? "";
                if (Empty(key)) {
                    if (masterTable.IdTemplateAktivitas.Required) // Required field and empty value
                ...
                if (validKeys) {
                    return KeyFilter(masterTable.IdTemplateAktivitas, keys["IdTemplateAktivitas"], IdTemplateAktivitas.DataType, DbId);
                }
                break;
            }
        ...
        public string DetailFilter(dynamic masterTable) // DN
        {
            return masterTable.TableVar switch {
                "TemplateAktivitas" => KeyFilter(IdTemplateAktivitas, masterTable.IdTemplateAktivitas.DbValue, masterTable.IdTemplateAktivitas.DataType, masterTable.DbId),
                _ => ""
            };
        }
        ...
        {
            get => _sqlFrom ?? "dbo.TemplateAktivitasDokumen";
            set => _sqlFrom = value;
        }
            ...
            where TEntity : class
        {
            ...
            var prop = typeof(TEntity).GetRuntimeProperties().FirstOrDefault(p => p.GetCustomAttribute(typeof(ColumnAttribute)) is ColumnAttribute attr &&
                attr.Name == "IdTemplateAktivitasDokumen");
        ...
        public async Task<int> InsertAsync(object data)
        {
            ...
            if (data is IDictionary<string, object> d)
                ...
                throw new ArgumentException("Data must be of anonymous or Dictionary<string, object> or Entities.TemplateAktivitasDokuman type", "data");
            row = row.Where(kvp => {
            ...
            try {
                ...
                if (row.ContainsKey("IdTemplateAktivitasDokumen"))
                    row["IdTemplateAktivitasDokumen"] = lastInsertedId;
                else
                    row.Add("IdTemplateAktivitasDokumen", lastInsertedId);
                IdTemplateAktivitasDokumen.SetDbValue(lastInsertedId);
                result = 1;
            } catch (Exception e) {
        ...
        private string _sqlKeyFilter => "[IdTemplateAktivitasDokumen] = @IdTemplateAktivitasDokumen@";
        ...
        public string ReturnUrl
        {
            get {
                ...
                if (!Empty(Session[name])) {
                    ...
                    return "TemplateAktivitasDokumenList";
                }
            }
            set {
        ...
        public string GetModalCaption(string pageName)
        {
            return pageName switch {
                "TemplateAktivitasDokumenView" => Language.Phrase("View"),
                "TemplateAktivitasDokumenEdit" => Language.Phrase("Edit"),
                "TemplateAktivitasDokumenAdd" => Language.Phrase("Add"),
                _ => String.Empty
            };
        }
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "TemplateAktivitasDokumenView",
                Config.ApiAddAction => "TemplateAktivitasDokumenAdd",
                Config.ApiEditAction => "TemplateAktivitasDokumenEdit",
                Config.ApiDeleteAction => "TemplateAktivitasDokumenDelete",
                Config.ApiListAction => "TemplateAktivitasDokumenList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "TemplateAktivitasDokumenList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("TemplateAktivitasDokumenView", parm);
            else
                url = KeyUrl("TemplateAktivitasDokumenView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "TemplateAktivitasDokumenAdd";
        ...
        public string GetAddUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = "TemplateAktivitasDokumenAdd?" + parm;
            else
                url = "TemplateAktivitasDokumenAdd";
        ...
        {
            string url = "";
            url = KeyUrl("TemplateAktivitasDokumenEdit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("TemplateAktivitasDokumenList", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("TemplateAktivitasDokumenAdd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("TemplateAktivitasDokumenList", "action=copy"))); // DN
        ...
        public string GetDeleteUrl(string parm = "")
        {
            return UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
                ...
                : AppPath(KeyUrl("TemplateAktivitasDokumenDelete", parm)); // DN
        }
        ...
        {
            if (CurrentMasterTable == "TemplateAktivitas" && !url.Contains(Config.TableShowMaster + "=")) {
                url += (url.Contains("?") ? "&" : "?") + Config.TableShowMaster + "=" + CurrentMasterTable;
                url += "&" + ForeignKeyUrl("fk_IdTemplateAktivitas", IdTemplateAktivitas.SessionValue); // Use Session Value
            }
            return url;
        }
        ...
        {
            string json = "";
            json += "\"IdTemplateAktivitasDokumen\":" + ConvertToJson(IdTemplateAktivitasDokumen.CurrentValue, "number", true);
            json = "{" + json + "}";
            if (htmlEncode)
        ...
        {
            if (!IsNull(IdTemplateAktivitasDokumen.CurrentValue)) {
                url += "/" + IdTemplateAktivitasDokumen.CurrentValue;
            } else {
        ...
        public List<string> GetRecordKeys()
        {
            ...
            if (Post("key_m[]", out sv) || Get("key_m[]", out sv)) { // DN
                ...
                if (RouteValues.TryGetValue("IdTemplateAktivitasDokumen", out object? v0)) { // IdTemplateAktivitasDokumen // DN
                    key = UrlDecode(v0); // DN
                } else if (IsApi() && !Empty(keyValues)) {
                    key = keyValues[0];
                } else {
                    key = Param("IdTemplateAktivitasDokumen");
                }
                keysList.Add(key);
            }
            // Check keys
            foreach (var keys in keysList) {
                if (KeyFields.Count > 1 && (!IsArray(keys) || keys.Length != KeyFields.Count))
                    continue;
                if (!IsNumeric(keys)) // IdTemplateAktivitasDokumen
                    continue;
                result.Add(keys);
            }
            return result;
        ...
        public string GetFilterFromRecordKeys(bool setCurrent = true)
        {
            ...
            foreach (var keys in recordKeys) {
                if (!Empty(keyFilter))
                    keyFilter += " OR ";
                if (setCurrent)
                    IdTemplateAktivitasDokumen.CurrentValue = keys;
                else
                    IdTemplateAktivitasDokumen.OldValue = keys;
        ...
        {
            string pageName = "TemplateAktivitasDokumenList";
        ...
        public async Task RenderListRow()
        {
            ...
            IdDokumen.ViewValue = Empty(curVal2) ? DbNullValue : FormatNumber(IdDokumen.CurrentValue, IdDokumen.FormatPattern);
            if (!Empty(curVal2)) {
