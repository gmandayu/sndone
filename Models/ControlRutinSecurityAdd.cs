...
public partial class SnDOne {
    ...
    public class ControlRutinSecurityAddBase : ControlRutinSecurity
    {
        ...
        // Constructor
        public ControlRutinSecurityAddBase(Controller? controller)
        {
        ...
        // Constructor
        public ControlRutinSecurityAddBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private string HandleNonPostBackAction()
        {
            ...
            if (RouteValues.TryGetValue("IdCRS", out rv)) {
                IdCRS.QueryValue = UrlDecode(rv);
        ...
        private IActionResult? HandleCopyAction(Dictionary<string, object>? rsold)
        {
            if (rsold == null) {
                ...
                return Terminate("ControlRutinSecurityList");
            }
            return null;
        }
        ...
        private void HandleAjaxActions(ref string returnUrl)
        {
            if (IsModal && UseAjaxActions) {
                ...
                if (GetPageName(returnUrl) != "ControlRutinSecurityList") {
                    ...
                    returnUrl = "ControlRutinSecurityList";
                }
            }
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'NomorCRS'
            NomorCRS.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NomorCRS, "NomorCRS", "x_NomorCRS");
            ...
            SetNormalField(Plant, "Plant", "x_Plant", true, validate, false);
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                Plant.EditValue = Empty(curVal2) ? DbNullValue : HtmlEncode(FormatNumber(Plant.CurrentValue, Plant.FormatPattern));
                if (!Empty(curVal2)) {
        ...
        private void ValidateCustomNomorCRS() {
            if (NomorCRS.Visible && NomorCRS.Required) {
                if (!NomorCRS.IsDetailKey && Empty(NomorCRS.FormValue)) {
                    NomorCRS.AddErrorMessage(ConvertToString(Catatan.RequiredErrorMessage).Replace("%s", Catatan.Caption));
                }
            }
        }
