...
public partial class SnDOne {
    ...
    public class UserLevelPermissionsListBase : UserLevelPermissions
    {
        ...
        public void ShowFilterList()
        {
            ...
            // Field TableName
            filter = QueryBuilderWhere("TableName");
            if (Empty(filter))
                BuildSearchSql(ref filter, _TableName, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + _TableName.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Permission
            filter = QueryBuilderWhere("Permission");
            if (Empty(filter))
                BuildSearchSql(ref filter, Permission, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Permission.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        protected void LoadSearchValues() {
            ...
            // _TableName
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x__TableName[]"))
            ...
            // Permission
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Permission[]"))
