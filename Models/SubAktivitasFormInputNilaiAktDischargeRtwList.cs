...
public partial class SnDOne {
    ...
    public class SubAktivitasFormInputNilaiAktDischargeRtwListBase : SubAktivitasFormInputNilaiAktDischargeRtw
    {
        ...
        // Constructor
        public SubAktivitasFormInputNilaiAktDischargeRtwListBase(Controller? controller)
        {
        ...
        // Constructor
        public SubAktivitasFormInputNilaiAktDischargeRtwListBase() : this(null) { }
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private void ProcessFieldFilters(Dictionary<string, string>? filter)
        {
            ...
            if (filter.TryGetValue("x_StatusAktivitas", out sv)) {
                RestoreFieldFilter(StatusAktivitas, filter, "StatusAktivitas", sv);
            }
            ...
            // Field Import
            if (filter.TryGetValue("x_Import", out sv)) {
                RestoreFieldFilter(Import, filter, "Import", sv);
            }

            // Field ApakahAdaROB
            if (filter.TryGetValue("x_ApakahAdaROB", out sv)) {
                RestoreFieldFilter(ApakahAdaROB, filter, "ApakahAdaROB", sv);
            }

            // Field Tujuan
            if (filter.TryGetValue("x_Tujuan", out sv)) {
                RestoreFieldFilter(Tujuan, filter, "Tujuan", sv);
            }

            // Field SFAD_KLObs
            if (filter.TryGetValue("x_SFAD_KLObs", out sv)) {
                RestoreFieldFilter(SFAD_KLObs, filter, "SFAD_KLObs", sv);
            }

            // Field SFAD_KL15
            if (filter.TryGetValue("x_SFAD_KL15", out sv)) {
                RestoreFieldFilter(SFAD_KL15, filter, "SFAD_KL15", sv);
            }

            // Field SFAD_Barrels
            if (filter.TryGetValue("x_SFAD_Barrels", out sv)) {
                RestoreFieldFilter(SFAD_Barrels, filter, "SFAD_Barrels", sv);
            }

            // Field SFAD_LT
            if (filter.TryGetValue("x_SFAD_LT", out sv)) {
                RestoreFieldFilter(SFAD_LT, filter, "SFAD_LT", sv);
            }

            // Field SFAD_MT
            if (filter.TryGetValue("x_SFAD_MT", out sv)) {
                RestoreFieldFilter(SFAD_MT, filter, "SFAD_MT", sv);
            }

            // Field NewBL_KLObs
            if (filter.TryGetValue("x_NewBL_KLObs", out sv)) {
                RestoreFieldFilter(NewBL_KLObs, filter, "NewBL_KLObs", sv);
            }

            // Field NewBL_KL15
            if (filter.TryGetValue("x_NewBL_KL15", out sv)) {
                RestoreFieldFilter(NewBL_KL15, filter, "NewBL_KL15", sv);
            }

            // Field NewBL_Barrels
            if (filter.TryGetValue("x_NewBL_Barrels", out sv)) {
                RestoreFieldFilter(NewBL_Barrels, filter, "NewBL_Barrels", sv);
            }

            // Field NewBL_LT
            if (filter.TryGetValue("x_NewBL_LT", out sv)) {
                RestoreFieldFilter(NewBL_LT, filter, "NewBL_LT", sv);
            }

            // Field NewBL_MT
            if (filter.TryGetValue("x_NewBL_MT", out sv)) {
                RestoreFieldFilter(NewBL_MT, filter, "NewBL_MT", sv);
            }
            ...
            // Field AR45B_klobs
            if (filter.TryGetValue("x_AR45B_klobs", out sv)) {
                RestoreFieldFilter(AR45B_klobs, filter, "AR45B_klobs", sv);
            }

            // Field AR45B_kl15
            if (filter.TryGetValue("x_AR45B_kl15", out sv)) {
                RestoreFieldFilter(AR45B_kl15, filter, "AR45B_kl15", sv);
            }

            // Field AR45B_Barrel
            if (filter.TryGetValue("x_AR45B_Barrel", out sv)) {
                RestoreFieldFilter(AR45B_Barrel, filter, "AR45B_Barrel", sv);
            }

            // Field AR45B_lt
            if (filter.TryGetValue("x_AR45B_lt", out sv)) {
                RestoreFieldFilter(AR45B_lt, filter, "AR45B_lt", sv);
            }

            // Field AR45B_mt
            if (filter.TryGetValue("x_AR45B_mt", out sv)) {
                RestoreFieldFilter(AR45B_mt, filter, "AR45B_mt", sv);
            }
        }
