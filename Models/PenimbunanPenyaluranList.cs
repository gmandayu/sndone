...
public partial class SnDOne {
    ...
    public class PenimbunanPenyaluranListBase : PenimbunanPenyaluran
    {
        ...
        // Constructor
        public PenimbunanPenyaluranListBase()
        {
        ...
        // Constructor
        public PenimbunanPenyaluranListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected bool RestoreFilterList() {
            ...
            // Field IdModa
            if (filter?.TryGetValue("x_IdModa", out sv) ?? false) {
                IdModa.AdvancedSearch.SearchValue = sv;
                IdModa.AdvancedSearch.SearchOperator = filter["z_IdModa"];
                IdModa.AdvancedSearch.SearchCondition = filter["v_IdModa"];
                IdModa.AdvancedSearch.SearchValue2 = filter["y_IdModa"];
                IdModa.AdvancedSearch.SearchOperator2 = filter["w_IdModa"];
                IdModa.AdvancedSearch.Save();
            }

            // Field LookupNoPenyaluran
            if (filter?.TryGetValue("x_LookupNoPenyaluran", out sv) ?? false) {
                LookupNoPenyaluran.AdvancedSearch.SearchValue = sv;
                LookupNoPenyaluran.AdvancedSearch.SearchOperator = filter["z_LookupNoPenyaluran"];
                LookupNoPenyaluran.AdvancedSearch.SearchCondition = filter["v_LookupNoPenyaluran"];
                LookupNoPenyaluran.AdvancedSearch.SearchValue2 = filter["y_LookupNoPenyaluran"];
                LookupNoPenyaluran.AdvancedSearch.SearchOperator2 = filter["w_LookupNoPenyaluran"];
                LookupNoPenyaluran.AdvancedSearch.Save();
            }
        ...
        public void ShowFilterList()
        {
            ...
            // Field IdTangki
            filter = QueryBuilderWhere("IdTangki");
            if (Empty(filter))
                BuildSearchSql(ref filter, IdTangki, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IdTangki.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field NoPenyaluran
            filter = QueryBuilderWhere("NoPenyaluran");
            if (Empty(filter))
                BuildSearchSql(ref filter, NoPenyaluran, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NoPenyaluran.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected void LoadSearchValues() {
            ...
            // IdTangki
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdTangki[]"))
            ...
            // LookupNoPenyaluran
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_LookupNoPenyaluran"))
                    LookupNoPenyaluran.AdvancedSearch.SearchValue = Get("x_LookupNoPenyaluran", Config.FilterOptionSeparator);
                else
            ...
            if (Query.ContainsKey("z_LookupNoPenyaluran"))
                LookupNoPenyaluran.AdvancedSearch.SearchOperator = Get("z_LookupNoPenyaluran", Config.FilterOptionSeparator);

            // NoPenyaluran
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NoPenyaluran[]"))
