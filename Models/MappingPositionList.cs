...
public partial class SnDOne {
    ...
    /// mappingPositionList
    /// </summary>
    public static MappingPositionList mappingPositionList
    {
        get => HttpData.Get<MappingPositionList>("mappingPositionList")!;
        set => HttpData["mappingPositionList"] = value;
    }

    /// <summary>
    /// Page class for MappingPosition
    /// </summary>
    public class MappingPositionList : MappingPositionListBase
    {
        // Constructor
        public MappingPositionList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MappingPositionList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class MappingPositionListBase : MappingPosition
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mappingPositionList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMappingPositionlist";
        ...
        // Constructor
        public MappingPositionListBase(Controller? controller)
        {
            TableName = "MappingPosition";

            // CSS class name as context
            TableVar = "MappingPosition";
        ...
        if (controller != null)
            ...
            // Table object (mappingPosition)
            if (mappingPosition == null || mappingPosition is MappingPosition)
                mappingPosition = this;

            // Initialize URLs
            AddUrl = "MappingPositionAdd";
            ...
            MultiDeleteUrl = "MappingPositionDelete";
            MultiUpdateUrl = "MappingPositionUpdate";
        ...
        // Page name
        public string PageName => "MappingPositionList";
        ...
        public void SetVisibility()
        {
            ...
            NamaRegion.Visible = false;
            IdPlant.SetVisibility();
            NamaPlant.Visible = false;
        }

        // Constructor
        public MappingPositionListBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                var result = new Dictionary<string, string> { { "url", GetUrl(url) }, { "modal", "1" } }; // modal=1
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "MappingPositionView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
...
private StringValues GetKeys(Dictionary<string, string>? dict)
{
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> PageRunBeginAsync()
        {
            ...
            await SetupLookupOptions(IdRegion);
        ...
        private void PageRunEnd()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mappingPositionList?.PageRender();
            }
        }
        #pragma warning restore 219
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, NamaRegion, def, true); // NamaRegion
            BuildSearchSql(ref where, IdPlant, def, true); // IdPlant
            BuildSearchSql(ref where, NamaPlant, def, true); // NamaPlant
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(NamaRegion);
            searchFlds.Add(IdPlant);
            searchFlds.Add(NamaPlant);
        ...
        protected bool CheckSearchParms() {
            ...
            if (IdRegion.AdvancedSearch.IssetSession)
                return true;
            if (NamaRegion.AdvancedSearch.IssetSession)
                return true;
            if (IdPlant.AdvancedSearch.IssetSession)
                return true;
            if (NamaPlant.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            NamaRegion.AdvancedSearch.UnsetSession();
            IdPlant.AdvancedSearch.UnsetSession();
            NamaPlant.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            NamaRegion.AdvancedSearch.Load();
            IdPlant.AdvancedSearch.Load();
            NamaPlant.AdvancedSearch.Load();
        }
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMappingPositionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMappingPositionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MappingPosition"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMappingPositionsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMappingPositionsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMappingPositionlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_MappingPosition");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mappingPositionList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mappingPositionList.RowCount) + "_MappingPosition");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mappingPositionList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mappingPositionList.RowType == RowType.Add || IsEdit && mappingPositionList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void LoadFieldSearchValues()
        {
            ...
            LoadFieldNamaRegionSearchValues();
            LoadFieldIdPlantSearchValues();
            LoadFieldNamaPlantSearchValues();
        }
        ...
        private void LoadFieldNamaRegionSearchValues()
        {
            // NamaRegion
            LoadPrimarySearchValueNamaRegion();
            LoadSearchOperatorNamaRegion();
        }

        private void LoadPrimarySearchValueNamaRegion()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaRegion[]")) {
                NamaRegion.AdvancedSearch.SearchValue = Get("x_NamaRegion[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NamaRegion.AdvancedSearch.SearchValue = Get("NamaRegion"); // Default Value // DN
            }
            if (!Empty(NamaRegion.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNamaRegion()
        {
            if (Query.ContainsKey("z_NamaRegion"))
                NamaRegion.AdvancedSearch.SearchOperator = Get("z_NamaRegion", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldNamaPlantSearchValues()
        {
            // NamaPlant
            LoadPrimarySearchValueNamaPlant();
            LoadSearchOperatorNamaPlant();
        }

        private void LoadPrimarySearchValueNamaPlant()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_NamaPlant[]")) {
                NamaPlant.AdvancedSearch.SearchValue = Get("x_NamaPlant[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                NamaPlant.AdvancedSearch.SearchValue = Get("NamaPlant"); // Default Value // DN
            }
            if (!Empty(NamaPlant.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNamaPlant()
        {
            if (Query.ContainsKey("z_NamaPlant"))
                NamaPlant.AdvancedSearch.SearchOperator = Get("z_NamaPlant", Config.FilterOptionSeparator);
        }
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            NamaRegion.SetDbValue(row["NamaRegion"]);
            IdPlant.SetDbValue(row["IdPlant"]);
            NamaPlant.SetDbValue(row["NamaPlant"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("NamaRegion", NamaRegion.DefaultValue ?? DbNullValue); // DN
            row.Add("IdPlant", IdPlant.DefaultValue ?? DbNullValue); // DN
            row.Add("NamaPlant", NamaPlant.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // NamaPlant
                    ...
                    // awallookupbung
                    // IdRegion
                    ResolveLookupView(IdRegion, "IdRegion", "number");
                    // akhirlookupbung
                    IdRegion.ViewCustomAttributes = "";

                    // NamaRegion
                    NamaRegion.ViewValue = ConvertToString(NamaRegion.CurrentValue); // DN
                    NamaRegion.ViewCustomAttributes = "";
                    ...
                    // NamaPlant
                    NamaPlant.ViewValue = ConvertToString(NamaPlant.CurrentValue); // DN
                    NamaPlant.ViewCustomAttributes = "";
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMappingPositionsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
