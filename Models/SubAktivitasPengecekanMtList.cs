...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasPengecekanMtList
    /// </summary>
    public static SubAktivitasPengecekanMtList subAktivitasPengecekanMtList
    {
        get => HttpData.Get<SubAktivitasPengecekanMtList>("subAktivitasPengecekanMtList")!;
        set => HttpData["subAktivitasPengecekanMtList"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasPengecekanMT
    /// </summary>
    public class SubAktivitasPengecekanMtList : SubAktivitasPengecekanMtListBase
    {
        // Constructor
        public SubAktivitasPengecekanMtList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasPengecekanMtList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasPengecekanMtListBase : SubAktivitasPengecekanMt
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasPengecekanMtList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubAktivitasPengecekanMTlist";
        ...
        // Constructor
        public SubAktivitasPengecekanMtListBase()
        {
            ...
            // Initialize URLs
            AddUrl = "SubAktivitasPengecekanMtAdd";
            ...
            MultiDeleteUrl = "SubAktivitasPengecekanMtDelete";
            MultiUpdateUrl = "SubAktivitasPengecekanMtUpdate";
        ...
        // Page name
        public string PageName => "SubAktivitasPengecekanMtList";
        ...
        // Constructor
        public SubAktivitasPengecekanMtListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fSubAktivitasPengecekanMTgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                subAktivitasPengecekanMtList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(JamPemeriksaan.AdvancedSearch.ToJson())); // Field JamPemeriksaan
        ...
        protected bool RestoreFilterList() {
            ...
            // Field JamPemeriksaan
            if (filter?.TryGetValue("x_JamPemeriksaan", out sv) ?? false) {
                JamPemeriksaan.AdvancedSearch.SearchValue = sv;
                JamPemeriksaan.AdvancedSearch.SearchOperator = filter["z_JamPemeriksaan"];
                JamPemeriksaan.AdvancedSearch.SearchCondition = filter["v_JamPemeriksaan"];
                JamPemeriksaan.AdvancedSearch.SearchValue2 = filter["y_JamPemeriksaan"];
                JamPemeriksaan.AdvancedSearch.SearchOperator2 = filter["w_JamPemeriksaan"];
                JamPemeriksaan.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(JamPemeriksaan, ctrl); // JamPemeriksaan
                UpdateSort(ExTangki, ctrl); // ExTangki
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""SubAktivitasPengecekanMT"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fSubAktivitasPengecekanMTlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasPengecekanMTlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("JamPemeriksaan")); // DN
                CreateColumnOption(option.Add("ExTangki")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fSubAktivitasPengecekanMTsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fSubAktivitasPengecekanMTsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubAktivitasPengecekanMTlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_SubAktivitasPengecekanMT");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subAktivitasPengecekanMtList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subAktivitasPengecekanMtList.RowCount) + "_SubAktivitasPengecekanMT");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subAktivitasPengecekanMtList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subAktivitasPengecekanMtList.RowType == RowType.Add || IsEdit && subAktivitasPengecekanMtList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fSubAktivitasPengecekanMTsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
