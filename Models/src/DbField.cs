...
public partial class SnDOne {
    ...
    public class DbField
    {
        ...
        public string? UserDefinedTypeName = null; // User defined type name (e.g. PostgreSQL enum)
        ...
        public bool IsEncrypted = false; // Field is encrypted
        ...
        public List<string> SearchOperators = []; // Search operators

        public Lazy<AdvancedSearch> _advancedSearch; // Advanced Search

        public List<AdvancedFilter> AdvancedFilters = []; // Advanced Filters
        ...
        public bool IsEnum = false; // DN

        public bool IsSet = false; // DN

        public List<string>? UserValues = null; // User values // DN
        ...
        // Search expression
        public string SearchExpression => Expression;

        // Search data type
        public DataType SearchDataType => DataType;
            ...
            if (IsMultiSelect) { // Multiple options
                ...
                foreach (string wrk in arwrk) {
                    if (Lookup.Options.TryGetValue(wrk.Trim(), out Dictionary<string, object>? ar)) // Lookup data found in cache
                        ...
                        res.Add(wrk); // Not found, use original value
                }
            } else {
        ...
        public virtual IHtmlContent SelectOptionListHtml(string name = "", bool? multiple = null)
        {
            ...
            string?[] curValues = isMultiple && !Empty(curValue)
                ? ConvertToString(curValue).Split(UseFilter ? Config.FilterOptionSeparator : Config.MultipleOptionSeparator)
                : new string?[] {};
        ...
        // Get View value
        public string GetViewValue() => ViewValue is IHtmlValue v ? (v.ToString() ?? "") : ConvertToString(ViewValue);
                ...
                if (!Empty(wrkfile)) {
                    if (!UploadMultiple) {
                        ...
                        if (FileExists(imagefn)) {
                            byte[] wrkdata = await FileReadAllBytesAsync(imagefn); // DN
                            if (ImageResize) {
                                int wrkwidth = ImageWidth;
                                int wrkheight = ImageHeight;
                                ResizeBinary(ref wrkdata, ref wrkwidth, ref wrkheight);
        ...
        public virtual object DbNotNullValue =>
            DataType switch {
                DataType.Number => 0,
        ...
        public void SetDbValue(Dictionary<string, object> dict, object? value, bool skip = false)
        {
            ...
            if (value is not SqlMapper.ICustomQueryParameter) {
                value = Type switch {
                    ...
                    7 or 133 or 135 => IsDate(value) ? ConvertToDateTime(value) : def, // Date
                    134 or 145 => value is DateTimeOffset // PostgreSQL timetz
                        ? value
                        : TimeSpan.TryParse(value?.ToString(), out TimeSpan ts) ? ts : def, // Time
                    146 => DateTimeOffset.TryParse(value?.ToString(), out DateTimeOffset dt) ? dt : def, // DateTimeOffset
                    201 or 203 or 129 or 130 or 200 or 202 => ConvertToString(value) is var s && !Empty(s) ? (IsEncrypted ? AesEncrypt(s) : value) : def, // String
        ...
        /// </summary>
        /// <param name="value">Date time format</param>
        public void SetDateTimeFormat(int value) => DateTimeFormat = value;
