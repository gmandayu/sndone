...
public partial class SnDOne {
    ...
    public class SubAktivitasLogbookMonitoringStockPipaEditBase : SubAktivitasLogbookMonitoringStockPipa
    {
        ...
        // Constructor
        public SubAktivitasLogbookMonitoringStockPipaEditBase()
        {
        ...
        // Constructor
        public SubAktivitasLogbookMonitoringStockPipaEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("SubAktivitasLogbookMonitoringStockPipaList"); // No matching record, return to list
                        }
                    break;
                    ...
                    if (res) { // Update record based on key
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "SubAktivitasLogbookMonitoringStockPipaList") {
                                ...
                                returnUrl = "SubAktivitasLogbookMonitoringStockPipaList"; // Return list page content
                            }
                        }
                        if (IsJsonResponse()) {
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'Volume_KL' before field var 'x_Volume_KL'
            val = CurrentForm.HasValue("Volume_KL") ? CurrentForm.GetValue("Volume_KL") : CurrentForm.GetValue("x_Volume_KL");
            if (!Volume_KL.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Volume_KL") && !CurrentForm.HasValue("x_Volume_KL")) // DN
                    Volume_KL.Visible = false; // Disable update for API request
                else
                    Volume_KL.SetFormValue(val);
            }
            ...
            bool validateForm = true;
                ...
                if (Volume_KL.Visible && Volume_KL.Required) {
                    if (!Volume_KL.IsDetailKey && Empty(Volume_KL.FormValue)) {
                        Volume_KL.AddErrorMessage(ConvertToString(Volume_KL.RequiredErrorMessage).Replace("%s", Volume_KL.Caption));
                    }
                }
