...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasHasilPemeriksaanPenKonEdit
    /// </summary>
    public static SubAktivitasHasilPemeriksaanPenKonEdit subAktivitasHasilPemeriksaanPenKonEdit
    {
        get => HttpData.Get<SubAktivitasHasilPemeriksaanPenKonEdit>("subAktivitasHasilPemeriksaanPenKonEdit")!;
        set => HttpData["subAktivitasHasilPemeriksaanPenKonEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasHasilPemeriksaanPenKon
    /// </summary>
    public class SubAktivitasHasilPemeriksaanPenKonEdit : SubAktivitasHasilPemeriksaanPenKonEditBase
    {
        // Constructor
        public SubAktivitasHasilPemeriksaanPenKonEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasHasilPemeriksaanPenKonEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasHasilPemeriksaanPenKonEditBase : SubAktivitasHasilPemeriksaanPenKon
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasHasilPemeriksaanPenKonEdit";
        ...
        // Constructor
        public SubAktivitasHasilPemeriksaanPenKonEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasHasilPemeriksaanPenKonEdit";
        ...
        // Constructor
        public SubAktivitasHasilPemeriksaanPenKonEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasHasilPemeriksaanPenKonList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasHasilPemeriksaanPenKonList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasHasilPemeriksaanPenKonList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasHasilPemeriksaanPenKonEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomNamaKapal() {
            if (NamaKapal.Visible && NamaKapal.Required) {
                if (!NamaKapal.IsDetailKey && Empty(NamaKapal.FormValue)) {
                    NamaKapal.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTanggal() {
            if (Tanggal.Visible && Tanggal.Required) {
                if (!Tanggal.IsDetailKey && Empty(Tanggal.FormValue)) {
                    Tanggal.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNamaKegiatan() {
            if (NamaKegiatan.Visible && NamaKegiatan.Required) {
                if (!NamaKegiatan.IsDetailKey && Empty(NamaKegiatan.FormValue)) {
                    NamaKegiatan.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomDensity() {
            if (Density.Visible && Density.Required) {
                if (!Density.IsDetailKey && Empty(Density.FormValue)) {
                    Density.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTemperature() {
            if (Temperature.Visible && Temperature.Required) {
                if (!Temperature.IsDetailKey && Empty(Temperature.FormValue)) {
                    Temperature.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        private void ValidateCustomPressure() {
            if (Pressure.Visible && Pressure.Required) {
                if (!Pressure.IsDetailKey && Empty(Pressure.FormValue)) {
                    Pressure.AddErrorMessage(ConvertToString(Keterangan.RequiredErrorMessage).Replace("%s", Keterangan.Caption));
                }
            }
        }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // NamaKapal
            NamaKapal.SetDbValue(rsnew, NamaKapal.CurrentValue, NamaKapal.ReadOnly);
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasHasilPemeriksaanPenKonList")), "", TableVar, true);
            string pageId = "edit";
