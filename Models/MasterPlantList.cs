...
public partial class SnDOne {
    ...
    /// masterPlantList
    /// </summary>
    public static MasterPlantList masterPlantList
    {
        get => HttpData.Get<MasterPlantList>("masterPlantList")!;
        set => HttpData["masterPlantList"] = value;
    }

    /// <summary>
    /// Page class for MasterPlant
    /// </summary>
    public class MasterPlantList : MasterPlantListBase
    {
        // Constructor
        public MasterPlantList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterPlantList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterPlantListBase : MasterPlant
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterPlantList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMasterPlantlist";
        ...
        // Constructor
        public MasterPlantListBase(Controller? controller)
        {
        ...
        if (controller != null)
            ...
            MultiDeleteUrl = "MasterPlantDelete";
            MultiUpdateUrl = "MasterPlantUpdate";
        ...
        // Page name
        public string PageName => "MasterPlantList";
        ...
        // Constructor
        public MasterPlantListBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, Terminal_Manager, def, true); // Terminal_Manager
            BuildSearchSql(ref where, PRL, def, true); // PRL
            BuildSearchSql(ref where, Status, def, true); // Status
            BuildSearchSql(ref where, Terminal_Induk, def, true); // Terminal_Induk
            BuildSearchSql(ref where, Nama_Terminal, def, true); // Nama_Terminal
            BuildSearchSql(ref where, Region, def, true); // Region
            BuildSearchSql(ref where, Fungsi, def, true); // Fungsi
            BuildSearchSql(ref where, Cost_Center, def, true); // Cost_Center
            BuildSearchSql(ref where, Jenis, def, true); // Jenis
            BuildSearchSql(ref where, Plant, def, true); // Plant
            BuildSearchSql(ref where, UTC, def, true); // UTC
            BuildSearchSql(ref where, TipeProduk, def, true); // TipeProduk
        ...
        {
            string filterList = "";
            filterList += ProcessFieldFilter("Terminal_Manager", Terminal_Manager, true, config);
            filterList += ProcessFieldFilter("PRL", PRL, true, config);
            filterList += ProcessFieldFilter("Status", Status, true, config);
            filterList += ProcessFieldFilter("Terminal_Induk", Terminal_Induk, true, config);
            filterList += ProcessFieldFilter("Nama_Terminal", Nama_Terminal, true, config);
            filterList += ProcessFieldFilter("Region", Region, true, config);
            filterList += ProcessFieldFilter("Fungsi", Fungsi, true, config);
            filterList += ProcessFieldFilter("Cost_Center", Cost_Center, true, config);
            filterList += ProcessFieldFilter("Jenis", Jenis, true, config);
            filterList += ProcessFieldFilter("Plant", Plant, true, config);
            filterList += ProcessFieldFilter("UTC", UTC, true, config);
            filterList += ProcessFieldFilter("TipeProduk", TipeProduk, true, config);
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Terminal_Manager);
            searchFlds.Add(PRL);
            searchFlds.Add(Status);
            searchFlds.Add(Terminal_Induk);
            searchFlds.Add(Nama_Terminal);
            searchFlds.Add(Region);
            searchFlds.Add(Fungsi);
            searchFlds.Add(Cost_Center);
            searchFlds.Add(Jenis);
            searchFlds.Add(Plant);
            searchFlds.Add(UTC);
            searchFlds.Add(TipeProduk);
            searchFlds.Add(JenisPlant);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (Terminal_Manager.AdvancedSearch.IssetSession)
                return true;
            if (PRL.AdvancedSearch.IssetSession)
                return true;
            if (Status.AdvancedSearch.IssetSession)
                return true;
            if (Terminal_Induk.AdvancedSearch.IssetSession)
                return true;
            if (Nama_Terminal.AdvancedSearch.IssetSession)
                return true;
            if (Region.AdvancedSearch.IssetSession)
                return true;
            if (Fungsi.AdvancedSearch.IssetSession)
                return true;
            if (Cost_Center.AdvancedSearch.IssetSession)
                return true;
            if (Jenis.AdvancedSearch.IssetSession)
                return true;
            if (Plant.AdvancedSearch.IssetSession)
                return true;
            if (UTC.AdvancedSearch.IssetSession)
                return true;
            if (TipeProduk.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            Terminal_Manager.AdvancedSearch.UnsetSession();
            PRL.AdvancedSearch.UnsetSession();
            Status.AdvancedSearch.UnsetSession();
            Terminal_Induk.AdvancedSearch.UnsetSession();
            Nama_Terminal.AdvancedSearch.UnsetSession();
            Region.AdvancedSearch.UnsetSession();
            Fungsi.AdvancedSearch.UnsetSession();
            Cost_Center.AdvancedSearch.UnsetSession();
            Jenis.AdvancedSearch.UnsetSession();
            Plant.AdvancedSearch.UnsetSession();
            UTC.AdvancedSearch.UnsetSession();
            TipeProduk.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            Terminal_Manager.AdvancedSearch.Load();
            PRL.AdvancedSearch.Load();
            Status.AdvancedSearch.Load();
            Terminal_Induk.AdvancedSearch.Load();
            Nama_Terminal.AdvancedSearch.Load();
            Region.AdvancedSearch.Load();
            Fungsi.AdvancedSearch.Load();
            Cost_Center.AdvancedSearch.Load();
            Jenis.AdvancedSearch.Load();
            Plant.AdvancedSearch.Load();
            UTC.AdvancedSearch.Load();
            TipeProduk.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Terminal_Manager, ctrl); // Terminal_Manager
                UpdateSort(PRL, ctrl); // PRL
                UpdateSort(Status, ctrl); // Status
                UpdateSort(Terminal_Induk, ctrl); // Terminal_Induk
                UpdateSort(Nama_Terminal, ctrl); // Nama_Terminal
                UpdateSort(Region, ctrl); // Region
                UpdateSort(Fungsi, ctrl); // Fungsi
                UpdateSort(Cost_Center, ctrl); // Cost_Center
                UpdateSort(Jenis, ctrl); // Jenis
                UpdateSort(Plant, ctrl); // Plant
                UpdateSort(UTC, ctrl); // UTC
                UpdateSort(TipeProduk, ctrl); // TipeProduk
                UpdateSort(JenisPlant, ctrl); // JenisPlant
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MasterPlant"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MasterPlant"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MasterPlant"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fMasterPlantlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMasterPlantlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(IdPlant.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MasterPlant"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fMasterPlantlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Terminal_Manager")); // DN
                CreateColumnOption(option.Add("PRL")); // DN
                CreateColumnOption(option.Add("Status")); // DN
                CreateColumnOption(option.Add("Terminal_Induk")); // DN
                CreateColumnOption(option.Add("Nama_Terminal")); // DN
                CreateColumnOption(option.Add("Region")); // DN
                CreateColumnOption(option.Add("Fungsi")); // DN
                CreateColumnOption(option.Add("Cost_Center")); // DN
                CreateColumnOption(option.Add("Jenis")); // DN
                CreateColumnOption(option.Add("Plant")); // DN
                CreateColumnOption(option.Add("UTC")); // DN
                CreateColumnOption(option.Add("TipeProduk")); // DN
                CreateColumnOption(option.Add("JenisPlant")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMasterPlantsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMasterPlantsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMasterPlantlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        private async Task<bool> HandleTemplateRow()
        {
            if ((IsGridAdd || IsGridEdit) && SameString(RowIndex, "$rowindex$")) { // Render template row first
                ...
                RowAttrs.Add("id", "r0_MasterPlant");
        ...
        private void SetupRowCountsAndAttributes()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(masterPlantList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(masterPlantList.RowCount) + "_MasterPlant");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(masterPlantList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && masterPlantList.RowType == RowType.Add || IsEdit && masterPlantList.RowType == RowType.Edit) // Inline-Add/Edit row
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            LoadFieldTerminal_ManagerSearchValues();
            LoadFieldPRLSearchValues();
            LoadFieldStatusSearchValues();
            LoadFieldTerminal_IndukSearchValues();
            LoadFieldNama_TerminalSearchValues();
            LoadFieldRegionSearchValues();
            LoadFieldFungsiSearchValues();
            LoadFieldCost_CenterSearchValues();
            LoadFieldJenisSearchValues();
            LoadFieldPlantSearchValues();
            LoadFieldUTCSearchValues();
            LoadFieldTipeProdukSearchValues();
        ...
        private void LoadFieldTerminal_ManagerSearchValues()
        {
            // Terminal_Manager
            LoadPrimarySearchValueTerminal_Manager();
            LoadSearchOperatorTerminal_Manager();
        }

        private void LoadPrimarySearchValueTerminal_Manager()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Terminal_Manager[]")) {
                Terminal_Manager.AdvancedSearch.SearchValue = Get("x_Terminal_Manager[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Terminal_Manager.AdvancedSearch.SearchValue = Get("Terminal_Manager"); // Default Value // DN
            }
            if (!Empty(Terminal_Manager.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTerminal_Manager()
        {
            if (Query.ContainsKey("z_Terminal_Manager"))
                Terminal_Manager.AdvancedSearch.SearchOperator = Get("z_Terminal_Manager", Config.FilterOptionSeparator);
        }

        private void LoadFieldPRLSearchValues()
        {
            // PRL
            LoadPrimarySearchValuePRL();
            LoadSearchOperatorPRL();
        }

        private void LoadPrimarySearchValuePRL()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_PRL[]")) {
                PRL.AdvancedSearch.SearchValue = Get("x_PRL[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                PRL.AdvancedSearch.SearchValue = Get("PRL"); // Default Value // DN
            }
            if (!Empty(PRL.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorPRL()
        {
            if (Query.ContainsKey("z_PRL"))
                PRL.AdvancedSearch.SearchOperator = Get("z_PRL", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldTerminal_IndukSearchValues()
        {
            // Terminal_Induk
            LoadPrimarySearchValueTerminal_Induk();
            LoadSearchOperatorTerminal_Induk();
        }

        private void LoadPrimarySearchValueTerminal_Induk()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Terminal_Induk[]")) {
                Terminal_Induk.AdvancedSearch.SearchValue = Get("x_Terminal_Induk[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Terminal_Induk.AdvancedSearch.SearchValue = Get("Terminal_Induk"); // Default Value // DN
            }
            if (!Empty(Terminal_Induk.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTerminal_Induk()
        {
            if (Query.ContainsKey("z_Terminal_Induk"))
                Terminal_Induk.AdvancedSearch.SearchOperator = Get("z_Terminal_Induk", Config.FilterOptionSeparator);
        }

        private void LoadFieldNama_TerminalSearchValues()
        {
            // Nama_Terminal
            LoadPrimarySearchValueNama_Terminal();
            LoadSearchOperatorNama_Terminal();
        }

        private void LoadPrimarySearchValueNama_Terminal()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Nama_Terminal[]")) {
                Nama_Terminal.AdvancedSearch.SearchValue = Get("x_Nama_Terminal[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Nama_Terminal.AdvancedSearch.SearchValue = Get("Nama_Terminal"); // Default Value // DN
            }
            if (!Empty(Nama_Terminal.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorNama_Terminal()
        {
            if (Query.ContainsKey("z_Nama_Terminal"))
                Nama_Terminal.AdvancedSearch.SearchOperator = Get("z_Nama_Terminal", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldCost_CenterSearchValues()
        {
            // Cost_Center
            LoadPrimarySearchValueCost_Center();
            LoadSearchOperatorCost_Center();
        }

        private void LoadPrimarySearchValueCost_Center()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Cost_Center[]")) {
                Cost_Center.AdvancedSearch.SearchValue = Get("x_Cost_Center[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Cost_Center.AdvancedSearch.SearchValue = Get("Cost_Center"); // Default Value // DN
            }
            if (!Empty(Cost_Center.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorCost_Center()
        {
            if (Query.ContainsKey("z_Cost_Center"))
                Cost_Center.AdvancedSearch.SearchOperator = Get("z_Cost_Center", Config.FilterOptionSeparator);
        }

        private void LoadFieldJenisSearchValues()
        {
            // Jenis
            LoadPrimarySearchValueJenis();
            LoadSearchOperatorJenis();
        }

        private void LoadPrimarySearchValueJenis()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_Jenis[]")) {
                Jenis.AdvancedSearch.SearchValue = Get("x_Jenis[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                Jenis.AdvancedSearch.SearchValue = Get("Jenis"); // Default Value // DN
            }
            if (!Empty(Jenis.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorJenis()
        {
            if (Query.ContainsKey("z_Jenis"))
                Jenis.AdvancedSearch.SearchOperator = Get("z_Jenis", Config.FilterOptionSeparator);
        }
        ...
        private void LoadFieldUTCSearchValues()
        {
            // UTC
            LoadPrimarySearchValueUTC();
            LoadSearchOperatorUTC();
        }

        private void LoadPrimarySearchValueUTC()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_UTC[]")) {
                UTC.AdvancedSearch.SearchValue = Get("x_UTC[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                UTC.AdvancedSearch.SearchValue = Get("UTC"); // Default Value // DN
            }
            if (!Empty(UTC.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorUTC()
        {
            if (Query.ContainsKey("z_UTC"))
                UTC.AdvancedSearch.SearchOperator = Get("z_UTC", Config.FilterOptionSeparator);
        }

        private void LoadFieldTipeProdukSearchValues()
        {
            // TipeProduk
            LoadPrimarySearchValueTipeProduk();
            LoadSearchOperatorTipeProduk();
        }

        private void LoadPrimarySearchValueTipeProduk()
        {
            if (!IsAddOrEdit && Query.ContainsKey("x_TipeProduk[]")) {
                TipeProduk.AdvancedSearch.SearchValue = Get("x_TipeProduk[]", Config.FilterOptionSeparator);
            }
            else if (!IsAddOrEdit) {
                TipeProduk.AdvancedSearch.SearchValue = Get("TipeProduk"); // Default Value // DN
            }
            if (!Empty(TipeProduk.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        }

        private void LoadSearchOperatorTipeProduk()
        {
            if (Query.ContainsKey("z_TipeProduk"))
                TipeProduk.AdvancedSearch.SearchOperator = Get("z_TipeProduk", Config.FilterOptionSeparator);
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // Terminal_Manager
                if (Terminal_Manager.UseFilter && !Empty(Terminal_Manager.AdvancedSearch.SearchValue)) {
                    Terminal_Manager.EditValue = ConvertToString(Terminal_Manager.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // PRL
                if (PRL.UseFilter && !Empty(PRL.AdvancedSearch.SearchValue)) {
                    PRL.EditValue = ConvertToString(PRL.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // Terminal_Induk
                if (Terminal_Induk.UseFilter && !Empty(Terminal_Induk.AdvancedSearch.SearchValue)) {
                    Terminal_Induk.EditValue = ConvertToString(Terminal_Induk.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // Nama_Terminal
                if (Nama_Terminal.UseFilter && !Empty(Nama_Terminal.AdvancedSearch.SearchValue)) {
                    Nama_Terminal.EditValue = ConvertToString(Nama_Terminal.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // Cost_Center
                if (Cost_Center.UseFilter && !Empty(Cost_Center.AdvancedSearch.SearchValue)) {
                    Cost_Center.EditValue = ConvertToString(Cost_Center.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // Jenis
                if (Jenis.UseFilter && !Empty(Jenis.AdvancedSearch.SearchValue)) {
                    Jenis.EditValue = ConvertToString(Jenis.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
                ...
                // UTC
                if (UTC.UseFilter && !Empty(UTC.AdvancedSearch.SearchValue)) {
                    UTC.EditValue = ConvertToString(UTC.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }

                // TipeProduk
                if (TipeProduk.UseFilter && !Empty(TipeProduk.AdvancedSearch.SearchValue)) {
                    TipeProduk.EditValue = ConvertToString(TipeProduk.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
...
protected void SetupExportOptions() {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMasterPlantsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
