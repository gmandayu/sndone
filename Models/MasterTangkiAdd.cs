...
public partial class SnDOne {
    ...
    /// masterTangkiAdd
    /// </summary>
    public static MasterTangkiAdd masterTangkiAdd
    {
        get => HttpData.Get<MasterTangkiAdd>("masterTangkiAdd")!;
        set => HttpData["masterTangkiAdd"] = value;
    }

    /// <summary>
    /// Page class for MasterTangki
    /// </summary>
    public class MasterTangkiAdd : MasterTangkiAddBase
    {
        // Constructor
        public MasterTangkiAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterTangkiAdd() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class MasterTangkiAddBase : MasterTangki
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterTangkiAdd";
        ...
        // Constructor
        public MasterTangkiAddBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "MasterTangkiAdd";
        ...
        // Constructor
        public MasterTangkiAddBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private IActionResult? HandleCopyAction(Dictionary<string, object>? rsold)
        {
            if (rsold == null) {
                ...
                return Terminate("MasterTangkiList");
            }
            return null;
        }
        ...
        private string DetermineReturnUrl()
        {
            ...
            else if (GetPageName(returnUrl) == "MasterTangkiView")
                returnUrl = ViewUrl;
            return returnUrl;
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'NamaTerminal'
            NamaTerminal.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(NamaTerminal, "NamaTerminal", "x_NamaTerminal");
            ...
            // Standard handling for 'Sloc'
            Sloc.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Sloc, "Sloc", "x_Sloc");

            // Standard handling for 'SeqTangki'
            SeqTangki.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(SeqTangki, "SeqTangki", "x_SeqTangki");

            // Standard handling for 'MatNo'
            MatNo.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(MatNo, "MatNo", "x_MatNo");

            // Standard handling for 'TglKalibrasi'
            TglKalibrasi.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(TglKalibrasi, "TglKalibrasi", "x_TglKalibrasi", true, validate, true);

            // Standard handling for 'ExpDate'
            ExpDate.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(ExpDate, "ExpDate", "x_ExpDate", true, validate, true);
            ...
            // Standard handling for 'TinggiMejaUk'
            TinggiMejaUk.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(TinggiMejaUk, "TinggiMejaUk", "x_TinggiMejaUk");

            // Standard handling for 'MaxDipping'
            MaxDipping.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(MaxDipping, "MaxDipping", "x_MaxDipping");

            // Standard handling for 'MaxCapacity'
            MaxCapacity.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(MaxCapacity, "MaxCapacity", "x_MaxCapacity");

            // Standard handling for 'UnpumpableQty'
            UnpumpableQty.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(UnpumpableQty, "UnpumpableQty", "x_UnpumpableQty");

            // Standard handling for 'PumpableQty'
            PumpableQty.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(PumpableQty, "PumpableQty", "x_PumpableQty");
        }
        #pragma warning restore 1998
        ...
        private void ValidateCustomNamaTerminal() {
            if (NamaTerminal.Visible && NamaTerminal.Required) {
                if (!NamaTerminal.IsDetailKey && Empty(NamaTerminal.FormValue)) {
                    NamaTerminal.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }
        ...
        private void ValidateCustomPlant() {
            if (Plant.Visible && Plant.Required) {
                if (!Plant.IsDetailKey && Empty(Plant.FormValue)) {
                    Plant.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomSloc() {
            if (Sloc.Visible && Sloc.Required) {
                if (!Sloc.IsDetailKey && Empty(Sloc.FormValue)) {
                    Sloc.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomSeqTangki() {
            if (SeqTangki.Visible && SeqTangki.Required) {
                if (!SeqTangki.IsDetailKey && Empty(SeqTangki.FormValue)) {
                    SeqTangki.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomMatNo() {
            if (MatNo.Visible && MatNo.Required) {
                if (!MatNo.IsDetailKey && Empty(MatNo.FormValue)) {
                    MatNo.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomTglKalibrasi() {
            if (TglKalibrasi.Visible && TglKalibrasi.Required) {
                if (!TglKalibrasi.IsDetailKey && Empty(TglKalibrasi.FormValue)) {
                    TglKalibrasi.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomExpDate() {
            if (ExpDate.Visible && ExpDate.Required) {
                if (!ExpDate.IsDetailKey && Empty(ExpDate.FormValue)) {
                    ExpDate.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatus() {
            if (Status.Visible && Status.Required) {
                if (!Status.IsDetailKey && Empty(Status.FormValue)) {
                    Status.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomTinggiMejaUk() {
            if (TinggiMejaUk.Visible && TinggiMejaUk.Required) {
                if (!TinggiMejaUk.IsDetailKey && Empty(TinggiMejaUk.FormValue)) {
                    TinggiMejaUk.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomMaxDipping() {
            if (MaxDipping.Visible && MaxDipping.Required) {
                if (!MaxDipping.IsDetailKey && Empty(MaxDipping.FormValue)) {
                    MaxDipping.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomMaxCapacity() {
            if (MaxCapacity.Visible && MaxCapacity.Required) {
                if (!MaxCapacity.IsDetailKey && Empty(MaxCapacity.FormValue)) {
                    MaxCapacity.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomUnpumpableQty() {
            if (UnpumpableQty.Visible && UnpumpableQty.Required) {
                if (!UnpumpableQty.IsDetailKey && Empty(UnpumpableQty.FormValue)) {
                    UnpumpableQty.AddErrorMessage(ConvertToString(PumpableQty.RequiredErrorMessage).Replace("%s", PumpableQty.Caption));
                }
            }
        }

        private void ValidateCustomPumpableQty() {
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ValidateCustomNamaTerminal();
                ValidateCustomPlant();
                ValidateCustomSloc();
                ValidateCustomSeqTangki();
                ValidateCustomMatNo();
                ValidateCustomTglKalibrasi();
                ...
                ValidateCustomTinggiMejaUk();
                ValidateCustomMaxDipping();
                ValidateCustomMaxCapacity();
                ValidateCustomUnpumpableQty();
                ValidateCustomPumpableQty();
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // NamaTerminal
                NamaTerminal.SetDbValue(rsnew, NamaTerminal.CurrentValue);

                // Plant
                Plant.SetDbValue(rsnew, Plant.CurrentValue);

                // Sloc
                Sloc.SetDbValue(rsnew, Sloc.CurrentValue);

                // SeqTangki
                SeqTangki.SetDbValue(rsnew, SeqTangki.CurrentValue);

                // MatNo
                MatNo.SetDbValue(rsnew, MatNo.CurrentValue);

                // TglKalibrasi
                TglKalibrasi.SetDbValue(rsnew, ConvertToDateTime(TglKalibrasi.CurrentValue, TglKalibrasi.FormatPattern));

                // ExpDate
                ExpDate.SetDbValue(rsnew, ConvertToDateTime(ExpDate.CurrentValue, ExpDate.FormatPattern));

                // Status
                Status.SetDbValue(rsnew, Status.CurrentValue);

                // TinggiMejaUk
                TinggiMejaUk.SetDbValue(rsnew, TinggiMejaUk.CurrentValue);

                // MaxDipping
                MaxDipping.SetDbValue(rsnew, MaxDipping.CurrentValue);

                // MaxCapacity
                MaxCapacity.SetDbValue(rsnew, MaxCapacity.CurrentValue);

                // UnpumpableQty
                UnpumpableQty.SetDbValue(rsnew, UnpumpableQty.CurrentValue);

                // PumpableQty
                PumpableQty.SetDbValue(rsnew, PumpableQty.CurrentValue);
                return rsnew;
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("TinggiMejaUk", out value)) // TinggiMejaUk
                TinggiMejaUk.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("MaxDipping", out value)) // MaxDipping
                MaxDipping.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("MaxCapacity", out value)) // MaxCapacity
                MaxCapacity.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("UnpumpableQty", out value)) // UnpumpableQty
                UnpumpableQty.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("PumpableQty", out value)) // PumpableQty
                PumpableQty.SetFormValue(ConvertToString(value));
        }
