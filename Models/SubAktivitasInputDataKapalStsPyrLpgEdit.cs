...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasInputDataKapalStsPyrLpgEdit
    /// </summary>
    public static SubAktivitasInputDataKapalStsPyrLpgEdit subAktivitasInputDataKapalStsPyrLpgEdit
    {
        get => HttpData.Get<SubAktivitasInputDataKapalStsPyrLpgEdit>("subAktivitasInputDataKapalStsPyrLpgEdit")!;
        set => HttpData["subAktivitasInputDataKapalStsPyrLpgEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasInputDataKapalSTSPyrLPG
    /// </summary>
    public class SubAktivitasInputDataKapalStsPyrLpgEdit : SubAktivitasInputDataKapalStsPyrLpgEditBase
    {
        // Constructor
        public SubAktivitasInputDataKapalStsPyrLpgEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasInputDataKapalStsPyrLpgEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasInputDataKapalStsPyrLpgEditBase : SubAktivitasInputDataKapalStsPyrLpg
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasInputDataKapalStsPyrLpgEdit";
        ...
        // Constructor
        public SubAktivitasInputDataKapalStsPyrLpgEditBase(Controller? controller)
        {
            TableName = "SubAktivitasInputDataKapalSTSPyrLPG";
        ...
        if (controller != null)
            ...
            // Table object (subAktivitasInputDataKapalStsPyrLpg)
            if (subAktivitasInputDataKapalStsPyrLpg == null || subAktivitasInputDataKapalStsPyrLpg is SubAktivitasInputDataKapalStsPyrLpg)
                subAktivitasInputDataKapalStsPyrLpg = this;
        ...
        // Page name
        public string PageName => "SubAktivitasInputDataKapalStsPyrLpgEdit";
        ...
        // Constructor
        public SubAktivitasInputDataKapalStsPyrLpgEditBase() : this(null) { }
            ...
            string pageName = GetPageName(url);
                ...
                // List / View / Master View page
                    if (!SameString(pageName, ListUrl))
                    {
                        ...
                        result.Add("view", pageName == "SubAktivitasInputDataKapalStsPyrLpgView" ? "1" : "0"); // If View page, no primary button
                    }
                    else
                    {
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasInputDataKapalStsPyrLpgList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasInputDataKapalStsPyrLpgList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasInputDataKapalStsPyrLpgList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private async Task<IActionResult?> PageRunBeginAsync()
        {
            ...
            // If null, the conditions aren't fulfilled
                    ...
                    // Set up lookup cache
                    await SetupLookupOptions(NoReferensi);
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasInputDataKapalStsPyrLpgEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'VoyageNumber'
            VoyageNumber.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(VoyageNumber, "VoyageNumber", "x_VoyageNumber");
        ...
        public void RestoreFormValues()
        {
            ...
            VoyageNumber.CurrentValue = VoyageNumber.FormValue;
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("VoyageNumber", VoyageNumber.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // userInput
                    ...
                    // awallookupbung
                    // NoReferensi (jaga leading zero)
                    ResolveLookupView(NoReferensi, "IdProses", "string");
                    // akhirlookupbung
                ...
                VoyageNumber.EditValue = HtmlEncode(VoyageNumber.CurrentValue);
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNamaKapal() {
            if (NamaKapal.Visible && NamaKapal.Required) {
                if (!NamaKapal.IsDetailKey && Empty(NamaKapal.FormValue)) {
                    NamaKapal.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }

        private void ValidateCustomVoyageNumber() {
            if (VoyageNumber.Visible && VoyageNumber.Required) {
                if (!VoyageNumber.IsDetailKey && Empty(VoyageNumber.FormValue)) {
                    VoyageNumber.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomTanggalTiba() {
            if (TanggalTiba.Visible && TanggalTiba.Required) {
                if (!TanggalTiba.IsDetailKey && Empty(TanggalTiba.FormValue)) {
                    TanggalTiba.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNominasi() {
            if (Nominasi.Visible && Nominasi.Required) {
                if (!Nominasi.IsDetailKey && Empty(Nominasi.FormValue)) {
                    Nominasi.AddErrorMessage(ConvertToString(JenisProduk.RequiredErrorMessage).Replace("%s", JenisProduk.Caption));
                }
            }
        }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // VoyageNumber
            VoyageNumber.SetDbValue(rsnew, VoyageNumber.CurrentValue, VoyageNumber.ReadOnly);
        ...
        {
            object? value;
            if (row.TryGetValue("VoyageNumber", out value)) // VoyageNumber
                VoyageNumber.CurrentValue = value;
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasInputDataKapalStsPyrLpgList")), "", TableVar, true);
            string pageId = "edit";
