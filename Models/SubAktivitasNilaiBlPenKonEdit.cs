...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasNilaiBlPenKonEdit
    /// </summary>
    public static SubAktivitasNilaiBlPenKonEdit subAktivitasNilaiBlPenKonEdit
    {
        get => HttpData.Get<SubAktivitasNilaiBlPenKonEdit>("subAktivitasNilaiBlPenKonEdit")!;
        set => HttpData["subAktivitasNilaiBlPenKonEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasNilaiBLPenKon
    /// </summary>
    public class SubAktivitasNilaiBlPenKonEdit : SubAktivitasNilaiBlPenKonEditBase
    {
        // Constructor
        public SubAktivitasNilaiBlPenKonEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasNilaiBlPenKonEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasNilaiBlPenKonEditBase : SubAktivitasNilaiBlPenKon
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasNilaiBlPenKonEdit";
        ...
        // Constructor
        public SubAktivitasNilaiBlPenKonEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasNilaiBlPenKonEdit";
        ...
        // Constructor
        public SubAktivitasNilaiBlPenKonEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasNilaiBlPenKonList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasNilaiBlPenKonList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasNilaiBlPenKonList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasNilaiBlPenKonEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        private void ValidateCustomBL_KLObs() {
            if (BL_KLObs.Visible && BL_KLObs.Required) {
                if (!BL_KLObs.IsDetailKey && Empty(BL_KLObs.FormValue)) {
                    BL_KLObs.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_KL15() {
            if (BL_KL15.Visible && BL_KL15.Required) {
                if (!BL_KL15.IsDetailKey && Empty(BL_KL15.FormValue)) {
                    BL_KL15.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_Barrels() {
            if (BL_Barrels.Visible && BL_Barrels.Required) {
                if (!BL_Barrels.IsDetailKey && Empty(BL_Barrels.FormValue)) {
                    BL_Barrels.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_LT() {
            if (BL_LT.Visible && BL_LT.Required) {
                if (!BL_LT.IsDetailKey && Empty(BL_LT.FormValue)) {
                    BL_LT.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomBL_MT() {
            if (BL_MT.Visible && BL_MT.Required) {
                if (!BL_MT.IsDetailKey && Empty(BL_MT.FormValue)) {
                    BL_MT.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_KLObs() {
            if (SFAL_KLObs.Visible && SFAL_KLObs.Required) {
                if (!SFAL_KLObs.IsDetailKey && Empty(SFAL_KLObs.FormValue)) {
                    SFAL_KLObs.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        private void ValidateCustomSFAL_KL15() {
            if (SFAL_KL15.Visible && SFAL_KL15.Required) {
                if (!SFAL_KL15.IsDetailKey && Empty(SFAL_KL15.FormValue)) {
                    SFAL_KL15.AddErrorMessage(ConvertToString(SFAL_MT.RequiredErrorMessage).Replace("%s", SFAL_MT.Caption));
                }
            }
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasNilaiBlPenKonList")), "", TableVar, true);
            string pageId = "edit";
