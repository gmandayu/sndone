...
public partial class SnDOne {
    /// <summary>
    /// vSampleOperasionalList
    /// </summary>
    public static VSampleOperasionalList vSampleOperasionalList
    {
        get => HttpData.Get<VSampleOperasionalList>("vSampleOperasionalList")!;
        set => HttpData["vSampleOperasionalList"] = value;
    }

    /// <summary>
    /// Page class for VSampleOperasional
    /// </summary>
    public class VSampleOperasionalList : VSampleOperasionalListBase
    {
        // Constructor
        public VSampleOperasionalList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public VSampleOperasionalList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class VSampleOperasionalListBase : VSampleOperasional
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "vSampleOperasionalList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fVSampleOperasionallist";
        ...
        // Constructor
        public VSampleOperasionalListBase()
        {
            TableName = "VSampleOperasional";

            // CSS class name as context
            TableVar = "VSampleOperasional";
            ...
            // Table object (vSampleOperasional)
            if (vSampleOperasional == null || vSampleOperasional is VSampleOperasional)
                vSampleOperasional = this;

            // Initialize URLs
            AddUrl = "VSampleOperasionalAdd";
            ...
            MultiDeleteUrl = "VSampleOperasionalDelete";
            MultiUpdateUrl = "VSampleOperasionalUpdate";
        ...
        // Page name
        public string PageName => "VSampleOperasionalList";
        ...
        public void SetVisibility()
        {
            ...
            StatusPrevProses.Visible = false;
        }

        // Constructor
        public VSampleOperasionalListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "VSampleOperasionalView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fVSampleOperasionalgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                vSampleOperasionalList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(RedirectLink.AdvancedSearch.ToJson())); // Field RedirectLink
            filters.Merge(JObject.Parse(Proses2.AdvancedSearch.ToJson())); // Field Proses
            filters.Merge(JObject.Parse(Plant.AdvancedSearch.ToJson())); // Field Plant
            filters.Merge(JObject.Parse(Moda.AdvancedSearch.ToJson())); // Field Moda
            filters.Merge(JObject.Parse(Tanggal.AdvancedSearch.ToJson())); // Field Tanggal
            filters.Merge(JObject.Parse(NamaDokumen.AdvancedSearch.ToJson())); // Field NamaDokumen
            filters.Merge(JObject.Parse(DownloadDokumen.AdvancedSearch.ToJson())); // Field DownloadDokumen
            filters.Merge(JObject.Parse(IdAktivitasDokumen.AdvancedSearch.ToJson())); // Field IdAktivitasDokumen
            filters.Merge(JObject.Parse(StatusUpload.AdvancedSearch.ToJson())); // Field StatusUpload
        ...
        protected bool RestoreFilterList() {
            ...
            // Field RedirectLink
            if (filter?.TryGetValue("x_RedirectLink", out sv) ?? false) {
                RedirectLink.AdvancedSearch.SearchValue = sv;
                RedirectLink.AdvancedSearch.SearchOperator = filter["z_RedirectLink"];
                RedirectLink.AdvancedSearch.SearchCondition = filter["v_RedirectLink"];
                RedirectLink.AdvancedSearch.SearchValue2 = filter["y_RedirectLink"];
                RedirectLink.AdvancedSearch.SearchOperator2 = filter["w_RedirectLink"];
                RedirectLink.AdvancedSearch.Save();
            }

            // Field Proses
            if (filter?.TryGetValue("x_Proses2", out sv) ?? false) {
                Proses2.AdvancedSearch.SearchValue = sv;
                Proses2.AdvancedSearch.SearchOperator = filter["z_Proses2"];
                Proses2.AdvancedSearch.SearchCondition = filter["v_Proses2"];
                Proses2.AdvancedSearch.SearchValue2 = filter["y_Proses2"];
                Proses2.AdvancedSearch.SearchOperator2 = filter["w_Proses2"];
                Proses2.AdvancedSearch.Save();
            }

            // Field Plant
            if (filter?.TryGetValue("x_Plant", out sv) ?? false) {
                Plant.AdvancedSearch.SearchValue = sv;
                Plant.AdvancedSearch.SearchOperator = filter["z_Plant"];
                Plant.AdvancedSearch.SearchCondition = filter["v_Plant"];
                Plant.AdvancedSearch.SearchValue2 = filter["y_Plant"];
                Plant.AdvancedSearch.SearchOperator2 = filter["w_Plant"];
                Plant.AdvancedSearch.Save();
            }

            // Field Moda
            if (filter?.TryGetValue("x_Moda", out sv) ?? false) {
                Moda.AdvancedSearch.SearchValue = sv;
                Moda.AdvancedSearch.SearchOperator = filter["z_Moda"];
                Moda.AdvancedSearch.SearchCondition = filter["v_Moda"];
                Moda.AdvancedSearch.SearchValue2 = filter["y_Moda"];
                Moda.AdvancedSearch.SearchOperator2 = filter["w_Moda"];
                Moda.AdvancedSearch.Save();
            }
            ...
            // Field NamaDokumen
            if (filter?.TryGetValue("x_NamaDokumen", out sv) ?? false) {
                NamaDokumen.AdvancedSearch.SearchValue = sv;
                NamaDokumen.AdvancedSearch.SearchOperator = filter["z_NamaDokumen"];
                NamaDokumen.AdvancedSearch.SearchCondition = filter["v_NamaDokumen"];
                NamaDokumen.AdvancedSearch.SearchValue2 = filter["y_NamaDokumen"];
                NamaDokumen.AdvancedSearch.SearchOperator2 = filter["w_NamaDokumen"];
                NamaDokumen.AdvancedSearch.Save();
            }
            ...
            // Field IdAktivitasDokumen
            if (filter?.TryGetValue("x_IdAktivitasDokumen", out sv) ?? false) {
                IdAktivitasDokumen.AdvancedSearch.SearchValue = sv;
                IdAktivitasDokumen.AdvancedSearch.SearchOperator = filter["z_IdAktivitasDokumen"];
                IdAktivitasDokumen.AdvancedSearch.SearchCondition = filter["v_IdAktivitasDokumen"];
                IdAktivitasDokumen.AdvancedSearch.SearchValue2 = filter["y_IdAktivitasDokumen"];
                IdAktivitasDokumen.AdvancedSearch.SearchOperator2 = filter["w_IdAktivitasDokumen"];
                IdAktivitasDokumen.AdvancedSearch.Save();
            }

            // Field StatusUpload
            if (filter?.TryGetValue("x_StatusUpload", out sv) ?? false) {
                StatusUpload.AdvancedSearch.SearchValue = sv;
                StatusUpload.AdvancedSearch.SearchOperator = filter["z_StatusUpload"];
                StatusUpload.AdvancedSearch.SearchCondition = filter["v_StatusUpload"];
                StatusUpload.AdvancedSearch.SearchValue2 = filter["y_StatusUpload"];
                StatusUpload.AdvancedSearch.SearchOperator2 = filter["w_StatusUpload"];
                StatusUpload.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, NoReferensi, def, false); // NoReferensi
            ...
            BuildSearchSql(ref where, Moda, def, true); // Moda
            ...
            BuildSearchSql(ref where, IdAktivitasDokumen, def, false); // IdAktivitasDokumen
            BuildSearchSql(ref where, StatusUpload, def, false); // StatusUpload
            ...
            if (!def && Command == "search") {
                ...
                RedirectLink.AdvancedSearch.Save(); // RedirectLink
                Proses2.AdvancedSearch.Save(); // Proses
                Plant.AdvancedSearch.Save(); // Plant
                Moda.AdvancedSearch.Save(); // Moda
                Tanggal.AdvancedSearch.Save(); // Tanggal
                NamaDokumen.AdvancedSearch.Save(); // NamaDokumen
                DownloadDokumen.AdvancedSearch.Save(); // DownloadDokumen
                IdAktivitasDokumen.AdvancedSearch.Save(); // IdAktivitasDokumen
                StatusUpload.AdvancedSearch.Save(); // StatusUpload
        ...
        public void ShowFilterList()
        {
            ...
            // Field Moda
            filter = QueryBuilderWhere("Moda");
            if (Empty(filter))
                BuildSearchSql(ref filter, Moda, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Moda.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field NamaDokumen
            filter = QueryBuilderWhere("NamaDokumen");
            if (Empty(filter))
                BuildSearchSql(ref filter, NamaDokumen, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + NamaDokumen.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected bool CheckSearchParms() {
            ...
            if (Plant.AdvancedSearch.IssetSession)
                return true;
            if (Moda.AdvancedSearch.IssetSession)
                return true;
            ...
            if (DownloadDokumen.AdvancedSearch.IssetSession)
                return true;
            if (IdAktivitasDokumen.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = Tanggal.Expression + " DESC"; // Set up default sort
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fVSampleOperasionallist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fVSampleOperasionallist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Moda")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fVSampleOperasionalsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fVSampleOperasionalsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fVSampleOperasionallist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_VSampleOperasional");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(vSampleOperasionalList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(vSampleOperasionalList.RowCount) + "_VSampleOperasional");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(vSampleOperasionalList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && vSampleOperasionalList.RowType == RowType.Add || IsEdit && vSampleOperasionalList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_NoReferensi"))
                    NoReferensi.AdvancedSearch.SearchValue = Get("x_NoReferensi");
                else
            ...
            if (Query.ContainsKey("z_NoReferensi"))
                NoReferensi.AdvancedSearch.SearchOperator = Get("z_NoReferensi");
            ...
            // Moda
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Moda[]"))
                    Moda.AdvancedSearch.SearchValue = Get("x_Moda[]", Config.FilterOptionSeparator);
                else
                    Moda.AdvancedSearch.SearchValue = Get("Moda"); // Default Value // DN
            if (!Empty(Moda.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Moda"))
                Moda.AdvancedSearch.SearchOperator = Get("z_Moda", Config.FilterOptionSeparator);
            ...
            // IdAktivitasDokumen
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IdAktivitasDokumen"))
                    IdAktivitasDokumen.AdvancedSearch.SearchValue = Get("x_IdAktivitasDokumen", Config.FilterOptionSeparator);
                else
                    IdAktivitasDokumen.AdvancedSearch.SearchValue = Get("IdAktivitasDokumen"); // Default Value // DN
            if (!Empty(IdAktivitasDokumen.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_IdAktivitasDokumen"))
                IdAktivitasDokumen.AdvancedSearch.SearchOperator = Get("z_IdAktivitasDokumen", Config.FilterOptionSeparator);

            // StatusUpload
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_StatusUpload"))
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            StatusPrevProses.SetDbValue(row["StatusPrevProses"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Moda", Moda.DefaultValue ?? DbNullValue); // DN
            ...
            row.Add("StatusPrevProses", StatusPrevProses.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            // NoReferensi
            NoReferensi.CellCssStyle = "white-space: nowrap;";
            ...
            // IdAktivitasDokumen
            IdAktivitasDokumen.CellCssStyle = "white-space: nowrap;";

            // StatusUpload
            StatusUpload.CellCssStyle = "white-space: nowrap;";

            // StatusPrevProses
            StatusPrevProses.CellCssStyle = "white-space: nowrap;";
            ...
            if (RowType == RowType.View) {
                ...
                // Moda
                Moda.ViewValue = ConvertToString(Moda.CurrentValue); // DN
                Moda.ViewCustomAttributes = "";
                ...
                // Moda
                if (Moda.UseFilter && !Empty(Moda.AdvancedSearch.SearchValue)) {
                    Moda.EditValue = ConvertToString(Moda.AdvancedSearch.SearchValue).Split(Config.FilterOptionSeparator).ToList();
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fVSampleOperasionalsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
