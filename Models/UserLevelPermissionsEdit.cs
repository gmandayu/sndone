...
public partial class SnDOne {
    /// <summary>
    /// userLevelPermissionsEdit
    /// </summary>
    public static UserLevelPermissionsEdit userLevelPermissionsEdit
    {
        get => HttpData.Get<UserLevelPermissionsEdit>("userLevelPermissionsEdit")!;
        set => HttpData["userLevelPermissionsEdit"] = value;
    }

    /// <summary>
    /// Page class for UserLevelPermissions
    /// </summary>
    public class UserLevelPermissionsEdit : UserLevelPermissionsEditBase
    {
        // Constructor
        public UserLevelPermissionsEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public UserLevelPermissionsEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class UserLevelPermissionsEditBase : UserLevelPermissions
    {
        // Page ID
        ...
        // Constructor
        public UserLevelPermissionsEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "UserLevelPermissionsEdit";
        ...
        // Constructor
        public UserLevelPermissionsEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<ActionSetupResult> SetupCurrentAction()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("UserLevelID", out rv)) {
                    UserLevelID.FormValue = UrlDecode(rv);
                ...
                if (RouteValues.TryGetValue("_TableName", out rv)) {
                    _TableName.FormValue = UrlDecode(rv);
                    _TableName.OldValue = _TableName.FormValue;
                } else if (CurrentForm.HasValue("x__TableName")) {
                    _TableName.FormValue = CurrentForm.GetValue("x__TableName");
                    _TableName.OldValue = _TableName.FormValue;
                } else if (!Empty(keyValues)) {
                    _TableName.OldValue = ConvertToString(keyValues[1]);
                } else {
                    loaded = false;
                }
                if (loaded)
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("UserLevelID", out rv)) {
                        UserLevelID.QueryValue = UrlDecode(rv);
                        loadByQuery = true;
                    ...
                    if (RouteValues.TryGetValue("_TableName", out rv)) {
                        _TableName.QueryValue = UrlDecode(rv);
                        loadByQuery = true;
                    } else if (Get("_TableName", out sv)) {
                        _TableName.QueryValue = sv.ToString();
                        loadByQuery = true;
                    } else {
                        _TableName.CurrentValue = DbNullValue;
                    }
                }
                if (IsShow) {
                    loaded = await LoadRow();
        ...
        private async Task HandleApiFormKeys()
        {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                _TableName.FormValue = ConvertToString(keyValues[1]);
            }
        }
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    ...
                    if (GetPageName(returnUrl) != "UserLevelPermissionsList") {
                        ...
                        returnUrl = "UserLevelPermissionsList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        userLevelPermissionsEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'UserLevelID'
            UserLevelID.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(UserLevelID, "UserLevelID", "x_UserLevelID", true, validate, false);
            if (CurrentForm.HasValue("o_UserLevelID"))
                UserLevelID.OldValue = CurrentForm.GetValue("o_UserLevelID");

            // Standard handling for '_TableName'
            _TableName.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(_TableName, "TableName", "x__TableName");
            if (CurrentForm.HasValue("o__TableName"))
                _TableName.OldValue = CurrentForm.GetValue("o__TableName");

            // Standard handling for 'Permission'
            Permission.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(Permission, "Permission", "x_Permission", true, validate, false);
        }
        #pragma warning restore 1998
        ...
        public void RestoreFormValues()
        {
            ...
            _TableName.CurrentValue = _TableName.FormValue;
            Permission.CurrentValue = Permission.FormValue;
        }
        ...
        public async Task RenderRow()
        {
            ...
            // TableName
            _TableName.RowCssClass = "row";

            // Permission
            Permission.RowCssClass = "row";
        ...
        private void ValidateCustomUserLevelID() {
            if (UserLevelID.Visible && UserLevelID.Required) {
                if (!UserLevelID.IsDetailKey && Empty(UserLevelID.FormValue)) {
                    UserLevelID.AddErrorMessage(ConvertToString(Permission.RequiredErrorMessage).Replace("%s", Permission.Caption));
                }
            }
        }

        private void ValidateCustom_TableName() {
            if (_TableName.Visible && _TableName.Required) {
                if (!_TableName.IsDetailKey && Empty(_TableName.FormValue)) {
                    _TableName.AddErrorMessage(ConvertToString(Permission.RequiredErrorMessage).Replace("%s", Permission.Caption));
                }
            }
        }

        private void ValidateCustomPermission() {
            if (Permission.Visible && Permission.Required) {
                if (!Permission.IsDetailKey && Empty(Permission.FormValue)) {
                    Permission.AddErrorMessage(ConvertToString(Permission.RequiredErrorMessage).Replace("%s", Permission.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ValidateCustomUserLevelID();
                if (!CheckInteger(UserLevelID.FormValue)) {
                    UserLevelID.AddErrorMessage(UserLevelID.GetErrorMessage(false));
                }
                ValidateCustom_TableName();
                ValidateCustomPermission();
                if (!CheckInteger(Permission.FormValue)) {
                    Permission.AddErrorMessage(Permission.GetErrorMessage(false));
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // TableName
            _TableName.SetDbValue(rsnew, _TableName.CurrentValue, _TableName.ReadOnly);

            // Permission
            Permission.SetDbValue(rsnew, Permission.CurrentValue, Permission.ReadOnly);
            return rsnew;
        }
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("UserLevelID", out value)) // UserLevelID
                UserLevelID.CurrentValue = value;
            if (row.TryGetValue("TableName", out value)) // TableName
                _TableName.CurrentValue = value;
            if (row.TryGetValue("Permission", out value)) // Permission
                Permission.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("UserLevelPermissionsList")), "", TableVar, true);
            string pageId = "edit";
