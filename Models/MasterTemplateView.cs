...
public partial class SnDOne {
    ...
    /// masterTemplateView
    /// </summary>
    public static MasterTemplateView masterTemplateView
    {
        get => HttpData.Get<MasterTemplateView>("masterTemplateView")!;
        set => HttpData["masterTemplateView"] = value;
    }

    /// <summary>
    /// Page class for MasterTemplate
    /// </summary>
    public class MasterTemplateView : MasterTemplateViewBase
    {
        // Constructor
        public MasterTemplateView(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MasterTemplateView() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MasterTemplateViewBase : MasterTemplate
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "masterTemplateView";
        ...
        public MasterTemplateViewBase()
        {
            ...
            if (IsApi()) {
                ...
                if (keys.Length > 0)
                    RecordKeys["IdTemplate"] = keys[0];
            } else {
                RecordKeys["IdTemplate"] = RouteValues.TryGetValue("IdTemplate", out obj) && obj != null ? UrlDecode(obj) : Get("IdTemplate"); // DN
            }
        ...
        // Page name
        public string PageName => "MasterTemplateView";
        ...
        // Constructor
        public MasterTemplateViewBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (RouteValues.TryGetValue("IdTemplate", out v) && !Empty(v)) { // DN
                IdTemplate.QueryValue = UrlDecode(v); // DN
                RecordKeys["IdTemplate"] = IdTemplate.QueryValue;
            } else if (Get("IdTemplate", out sv)) {
                IdTemplate.QueryValue = sv.ToString();
                RecordKeys["IdTemplate"] = IdTemplate.QueryValue;
            } else if (IsApi() && !Empty(keyValues)) {
                IdTemplate.QueryValue = ConvertToString(keyValues[0]);
                RecordKeys["IdTemplate"] = IdTemplate.QueryValue;
            } else if (!loadCurrentRecord) {
                returnUrl = "MasterTemplateList"; // Return to list
            }
                ...
                case "show": // Get a record to display
                        ...
                        if (!res) { // Load record based on key
                            ...
                            if (IsApi()) {
                                ...
                                return Terminate("MasterTemplateList"); // Return to list page
                            }
                        }
                    break;
            }
        ...
        public void SetupOtherOptions()
        {
            ...
            item = option.Add("detail_TemplateProses");
            body = Language.Phrase("ViewPageDetailLink") + Language.TablePhrase("TemplateProses", "TblCaption");
            body = "<a class=\"btn btn-default btn-sm ew-row-link ew-detail\" data-action=\"list\" href=\"" + HtmlEncode(AppPath("TemplateProsesList?" + Config.TableShowMaster + "=MasterTemplate&" + ForeignKeyUrl("fk_IdTemplate", IdTemplate.CurrentValue) + "")) + "\">" + body + "</a>";
            links = "";
            ...
            if (detailPage?.DetailView && Security.CanView && Security.AllowView(CurrentProjectID + "MasterTemplate") ?? false) {
                links += "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" + Language.Phrase("MasterDetailViewLink", true) + "\" href=\"" + HtmlEncode(AppPath(GetViewUrl(Config.TableShowDetail + "=TemplateProses"))) + "\">" + Language.Phrase("MasterDetailViewLink", null) + "</a></li>";
            ...
            if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "MasterTemplate") ?? false) {
                links += "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" + Language.Phrase("MasterDetailEditLink", true) + "\" href=\"" + HtmlEncode(AppPath(GetEditUrl(Config.TableShowDetail + "=TemplateProses"))) + "\">" + Language.Phrase("MasterDetailEditLink", null) + "</a></li>";
            ...
            if (detailPage?.DetailAdd && Security.CanAdd && Security.AllowAdd(CurrentProjectID + "MasterTemplate") ?? false) {
                links += "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" + Language.Phrase("MasterDetailCopyLink", true) + "\" href=\"" + HtmlEncode(AppPath(GetCopyUrl(Config.TableShowDetail + "=TemplateProses"))) + "\">" + Language.Phrase("MasterDetailCopyLink", null) + "</a></li>";
        ...
        public async Task ExportData(dynamic? doc, string[] keys)
        {
            ...
            // Export detail records (TemplateProses)
            if (Config.ExportDetailRecords && CurrentDetailTables.Contains("TemplateProses")) {
                templateProses = new TemplateProsesList();
                if (templateProses != null) {
                    var c = await GetConnection2Async(templateProses.DbId); // Note: Use new connection for detail records // DN
                    using var rsdetail = await c.ExecuteReaderAsync(templateProses.GetSql(templateProses.DetailFilterFromSession, templateProses.SessionOrderBy)); // Load detail records
                    if (rsdetail?.HasRows ?? false) { // DN
                        exportStyle = doc.Style;
                        doc.SetStyle("h"); // Change to horizontal
                        if (Export != "csv" || Config.ExportDetailRecordsForCsv) {
                            doc.ExportEmptyRow();
                            int detailcnt = await templateProses.LoadRecordCountAsync(templateProses.DetailFilterFromSession); // DN
                            var oldtbl = doc.Table;
                            doc.Table = templateProses;
                            await templateProses.ExportDocument(doc, rsdetail, 1, detailcnt);
                            doc.Table = oldtbl;
                        }
        ...
        protected void SetupDetailParms() {
            ...
            if (!Empty(detailTblVar)) {
                ...
                if (detailTblVars.Contains("TemplateProses")) {
                    ...
                    if (templateProsesGrid?.DetailView ?? false) {
                        templateProsesGrid.CurrentMode = "view";
