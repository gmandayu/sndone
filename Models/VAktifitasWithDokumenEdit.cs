...
public partial class SnDOne {
    /// <summary>
    /// vAktifitasWithDokumenEdit
    /// </summary>
    public static VAktifitasWithDokumenEdit vAktifitasWithDokumenEdit
    {
        get => HttpData.Get<VAktifitasWithDokumenEdit>("vAktifitasWithDokumenEdit")!;
        set => HttpData["vAktifitasWithDokumenEdit"] = value;
    }

    /// <summary>
    /// Page class for v_aktifitas_with_dokumen
    /// </summary>
    public class VAktifitasWithDokumenEdit : VAktifitasWithDokumenEditBase
    {
        // Constructor
        public VAktifitasWithDokumenEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public VAktifitasWithDokumenEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class VAktifitasWithDokumenEditBase : VAktifitasWithDokumen
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "vAktifitasWithDokumenEdit";
        ...
        // Constructor
        public VAktifitasWithDokumenEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "VAktifitasWithDokumenEdit";
        ...
        // Constructor
        public VAktifitasWithDokumenEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("VAktifitasWithDokumenList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "VAktifitasWithDokumenList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "VAktifitasWithDokumenList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        vAktifitasWithDokumenEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        {
            // Get upload data
            PathFile.Upload.Index = CurrentForm.Index;
            if (!await PathFile.Upload.UploadFile()) // DN
                End(PathFile.Upload.Message);
            PathFile.CurrentValue = PathFile.Upload.FileName;
        }
        #pragma warning restore 1998
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (IsShow && !EventCancelled)
                    await RenderUploadField(PathFile);
        ...
        private void ValidateCustomIdDokumen() {
            if (IdDokumen.Visible && IdDokumen.Required) {
                if (!IdDokumen.IsDetailKey && Empty(IdDokumen.FormValue)) {
                    IdDokumen.AddErrorMessage(ConvertToString(IdProses.RequiredErrorMessage).Replace("%s", IdProses.Caption));
                }
            }
        }
        ...
        private void ValidateCustomNamaDokumen() {
            if (NamaDokumen.Visible && NamaDokumen.Required) {
                if (!NamaDokumen.IsDetailKey && Empty(NamaDokumen.FormValue)) {
                    NamaDokumen.AddErrorMessage(ConvertToString(IdProses.RequiredErrorMessage).Replace("%s", IdProses.Caption));
                }
            }
        }
        ...
        private void ValidateCustomPathFile() {
            if (PathFile.Visible && PathFile.Required) {
                if (PathFile.Upload.FileName == "" && !PathFile.Upload.KeepFile) {
                    PathFile.AddErrorMessage(ConvertToString(IdProses.RequiredErrorMessage).Replace("%s", IdProses.Caption));
                }
            }
        }
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            if (PathFile.Visible && !PathFile.Upload.KeepFile) {
                PathFile.UploadPath = PathFile.GetUploadPath();
                if (!Empty(PathFile.Upload.FileName)) {
                    FixUploadFileNames(PathFile);
                    PathFile.SetDbValue(rsnew, PathFile.Upload.FileName, PathFile.ReadOnly);
                }
            }
            ...
            if (updateRow) {
                try {
                    ...
                    if (result) {
                        if (PathFile.Visible && !PathFile.Upload.KeepFile) {
                            if (!await SaveUploadFiles(PathFile, ConvertToString(rsnew["PathFile"]), false))
                            {
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // PathFile
            if (PathFile.Visible && !PathFile.ReadOnly && !PathFile.Upload.KeepFile) {
                if (Empty(PathFile.Upload.FileName)) {
                    rsnew["PathFile"] = DbNullValue;
                } else {
                    FixUploadTempFileNames(PathFile);
                    rsnew["PathFile"] = PathFile.Upload.FileName;
                }
            }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("VAktifitasWithDokumenList")), "", TableVar, true);
            string pageId = "edit";
