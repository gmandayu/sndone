...
public partial class SnDOne {
    /// <summary>
    /// subAktivitasDataStopPembongkaranPipaEdit
    /// </summary>
    public static SubAktivitasDataStopPembongkaranPipaEdit subAktivitasDataStopPembongkaranPipaEdit
    {
        get => HttpData.Get<SubAktivitasDataStopPembongkaranPipaEdit>("subAktivitasDataStopPembongkaranPipaEdit")!;
        set => HttpData["subAktivitasDataStopPembongkaranPipaEdit"] = value;
    }

    /// <summary>
    /// Page class for SubAktivitasDataStopPembongkaranPipa
    /// </summary>
    public class SubAktivitasDataStopPembongkaranPipaEdit : SubAktivitasDataStopPembongkaranPipaEditBase
    {
        // Constructor
        public SubAktivitasDataStopPembongkaranPipaEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SubAktivitasDataStopPembongkaranPipaEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SubAktivitasDataStopPembongkaranPipaEditBase : SubAktivitasDataStopPembongkaranPipa
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "subAktivitasDataStopPembongkaranPipaEdit";
        ...
        // Constructor
        public SubAktivitasDataStopPembongkaranPipaEditBase(Controller? controller)
        {
        ...
        // Page name
        public string PageName => "SubAktivitasDataStopPembongkaranPipaEdit";
        ...
        // Constructor
        public SubAktivitasDataStopPembongkaranPipaEditBase() : this(null) { }
    ...
    return Post("keys[]", out StringValues k) ? k : StringValues.Empty;
        ...
        private async Task<IActionResult?> HandleShowAction(bool loaded)
        {
            ...
            if (!loaded) {
                if (Empty(FailureMessage))
                    FailureMessage = Language.Phrase("NoRecord");
                return Terminate("SubAktivitasDataStopPembongkaranPipaList");
            }
            return null;
        }
        ...
        private async Task<IActionResult?> HandleUpdateAction()
        {
            ...
            if (res) {
                ...
                if (IsModal && UseAjaxActions) {
                    IsModal = false;
                    if (GetPageName(returnUrl) != "SubAktivitasDataStopPembongkaranPipaList") {
                        TempData["Return-Url"] = returnUrl;
                        returnUrl = "SubAktivitasDataStopPembongkaranPipaList";
                    }
                }
                if (IsJsonResponse()) {
                    ClearMessages();
        ...
        private void PageRunEnd()
        {
                    ...
                    if (!IsApi() && !IsTerminated) {
                        ...
                        // Page Render event
                        subAktivitasDataStopPembongkaranPipaEdit?.PageRender();
                    }
        }
...
private void ResolveLookupView(dynamic fld, string keyFieldName, string fallbackType = "auto")
{
    ...
    if (rswrk?.Count > 0 && fld.Lookup != null)
    {
        ...
        protected async Task LoadFormValues() {
            ...
            // Standard handling for 'tglStopBongkar'
            tglStopBongkar.MultiUpdate = string.Empty; // initialize (will be set later for update control if needed)
            SetNormalField(tglStopBongkar, "tglStopBongkar", "x_tglStopBongkar", true, validate, true);
            if (!id.IsDetailKey) {
        ...
        public void RestoreFormValues()
        {
            ...
            tglStopBongkar.CurrentValue = tglStopBongkar.FormValue;
            tglStopBongkar.CurrentValue = UnformatDateTime(tglStopBongkar.CurrentValue, tglStopBongkar.FormatPattern);
        }
        ...
        public async Task RenderRow()
        {
            ...
            // tglStopBongkar
            tglStopBongkar.RowCssClass = "row";
        ...
        private void ValidateCustomNoReferensi() {
            if (NoReferensi.Visible && NoReferensi.Required) {
                if (!NoReferensi.IsDetailKey && Empty(NoReferensi.FormValue)) {
                    NoReferensi.AddErrorMessage(ConvertToString(tglStopBongkar.RequiredErrorMessage).Replace("%s", tglStopBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidProses() {
            if (idProses.Visible && idProses.Required) {
                if (!idProses.IsDetailKey && Empty(idProses.FormValue)) {
                    idProses.AddErrorMessage(ConvertToString(tglStopBongkar.RequiredErrorMessage).Replace("%s", tglStopBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomidAktifitas() {
            if (idAktifitas.Visible && idAktifitas.Required) {
                if (!idAktifitas.IsDetailKey && Empty(idAktifitas.FormValue)) {
                    idAktifitas.AddErrorMessage(ConvertToString(tglStopBongkar.RequiredErrorMessage).Replace("%s", tglStopBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomStatusAktivitas() {
            if (StatusAktivitas.Visible && StatusAktivitas.Required) {
                if (!StatusAktivitas.IsDetailKey && Empty(StatusAktivitas.FormValue)) {
                    StatusAktivitas.AddErrorMessage(ConvertToString(tglStopBongkar.RequiredErrorMessage).Replace("%s", tglStopBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomLastUpdatedBy() {
            if (LastUpdatedBy.Visible && LastUpdatedBy.Required) {
                if (!LastUpdatedBy.IsDetailKey && Empty(LastUpdatedBy.FormValue)) {
                    LastUpdatedBy.AddErrorMessage(ConvertToString(tglStopBongkar.RequiredErrorMessage).Replace("%s", tglStopBongkar.Caption));
                }
            }
        }
        ...
        private void ValidateCustomlastUpdatedDate() {
            if (lastUpdatedDate.Visible && lastUpdatedDate.Required) {
                if (!lastUpdatedDate.IsDetailKey && Empty(lastUpdatedDate.FormValue)) {
                    lastUpdatedDate.AddErrorMessage(ConvertToString(tglStopBongkar.RequiredErrorMessage).Replace("%s", tglStopBongkar.Caption));
                }
            }
        }

        private void ValidateCustomtglStopBongkar() {
            if (tglStopBongkar.Visible && tglStopBongkar.Required) {
                if (!tglStopBongkar.IsDetailKey && Empty(tglStopBongkar.FormValue)) {
                    tglStopBongkar.AddErrorMessage(ConvertToString(tglStopBongkar.RequiredErrorMessage).Replace("%s", tglStopBongkar.Caption));
                }
            }
        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                ValidateCustomtglStopBongkar();
                if (!CheckDate(tglStopBongkar.FormValue, tglStopBongkar.FormatPattern)) {
                    tglStopBongkar.AddErrorMessage(tglStopBongkar.GetErrorMessage(false));
                }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // tglStopBongkar
            tglStopBongkar.SetDbValue(rsnew, ConvertToDateTime(tglStopBongkar.CurrentValue, tglStopBongkar.FormatPattern), tglStopBongkar.ReadOnly);
            return rsnew;
        }
        ...
        {
            object? value;
            if (row.TryGetValue("tglStopBongkar", out value)) // tglStopBongkar
                tglStopBongkar.CurrentValue = value;
        }
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("SubAktivitasDataStopPembongkaranPipaList")), "", TableVar, true);
            string pageId = "edit";
